...
Maju-native tasks declare one of two evaluation layers in `task.toml`.
...
`--path benchmarks/maju-dataset --layer regression` or `--layer workflow`;
task identities stay unchanged.

When the Maju dataset root is passed without `--layer` or `--attempts`, the
...
just benchmark --path benchmarks/maju-dataset --layer regression   # Maju k=1
just benchmark --path benchmarks/maju-dataset --layer workflow     # Maju k=3
