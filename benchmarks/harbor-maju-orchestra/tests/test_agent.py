...
from harbor_maju_orchestra import (
    AgentCredential,
    MajuOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = MajuOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = MajuOrchestraAgent(
        ...
        maju_acp_binary="/pinned/maju-acp",
        maju_agent_binary="/pinned/maju-agent",
        maju_dev_mcp_binary="/pinned/maju-dev-mcp",
        maju_cli_binary="/pinned/maju",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.maju_acp_binary == "/pinned/maju-acp"
    assert agent.runtime.maju_agent_binary == "/pinned/maju-agent"
    assert agent.runtime.maju_dev_mcp_binary == "/pinned/maju-dev-mcp"
    assert agent.runtime.maju_cli_binary == "/pinned/maju"
