...
from harbor_maju_orchestra.provisioning import (
    AgentCredential,
    FixtureActor,
    TrialHandle,
)
from harbor_maju_orchestra.task_fixtures import fixture_for
...
def test_prompt_hash_and_identity_override_are_fail_closed(tmp_path):
    ...
    endpoint = EndpointLaunchConfig(
        "anthropic", "ANTHROPIC_API_KEY", {"MAJU_ACP_MCP_COMMAND": "evil"}
    )
...
async def test_collects_task_declared_channel_membership(tmp_path, monkeypatch):
    ...
    async def maju_json(credential_arg, trial_arg, *args):
    ...
    monkeypatch.setattr(rt, "_maju_json", maju_json)
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("maju-acp", "maju-agent", "maju-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        maju_acp_binary=binaries["maju-acp"],
        maju_agent_binary=binaries["maju-agent"],
        maju_dev_mcp_binary=binaries["maju-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/maju-acp",
        f"{REMOTE_BIN}/maju-agent",
        f"{REMOTE_BIN}/maju-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, maju_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_maju_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"maju-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/maju-acp" in command
    # The real product wiring: acp spawns maju-agent, which gets maju-dev-mcp.
    assert env["MAJU_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/maju-agent"
    assert env["MAJU_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/maju-dev-mcp"
    assert env["MAJU_RELAY_URL"] == trial.relay_ws_url
    assert env["MAJU_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["NOSTR_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["MAJU_AGENT_NO_HINTS"] == "1"
    assert env["MAJU_AGENT_MAX_ROUNDS"] == expected
    assert env["MAJU_ACP_SYSTEM_PROMPT_FILE"].endswith("orch-1.system-prompt.md")
    # The composed prompt was uploaded into the container.
    assert any(
        target == env["MAJU_ACP_SYSTEM_PROMPT_FILE"]
...
def test_memory_task_disables_auto_memory_injection(tmp_path):
    ...
    assert env["MAJU_ACP_CHANNELS"] == "channel"
    assert env["MAJU_ACP_NO_MEMORY"] == "true"
...
async def test_memory_seed_uses_agent_credentials_and_stdin(tmp_path, monkeypatch):
    ...
    for invocation, seed in zip(captured, seeds, strict=True):
        ...
        assert invocation["env"]["MAJU_PRIVATE_KEY"] == orch.nostr_secret_key
        assert invocation["value"] == seed.value.encode()


def test_runtime_validates_construction_bounds(tmp_path):
    # 0 is legal and means unbounded (MAJU_AGENT_MAX_ROUNDS=0); the trial
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_maju_orchestra.container_runtime import _Agent
...
async def test_send_mentions_by_pubkey_so_task_text_stays_inert(tmp_path, monkeypatch):
    ...
    async def maju_json(credential, trial, *args):
        calls.append(args)
        return {}
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def maju_json(credential, *args, **kwargs):
...
async def test_solo_turn_end_completes_without_done_message(tmp_path, monkeypatch):
    ...
    async def maju_json(*args, **kwargs):
        return []
...
def test_turn_status_parses_completed_batch_and_successful_steer_receipts():
    ...
    output = "\n".join(
        [
            ...
            f"turn delivered Maju events for channel test: {batch_event_id}",
    ...
    assert MajuContainerRuntime._parse_turn_status(output) == (
        1,
        1,
...
async def test_collect_evidence_uploads_verifier_artifact(tmp_path, monkeypatch):
    ...
    evidence = json.loads((trial_dir / "maju-evidence.json").read_text())
...
async def test_failed_evidence_snapshot_records_the_reason(tmp_path, monkeypatch):
    ...
    assert "relay unreachable" in (trial_dir / "maju-evidence-error.txt").read_text()
    assert not (trial_dir / "maju-evidence.json").exists()


def test_runtime_logging_keeps_readiness_and_turn_completion_signals(tmp_path):
    rt = runtime(tmp_path)
    assert rt._rust_log(None) == "maju_acp=info,pool::prompt=info"
    assert rt._rust_log("custom=debug") == (
        "custom=debug,maju_acp=info,pool::prompt=info"
    )
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await MajuContainerRuntime._stop_agents(environment, agents)
...
async def test_thinking_effort_reaches_the_agent(tmp_path, pinned, expected):
    ...
    assert env["MAJU_AGENT_THINKING_EFFORT"] == expected
