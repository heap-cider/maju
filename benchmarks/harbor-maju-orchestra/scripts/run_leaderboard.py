...
importable:
    ...
    uv run --project benchmarks/harbor-maju-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux maju-acp/maju-agent/maju-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_maju_orchestra:MajuOrchestraAgent"
PROVISIONER_FACTORY = "harbor_maju_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Maju relay/Postgres provisioner",
    )
    parser.add_argument(
        "--maju-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host maju CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of maju-acp/maju-agent/maju-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"maju binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --maju-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "maju_acp_binary": agent_binaries["maju-acp"],
        "maju_agent_binary": agent_binaries["maju-agent"],
        "maju_dev_mcp_binary": agent_binaries["maju-dev-mcp"],
        "maju_cli_binary": binaries["maju"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/heap-cider/maju",
        "agent_display_name": f"Maju Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.maju_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.maju_bin_dir)
