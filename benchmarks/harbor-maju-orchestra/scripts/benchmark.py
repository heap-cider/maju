#!/usr/bin/env python3
"""One-command benchmark: bring up the Maju stack in Docker and run it.
...
Maju-native tasks use their ``evaluation_layer`` metadata: regression runs
default to 1 attempt and workflow runs default to 3. The script owns
everything around the run:

- A dedicated ``maju-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Maju
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-maju-orchestra/testbed \
        benchmarks/harbor-maju-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "maju-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.maju.app.benchmark"

DEFAULT_DATASET = "terminal-bench/terminal-bench-2-1"
DEFAULT_ATTEMPTS = 5
MAJU_DATASET_ROOT = REPO_ROOT / "benchmarks" / "maju-dataset"
...
AGENT_BINARIES = ("maju-acp", "maju-agent", "maju-dev-mcp")
...
class MajuTask:
    """The identity and evaluation layer declared by one Maju task."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Maju evaluation layer to run (selected from task metadata)",
    )
    parser.add_argument(
        "--attempts",
        "-k",
        type=int,
        default=None,
        help="Runs per problem (default: Terminal-Bench 5, Maju regression 1, "
        "Maju workflow 3)",
    )
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the Maju desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def _read_maju_task(task_toml: Path) -> MajuTask:
    ...
    try:
        ...
        raise SystemExit(
            f"invalid Maju task metadata in {task_toml}: {error}"
        ) from error
    if not isinstance(name, str) or not name:
        raise SystemExit(f"invalid Maju task name in {task_toml}: expected a string")
    ...
    return MajuTask(name=name, layer=layer, path=task_toml.parent)


def maju_tasks_for_path(path: Path | None) -> tuple[MajuTask, ...] | None:
    """Return validated Maju tasks, or ``None`` for an unrelated problem set."""
    if path is None:
        return None
    root = MAJU_DATASET_ROOT.resolve()
    ...
    if not task_files:
        raise SystemExit(f"no Maju tasks found under {path}")
    tasks = tuple(_read_maju_task(task_file) for task_file in task_files)
    names = [task.name for task in tasks]
    if len(names) != len(set(names)):
        raise SystemExit(f"duplicate Maju task names found under {path}")
    return tasks


def _matches_task(task: MajuTask, pattern: str) -> bool:
    return fnmatch.fnmatchcase(task.name, pattern) or fnmatch.fnmatchcase(
        task.path.name, pattern
    )


def select_maju_tasks(
    tasks: tuple[MajuTask, ...],
    *,
    layer: str | None,
    include: list[str],
    exclude: list[str],
    ...
    if not selected:
        ...
        raise SystemExit(f"no Maju tasks selected{detail}")
    return selected
...
def plan_benchmark_runs(
    ...
    tasks = maju_tasks_for_path(args.path)
    if args.layer and tasks is None:
        raise SystemExit(
            "--layer is only valid with --path under benchmarks/maju-dataset"
        )
    if tasks is None:
        return (
            _copy_run_args(
...
def load_state() -> dict[str, str]:
    ...
    from harbor_maju_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_maju_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        ...
        "MAJU_MEDIA_SERVER_DOMAIN": "localhost",
        "MAJU_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "MAJU_REQUIRE_AUTH_TOKEN": "true",
        "MAJU_REQUIRE_RELAY_MEMBERSHIP": "true",
        "MAJU_ALLOW_NIP_OA_AUTH": "true",
        "MAJU_AUTO_MIGRATE": "true",
        "MAJU_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "maju_relay=info,maju_db=info,maju_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "MAJU_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "MAJU_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "maju",
        "POSTGRES_USER": "maju",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "MAJU_S3_ACCESS_KEY": state["s3_access_key"],
        "MAJU_S3_SECRET_KEY": state["s3_secret_key"],
        "MAJU_S3_BUCKET": "maju-media",
        "MAJU_HTTP_PORT": str(RELAY_HTTP_PORT),
        "MAJU_PG_HOST_PORT": str(PG_HOST_PORT),
        "MAJU_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://maju:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/maju"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host maju CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host maju CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "maju-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    maju-acp → maju-agent → maju-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Maju desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via maju-admin inside the container —
    ...
    for name in (
        ...
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"maju-{triple}"
    real_cli.write_bytes(binaries["maju"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Maju GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides MAJU_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Maju Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "MAJU_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "MAJU_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # MAJU_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('MAJU_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
