"""Run the production Maju agent stack inside the Harbor task container.

Each provisioned identity is a full ``maju-acp`` → ``maju-agent`` →
``maju-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``maju`` CLI on
PATH) that the desktop app gives a Maju agent. The harness stays outside:
...
DELIVERY_RECEIPT_MARKER = "turn delivered Maju events for channel"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Maju agent process cannot be launched or exits early."""
...
class MajuContainerRuntime:
    """Launch one production Maju agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        maju_acp_binary: str = "maju-acp",
        maju_agent_binary: str = "maju-agent",
        maju_dev_mcp_binary: str = "maju-dev-mcp",
        maju_cli_binary: str = "maju",
        relay_gateway: str = "",
        ...
        self.maju_acp_binary = maju_acp_binary
        self.maju_agent_binary = maju_agent_binary
        self.maju_dev_mcp_binary = maju_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.maju_cli_binary = maju_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            # The task arrives exactly as it would in production Maju: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/maju-acp": self.maju_acp_binary,
            f"{REMOTE_BIN}/maju-agent": self.maju_agent_binary,
            f"{REMOTE_BIN}/maju-dev-mcp": self.maju_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/maju-acp')} </dev/null "
        ...
        *,
        ...
        return {
            **endpoint.env,
            ...
            # so maju-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "MAJU_AUTH_TAG": credential.nostr_auth_tag,
            "MAJU_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/maju-agent",
            "MAJU_ACP_AGENT_ARGS": "",
            "MAJU_ACP_MCP_COMMAND": f"{REMOTE_BIN}/maju-dev-mcp",
            "MAJU_ACP_CHANNELS": trial.channel_id,
            "MAJU_ACP_SUBSCRIBE": "mentions",
            "MAJU_ACP_RESPOND_TO": "anyone",
            "MAJU_ACP_NO_MEMORY": "true",
            ...
            "MAJU_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "MAJU_AGENT_MAX_ROUNDS": str(
    ...
    async def _turn_counts(
        ...
        starts, ends, _ = await MajuContainerRuntime._turn_status(environment, agent)
        return starts, ends
    ...
    async def _turn_status(
        ...
        return MajuContainerRuntime._parse_turn_status(result.stdout or "")
        ...
        for seed in fixture_for(trial.task_name).memory_seeds:
            try:
                process = await asyncio.create_subprocess_exec(
                    ...
                    env={
                        **os.environ,
                        "MAJU_RELAY_URL": self._user_relay_url(trial),
                        "MAJU_PRIVATE_KEY": credential.nostr_secret_key,
                        "MAJU_AUTH_TAG": credential.nostr_auth_tag,
                    },
                )
            ...
            if process.returncode != 0:
                ...
                raise RuntimeLaunchError(
                    f"maju mem set {seed.slug} - exited {process.returncode}: {detail}"
                )
    ...
    async def _maju_json(
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"maju {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("maju returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return MajuContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Maju workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Maju channel `{trial.channel_id}`.",
            (
