"""Public setup declarations for Maju-native benchmark tasks."""
...
_CREATE_CHANNEL_FIXTURE = MajuTaskFixture(
    directory=tuple(
        [
...
_USER_MENTION_FIXTURE = MajuTaskFixture(
    user_display_name=USER_MENTION_DISPLAY_NAME,
    requires_evidence=True,
)

_NARRATIVE_AGENT_NAMES_FIXTURE = MajuTaskFixture(
    directory=(
...
_INTERLEAVED_AGENT_REPORTS_FIXTURE = MajuTaskFixture(
    directory=(
...
_CROSS_THREAD_REQUESTS_FIXTURE = MajuTaskFixture(
    scripted_messages=(
        ScriptedMessage(
            label="beta-request",
            actor="user",
...
_AMBIGUOUS_USER_MENTION_FIXTURE = MajuTaskFixture(
    directory=(
        DirectoryEntry(
            "Taylor Morgan Lee",
            "user",
...
# Noisy memories test retrieval of one relevant value through `maju mem ls/get`.
_MEMORY_RETRIEVAL_FIXTURE = MajuTaskFixture(
...
_FIXTURES = {
    ...
    REPLY_TO_THREAD_TASK: MajuTaskFixture(requires_evidence=True),
    READ_NAMED_PATH_TASK: MajuTaskFixture(requires_evidence=True),
    MULTILINE_MESSAGE_TASK: MajuTaskFixture(
...
def fixture_for(task_name: str | None) -> MajuTaskFixture:
    """Return the declared setup for a task, or an empty setup."""
    return _FIXTURES.get(task_name or "", MajuTaskFixture())
