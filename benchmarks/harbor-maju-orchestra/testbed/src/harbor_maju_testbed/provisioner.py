"""Channel-per-trial Maju provisioning against a local benchmark stack."""
...
from harbor_maju_orchestra.task_fixtures import DirectoryEntry, fixture_for
...
def provisioner_from_dict(config: dict[str, object]) -> MajuTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return MajuTrialProvisioner(TestbedConfig(**config))


class MajuTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Maju relay.

    Guarantees (contract PLANS/HARBOR_MAJU_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Maju, where a human owns the channel their
        # agents work in.
    ...
    def _seed_directory(
        self, task_name: str | None, observer: MajuCli
    ...
    def _stable_credential(
        ...
        digest = hashlib.sha256(
            b"maju-benchmark-directory-v1\0"
    ...
    def _cli_for(self, credential: AgentCredential) -> MajuCli:
        return MajuCli(
