...
Gated by MAJU_TESTBED_LIVE=1 with stack coordinates in the environment:
  MAJU_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  MAJU_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  MAJU_TESTBED_OWNER_KEY    relay owner secret key (hex)
  MAJU_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
from harbor_maju_testbed.maju_cli import MajuCli, MajuCliError
from harbor_maju_testbed.provisioner import (
...
pytestmark = pytest.mark.skipif(
    os.environ.get("MAJU_TESTBED_LIVE") != "1",
    reason="live testbed suite; set MAJU_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> MajuTrialProvisioner:
    owner_key = os.environ.get("MAJU_TESTBED_OWNER_KEY")
    dsn = os.environ.get("MAJU_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("MAJU_TESTBED_OWNER_KEY and MAJU_TESTBED_PG_DSN are required")
    return MajuTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "MAJU_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "MAJU_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: MajuTrialProvisioner, credential) -> MajuCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(MajuCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["MAJU_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
