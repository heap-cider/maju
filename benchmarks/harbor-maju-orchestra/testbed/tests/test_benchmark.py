...
def test_maju_task_metadata_defines_the_expected_layers():
    tasks = benchmark.maju_tasks_for_path(benchmark.MAJU_DATASET_ROOT)
...
def test_maju_task_metadata_rejects_missing_or_unknown_layers(
    tmp_path, monkeypatch, layer
):
    dataset = tmp_path / "maju-dataset"
    task = dataset / "example"
    task.mkdir(parents=True)
    metadata = "" if layer is None else f'evaluation_layer = "{layer}"\n'
    (task / "task.toml").write_text(
        'schema_version = "1.3"\n\n'
        '[task]\nname = "maju-native/example"\n\n'
        f'[metadata]\n{metadata}difficulty = "easy"\n'
    )
    monkeypatch.setattr(benchmark, "MAJU_DATASET_ROOT", dataset)

    with pytest.raises(SystemExit, match="evaluation_layer|metadata"):
        benchmark.maju_tasks_for_path(dataset)
...
def test_layer_selects_metadata_and_uses_its_default_attempts(layer, attempts):
    args = benchmark.parse_args(
        ["--path", str(benchmark.MAJU_DATASET_ROOT), "--layer", layer]
    )
    (run,) = benchmark.plan_benchmark_runs(args)

    assert run.attempts == attempts
    selected = benchmark.maju_tasks_for_path(benchmark.MAJU_DATASET_ROOT)
...
def test_layer_selectors_resolve_through_harbor_local_dataset_filter(layer):
    ...
    configs = asyncio.run(
        DatasetConfig(
            path=benchmark.MAJU_DATASET_ROOT,
...
def test_omitted_layer_splits_maju_dataset_into_two_jobs():
    args = benchmark.parse_args(["--path", str(benchmark.MAJU_DATASET_ROOT)])
...
def test_single_maju_task_infers_its_layer_default():
    task_path = benchmark.MAJU_DATASET_ROOT / "cross-thread-requests"
...
def test_explicit_attempts_override_keeps_one_mixed_maju_job():
    args = benchmark.parse_args(
        ["--path", str(benchmark.MAJU_DATASET_ROOT), "--attempts", "7"]
    )
    (run,) = benchmark.plan_benchmark_runs(args)
    ...
    layered = benchmark.parse_args(
        [
            "--path",
            str(benchmark.MAJU_DATASET_ROOT),
            "--layer",
            "workflow",
            "-k",
            "2",
...
def test_layer_dry_run_constructs_exact_task_selectors_and_attempts():
    ...
    binaries = {"maju": Path("host-bin/maju")}
    agent_binaries = {
...
def test_env_file_wires_owner_and_ports(state_dir):
    ...
    assert env["MAJU_HTTP_PORT"] == str(benchmark.RELAY_HTTP_PORT)
    assert env["MAJU_PG_HOST_PORT"] == str(benchmark.PG_HOST_PORT)
    assert env["MAJU_REQUIRE_RELAY_MEMBERSHIP"] == "true"
