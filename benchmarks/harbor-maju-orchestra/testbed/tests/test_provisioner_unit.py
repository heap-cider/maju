...
import coincurve
import pytest
from harbor_maju_orchestra.task_fixtures import DirectoryEntry
...
def test_mint_credentials_expands_roster(manifest):
    credentials = MajuTrialProvisioner(config())._mint_credentials(manifest)
...
def test_mint_credentials_keys_are_fresh_and_attested(manifest):
    provisioner = MajuTrialProvisioner(config())
...
def test_user_mention_task_gets_stable_three_word_user_identity():
    provisioner = MajuTrialProvisioner(config(user_secret_key="7" * 64))
...
def test_pinned_user_secret_reuses_one_identity():
    pinned = "7" * 64
    provisioner = MajuTrialProvisioner(config(user_secret_key=pinned))
...
def test_teardown_skips_archiving_when_disabled():
    provisioner = MajuTrialProvisioner(config(archive_on_teardown=False))
...
def test_mint_credentials_missing_api_key_is_explicit(manifest):
    provisioner = MajuTrialProvisioner(config(llm_api_keys={}))
...
def test_lock_key_is_deterministic_and_distinct():
    ...
    MajuTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-1")
    MajuTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-1")
    MajuTrialProvisioner._lock_trial(FakeConn(), "run-a", "trial-2")
...
def test_healthcheck_fails_fast_when_relay_down():
    provisioner = MajuTrialProvisioner(
