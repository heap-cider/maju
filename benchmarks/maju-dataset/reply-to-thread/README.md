...
> the behavior under test, and it must come from `maju-acp`'s production base
...
container's shell. `MajuOrchestraAgent` launches the real `maju-acp` /
`maju-agent` stack against a dedicated relay, and the agent works entirely
through Maju. Agent timeout 300s; the manifest's `trial_budget` is the
effective clock.

## Verifier

Reads the post-agent `/logs/artifacts/maju-evidence.json` snapshot (written by
`MajuContainerRuntime._collect_evidence` after the agent stops, so the agent
...
shipped: the Oracle agent replaces `MajuOrchestraAgent`, so no relay trial is
...
`../harbor-maju-orchestra/tests/test_reply_to_thread_verifier.py` (run from
`benchmarks/harbor-maju-orchestra`: `uv run --extra dev pytest -q`).
