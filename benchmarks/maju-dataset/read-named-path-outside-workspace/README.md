...
`python:3.12-slim-bookworm` with `HOME=/home/maju`, so `~` in the instruction
...
Reads the post-agent `/logs/artifacts/maju-evidence.json` snapshot plus the
...
```bash
just benchmark \
  --path benchmarks/maju-dataset/read-named-path-outside-workspace \
  --attempts 1 \
...
`../harbor-maju-orchestra/tests/test_read_named_path_outside_workspace_verifier.py`.
