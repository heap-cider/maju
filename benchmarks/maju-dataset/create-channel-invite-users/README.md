...
`maju-agent` stack against a dedicated relay, and the agent does all its work
through `maju channels create` / `channels invite`. Agent timeout 300s.

Directory identities are derived deterministically from the owner key
(`MajuTrialProvisioner._stable_credential`) without persisting any secret, and
`_seed_directory` skips profiles already published — so reruns are idempotent
and pubkeys are stable across trials.

## Verifier

Reads the post-agent `/logs/artifacts/maju-evidence.json` snapshot. The
...
```bash
just benchmark \
  --path benchmarks/maju-dataset/create-channel-invite-users \
  --attempts 1 \
...
`../harbor-maju-orchestra/tests/test_create_channel_invite_users_verifier.py`.
