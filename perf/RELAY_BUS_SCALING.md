# Maju relay bus scaling harness
...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which it has local subscribers (`maju:{community_id}:global` or `maju:{community_id}:channel:{channel_id}`).
...
The scoped Redis channel format corresponds to `maju_pubsub::EventTopicKey::redis_channel()` in `crates/maju-pubsub/src/topic.rs`:

- global: `maju:{community_id}:global`
- channel: `maju:{community_id}:channel:{channel_id}`
