...
final class MajuPushSnapshotBridge {
  private let appGroupIdentifier: String?
  private let endpointGrantStore: MajuPushEndpointGrantKeychainStore
  private let keychainAccessGroup: String?
  private let queue = DispatchQueue(
    label: "xyz.block.maju.push-snapshot",
    qos: .utility
  )
  private lazy var store: MajuPushPresentationCacheStore? = {
    guard let appGroupIdentifier,
      let container = FileManager.default.containerURL(
        forSecurityApplicationGroupIdentifier: appGroupIdentifier
      )
    else { return nil }
    return MajuPushPresentationCacheStore(containerURL: container)
  }()

  init(
    appGroupIdentifier: String?,
    endpointGrantStore: MajuPushEndpointGrantKeychainStore,
  ...
  private func syncCommunities(_ arguments: [String: Any], result: @escaping FlutterResult) {
    guard let communities = arguments["communities"] as? [[String: Any]],
      ...
      communities.count <= MajuPushPresentationCacheStore.maximumCommunities
    else {
      result(
        FlutterError(
  ...
  static func relayMetadataPubkey(
    relayURL: String,
    grants: [MajuPushEndpointGrantRecord]
  ) -> String? {
    guard let origin = MajuPushPresentationCacheStore.canonicalRelayOrigin(relayURL) else {
      return nil
    }
    return grants.filter {
      $0.appProfile == MajuDevPushEnrollmentDriver.appProfile
        && MajuPushPresentationCacheStore.canonicalRelayOrigin($0.relayOrigin) == origin
    }.max {
  ...
  private func cacheProfiles(_ rawArguments: Any?, result: @escaping FlutterResult) {
    guard let arguments = rawArguments as? [String: Any],
      ...
      rawEvents.count <= MajuPushPresentationCacheStore.maximumProfiles
    else {
      result(
        FlutterError(
          code: "invalid_arguments",
    ...
    queue.async { [weak self] in
      do {
        ...
        try store?.updateProfiles(
          ...
          updates: events.map { MajuPushProfileCacheUpdate(event: $0) }
        )
        Self.complete(result, value: nil)
      } catch {
        Self.complete(
  ...
  private func cacheChannels(_ rawArguments: Any?, result: @escaping FlutterResult) {
    guard let arguments = rawArguments as? [String: Any],
      ...
      rawMetadataEvents.count <= MajuPushPresentationCacheStore.maximumChannels,
      rawMembershipEvents.count <= MajuPushPresentationCacheStore.maximumChannels
    else {
      result(
        FlutterError(
          code: "invalid_arguments",
  ...
  private func community(id: String) -> PushLeaseCommunity? {
    guard let appGroupIdentifier,
      ...
      let data = try? Data(contentsOf: container.appendingPathComponent(MajuPushPresentationCacheStore.fileName)),
      let snapshot = try? JSONDecoder().decode(MajuPushPresentationCacheSnapshot.self, from: data)
    else { return nil }
