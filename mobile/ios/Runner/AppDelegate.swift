...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  private let pushNavigationBuffer = MajuPushNavigationBuffer()
  private var apnsDeviceToken: Data?
  private lazy var endpointGrantStore = MajuPushEndpointGrantKeychainStore(
    accessGroup: Bundle.main.object(forInfoDictionaryKey: "MajuKeychainAccessGroup") as? String
  )
  private var enrollmentTask: Task<Void, Never>?
  private var appGroupIdentifier: String? {
    Bundle.main.object(forInfoDictionaryKey: "MajuAppGroupIdentifier") as? String
  }
  private var pushKeychainAccessGroup: String? {
    Bundle.main.object(forInfoDictionaryKey: "MajuKeychainAccessGroup") as? String
  }
  private lazy var pushSnapshotBridge = MajuPushSnapshotBridge(
  ...
  func didInitializeImplicitFlutterEngine(_ engineBridge: FlutterImplicitEngineBridge) {
    ...
    if let inlinePhotoPickerRegistrar = engineBridge.pluginRegistry.registrar(
      forPlugin: "MajuInlinePhotoPicker"
    ) {
      inlinePhotoPickerRegistrar.register(
        ...
        withId: "maju/inline_photo_picker"
      )
    }
    ...
    if let concentricSheetRegistrar = engineBridge.pluginRegistry.registrar(
      ...
      concentricSheetSurfaceChannel = FlutterMethodChannel(
        name: "maju/concentric_sheet_surface",
        binaryMessenger: messenger
      )
  ...
  private func deliverPushNavigationTarget(_ target: MajuPushNavigationTarget) {
  ...
  private func startPushRegistration(result: @escaping FlutterResult) {
    UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) {
      ...
      if let error {
        os_log(
          "Maju notification authorization request failed: %{public}@",
          type: .error,
  ...
  private func handleDevPushEnrollment(
    ...
    do {
      let driver = try MajuDevPushEnrollmentDriver(
        gatewayBaseURL: gatewayURL,
        store: endpointGrantStore,
        appAttestKeychainAccessGroup: Bundle.main.object(
          forInfoDictionaryKey: "MajuKeychainAccessGroup"
        ) as? String
      )
  ...
  private func transcodeVideoToMp4(
    ...
    guard
      let exportSession = AVAssetExportSession(
        ...
        // metadata. Maju accepts only canonical H.264/AAC MP4s with no
    ...
    exportSession.exportAsynchronously {
      ...
      case .completed:
        do {
          ...
          // Older Maju relays mistook that playback-only box for metadata. Keep
...
extension MajuPushNavigationTarget {
