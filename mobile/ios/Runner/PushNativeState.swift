...
final class MajuOneShotCompletion {
...
enum MajuPushNotificationResponseCoordinator {
  static func handle(
    actionIdentifier: String,
    userInfo: [AnyHashable: Any],
    onTarget: (MajuPushNavigationTarget) -> Void,
    forwardToFlutter: (@escaping () -> Void) -> Void,
    completion: @escaping () -> Void
  ) {
    let completionGate = MajuOneShotCompletion(completion)
    defer { completionGate.call() }

    if actionIdentifier == UNNotificationDefaultActionIdentifier,
      let target = MajuPushNavigationTarget.decodeIfPresent(from: userInfo)
    {
      onTarget(target)
    }
...
enum MajuPushKeychain {
  static let service = "maju.push.nse.signing"
