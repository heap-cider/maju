...
final class MajuDevPushEnrollmentDriverTests: XCTestCase {
  ...
  fileprivate static let assertion = Data("maju-dev-app-assertion-v1".utf8).base64EncodedString()
  ...
  func testLegacyGrantDecodesWithoutMetadataAuthority() throws {
    ...
    let record = try JSONDecoder().decode(MajuPushEndpointGrantRecord.self, from: data)
  ...
  func testRealAppAttestFailsLoudlyWhenUnsupported() async throws {
    ...
    let provider = MajuDCAppAttestProvider(
      service: service,
    ...
    do {
      ...
      XCTAssertEqual(error as? MajuDevPushEnrollmentError, .appAttestUnsupported)
    }
  ...
  func testRealAppAttestGeneratesPersistsAndMapsAttestation() async throws {
    ...
    let provider = MajuDCAppAttestProvider(service: service, keyIdStore: keyIdStore)
    ...
    XCTAssertEqual(prepared, MajuDevAttestation(keyId: Self.keyId, attestation: ""))
  ...
  func testRealAppAttestRejectsInvalidGeneratedKeyBeforePersistence() async throws {
    for invalidKeyId in [
      ...
      do {
        ...
        XCTAssertEqual(error as? MajuDevPushEnrollmentError, .invalidAppAttestKeyId)
      }
  ...
  func testRealAppAttestRejectsMismatchedPreparedKey() async throws {
    ...
    do {
      _ = try await provider.attestation(
        MajuDevAttestation(keyId: otherKeyId, attestation: ""),
  ...
  func testKeychainStoreReadsKeyIdAndIncludesAccessGroup() throws {
    ...
    let store = MajuAppAttestKeyIdKeychainStore(
      accessGroup: "group.maju",
    ...
    XCTAssertEqual(capturedQuery[kSecAttrService as String] as? String, "maju.push.app-attest")
    XCTAssertEqual(capturedQuery[kSecAttrAccount as String] as? String, "key-id-v1")
    XCTAssertEqual(capturedQuery[kSecAttrAccessGroup as String] as? String, "group.maju")
  ...
  func testKeychainStoreReturnsNilOnMissAndRejectsInvalidData() throws {
    let missing = MajuAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    for invalidKeyId in [
      ...
      let invalid = MajuAppAttestKeyIdKeychainStore(
        accessGroup: nil,
      ...
      XCTAssertThrowsError(try invalid.keyId(), "Accepted invalid key ID: \(invalidKeyId)") {
        XCTAssertEqual($0 as? MajuDevPushEnrollmentError, .invalidAppAttestKeyId)
      }
    }
  }
  ...
  func testKeychainStoreUpdatesExistingKeyId() throws {
    ...
    XCTAssertEqual(updatedQuery[kSecAttrService as String] as? String, "maju.push.app-attest")
  ...
  func testKeychainStoreAddsMissingKeyIdWithDeviceOnlyAccessibility() throws {
    ...
    XCTAssertEqual(addedItem[kSecAttrAccessGroup as String] as? String, "group.maju")
  }

  func testKeychainStoreSurfacesReadUpdateAndAddErrors() throws {
    let readFailure = MajuAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    let updateFailure = MajuAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    let addFailure = MajuAppAttestKeyIdKeychainStore(
      accessGroup: nil,
      update: { _, _ in errSecItemNotFound },
  ...
  func testReusesPersistedUnexpiredGrant() async throws {
    let existing = MajuPushEndpointGrantRecord(
      relayOrigin: "wss://relay.example",
      relayPubkey: Self.relayPubkey,
      relayMetadataPubkey: Self.relayPubkey,
      installationId: Self.installationId,
  ...
  func testRejectsMultipleCurrentRelayKeysBeforeGatewayEnrollment() async throws {
    ...
    do {
      ...
      XCTAssertEqual(error as? MajuDevPushEnrollmentError, .invalidRelayDescriptor)
    }
    XCTAssertEqual(URLProtocolStub.requests.count, 1)
  }
  ...
  func testFailsLoudlyOnUnexpectedGatewayStatus() async throws {
    ...
    } catch let error as MajuDevPushEnrollmentError {
      XCTAssertEqual(
        error,
        .unexpectedStatus(
  ...
  ) throws -> MajuDevPushEnrollmentDriver {
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [URLProtocolStub.self]
    return try MajuDevPushEnrollmentDriver(
...
private final class MemoryGrantStore: MajuPushEndpointGrantStore {
  var saved: [MajuPushEndpointGrantRecord]
  var pending: [MajuPushPendingEnrollmentRecord] = []
  var grantSaveFailuresRemaining: Int
  init(
    records: [MajuPushEndpointGrantRecord] = [],
    grantSaveFailuresRemaining: Int = 0
  ) {
    saved = records
    self.grantSaveFailuresRemaining = grantSaveFailuresRemaining
  }
  func records() throws -> [MajuPushEndpointGrantRecord] { saved }
...
private final class RecordingAppAttest: MajuDevAppAttesting {
  var clientData: [Data] = []

  func prepareAttestation() async throws -> MajuDevAttestation {
    MajuDevAttestation(
      keyId: MajuDevPushEnrollmentDriverTests.keyId,
      attestation: MajuDevPushEnrollmentDriverTests.attestation
    )
  }
  ...
  func assertion(clientData: Data) async throws -> String {
    self.clientData.append(clientData)
    return MajuDevPushEnrollmentDriverTests.assertion
  }
}

private final class MemoryAppAttestKeyIdStore: MajuAppAttestKeyIdStoring {
...
private final class RecordingDCAppAttestService: MajuDCAppAttestServicing {
  ...
  init(
    ...
    generatedKeyId: String = MajuDevPushEnrollmentDriverTests.keyId,
