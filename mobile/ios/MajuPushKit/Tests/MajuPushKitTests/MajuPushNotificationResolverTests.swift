...
final class MajuPushNotificationResolverTests: XCTestCase {
  ...
  func testDecodeResolutionFiltersOwnPubkeyEvent() {
    let result = MajuPushNotificationResolver.decodeResolution(
  ...
  func testPreviewBodySanitizesCodeLinksAndWhitespace() {
    ...
    XCTAssertEqual(
      MajuPushNotificationResolver.previewBody(content),
      "Before [code] inline docs image [link] After"
    )
  }

  func testPreviewBodyTruncatesTo178CharactersIncludingEllipsis() {
    let preview = MajuPushNotificationResolver.previewBody(String(repeating: "x", count: 200))
  ...
  func testResolveSucceedsAndMutatesGatewayContent() throws {
    ...
    XCTAssertEqual(result.body, "Hello Maju")
    XCTAssertEqual(result.subtitle, "Community")
    XCTAssertEqual(
      result.threadIdentifier,
      MajuPushPresentationIdentity.conversation(
  ...
  func testStaleVerifiedCacheIsUsedWhileOneBoundedRefreshFails() throws {
    ...
    let relayPubkey = try Self.pubkey(for: Self.relayPrivateKey)
    let staleAt = Self.now - Int(MajuPushPresentationCacheStore.freshnessLifetime) - 1
    let snapshot = MajuPushPresentationCacheSnapshot(
      profiles: [
        MajuPushCachedProfile(
          communityID: "community-id",
  ...
  func testOversizedPresentationRefreshFallsBackWithoutBlockingMessage() throws {
    ...
    URLProtocolStub.handler = { request in
      ...
      var oversized = Data(
        repeating: 0x20,
        count: MajuPushNotificationResolver.maximumPresentationResponseBytes
      )
  ...
  func testBoundedInlineAvatarProfileRefreshStillResolvesDisplayName() throws {
    ...
    XCTAssertLessThan(
      ...
      MajuPushNotificationResolver.maximumPresentationResponseBytes
    )
    URLProtocolStub.handler = { request in
      Self.response(
        request,
  ...
  func makeResolver(
    ...
    now: Date = Date()
  ) -> MajuPushNotificationResolver {
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [URLProtocolStub.self]
    return MajuPushNotificationResolver(
  ...
  func resolve(_ resolver: MajuPushNotificationResolver) -> MajuPushResolution? {
    let completed = expectation(description: "resolver completed")
    var result: MajuPushResolution?
    resolver.resolve {
      result = $0
      completed.fulfill()
    }
