...
struct MajuPushPresentationCacheTests {
  ...
  func verifiedProfilePrecedenceAndAvatar() throws {
    ...
    let store = MajuPushPresentationCacheStore(
    ...
    let needsAvatar = try store.updateProfiles(
      ...
      updates: [MajuPushProfileCacheUpdate(event: event)]
    )
  ...
  func verifiedInlineRasterProfileAndAvatar() throws {
    let directory = try temporaryDirectory()
    defer { try? FileManager.default.removeItem(at: directory) }
    let store = MajuPushPresentationCacheStore(containerURL: directory)
  ...
  func malformedAndUnverifiedProfileFallback() throws {
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [MajuPushProfileCacheUpdate(event: named)]
    )
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [MajuPushProfileCacheUpdate(event: malformed)]
    )
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [MajuPushProfileCacheUpdate(event: tampered)]
    )
    ...
    #expect(
      cached.memberDigests
        == [firstMember, secondMember].map {
          MajuPushPresentationIdentity.channelMember(
            communityID: "community-a",
            channelID: "opaque-channel",
            pubkey: $0
          )
  ...
  func oversizedChannelBatchIsIgnored() throws {
    ...
    try store.updateChannels(
      ...
      metadataEvents: Array(
        ...
        count: MajuPushPresentationCacheStore.maximumChannels + 1
      ),
      membershipEvents: []
    )
  ...
  func globalMembershipDigestBound() throws {
    let membersPerChannel = MajuPushPresentationCacheStore.maximumMembersPerChannel
    let channelCount =
      MajuPushPresentationCacheStore.maximumTotalMemberDigests / membersPerChannel + 1
    let digests = (0..<membersPerChannel).map { String(format: "%064x", $0 + 1) }
    let snapshot = MajuPushPresentationCacheSnapshot(
      channels: (0..<channelCount).map { index in
        MajuPushCachedChannel(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
    ...
    let encoded = try MajuPushPresentationCacheStore.encodedBoundedSnapshot(snapshot)
    let decoded = try JSONDecoder().decode(MajuPushPresentationCacheSnapshot.self, from: encoded)
    let totalDigests = decoded.channels.reduce(0) { $0 + ($1.memberDigests?.count ?? 0) }

    #expect(totalDigests == MajuPushPresentationCacheStore.maximumTotalMemberDigests)
  ...
  func oversizedMembershipFallback() throws {
    ...
    let memberCount = MajuPushPresentationCacheStore.maximumMembersPerChannel + 25
    let membership = try signedEvent(
      privateKey: relayKey,
      kind: 39_002,
    ...
    #expect(
      cached.memberCount == MajuPushPresentationCacheStore.maximumMembersPerChannel + 1
    )
    #expect(cached.memberDigests == nil)
  ...
  func communityPruning() throws {
    ...
    try store.updateProfiles(
      communityID: "removed",
      relayOrigin: "https://relay.example",
      updates: [MajuPushProfileCacheUpdate(event: profile)]
    )
    try store.updateChannels(
      communityID: "removed",
      relayOrigin: "https://relay.example",
  ...
  func boundedEviction() {
    var snapshot = MajuPushPresentationCacheSnapshot(
      profiles: (0...MajuPushPresentationCacheStore.maximumProfiles).map { index in
        MajuPushCachedProfile(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
      ...
      channels: (0...MajuPushPresentationCacheStore.maximumChannels).map { index in
        MajuPushCachedChannel(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
          channelID: "channel-\(index)",
    ...
    MajuPushPresentationCacheStore.enforceBounds(&snapshot)

    #expect(snapshot.profiles.count == MajuPushPresentationCacheStore.maximumProfiles)
    #expect(snapshot.channels.count == MajuPushPresentationCacheStore.maximumChannels)
  ...
  func encodedCacheByteBound() throws {
    ...
    let avatar =
      ...
      + Data(
        repeating: 0,
        count: MajuPushPresentationCacheStore.maximumAvatarBytes - 8
      )
    let snapshot = MajuPushPresentationCacheSnapshot(
    ...
    let decoded = try JSONDecoder().decode(
      MajuPushPresentationCacheSnapshot.self,
      from: encoded
    )

    #expect(encoded.count <= MajuPushPresentationCacheStore.maximumSnapshotBytes)
  ...
  func displayNameUTF8Bound() throws {
    ...
    let metadata = MajuPushPresentationCacheStore.profileMetadata(event)
  ...
  private func temporaryDirectory() throws -> URL {
    let url = FileManager.default.temporaryDirectory
      .appendingPathComponent("maju-push-cache-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
    return url
  }

  private func loadSnapshot(_ directory: URL) throws -> MajuPushPresentationCacheSnapshot {
    let data = try Data(
      contentsOf: directory.appendingPathComponent(MajuPushPresentationCacheStore.fileName)
    )
    return try JSONDecoder().decode(MajuPushPresentationCacheSnapshot.self, from: data)
  }
