...
/// Content resolved from unread Maju events for a mutable push notification.
public struct MajuPushResolution: Decodable, Equatable, Sendable {
  public let title: String
  public let body: String
  public let subtitle: String?
  public let threadIdentifier: String?
  public let navigationTarget: MajuPushNavigationTarget?
  ...
  public init(
    ...
    navigationTarget: MajuPushNavigationTarget? = nil,
...
/// Resolves the content used to mutate a generic Maju push notification.
public protocol MajuPushNotificationResolving {
  func resolve(completion: @escaping (MajuPushResolution?) -> Void)
}

/// Reads configured Maju communities and resolves their newest unread event.
public final class MajuPushNotificationResolver: MajuPushNotificationResolving {
  ...
  public init(
    ...
    presentationCacheLifetime: TimeInterval = MajuPushPresentationCacheStore.freshnessLifetime
  ) {
  ...
  public func resolve(completion: @escaping (MajuPushResolution?) -> Void) {
  ...
  private func resolvePresentation(
    ...
    completion: @escaping (MajuPushResolution?) -> Void
  ) {
    let snapshot = MajuPushPresentationCacheSnapshot.decode(loadPresentationCacheData())
    let relayOrigin = MajuPushPresentationCacheStore.canonicalRelayOrigin(community.relayUrl)
    ...
    let membershipNeedsRefresh: Bool = {
      ...
      guard memberCount <= MajuPushPresentationCacheStore.maximumMembersPerChannel
      else { return false }
    ...
    refreshPresentation(
      ...
      let profile =
        refreshedProfileEvent.flatMap {
          guard
            MajuPushPresentationCacheStore.shouldReplace(
  ...
  ) -> (MajuPushResolution, VerifiedNostrEvent)? {
  ...
  private static func makeResolution(
    event: VerifiedNostrEvent,
    community: PushLeaseCommunity,
    profile: MajuPushCachedProfile?,
    channel: MajuPushCachedChannel?
  ) -> MajuPushResolution? {
    let body = previewBody(event.content)
    guard !body.isEmpty else { return nil }
    let channelID = tagValue("h", in: event)
    let conversationIdentifier = channelID.map {
      MajuPushPresentationIdentity.conversation(communityID: community.id, channelID: $0)
    }
  ...
  private static func ephemeralChannel(
    ...
    let membership = membershipEvent.flatMap {
      MajuPushPresentationCacheStore.normalizedChannelMembership(
        $0,
        communityID: communityID,
        channelID: channelID
      )
    ...
    return MajuPushCachedChannel(
      communityID: communityID,
      relayOrigin: relayOrigin,
      channelID: channelID,
      relayMetadataPubkey: relayMetadataPubkey,
      displayName: metadataEvent.map {
        MajuPushPresentationCacheStore.normalizedDisplayName(tagValue("name", in: $0))
      } ?? cached?.displayName,
      channelType: metadataEvent.map {
        MajuPushPresentationCacheStore.normalizedChannelType(tagValue("t", in: $0))
      } ?? cached?.channelType,
  ...
  private static func communicationConversation(
    ...
    let currentUserDigest = MajuPushPresentationIdentity.channelMember(
      communityID: community.id,
      channelID: channel.channelID,
      pubkey: currentUser
    )
    guard memberDigests.contains(currentUserDigest) else { return nil }
    let senderDigest = MajuPushPresentationIdentity.channelMember(
      communityID: community.id,
      channelID: channel.channelID,
      pubkey: event.pubkey
    )
