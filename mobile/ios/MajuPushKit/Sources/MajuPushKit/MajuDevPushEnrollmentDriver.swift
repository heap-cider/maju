...
public struct MajuPushEndpointGrantRecord: Codable, Equatable, Sendable {
...
public protocol MajuPushEndpointGrantStore {
...
public enum MajuDevPushEnrollmentError: Error, LocalizedError, Equatable {
...
protocol MajuDevAppAttesting {
  func prepareAttestation() async throws -> MajuDevAttestation
  func attestation(_ prepared: MajuDevAttestation, clientData: Data) async throws
    -> MajuDevAttestation
  func assertion(clientData: Data) async throws -> String
}

struct MajuDevAttestation: Equatable {
  let keyId: String
  let attestation: String
}

private enum MajuSecureRandom {
  static func bytes(count: Int) throws -> Data {
    var bytes = [UInt8](repeating: 0, count: count)
    let status = SecRandomCopyBytes(kSecRandomDefault, bytes.count, &bytes)
    guard status == errSecSuccess else {
      throw MajuDevPushEnrollmentError.randomGenerationFailed(status)
    }
    return Data(bytes)
  }
}

private enum MajuAppAttestKeyId {
...
protocol MajuAppAttestKeyIdStoring {
  func keyId() throws -> String?
  func saveKeyId(_ keyId: String) throws
}

struct MajuAppAttestKeyIdKeychainStore: MajuAppAttestKeyIdStoring {
  private static let service = "maju.push.app-attest"
  ...
  func keyId() throws -> String? {
    ...
    guard let data = result as? Data,
      ...
      MajuAppAttestKeyId.isValid(keyId)
    else {
      throw MajuDevPushEnrollmentError.invalidAppAttestKeyId
    }
    return keyId
  }

  func saveKeyId(_ keyId: String) throws {
    guard MajuAppAttestKeyId.isValid(keyId) else {
      throw MajuDevPushEnrollmentError.invalidAppAttestKeyId
    }
...
protocol MajuDCAppAttestServicing {
...
extension DCAppAttestService: MajuDCAppAttestServicing {}

struct MajuDCAppAttestProvider: MajuDevAppAttesting {
  private let service: MajuDCAppAttestServicing
  private let keyIdStore: MajuAppAttestKeyIdStoring

  init(
    service: MajuDCAppAttestServicing = DCAppAttestService.shared,
    keyIdStore: MajuAppAttestKeyIdStoring
  ) {
  ...
  func prepareAttestation() async throws -> MajuDevAttestation {
    ...
    return MajuDevAttestation(keyId: keyId, attestation: "")
  }

  func attestation(
    _ prepared: MajuDevAttestation,
    clientData: Data
  ) async throws -> MajuDevAttestation {
    precondition(!clientData.isEmpty, "Enrollment client data must not be empty")
    try requireSupportedService()
    guard MajuAppAttestKeyId.isValid(prepared.keyId),
...
public final class MajuDevPushEnrollmentDriver {
  ...
  public convenience init(
    gatewayBaseURL: URL,
    store: MajuPushEndpointGrantStore,
    ...
    try self.init(
      ...
      store: store,
      session: session,
      appAttest: MajuDCAppAttestProvider(
        keyIdStore: MajuAppAttestKeyIdKeychainStore(
          accessGroup: appAttestKeychainAccessGroup
        )
      ),
      now: Date.init,
      lifetimeSeconds: 2_592_000,
      installationIdBytes: { try MajuSecureRandom.bytes(count: 16) }
    )
  }
  ...
  init(
    ...
    store: MajuPushEndpointGrantStore,
    session: URLSession,
    appAttest: MajuDevAppAttesting,
    now: @escaping () -> Date,
    lifetimeSeconds: Int64,
    installationIdBytes: @escaping () throws -> Data = {
      try MajuSecureRandom.bytes(count: 16)
    }
  ) throws {
    guard Self.isHTTPOrigin(gatewayBaseURL), lifetimeSeconds > 0 else {
      throw MajuDevPushEnrollmentError.invalidGatewayURL
    }
  ...
  public func endpointGrants() throws -> [MajuPushEndpointGrantRecord] {
    try store.records()
  }
  ...
  public func enroll(
    deviceToken: Data,
    relayURL: URL
  ) async throws -> MajuPushEndpointGrantRecord {
      ...
      current.expiresAt > nowSeconds + 300
    {
      ...
      let refreshed = MajuPushEndpointGrantRecord(
    ...
    // to a Maju community. A second origin served by the same relay therefore
    ...
    {
      let record = MajuPushEndpointGrantRecord(
        relayOrigin: relayOrigin.text,
    ...
    var pending: MajuPushPendingEnrollmentRecord
      ...
      let existing = UUID(uuidString: handle)
    {
      let expiresAt =
        ...
        : renewedExpiration
      pending = MajuPushPendingEnrollmentRecord(
        relayOrigin: relayOrigin.text,
        relayPubkey: relayPubkey,
        endpointHash: endpointHash,
        appProfile: Self.appProfile,
      ...
      let enrollmentClientData = try MajuPushTranscript.enroll(
      ...
      guard attestation.keyId == preparedAttestation.keyId else {
        throw MajuDevPushEnrollmentError.invalidResponse(route: "development attestation")
      }
      pending = MajuPushPendingEnrollmentRecord(
        relayOrigin: relayOrigin.text,
        relayPubkey: relayPubkey,
      ...
      handle == existing.uuidString.lowercased()
    {
      ...
      guard let challengeId = pending.challengeId,
        ...
        throw MajuDevPushEnrollmentError.invalidResponse(
      ...
      do {
        installation = try await enrollInstallation(
          ...
          attestation: MajuDevAttestation(keyId: keyId, attestation: attestation)
        )
      } catch MajuDevPushEnrollmentError.unexpectedStatus(
    ...
    if generationBase > 0 {
      ...
      guard !overflow, next > 0 else {
        throw MajuDevPushEnrollmentError.generationExhausted
      }
      generation = next
    } else {
      generation = 1
    ...
    let delegationClientData = try MajuPushTranscript.delegate(
  ...
  private func challenge() async throws -> Challenge {
    ...
    guard let id = UUID(uuidString: response.challengeId),
      ...
      throw MajuDevPushEnrollmentError.invalidResponse(route: "v1/installations/challenges")
    }
  ...
  private func enrollInstallation(
    ...
    endpoint: String,
    expiresAt: Int64,
    attestation: MajuDevAttestation
  ) async throws -> UUID {
    ...
    guard let installation = UUID(uuidString: response.installationHandle),
      ...
      throw MajuDevPushEnrollmentError.invalidResponse(route: "v1/installations")
    }
    return installation
  }
  ...
  private func delegate(
    ...
    guard !response.endpointGrant.isEmpty, response.endpointGrant.utf8.count <= 4_096 else {
      throw MajuDevPushEnrollmentError.invalidResponse(route: "v1/delegations")
    }
  ...
  private func fetchCurrentRelayKeys(from relayOrigin: URL) async throws -> RelayKeys {
    ...
    do {
      ...
      throw MajuDevPushEnrollmentError.invalidRelayDescriptor
    }
  ...
  private func post<Request: Encodable, Response: Decodable>(
    ...
    do {
      ...
      throw MajuDevPushEnrollmentError.invalidResponse(route: route)
    }
  }
  ...
  private static func expectStatus(
    ...
    guard http.statusCode == expected else {
      ...
      throw MajuDevPushEnrollmentError.unexpectedStatus(
  ...
  private static func relayOrigin(_ url: URL) throws -> (url: URL, text: String) {
    guard url.scheme == "ws" || url.scheme == "wss",
      ...
      throw MajuDevPushEnrollmentError.invalidRelayURL
    }
