...
public struct MajuPushCachedProfile: Codable, Equatable, Sendable {
...
public struct MajuPushCachedChannel: Codable, Equatable, Sendable {
  public let communityID: String
  public let relayOrigin: String
  public let channelID: String
  public let relayMetadataPubkey: String
  public let displayName: String?
  /// Relay-verified Maju channel type, when recognized.
...
public struct MajuPushPresentationCacheSnapshot: Codable, Equatable, Sendable {
  public static let currentVersion = 1

  public let version: Int
  public var communities: [PushLeaseCommunity]
  public var profiles: [MajuPushCachedProfile]
  public var channels: [MajuPushCachedChannel]

  public init(
    version: Int = currentVersion,
    communities: [PushLeaseCommunity] = [],
    profiles: [MajuPushCachedProfile] = [],
    channels: [MajuPushCachedChannel] = []
  ) {
...
public struct MajuPushProfileCacheUpdate: Sendable {
...
public final class MajuPushPresentationCacheStore: @unchecked Sendable {
  ...
  public func updateProfiles(
    communityID: String,
    relayOrigin: String,
    updates: [MajuPushProfileCacheUpdate]
  ) throws -> Set<String> {
    ...
    for update in updates {
      ...
      let entry = MajuPushCachedProfile(
  ...
  public func updateChannels(
    ...
    for event in metadataEvents {
      ...
      let entry = MajuPushCachedChannel(
        communityID: communityID,
        relayOrigin: canonicalRelayOrigin,
        channelID: channelID,
    ...
    for event in membershipEvents {
      ...
      snapshot.channels[index] = MajuPushCachedChannel(
  ...
  public func updateAvatar(
      ...
      && snapshot.profiles[index].avatarPNG != normalizedPNG
    {
      ...
      snapshot.profiles[index] = MajuPushCachedProfile(
  ...
  private func loadLocked() -> MajuPushPresentationCacheSnapshot {
    guard let values = try? fileURL.resourceValues(forKeys: [.fileSizeKey]),
      let fileSize = values.fileSize,
      fileSize <= Self.maximumSnapshotBytes
    else { return MajuPushPresentationCacheSnapshot() }
    return MajuPushPresentationCacheSnapshot.decode(try? Data(contentsOf: fileURL))
  }

  private func writeLocked(_ snapshot: MajuPushPresentationCacheSnapshot) throws {
  ...
  static func encodedBoundedSnapshot(
    _ snapshot: MajuPushPresentationCacheSnapshot
  ) throws -> Data {
  ...
  static func enforceBounds(_ snapshot: inout MajuPushPresentationCacheSnapshot) {
  ...
  private static func enforceChannelCountBound(
    _ snapshot: inout MajuPushPresentationCacheSnapshot
  ) {
  ...
  private static func removingAvatar(
    from profile: MajuPushCachedProfile
  ) -> MajuPushCachedProfile {
    MajuPushCachedProfile(
      communityID: profile.communityID,
      relayOrigin: profile.relayOrigin,
  ...
  private static func removingMemberDigests(
    from channel: MajuPushCachedChannel
  ) -> MajuPushCachedChannel {
    MajuPushCachedChannel(
  ...
  private static func removeOldestEntries(
    _ count: Int,
    from snapshot: inout MajuPushPresentationCacheSnapshot
  ) {
    for _ in 0..<count {
  ...
  private static func channelLastCachedAt(_ channel: MajuPushCachedChannel) -> Int {
...
public enum MajuPushPresentationIdentity {
  ...
  private static func scoped(namespace: String, values: [String]) -> String {
    ...
    return "maju.\(namespace).\(VerifiedNostrEvent.hex(SHA256.hash(data: encoded)))"
  }
}
