...
public struct MajuPushNavigationTarget: Codable, Equatable, Sendable {
  public static let userInfoKey = "maju_push_navigation"
  ...
  public static func decodeIfPresent(
    ...
    return MajuPushNavigationTarget(
      eventID: eventID,
...
public final class MajuPushNavigationBuffer: @unchecked Sendable {
  private let lock = NSLock()
  private var target: MajuPushNavigationTarget?

  public init() {}

  public func record(_ target: MajuPushNavigationTarget) {
    lock.lock()
    self.target = target
    lock.unlock()
  }

  public func peek() -> MajuPushNavigationTarget? {
    lock.lock()
    defer { lock.unlock() }
    return target
  }

  public func take() -> MajuPushNavigationTarget? {
    lock.lock()
    defer { lock.unlock() }
    let current = target
    target = nil
  ...
  public func remove(ifMatching expected: MajuPushNavigationTarget) {
    lock.lock()
    defer { lock.unlock() }
    if target == expected {
      target = nil
