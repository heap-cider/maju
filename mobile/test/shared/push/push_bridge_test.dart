...
import 'package:maju/shared/relay/relay_provider.dart';
import 'package:maju/shared/relay/app_lifecycle_provider.dart';
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await startMajuPushRegistration();
    },
  );
  ...
  test('reads native notification authorization status', () async {
    ...
    expect(
      await readMajuPushAuthorizationStatus(),
  ...
  test('opens native notification settings', () async {
    ...
    expect(await openMajuPushNotificationSettings(), isTrue);
  });
  ...
  test('refreshes not-determined permission to denied on resume', () async {
    ...
    final container = ProviderContainer(
      overrides: [
        ...
        majuPushAuthorizationStatusReaderProvider.overrideWithValue(
    ...
    expect(
      await container.read(majuPushAuthorizationStatusProvider.future),
      MajuPushAuthorizationStatus.notDetermined,
    );
    final lifecycle =
    ...
    await _waitForAuthorization(container, MajuPushAuthorizationStatus.denied);
  });
  ...
  test('reads and exposes persisted endpoint grants on iOS', () async {
    ...
    final grants = await readMajuPushEndpointGrants();
  ...
  test(
    ...
    () async {
      ...
      final firstGrant = await enrollMajuPush(
        'wss://relay.example/',
        'https://gateway-one.example/',
      );
      final secondGrant = await enrollMajuPush(
        'wss://relay.example/',
...
Future<void> _waitForAuthorization(
  ...
  MajuPushAuthorizationStatus expected,
) async {
  for (var attempt = 0; attempt < 20; attempt++) {
    if (container.read(majuPushAuthorizationStatusProvider).value == expected) {
      return;
    }
