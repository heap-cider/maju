...
import 'package:maju/shared/push/push_lease_revocation_outbox.dart';
...
void main() {
  ...
  late MajuPushLeaseRevocationStorage storage;
  late _Clock clock;
  ...
  setUp(() {
    ...
    storage = MajuPushLeaseRevocationStorage(secure: secure);
  ...
  test('community removal journals one precise lease address', () async {
    ...
    final community =
        Community.create(
          ...
          pushSubscriptionState: const MajuPushLeaseSubscriptionState.desired()
...
MajuPushLeaseRevocationOutbox _outbox({
  required MajuPushLeaseRevocationStorage storage,
  required _Clock clock,
  required _WakeScheduler scheduler,
  required MajuPushLeaseRevocationPublisher publisher,
...
MajuPushLeaseRevocationRecord _record({
...
MajuPushEndpointGrant _grant({required int expiresAt}) => MajuPushEndpointGrant(
  relayOrigin: 'wss://relay.example',
  relayPubkey: 'a' * 64,
  installationId: '0' * 32,
