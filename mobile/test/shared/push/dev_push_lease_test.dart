...
import 'package:maju/shared/auth/auth_provider.dart';
import 'package:maju/shared/crypto/nip44.dart';
import 'package:maju/shared/push/dev_push_lease.dart';
import 'package:maju/shared/push/push_bridge.dart';
import 'package:maju/shared/push/push_subscription.dart';
import 'package:maju/shared/relay/nostr_models.dart';
import 'package:maju/shared/relay/relay_session.dart';
import 'package:maju/shared/relay/relay_socket.dart';
import 'package:maju/shared/relay/signed_event_relay.dart';
...
void main() {
  ...
  test('publishes strict kind-30350 lease and waits for accepted OK', () async {
    ...
    final publication = await publishMajuDevPushLease(
      grant: grant,
      leaseGeneration: 7,
      ...
      subscriptions: [
        MajuPushSubscription(
          filter: MajuPushFilter(kinds: const [9], pTags: [signer.public]),
    ...
    expect(submitted!['kind'], majuPushLeaseKind);
  ...
  test(
    ...
    () async {
      ...
      expect(events.map((event) => event.kind), [40002, majuPushLeaseKind]);
  ...
  test('publishes a minimal higher-generation inactive tombstone', () async {
    Map<String, dynamic>? submitted;
    final publication = await publishMajuPushLeaseTombstone(
      descriptor: descriptor,
  ...
  test('descriptor rejects canonical origin with a trailing slash', () {
    ...
    expect(
      () => MajuPushLeaseDescriptor.fromRelayInformation(information),
...
MajuPushLeaseDescriptor _descriptor(String relayPubkey) =>
    MajuPushLeaseDescriptor.fromRelayInformation(_descriptorJson(relayPubkey));
...
MajuPushEndpointGrant _grant(String relayPubkey) => MajuPushEndpointGrant(
