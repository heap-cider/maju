...
void main() {
  ...
  group('themeGroups', () {
    ...
    test('Maju is first and remaining themes are sorted by display name', () {
      final groups = themeGroups();

      expect(groups.paired.first.name, majuThemeName);
      expect(groups.light.first.name, majuThemeName);
      expect(groups.dark.first.name, majuDarkThemeName);
  ...
  group('resolveSchemes', () {
    ...
    test('dark mode falls back to the default pair when pick is unpaired', () {
      ...
      expect(resolved.dark, generateColorScheme(findTheme(majuDarkThemeName)!));
    });
    ...
    test('light mode falls back to the default pair when pick is unpaired', () {
      ...
      expect(resolved.light, generateColorScheme(findTheme(majuThemeName)!));
    });
