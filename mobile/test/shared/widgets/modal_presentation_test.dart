import 'package:maju/shared/widgets/concentric_sheet_surface.dart';
import 'package:maju/shared/theme/theme.dart';
import 'package:maju/shared/widgets/modal_presentation.dart';
...
void main() {
  Border sheetHeaderBorder(WidgetTester tester) {
    final decoration =
        tester
                .widget<DecoratedBox>(
                  find.byKey(const ValueKey('maju-sheet-scroll-divider')),
                )
                .decoration
            as BoxDecoration;
  ...
  testWidgets(
    ...
    (tester) async {
      debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
      const surfaceChannel = MethodChannel('maju/concentric_sheet_surface');
  ...
  testWidgets('native surface colors follow live theme changes', (
    ...
    debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
    const supportChannel = MethodChannel('maju/concentric_sheet_surface');
    const viewChannel = MethodChannel('maju/concentric_sheet_surface/42');
  ...
  testWidgets('native glass surfaces receive concentric composer geometry', (
    ...
    debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
    const supportChannel = MethodChannel('maju/concentric_sheet_surface');
    const viewChannel = MethodChannel('maju/concentric_sheet_surface/84');
    ...
    try {
          ...
          home: Scaffold(
            body: Builder(
              builder: (context) => FilledButton(
                onPressed: () => showMajuModalBottomSheet<void>(
                  context: context,
                  title: 'Members',
      ...
      final nativeSurface = tester.widget<UiKitView>(
        find.byWidgetPredicate(
          (widget) =>
              widget is UiKitView &&
              widget.viewType == 'maju/concentric_sheet_surface',
        ),
      );
      expect(
        nativeSurface.creationParams,
      ...
      expect(
        find.byKey(const ValueKey('maju-sheet-header-gradient')),
        findsNothing,
      );
      expect(
        find.byKey(const ValueKey('maju-sheet-surface-clip')),
        findsNothing,
      );
      final contentClip = tester.widget<ClipRSuperellipse>(
        find.byKey(const ValueKey('concentric-sheet-content-clip')),
      ...
      final nativeClose = tester.widget<UiKitView>(
        find.byWidgetPredicate(
          (widget) =>
              widget is UiKitView && widget.viewType == 'maju/navigation_glass',
        ),
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      try {
        ...
        final title = find.byKey(const ValueKey('maju-sheet-title'));
        ...
        expect(
          tester
              .widget<ColoredBox>(
                find.byKey(const ValueKey('maju-sheet-surface')),
              )
              .color,
        ...
        expect(
          tester.getTopLeft(find.text('Sheet body')).dy -
              tester
                  .getTopLeft(find.byKey(const ValueKey('maju-sheet-surface')))
                  .dy,
          80,
        );
        ...
        expect(
          find.byKey(const ValueKey('maju-sheet-drag-handle')),
          findsOneWidget,
        );
        expect(
  ...
  testWidgets('iOS paints the drag handle inside the concentric surface', (
    ...
    try {
      ...
      final internalHandle = find.byKey(
        const ValueKey('maju-sheet-drag-handle'),
      );
