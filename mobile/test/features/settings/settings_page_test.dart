...
import 'package:maju/shared/community/community_membership_provider.dart';
...
import 'package:maju/shared/widgets/app_list.dart';
...
void main() {
      ...
      ProviderScope(
        overrides: [
          ...
          majuPushAuthorizationStatusReaderProvider.overrideWithValue(
            () async => MajuPushAuthorizationStatus.authorized,
          ),
        ],
        child: MaterialApp(
  ...
  testWidgets('shows denied display permission and opens iOS settings', (
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          appLifecycleProvider.overrideWith(_SettingsLifecycleNotifier.new),
          majuPushAuthorizationStatusReaderProvider.overrideWithValue(
            () async => MajuPushAuthorizationStatus.denied,
          ),
          majuPushNotificationSettingsOpenerProvider.overrideWithValue(
            () async {
              openSettingsCalls += 1;
              return true;
            },
    ...
    expect(tester.widget<Switch>(find.byType(Switch)).value, isTrue);
    expect(
      find.text('Enabled in Maju, but disabled in iOS Settings'),
      findsOneWidget,
    );
    await tester.tap(
  ...
  testWidgets('shows permission lookup errors with settings recovery', (
    ...
    expect(
      find.text('Enabled in Maju; iOS permission status unavailable'),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey('push-notifications-open-settings')),
  ...
  testWidgets('uses the native glass close control on iOS', (tester) async {
    ...
    expect(nativeClose.viewType, 'maju/navigation_glass');
    expect(nativeClose.creationParams, containsPair('icon', 'close'));
    expect(nativeEdit.viewType, 'maju/navigation_glass');
