  ...
  group('ThemePickerPage', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.byKey(const ValueKey('theme-preview-name-maju')),
          findsOneWidget,
        );
        ...
        final previewBounds = tester.getRect(
          find.byKey(const ValueKey('theme-device-pair-preview-maju')),
        );
        ...
        expect(
          tester
              .widget<FittedBox>(
                find.byKey(const ValueKey('theme-full-preview-maju')),
              )
              .alignment,
          Alignment.center,
        );
    ...
    testWidgets('keeps the theme name inside the card in landscape', (
      ...
      final cardBounds = tester.getRect(
        find.byKey(const ValueKey('theme-preview-page-maju')),
      );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final chatFrame = tester.widget<Container>(
          find
              .descendant(
                of: find.byKey(const ValueKey('theme-full-chat-maju')),
    ...
    testWidgets('theme name trails the preview during a swipe', (tester) async {
      ...
      final titleMotion = tester.widget<Transform>(
        find.byKey(const ValueKey('theme-preview-name-motion-maju')),
      );
    ...
    testWidgets('matches the updated Figma Home skeleton', (tester) async {
      ...
      final home = tester.getRect(
        find.byKey(const ValueKey('theme-full-home-maju')),
      );
    ...
    testWidgets('keeps the chat header left aligned and accents only Send', (
      ...
      final chatFrame = tester.getRect(
        find.byKey(const ValueKey('theme-full-chat-maju')),
      );
      expect(
    ...
    testWidgets('uses native liquid-glass pagination on iOS', (tester) async {
      ...
      final nativePagination = tester.widget<UiKitView>(
        find.byWidgetPredicate(
          (widget) =>
              widget is UiKitView &&
              widget.viewType == 'maju/theme_pagination_glass',
        ),
      );
    ...
    testWidgets('uses matching native glass bottom actions on iOS', (
      ...
      final nativeActions = tester
          ...
          .where((view) => view.viewType == 'maju/navigation_glass');
    ...
    testWidgets('swipes between drafts and saves only when Set is pressed', (
      ...
      final instance = await _pumpPicker(
        tester,
        prefs: {'maju_theme_mode': 'system', 'maju_color_scheme': 'maju'},
      );

      await _swipeToNextTheme(tester);
      expect(instance.getString('maju_color_scheme'), 'maju');
      expect(
        find.byKey(const ValueKey('theme-preview-name-maju')),
        findsNothing,
      );
      ...
      await tester.pumpAndSettle();
      expect(instance.getString('maju_color_scheme'), isNot('maju'));
    });
    ...
    testWidgets('preserves the stored accent when leaving Maju', (
      ...
      final instance = await _pumpPicker(
        tester,
        prefs: {
          'maju_theme_mode': 'system',
          'maju_color_scheme': 'maju',
          'maju_accent_color': legacyGreen,
        },
      );
      ...
      expect(instance.getString('maju_color_scheme'), isNot('maju'));
      expect(instance.getInt('maju_accent_color'), green);
    });
    ...
    testWidgets('scrubs directly across the pagination control', (
      ...
      final instance = await _pumpPicker(
        tester,
        prefs: {
          'maju_theme_mode': 'dark',
          'maju_color_scheme': 'catppuccin-macchiato',
        },
      );
      final pageView = tester.widget<PageView>(
        find.byKey(const ValueKey('theme-preview-pages')),
      ...
      expect(instance.getString('maju_color_scheme'), 'catppuccin-macchiato');
    });
    ...
    testWidgets('cycles appearance inline with scale and opacity motion', (
      ...
      await tester.pumpAndSettle();
      expect(instance.getString('maju_theme_mode'), 'light');
      await tester.tap(find.byKey(const ValueKey('theme-preview-set')));
      await tester.pumpAndSettle();
      expect(instance.getString('maju_theme_mode'), 'dark');
    });
    ...
    testWidgets('accent remains a sheet and is committed with Set', (
      ...
      final storedBefore = instance.getInt('maju_accent_color');
      await tester.tap(
        find.byKey(const ValueKey('theme-preview-accent-action-button')),
      );
      await tester.pumpAndSettle();
      ...
      await tester.pumpAndSettle();
      expect(instance.getInt('maju_accent_color'), storedBefore);
    ...
    testWidgets('Maju keeps the accent action unavailable', (tester) async {
      await _pumpPicker(tester, prefs: {'maju_color_scheme': 'maju'});
      expect(
        find.byKey(const ValueKey('theme-preview-accent-action-button')),
        findsNothing,
      );
