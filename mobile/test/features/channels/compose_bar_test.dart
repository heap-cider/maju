...
import 'package:maju/features/channels/compose_bar.dart';
import 'package:maju/features/channels/channels_provider.dart';
import 'package:maju/features/channels/photo_library.dart';
import 'package:maju/features/channels/voice_note_play_pause_icon.dart';
...
import 'package:maju/shared/custom_emoji/custom_emoji.dart';
import 'package:maju/shared/custom_emoji/custom_emoji_provider.dart';
...
import 'package:maju/shared/widgets/mobile_tab_footer_backdrop.dart';
...
const _mediaUploadPlatformChannel = MethodChannel('maju/media_upload');
const _nativeAttachmentPopoverChannel = MethodChannel(
  'maju/native_attachment_popover',
);
...
void main() {
  ...
  group('ComposeBar', () {
    ...
    testWidgets('renders all five permalink types as composer chips', (
      ...
      expect(
        find.byKey(
          const ValueKey('composer-maju-link-chip:engineering · cdcdcdcd'),
        ),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('composer-maju-link-chip:engineering')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('composer-maju-link-chip:maju')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('composer-maju-link-chip:maju · cdcdcdcd')),
        findsNWidgets(2),
      );
      expect(
        tester.widget<TextField>(find.byType(TextField)).controller!.text,
    ...
    testWidgets('preserves underscore d-tags and Markdown delimiters', (
      ...
      final owner = 'ab' * 32;
      final url = 'maju://repo?owner=$owner&d=my_repo';
      final source = '**$url**';
      await tester.pumpWidget(
        _buildComposeBar(
          uploadService: _testUploadService(nostr.Keys.generate().nsec),
      ...
      expect(
        find.byKey(const ValueKey('composer-maju-link-chip:my_repo')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('composer-maju-link-chip:my')),
        findsNothing,
      );
      expect(
        tester.widget<TextField>(find.byType(TextField)).controller!.text,
