...
import 'package:maju/features/channels/camera_capture_cleanup.dart';

void main() {
  test('deletes the inline camera file after upload succeeds', () async {
    final file = File(
      '${Directory.systemTemp.path}/maju-camera-${DateTime.now().microsecondsSinceEpoch}.jpg',
    );
  ...
  test('deletes every native picker file after processing', () async {
    ...
    final files = [
      File('${Directory.systemTemp.path}/maju-photo-$suffix-1.jpg'),
      File('${Directory.systemTemp.path}/maju-photo-$suffix-2.jpg'),
    ];
  ...
  test('retains a composer-owned copy before native cleanup', () async {
    final source = File(
      '${Directory.systemTemp.path}/maju-native-${DateTime.now().microsecondsSinceEpoch}.png',
    );
