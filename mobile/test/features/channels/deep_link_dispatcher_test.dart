...
import 'package:maju/features/channels/deep_link_dispatcher.dart';
import 'package:maju/features/invites/invite_join_provider.dart';
...
void main() {
  testWidgets('queues deep links in arrival order until acknowledged', (
    ...
    controller
      ..add(Uri.parse('maju://message?channel=$channelId&id=$firstId'))
      ..add(Uri.parse('maju://message?channel=$channelId&id=$secondId'));
    await tester.pump();
  ...
  testWidgets('prepares an invite once while listeners re-enter', (
    ...
    expect(find.text('Join this Maju community?'), findsOneWidget);
  });
...
class _QueuedPendingDeepLinkNotifier extends PendingDeepLinkNotifier {
  _QueuedPendingDeepLinkNotifier(List<MajuDeepLink> links)
    : _links = List.of(links);

  final List<MajuDeepLink> _links;
  int consumeCalls = 0;

  MajuDeepLink? get _firstOrNull => _links.isEmpty ? null : _links.first;
  MajuDeepLink? get current => _firstOrNull;

  @override
  MajuDeepLink? build() => _firstOrNull;
...
class _RecordingPendingDeepLinkNotifier extends PendingDeepLinkNotifier {
  ...
  final MajuDeepLink link;
  int consumeCalls = 0;

  @override
  MajuDeepLink? build() => link;
