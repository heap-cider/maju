...
import 'package:maju/features/channels/recent_emoji_provider.dart';
...
const _relayCustomEmoji = [
  CustomEmoji(
    shortcode: 'majubee',
    url: 'https://relay.example/media/majubee.png',
  ),
];
...
const _nativeEmojiPickerChannel = MethodChannel('maju/native_emoji_picker');
...
void main() {
  group('EmojiPickerSheet', () {
    ...
    testWidgets('a non-reaction selection does not record a quick reaction', (
      ...
      expect(
        prefs.getString(
          'maju.quick-reaction-emojis.v1:http://localhost:3000:self',
        ),
        isNull,
      );
    });
    ...
    testWidgets('passes shared theme and custom emoji into the native sheet', (
      ...
      try {
        ...
        final relayHeaders = await _sendNativeEmojiPickerCall(
          tester,
          'mediaHeaders',
          'https://relay.example/media/majubee.png',
        );
        expect(
        ...
        expect(selected, ['\u{1F525}']);
        expect(dismissals, 1);
        expect(prefs.getInt('maju.emoji-picker.skin-tone.v1'), 4);
      } finally {
