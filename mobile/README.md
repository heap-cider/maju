...
(`xyz.block.maju.dogfood.mobile.<slug>` on iOS,
...
and profile builds keep the production `Maju` name and application ID.
...
For an Android debug build that must remain installed alongside other Maju
...
`xyz.block.maju.mobile.huddles_829c` with the launcher label `Maju Huddles`.
...
descriptor, Maju independently requests display permission and registers with
...
`MAJU_PUSH_ENABLED=true` advertise the descriptor and process push. See
...
```xcconfig
BUNDLE_IDENTIFIER = xyz.block.maju.mobile
MAJU_DEVELOPMENT_TEAM = EYF346PHUG
MAJU_IOS_PUSH_ENVIRONMENT = development
MAJU_APP_ATTEST_ENVIRONMENT = development
```
