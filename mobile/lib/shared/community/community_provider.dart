...
final communitySnapshotWriterProvider = Provider<CommunitySnapshotWriter>((
  ...
  return registerMajuPushCommunitySnapshot;
});
...
final communityPushLeaseRevocationEnqueuerProvider =
    Provider<CommunityPushLeaseRevocationEnqueuer>((ref) {
      return ref.read(majuPushLeaseRevocationOutboxProvider).enqueueCommunity;
    });
...
final communityPushLeaseRevocationTriggerProvider =
    Provider<CommunityPushLeaseRevocationTrigger>((ref) {
      return ref.read(majuPushLeaseRevocationOutboxProvider).trigger;
    });
...
Future<void> _deactivateCommunityPushLease(
  ...
  final descriptor = await fetchMajuPushLeaseDescriptor(community.relayUrl);
  final matchingGrant = (await readMajuPushEndpointGrants())
      .where(
        (grant) =>
    ...
    final fingerprint = communities
        .map(
          (community) => [
            ...
            majuPushSubscriptionStateFingerprint(
...
class CommunityListNotifier extends AsyncNotifier<List<Community>> {
  ...
  Future<void> updateDesiredPushSubscriptions(
    String id,
    List<MajuPushSubscription> desired,
    ...
    if (majuPushSubscriptionsFingerprint(
          community.pushSubscriptionState.desired,
        ) ==
        majuPushSubscriptionsFingerprint(desired)) {
      return;
    }
  ...
  Future<void> setPushNotificationsEnabled(String id, bool enabled) async {
    ...
    if (shouldDeactivate) {
      try {
        ...
        // The durable journal remains pending. MajuPushBootstrap retries it
