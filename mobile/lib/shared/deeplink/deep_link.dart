/// Parsing for `maju://` deep links.
///
/// Mirrors the desktop handler in `desktop/src-tauri/src/deep_link.rs`:
/// `maju://message?channel=<uuid>&id=<hex>[&thread=<hex>]` references a
...
/// `maju://join?relay=<ws(s)://relay>&code=<code>` form is only an installed-app
/// handoff from the web landing page.
class InviteDeepLink extends MajuDeepLink {
...
///
/// Canonical form: `maju://channel/<channel-uuid>`.
class ChannelDeepLink extends MajuDeepLink {
...
/// A parsed `maju://message` deep link.
class MessageDeepLink extends MajuDeepLink {
...
/// Build a canonical `maju://message` link for a channel message.
///
...
/// Parse a canonical `maju://channel/<channel-uuid>` URI.
///
...
ChannelDeepLink? parseChannelDeepLink(Uri uri) {
  if (uri.scheme != 'maju' || uri.host != 'channel') return null;
  if (uri.hasQuery ||
      uri.hasFragment ||
...
/// Parse a `maju://message?…` URI into a [MessageDeepLink].
///
...
MessageDeepLink? parseMessageDeepLink(Uri uri) {
  if (uri.scheme != 'maju' || uri.host != 'message') return null;
...
/// Parse canonical HTTPS invite links and `maju://join` app handoffs.
///
/// Accepted forms:
/// - `https://<relay>/invite/<code>` -> `wss://<relay>` + code
/// - `http://localhost/invite/<code>` -> `ws://localhost` + code in debug builds
/// - `maju://join?relay=<wss://relay>&code=<code>` -> relay + code
/// - `maju://join?relay=<ws://localhost>&code=<code>` -> local relay in debug
///
...
/// Parse any supported Maju deep link.
MajuDeepLink? parseMajuDeepLink(Uri uri) =>
    parseInviteDeepLink(uri) ??
    parseChannelDeepLink(uri) ??
    parseMessageDeepLink(uri);

/// A validated Maju repository, pull request, or issue permalink.
class EntityDeepLink extends MajuDeepLink {
  final String type;
  final String owner;
...
/// Parse canonical `maju://repo|pr|issue` permalinks for inline presentation.
EntityDeepLink? parseEntityDeepLink(Uri uri) {
  if (uri.scheme != 'maju' || !{'repo', 'pr', 'issue'}.contains(uri.host)) {
    return null;
  }
  if (uri.path.isNotEmpty ||
      uri.hasFragment ||
