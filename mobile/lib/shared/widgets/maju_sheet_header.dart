...
class MajuSheetHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: SizedBox(
        height: majuNavigationRowHeight,
        child: Stack(
            ...
            if (title case final title?)
              Positioned(
                ...
                child: Center(
                  child: Text(
                    title,
                    key: titleKey ?? const ValueKey('maju-sheet-title'),
                    maxLines: 1,
  ...
  Widget build(BuildContext context) {
    ...
    if (Theme.of(context).platform == TargetPlatform.iOS) {
      return IosGlassNavigationButton(
        key: const ValueKey('maju-sheet-ios-glass-close'),
        icon: IosGlassNavigationIcon.close,
        semanticLabel: 'Close sheet',
        onPressed: closeSheet,
        width: majuNavigationActionSize,
        height: majuNavigationActionSize,
        foregroundColor: context.colors.primary,
      );
    }

    return SizedBox.square(
      dimension: majuNavigationActionSize,
      child: IconButton(
        tooltip: 'Close sheet',
        onPressed: closeSheet,
