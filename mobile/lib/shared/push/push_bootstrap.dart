...
@visibleForTesting
class MajuPushAttemptGate {
  MajuPushAttemptGate({this.retryDelay = _pushBootstrapRetryDelay});
...
@visibleForTesting
String majuPushPublicationAttemptKey({
  ...
  majuPushSubscriptionsFingerprint(subscriptions),
].join('|');

@visibleForTesting
bool majuPushLifecycleEnabled({
  required Community? community,
  required MajuPushLeaseDescriptor? descriptor,
}) => community?.pushNotificationsEnabled == true && descriptor != null;

@visibleForTesting
Future<int> publishMajuPushLeaseRecoverably({
...
class MajuPushBootstrap extends HookConsumerWidget {
  const MajuPushBootstrap({required this.child, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final registrationAttempt = useMemoized(MajuPushAttemptGate.new);
    final publicationAttempt = useMemoized(MajuPushAttemptGate.new);
    final tombstoneAttempt = useMemoized(MajuPushAttemptGate.new);
    final registrationRetry = useState(0);
    final publicationRetry = useState(0);
    final tombstoneRetry = useState(0);
    final revocationOutbox = ref.watch(majuPushLeaseRevocationOutboxProvider);
        ...
        unawaited(() async {
          try {
            await startMajuPushRegistrationIfCapable(
              activeDescriptor,
              startRegistration: startMajuPushRegistration,
            );
          } catch (error, stack) {
    ...
    useEffect(
      () {
        ...
        final attempt = majuPushPublicationAttemptKey(
  ...
  static Future<MajuPushEndpointGrant> _publish(
    WidgetRef ref,
    RelayConfig config,
    Community community,
    String memberPubkey,
    ...
    final descriptor = await fetchMajuPushLeaseDescriptor(config.baseUrl);
    final grant = await enrollMajuPush(
      config.wsUrl,
      Env.pushGatewayUrl,
    ...
    await publishMajuPushLeaseRecoverably(
      ...
      publish: (leaseGeneration) => publishMajuDevPushLeaseThroughRelay(
        grant: grant,
