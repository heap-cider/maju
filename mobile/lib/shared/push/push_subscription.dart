...
/// User-visible Maju message kinds. This mirrors
/// `EventKind.channelMessageEventKinds` without importing feature code into
/// the shared push layer.
const majuPushEligibleKinds = [9, 40002, 45001, 45003];
const majuPushSelfDirectedKinds = majuPushEligibleKinds;
const majuPushRenderableKinds = majuPushEligibleKinds;
const majuPushChannelKinds = [9];
const majuPushChannelChunkSize = 50;
const majuPushMaxSubscriptions = 16;
const majuPushMaxIgnoreFilters = 8;
const majuPushHellthreadParticipantLimit = 20;
...
enum MajuPushLeaseSubscriptionAuthority { desired, accepted }

class MajuPushFilter {
  ...
  factory MajuPushFilter.fromJson(Map<String, dynamic> json) {
    _rejectUnknownKeys(json, _filterKeys, 'push filter');
    return MajuPushFilter(
  ...
  void _validate() {
    if (kinds.isEmpty ||
        kinds.any((kind) => !majuPushEligibleKinds.contains(kind))) {
...
class MajuPushSuppression {
  final int pTagsMax;

  const MajuPushSuppression({required this.pTagsMax}) : assert(pTagsMax > 0);

  Map<String, dynamic> toJson() => {'p_tags_max': pTagsMax};

  factory MajuPushSuppression.fromJson(Map<String, dynamic> json) {
    ...
    return MajuPushSuppression(pTagsMax: value);
  }
}

class MajuPushSubscription {
  final MajuPushFilter filter;
  final String notificationClass;
  final List<MajuPushFilter> ignore;
  final MajuPushSuppression? suppress;

  MajuPushSubscription({
    required this.filter,
    required this.notificationClass,
    Iterable<MajuPushFilter> ignore = const [],
    this.suppress,
    ...
    if (this.ignore.length > majuPushMaxIgnoreFilters) {
      throw const FormatException(
  ...
  factory MajuPushSubscription.fromJson(Map<String, dynamic> json) {
    ...
    return MajuPushSubscription(
      filter: MajuPushFilter.fromJson(Map<String, dynamic>.from(filter)),
      notificationClass: notificationClass,
      ignore: [
        for (final raw in ignore as List<dynamic>? ?? const [])
          if (raw is Map)
            MajuPushFilter.fromJson(Map<String, dynamic>.from(raw))
          else
            throw const FormatException('Malformed push ignore filter.'),
      ],
      suppress: suppress == null
          ? null
          : MajuPushSuppression.fromJson(Map<String, dynamic>.from(suppress)),
    );
  }
}
...
class MajuPushLeaseSubscriptionState {
  final MajuPushLeaseSubscriptionAuthority authority;
  final List<MajuPushSubscription> desired;
  final List<MajuPushSubscription>? accepted;
  ...
  const MajuPushLeaseSubscriptionState.desired({
    this.desired = const [],
    this.accepted,
    ...
    this.pendingTombstoneGeneration,
       ...
       authority = MajuPushLeaseSubscriptionAuthority.desired;

  MajuPushLeaseSubscriptionState.accepted({
    required Iterable<MajuPushSubscription> desired,
    required Iterable<MajuPushSubscription> acceptedSubscriptions,
    required this.acceptedGeneration,
    this.generationCursor,
    this.pendingTombstoneGeneration,
  }) : authority = MajuPushLeaseSubscriptionAuthority.accepted,
  ...
  List<MajuPushSubscription> get authoritative => switch (authority) {
    MajuPushLeaseSubscriptionAuthority.desired => desired,
    MajuPushLeaseSubscriptionAuthority.accepted => accepted!,
  };

  MajuPushLeaseSubscriptionState withDesired(
    Iterable<MajuPushSubscription> subscriptions,
  ) {
    final updated = List<MajuPushSubscription>.unmodifiable(subscriptions);
    return switch (authority) {
      MajuPushLeaseSubscriptionAuthority.desired =>
        MajuPushLeaseSubscriptionState.desired(
          desired: updated,
          accepted: accepted,
      ...
      MajuPushLeaseSubscriptionAuthority.accepted =>
        MajuPushLeaseSubscriptionState.accepted(
          desired: updated,
  ...
  MajuPushLeaseSubscriptionState withAccepted({
  ...
  MajuPushLeaseSubscriptionState withReservedGeneration(int generation) {
  ...
  MajuPushLeaseSubscriptionState withPendingTombstone(int generation) {
    if (generation <= (generationCursor ?? acceptedGeneration ?? 0)) {
      throw const FormatException(
        'Pending push tombstone generation must advance monotonically.',
      );
    }
    return MajuPushLeaseSubscriptionState.desired(
      desired: desired,
      accepted: accepted,
      acceptedGeneration: acceptedGeneration,
      generationCursor: generation,
  ...
  MajuPushLeaseSubscriptionState withPendingTombstoneAtCursor() {
  ...
  MajuPushLeaseSubscriptionState withAcceptedTombstone(int generation) {
  ...
  factory MajuPushLeaseSubscriptionState.fromJson(Map<String, dynamic> json) {
    _rejectUnknownKeys(json, const {
      'authority',
      'desired',
      'accepted',
    ...
    return switch (authority) {
      'desired' => MajuPushLeaseSubscriptionState.desired(
        desired: desired,
        accepted: accepted,
...
List<MajuPushSubscription> buildDesiredMajuPushSubscriptions({
  ...
  final mutedIgnoreFilters = <MajuPushFilter>[];
  for (final chunk in _chunks(activeMuted, majuPushChannelChunkSize)) {
    mutedIgnoreFilters.add(
      MajuPushFilter(kinds: majuPushChannelKinds, hTags: chunk),
    );
  }
  if (mutedIgnoreFilters.length + 1 > majuPushMaxIgnoreFilters) {
    throw const FormatException('Too many muted channels for a push lease.');
  }

  final selfAuthored = MajuPushFilter(
    kinds: majuPushRenderableKinds,
    authors: [normalizedPubkey],
  );
  final ignores = [selfAuthored, ...mutedIgnoreFilters];
  const suppression = MajuPushSuppression(
    pTagsMax: majuPushHellthreadParticipantLimit,
  );
  final subscriptions = <MajuPushSubscription>[
    MajuPushSubscription(
      filter: MajuPushFilter(
        kinds: majuPushSelfDirectedKinds,
  ...
  final channels = normalizedChannelIds.difference(normalizedMuted).toList()
    ..sort();
  for (final chunk in _chunks(channels, majuPushChannelChunkSize)) {
    subscriptions.add(
      MajuPushSubscription(
        filter: MajuPushFilter(kinds: majuPushChannelKinds, hTags: chunk),
        notificationClass: 'default',
        ignore: ignores,
        suppress: suppression,
      ),
  ...
  if (subscriptions.length > majuPushMaxSubscriptions) {
    throw const FormatException('Too many channels for a push lease.');
  }
  return List.unmodifiable(subscriptions);
}

String majuPushSubscriptionsFingerprint(
  List<MajuPushSubscription> subscriptions,
) => jsonEncode([
  for (final subscription in subscriptions) subscription.toJson(),
]);

String majuPushSubscriptionStateFingerprint(
  MajuPushLeaseSubscriptionState state,
...
List<MajuPushSubscription> _subscriptionList(
  Object? raw,
  String label, {
  ...
  return [
    for (final item in raw)
      if (item is Map)
        MajuPushSubscription.fromJson(Map<String, dynamic>.from(item))
      else
