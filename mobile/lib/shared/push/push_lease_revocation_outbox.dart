...
typedef MajuPushLeaseRevocationPublisher =
    Future<void> Function(MajuPushLeaseRevocationRecord record);
typedef MajuPushLeaseRevocationClock = DateTime Function();
typedef MajuPushLeaseRevocationJitter = double Function();
typedef MajuPushLeaseRevocationErrorReporter =
    void Function(Object error, StackTrace stackTrace);
typedef MajuPushLeaseRevocationWakeScheduler =
...
@immutable
class MajuPushLeaseRevocationRecord {
  ...
  MajuPushLeaseRevocationRecord({
    ...
    if (canonicalMajuPushRelayOrigin(relayUrl) != relayOrigin) {
  ...
  MajuPushLeaseRevocationRecord copyWith({
    int? generation,
    int? expiresAt,
    int? attemptCount,
    int? nextAttemptAt,
  ...
  factory MajuPushLeaseRevocationRecord.fromJson(Map<String, dynamic> json) {
    const keys = {
      'version',
      'relayUrl',
      'relayOrigin',
    ...
    return MajuPushLeaseRevocationRecord(
...
class MajuPushLeaseRevocationStorage {
  static const _key = 'maju_push_lease_revocations_v1';

  final FlutterSecureStorage _secure;

  MajuPushLeaseRevocationStorage({FlutterSecureStorage? secure})
    : _secure = secure ?? const FlutterSecureStorage();

  Future<List<MajuPushLeaseRevocationRecord>> loadAll() async {
    final raw = await _secure.read(key: _key);
    if (raw == null) return <MajuPushLeaseRevocationRecord>[];
    ...
    final records = [
      for (final value in decoded)
        MajuPushLeaseRevocationRecord.fromJson(
  ...
  Future<void> replaceAll(
    Iterable<MajuPushLeaseRevocationRecord> records,
  ) async {
...
/// accepted.
class MajuPushLeaseRevocationOutbox {
  MajuPushLeaseRevocationOutbox({
    required this.storage,
    required this.publisher,
    this.now = DateTime.now,
    MajuPushLeaseRevocationJitter? jitter,
    this.reportError = reportPushLeaseCleanupError,
    MajuPushLeaseRevocationWakeScheduler? scheduleWake,
  }) : jitter = jitter ?? Random.secure().nextDouble,
       scheduleWake = scheduleWake ?? _scheduleTimer;

  final MajuPushLeaseRevocationStorage storage;
  final MajuPushLeaseRevocationPublisher publisher;
  final MajuPushLeaseRevocationClock now;
  final MajuPushLeaseRevocationJitter jitter;
  final MajuPushLeaseRevocationErrorReporter reportError;
  final MajuPushLeaseRevocationWakeScheduler scheduleWake;
  ...
  Future<void> enqueue(MajuPushLeaseRevocationRecord record) async {
  ...
  Future<bool> enqueueCommunity(
    Community community, {
    Future<List<MajuPushEndpointGrant>> Function()? readGrants,
  }) async {
    ...
    final relayUrl = canonicalMajuPushRelayHttpUrl(community.relayUrl);
    final relayOrigin = canonicalMajuPushRelayOrigin(relayUrl);
    final grants = await (readGrants ?? readMajuPushEndpointGrants)();
    final matching = grants
        .where(
          (grant) =>
              grant.relayOrigin == relayOrigin &&
              grant.appProfile == majuDevPushAppProfile,
        )
        .toList();
  ...
  Future<MajuPushLeaseRevocationRecord?> _reserveNextDueAttempt() => _serialize(
    () async {
      final currentSeconds = now().millisecondsSinceEpoch ~/ 1000;
      final records = await storage.loadAll();
  ...
  Future<void> _removeAccepted(MajuPushLeaseRevocationRecord attempted) =>
      _serialize(() async {
        final records = await storage.loadAll();
        records.removeWhere(
          (record) =>
...
String canonicalMajuPushRelayOrigin(String relayUrl) {
  final uri = _majuPushRelayUri(relayUrl);
...
String canonicalMajuPushRelayHttpUrl(String relayUrl) {
  final uri = _majuPushRelayUri(relayUrl);
  final scheme = switch (uri.scheme) {
    'https' || 'wss' => 'https',
    'http' || 'ws' => 'http',
...
Uri _majuPushRelayUri(String relayUrl) {
...
Future<void> publishMajuPushLeaseRevocation(
  MajuPushLeaseRevocationRecord record,
) async {
  final descriptor = await fetchMajuPushLeaseDescriptor(record.relayUrl);
  ...
  final wsUrl = uri
      ...
      .toString();
  await publishMajuPushLeaseTombstone(
...
final majuPushLeaseRevocationStorageProvider =
    Provider<MajuPushLeaseRevocationStorage>(
      (ref) => MajuPushLeaseRevocationStorage(),
    );

final majuPushLeaseRevocationOutboxProvider =
    Provider<MajuPushLeaseRevocationOutbox>((ref) {
      final outbox = MajuPushLeaseRevocationOutbox(
        storage: ref.read(majuPushLeaseRevocationStorageProvider),
        publisher: publishMajuPushLeaseRevocation,
      );
