...
const majuPushLeaseKind = 30350;
const majuDevPushAppProfile = 'buzz-ios-dogfood';
const majuPushTransport = 'apns';
...
class MajuPushLeaseDescriptor {
  ...
  factory MajuPushLeaseDescriptor.fromRelayInformation(
    ...
    for (final profile in profiles) {
      ...
      if (id == majuDevPushAppProfile) transport = candidate;
    }
    if (transport != majuPushTransport) {
      throw const FormatException(
    ...
    if (!majuPushEligibleKinds.every(pushKinds.contains)) {
      throw const FormatException(
        'NIP-11 does not advertise every Maju message kind for push',
      );
    }
    ...
    final supportedClasses = _stringList(
      classSupport[majuPushTransport],
    ...
    return MajuPushLeaseDescriptor(
      origin: origin,
...
Future<MajuPushLeaseDescriptor> fetchMajuPushLeaseDescriptor(
  String relayBaseUrl, {
  http.Client? client,
  ...
  try {
    ...
    return MajuPushLeaseDescriptor.fromRelayInformation(decoded);
  } finally {
    if (client == null) ownedClient.close();
  }
}

class MajuPushLeasePublication {
  final String eventId;
  final int expiration;
  final String plaintext;

  const MajuPushLeasePublication({
...
Future<MajuPushLeasePublication> publishMajuDevPushLease({
  required MajuPushEndpointGrant grant,
  String? leaseInstallationId,
  int? leaseGeneration,
  required MajuPushLeaseDescriptor descriptor,
  required String nsec,
  required String memberPubkey,
  required List<MajuPushSubscription> subscriptions,
  required MajuPushLeaseSubmit submit,
  ...
  return MajuPushLeasePublication(
...
Future<MajuPushLeasePublication> publishMajuDevPushLeaseThroughRelay({
  required MajuPushEndpointGrant grant,
  String? leaseInstallationId,
  int? leaseGeneration,
  required MajuPushLeaseDescriptor descriptor,
...
/// community.
Future<MajuPushLeasePublication> publishMajuPushLeaseTombstone({
  required MajuPushLeaseDescriptor descriptor,
...
Future<MajuPushLeasePublication> publishMajuPushLeaseTombstoneThroughRelay({
  required MajuPushLeaseDescriptor descriptor,
  required String installationId,
  required int generation,
  required String nsec,
...
void _validateGrant(
  ...
  if (grant.appProfile != majuDevPushAppProfile) {
