...
final _majuToolNames = <String>{..._majuReadTools, ..._majuWriteTools};

final _majuToolNamesByLength = _majuToolNames.toList()
  ..sort((a, b) => b.length.compareTo(a.length));

final _majuToolTitleAliases = <(RegExp, String)>[
...
String? _findMajuToolName(String value, bool includeShortNames) {
  final alias = _findMajuToolAlias(value);
  if (alias != null) return alias;

  final normalized = _normalizeToolNameText(value);
  for (final name in _majuToolNamesByLength) {
...
String? _findMajuToolAlias(String value) {
  final normalizedPhrase = value
      .trim()
      .toLowerCase()
      .replaceAll(RegExp(r'[_-]+'), ' ')
      .replaceAll(RegExp(r'\s+'), ' ');
  for (final (pattern, name) in _majuToolTitleAliases) {
...
_parsePromptText(String text) {
  ...
  for (final section in sections) {
    if (section.title.toLowerCase().startsWith('maju event')) {
...
({String title, String toolName, String? majuToolName}) _extractToolIdentity(
  Map<String, dynamic> update,
) {
  final candidates = _collectToolNameCandidates(update);
  String? knownName;
  for (final c in candidates) {
    knownName = _findMajuToolName(c, true);
    if (knownName != null) break;
  }
  knownName ??= _findMajuToolName(_safeJsonEncode(update), false);
...
List<TranscriptItem> buildTranscript(List<ObserverFrame> events) {
  ...
  void upsertTool(
    ...
    final existing = itemsById[id];
    final canonicalMajuToolName =
        majuToolName ?? _findMajuToolName(toolName, true);
    if (existing is ToolItem) {
      if (!_isGenericToolTitle(title)) {
        existing.title = title;
      }
      if (canonicalMajuToolName != null) {
        existing.majuToolName = canonicalMajuToolName;
        existing.toolName = canonicalMajuToolName;
      } else if (existing.majuToolName == null &&
    ...
    final item = ToolItem(
      id: id,
      title: title,
      toolName: canonicalMajuToolName ?? toolName,
      majuToolName: canonicalMajuToolName,
      status: status,
      args: args,
      result: result,
      isError: isError,
