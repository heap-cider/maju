...
class MessageContent extends HookConsumerWidget {
    ...
    final resolvedChannelTap =
        ...
        (String channelId) {
          ref
              ...
              .open(Uri(scheme: 'maju', host: 'channel', path: channelId));
        };
  ...
  Widget _buildLink(
    ...
    final majuLink = uri?.scheme == 'maju'
        ? parseMajuDeepLink(uri!) ?? parseEntityDeepLink(uri)
        : null;
    final isMajuLink =
        majuLink is ChannelDeepLink ||
        majuLink is MessageDeepLink ||
        majuLink is EntityDeepLink;
    final isCanonicalMajuLabel = isMajuLink && text == url;
    final majuPresentation = switch (majuLink) {
    ...
    final linkTextWidget = isCanonicalMajuLabel
        ? Text(
            text,
    ...
    final renderedLink = isCanonicalMajuLabel && majuPresentation != null
        ? _TokenPill(
            key: ValueKey('maju-link-chip:$url'),
            icon: majuPresentation.icon,
            interactive: majuPresentation.interactive,
            semanticLabel: majuPresentation.semanticLabel,
            text: majuPresentation.label,
    ...
    return GestureDetector(
      onTap: () async {
        ...
        // dispatcher.
        if (uri.scheme == 'maju') {
          final deepLink = parseMajuDeepLink(uri);
