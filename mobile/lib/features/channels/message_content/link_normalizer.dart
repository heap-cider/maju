...
final _autolinkPattern = RegExp(
  r'<((?:https?://|maju://(?:message\?|join\?|channel/|(?:pr|issue|repo)\?))[^>]+)>',
);
final _bareLinkPattern = RegExp(
  r'(?<![(\]=])(?:https?://|maju://(?:message\?|join\?|channel/|(?:pr|issue|repo)\?))[^\s)>\]]+',
);
final _trailingPunctuationPattern = RegExp(r'[.,!?:;]+$');
final _backtickRunPattern = RegExp(r'`+');

/// Converts supported Maju and HTTP(S) autolinks and bare links into Markdown
/// links while leaving inline and fenced code untouched. Punctuation peeling
/// is limited to Maju URLs so existing HTTP(S) destinations stay unchanged.
...
String _normalizeBareLink(String segment, Match match) {
  ...
  var url = matched;
  var trailing = '';
  final isMajuUrl = matched.startsWith('maju://');
