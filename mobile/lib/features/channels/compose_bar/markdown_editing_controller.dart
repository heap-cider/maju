...
class _MarkdownEditingController extends TextEditingController {
  ...
  List<InlineSpan>? _buildComposerTokenSpans(
    ...
    final expression = RegExp(
      r'''maju://(?:message\?|channel/|(?:repo|pr|issue)\?)[^\s<>"']+''',
      caseSensitive: false,
    );
  ...
  (IconData, String, String)? _composerLinkPresentation(String raw) {
    ...
    final link = parseMajuDeepLink(uri) ?? parseEntityDeepLink(uri);
    return switch (link) {
...
class _ComposerMajuLinkChip extends StatelessWidget {
  final IconData icon;
  final String label;
  final String semanticLabel;
  final TextStyle textStyle;

  const _ComposerMajuLinkChip({
    required this.icon,
    required this.label,
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: Container(
        key: ValueKey('composer-maju-link-chip:$label'),
