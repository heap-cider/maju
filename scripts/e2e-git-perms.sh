...
# =============================================================================
# Two bots collaborate on a simple web page via the Maju relay's git server.
#
# Prerequisites:
#   - Docker services running (postgres, redis, minio)
#   - Relay built: cargo build --release --bin maju-relay
...
check_deps() {
    ...
    if [[ ! -x "${REPO_ROOT}/target/release/maju-relay" ]]; then
        missing+=("maju-relay (cargo build --release --bin maju-relay)")
    fi
...
export MAJU_GIT_REPO_PATH="${REPO_ROOT}/repos"
export MAJU_GIT_HOOK_HMAC_SECRET="${HMAC_SECRET}"
...
./target/release/maju-relay > /tmp/maju-relay-e2e.log 2>&1 &
RELAY_PID=$!

# Wait for relay to be ready (poll, not sleep)
for i in $(seq 1 "$RELAY_STARTUP_TIMEOUT"); do
    if curl -sf --max-time 2 "${RELAY_HTTP}/" -H "Accept: application/nostr+json" | grep -q "Maju"; then
        break
    fi
    if [[ $i -eq "$RELAY_STARTUP_TIMEOUT" ]]; then
        fail "Relay did not start within ${RELAY_STARTUP_TIMEOUT}s. Check /tmp/maju-relay-e2e.log"
    fi
    sleep 1
done
...
CREATE_REPO=$(send_event "$OWNER_PRIVKEY" "$KIND_CREATE_REPO" "" \
    "[\"d\", \"$REPO_NAME\"], [\"maju-channel\", \"$CHANNEL_ID\"]")
...
<html>
<head>
    <title>Maju E2E Test Page</title>
    <style>
...
<body>
    <h1>🐝 Maju Collaborative Page</h1>
    <p>This page was created by two bots collaborating via Maju's git server.</p>
...
git -C "$BOT1_DIR" -c user.name="Bot1" -c user.email="bot1@maju.test" \
...
if git_push "$BOT1_PRIVKEY" "$BOT1_DIR" -u origin main; then
    ...
    tail -20 /tmp/maju-relay-e2e.log
...
sed -i.bak '/<\/body>/i\
    ...
    <footer>\
        <p><em>Built with Maju sovereign git hosting</em></p>\
    </footer>' "$BOT2_DIR/index.html"
rm -f "$BOT2_DIR/index.html.bak"

git -C "$BOT2_DIR" add -A
git -C "$BOT2_DIR" -c user.name="Bot2" -c user.email="bot2@maju.test" \
...
git -C "$UNSIGNED_DIR" -c user.name="Bot1" -c user.email="bot1@maju.test" \
...
log "Signing with owner attestation (MAJU_AUTH_TAG)..."
...
NOSTR_PRIVATE_KEY="$BOT1_PRIVKEY" MAJU_AUTH_TAG="$OA_TAG" \
git -C "$OA_DIR" \
    -c user.name="Bot1" \
    -c user.email="bot1@maju.test" \
    -c gpg.format=x509 \
...
if echo "$DECODED_SIG" | grep -q '"oa"'; then
    ...
    fail "Owner attestation missing from signature — MAJU_AUTH_TAG not picked up"
fi
...
(
    ...
    git -C "$SYMLINK_DIR" -c user.name="Bot1" -c user.email="bot1@maju.test" \
...
(
    ...
    git -C "$MISSING_DIR" -c user.name="Bot1" -c user.email="bot1@maju.test" \
