...
grep -q '^BUNDLE_IDENTIFIER = xyz\.block\.maju\.dogfood\.mobile\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = Maju (Fix_Thing-2)$' "$ios" \
...
grep -q '^appName=Maju (Fix_Thing-2)$' "$android" \
...
grep -Eq "^APP_DISPLAY_NAME = Maju \([A-Za-z0-9._-]+\)$" "$ios" \
...
MAJU_ANDROID_DEBUG_APP_NAME="Maju Huddles" \
  MAJU_ANDROID_DEBUG_ID_SUFFIX=".huddles_829c" \
  "$wt/scripts/mobile-worktree-overrides.sh" > /dev/null
grep -q '^appName=Maju Huddles$' "$android" \
...
grep -q '^APP_DISPLAY_NAME = Maju (' "$ios" \
  && ! grep -q 'Maju Huddles' "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = xyz\.block\.maju\.dogfood\.mobile$' "$debug_xcconfig" \
  && pass "Debug.xcconfig defaults to the dogfood bundle identifier" \
  || fail "Debug.xcconfig must default to xyz.block.maju.dogfood.mobile"
...
grep -q '^ios_prefix="xyz.block.maju.dogfood.mobile\."$' "$clean_script" \
...
grep -q '^BUNDLE_IDENTIFIER = xyz\.block\.maju\.mobile$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = xyz.block.maju.mobile"
grep -q '^APP_DISPLAY_NAME = Maju$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = Maju"
...
grep -q 'resValue("string", "app_name", "Maju")' "$gradle" \
  && pass "Gradle default app_name stays Maju" \
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:xyz.block.maju.mobile\n'
  printf 'package:xyz.block.maju.mobile.feature_work_1\n'
  printf 'package:xyz.block.maju.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*xyz\.block\.maju\.mobile$'; then
