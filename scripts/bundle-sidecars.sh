...
SIDECARS=(maju-acp maju-agent maju-dev-mcp git-credential-nostr maju)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p maju-acp -p maju-agent -p maju-dev-mcp -p git-credential-nostr -p maju-cli' first." >&2
    exit 1
fi
