...
run_identity="${MAJU_TEST_RUN_ID:-${USER:-maju}:$$:$(date -u +%s):${RANDOM:-0}}"
run_hash="$(printf '%s' "$run_identity" | sha256_hex)"
run_hash="${run_hash:0:20}"
template_database="maju_nt_${run_hash}_desired"
export MAJU_TEST_RUN_ID="$run_identity"
export MAJU_POSTGRES_DESIRED_TEMPLATE="$template_database"
