...
require_canonical_repository() {
  ...
  origin_url="$(git config --get remote.origin.url 2>/dev/null)" || \
    fail_release_ruleset "origin is required and must point to heap-cider/maju" || return 1
  case "$origin_url" in
    git@github.com:heap-cider/maju.git|ssh://git@github.com/heap-cider/maju.git|https://github.com/heap-cider/maju.git|https://github.com/heap-cider/maju)
      ;;
    *)
      fail_release_ruleset "origin must point to canonical heap-cider/maju, not '$origin_url'" || return 1
      ;;
  esac
}
...
require_release_tag_ruleset() {
  ...
  ruleset_endpoint="repos/heap-cider/maju/rulesets/$RELEASE_TAG_RULESET_ID"
