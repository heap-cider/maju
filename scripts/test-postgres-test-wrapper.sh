...
fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/maju-postgres-wrapper.XXXXXX")"
...
#!/usr/bin/env bash
printf '%s\n' "$MAJU_TEST_SCHEMA_MODE" >"$MAJU_SCHEMA_MODE_LOG"
SH
...
run_case() {
  ...
  env \
    ...
    MAJU_POSTGRES_ADMIN_URL=postgres://maju@localhost/postgres \
    MAJU_POSTGRES_DESIRED_TEMPLATE=desired_template \
    MAJU_CREATEDB_LOG="$createdb_log" \
    MAJU_SCHEMA_MODE_LOG="$mode_log" \
