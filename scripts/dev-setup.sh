...
load_env() {
  ...
  # Smooth the local rename path for developers with a pre-Maju .env copied
  ...
  if [[ "${DATABASE_URL:-}" == "postgres://sprout:sprout_dev@localhost:5432/sprout" ]]; then
    warn "Migrating legacy default DATABASE_URL from sprout to maju for this setup run"
    DATABASE_URL="postgres://maju:maju_dev@localhost:5432/maju"
  fi
  if [[ "${PGUSER:-}" == "sprout" ]]; then PGUSER="maju"; fi
  if [[ "${PGPASSWORD:-}" == "sprout_dev" ]]; then PGPASSWORD="maju_dev"; fi
  if [[ "${PGDATABASE:-}" == "sprout" ]]; then PGDATABASE="maju"; fi

  export DATABASE_URL="${DATABASE_URL:-postgres://maju:maju_dev@localhost:5432/maju}"
...
cleanup_legacy_sprout_containers() {
  ...
  warn "Stopping/removing legacy sprout-* dev containers so maju-* containers can bind the standard ports"
...
fail_if_local_redis_blocks_compose() {
  ...
  if docker ps --format '{{.Names}}' | grep -qx 'maju-redis'; then
    return
  fi
  local redis_pids
...
"${REPO_ROOT}/bin/cargo" run -p maju-admin -- migrate
...
echo ""
echo -e "${GREEN}=======================================================${NC}"
echo -e "${GREEN}  Maju dev environment is ready!${NC}"
echo -e "${GREEN}=======================================================${NC}"
echo ""
