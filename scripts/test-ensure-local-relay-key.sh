...
TEST_DIR="$(mktemp -d "${TMPDIR:-/tmp}/maju-relay-key-test.XXXXXX")"
...
first_key="$(sed -n 's/^MAJU_RELAY_PRIVATE_KEY=//p' "${ENV_FILE}")"
...
"${SCRIPT_DIR}/ensure-local-relay-key.sh" "${ENV_FILE}" >/dev/null
second_key="$(sed -n 's/^MAJU_RELAY_PRIVATE_KEY=//p' "${ENV_FILE}")"
...
if [[ "$(grep -c '^MAJU_RELAY_PRIVATE_KEY=' "${ENV_FILE}")" -ne 1 ]]; then
