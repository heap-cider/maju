#!/usr/bin/env bash
# grab-emoji.sh — Register custom Slack emoji in Maju
#
# Looks up each emoji name in your Slack workspace and registers it in Maju
# via `maju emoji set`, making it available as :name: in the Maju emoji picker.
#
# Usage:
#   SLACK_TOKEN=xoxp-... ./scripts/grab-emoji.sh [--name <maju-name>] <emoji-name> [emoji-name ...]
#
# Options:
#   --name <maju-name>  Override the shortcode used in Maju (only valid with a single emoji)
#
# Env:
#   SLACK_TOKEN  — Slack user token (xoxp-...) with emoji:read scope
#
# Output:
#   name → registered as :name: in Maju   on success
...
if [[ $# -eq 0 ]]; then
  echo "Usage: SLACK_TOKEN=xoxp-... $0 [--name <maju-name>] <emoji-name> [emoji-name ...]" >&2
  exit 1
fi

if [[ -n "$MAJU_NAME" && $# -ne 1 ]]; then
...
if ! command -v maju &>/dev/null; then
  echo "ERROR: 'maju' not found in PATH. Install the Maju CLI and retry." >&2
  exit 1
fi
...
for emoji_name in "$@"; do
  ...
  maju_shortcode="${MAJU_NAME:-$emoji_name}"
  ...
  # Register in Maju
  set_output=$(maju emoji set --shortcode "$maju_shortcode" --url "$emoji_url" 2>&1) || {
    echo "${emoji_name} → ERROR: maju emoji set failed — ${set_output}"
    continue
  }

  echo "${emoji_name} → registered as :${maju_shortcode}: in Maju"
done
