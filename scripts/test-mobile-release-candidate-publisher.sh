...
case "${1:-}:${2:-}" in
  api:repos/heap-cider/maju/rulesets/14378754)
    ...
    ;;
  api:repos/heap-cider/maju/git/ref/heads/main) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:repos/heap-cider/maju/commits/*) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:--paginate)
    [[ "$3" == "repos/heap-cider/maju/git/matching-refs/tags/mobile-v1.2.3-rc." ]]
    printf '%s' "${GH_EXISTING_REFS:-}"
    ;;
  api:--method)
    endpoint="$4"
    case "$endpoint" in
      repos/heap-cider/maju/git/tags)
        record "$*"
        printf '%s\n' "$GH_TAG_OBJECT_SHA"
        ;;
      repos/heap-cider/maju/git/refs)
        record "$*"
        ;;
      *) exit 2 ;;
    esac
    ;;
  api:repos/heap-cider/maju/git/ref/tags/mobile-v1.2.3-rc.*)
    if [[ "$*" == *'.object.type'* ]]; then
    ...
    ;;
  api:repos/heap-cider/maju/git/tags/*)
    if [[ "$*" == *'.object.type'* ]]; then
...
export GITHUB_REPOSITORY=heap-cider/maju
...
grep -Fq -- '-f message=Maju Mobile 1.2.3 release candidate 1' "$GH_CALLS"
...
if GITHUB_REPOSITORY=attacker/maju "$publisher" 1.2.3 1 "$GH_TARGET_SHA" >/dev/null 2>&1; then
