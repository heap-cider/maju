...
ARTIFACT_DIR="${MAJU_RELEASE_SMOKE_ARTIFACT_DIR:-${ROOT}/desktop/test-results/release-smoke}"
DB_NAME="${MAJU_RELEASE_SMOKE_DB:-maju_release_smoke_${$}}"
REDIS_DB="${MAJU_RELEASE_SMOKE_REDIS_DB:-}"
LOCK_DIR="${TMPDIR:-/tmp}/maju-desktop-release-smoke.lock"
RELAY_PID=""
LOCK_HELD=false
...
RELAY_PORT="${MAJU_RELEASE_SMOKE_RELAY_PORT:-$(free_port)}"
HEALTH_PORT="${MAJU_RELEASE_SMOKE_HEALTH_PORT:-$(free_port)}"
METRICS_PORT="${MAJU_RELEASE_SMOKE_METRICS_PORT:-$(free_port)}"
...
cleanup() {
  ...
  docker exec maju-redis redis-cli -n "${REDIS_DB}" FLUSHDB >/dev/null 2>&1 || true
  docker exec maju-postgres dropdb -U maju --if-exists "${DB_NAME}" >/dev/null 2>&1 || true
...
# isolated Redis DB may opt out by setting MAJU_RELEASE_SMOKE_REDIS_DB.
if [[ -z "${REDIS_DB}" ]]; then
  mkdir "${LOCK_DIR}" 2>/dev/null || {
    log "another release-smoke run owns ${LOCK_DIR}; set MAJU_RELEASE_SMOKE_REDIS_DB only for an isolated runner"
    exit 1
  }
  LOCK_HELD=true
  REDIS_DB=15
...
for container in maju-postgres maju-redis maju-minio; do
...
docker exec maju-postgres createdb -U maju "${DB_NAME}"
export PGHOST=localhost PGPORT=5432 PGUSER=maju PGPASSWORD=maju_dev PGDATABASE="${DB_NAME}"
...
phase_start="$(date +%s)"
if [[ -n "${MAJU_E2E_RELAY_BIN:-}" ]]; then
  RELAY_BIN="${MAJU_E2E_RELAY_BIN}"
else
  log "building relay"
  cargo build --profile ci -p maju-relay
...
env \
  ...
  MAJU_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN=1000000 \
  MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC=100000 \
...
phase_start="$(date +%s)"
if [[ "${MAJU_RELEASE_SMOKE_NO_BUILD:-0}" == "1" ]]; then
...
MAJU_E2E_RELAY_URL="${RELAY_HTTP_URL}" \
MAJU_RELEASE_SMOKE_ARTIFACT_DIR="${ARTIFACT_DIR}" \
