...
check_instance() {
    ...
    clean_env bash -euo pipefail -c '
        cd "$1"
        export MAJU_RELAY_URL=wss://relay.example.test
        source "$2" >/dev/null
        python3 - "$3" "$4" <<'"'"'PY'"'"'
import json
import os
...
config = json.loads(os.environ["MAJU_TAURI_CONFIG"])
assert config["identifier"] == sys.argv[1], config
assert os.environ.get("VITE_DEV_BRANCH", "") == sys.argv[2]
assert os.environ["MAJU_RELAY_URL"] == "wss://relay.example.test"
assert "MAJU_PRIVATE_KEY" not in os.environ
assert config["build"]["devUrl"] == "http://localhost:" + os.environ["MAJU_VITE_PORT"]
PY
...
check_instance "$main" xyz.block.maju.app.dev ''
check_instance "$main/desktop" xyz.block.maju.app.dev ''
...
check_instance "$linked" xyz.block.maju.app.dev.feature-other other
check_instance "$linked/desktop" xyz.block.maju.app.dev.feature-other other
...
check_instance "$linked/desktop" xyz.block.maju.app.dev.head HEAD

ln -s "$main" "$tmp/main-link"
check_instance "$tmp/main-link/desktop" xyz.block.maju.app.dev ''
