...
def normalized(path: pathlib.Path, data: bytes) -> bytes:
    # Git and local tooling may materialize CRLF on Windows. Cargo treats the
    # two newline styles identically, so keep them from splitting equivalent
    # release caches.
