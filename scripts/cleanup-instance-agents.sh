...
# can finish, so agent workers (goose, maju-agent, ...) it spawned in their
...
# `MAJU_MANAGED_AGENT` env var from the shell: on macOS `pkill -f` matches only
...
#   <instance-id> is the desktop bundle identifier, e.g. `xyz.block.maju.app.dev`
#   (main checkout) or `xyz.block.maju.app.dev.my-branch` (a worktree).
