...
# default `maju-*` dev stack. Backing services run under the dedicated
# `maju-harness` Compose project (docker-compose.harness.yml); the relay runs
# in the foreground on override ports.
#
#   Topology (reuse this exact tuple for desktop parity runs):
#     compose project : maju-harness
#     postgres        : localhost:5471  (db=maju, user=maju, pass=maju_dev)
#     redis           : localhost:6471
#     minio           : localhost:9471 (console 9472)
#     relay main      : localhost:3030   ← MAJU_E2E_RELAY_URL=http://localhost:3030
...
#   docker compose -p maju-harness -f docker-compose.harness.yml down -v
# =============================================================================
set -euo pipefail
...
# ── Backing services (scoped to maju-harness only) ───────────────────────────
...
wait_pg() {
  for _ in $(seq 1 60); do
    if docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
         pg_isready -U maju >/dev/null 2>&1; then
...
psql_h() { docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
  psql -U maju -d maju -v ON_ERROR_STOP=1 "$@"; }

log "Resetting isolated database and applying schema..."
# This database belongs only to the maju-harness Compose project. Reset it on
...
export PGSCHEMA_PLAN_DB=maju PGSCHEMA_PLAN_USER=maju PGSCHEMA_PLAN_PASSWORD=maju_dev
export PGHOST=localhost PGPORT=${PG_PORT} PGUSER=maju PGDATABASE=maju
...
# MAJU_DB_HOST/PORT rather than the shared `maju-postgres` container.)
log "Seeding community (host=${COMMUNITY_HOST}), channels, and members..."
MAJU_COMMUNITY_HOST="${COMMUNITY_HOST}" \
  MAJU_DB_HOST=localhost MAJU_DB_PORT=${PG_PORT} MAJU_DB_USER=maju \
  MAJU_DB_PASS=maju_dev MAJU_DB_NAME=maju \
  MAJU_DB_DOCKER_CONTAINER="${PROJECT}-postgres-1" \
...
cargo build --profile "${CARGO_BUILD_PROFILE}" -p maju-relay
ok "Relay built"
...
tmux new-session -d -s "${TMUX_SESSION}" "cd '${REPO_ROOT}' && env \
  DATABASE_URL=postgres://maju:maju_dev@localhost:${PG_PORT}/maju \
  REDIS_URL=redis://localhost:${REDIS_PORT} \
  RELAY_URL=ws://localhost:${RELAY_MAIN} \
  MAJU_BIND_ADDR=0.0.0.0:${RELAY_MAIN} \
  MAJU_HEALTH_PORT=${RELAY_HEALTH} \
  MAJU_METRICS_PORT=${RELAY_METRICS} \
  MAJU_S3_ENDPOINT=http://localhost:${MINIO_PORT} \
...
for _ in $(seq 1 30); do
  if curl -s -o /dev/null "http://localhost:${RELAY_MAIN}/"; then
    ok "Relay live — MAJU_E2E_RELAY_URL=http://localhost:${RELAY_MAIN}"
