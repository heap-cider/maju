...
if [[ "$android_app_name" == *$'\n'* || "$android_app_name" == *$'\r'* ]] || \
    ...
    echo "MAJU_ANDROID_DEBUG_APP_NAME must contain only letters, numbers, spaces, ., _, -, or parentheses" >&2
    exit 2
fi
if [[ ! "$android_suffix" =~ ^\.[a-z][a-z0-9_]*$ ]]; then
    echo "MAJU_ANDROID_DEBUG_ID_SUFFIX must match \\.[a-z][a-z0-9_]* (for example .huddles_829c)" >&2
    exit 2
fi
...
echo "📱 Worktree ${worktree_name}: iOS label \"${label}\" (${ios_bundle_id}); Android label \"${android_app_name}\" (xyz.block.maju.mobile${android_suffix})"
