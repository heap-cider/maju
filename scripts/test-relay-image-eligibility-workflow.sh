...
require_literal "https://github.com/heap-cider/maju/attestations/deployment-eligibility/v1"
require_literal "actions/attest@1e69f48acb82d1966a394da916b4c1698aa569d6"
require_literal "if: matrix.variant == 'release'"
require_literal "MAJU_SOURCE_SHA"
require_literal "MAJU_BUILD_ID"
require_literal "MAJU_BUILD_URL"
require_literal '- "deploy/charts/maju/Chart.yaml"'
...
if grep -Fq "maju-staging-dev" "$workflow"; then
...
predicate=$(jq -n \
  --arg source_repository "heap-cider/maju" \
  --arg source_ref "refs/tags/v0.2.26" \
  ...
  --arg build_run_url "https://github.com/heap-cider/maju/actions/runs/200/attempts/2" \
  --arg qualification_workflow ".github/workflows/ci.yml" \
  --argjson qualification_run_id 201 \
  --argjson qualification_run_attempt 1 \
  --arg qualification_run_url "https://github.com/heap-cider/maju/actions/runs/201" \
...
jq -e '
  ...
  .helm_chart == {"name":"maju","compatible_version":"0.1.8"} and
  (has("schema") | not) and
