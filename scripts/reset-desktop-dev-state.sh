...
# Production state (`xyz.block.maju.app`, `~/.maju`, and `maju-desktop`) is
...
remove_bundle_state() {
  ...
  [[ -d "$base" ]] || return 0
  shopt -s nullglob
  for prefix in xyz.block.maju.app.dev xyz.block.sprout.app.dev; do
...
case "$(uname -s)" in
  Darwin)
    ...
    if command -v security >/dev/null 2>&1; then
      while security delete-generic-password -s maju-desktop-dev >/dev/null 2>&1; do :; done
...
remove_path "$HOME/.maju-dev"
remove_path "$HOME/.sprout-dev"

# A fresh dev nest must not re-import the installed app's ~/.maju contents on
# its next boot. The sentinel is the same one used by migrate_dev_nest().
mkdir -p "$HOME/.maju-dev"
: > "$HOME/.maju-dev/.dev-nest-migrated"

log "Development desktop state removed; production Maju state was not touched"
