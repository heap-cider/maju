#!/usr/bin/env bash
# Build Sprig — one deploy-anywhere multicall binary for the Maju ACP
...
Sprig is the all-in-one Maju agent binary for deploy-anywhere environments.
...
```bash
# Agent provider
export MAJU_AGENT_PROVIDER=anthropic            # or openai
export ANTHROPIC_API_KEY=sk-...
export ANTHROPIC_MODEL=claude-sonnet-4-20250514

# Nostr identity (shared by maju-acp, git auth, signing, and maju CLI)
export NOSTR_PRIVATE_KEY=nsec1...
export MAJU_PRIVATE_KEY="$NOSTR_PRIVATE_KEY"
export MAJU_RELAY_URL=https://your-relay.example.com
```
README
