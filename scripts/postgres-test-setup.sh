...
: "${MAJU_POSTGRES_ADMIN_URL:?set MAJU_POSTGRES_ADMIN_URL to an administrator database URL}"
: "${MAJU_POSTGRES_DESIRED_TEMPLATE:?postgres-test-run.sh must provide the desired-state database name}"
...
template_database="$MAJU_POSTGRES_DESIRED_TEMPLATE"
if [[ ! "$template_database" =~ ^maju_nt_[0-9a-f]{20}_desired$ ]]; then
  echo "refusing unsafe desired-state database name: $template_database" >&2
  exit 1
fi
run_hash="${template_database#maju_nt_}"
run_hash="${run_hash%_desired}"

"$dropdb" --if-exists --force \
  --maintenance-db="$MAJU_POSTGRES_ADMIN_URL" \
...
schema_log="${TMPDIR:-/tmp}/maju-pgschema-${run_hash}.log"
...
printf 'MAJU_POSTGRES_DESIRED_TEMPLATE=%s\n' "$template_database" >>"$NEXTEST_ENV"
