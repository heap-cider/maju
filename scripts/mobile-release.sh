...
candidate  Publish the next immutable mobile-vX.Y.Z-rc.N candidate tag at the
           exact current commit of heap-cider/maju's remote main branch.
USAGE
  exit 2
}
...
case "$command" in
  candidate)
    ...
    run_id="${run_url##*/}"
    gh run watch "$run_id" --repo heap-cider/maju --exit-status --compact || \
    ...
    printf 'Published %s at origin/main commit %s through maju-release-bot. Use this exact tag in Release Mobile.\n' \
      "$tag" "$main_sha"
    ;;
