...
export PGUSER="${PGUSER:-maju}"
export PGPASSWORD="${PGPASSWORD:-maju_dev}"
export PGDATABASE="${PGDATABASE:-maju}"
...
elif docker exec maju-postgres psql --version >/dev/null 2>&1; then
  run_psql() {
    docker exec -i -e PGPASSWORD="${PGPASSWORD}" maju-postgres \
      psql -U "${PGUSER}" -d "${PGDATABASE}" "$@"
  }
else
  echo "error: neither psql nor maju-postgres docker psql is available" >&2
  exit 1
fi
...
upload_fixture() {
  ...
  docker exec -i maju-minio mc pipe --quiet --attr "Content-Type=${mime}" \
    "local/${MAJU_S3_BUCKET:-maju-media}/${hash}.${extension}" < "${path}"
  printf '%s' "${sidecar}" | docker exec -i maju-minio mc pipe --quiet \
    --attr "Content-Type=application/json" \
    "local/${MAJU_S3_BUCKET:-maju-media}/_meta/${community_id}/${hash}.json"
}

fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/maju-admin-feedback.XXXXXX")"
...
printf '%s\n' "maju feedback diagnostics" "area: composer" \
  "event: resumed_from_sleep" "result: composer_unresponsive" > "${composer_diagnostics}"
printf '%s\n' "maju feedback diagnostics" "area: workspace-switching" \
...
if ! docker exec maju-minio mc alias set local http://localhost:9000 \
  "${MAJU_S3_ACCESS_KEY:-maju_dev}" "${MAJU_S3_SECRET_KEY:-maju_dev_secret}" >/dev/null; then
