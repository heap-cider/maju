...
# =============================================================================
# Provisions a membership-gated maju-relay and runs the full relay-driven
# mesh lifecycle harness (crates/maju-relay/examples/mesh_relay_lifecycle_smoke.rs):
...
# CPU model, real multi-process mesh), but with the Maju relay as the control
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  ...
  cargo build --profile "${CARGO_PROFILE}" -p maju-relay -p maju-admin -p git-credential-nostr
  cargo build --profile "${CARGO_PROFILE}" -p maju-relay --example mesh_relay_lifecycle_smoke
fi

ADMIN_BIN="target/${CARGO_PROFILE}/maju-admin"
HARNESS_BIN="target/${CARGO_PROFILE}/examples/mesh_relay_lifecycle_smoke"
for bin in "${ADMIN_BIN}" "${HARNESS_BIN}" "target/${CARGO_PROFILE}/maju-relay"; do
  if [[ ! -x "${bin}" ]]; then
...
MAJU_RELAY_PRIVATE_KEY="$(echo "${SIGNER_OUT}" | awk '/Secret key:/ {print $3}')"
if [[ -z "${RELAY_OWNER_PUBKEY}" || -z "${MAJU_RELAY_PRIVATE_KEY}" ]]; then
  err "Failed to generate relay identities via maju-admin generate-key"
  exit 1
fi
export MAJU_REQUIRE_RELAY_MEMBERSHIP=true
export RELAY_OWNER_PUBKEY
export MAJU_RELAY_PRIVATE_KEY
...
cleanup() {
  log "Stopping relay..."
  if [[ -f /tmp/maju-relay.pid ]]; then
    kill "$(cat /tmp/maju-relay.pid)" 2>/dev/null || true
  fi
