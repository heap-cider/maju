...
startup migration. It takes a pre-1321 (single-community) Maju Postgres to the
...
Boot the 1321 relay with **`MAJU_AUTO_MIGRATE=false`**. The schema is already
...
later `MAJU_AUTO_MIGRATE=true` boot doesn't try to re-run it), seed its
...
prefer leaving `MAJU_AUTO_MIGRATE=false` unless you have a reason to seed.
