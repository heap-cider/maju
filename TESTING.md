...
`maju-test-client` — those are marked `#[ignore]` and require a running relay:

```bash
# Start a relay first (see below), then:
cargo test -p maju-test-client -- --ignored
```
...
binaries once, run `maju-relay`, and drive it with the `maju` CLI. The
...
> **Already running Maju Desktop?** Desktop uses the same Docker container
> names (`maju-postgres`, `maju-redis`) and the same
...
> (`COMPOSE_PROJECT_NAME=maju-dev docker compose …`).

`just reset` wipes all local data and starts over — **including Maju
...
> `MAJU_AUTH_TAG`, `MAJU_RELAY_URL`, or `MAJU_PRIVATE_KEY` from a
> prior session (or a staging config), `unset` them before continuing.
> A stale `MAJU_AUTH_TAG` fails the **local dev relay** with
> `auth_error: signature verification failed` on the first CLI write —
> it is *not* tolerated.
> ```bash
> unset MAJU_AUTH_TAG MAJU_RELAY_URL MAJU_PRIVATE_KEY
> ```

### 2. Build the binaries

```bash
cargo build --release -p maju-relay -p maju-cli -p maju-admin
...
```bash
maju-relay                     # release binary from step 2, serves ws://localhost:3000
# alternatives:
# cargo run --release -p maju-relay     # rebuild + run in release
...
> `MAJU_HEALTH_PORT`) so K8s probes bypass auth middleware. The main app
> port also exposes `/health` for convenience.

The relay starts in dev mode (`MAJU_REQUIRE_AUTH_TOKEN=false`). The startup
...
> **Already running Maju Desktop (or another relay) on `:3000` / `:8080` /
> `:9102`?** Maju binds three ports — main, health, metrics — and any of
> them can collide. Use a separate terminal per role and export the right
> vars in each:
>
> **In the relay terminal** (before launching `maju-relay`):
> ```bash
> export MAJU_BIND_ADDR=0.0.0.0:3030
> export MAJU_HEALTH_PORT=8088
> export MAJU_METRICS_PORT=9202
...
> ```bash
> export MAJU_RELAY_URL=http://localhost:3030    # CLI target
...
> overrides — or the CLI will end up talking to Maju Desktop's relay.

> **Ignore `just setup`'s "Next steps" banner.** It still prints
> `just relay` (a debug build). Use `maju-relay` from step 2 here —
> step 2 already built the release binary.

When you're done, stop the relay (Ctrl-C in its terminal). If it's
backgrounded or you lost the terminal: `pkill -f maju-relay`. Leaving
...
```bash
# Generate a keypair
GEN=$(maju-admin generate-key)
export MAJU_PRIVATE_KEY=$(echo "$GEN" | awk '/Secret key:/ {print $3}')
PUBKEY=$(echo "$GEN"           | awk '/Public key:/ {print $3}')
echo "pubkey: $PUBKEY"

# Create a channel — the UUID is returned in the response
CHANNEL=$(maju channels create --name "smoke-$$" --type stream --visibility open | jq -r '.channel_id')
echo "channel: $CHANNEL"

# Send a message and read it back
SEND=$(maju messages send --channel "$CHANNEL" --content "hello from smoke test")
EVENT_ID=$(echo "$SEND" | jq -r '.event_id')
maju messages get --channel "$CHANNEL" --limit 5 | jq .

# Fetch the reply chain for a specific message (empty array on a leaf — that's fine)
maju messages thread --channel "$CHANNEL" --event "$EVENT_ID" | jq .
```
...
```bash
export PATH="$PWD/target/release:$PATH"
export DATABASE_URL="postgres://maju:maju_dev@localhost:5432/maju_roster_e2e"
export MAJU_RELAY_URL="http://localhost:3030"  # match the relay from step 3
export RELAY_URL="ws://localhost:3030"
export MAJU_RELAY_PRIVATE_KEY="<same key used by maju-relay>"
...
`POST /query` with an `#e` filter, or `maju messages thread`.

---

## ACP Harness (optional, end-to-end with a real agent)

`maju-acp` connects an ACP-speaking agent (goose, codex, claude code,
maju-agent) to the relay. The harness listens for events, drives the
...
from the sender identity (`MAJU_ACP_RESPOND_TO=anyone` still skips events
the agent signed itself).

```bash
cargo build --release -p maju-acp
export PATH="$PWD/target/release:$PATH"

# 1. Save your sender identity from step 4 — you'll need it to @mention the agent
SENDER_SK="$MAJU_PRIVATE_KEY"

# 2. Mint a fresh agent identity and capture its pubkey
AGENT_GEN=$(maju-admin generate-key)
...
maju channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member

# 4. Switch to the agent identity and start it.
#    maju-acp wants ws:// (not http://). If you set MAJU_RELAY_URL to an
#    http:// URL in step 3, set the ws:// equivalent here — same host/port.
export MAJU_PRIVATE_KEY="$AGENT_SK"
export MAJU_RELAY_URL=ws://localhost:3000   # match step 3 (e.g. ws://localhost:3030 if overridden)
export MAJU_ACP_RESPOND_TO=anyone           # default is owner-only; opens the gate for testing
# NIP-AE core-memory prompt injection is on by default; set MAJU_ACP_NO_MEMORY=true to opt out.
export GOOSE_MODE=auto                        # must be 'auto' or goose hangs on prompts

maju-acp                                    # foreground; logs to stdout (run in a separate terminal)

# Optional: turn on per-turn tracing if the default log is too quiet.
# RUST_LOG=maju_acp=debug maju-acp
```
...
> code / maju-agent, set `MAJU_ACP_AGENT_COMMAND` and `MAJU_ACP_AGENT_ARGS`
> accordingly — see `crates/maju-acp/README.md`. Without these, maju-acp
...
same env. See `crates/maju-acp/README.md` for parallel agents, heartbeats,
respond-to gates, and forum subscriptions.

To exercise deferred ACP startup, add `MAJU_ACP_LAZY_POOL=true` before launching
`maju-acp`. The harness should connect, authenticate, subscribe, and publish
...
```bash
export MAJU_PRIVATE_KEY=$SENDER_SK          # the key from step 4
maju messages send --channel "$CHANNEL" \
...
# `maju messages get` is how you confirm it ran.
maju messages get --channel "$CHANNEL" --limit 5 | jq '.[] | {pubkey, content}'
```

Replies are kind:9 in the same channel; `maju messages thread --channel <id>
...
|-----------------------------------|-----------------------------|-------|
| `MAJU_BIND_ADDR`                | `0.0.0.0:3000`              | Main app port |
| `MAJU_HEALTH_PORT`              | `8080`                      | `/_liveness`, `/_readiness` |
| `MAJU_METRICS_PORT`             | `9102`                      | Prometheus `/metrics` |
| `RELAY_URL`                       | `ws://localhost:3000`       | Advertised in NIP-11 / NIP-42 challenges. **Note: no `MAJU_` prefix.** |
| `DATABASE_URL`                    | `postgres://maju:maju_dev@localhost:5432/maju` | |
| `REDIS_URL`                       | `redis://localhost:6379`    | |
| `MAJU_REQUIRE_AUTH_TOKEN`       | `false`                     | When true, REST requires NIP-98 (no `X-Pubkey` fallback) |
| `MAJU_REQUIRE_RELAY_MEMBERSHIP` | `false`                     | When true, only pubkeys in `relay_members` can connect |
| `MAJU_DRAIN_JITTER_MS`          | `0` (off)                   | Per-connection upper bound, in ms, for the random delay before each live WebSocket gets its `1012 Service Restart` close on graceful shutdown. `0` closes every socket at once (the previous behavior). A positive value spreads closes uniformly over `[1, value]` ms to avoid a reconnect thundering herd on rolling deploys. Values above `20000` are capped to `20000` (`MAX_DRAIN_JITTER_MS`) to leave close-frame delivery headroom under the relay's 30s hard-drain timeout. Empty or whitespace-only is treated as unset (off); a non-integer fails startup loudly. |
| `MAJU_AUDIT_ENABLED`            | `true`                      | Tamper-evident event/media audit log. Set `false`/`0`/`off` to skip its DB pool and writes. Does not disable the separate moderation audit trail. |
| `MAJU_AUTO_MIGRATE`             | `false`                     | Opt in with `true`/`1`/`yes`/`on` to run embedded SQLx migrations on relay startup |
| `RELAY_OWNER_PUBKEY`              | unset                       | Bootstrapped as `owner` in `relay_members` at first start |
| `MAJU_ALLOW_NIP_OA_AUTH`        | `false`                     | Enable NIP-OA owner attestation for membership |
| `MAJU_WEB_DIR`                  | unset (source), `/srv/maju/web` (container) | Directory containing the invite landing bundle; the production container enables it so `/invite/{code}` always works |
| `MAJU_SERVE_GIT_WEB_GUI`        | `false`                     | Set to `true` or `1` to expose the bundled Git repository browser at `/` and `/repos/...`; invite routes do not depend on this flag |

CLI-side, only two matter for testing:

| Variable                | Default                  | Notes |
|-------------------------|--------------------------|-------|
| `MAJU_RELAY_URL`      | `http://localhost:3000`  | CLI relay base; accepts `ws(s)://` and normalises |
| `MAJU_PRIVATE_KEY`    | — (**required**)         | `nsec1…` or 64-char hex |
| `MAJU_AUTH_TAG`       | unset                    | Optional NIP-OA owner attestation JSON |
...
| `Address already in use` on relay start (os error 48 on macOS, 98 on Linux) | Another relay (or stale process) holding `:3000` / `:8080` / `:9102` (or your override ports) | The panic line names the failing port — read it first. Then `lsof -iTCP:3000,8080,9102 -sTCP:LISTEN` (or your override equivalents). Kill the offender (`pkill -f maju-relay`) or use the port-override block in step 3. If you already overrode and *still* collide, a prior reviewer left a relay running on the same alt ports — kill it or pick fresh ports |
| `auth_error: MAJU_PRIVATE_KEY is required` | Env not exported into the CLI's shell | `export MAJU_PRIVATE_KEY=...` (or pass `--private-key`) |
| `auth_error: MAJU_AUTH_TAG verification failed … signature verification failed` | A stale `MAJU_AUTH_TAG` inherited from a parent shell. The local dev relay rejects it. | `unset MAJU_AUTH_TAG` (see the scrub block in step 1) |
| `auth-required: verification failed` on a closed relay | NIP-OA attestation needed | Set `MAJU_AUTH_TAG` to the owner-issued JSON, or relax `MAJU_REQUIRE_RELAY_MEMBERSHIP` |
...
| ACP agent ignores all events | `MAJU_ACP_RESPOND_TO=owner-only` (default) with no owner configured | Set `MAJU_ACP_RESPOND_TO=anyone` for testing |
| ACP logs `discovered 0 channel(s)` / `no channel subscriptions resolved` | Agent identity isn't a member of any channel | `maju channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member` from another identity |
