...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(hashtextextended(
        'maju_channel_membership:' || NEW.community_id::text || ':' || NEW.channel_id::text,
        0
    ));
...
-- crates/maju-db/src/replica_fence.rs and migrations/0026). Exactly one row;
...
LANGUAGE SQL IMMUTABLE STRICT PARALLEL SAFE AS $$
    SELECT hashtextextended('maju-community-deletion:' || target::text, 0)
$$;
...
BEGIN
    ...
    executor_community := current_setting('maju.deletion_executor_community', true);
    executor_generation := current_setting('maju.deletion_fence_generation', true);
    ...
    serving_community := current_setting('maju.serving_write_community', true);
    serving_lease_id := current_setting('maju.serving_write_lease_id', true);
    serving_owner := current_setting('maju.serving_write_owner', true);
    serving_generation := current_setting('maju.serving_write_generation', true);
    serving_fence_generation := current_setting('maju.serving_write_fence_generation', true);
...
LANGUAGE plpgsql AS $$
DECLARE
    executor_community TEXT := current_setting('maju.deletion_executor_community', true);
    executor_generation TEXT := current_setting('maju.deletion_fence_generation', true);
