...
const stubModules = new Map([
  [
    "emoji-mart",
    "export const init = (...args) => globalThis.__MAJU_TEST_EMOJI_MART_INIT__?.(...args);\n" +
...
const STUB_URL_PREFIX = "maju-test-stub:";
...
const ASSET_URL_PREFIX = "maju-test-asset:";
...
export function resolve(specifier, context, nextResolve) {
  ...
  if (specifier === "@protected-features") {
    const variant =
      process.env.VITE_MAJU_BESTIE === "1" ? "internal.ts" : "public.ts";
    const resolved = path.join(srcRoot, "protectedFeatures", variant);
    return nextResolve(toFileSpecifier(resolved), context);
  }
  if (specifier === "@protected-feature-components") {
    const variant =
      process.env.VITE_MAJU_BESTIE === "1" ? "internalUi.ts" : "publicUi.ts";
    const resolved = path.join(srcRoot, "protectedFeatures", variant);
    return nextResolve(toFileSpecifier(resolved), context);
  }
