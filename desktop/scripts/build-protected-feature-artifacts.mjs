...
export function selectInternalVariant({ processEnv, modeEnv }) {
  return (processEnv.VITE_MAJU_BESTIE ?? modeEnv.VITE_MAJU_BESTIE) === "1";
}
...
function main() {
  ...
  const scratchRoot = mkdtempSync(
    path.join(tmpdir(), "maju-protected-feature-artifacts-"),
  );
  const selectedOutput = process.env.MAJU_PROTECTED_BUILD_OUTPUT
    ? path.resolve(process.env.MAJU_PROTECTED_BUILD_OUTPUT)
