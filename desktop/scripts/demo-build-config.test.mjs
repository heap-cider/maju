...
const expected = (name, slug) => ({
  name,
  slug,
  productName: `Maju ${name}`,
  dmgVolumeName: `Maju ${name}`,
  dmgFileStem: `Maju_${name.replace(/ /g, "_")}`,
  identifier: `xyz.block.maju.app.demo.${slug}`,
  appDataIdentity: `xyz.block.maju.app.demo.${slug}`,
  deepLinkScheme: `maju-demo-${slug}`,
  keyringService: `maju-desktop-demo.${slug}`,
  nestName: `.maju-demo-${slug}`,
  cliName: `maju-demo-${slug}`,
  tauriConfig: {
    productName: `Maju ${name}`,
    identifier: `xyz.block.maju.app.demo.${slug}`,
    plugins: { "deep-link": { desktop: { schemes: [`maju-demo-${slug}`] } } },
    bundle: { targets: ["app"] },
  },
});
...
test("production identity remains unchanged", () => {
  assert.deepEqual(productionBuildIdentity, {
    productName: "Maju",
    identifier: "xyz.block.maju.app",
    deepLinkScheme: "maju",
    keyringService: "maju-desktop",
    nestName: ".maju",
    cliName: "maju",
  });
});
