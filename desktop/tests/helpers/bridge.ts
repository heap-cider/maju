...
type MockBridgeOptions = {
   ...
   * `__MAJU_E2E_RELEASE_THREAD_REPLIES__()` is called — a manual gate the test
...
const WELCOME_CHANNEL_ENSURED_STORAGE_KEY_PREFIX =
  "maju-welcome-channel-ensured.v2:";
const ONBOARDING_COMPLETION_STORAGE_KEY_PREFIX = "maju-onboarding-complete.v1:";
const DEFAULT_MOCK_PUBKEY = "deadbeef".repeat(8);
// The relay HTTP/WS URLs follow MAJU_E2E_RELAY_URL (same env var seed.ts reads),
...
See [[mem/preferences/ui-density]] and [[mem/projects/maju-memory-viewer]] for details.
...
A retired launch checklist used to live at [[mem/archive/deleted-launch-checklist]], but that memory was deleted after the plan changed.`,
      ...
      outgoingRefs: [
        ...
        "mem/projects/maju-memory-viewer",
    ...
    memories: [
      ...
      {
        slug: "mem/projects/maju-memory-viewer",
        body: "Building the IXI-7 read-only memory viewer in the profile panel.\n\nChild memory: [[mem/projects/maju-memory-viewer/notes]]",
        eventId: "mock-project",
        createdAt: 1_700_000_300,
        outgoingRefs: ["mem/projects/maju-memory-viewer/notes"],
      },
      {
        slug: "mem/projects/maju-memory-viewer/notes",
...
async function seedDefaultCommunity(
  ...
  await page.addInitScript(
    ({ fallback, identityOverrideKey, relayUrl }) => {
      ...
      window.localStorage.setItem("maju-active-community-id", communityId);
    },
    {
      fallback: fallbackPubkey,
      identityOverrideKey: "maju:e2e-identity-override.v1",
      relayUrl: relayWsUrl ?? DEFAULT_RELAY_WS_URL,
    },
  );
}
  ...
  await page.addInitScript(
    ({
      ...
      const testWindow = window as Window & {
        __MAJU_E2E__?: Record<string, unknown>;
      ...
      const currentConfig = testWindow.__MAJU_E2E__ ?? {};
      ...
      testWindow.__MAJU_E2E_APP_BADGE_COUNT__ = 0;
      testWindow.__MAJU_E2E_APP_BADGE_STATE__ = "none";
      testWindow.__MAJU_E2E_CLICK_NOTIFICATION__ = (index: number) => {
      ...
      testWindow.__MAJU_E2E_NOTIFICATIONS__ = notificationLog;
    },
    {
      identity,
      mock: options.mock,
...
export async function installRelayBridge(
  ...
  await installBridge(page, {
    mode: "relay",
    user,
    // Thread MAJU_E2E_RELAY_URL into BOTH transports. The app defaults these to
