#!/usr/bin/env node
//
// Standalone Playwright screenshot helper for the Maju desktop app.
//
...
// `MAJU_HEADED=1` runs a real browser window instead of headless.
//
...
// CI is unaffected.
const browser = await chromium.launch({ headless: !process.env.MAJU_HEADED });
...
await page.addInitScript(() => {
  ...
  window.localStorage.setItem("maju-communities", JSON.stringify([community]));
...
await page.addInitScript(
  ({ updateReady }) => {
    ...
    window.__MAJU_E2E_APP_BADGE_COUNT__ = 0;
  },
...
try {
  if (args.messages) {
    ...
    for (const msg of messages) {
      await page.evaluate(
        (m) => {
          window.__MAJU_E2E_EMIT_MOCK_MESSAGE__?.(m);
        },
