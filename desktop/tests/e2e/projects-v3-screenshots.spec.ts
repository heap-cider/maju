...
test("repository-only relays keep the Repositories section available", async ({
  ...
  await page.addInitScript(() => {
    window.__MAJU_E2E_REPOSITORY_ONLY_PROJECTS__ = true;
  });
...
test("projects activity overview screenshot", async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.setItem("maju-theme", "light");
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-projects-view").click();
...
test("sidebar project add flow browses before creating", async ({ page }) => {
  ...
  await search.fill("maju");
  await browser.getByTestId("project-browser-result-maju").click();
  await expect(browser).toBeHidden();
  await expect(
    page.getByRole("navigation", { name: "Project breadcrumb" }),
  ).toContainText("maju");
  const addedProject = page.getByTestId("sidebar-project-maju");
...
test("repository pages show a centered Maju loader while fetching", async ({
  page,
}) => {
  await installMockBridge(page, { projectRepoSnapshotDelayMs: 750 });
  await openMajuProject(page);

  const loader = page.getByTestId("maju-loading-state");
  await expect(loader).toBeVisible();
  await expect(
    loader.getByRole("img", { name: "Loading repository" }),
  ).toBeVisible();
  const animatedMark = loader.locator(".maju-logo__mark");
  await expect(animatedMark).toHaveCSS(
    "animation-name",
    "maju-logo-scale-pulse",
  );
...
test("projects v3 workspace screenshot states", async ({ page }) => {
  ...
  await expect(agentContext).not.toContainText("Maju /");
  ...
  await page.evaluate(
    async ({ channelId, parentEventId }) => {
      ...
      await window.__MAJU_E2E_INVOKE_MOCK_COMMAND__?.("send_channel_message", {
        channelId,
...
test("projects v3 work-item list metadata", async ({ page }) => {
  ...
  await expect(pullRequestRow).toContainText(/relay-tools|maju|design-system/);
  await waitForAnimations(page);
  ...
  await expect(issueRow).toContainText(/relay-tools|maju|design-system/);
  await waitForAnimations(page);
  ...
  await expect(channelRow).toContainText("#maju");
  await expect(
