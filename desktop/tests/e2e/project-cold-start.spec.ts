    ...
    .poll(() =>
      page.evaluate(() =>
        Object.keys(window.localStorage).some((key) =>
          key.startsWith("maju-projects.v1:"),
        ),
      ),
    )
    .toBe(true);
...
async function mutateProjectCache(
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          "__MAJU_E2E_QUERY_CLIENT__" in window &&
          Boolean(window.__MAJU_E2E_QUERY_CLIENT__),
      ),
    )
    .toBe(true);
  await page.evaluate(() => {
...
test("snapshot project home cannot publish repository healing", async ({
  ...
  await page.getByTestId("channel-maju").click();
  await expect(page.getByTestId("project-home-context-panel")).toBeVisible();

  await page.addInitScript(() => {
    window.__MAJU_E2E_DEFER_FULL_PROJECT_QUERIES__ = true;
  ...
  const projectPublications = await page.evaluate(
    () =>
      window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__?.filter(
        (event) =>
          event.kind === 30621 &&
          event.tags.some((tag) => tag[0] === "d" && tag[1] === "maju"),
      ) ?? [],
  );
...
test("stale non-matching snapshot uses the scoped project-home lookup", async ({
  ...
  await page.evaluate(() => {
    const key = Object.keys(window.localStorage).find((candidate) =>
      candidate.startsWith("maju-projects.v1:"),
    );
  ...
  const usedScopedLookup = await page.evaluate((channelId) => {
    return window.__MAJU_E2E_PROJECT_QUERY_FILTERS__?.some((filter) =>
      filter["#maju-channel"]?.includes(channelId),
    );
