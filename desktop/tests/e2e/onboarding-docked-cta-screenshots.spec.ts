...
test("machine onboarding: landing, backup, setup docked CTAs", async ({
  ...
  await expect(
    page.getByTestId("backup-options").locator(".maju-card-textured"),
  ).toHaveCount(0);
  ...
  await expect(passwordPanel).not.toHaveClass(/maju-card-textured/);
  ...
  await expect(generatorPopover).not.toHaveClass(/maju-card-textured/);
  await waitForAnimations(page);
