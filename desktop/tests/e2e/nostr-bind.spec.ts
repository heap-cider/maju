...
async function openNostrBind(
  ...
  await page.waitForFunction(
    () =>
      ...
      ).__MAJU_E2E_INVOKE_MOCK_COMMAND__ === "function",
  );
  ...
  await page.addInitScript(
    ({ fail }) => {
      const testWindow = window as Window & {
        __MAJU_E2E_CLIPBOARD_TEXT__?: string;
      };
      Object.defineProperty(navigator, "clipboard", {
        ...
        value: {
          writeText: async (text: string) => {
            ...
            testWindow.__MAJU_E2E_CLIPBOARD_TEXT__ = text;
          },
        },
      });
    },
...
async function installShakeCounter(page: Page) {
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __MAJU_E2E_CODE_SHAKE_CALLS__?: number;
    };
    testWindow.__MAJU_E2E_CODE_SHAKE_CALLS__ = 0;
    ...
    Element.prototype.animate = function animate(keyframes, options) {
      if (
        ...
        testWindow.__MAJU_E2E_CODE_SHAKE_CALLS__ =
          (testWindow.__MAJU_E2E_CODE_SHAKE_CALLS__ ?? 0) + 1;
      }
...
async function shakeCount(page: Page): Promise<number> {
  return page.evaluate(
    () =>
      ...
      ).__MAJU_E2E_CODE_SHAKE_CALLS__ ?? 0,
  );
}
...
test("returns a signed response in the callback fragment after consent", async ({
  ...
  await openNostrBind(page, {
    ...VALID_REQUEST,
    callbackUrl: "https://admin.example.com/maju?source=bind#stale",
  ...
  expect(callbackUrl.pathname).toBe("/maju");
  expect(callbackUrl.search).toBe("?source=bind");
  expect(callbackUrl.searchParams.has("maju_bind")).toBe(false);
  expect(callbackUrl.hash).toMatch(/^#maju_bind=v1\.[A-Za-z0-9_-]+$/);
});
...
test("opens the manual fallback when returning to the browser fails", async ({
  ...
  await openNostrBind(
    ...
    {
      ...VALID_REQUEST,
      callbackUrl: "https://admin.example.com/maju",
      returnMode: "browser_fragment_v1",
    },
...
test("keeps the signed response available when clipboard access fails", async ({
  ...
  await expect(
    page
      ...
      .getByText("Maju couldn't access the clipboard. Try again."),
  ).toBeVisible();
