...
function snapshotKey(relayUrl: string) {
  return `maju-channels.v1:${relayUrl}:${OWNER_PUBKEY.toLowerCase()}`;
}
...
async function expectContentSurfaceHorizontalGutters(
  ...
  const [mainInsetBox, contentBox] = await Promise.all([
    page.locator("[data-maju-glass-inset]").boundingBox(),
    page.locator("[data-maju-content-surface]").first().boundingBox(),
  ]);
...
async function seedCommunities(
  ...
  await page.addInitScript(
    ({ list, active }) => {
      window.localStorage.setItem("maju-communities", JSON.stringify(list));
      window.localStorage.setItem("maju-active-community-id", active);
    },
...
test.describe("community rail", () => {
  ...
  test("restores pointer events after dismissing community settings", async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() =>
          window.localStorage.getItem("maju-active-community-id"),
        ),
      )
      .toBe(COMMUNITY_B.id);
  });
  ...
  test("keeps profile community actions available to members without invite access", async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          return (window.__MAJU_E2E_COMMAND_LOG__ ?? []).findLast(
  ...
  test("community switch cancels a send after its link preview settles", async ({
    ...
    const previewUrl =
      "https://github.com/heap-cider/maju/pull/5697?community=reset";
  ...
  test("community switch stops preview media before it reaches the new community", async ({
    ...
    const input = page.getByTestId("message-input");
    await input.fill(
      "https://github.com/heap-cider/maju/pull/5697?media=reset",
    );
    await page.getByTestId("send-message").click();
    await expect
      .poll(() =>
    ...
    expect(
      await page.evaluate(
        () => window.__MAJU_E2E_RELEASE_LINK_PREVIEW_METADATA__?.() ?? 0,
      ),
    ).toBeGreaterThan(0);
    await page.waitForTimeout(250);
  ...
  test("community switch cancellation wins before native upload registration", async ({
    ...
    const input = page.getByTestId("message-input");
    await input.fill(
      "https://github.com/heap-cider/maju/pull/5697?native=reset",
    );
    await page.getByTestId("send-message").click();
    await expect
      .poll(() =>
    ...
    expect(
      await page.evaluate(
        () => window.__MAJU_E2E_RELEASE_LINK_PREVIEW_UPLOADS__?.() ?? 0,
      ),
    ).toBe(2);
    await page.waitForTimeout(250);
  ...
  test("enters a remembered channel before live validation completes", async ({
    ...
    await page.evaluate(() => {
      const testWindow = window as typeof window & {
        __MAJU_E2E__?: { mock?: { channelsReadDelayMs?: number } };
      };
      if (!testWindow.__MAJU_E2E__) {
        throw new Error("missing E2E config");
      }
      testWindow.__MAJU_E2E__.mock = {
        ...testWindow.__MAJU_E2E__.mock,
        channelsReadDelayMs: 4_000,
      };
    });
    await page.getByTestId(`community-rail-button-${COMMUNITY_B.id}`).click();
  ...
  test("does not repair a remembered channel until live validation succeeds", async ({
    ...
    expect(
      await page.evaluate(async () => {
        ...
        const deferNext = testWindow.__MAJU_E2E_DEFER_NEXT_CHANNELS_READ__;
        const release = testWindow.__MAJU_E2E_RELEASE_CHANNELS_READ__;
        const invoke = testWindow.__MAJU_E2E_INVOKE_MOCK_COMMAND__;
        ...
        return {
          released,
          pending: testWindow.__MAJU_E2E_CHANNELS_READ_PENDING__,
        };
      }),
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () => window.__MAJU_E2E__?.mock?.channelsReadErrors?.length ?? 0,
        ),
      )
      .toBe(1);
    await page.evaluate(() => {
      ...
      const deferNextChannelsRead =
        testWindow.__MAJU_E2E_DEFER_NEXT_CHANNELS_READ__;
      const invalidateChannels = testWindow.__MAJU_E2E_INVALIDATE_CHANNELS__;
    ...
    await page.waitForFunction(
      () =>
        ...
        ).__MAJU_E2E_CHANNELS_READ_PENDING__ === 1,
    );
    expect(
      await page.evaluate(async () => {
        const invoke = (
    ...
    expect(
      await page.evaluate(
        () =>
          ...
          ).__MAJU_E2E_CHANNELS_READ_PENDING__,
      ),
    ).toBe(1);
    await page.getByTestId(`community-rail-button-${COMMUNITY_B.id}`).click();
    ...
    const released = await page.evaluate(
      () =>
        ...
        ).__MAJU_E2E_RELEASE_CHANNELS_READ__?.() ?? 0,
    );
    ...
    await page.evaluate(async () => {
      ...
      const config = testWindow.__MAJU_E2E__;
      const invalidateChannels = testWindow.__MAJU_E2E_INVALIDATE_CHANNELS__;
  ...
  test("leaving the final community returns to setup without resetting identity", async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() => typeof window.__MAJU_E2E_INVOKE_MOCK_COMMAND__),
      )
      .toBe("function");
    const identityBefore = await page.evaluate(async () =>
      window.__MAJU_E2E_INVOKE_MOCK_COMMAND__("get_identity"),
    );
    await page.getByTestId("sidebar-profile-avatar-button").click();
    await page.getByTestId("community-switcher").click();
    await page
    ...
    await expect
      .poll(() =>
        page.evaluate(() =>
          window.localStorage.getItem("maju-community-discovery-after-leave"),
        ),
      )
      .toBe("1");
  ...
  test("hides the rail with a single community", async ({ page }) => {
    await page.addInitScript((themeStorageKey) => {
      window.localStorage.setItem(themeStorageKey, "maju-dark");
    }, THEME_STORAGE_KEY);
    await installMockBridge(page, undefined, { skipCommunitySeed: true });
    await seedCommunities(page, [COMMUNITY_A], COMMUNITY_A.id);
    await page.goto("/");
  ...
  test("clears the macOS traffic lights", async ({ page }) => {
    ...
    const appSurfaceBox = await page
      .locator(".maju-huddle-app-surface")
      .boundingBox();
    const contentBox = await page
      .locator("[data-maju-content-surface]")
      .first()
      .boundingBox();
  ...
  test("drag-to-reorder updates the stored community order and survives reload", async ({
    ...
    await page.addInitScript(
      ({ list, active }) => {
        if (!window.localStorage.getItem("maju-communities")) {
          window.localStorage.setItem("maju-communities", JSON.stringify(list));
        }
        if (!window.localStorage.getItem("maju-active-community-id")) {
          window.localStorage.setItem("maju-active-community-id", active);
        }
      },
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          const raw = window.localStorage.getItem("maju-communities");
          if (!raw) return null;
