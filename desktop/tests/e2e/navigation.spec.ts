...
test("mixed Maju permalinks render as chips in the composer", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  ...
  const links = [
    `maju://message?channel=${channelId}&id=mock-general-welcome`,
    `maju://channel/${channelId}`,
    `maju://repo?owner=${owner}&d=maju-world`,
    `maju://pr?id=${pullRequestId}&owner=${owner}&d=maju-world`,
    `maju://issue?id=${issueId}&owner=${owner}&d=maju-world`,
  ].join(" ");
  ...
  const chips = composerInput.locator('[data-composer-maju-link=""]');
  await expect(chips).toHaveCount(5);
  await expect(chips.nth(0)).toHaveText("general");
  await expect(chips.nth(1)).toHaveText("general");
  await expect(chips.nth(2)).toHaveText("maju-world");
  // PR and issue chips both use repository identity only, matching rendered chips.
  await expect(chips.nth(3)).toHaveText("maju-world");
  await expect(chips.nth(4)).toHaveText("maju-world");
  ...
  await expect(composerInput).not.toContainText("maju://");
});

test("composer Maju chip labels wrap without orphaning their icons", async ({
  page,
}) => {
  ...
  const composerInput = page.getByTestId("message-input");
  const repoLink =
    "maju://repo?owner=953d3363262e86b770419834c53d2446409db6d918a57f8f339d495d54ab001f&d=relaytoolsobservabilityconsole-main";
  await composerInput.evaluate((element, text) => {
    const clipboardData = new DataTransfer();
    clipboardData.setData("text/plain", text);
    element.dispatchEvent(
  ...
  const chip = composerInput.locator('[data-composer-maju-link=""]');
...
test("message links to visible root messages open the thread panel", async ({
  ...
  await page.evaluate(() => {
    (
      window as Window & { __MAJU_E2E_DEFER_GET_EVENT__?: string | null }
    ).__MAJU_E2E_DEFER_GET_EVENT__ = "mock-general-welcome";
  });

  const link =
    "maju://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=mock-general-welcome";
  const composerInput = page.getByTestId("message-input");
  ...
  await expect(composerLink).toHaveAttribute("data-maju-link", "");
  await expect(composerLink).toHaveAttribute("title", "Thread in #general");
  await expect(composerInput).not.toContainText("maju://message");
  await page.getByTestId("send-message").click();
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (window as Window & { __MAJU_E2E_GET_EVENT_CALL_COUNT__?: number })
            .__MAJU_E2E_GET_EVENT_CALL_COUNT__ ?? 0,
      ),
    )
    .toBe(1);
  await page.evaluate(() => {
    (
      window as Window & { __MAJU_E2E_RELEASE_GET_EVENT__?: () => number }
    ).__MAJU_E2E_RELEASE_GET_EVENT__?.();
  });
  await expect(rootThreadLink).toHaveText("general");
    ...
    .poll(() =>
      page.evaluate(
        () =>
          (
            window as Window & {
              __MAJU_E2E_COMMAND_LOG__?: Array<{ command: string }>;
            }
          ).__MAJU_E2E_COMMAND_LOG__?.filter(
  ...
  await expect(
    messageTooltip.locator('[data-maju-tooltip-metadata-content=""]'),
  ...
  await expect(
    channelTooltip.locator('[data-maju-tooltip-metadata-content=""]'),
  ...
  const linkMenu = page.locator("[data-maju-link-context-menu]");
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        ).__MAJU_E2E_COMMAND_LOG__?.findLast(
...
test("direct-message tooltip metadata stays on one physical line", async ({
  ...
  await page
    ...
    .fill(`DM link maju://message?channel=${dmChannelId}&id=${dmMessageId}`);
  await page.getByTestId("send-message").click();
  ...
  const footer = page
    .getByRole("tooltip")
    .locator('[data-maju-tooltip-metadata-type=""]');
...
test("message links explain when preview metadata is unavailable", async ({
  ...
  await page
    .getByTestId("message-input")
    .fill(
      `Missing preview maju://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=${missingMessageId}`,
    );
  await page.getByTestId("send-message").click();
  ...
  await expect(missingMessageLink).toHaveClass(/maju-link-unavailable/);
