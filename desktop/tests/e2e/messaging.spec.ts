        ...
        : testInfo.title.includes("fragment link previews")
          ? {
              ...
              linkPreviewMetadataByHref: {
                "https://github.com/heap-cider/maju/pull/3767": {
                  title: "Maju pull request 3767",
                  siteName: "GitHub",
                  description: "Fragment-bearing PR link.",
                  imageDataUrl: null,
                  imageDomain: null,
                },
                "https://github.com/heap-cider/maju/pull/3867": {
                  title: "Maju pull request 3867",
                  siteName: "GitHub",
          ...
          : testInfo.title.includes("mixed link preview image outcomes")
            ? {
                linkPreviewMetadataByHref: {
                  "https://github.com/heap-cider/maju/pull/4001": {
                  ...
                  "https://github.com/heap-cider/maju/pull/4002": {
                ...
                : testInfo.title.includes("link preview no-image layout") ||
                  ...
                  ? {
                      linkPreviewMetadata: {
                        ...
                        siteName: "GitHub",
                        description:
                          "Open-source collaboration for the Maju app.",
                        imageDataUrl: null,
                        imageDomain: null,
                        faviconDataUrl:
                          "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
...
test("sent link preview media uses the authenticated proxy in compact and rich cards", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?proxy=1";
  const fallbackMediaPattern =
    /^maju-media:\/\/localhost\/media\/[\da-f]{64}\.png$/;
...
test("link preview style defaults to compact and Rich unfurls descriptions", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?inline=1";
  ...
  if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
    await waitForAnimations(page);
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/compact-composer-loading.png`,
    });
  }
  await waitForReadyComposerSnapshots(page);
  ...
  if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
    ...
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/compact-composer-ready.png`,
    });
  }
  await page.getByTestId("send-message").click();
  ...
  if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
    ...
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/recipient-compact.png`,
    });
  }
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        localStorage.getItem("maju.appearance.linkPreviewStyle"),
      ),
    )
    .toBe("rich");
  ...
  if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
    ...
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/recipient-rich.png`,
    });
    ...
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/rich-composer-loading.png`,
    });
    await waitForReadyComposerSnapshots(page);
    await expect(richComposerPreview).toHaveAttribute(
      "data-image-state",
    ...
    await page.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/rich-composer-ready.png`,
    });
...
for (const [pasteShape, wrapUrl] of [
  ...
  test(`${pasteShape} link preview paste paints before cold resolver work`, async ({
    ...
    const previewUrl = `https://github.com/heap-cider/maju/pull/3246?paste=${pasteShape}`;
    await page.goto("/");
    await page.getByTestId("channel-general").click();
    ...
    const firstPaint = await input.evaluate(async (element, pasteText) => {
      ...
      return {
        ...
        resolverStarted: (window.__MAJU_E2E_COMMAND_PAYLOADS__ ?? []).some(
...
test("display-text link preview produces and sends its preview", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?display=text";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
...
test("rich link preview preserves description newlines after sending", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?newlines=1";
  await page.addInitScript(() =>
    localStorage.setItem("maju.appearance.linkPreviewStyle", "rich"),
  );
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page.getByTestId("message-input").fill(previewUrl);
...
test("completed link previews normalize a trailing-fragment URL and still send", async ({
  ...
  const previewUrls = [
    ...
    "https://github.com/heap-cider/maju/pull/3246",
...
test("explicit cancellation suppresses a pending link preview and sends without it", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?send=pending";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page.getByTestId("message-input").fill(previewUrl);
...
test("Enter during an in-flight snapshot upload hands off and sends once", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
...
test("async metadata beyond old cutoff still produces preview image", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?slow=metadata";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("async upload beyond metadata budget retains preview image", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?slow=image";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("Skip wins the upload race and sends without preview", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?skip=1";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("promoted link preview send clears Sending after REST publication", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?pending=preview";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("settled-empty promoted link preview send uses REST and clears Sending after Skip", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?pending=empty";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
  ...
  const result = await page.evaluate(() => ({
    restCalls: (window.__MAJU_E2E_COMMAND_PAYLOADS__ ?? []).filter(
      (entry) => entry.command === "send_channel_message",
    ),
    websocketSends: (window.__MAJU_E2E_SIGNED_EVENTS__ ?? []).filter(
      (event) => event.kind === 9,
    ),
  }));
...
test("draft auto-send promotes link preview preparation and sends exactly once", async ({
  ...
  const previewUrl =
    "https://github.com/heap-cider/maju/pull/3246?draft=autosend";
  ...
  await page.addInitScript(
    ...
    {
      storeKey: `maju-drafts.v1:${"deadbeef".repeat(8)}`,
      draftKey: channelId,
      content: previewUrl,
      channel: channelId,
    },
...
test("rapid Enter presses on a ready link preview send exactly once", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?rapid=1";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("pasting a link and immediately pressing Enter prepares it after submit", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?fast=send";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
...
test("a snapshot media upload failure preserves a metadata-only preview", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?upload=fail";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
  await input.fill(previewUrl);
...
test("editing a message excludes link previews entirely", async ({ page }) => {
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?edit=1";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  const input = page.getByTestId("message-input");
...
test("hiding composer link previews suppresses the whole draft and emits the blanket marker", async ({
  ...
  const firstUrl = "https://github.com/heap-cider/maju/pull/3246?hide=all";
...
test("composer link preview embeds stay attachment-sized while loading and ready", async ({
  ...
  for (const width of [800, 420]) {
    ...
    if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
      ...
      await page.screenshot({
        animations: "disabled",
        path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/composer-${width}-loading.png`,
      });
    }
    ...
    if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
      ...
      await page.screenshot({
        animations: "disabled",
        path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/composer-${width}-ready.png`,
      });
    }
...
test("compact link preview image geometry truncates long titles to one line", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?geometry=1";
  ...
  if (process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR) {
    ...
    await row.screenshot({
      animations: "disabled",
      path: `${process.env.MAJU_LINK_PREVIEW_SCREENSHOTS_DIR}/recipient-compact-long-title.png`,
    });
  }
});
...
test("composer no-image link embeds keep the attachment footprint", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/3246?inline=none";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page.getByTestId("message-input").fill(previewUrl);
  const card = page
...
test("mixed link preview image outcomes keep Compact and Rich fallbacks stable", async ({
  ...
  const loadedUrl = "https://github.com/heap-cider/maju/pull/4001";
  const rateLimitedUrl = "https://github.com/heap-cider/maju/pull/4002";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page
    .getByTestId("message-input")
...
test("fragment link previews render a card per canonical URL", async ({
  ...
  const fragmentUrlA =
    "https://github.com/heap-cider/maju/pull/3767#pullrequestreview-4857569498";
  const fragmentUrlB =
    "https://github.com/heap-cider/maju/pull/3767#issuecomment-1";
  const plainUrl = "https://github.com/heap-cider/maju/pull/3867";
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page
    .getByTestId("message-input")
...
test("link preview browser image errors render a fallback", async ({
  ...
  await page
    .getByTestId("message-input")
    .fill("https://github.com/heap-cider/maju/pull/4003");
  await waitForReadyComposerSnapshots(page);
  await page.getByTestId("send-message").click();
...
test("relay GIF search selects content-only media and reports the share", async ({
    ...
    .poll(() =>
      page.evaluate((expectedUrl) => {
        return Boolean(
          ...
          ).__MAJU_E2E_SIGNED_EVENTS__?.some(
  ...
  const matchingEvent = await page.evaluate((expectedUrl) => {
    ...
    ).__MAJU_E2E_SIGNED_EVENTS__?.find(
      (event) => event.kind === 9 && event.content?.includes(expectedUrl),
    );
  }, gifUrl);
...
test("sends a thread message to its parent channel with a root-thread link", async ({
  ...
  const previewUrl = "https://github.com/heap-cider/maju/pull/5305";
  ...
  const { ownReplyId, rootId } = await page.evaluate(
    ({ alicePubkey, ownReply, root, semanticTags }) => {
      const emit = window.__MAJU_E2E_EMIT_MOCK_MESSAGE__;
  ...
  await expect
    .poll(() =>
      page.evaluate((content) => {
        return Boolean(
          (window.__MAJU_E2E_COMMAND_LOG__ ?? []).findLast(
            (entry) =>
  ...
  expect(sentPayload).toMatchObject({
    ...
    parentEventId: null,
    sentFromThreadTag: ["maju:sent-from-thread", rootId, rootContent],
  });
...
test("editing a thread root in focus mode dismisses the drawer before focusing the main composer", async ({
  ...
  await page.addInitScript(() => {
    localStorage.setItem("maju.channels.threadViewMode", "focus");
  });
...
test("a refused message deep link retries after the thread edit is canceled", async ({
  ...
  await mainInput.fill(
    `Retry link maju://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=${destinationId}`,
  );
  await mainInput.press("Enter");
  ...
  const { destinationRootId, sourceRootId } = await page.evaluate(
    ({ destinationRoot, sharedMessage, sourceReply, sourceRoot }) => {
      ...
      emit({
        ...
        extraTags: [["maju:sent-from-thread", destination.id, destinationRoot]],
      });
  ...
  test(`a refused same-thread ${targetKind} target preserves the edit and retries after cancel`, async ({
    ...
    const { sourceReplyId, sourceRootId } = await page.evaluate(
      ({ sourceReply, sourceRoot, targetKind }) => {
        ...
        emit({
          channelName: "general",
          content: `Same-thread ${targetKind} target maju://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=${targetId}&thread=${root.id}`,
        });
