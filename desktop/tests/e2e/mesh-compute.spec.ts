...
test("Share compute selects the curated default and starts and stops sharing", async ({
  ...
  await expect(card).toContainText(
    "Maju downloads remote models when sharing starts",
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() => (window as E2eWindow).__MAJU_E2E_COMMANDS__ ?? []),
    )
    .toContain("mesh_start_node");
  await expect
    .poll(() =>
      page.evaluate(
        () => (window as E2eWindow).__MAJU_E2E_COMMAND_PAYLOADS__ ?? [],
      ),
    )
    .toContainEqual({
...
test("a consuming client can switch to sharing its saved local model", async ({
  ...
  await page.addInitScript((model) => {
    window.localStorage.setItem("maju.mesh-compute.share.model.v1", model);
  }, localModel);
  await installMockBridge(page);
  await page.goto("/");
  ...
  await page.waitForFunction(
    () => typeof (window as E2eWindow).__MAJU_E2E_SET_MESH__ === "function",
  );
  await page.evaluate(() => {
    (window as E2eWindow).__MAJU_E2E_SET_MESH__?.({
      nodeState: "running",
      nodeMode: "client",
    });
  });
  ...
  await expect(card).toContainText("Maju may briefly restart");
  await expect(toggle).not.toBeChecked();
  ...
  const commands = await page.evaluate(() => ({
    names: (window as E2eWindow).__MAJU_E2E_COMMANDS__ ?? [],
    payloads: (window as E2eWindow).__MAJU_E2E_COMMAND_PAYLOADS__ ?? [],
  }));
