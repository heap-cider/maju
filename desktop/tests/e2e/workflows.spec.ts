...
test("rejects a stale card toggle without overwriting a newer edit", async ({
  ...
  await page.evaluate(async (name) => {
    ...
    const createCall = [...(window.__MAJU_E2E_COMMAND_PAYLOADS__ ?? [])]
      .reverse()
...
test("reports a rejected workflow status change", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    window.__MAJU_E2E__.mock.workflowUpdateError = "relay refused the update";
  });
...
test("duplicates a workflow", async ({ page }) => {
  ...
  const copiedYaml = await page.evaluate(() => {
    const calls = (window.__MAJU_E2E_COMMAND_PAYLOADS__ ?? []).filter(
      (call) => call.command === "create_workflow",
    );
...
test("rejected deletion keeps the confirmation, editor, and draft", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__ ??= {};
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.workflowDeleteError = "relay refused deletion";
  });
