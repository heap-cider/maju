...
test("top-level project lists align dates and overflow actions", async ({
  ...
  // Subtitle is the repository location (owner/repo for Maju-hosted repos).
  await expect(repositoryRow.locator("p")).toHaveText(/\/maju$/);
...
test("multi-repository projects switch the active repository", async ({
  ...
  await expect(picker).toContainText("maju");
  await picker.click();
  await expect(
  ...
  ).toContainEqual(["maju-channel", "9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50"]);
...
test("navigating via a 30617 entity-link route opens the correct non-primary repository and renders its PR", async ({
  ...
  // The repository picker must show "relay-tools" (non-primary), not "maju" (primary).
  const picker = page.getByTestId("project-repository-picker");
  await expect(picker).toContainText("relay-tools", { timeout: 10_000 });
  await expect(picker).not.toContainText("maju");
