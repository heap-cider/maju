...
test("top-level project lists show metadata and overflow actions", async ({
  ...
  await expect(page.getByTestId("repository-row-maju")).toBeVisible();
  await expect(page.getByTestId("repository-row-relay-tools")).toBeVisible();
  const repositoryRow = page.getByTestId("repository-row-maju");
  await expect(
...
test("creating a project opens its channel conversation", async ({ page }) => {
  ...
  const createdEvents = await page.evaluate(
    () =>
      window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__?.filter((event) =>
        event.tags.some(
  ...
  expect(
    projectEvent?.tags.find((tag) => tag[0] === "maju-channel")?.[1],
  ).toMatch(
...
test("unsupported relays cannot create a channel-first project", async ({
  ...
  await enableProjectsFeature(page);
  await page.addInitScript(() => {
    window.__MAJU_E2E_UNSUPPORTED_PROJECT_ANNOUNCEMENTS__ = true;
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-projects-view").click();
...
test("project creation can retry after its repository publication fails", async ({
  ...
  await enableProjectsFeature(page);
  await page.addInitScript(() => {
    window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__ = [30621];
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-projects-view").click();
...
test("project creation is idempotent after a lost publish acknowledgement", async ({
  ...
  await enableProjectsFeature(page);
  await page.addInitScript(() => {
    window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__ = [30621];
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-projects-view").click();
...
test("project sidebar rows open the home channel and nest extra channels", async ({
  ...
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await addProjectToSidebar(page, "maju");

  const projectRow = page.getByTestId("sidebar-project-maju");
  const expand = page.getByTestId("sidebar-project-expand-maju");
  const nestedChannel = page.getByTestId("sidebar-project-channel-maju-random");
  await expect(page).toHaveURL(/\/projects\//);
  await expect(page.getByTestId("project-channel-home")).toBeVisible();
  await expect(projectRow).toHaveAttribute("data-active", "true");
  await expect(expand).toHaveAttribute("aria-expanded", "false");
  await expect(nestedChannel).toBeHidden();
  await expect(page.getByTestId("sidebar-project-repository-maju")).toHaveCount(
    0,
  );
  await expect(
    page.getByTestId("sidebar-project-home-channel-maju"),
  ).toHaveCount(0);
  ...
  await page.getByTestId("sidebar-project-maju").click();
  await expect(page.getByTestId("project-home-context-panel")).toBeVisible();
  ...
  const addedEvents = await page.evaluate(
    () =>
      window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__?.filter(
        (event) =>
  ...
  ).toContainEqual(["maju-channel", "cf63feec-21bb-5bf0-a2f8-0e4c3de8ec73"]);
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__?.some(
            (event) =>
              event.kind === 30621 &&
              event.tags.some(
...
test("latest files commit opens its detail without a divider", async ({
  ...
  await page.getByTestId("project-home-context-repo-maju").click();
...
test("adding a repository retries and reports an error when the 30617 publication is rejected", async ({
  ...
  await page.addInitScript(() => {
    ...
    window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__ = [30617, 30617];
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await addProjectToSidebar(page, "maju");
...
test("adding a repository treats a lost 30617 acknowledgement as success", async ({
  ...
  await page.addInitScript(() => {
    window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__ = [30617];
  });
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await addProjectToSidebar(page, "maju");
...
test("adding a repository blocks when a standalone 30617 already exists at that coordinate", async ({
  ...
  // even though it is not yet in the "maju" project's member list.
...
test("navigating via a 30617 entity-link route opens the correct non-primary repository and renders its PR", async ({
  ...
  await enableProjectsFeature(page);
  // Seed a known pull-request for relay-tools (the non-primary member of "maju")
  ...
  await expect(page.getByTestId("sidebar-project-maju")).toHaveCount(0);
