 ...
 *
 * Covers:
 *   1. Global defaults Advanced shows numeric inputs for maju-agent.
...
test("global_advanced_maju_agent_shows_all_numeric_controls", async ({
  page,
}) => {
  // The mock bridge's withMockRuntimeConfigMetadata injects the numeric env var
  // fields for maju-agent. When maju-agent is selected and Advanced is opened,
  ...
  // All three numeric inputs must be present for maju-agent.
...
test("delayed_catalog_per_agent_saved_tuning_values_visible_then_structured_controls_appear", async ({
    ...
    managedAgents: [
      {
        ...
        channelNames: ["agents"],
        envVars: {
          MAJU_AGENT_MAX_OUTPUT_TOKENS: "4096",
  ...
  // while the catalog hasn't settled: MAJU_AGENT_MAX_OUTPUT_TOKENS and
  // MAJU_AGENT_MAX_ROUNDS should appear in the env-vars editor.
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="MAJU_AGENT_MAX_OUTPUT_TOKENS"]',
    ),
  ).toBeVisible();
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="MAJU_AGENT_MAX_ROUNDS"]',
    ),
  ).toBeVisible();
