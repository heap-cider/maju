...
const SIDEBAR_WIDTH_STORAGE_KEY = "maju-sidebar-width";
...
test("supports API tokens without cluttering the default join form", async ({
  ...
  await page.getByLabel("API token").fill("maju_secret");
...
for (const theme of ["maju", "github-light", "catppuccin-mocha"]) {
...
test("aligns the sidebar search with the channel title outside the Maju theme", async ({
  page,
}) => {
...
test("shows a sidebar update card when an update is ready", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __MAJU_E2E__?: { mock?: { updateAvailable?: boolean } };
    };
