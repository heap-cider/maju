...
async function setMockWebsocketUnavailable(
  ...
  await page.evaluate((value) => {
    const setUnavailable = window.__MAJU_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__;
    if (!setUnavailable) {
...
async function activateRelayRateLimit(
  ...
  await page.evaluate((duration) => {
    const activate = window.__MAJU_E2E_ACTIVATE_RELAY_RATE_LIMIT__;
    if (!activate) {
...
async function getMockWebsocketConnectAttempts(
  ...
  return page.evaluate(() => {
    const getAttempts = window.__MAJU_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__;
    return getAttempts?.() ?? [];
  });
}
...
async function queueAuthResponses(
  ...
  await page.evaluate((queued) => {
    const queue = window.__MAJU_E2E_QUEUE_AUTH_RESPONSES__;
...
async function closeLiveSubscriptions(
  ...
  const closed = await page.evaluate((message) => {
    const close = window.__MAJU_E2E_CLOSE_LIVE_SUBSCRIPTIONS__;
...
async function queueChannelHistoryCloses(
  ...
  await page.evaluate((queued) => {
    const queue = window.__MAJU_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__;
    if (!queue) {
...
test("sub-2s degraded flap invalidates relay queries on recovery", async ({
  ...
  await page.evaluate(() => {
    const queryClient = window.__MAJU_E2E_QUERY_CLIENT__ as
      | {
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (
            window.__MAJU_E2E_QUERY_CLIENT__ as unknown as {
              __rawHealInvalidations?: number;
            }
