...
const SNAPSHOT_DIAGNOSTIC_MARK = "maju:sidebar:snapshot-diagnostic";
const FULL_SIDEBAR_PAINT_MARK = "maju:sidebar:full-list-painted";
const BOOT_TO_FULL_SIDEBAR_MEASURE = "maju:sidebar:boot-to-full-list-painted";
...
async function trackSnapshotRows(page: Page) {
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __MAJU_E2E_SNAPSHOT_ROWS_SEEN__?: string[];
    };
    const seen: string[] = [];
    testWindow.__MAJU_E2E_SNAPSHOT_ROWS_SEEN__ = seen;
...
async function getTrackedSnapshotRows(page: Page) {
  return page.evaluate(
    () =>
      ...
      ).__MAJU_E2E_SNAPSHOT_ROWS_SEEN__ ?? [],
  );
}
