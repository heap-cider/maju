...
test("setup shows all bundled harnesses as detected", async ({ page }) => {
  ...
  const [lastHarnessBox, harnessNoteBox] = await Promise.all([
    page.getByTestId("onboarding-runtime-maju-agent").boundingBox(),
...
test("Back preserves incomplete defaults draft without writing", async ({
  ...
  await harness.click();
  await page
    .getByTestId("global-agent-default-harness-option-maju-agent")
    .click();
  ...
  await expect(harness).toHaveText("Maju");
