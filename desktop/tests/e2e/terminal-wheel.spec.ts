...
const TERM = 'section[aria-label="Maju Term"]';
const NAMED = 0x0100_0000;
const FG = NAMED | 256;
const BG = NAMED | 257;

/**
 * The shipped mock bridge throws on every `terminal_*` command, so Maju Term is
...
async function reveal(page: Page) {
  ...
  // Maju Term needs a channel: TerminalBootstrap's context is null on Home, so
...
test("project terminal button opens Maju Term for the repository", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 800 });
  await installTerminalBackend(page);
...
test("scrollback: wheel over Maju Term reaches terminal_scroll", async ({
  page,
}) => {
  await reveal(page);
  await pushFrame(page, [{ line: 0, text: "maju@term:~$ " }], {
    line: 0,
    column: 13,
  });
...
test("terminal viewport click retains keyboard input ownership", async ({
  ...
  await page
    .locator(".maju-terminal-viewport")
...
test("concealed terminal viewport does not steal Maju focus", async ({
  page,
}) => {
  await reveal(page);
  await page.keyboard.press("Meta+j");
  ...
  await page.keyboard.type("MAJU_KEYSTROKE");
  const terminalInputs = await page.evaluate(
    () =>
      (window as typeof window & { __SAMI_TERM__: { inputs: string[] } })
        .__SAMI_TERM__.inputs,
  );
  expect(terminalInputs.join("")).not.toContain("MAJU_KEYSTROKE");
});
