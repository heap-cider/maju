...
export class TwoRelayHarness {
  ...
  static async create(relays: readonly RelaySpec[]) {
    return new TwoRelayHarness(
      await mkdtemp(join(tmpdir(), "maju-ae-e2e-")),
      relays,
    );
  }

  async startRelays(binary = process.env.MAJU_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("MAJU_E2E_RELAY_BIN is required for the live gate");
    await Promise.all(
  ...
  async restartRelay(name: string, binary = process.env.MAJU_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("MAJU_E2E_RELAY_BIN is required for the live gate");
  ...
  async startAcp(
    ...
    const binary = process.env.MAJU_E2E_ACP_BIN;
    if (!binary)
      throw new Error("MAJU_E2E_ACP_BIN is required for the live gate");
    return this.spawnOwned(name, binary, [], {
      MAJU_RELAY_URL: relayWsUrl,
      MAJU_PRIVATE_KEY: privateKey,
      MAJU_AUTH_TAG: "",
      MAJU_ACP_LAZY_POOL: "true",
      MAJU_ACP_AGENT_COMMAND: process.execPath,
      MAJU_ACP_AGENT_ARGS: resolve("tests/e2e/fixtures/fake-acp-agent.mjs"),
      MAJU_E2E_CLI_BIN: process.env.MAJU_E2E_CLI_BIN,
      ...extraEnv,
    });
  }
  ...
  private async startRelay(binary: string, relay: RelaySpec) {
    const child = this.spawnOwned(relay.name, binary, [], {
      ...
      MAJU_BIND_ADDR: `127.0.0.1:${relay.ports.main}`,
      MAJU_HEALTH_PORT: String(relay.ports.health),
