  ...
  await page
    ...
    .evaluate((editor) => {
      ...
      clipboardData.setData(
        "text/html",
        `<a data-maju-agent-snapshot="${payload}" href="https://relay.example/media/shared.agent.png">Snapshot</a>`,
      );
...
test("discards while voice-note processing is pending", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    (
      window as Window & {
        __MAJU_E2E_RESOLVE_VOICE_NOTE_DECODE__?: () => void;
      }
    ).__MAJU_E2E_RESOLVE_VOICE_NOTE_DECODE__ = () => {
...
test("hard-caps audio work and cancels active and queued loads on unmount", async ({
  ...
  await page.addInitScript(() => {
    window.__MAJU_E2E_HOLD_MEDIA_FETCHES__ = true;
    const originalCreate = URL.createObjectURL.bind(URL);
    const originalRevoke = URL.revokeObjectURL.bind(URL);
    const counters = { created: 0, revoked: 0 };
    (
      window as Window & {
        __MAJU_E2E_AUDIO_OBJECT_URLS__?: typeof counters;
      }
    ).__MAJU_E2E_AUDIO_OBJECT_URLS__ = counters;
  ...
  expect(
    await page.evaluate(
      () => window.__MAJU_E2E_AUDIO_OBJECT_URLS__?.created ?? 0,
    ),
  ).toBe(0);
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__MAJU_E2E_MEDIA_FETCH_STATE__?.active ?? -1),
    )
    .toBe(0);
  const commandCounts = await page.evaluate(() => {
    const commands = window.__MAJU_E2E_COMMANDS__ ?? [];
    return {
  ...
  expect(
    await page.evaluate(() => window.__MAJU_E2E_AUDIO_OBJECT_URLS__),
