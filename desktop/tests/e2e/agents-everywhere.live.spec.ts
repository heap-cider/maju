...
const enabled = process.env.MAJU_E2E_AGENTS_EVERYWHERE === "1";
const relayBin = process.env.MAJU_E2E_RELAY_BIN;
const cliBin = process.env.MAJU_E2E_CLI_BIN;
const adminBin = process.env.MAJU_E2E_ADMIN_BIN;
...
async function run(
  ...
  const { stdout } = await exec(binary, args, {
    cwd: "..",
    env: { ...process.env, MAJU_AUTH_TAG: "", ...env },
  });
  return stdout;
}
...
test.describe("agents everywhere live two-relay gate", () => {
  test.skip(!enabled, "set MAJU_E2E_AGENTS_EVERYWHERE=1 to run live gate");
    ...
    const specs: [RelaySpec, RelaySpec] = [
      {
        ...
        redisUrl: process.env.MAJU_E2E_REDIS_A ?? "redis://127.0.0.1:6379/11",
      },
      {
        name: "relay-b",
        ports: {
        ...
        redisUrl: process.env.MAJU_E2E_REDIS_B ?? "redis://127.0.0.1:6379/12",
      },
    ];
    const harness = await TwoRelayHarness.create(specs);
    try {
      await harness.startRelays(required("MAJU_E2E_RELAY_BIN", relayBin));
      const senderOutput = await run(required("MAJU_E2E_ADMIN_BIN", adminBin), [
        "generate-key",
      ]);
      const agentOutput = await run(required("MAJU_E2E_ADMIN_BIN", adminBin), [
        "generate-key",
      ]);
      ...
      for (const relay of specs) {
        ...
        const senderEnv = {
          MAJU_RELAY_URL: relayHttp,
          MAJU_PRIVATE_KEY: senderKey,
        };
        const created = JSON.parse(
          await run(
            required("MAJU_E2E_CLI_BIN", cliBin),
            [
              "channels",
              "create",
              "--name",
        ...
        await run(
          ...
          { MAJU_RELAY_URL: relayHttp, MAJU_PRIVATE_KEY: agentKey },
        );
      ...
      for (const { relay } of channels) {
        acpChildren.push(
          await harness.startAcp(
            ...
            agentKey,
            { MAJU_ACP_RESPOND_TO: "anyone", MAJU_ACP_NO_MEMORY: "true" },
          ),
        );
      }
      ...
      for (const { relay, id } of channels) {
        ...
        await run(
          ...
          { MAJU_RELAY_URL: relayHttp, MAJU_PRIVATE_KEY: senderKey },
        );
