...
async function hasOutgoingEventWithContent(
  ...
  return page.evaluate((expectedContent) => {
    return (
      ...
      ).__MAJU_E2E_COMMAND_LOG__?.some((entry) => {
  ...
  await page.addInitScript(
    ({ alicePubkey }) => {
      window.localStorage.setItem(
        "maju-user-labels.v1:ws://localhost:3000",
        JSON.stringify({
          version: 1,
          updatedAt: Date.now(),
          profiles: {
...
test("start a new direct message from the sidebar", async ({ page }) => {
  ...
  await expect(page.locator(".maju-poof-burst")).toHaveCount(1);
  await page.mouse.up();
...
test("closes direct message results while opening", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __MAJU_E2E__?: { mock?: { openDmDelayMs?: number } };
    };
    testWindow.__MAJU_E2E__ ??= {};
    testWindow.__MAJU_E2E__.mock ??= {};
    testWindow.__MAJU_E2E__.mock.openDmDelayMs = 1_000;
  });
...
test("opens a sent direct message without waiting for a channel-list refresh", async ({
  ...
  await page.evaluate(() => {
    ...
    testWindow.__MAJU_E2E__ ??= {};
    testWindow.__MAJU_E2E__.mock ??= {};
    testWindow.__MAJU_E2E__.mock.channelsReadDelayMs = 3_000;
  });
...
test("create ephemeral stream shows sidebar and header affordances", async ({
  ...
  await page.evaluate(
    ({ agentPubkey, channelId: activeChannelId }) => {
      (window as MockFeedWindow).__MAJU_E2E_SEED_ACTIVE_TURNS__?.({
        agentPubkey,
  ...
  await page.evaluate(
    ({ channelName: targetChannelName, mentionPubkey, senderPubkey }) => {
      (window as MockFeedWindow).__MAJU_E2E_EMIT_MOCK_MESSAGE__?.({
...
test("profile renders live activity for a viewer-owned relay agent", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__MAJU_E2E_SEED_ACTIVE_TURNS__ ===
      "function",
  );
  await page.evaluate(
    ({ agentPubkey, channelId }) => {
      const seedActiveTurns = (window as MockFeedWindow)
        .__MAJU_E2E_SEED_ACTIVE_TURNS__;
      if (!seedActiveTurns) {
...
test("channel management keeps both description paragraphs visible for editors and readers", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof window.__MAJU_E2E_MUTATE_CHANNEL__ === "function" &&
      typeof window.__MAJU_E2E_INVALIDATE_CHANNELS__ === "function",
  );
  await page.evaluate(
    async ({ generalChannelId, randomChannelId, description }) => {
      ...
      for (const channelId of [generalChannelId, randomChannelId]) {
        bridge.__MAJU_E2E_MUTATE_CHANNEL__({ channelId, description });
      }
      await bridge.__MAJU_E2E_INVALIDATE_CHANNELS__();
    },
    {
...
test("channel settings only prompt editors to add an empty description", async ({
  ...
  await page.evaluate(
    async ({ generalChannelId, randomChannelId }) => {
      ...
      bridge.__MAJU_E2E_MUTATE_CHANNEL__({
...
async function seedHomeInboxMention(
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__MAJU_E2E_PUSH_MOCK_FEED_ITEM__ ===
      "function",
  );
  ...
  await page.evaluate(
    ({
      ...
      const pushFeedItem = (window as MockFeedWindow)
        .__MAJU_E2E_PUSH_MOCK_FEED_ITEM__;
      if (!pushFeedItem) {
...
test("Inbox All excludes generic channel traffic", async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return typeof win.__MAJU_E2E_PUSH_MOCK_FEED_ITEM__ === "function";
  });
...
test("Inbox type labels keep the same height with and without a channel chip", async ({
  ...
  await page.waitForFunction(() => {
    const win = window as MockFeedWindow;
    return (
      typeof win.__MAJU_E2E_EMIT_MOCK_MESSAGE__ === "function" &&
      typeof win.__MAJU_E2E_PUSH_MOCK_FEED_ITEM__ === "function"
    );
  });
  ...
  await page.evaluate(
    ({
      ...
      const win = window as MockFeedWindow;
      const emitMessage = win.__MAJU_E2E_EMIT_MOCK_MESSAGE__;
      const pushFeedItem = win.__MAJU_E2E_PUSH_MOCK_FEED_ITEM__;
...
test("Inbox All never lists drafts and unread-only hides reminders", async ({
  ...
  await page.addInitScript(
    ...
    },
    {
      ...
      draftStoreKey: `maju-drafts.v2:ws://localhost:3000:${MOCK_IDENTITY_PUBKEY}`,
    },
  );
  await page.goto("/");
  await page.waitForFunction(() => {
  ...
  await page.evaluate(
    async ({
      ...
      window.__MAJU_E2E_SEED_MOCK_REMINDERS__?.([
        {
          id: reminderId,
...
test("stopping a managed bot in one community leaves its other communities running", async ({
  ...
  // (`seedDefaultCommunity` follows MAJU_E2E_RELAY_URL); the agent also has a
