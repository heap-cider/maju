...
// pickers render for it just as they do for maju-agent.
...
/**
 * Wait for the LLM provider field to become visible (maju-agent
...
test.describe("agent readiness gate screenshots", () => {
  ...
  test("01-create-majuagent-uses-ai-defaults", async ({ page }) => {
    ...
    await page.getByRole("dialog").screenshot({
      path: `${SHOTS}/01-create-majuagent-uses-ai-defaults.png`,
    });
  });

  // Shot 02: customized Anthropic configuration uses Automatic model instead
  // of presenting an obsolete model-required error.
  test("02-create-majuagent-automatic-model", async ({ page }) => {
    await installMockBridge(page);
    await openCreateDialog(page);
    await selectProvider(page, "Maju shared compute");
    ...
    await page.getByRole("dialog").screenshot({
      path: `${SHOTS}/02-create-majuagent-automatic-model.png`,
    });
  });
  ...
  test("05-create-cli-login-runtime-no-provider-required", async ({ page }) => {
    ...
    // Switch the auto-selected maju-agent runtime to Claude Code.
    await selectDropdownOption(
      page,
  ...
  // Shot 08: goose runtime, provider empty + no global → save BLOCKED (same rule as maju-agent).
