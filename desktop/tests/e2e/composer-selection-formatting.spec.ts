...
test("right-clicking selected composer text hides the selection formatter", async ({
  ...
  await input.evaluate((element) => {
    (
      window as Window & {
        __MAJU_E2E_CONTEXTMENU_DEFAULT_PREVENTED__?: boolean;
      }
    ).__MAJU_E2E_CONTEXTMENU_DEFAULT_PREVENTED__ = false;
    element.addEventListener(
      "contextmenu",
      (event) => {
        (
        ...
        ).__MAJU_E2E_CONTEXTMENU_DEFAULT_PREVENTED__ = event.defaultPrevented;
      },
      { once: true },
    );
  });
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__MAJU_E2E_CONTEXTMENU_DEFAULT_PREVENTED__,
      ),
    )
    .toBe(false);
