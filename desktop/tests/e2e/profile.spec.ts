...
test("reveals emoji background colors only after choosing an emoji", async ({
  ...
  const collapsedColorGridBox = await colorGridShell.boundingBox();
  ...
  const expandedColorGridBox = await colorGridShell.boundingBox();
  expect(expandedColorGridBox?.height ?? 0).toBeGreaterThan(
    (collapsedColorGridBox?.height ?? 0) + 8,
  );
});
  ...
  const emitAgentPresence = (status: "online" | "offline") =>
    page.evaluate(
      ({ pubkey, status }) => {
        ...
        ).__MAJU_E2E_EMIT_MOCK_PRESENCE__;
      ...
      return {
        heroHeight: Number.parseFloat(
          getComputedStyle(layout).getPropertyValue(
            "--maju-profile-sticky-hero-height",
          ),
        ),
...
test("storage clear resets composed font size and keyboard zoom across windows", async ({
  ...
  const readTypographyState = () =>
    page.evaluate(() => {
      ...
      probe.style.fontSize = "var(--maju-type-rem)";
