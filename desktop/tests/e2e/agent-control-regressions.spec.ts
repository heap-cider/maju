...
async function waitForActiveTurnSeed(page: Page) {
  await page.waitForFunction(
    () => typeof window.__MAJU_E2E_SEED_ACTIVE_TURNS__ === "function",
    null,
    { timeout: 10_000 },
  );
}
...
async function readControlRequests(page: Page): Promise<ControlRequest[]> {
  return page.evaluate(
    () => (window.__MAJU_E2E_OBSERVER_CONTROLS__ ?? []) as ControlRequest[],
  );
}
...
test.describe("agent control browser regressions", () => {
    ...
    await page.waitForFunction(
      () =>
        typeof (window as E2eControlWindow)
          .__MAJU_E2E_MOUNT_AGENT_SESSION_PANEL__ === "function",
    );
    await waitForActiveTurnSeed(page);
    await seedActiveTurn(page, CHANNEL_AGENTS);
    await page.evaluate(
      ({ agentPubkey, channelId }) =>
        (window as E2eControlWindow).__MAJU_E2E_MOUNT_AGENT_SESSION_PANEL__?.({
          agentPubkey,
          channelId,
        }),
  ...
  test("model switch sends requestId and reports ambiguous_target", async ({
    ...
    await page.goto("/", { waitUntil: "domcontentloaded" });
    await page.waitForFunction(
      () => typeof window.__MAJU_E2E_RUN_MODEL_SWITCH__ === "function",
    );
    const result = await page.evaluate(
      ({ agentPubkey, channelId, modelId, requestId }) =>
        (window as E2eControlWindow).__MAJU_E2E_RUN_MODEL_SWITCH__?.({
          agentPubkey,
