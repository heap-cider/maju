...
const DIFF_ENTRIES = [
  ...
  { field: "env.MAJU_LOG", change: { kind: "added" as const } },
  {
    field: "relay_url",
...
test.describe("restart-diff screenshots", () => {
  ...
  test("restart action meets WCAG AA contrast in dark mode", async ({
    ...
    await page.addInitScript(() => {
      window.localStorage.setItem("maju-theme", "catppuccin-mocha");
    });
    await installMockBridge(page, {
      managedAgents: [RESTART_AGENT],
    });
  ...
  test("08-runtime-tab-restart-banner-auto-on", async ({ page }) => {
    ...
    await expect(banner).toBeVisible({ timeout: 10_000 });
    await expect(
      banner.getByText("Maju can restart it automatically"),
    ).toBeVisible();
