...
async function sent(page: Page) {
  return page.evaluate(() => {
    ...
    return (window.__MAJU_E2E_COMMAND_LOG__ ?? []).flatMap((call) => {
  ...
  test(`owned ${role} with empty local roster emits exact p tag`, async ({
    ...
    await page.evaluate(
      async ({ pubkey, channelId, role }) => {
        await window.__MAJU_E2E_INVOKE_MOCK_COMMAND__?.("add_channel_members", {
          channelId,
          pubkeys: [pubkey],
          role,
        });
...
for (const error of [
  ...
  test(`failed add keeps draft and sends nothing: ${error}`, async ({
    ...
    await page.evaluate((error) => {
      ...
      window.__MAJU_E2E__.mock.addChannelMembersErrors = [error];
    }, error);
    await page.getByTestId("send-message").click();
...
test("membership revoked at final publish keeps draft and emits no message", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.relayAgentListErrors = [
      null,
      null,
