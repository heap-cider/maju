...
// Regression coverage for maju:// entity links posted WITHOUT sender
...
// maju://pr|issue|repo cards from #4695. These cards resolve their titles
...
test("agent-style message with angle-bracket maju:// links renders entity cards without snapshot tags", async ({
  page,
}) => {
  ...
  const prCard = row.locator('[data-link-preview="maju-pull-request"]');
  ...
  await expect(
    prCard.locator("[data-link-preview-hostname-maju-mark]"),
  ).toBeVisible();
  await expect(
  ...
  // image-less.
  const repoCard = row
    .locator('[data-link-preview="maju-repository"]')
  ...
  await expect(
    repoCard.locator("[data-link-preview-hostname-maju-mark]"),
  ).toBeVisible();
  await expect(
...
test("desktop composer shows entity card and send is not blocked by missing snapshot", async ({
  ...
  // maju:// links never produce snapshot tags, so the composer card must
  ...
  const repoCard = row.locator('[data-link-preview="maju-repository"]');
  await expect(repoCard).toBeVisible();
  await expect(repoCard).toContainText("relay-tools");
