...
// Regression coverage for Maju-native entity links after standalone cards were
...
test("agent-style Maju links stay chip-only with metadata tooltips", async ({
  page,
}) => {
  await page.addInitScript(
    ({ repoAddress, prId, issueId, alicePubkey, prSubject, issueSubject }) => {
      const createdAt = Math.floor(Date.now() / 1000) - 60;
      window.__MAJU_E2E_EXTRA_PROJECT_EVENTS__ = [
        {
          id: prId,
  ...
  // maju:// URLs in a Markdown list and NO link-preview snapshot tags.
  await page.evaluate(
    ({ prId, issueId, alicePubkey, externalHref, cloneHref }) => {
      window.__MAJU_E2E_EMIT_MOCK_MESSAGE__?.({
        ...
        content: [
          ...
          "",
          `- Pull request with enough leading context to wrap: <maju://pr?id=${prId}&owner=${alicePubkey}&d=relay-tools>`,
          `- Issue: <maju://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools>`,
          `- Repository: <maju://repo?owner=${alicePubkey}&d=relay-tools>`,
          `- Labeled issue: [triage this issue](maju://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools)`,
          `- Labeled clone: [clone relay-tools](${cloneHref})`,
          `- Missing repo: <maju://repo?owner=${alicePubkey}&d=missing-repo>`,
  ...
  await expect(row.locator('[data-link-preview^="maju-"]')).toHaveCount(0);
  ...
  const prContext = prTooltip.locator(
    '[data-maju-tooltip-metadata-content=""]',
  );
  ...
  const prFooter = prTooltip.locator('[data-maju-tooltip-metadata-type=""]');
  ...
  const issueChip = row.locator('[data-maju-link-kind="issue"]');
  ...
  await expect
    ...
    .toBe(true);
  await expect(
    issueTooltip.locator('[data-maju-tooltip-metadata-type=""]'),
  ...
  await expect(
    page
      .getByRole("tooltip")
      .locator('[data-maju-tooltip-metadata-content=""]'),
  ...
  await expect(
    repoTooltip.locator('[data-maju-tooltip-metadata-content=""]'),
  ).toContainText("Operator tooling and admin CLI for relay deployments.");
  await expect(
    repoTooltip.locator('[data-maju-tooltip-metadata-type=""]'),
  ...
  await expect(
    cloneTooltip.locator('[data-maju-tooltip-metadata-content=""]'),
  ).toContainText("Operator tooling and admin CLI for relay deployments.");
  ...
  await expect(missingRepoChip).not.toHaveClass(/maju-link-unavailable/);
  ...
  await expect(
    missingRepoTooltip.locator('[data-maju-tooltip-metadata-content=""]'),
  ).toHaveCount(0);
  const missingRepoFooter = missingRepoTooltip.locator(
    '[data-maju-tooltip-metadata-type=""]',
  );
  ...
  await page.evaluate(
    ({ issueId, alicePubkey }) => {
      window.__MAJU_E2E_EMIT_MOCK_MESSAGE__?.({
        channelName: "general",
        pubkey: alicePubkey,
        content: `Issue link: maju://issue?id=${issueId}&owner=${alicePubkey}&d=relay-tools`,
      });
    },
...
test("entity tooltip uses project context while relay metadata is delayed", async ({
  ...
  await page.getByTestId("channel-general").click();
  await page.evaluate(() =>
    window.__MAJU_E2E_ACTIVATE_RELAY_RATE_LIMIT__?.(300),
  );
  await page.waitForFunction(
    () => typeof window.__MAJU_E2E_EMIT_MOCK_MESSAGE__ === "function",
  );
  await page.evaluate(
    ({ issueId, owner }) => {
      window.__MAJU_E2E_EMIT_MOCK_MESSAGE__?.({
        channelName: "general",
        content: `Delayed issue: maju://issue?id=${issueId}&owner=${owner}&d=maju`,
      });
    },
  ...
  const issueChip = page.getByRole("button", {
    name: /Open issue .* in repository maju/,
  });
  await issueChip.hover();
  await expect(
    page
      .getByRole("tooltip")
      .locator('[data-maju-tooltip-metadata-content=""]'),
  ).toHaveText("maju · The complete Maju community platform.");
});

test("desktop composer and sent message keep Maju entities chip-only", async ({
  page,
}) => {
  await installMockBridge(page);
  await page.setViewportSize({ width: 900, height: 800 });
  ...
  const repoLink = `maju://repo?owner=${ALICE_PUBKEY}&d=relay-tools`;
  await page.getByTestId("message-input").fill(`Check out ${repoLink}`);

  // Maju-native links do not enter the standalone composer-preview surface.
...
test("composer classifies a same-relay clone URL as a repository chip, not a card", async ({
  ...
  const cloneHref = `${relayHttpUrl}/git/${DEFAULT_MOCK_PUBKEY}/maju.git`;
  ...
  const repoChip = row.getByRole("button", { name: "Open repository maju" });
  await expect(repoChip).toBeVisible();
  await repoChip.hover();
  await expect(
    page
  ...
  // canonical maju://repo target rather than being handed to the OS.
...
test("reopening the same entity link reapplies its workspace state", async ({
  ...
  const repoAddress = `30617:${DEFAULT_MOCK_PUBKEY}:maju`;
  await page.addInitScript(
  ...
  const repoLink = `maju://repo?owner=${DEFAULT_MOCK_PUBKEY}&d=maju&tab=prs`;
  const prLink = `maju://pr?id=${PR_ID}&owner=${DEFAULT_MOCK_PUBKEY}&d=maju`;
  const issueLink = `maju://issue?id=${ISSUE_ID}&owner=${DEFAULT_MOCK_PUBKEY}&d=maju`;
...
test("deleted reply links identify deletion and fall back to their thread root", async ({
  ...
  const link = `maju://message?channel=${channelId}&id=${deletedReplyId}&thread=${threadRootId}`;
...
test("deleted top-level message links identify deletion and fall back to channel navigation", async ({
  ...
  await page.addInitScript(() => {
    window.localStorage.setItem("maju-theme", "maju-dark");
    window.localStorage.setItem("maju-accent-color", "#c0a2f1");
  });
  const missingMessageId = "d".repeat(64);
  const channelId = "9dae0116-799b-5071-a0a8-fdd30a91a35d";
  const link = `maju://message?channel=${channelId}&id=${missingMessageId}`;
  ...
  await expect(deletedLink).toHaveClass(/maju-link-deleted/);
  await expect(deletedLink).not.toHaveClass(/maju-link-unavailable/);
  const unavailableColors = await linkMessage.evaluate((message) => {
    const probe = document.createElement("span");
    probe.className = "mention-chip maju-link-unavailable";
...
test("cold-start entity links drain after the React listener mounts", async ({
  ...
  const href = `maju://repo?owner=${DEFAULT_MOCK_PUBKEY}&d=maju&tab=prs`;
  await installMockBridge(page, {
