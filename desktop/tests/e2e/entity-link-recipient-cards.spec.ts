...
test("agent-style message with bare maju:// links renders entity cards without snapshot tags", async ({
  page,
}) => {
  ...
  // Simulate an agent/CLI sender: plain kind-9 message with bare maju://
    ...
    ({ prId, alicePubkey }) => {
      window.__MAJU_E2E_EMIT_MOCK_MESSAGE__?.({
        ...
        content: [
          ...
          `maju://pr?id=${prId}&owner=${alicePubkey}&d=relay-tools`,
          `Repo: maju://repo?owner=${alicePubkey}&d=relay-tools`,
        ].join("\n"),
      });
    },
