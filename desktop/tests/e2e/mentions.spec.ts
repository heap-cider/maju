...
test.afterEach(async ({ page }, testInfo) => {
  if (testInfo.status !== testInfo.expectedStatus) {
    await testInfo.attach("outgoing-diagnostic", {
      body: JSON.stringify(
        await page.evaluate(() => ({
          events: window.__MAJU_E2E_SIGNED_EVENTS__,
          commands: window.__MAJU_E2E_COMMAND_LOG__,
        })),
      ),
...
test("duplicate owned agents preserve provenance and exact pubkey selection", async ({
  ...
  await page.evaluate(
    async ({ channelId, pubkey }) => {
      const invoke = window.__MAJU_E2E_INVOKE_MOCK_COMMAND__;
...
test("cached relay-agent suggestions are removed when channel authorization disappears", async ({
  ...
  await page.evaluate(async (channelId) => {
    ...
    bridge.__MAJU_E2E_MUTATE_CHANNEL__?.({ channelId, channelType: null });
...
test("forum sends revalidate relay-agent authorization before signing", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.agentListDelayMs = 1_000;
    window.__MAJU_E2E__.mock.relayAgentListErrors = Array(100).fill(
...
test("managed agents use the channel roster for membership labels", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (channelId) =>
          window.__MAJU_E2E_QUERY_CLIENT__?.getQueryState([
            "channels",
            channelId,
            "members",
          ])?.status,
...
test("relay-agent directory errors fail closed and recover after a fresh fetch", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.agentListDelayMs = 1_000;
    void window.__MAJU_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["relay-agents"],
    });
  });
  await expect
    .poll(async () =>
      page.evaluate(
        () =>
          window.__MAJU_E2E_QUERY_CLIENT__?.getQueryState(["relay-agents"])
            ?.fetchStatus,
      ),
    )
    .toBe("fetching");
...
test("managed agents keep their p tag when relay discovery fails before send", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.relayAgentListErrors = Array(5).fill(
...
test("targeted revocation before send causes no agent side effects", async ({
  ...
  await page.evaluate((pubkey) => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.relayAgentRevalidationRevokedPubkeys = [pubkey];
...
test("sends that attach a mentioned agent revalidate at the publish boundary", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.addChannelMembersDelayMs = 1_500;
  });
  const baselineCommands = await readCommandLog(page);
  await page.getByTestId("send-message").click();
  await expect(page.getByRole("alertdialog")).toHaveCount(0);
...
test("a send held open by a no-write step still revalidates at the publish boundary", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () => window.__MAJU_E2E_RELEASE_HUDDLE_AGENT_SYNCS__?.() ?? 0,
      ),
    )
    .toBeGreaterThan(0);
...
test("selected relay agents revoked during send emit no p tag", async ({
  ...
  await page.evaluate(() => {
    window.__MAJU_E2E__.mock ??= {};
    window.__MAJU_E2E__.mock.agentListDelayMs = 300;
  });
  await page.getByTestId("send-message").click();
...
test("a deploy held across an A→B→A community round-trip is not fired twice", async ({
  ...
  const released = await page.evaluate(
    () =>
      ...
      ).__MAJU_E2E_RELEASE_MANAGED_AGENT_STARTS__?.() ?? 0,
  );
