...
/**
 * SCROLL-BACK latency profile for one channel (#maju-bugs) against a LIVE
 ...
 *   MAJU_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   MAJU_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   MAJU_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-majubugs.perf.ts
 */

const RELAY_HTTP =
  process.env.MAJU_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.MAJU_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.MAJU_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.MAJU_PERF_CHANNEL ?? "maju-bugs";
const PAGES = Number(process.env.MAJU_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "maju:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "maju-onboarding-complete.v1:";
const WELCOME_PREFIX = "maju-welcome-channel-ensured.v2:";
...
function deriveIdentity(nsec: string) {
  ...
  if (decoded.type !== "nsec") throw new Error("MAJU_PERF_NSEC is not an nsec");
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set MAJU_PERF_NSEC to a real member nsec");
  ...
  await page.addInitScript(
    ({ ident, onboardingPrefix, welcomePrefix, relayUrl, overrideKey }) => {
      ...
      const w = window as unknown as { __MAJU_E2E__?: Record<string, unknown> };
      w.__MAJU_E2E__ = { ...(w.__MAJU_E2E__ ?? {}), identity: ident };
    },
    {
      ident: identity,
