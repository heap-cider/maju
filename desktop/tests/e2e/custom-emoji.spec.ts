...
//
// The `:maju:` shortcode lives in a member-authored kind:30030 set
// (d=`maju:custom-emoji`) served by the mock bridge from two distinct
    ...
    .poll(async () =>
      page.evaluate(
        ({ ch }) =>
          ...
          ).__MAJU_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({ channelName: ch }) ??
          false,
        { ch: channelName },
      ),
    )
...
// real 127.0.0.1 URL rather than the maju-media:// fallback.
...
async function quickReactionStorageContains(
  ...
  return page.evaluate((selectedEmoji) => {
    for (let index = 0; index < window.localStorage.length; index += 1) {
      ...
      if (!key?.startsWith("maju.quick-reaction-emojis.v1")) continue;
...
test("adding a custom emoji while editing keeps the image after save (Bug 2)", async ({
  ...
  // <img>, not a bare `:maju:`. The pre-fix edit path shipped no emoji tags,
