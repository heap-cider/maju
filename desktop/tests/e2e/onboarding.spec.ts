...
async function setRelayConnectionState(
  ...
  await page.waitForFunction(
    () =>
      typeof (
        window as Window & {
          __MAJU_E2E_SET_RELAY_CONNECTION_STATE__?: unknown;
        }
      ).__MAJU_E2E_SET_RELAY_CONNECTION_STATE__ === "function",
  );
  await page.evaluate((nextState) => {
    ...
    const setConnectionState =
      testWindow.__MAJU_E2E_SET_RELAY_CONNECTION_STATE__;
...
const HOME_SEEN_STORAGE_KEY_PREFIX = "maju-home-feed-seen.v1:";
...
async function seedOnboardingCompletion(page: Page, pubkey: string) {
  await page.addInitScript(
    ...
    {
      storageKey: `maju-onboarding-complete.v1:${pubkey}`,
    },
  );
}
...
async function getMockChannels(page: Page) {
  return page.evaluate(async () => {
    ...
    const invoke =
      bridgeWindow.__MAJU_E2E_INVOKE_MOCK_COMMAND__ ??
...
async function seedCurrentAvatar(page: Page, avatarUrl: string) {
  await page.waitForFunction(() => {
    ...
    return (
      typeof bridgeWindow.__MAJU_E2E_INVOKE_MOCK_COMMAND__ === "function" ||
...
test("fresh existing-identity path leads with private-key recovery", async ({
  ...
  await expect(
    page.getByText("Paste your private key to sign in to Maju."),
  ).toBeVisible();
  ...
  await expect(
    phoneDialog.getByRole("heading", { name: "Use your Maju identity" }),
  ).toBeVisible();
...
test("community onboarding reuses an existing relay profile", async ({
    ...
    ({ pubkey, transactionStorageKey }) => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          stage: "profile",
          relayUrl: "wss://onboarding.communities.maju.xyz",
  ...
  await installMockBridge(
    ...
    {
      relayWsUrl: "wss://onboarding.communities.maju.xyz",
      skipOnboardingSeed: true,
    },
  );
  await page.goto("/");
  ...
  await expect(
    page
      ...
      .locator(".maju-onboarding-transition-line"),
...
test("first-community direct join cancel returns to request access", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (storageKey) => ({
          communities: window.localStorage.getItem("maju-communities"),
...
test("connected first-community profile keeps Back bottom-left and balances the avatar editor", async ({
  ...
  await expect(liveEmoji).toHaveClass(/maju-avatar-squish/);
  await expect(
...
test("replacing a pending upload disposes its verifier and local preview", async ({
  ...
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __MAJU_E2E_REVOKED_OBJECT_URLS__?: string[];
    };
    const revokedUrls: string[] = [];
    const revokeObjectUrl = URL.revokeObjectURL.bind(URL);
    testWindow.__MAJU_E2E_REVOKED_OBJECT_URLS__ = revokedUrls;
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return testWindow.__MAJU_E2E_REVOKED_OBJECT_URLS__ ?? [];
      }),
    )
...
test("no-event profile cached then reloaded still sees onboarding", async ({
  ...
  const SELF_PROFILE_CACHE_KEY = `maju-self-profile.v1:ws://localhost:3000:${TEST_IDENTITIES.tyler.pubkey}`;
  await page.addInitScript(
    ({ key, cache }) => {
...
test("failed avatar saves can continue without saving the avatar", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __MAJU_E2E__?: { mock?: { profileUpdateError?: string } };
    };
    if (testWindow.__MAJU_E2E__?.mock) {
      testWindow.__MAJU_E2E__.mock.profileUpdateError =
...
async function commandCount(page: Page, command: string) {
  return page.evaluate(
    (target) =>
      window.__MAJU_E2E_COMMANDS__?.filter((entry) => entry === target)
        .length ?? 0,
    command,
  );
}
...
test("first-run onboarding posts the live Fizz kickoff", async ({ page }) => {
  ...
  await expect(page.getByTestId("message-timeline")).not.toContainText(
    "Hi Morty QA, I'm Fizz. Welcome to Maju.",
  );
  await publishWelcomeTeamPresence(page);
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__MAJU_E2E_COMMANDS__?.includes("plugin:websocket|disconnect") ??
          false,
      ),
    )
    .toBe(true);
...
test("same-relay identity replacement rebuilds the community boundary (A→B→A cache and draft isolation)", async ({
  ...
  const relayScope = (process.env.MAJU_E2E_RELAY_URL ?? "http://localhost:3000")
    .replace(/^http/, "ws")
    .replace(/\/+$/, "");
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      ...
      __MAJU_E2E_PREVIOUS_QUERY_CLIENT__?: unknown;
    };
    const client = testWindow.__MAJU_E2E_QUERY_CLIENT__;
    if (!client) {
    ...
    testWindow.__MAJU_E2E_PREVIOUS_QUERY_CLIENT__ = client;
  });
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        if (client === testWindow.__MAJU_E2E_PREVIOUS_QUERY_CLIENT__) {
...
test("denied on relay A then paste relay B invite URL switches community to B", async ({
  ...
  await page
    .getByLabel("I agree to the Maju Terms of Service and Privacy Policy.")
    .check();
  await page.getByTestId("invite-redeem-submit").click();
