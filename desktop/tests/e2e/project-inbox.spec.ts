...
const MAJU_REPO_ADDRESS = `30617:${DEFAULT_MOCK_PUBKEY}:maju`;

test("Maju Git pull request renders and stays actionable in Inbox", async ({
  page,
}) => {
  ...
  const repositoryCardBody = page
    .getByTestId("repository-card-maju")
  ...
  await page.evaluate(
    ({ author, id, repoAddress, viewer }) => {
      window.__MAJU_E2E_PUSH_MOCK_FEED_ITEM__?.({
        id,
        kind: 1618,
        pubkey: author,
