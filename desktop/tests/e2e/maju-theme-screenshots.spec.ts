...
const GLASS_BACKGROUND_STORAGE_KEY = "maju-glass-background";
const GLASS_OPACITY_STORAGE_KEY = "maju-glass-opacity";
const PROMINENT_ACTIVE_TAB_STORAGE_KEY = "maju-prominent-active-tab";
const CONVERSATION_DENSITY_STORAGE_KEY = "maju.appearance.conversationDensity";
const FONT_SIZE_STORAGE_KEY = "maju.appearance.fontSize";
...
test("appearance groups theme and preferences into labeled rows", async ({
  ...
  await themeCard.getByTestId("theme-option-maju").click();
  await expect(themeStyleTrigger).toHaveAttribute("aria-expanded", "true");
  await expect(themeStyleOptions).toBeVisible();
  await expect(
    themeCard.getByTestId("prominent-active-tab-toggle"),
...
test("app font size and conversation density apply independently", async ({
  ...
  // type tokens ride on `--maju-type-rem`, which itself derives from the
  ...
  const readScale = () =>
    root.evaluate((element) => {
      const PROBE_ID = "maju-e2e-conversation-scale-probe";
      const tokens = {
  ...
  await expect(densityDescription).toHaveText(
    "Spacing in conversations and Markdown content across Maju",
  );
...
for (const { mode, theme } of [
  ...
  test(`${theme} ignores the Maju prominent preference`, async ({ page }) => {
    await seedTheme(page, theme);
    await page.addInitScript(
      ({ key }) => window.localStorage.setItem(key, "true"),
      { key: PROMINENT_ACTIVE_TAB_STORAGE_KEY },
...
test("glass background keeps the content panel solid", async ({ page }) => {
  ...
  await expect(page.locator(".maju-theme-gradient-underlay")).not.toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect(opacitySlider).toHaveClass(/maju-avatar-framing-slider/);
  ...
  await expect(
    opacitySlider.locator(".maju-avatar-framing-slider-handle"),
  ).toHaveCSS("opacity", "1");
  await expect(root).toHaveAttribute("data-glass-background", "");
  const majuSettingOrder = await page
  ...
  expect(majuSettingOrder).toEqual([
  ...
  await expect(page.locator(".maju-theme-gradient-underlay")).toHaveCSS(
    "background-image",
    "none",
  );
...
test("non-Maju glass preserves the selected theme sidebar tint", async ({
  page,
}) => {
  ...
  const root = page.locator("html");
  await expect(root).not.toHaveAttribute("data-maju-sidebar", "");
  await page.getByTestId("glass-background-toggle").click();
  await expect(root).toHaveAttribute("data-glass-background", "");

  const tint = await page
    .locator(".maju-theme-gradient-layer")
    .evaluate((element) => {
...
test("accent picker reveals/hides when toggling Maju", async ({ page }) => {
  ...
  const nonMajuSettingOrder = await page
    .getByTestId("appearance-theme-card")
    .locator(
    ...
    .evaluateAll((rows) => rows.map((row) => row.getAttribute("data-testid")));
  expect(nonMajuSettingOrder).toEqual([
    "appearance-color-mode-row",
    "theme-style-row",
