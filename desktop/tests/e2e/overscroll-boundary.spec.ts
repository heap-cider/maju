...
test("locks viewport rubber-band outside conversation scrollers", async ({
  ...
  // Maju Term consumes wheel gestures as custom scrollback rather than through
...
test("locks horizontal viewport pan everywhere", async ({ page }) => {
  ...
  // space locked even if a future layout places Maju content inside it.
  await page.evaluate(() => {
    const terminal = document.querySelector<HTMLElement>(
      '[data-testid="terminal-wheel-target"]',
    );
    const majuContent = document.createElement("div");
    majuContent.dataset.testid = "concealed-terminal-dead-space";
    terminal?.append(majuContent);
    if (terminal) terminal.dataset.terminalOwner = "maju";
  });
  await expect(
