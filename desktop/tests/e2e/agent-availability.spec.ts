...
test("saved deployment with offline presence is not shown as online", async ({
  ...
  for (const status of ["online", "away", "offline"] as const) {
    await page.evaluate(
      ({ pubkey, status }) => {
        const emit = window.__MAJU_E2E_EMIT_MOCK_PRESENCE__;
    ...
    if (status === "online") {
      ...
      expect(
        await page.evaluate(() =>
          (window.__MAJU_E2E_COMMANDS__ ?? []).filter((command) =>
  ...
  await page.evaluate(() =>
    window.__MAJU_E2E_SET_RELAY_CONNECTION_STATE__?.("disconnected"),
  );
...
test("missing snapshot is offline but failed reads cannot reuse cached online", async ({
  ...
  for (const response of ["online", "missing", "online", "error"] as const) {
    await page.evaluate(async (response) => {
      ...
      if (!w.__MAJU_E2E_QUERY_CLIENT__)
        throw new Error("Query client unavailable");
      await w.__MAJU_E2E_QUERY_CLIENT__.invalidateQueries({
        queryKey: ["presence"],
      });
    }, response);
    const label =
...
test("stopped local with authored presence has a dot, not an invokable Start", async ({
  ...
  await page.evaluate(
    (pubkey) =>
      window.__MAJU_E2E_EMIT_MOCK_PRESENCE__?.({ pubkey, status: "online" }),
    LOCAL,
  );
  await expect(
...
test("member menu cannot start a present stopped local runtime", async ({
  ...
  await page.evaluate(
    (pubkey) =>
      window.__MAJU_E2E_EMIT_MOCK_PRESENCE__?.({ pubkey, status: "away" }),
    LOCAL,
  );
  await row.hover();
  ...
  await page.evaluate(
    (pubkey) =>
      window.__MAJU_E2E_EMIT_MOCK_PRESENCE__?.({ pubkey, status: "offline" }),
    LOCAL,
  );
