...
const THEMES = ["maju", "catppuccin-mocha"] as const;

async function seedTheme(page: Page, theme: (typeof THEMES)[number]) {
  await page.addInitScript((value) => {
    window.localStorage.setItem("maju-theme", value);
  }, theme);
}
...
for (const theme of THEMES) {
  ...
  test(`link tooltip supporting text uses the muted secondary foreground — ${theme}`, async ({
    ...
    const composerChips = composer.locator('[data-composer-maju-link=""]');
    ...
    await expectMutedSupportingText(
      row.getByRole("button", {
        name: /Open issue .* in repository maju-world/,
      }),
      "Issue · maju-world",
    );
  });
