...
//!
//! Maju's own process holds `MAJU_PRIVATE_KEY` (an nsec), `MAJU_AUTH_TAG`, and
...
/// Keys the child is allowed to inherit from Maju's own environment.
///
...
/// Values Maju sets on the child unconditionally, overriding any inherited
...
/// it is injected rather than inherited. Maju's own `SHELL` and the shell we
...
pub fn fence_env(cmd: &mut CommandBuilder, path: &str, shell: &str) {
    ...
    // 3. Apply Maju's own terminal identity.
