...
//!
//! Maju's own process runs under Hermit activation, so its `PATH` leads with
//! the repo's hermit `bin` and the hermit cache. Inheriting that verbatim
//! hands the user a shell whose `cargo`, `node`, and `python` are Maju's
...
//! answer for the user's machine, not for Maju's build.
//!
...
//! Maju never been in the picture.

/// The default user `PATH` for a spawned shell.
///
/// This intentionally does not consult Maju's own `PATH`. A login shell reads
...
/// normally instead of layering it on top of Maju's build toolchain.
#[cfg(unix)]
