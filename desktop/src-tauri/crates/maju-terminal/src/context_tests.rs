...
/// `MAJU_CHANNEL_ID` is always the UUID, so a script has an identifier that no
...
fn channel_id_is_never_the_name() {
    ...
    let id = vars.iter().find(|(k, _)| *k == "MAJU_CHANNEL_ID").unwrap();
    assert_eq!(
        id.1, UUID,
...
/// Absent rather than empty: `${MAJU_THREAD_ID+set}` and `-n` must agree.
#[test]
fn thread_id_is_absent_when_there_is_no_thread() {
    let vars = context_vars(&context_named("maju-tui"));
    assert!(!vars.iter().any(|(k, _)| *k == "MAJU_THREAD_ID"));

    let mut context = context_named("maju-tui");
    context.thread_id = Some("thread-1".to_owned());
    let vars = context_vars(&context);
    assert_eq!(
        vars.iter()
            .find(|(k, _)| *k == "MAJU_THREAD_ID")
            .map(|(_, v)| v.as_str()),
        Some("thread-1")
    );
}
...
fn every_injected_key_is_well_formed() {
    for (key, _) in context_vars(&context_named("maju-tui")) {
        assert!(
...
fn well_formed_key_rejects_the_equals_bypass() {
    for good in ["MAJU_CHANNEL", "_UNDERSCORE", "A1"] {
