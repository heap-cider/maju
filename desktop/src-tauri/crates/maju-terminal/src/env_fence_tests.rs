...
/// Secrets Maju's own process holds. Sourced from the desktop crate's
...
/// Uniquely-named executable seeded into Maju's own PATH; the child must not
/// be able to run it.
const CANARY_BIN: &str = "maju-hermit-canary-tool";
...
/// canary value they set themselves, so a real `MAJU_PRIVATE_KEY` in the
...
fn fence_still_delivers_the_terminal_contract() {
    ...
    for (key, value) in [("TERM", "xterm-256color"), ("TERM_PROGRAM", "Maju")] {
        assert!(
...
fn fence_excludes_keys_it_has_never_heard_of() {
    std::env::set_var("MAJU_SOME_FUTURE_CREDENTIAL", CANARY);
    let out = fenced_child_environment();

    assert!(
        !out.contains("MAJU_SOME_FUTURE_CREDENTIAL"),
        "an unknown key reached the child:\n{out}"
    );
}

/// `PATH` is constructed, not inherited, so Maju's Hermit build toolchain
...
#[test]
fn child_path_is_free_of_maju_toolchain() {
    let dir = std::env::temp_dir().join("maju-terminal-path-canary");
    ...
    // Stand in for Hermit activation: Maju's own PATH leads with a directory
    ...
    assert!(
        !path_line.contains("maju-terminal-path-canary"),
        "Maju's toolchain leaked into the child PATH: {path_line}"
    );
    ...
    assert!(
        ...
        "a Maju-only executable was reachable from the child shell: {resolved}"
    );
}
...
fn resolve_shell_rejects_a_file_the_user_cannot_execute() {
    ...
    let path = fixture_file("maju-terminal-group-only-exec", "#!/bin/sh\ntrue\n", 0o010);
...
fn resolve_shell_rejects_a_non_executable_file() {
    let path = fixture_file("maju-terminal-not-a-shell", "not a shell", 0o644);
    assert_ne!(resolve_shell(path.to_str()), path.to_str().unwrap());
}
...
/// The child must be told the shell we actually spawned, not the one Maju
