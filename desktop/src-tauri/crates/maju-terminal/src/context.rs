...
//! shells interpolate into prompts. A `PS1` containing `$MAJU_CHANNEL` turns a
...
/// Returns true if `name` is safe to expose as `MAJU_CHANNEL`.
///
...
/// The value to expose as `MAJU_CHANNEL`: the name when it is safe, otherwise
/// the channel UUID.
...
/// first `=`. A key `MAJU_CHANNEL=x` with value `y` lands as
/// `MAJU_CHANNEL=x=y`, so `getenv("MAJU_CHANNEL")` returns `"x=y"` — a way to
...
///
/// `MAJU_CHANNEL` carries the validated display value; `MAJU_CHANNEL_ID` is
...
pub fn context_vars(context: &GuiContext) -> Vec<(&'static str, String)> {
    let mut vars = vec![
        ("MAJU_CHANNEL_ID", context.channel_id.clone()),
        ("MAJU_CHANNEL", channel_display(context).to_owned()),
        ("MAJU_NPUB", context.npub.clone()),
        ("MAJU_RELAY_URL", context.relay_url.clone()),
        ("MAJU_TERM_SESSION", context.session_id.clone()),
        ("MAJU_TERM_VERSION", env!("CARGO_PKG_VERSION").to_owned()),
    ];
    // Absent rather than empty when the user is not in a thread: `-n
    // "$MAJU_THREAD_ID"` and `${MAJU_THREAD_ID+set}` should agree.
    if let Some(thread_id) = &context.thread_id {
        vars.push(("MAJU_THREAD_ID", thread_id.clone()));
    }
    vars
}
