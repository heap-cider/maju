...
use maju_terminal::damage::Encoder;
use maju_terminal::fences::Fences;
use maju_terminal::{SharedTerminal, Size, Terminal};
...
fn flood(shared: &SharedTerminal, stop: &AtomicBool) {
    ...
    while payload.len() < (2 << 20) {
        payload.extend_from_slice(b"\x1b[1;32mmaju\x1b[0m substrate line of output 0123456789\r\n");
    }
...
fn measure(fences: Fences) -> maju_terminal::AcquireStats {
    let size = Size {
        columns: 200,
        screen_lines: 50,
        scrollback: 10_000,
