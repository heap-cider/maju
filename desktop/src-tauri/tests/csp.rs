...
#[test]
fn media_directives_allow_the_maju_media_scheme() {
    // `rewriteRelayUrl` emits `maju-media://localhost/...` until the loopback
    // proxy port resolves, so cold-start media renders through the custom
    // scheme (mapped to `http://maju-media.localhost` on Windows).
    for directive in ["img-src", "media-src", "connect-src"] {
        let allowed = sources(directive);
        assert!(
            allowed.contains(&"maju-media:".to_owned()),
            "{directive} must allow maju-media:"
        );
        assert!(
            allowed.contains(&"http://maju-media.localhost".to_owned()),
            "{directive} must allow http://maju-media.localhost"
        );
    }
}
