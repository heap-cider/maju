...
pub fn run_event_sync(
    ...
    reconcile_team_catalog_heads(app, store_dir, owner_keys, db_path);
    reconcile_deleted_heads(store_dir, owner_keys, db_path);
    Ok(())
}
...
pub async fn run_event_sync_blocking(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        run_event_sync(Some(&app), &owner_keys, &db_path, &store_dir, &bootstrap)
    })
    .await
...
fn migrate_personas_in_dir_at(
    ...
    for record in &records {
        ...
        scoped_record.shared = existing
            ...
            .is_some_and(|event| maju_core_pkg::kind::event_is_shared(&event));
...
fn reconcile_team_catalog_heads(
    ...
    match reconcile_team_catalog_heads_core(app, store_dir, keys, db_path) {
        Ok(0) => {}
        Ok(reconciled) => {
            eprintln!(
                "maju-desktop: team-catalog-reconcile: {reconciled} shared team heads refreshed"
            );
        }
        Err(e) => {
            eprintln!("maju-desktop: team-catalog-reconcile: {e}");
        }
    }
}
...
fn reconcile_team_catalog_heads_core(
    ...
    use maju_core_pkg::kind::{event_is_shared, KIND_TEAM_CATALOG};
    use nostr::JsonUtil;
    ...
    for head in &all_heads {
        ...
        let Some(team) = teams.iter().find(|t| t.id == head.d_tag) else {
            ...
            eprintln!("maju-desktop: team-catalog-reconcile: tombstoning '{team_name}' — {reason}");
            ...
            if let Err(e) = tombstone_team_catalog_coordinate(db_path, keys, &head.d_tag) {
                eprintln!(
                    "maju-desktop: team-catalog-reconcile: tombstone failed for '{}': {e}",
                    head.d_tag
                );
            } else {
                reconciled += 1;
        ...
        let builder = match rebuilt {
            ...
            Err(reason) => {
                eprintln!(
                    "maju-desktop: team-catalog-reconcile: tombstoning '{}' — {reason}",
                    team.name
                );
...
fn emit_team_catalog_auto_retracted(app: &tauri::AppHandle, team_name: &str, reason: &str) {
    ...
    if let Err(e) = app.emit(
        ...
        eprintln!("maju-desktop: team-catalog-reconcile: failed to emit retraction notice: {e}");
    }
}
...
fn tombstone_orphan_heads(
    ...
    for head in get_retained_events_by_kind(conn, kind, pubkey)? {
        ...
        eprintln!(
            "maju-desktop: deletion-reconcile: tombstoning orphan kind:{kind} head '{}'",
            head.d_tag
        );
        match tombstone(db_path, keys, &head.d_tag) {
            Ok(()) => tombstoned += 1,
            Err(e) => eprintln!(
                "maju-desktop: deletion-reconcile: tombstone failed for kind:{kind} '{}': {e}",
                head.d_tag
            ),
        }
    }
...
fn reconcile_deleted_heads(store_dir: &Path, keys: &nostr::Keys, db_path: &Path) {
    match reconcile_deleted_heads_at(store_dir, keys, db_path) {
        Ok(0) => {}
        Ok(tombstoned) => {
            eprintln!("maju-desktop: deletion-reconcile: {tombstoned} orphan heads tombstoned");
        }
        Err(e) => eprintln!("maju-desktop: deletion-reconcile: {e}"),
    }
}
