...
/// `MAJU_PRIVATE_KEY` is process-global; serialize the env-mutating tests
/// so they don't race each other under the parallel test runner.
static ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Run `body` with `MAJU_PRIVATE_KEY` set to `value` (or unset when `None`),
/// restoring the prior value afterward.
fn with_env_key<T>(value: Option<&str>, body: impl FnOnce() -> T) -> T {
    let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let prior = std::env::var("MAJU_PRIVATE_KEY").ok();
    match value {
        Some(v) => std::env::set_var("MAJU_PRIVATE_KEY", v),
        None => std::env::remove_var("MAJU_PRIVATE_KEY"),
    }
    let out = body();
    match prior {
        Some(v) => std::env::set_var("MAJU_PRIVATE_KEY", v),
