...
fn test_demo_reset_preserves_shared_and_other_build_state() {
    ...
    let app_data = tmp
        ...
        .join("xyz.block.maju.app.demo.current-1234567812345678");
    let demo_nest = home.join(".maju-demo-current-1234567812345678");
    let prod_nest = home.join(".maju");
    let other_demo_nest = home.join(".maju-demo-other-8765432187654321");
    let shared_sprout = home.join(".sprout");
    let shared_agent = home.join(".config").join("maju-agent");
    let demo_config = home
        .join("Library")
        .join("Application Support")
        .join("maju-demo-current-1234567812345678");
    let demo_oauth = demo_config.join("maju-agent").join("oauth");
    let other_demo_config = home
        .join("Library")
        .join("Application Support")
        .join("maju-demo-other-8765432187654321");
    let other_demo_oauth = other_demo_config.join("maju-agent").join("oauth");
...
fn demo_config_delete_failure_keeps_sentinel_until_retry() {
    ...
    let token = config.join("maju-agent/oauth/databricks/token.json");
...
fn demo_oauth_permission_failure_preserves_retry_intent() {
    ...
    let app_data = make_app_data(&tmp);
    let config = tmp.path().join("demo-config");
    let oauth = config.join("maju-agent/oauth/databricks");
