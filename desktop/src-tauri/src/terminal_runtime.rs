//! Rust-owned PTY sessions and the typed Tauri transport for Maju Substrate.
...
use maju_terminal::context::{context_vars, GuiContext};
use maju_terminal::damage::{Frame, Style};
use maju_terminal::{Fences, SharedTerminal, Size, Terminal, Viewport};
...
impl maju_terminal::lifecycle::DrainingReader for ReaderThread {
...
struct Session {
    ...
    reader: Option<Box<dyn maju_terminal::lifecycle::DrainingReader + Send>>,
...
impl Session {
    ...
    fn shutdown(mut self) {
        ...
        if let Some(reader) = self.reader.take() {
            #[cfg(unix)]
            {
                let _ = maju_terminal::lifecycle::shutdown_draining(&mut self.child, reader);
            }
            #[cfg(not(unix))]
            {
...
pub(crate) fn terminal_attach(
    ...
    if let Some(existing_id) = request.session_id.as_deref() {
        ...
        let mut encoder = maju_terminal::damage::Encoder::new();
    ...
    let shell = {
        #[cfg(unix)]
        {
            maju_terminal::shell::resolve_shell(std::env::var("SHELL").ok().as_deref())
        }
        #[cfg(windows)]
        {
            maju_terminal::shell::resolve_shell(std::env::var("ComSpec").ok().as_deref())
        }
    };
    let mut command = CommandBuilder::new_default_prog();
    maju_terminal::env_fence::fence_env(
        &mut command,
        &maju_terminal::path::user_shell_path(),
        &shell,
    );
    ...
    std::thread::spawn(move || {
        while let Ok(action) = actions.recv() {
            let presentation = match &action {
                maju_terminal::Action::Title(title) => Some(TerminalMessage::Title(title.clone())),
                maju_terminal::Action::ResetTitle => Some(TerminalMessage::ResetTitle),
                maju_terminal::Action::Bell => Some(TerminalMessage::Bell),
                _ => None,
            };
            ...
            if let Some(reply) = maju_terminal::listener::reply(
                action,
                size.cols,
                size.rows,
                size.pixel_width,
    ...
    let reader_handle = std::thread::spawn(move || {
        ...
        let mut encoder = maju_terminal::damage::Encoder::new();
        let mut snapshot_encoder = maju_terminal::damage::Encoder::new();
        loop {
...
mod tests {
    use super::*;
    use maju_terminal::damage::{CursorFrame, RowFrame, Span};

    fn publication(spans: Vec<Span>) -> Publication {
        Publication {
            subscription_id: SubscriptionId::new(),
            sequence: 7,
            frame: maju_terminal::damage::Frame {
