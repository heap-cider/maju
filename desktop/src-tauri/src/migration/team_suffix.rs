...
//! `compose_prompt()` in maju-persona:
//!
//! ```text
//! {persona_prompt}\n\n---\n# Team Instructions\n{instructions}
//! ```
//!
//! `with_team()` in `maju-acp/src/pool.rs` now appends the LIVE
...
pub fn strip_baked_team_instructions(app: &tauri::AppHandle) {
    ...
    match strip_baked_team_instructions_in_dir(&base_dir) {
        ...
        Ok(stripped) => eprintln!(
            "maju-desktop: team-suffix-strip: removed the baked team-instructions suffix from \
             {stripped} record(s)"
        ),
        Err(e) => eprintln!("maju-desktop: team-suffix-strip: {e}"),
    }
}
