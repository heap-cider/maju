...
fn orchestrate_repair_then_detach(
    ...
    match repair() {
        Ok(repaired) => {
            if repaired > 0 {
                eprintln!("maju-desktop: team-membership-repair: repaired {repaired} record(s)");
            }
            match detach() {
                Ok(0) => {}
                Ok(n) => {
                    eprintln!(
                        "maju-desktop: detach-dir-teams: detached {n} directory-backed team(s)"
                    )
                }
        ...
        Err(e) => eprintln!(
            "maju-desktop: team-membership-repair: {e} — skipping directory-backed detach this \
    ...
    for team in teams.iter_mut() {
        ...
        for id in team.persona_ids.iter_mut() {
            ...
            let [only] = candidates.as_slice() else {
                eprintln!(
                    "maju-desktop: team-membership-repair: team {:?}: leaving unresolvable \
    ...
    for agent in agents.iter_mut() {
        ...
        match agent.team_id.as_deref() {
            ...
            Some(_) => match persona_to_team.get(persona_id) {
                ...
                _ => {
                    eprintln!(
                        "maju-desktop: team-membership-repair: unbinding instance {:?} — persona \
            ...
            None => match persona_to_team.get(persona_id) {
                ...
                Some(None) => eprintln!(
                    "maju-desktop: team-membership-repair: leaving instance {:?} unbound — persona \
