...
/// with the key in goose's own keyring rather than Maju's env) would otherwise
...
impl DiscoveryProvider {
    ...
    /// the caller must fall through to the `maju-acp models` subprocess rather
...
/// — even on an internal build that bakes `MAJU_AGENT_PROVIDER=databricks_v2`
/// and a `DATABRICKS_HOST`. Discovery then degrades to the `maju-acp models`
