...
/// `launch.policy_env` as `MAJU_ACP_REPLAY_FLOOR`, so the remote harness's
...
mod tests {
    ...
    fn launch_payload() -> serde_json::Value {
        serde_json::json!({
            "launch": {
                "env": { "KEEP_ME": "yes" },
                "policy_env": { "MAJU_ACP_LAZY_POOL": "true" },
            },
        })
    }
    ...
    fn caller_replay_floor_rides_launch_policy_env() {
        ...
        // as MAJU_ACP_REPLAY_FLOOR, exactly like a local spawn's env.
        let mut payload = launch_payload();
        apply_replay_floor(&mut payload, Some(1_756_600_000));
        assert_eq!(
            payload["launch"]["policy_env"]["MAJU_ACP_REPLAY_FLOOR"],
            "1756600000"
        );
        assert_eq!(payload["launch"]["env"]["KEEP_ME"], "yes");
        assert_eq!(
            payload["launch"]["policy_env"]["MAJU_ACP_LAZY_POOL"],
            "true"
        );
    }
    ...
    fn caller_replay_floor_strips_user_env_shadow() {
        ...
        let mut payload = launch_payload();
        payload["launch"]["env"]["MAJU_ACP_REPLAY_FLOOR"] = "1".into();
        payload["launch"]["env"]["maju_acp_replay_floor"] = "2".into();
        apply_replay_floor(&mut payload, Some(42));
        assert_eq!(
            payload["launch"]["policy_env"]["MAJU_ACP_REPLAY_FLOOR"],
            "42"
        );
        assert!(payload["launch"]["env"]["MAJU_ACP_REPLAY_FLOOR"].is_null());
        assert!(payload["launch"]["env"]["maju_acp_replay_floor"].is_null());
        assert_eq!(payload["launch"]["env"]["KEEP_ME"], "yes");
    }
