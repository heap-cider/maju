...
fn maju_mesh_name(state: &AppState) -> String {
...
pub(crate) async fn resolve_maju_mesh_join_targets(
    state: &AppState,
...
async fn resolve_maju_mesh_startup(state: &AppState) -> (Vec<String>, Option<String>) {
...
pub(crate) async fn restore_mesh_sharing(app: &AppHandle, state: &AppState) -> CmdResult<()> {
    ...
    let (trusted_owner_ids, join_token) = resolve_maju_mesh_startup(state).await;
...
pub async fn mesh_start_node(
    ...
    request.mesh_name = Some(maju_mesh_name(&state));
    let mut runtime = state.mesh_llm_runtime.lock().await;
