...
const RELAY_MESH_RUNTIME_NO_TARGET: &str =
    "Maju shared compute requires a live serving member; start serving the selected model on a member, then try again";
...
fn restarting_share_status(config: &MeshSharingConfig) -> mesh_llm::MeshNodeStatus {
    mesh_llm::MeshNodeStatus {
        ...
        health: mesh_llm::MeshHealth {
            ...
            reason: Some("Maju is restarting to switch this machine to sharing".to_string()),
        },
        api_base_url: None,
        console_url: None,
...
fn maju_mesh_name_for_relay(relay_url: &str) -> String {
    ...
    format!("maju-community-{}", &digest[..32])
}

pub(super) fn maju_mesh_name(state: &AppState) -> String {
    maju_mesh_name_for_relay(&relay::relay_ws_url_with_override(state))
}
...
pub(crate) async fn resolve_trusted_owner_ids_or_self_only(state: &AppState) -> Vec<String> {
    match resolve_trusted_owner_ids(state).await {
        Ok(owners) => owners,
        Err(error) => {
            eprintln!("maju-mesh: roster query failed; allowing only this node: {error}");
            Vec::new()
        }
    }
}

/// Choose validated live endpoints from other runtimes in this Maju community.
...
/// existing Maju community mesh. `Ok(None)` means this machine is the first
/// live serving member (or is itself the shared bootstrap contact).
pub(crate) async fn resolve_maju_mesh_join_targets_at(
    state: &AppState,
    relay_url: &str,
...
async fn resolve_maju_mesh_startup_at(
    state: &AppState,
    relay_url: &str,
    ...
    match query_mesh_discovery_events_at(state, relay_url).await {
        ...
        Err(error) => {
            ...
            eprintln!(
                "maju-mesh: startup discovery failed; allowing only this node and starting isolated for now: {error}"
            );
            (Vec::new(), None)
        }
    }
...
pub(crate) async fn restore_mesh_sharing(app: &AppHandle, state: &AppState) -> CmdResult<()> {
    ...
    let (trusted_owner_ids, join_token) = resolve_maju_mesh_startup_at(state, &relay_url).await;
    ...
    if config.start_on_next_launch {
        ...
        // work. If Maju exits during that work, the next launch stays stopped.
    ...
    let request = mesh_llm::StartMeshNodeRequest {
        ...
        join_token,
        mesh_name: Some(maju_mesh_name_for_relay(&relay_url)),
    ...
    if let Err(error) = wait_for_mesh_inference(&config.model_id).await {
        ...
        if let Err(cleanup_error) = cleanup {
            eprintln!(
                "maju-mesh: restored node failed inference readiness and cleanup was incomplete: {cleanup_error:#}"
            );
        }
...
pub async fn mesh_start_node(
    ...
    let status = match started.status().await {
        ...
        Err(error) => {
            ...
            if let Err(cleanup_error) = &cleanup {
                eprintln!(
                    "maju-mesh: started node status failed and cleanup was incomplete: {cleanup_error:#}"
                );
            }
            ...
            app.request_restart();
            return Err(format!(
                "mesh node started but status probe failed: {error:#}; Maju is restarting to guarantee cleanup"
            ));
        }
    };
    if let Some(config) = sharing_config.as_ref() {
        if let Err(error) = wait_for_mesh_inference(&config.model_id).await {
            ...
            if let Err(cleanup_error) = &cleanup {
                eprintln!(
                    "maju-mesh: started node failed inference readiness and cleanup was incomplete: {cleanup_error:#}"
                );
            }
            drop(runtime);
            app.request_restart();
            return Err(format!(
                "mesh node started but inference never became ready: {error}; Maju is restarting to guarantee cleanup"
            ));
        }
    }
    *runtime = Some(started);
...
fn mesh_readiness_failure_message(
    ...
    match failure {
        MeshReadinessFailure::CatalogNeverSynced => format!(
            "Maju shared compute connected to the serving member but could not sync \
        ...
        MeshReadinessFailure::RoutingNeverCompleted => format!(
            "Maju shared compute found \"{model_id}\" on a serving member but inference \
...
pub(crate) async fn ensure_client_node_for_model(
    ...
    let start = mesh_llm::StartMeshNodeRequest {
        ...
        mesh_name: Some(maju_mesh_name(state)),
...
/// encrypted iroh relay addresses, so no Maju relay connection coordination is
...
pub(crate) async fn ensure_relay_mesh_for_record(
    ...
    if state.mesh_llm_runtime.lock().await.is_some() {
        ...
        .await
        {
            ...
            mesh_llm::MeshRuntimeRecovery::Debouncing => {
                return Err(
                    "Maju shared compute ingress is temporarily unresponsive; recovery is already scheduled. Try again shortly."
                        .to_string(),
                );
            }
            mesh_llm::MeshRuntimeRecovery::ReleasePending => {
                return Err(
                    "Maju shared compute is still shutting down its previous local ingress. Try again shortly."
                        .to_string(),
                );
            }
            ...
            mesh_llm::MeshRuntimeRecovery::RestartRequired => {
                app.request_restart();
                return Err(
                    "Maju shared compute startup lost its local ingress before shutdown control became available. Maju is restarting to recover it."
                        .to_string(),
                );
            }
        }
    ...
    let target = match resolve_mesh_bootstrap_target(&state, model_id).await {
        ...
        Ok(None) => {
            return Err(
                "Maju shared compute cannot start because no live member is serving this model. Start serving it on a member, then try again."
                    .to_string(),
            );
        }
        Err(error) => {
            return Err(format!(
                "could not refresh Maju shared compute serving members: {error}"
            ));
        }
    };
