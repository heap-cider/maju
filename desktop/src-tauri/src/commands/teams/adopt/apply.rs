...
pub(super) fn commit_and_enqueue(
    ...
    match resolve_scope() {
        ...
        Err(e) => eprintln!("maju-desktop: adopt-retain scope unavailable: {e}"),
    }
...
pub(super) fn enqueue_adoption_retention(
    ...
    for persona in retain_personas {
        if let Err(e) = crate::commands::personas::retain_persona_pending_at(scope, persona) {
            eprintln!("maju-desktop: adopt persona-retain: {e}");
        }
    }
    if let Err(e) = crate::commands::teams::retain_team_pending_at(scope, team) {
        eprintln!("maju-desktop: adopt team-retain: {e}");
    }
}
...
pub(super) fn plan_add(
    ...
    let assigned_personas: std::collections::HashSet<&str> = teams_before
        .iter()
    ...
    let resolved = content
        .members
        .iter()
        .map(|member| resolve_member(&mut personas, source, member, now, &assigned_personas))
    ...
    let persona_ids: Vec<String> = resolved
        .into_iter()
        .map(|resolved| resolved.id)
        .filter(|id| seen_personas.insert(id.clone()))
        .collect();
    crate::managed_agents::ensure_single_team_per_persona(teams_before, &persona_ids, None)?;
    let team = TeamRecord {
...
fn resolve_member(
    ...
    now: &str,
    assigned_personas: &std::collections::HashSet<&str>,
) -> Result<ResolvedMember, String> {
    if let Some(local_id) =
        reusable_builtin(personas, member).filter(|id| !assigned_personas.contains(id.as_str()))
    {
    ...
    if let Some(existing) = personas.iter_mut().find(|persona| {
        !assigned_personas.contains(persona.id.as_str())
            && member_provenance_matches(persona, source, member)
    }) {
    ...
    // Catalog adoption explicitly copies a blueprint into the owner's library.
    // A definition already assigned elsewhere cannot be borrowed by this team;
    // keep that definition and its running identity intact and create this
    // copy's own definition. Channel deployment still reuses each definition's
    // stable agent identity and never creates another identity for membership.
