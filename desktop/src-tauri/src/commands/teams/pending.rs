...
fn retained_team_is_shared(row: Option<&RetainedEvent>) -> bool {
    use maju_core_pkg::kind::event_is_shared;
    use nostr::JsonUtil;
...
fn project_scoped_team_sharing(scope: Result<RetentionScope, String>, teams: &mut [TeamRecord]) {
    ...
    if let Err(error) = projected {
        eprintln!(
            "maju-desktop: team-share-projection unavailable, reporting every team as unshared: {error}"
        );
        for team in teams {
            team.shared = false;
        }
...
pub(super) fn tombstone_team_catalog_pending<R: tauri::Runtime>(
    ...
    if let Err(e) = result {
        eprintln!("maju-desktop: team-catalog-tombstone: {e}");
    }
}
...
pub(super) fn refresh_shared_team_catalog_head_resolving<R: tauri::Runtime>(
    ...
    match result {
        Ok(RefreshOrRetractOutcome::RemovalQueued { ref reason }) => {
            eprintln!(
                "maju-desktop: team-catalog-refresh: retracting '{}' — {reason}",
                team.name
            );
            emit_team_catalog_auto_retracted(app, &team.name, reason);
        }
        Err(ref e) => {
            eprintln!("maju-desktop: team-catalog-refresh: '{}' — {e}", team.name);
        }
        _ => {}
    }
}
...
pub(super) fn refresh_shared_team_catalog_heads_for_persona<R: tauri::Runtime>(
    ...
    let result = (|| -> Result<(), String> {
        ...
        for team in &teams {
            ...
            match outcome {
                Ok(RefreshOrRetractOutcome::RemovalQueued { ref reason }) => {
                    eprintln!(
                        "maju-desktop: team-catalog-refresh: retracting '{}' after persona edit — {reason}",
                        team.name
                    );
                    emit_team_catalog_auto_retracted(app, &team.name, reason);
                }
                Err(ref e) => {
                    eprintln!(
                        "maju-desktop: team-catalog-refresh: '{}' after persona edit — {e}",
                        team.name
                    );
                }
                _ => {}
    ...
    if let Err(e) = result {
        eprintln!("maju-desktop: team-catalog-refresh-for-persona: {e}");
    }
}
...
fn emit_team_catalog_auto_retracted<R: tauri::Runtime>(
    ...
    if let Err(e) = app.emit(
        ...
        eprintln!("maju-desktop: team-catalog-auto-retracted: failed to emit notice: {e}");
    }
}
