...
pub(super) fn transcode_voice_note_to_mp4_with_cancellation(
    ...
    let output = std::env::temp_dir().join(format!("maju-voice-note-{}.mp4", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    fn test_voice_note_envelope_passes_relay_video_validation() {
        ...
        let source =
            std::env::temp_dir().join(format!("maju-voice-test-{}.wav", uuid::Uuid::new_v4()));
        ...
        let metadata = maju_media_pkg::validation::validate_video_file(&output, &relay_config)
