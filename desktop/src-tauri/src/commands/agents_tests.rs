...
fn deploy_payload_for_policy(
    ...
    deploy_payload_json(
        ...
        // Access projection is the subject here; launch behavior is exercised
        // by the payload completeness test below.
...
fn current_build_deploy_payload_forwards_compiled_policy() {
    ...
    let expected_owner_only = match std::env::var("MAJU_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY") {
        Ok(value) => value
            .parse::<bool>()
        ...
        Err(std::env::VarError::NotPresent) => {
            panic!(
                "MAJU_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY must be set for owner-only-access-build tests"
            )
        }
        Err(std::env::VarError::NotUnicode(_)) => {
            panic!("MAJU_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY must be valid UTF-8")
        }
    };
