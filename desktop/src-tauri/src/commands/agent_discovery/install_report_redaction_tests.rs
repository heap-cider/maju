...
/// An install inherits Maju's environment and installers echo it back — npm
...
fn test_log_redacts_an_environment_secret_with_no_recognizable_prefix() {
    ...
    std::env::set_var("MAJU_TEST_REGISTRY_TOKEN", secret);
    let h = harness();
    std::env::remove_var("MAJU_TEST_REGISTRY_TOKEN");
