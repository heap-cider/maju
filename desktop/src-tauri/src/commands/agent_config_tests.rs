...
fn with_no_goose_config<T>(body: impl FnOnce() -> T) -> T {
    ...
    std::env::set_var("GOOSE_PATH_ROOT", "/nonexistent-maju-test-path");
    let output = body();
    match prior {
...
fn reserved_key_in_inherited_persona_env_is_stripped() {
    ...
    // MAJU_PRIVATE_KEY is a reserved key — must be stripped.
    persona
        .env_vars
        .insert("MAJU_PRIVATE_KEY".to_string(), "nsec-secret".to_string());
    ...
    assert!(
        !tiers.persona_env.contains_key("MAJU_PRIVATE_KEY"),
...
fn reserved_key_in_definition_env_shaped_map_is_stripped_by_sanitize() {
    ...
    raw.insert("MAJU_PRIVATE_KEY".to_string(), "nsec-secret".to_string());
    raw.insert("GOOSE_MODEL".to_string(), "harness-model".to_string());

    let sanitized = sanitize_inherited_env(&raw);

    assert!(
        !sanitized.contains_key("MAJU_PRIVATE_KEY"),
...
fn baked_env_thinking_summary_is_unmasked() {
    // MAJU_AGENT_THINKING_SUMMARY is a non-secret enum — must not be masked.
    let entries = baked_env_from_map(&[("MAJU_AGENT_THINKING_SUMMARY", "detailed")]);
    assert_eq!(entries.len(), 1);
    let summary = entries
        .iter()
        .find(|e| e.key == "MAJU_AGENT_THINKING_SUMMARY")
        .unwrap();
...
fn baked_env_allowlist_is_case_insensitive() {
    ...
    assert!(super::is_safe_to_reveal("MAJU_AGENT_THINKING_EFFORT"));
    assert!(super::is_safe_to_reveal("maju_agent_thinking_summary"));
    assert!(super::is_safe_to_reveal("MAJU_AGENT_THINKING_SUMMARY"));
