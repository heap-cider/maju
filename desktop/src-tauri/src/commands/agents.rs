...
use crate::{
    app_state::AppState,
    managed_agents::{
        build_managed_agent_summary, current_instance_id, ensure_definition_identity_is_unique,
        ensure_persona_is_active, find_managed_agent_mut, load_managed_agents, load_personas,
        load_teams, managed_agent_avatar_url, normalize_agent_args, resolve_definition_team_id,
...
/// Build a deleted agent's owner-authenticated NIP-IA archive request, retaining its persona id.
...
pub async fn delete_managed_agent(
    ...
    tokio::task::spawn_blocking(move || {
        let state = app.state::<AppState>();
        {
            ...
            // Archive the deleted identity so it leaves member pickers and autocomplete;
            // this is best-effort under the same lock contract as the tombstone.
