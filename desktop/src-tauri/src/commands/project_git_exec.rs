...
pub(crate) fn validate_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    // Maju git remotes are served at `…/git/<owner-pubkey>/<repo-id>` — a
    ...
    let segments = parsed
        ...
        .unwrap_or_default();
    let is_maju_repo_path = segments
        .iter()
        ...
        .unwrap_or(false);
    if !is_maju_repo_path {
        return Err("clone URL must point at a Maju git repository".into());
    }
    Ok(())
}
...
pub(crate) fn validate_local_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    Err("clone URL must point at a Maju repository or public GitHub repository".into())
}
...
mod tests {
    ...
    fn credential_helper_config_value_uses_forward_slashes() {
        let path =
            std::path::PathBuf::from(r"C:\Users\x\AppData\Local\Maju\git-credential-nostr.exe");
        assert_eq!(
            credential_helper_config_value(&path),
            "C:/Users/x/AppData/Local/Maju/git-credential-nostr.exe",
        );
    }
    ...
    #[test]
    fn validate_clone_url_requires_maju_repo_shape() {
    ...
    fn local_clone_url_allows_only_public_github_https_urls() {
        assert!(validate_local_clone_url("https://github.com/heap-cider/maju").is_ok());
        assert!(validate_local_clone_url("https://github.com/heap-cider/maju.git").is_ok());
        assert!(validate_local_clone_url("http://github.com/heap-cider/maju").is_err());
        assert!(validate_local_clone_url("https://github.com/heap-cider/maju/issues").is_err());
        assert!(validate_local_clone_url("https://user@github.com/heap-cider/maju").is_err());
        assert!(validate_local_clone_url("https://github.com.evil.test/heap-cider/maju").is_err());
        assert!(validate_local_clone_url("https://gitlab.com/heap-cider/maju").is_err());
    }
}
