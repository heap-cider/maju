...
const MAJU_RELEASE_IDENTIFIER_PREFIX: &str = "xyz.block.maju.app";
const SPROUT_RELEASE_IDENTIFIER: &str = "xyz.block.sprout.app";
const MAJU_DEV_IDENTIFIER_PREFIX: &str = "xyz.block.maju.app.dev";
...
fn legacy_identifier(current_identifier: &str) -> Option<String> {
    if current_identifier.starts_with(MAJU_DEV_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            MAJU_DEV_IDENTIFIER_PREFIX,
            SPROUT_DEV_IDENTIFIER_PREFIX,
            1,
        ))
    } else if current_identifier.starts_with(MAJU_RELEASE_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            MAJU_RELEASE_IDENTIFIER_PREFIX,
...
/// data directory so the frontend can seed Maju localStorage before first
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for database in databases {
            match read_legacy_workspace_storage_db(&database) {
                ...
                Err(error) => eprintln!(
                    "maju-desktop: legacy-local-storage-migration: {}: {error}",
                    database.display()
                ),
            }
        }
...
mod tests {
    ...
    fn legacy_identifier_maps_release_identifier() {
        assert_eq!(
            legacy_identifier("xyz.block.maju.app"),
    ...
    fn legacy_identifier_maps_dev_worktree_identifier() {
        assert_eq!(
            legacy_identifier("xyz.block.maju.app.dev.my-branch"),
