...
/// `read_config_surface` tag those injected fields `MajuExplicit`, then re-tag
...
/// (c) the reader tagged it `MajuExplicit`. A value the user set explicitly in
/// Maju keeps `had_* == true` and is never re-tagged.
fn resolve_config_surface(
    ...
    // so a stale byte can never masquerade as MajuExplicit and suppress
    ...
    let had_prompt =
        record.system_prompt.is_some() || record.env_vars.contains_key("MAJU_ACP_SYSTEM_PROMPT");
    ...
    //     record's own model is the baseline (MajuExplicit). Gated behind
    ...
    let baseline = if had_model {
        if model_overridden {
            record
                .model
                .clone()
                .map(|m| (m, ConfigOrigin::MajuExplicit))
        } else {
            None
        }
    } else {
    ...
    if !had_prompt {
        if let Some(p) = persona_prompt {
            record
                .env_vars
                .insert("MAJU_ACP_SYSTEM_PROMPT".to_string(), p);
        }
    }
    if !had_model {
    ...
    // Re-tag persona-sourced fields from MajuExplicit to PersonaDefault.
    if !had_prompt {
    ...
    // Re-tag global-sourced fields from MajuExplicit to GlobalDefault.
    if inject_global_model {
...
/// Re-tag a field's origin from `MajuExplicit` to `PersonaDefault`, leaving any
...
/// the binary at compile time via `MAJU_BUILD_AGENT_ENV`. The backend readiness
...
///
/// Provider and model arrive as `MAJU_AGENT_PROVIDER` / `MAJU_AGENT_MODEL`
...
/// Re-tag a field's origin from `MajuExplicit` to `GlobalDefault`, leaving any
/// other origin untouched. No-op when the field is absent.
...
mod tests {
    ...
    /// not the stale byte as `MajuExplicit`.
    #[test]
    ...
    /// instances only.
    #[test]
    fn definition_less_explicit_record_model_keeps_maju_explicit_origin() {
        let mut record = agent_record();
    ...
    /// `MajuExplicit` (its true source — NOT `PersonaDefault`). FAILS against the
    /// shipped no-persona early-return, which left X as primary and Y struck.
    #[test]
    fn genuine_explicit_live_switch_renders_y_over_x_maju_explicit_secondary() {
        let mut record = agent_record();
        record.persona_id = None;
        record.model = Some("model-x".to_string());
        let personas: Vec<AgentDefinition> = vec![];
        ...
        assert_eq!(model.overridden_origin, Some(ConfigOrigin::MajuExplicit));
    }
    ...
    fn global_default_live_switch_renders_global_model_as_secondary_global_default() {
        ...
        assert_eq!(
            ...
            "override baseline origin must be GlobalDefault, not PersonaDefault or MajuExplicit"
        );
    }
    ...
    fn baked_env_non_secret_key_shows_real_value() {
        let entries = baked_env_from_map(&[("MAJU_AGENT_PROVIDER", "databricks_v2")]);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].key, "MAJU_AGENT_PROVIDER");
    ...
    fn baked_env_empty_value_filtered_out() {
        let entries = baked_env_from_map(&[("MAJU_AGENT_PROVIDER", "")]);
    ...
    fn baked_env_mixed_keys_correct_masking() {
        let entries = baked_env_from_map(&[
            ...
            ("MAJU_AGENT_MODEL", "goose-claude-opus-4-8"),
        ...
        let provider = entries
            .iter()
            .find(|e| e.key == "MAJU_AGENT_PROVIDER")
            .unwrap();
    ...
    fn baked_env_thinking_effort_is_unmasked() {
        // MAJU_AGENT_THINKING_EFFORT is a non-secret enum — must not be masked.
        let entries = baked_env_from_map(&[("MAJU_AGENT_THINKING_EFFORT", "medium")]);
        assert_eq!(entries.len(), 1);
        let effort = entries
            .iter()
            .find(|e| e.key == "MAJU_AGENT_THINKING_EFFORT")
            .unwrap();
    ...
    fn baked_env_allowlist_is_case_insensitive() {
        ...
        assert!(super::is_safe_to_reveal("maju_agent_provider"));
        assert!(super::is_safe_to_reveal("MAJU_AGENT_PROVIDER"));
        assert!(super::is_safe_to_reveal("maju_agent_model"));
        assert!(super::is_safe_to_reveal("MAJU_AGENT_MODEL"));
        assert!(super::is_safe_to_reveal("maju_agent_thinking_effort"));
        assert!(super::is_safe_to_reveal("MAJU_AGENT_THINKING_EFFORT"));
