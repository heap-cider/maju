...
pub(super) async fn discover_databricks_models(
    ...
    let entries = match maju_agent_pkg::discover_databricks_models(&config).await {
        Ok(entries) => entries,
        Err(maju_agent_pkg::AgentError::LlmAuth(_)) if should_start_interactive_auth(&api_key) => {
            let _auth = AUTH_GATE.lock().await;
            match maju_agent_pkg::discover_databricks_models(&config).await {
                ...
                Err(maju_agent_pkg::AgentError::LlmAuth(_)) => {
                    ...
                    run_interactive_databricks_auth(
                        maju_agent_pkg::authenticate_databricks(&host),
                        AUTH_FLOW_TIMEOUT,
                        &AUTH_COOLDOWNS,
                        &host,
                        &redaction_env,
                    ...
                    .await?;
                    maju_agent_pkg::discover_databricks_models(&config)
                        .await
                        .map_err(|error| {
