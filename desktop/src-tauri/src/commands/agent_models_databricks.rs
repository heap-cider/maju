...
fn databricks_agent_provider(provider: &str) -> maju_agent_pkg::config::Provider {
    if provider.trim().eq_ignore_ascii_case("databricks_v2")
        || provider.trim().eq_ignore_ascii_case("databricks-v2")
    {
        maju_agent_pkg::config::Provider::DatabricksV2
    } else {
        maju_agent_pkg::config::Provider::Databricks
    }
}
...
pub(super) fn databricks_sign_in_required_error() -> String {
    "Databricks sign-in is required; save this agent, then open its model picker to sign in, or run `maju-agent auth databricks`"
        .to_string()
}

pub(super) fn databricks_sign_in_timed_out_error() -> String {
    "Databricks sign-in timed out; open the model picker to retry, or run `maju-agent auth databricks`"
        .to_string()
}
...
pub(super) async fn discover_databricks_models(
    ...
    let parsed_filter = maju_agent_pkg::config::DatabricksModelFilter::parse(filter.as_deref())
        .map_err(|error| format!("invalid DATABRICKS_MODEL_FILTER: {error}"))?;
    let config = maju_agent_pkg::config::Config::for_discovery(
    ...
    let entries = match maju_agent_pkg::discover_databricks_models_with_cache_dir(
        &config,
    ...
    {
        Ok(entries) => entries,
        Err(maju_agent_pkg::AgentError::LlmAuth(_)) if should_start_interactive_auth(&api_key) => {
            let _auth = AUTH_GATE.lock().await;
            match maju_agent_pkg::discover_databricks_models_with_cache_dir(
                &config,
                oauth_cache_dir.as_deref(),
            )
            .await
            {
                ...
                Ok(entries) => entries,
                Err(maju_agent_pkg::AgentError::LlmAuth(_)) => {
                    ...
                    run_interactive_databricks_auth(
                        maju_agent_pkg::authenticate_databricks_with_cache_dir(
                            &host,
                            oauth_cache_dir.as_deref(),
                        ),
                        AUTH_FLOW_TIMEOUT,
                    ...
                    .await?;
                    maju_agent_pkg::discover_databricks_models_with_cache_dir(
                        &config,
                        oauth_cache_dir.as_deref(),
                    )
                    .await
        ...
        Err(maju_agent_pkg::AgentError::LlmAuth(error)) if !api_key.is_empty() => {
...
pub(super) fn databricks_models_response(
    provider_name: &str,
    entries: Vec<maju_agent_pkg::ModelEntry>,
    selected_model: Option<String>,
    filter: Option<&maju_agent_pkg::config::DatabricksModelFilter>,
...
where
    Fut: std::future::Future<Output = Result<(), maju_agent_pkg::AgentError>>,
{
