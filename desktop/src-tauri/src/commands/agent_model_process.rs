...
use crate::managed_agents::{
    build_maju_agent_provider_defaults, default_agent_workdir, known_acp_runtime,
...
pub(super) async fn run_agent_models_command(
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        cmd.arg("models")
            .arg("--json")
            .env("MAJU_ACP_AGENT_COMMAND", &agent_command)
            .env("MAJU_ACP_AGENT_ARGS", agent_args.join(","));
        if let Some(model) = model_for_probe
            .as_deref()
            .map(str::trim)
            .filter(|m| !m.is_empty())
        {
        ...
        // User env layering — written LAST so it overrides any Maju-set env above.
        ...
        cmd.stdout(std::process::Stdio::piped())
            ...
            .output()
            .map_err(|e| format!("failed to spawn maju-acp models: {e}"))
    })
    .await
    ...
    if !output.status.success() {
        ...
        return Err(format!(
            "maju-acp models failed (exit {}): {stderr_redacted}",
