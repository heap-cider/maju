...
pub fn auto_connect_default_relay_enabled() -> bool {
    option_env!("MAJU_DESKTOP_BUILD_AUTO_CONNECT_DEFAULT_RELAY").is_some()
}
...
mod auto_connect_default_relay_tests {
    ...
    fn compiled_flag_matches_expected() {
        let expected = std::env::var("MAJU_TEST_EXPECTED_AUTO_CONNECT_DEFAULT_RELAY")
...
pub fn is_shared_identity() -> bool {
    std::env::var("MAJU_SHARE_IDENTITY")
        .map(|v| v == "1")
        .unwrap_or(false)
        && std::env::var("MAJU_PRIVATE_KEY")
            .ok()
...
pub async fn decrypt_observer_event(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        maju_core_pkg::observer::decrypt_observer_payload(&keys, &event)
...
pub fn build_observer_control_event(
    ...
    let encrypted =
        maju_core_pkg::observer::encrypt_observer_payload(&keys, &agent_pubkey, &payload)
            .map_err(|error| format!("encrypt observer control failed: {error}"))?;
    let builder = maju_sdk_pkg::build_agent_observer_frame(
        &agent_pubkey_hex,
        &agent_pubkey_hex,
        maju_core_pkg::observer::OBSERVER_FRAME_CONTROL,
        &encrypted,
    )
...
pub async fn import_identity(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        eprintln!("maju-desktop: imported identity pubkey {}", pubkey_hex);
...
pub(crate) fn commit_imported_identity(
    ...
    if let Err(e) = crate::key_backup::cleanup_stale_backup(&previous_pubkey, &pubkey, data_dir) {
        eprintln!(
            "maju-desktop: import committed, but stale key backup cleanup failed: {e}; \
...
/// open.
///
/// Not available in shared-identity mode (`MAJU_SHARE_IDENTITY=1`): the key
...
pub async fn sign_out(app: tauri::AppHandle) -> Result<(), String> {
    if is_shared_identity() {
        return Err(
            "Sign out isn't available while MAJU_SHARE_IDENTITY provides your identity. Unset MAJU_SHARE_IDENTITY and MAJU_PRIVATE_KEY, then relaunch to sign out."
                .to_string(),
        );
    }
    ...
    if let Err(e) = crate::shutdown::shutdown_managed_agents(&app) {
        eprintln!("maju-desktop sign-out: agent shutdown: {e}");
    }
...
mod nostr_identity_binding_tests {
    ...
    fn build_nostr_identity_binding_event_signs_exact_shape() {
        ...
        assert!(tags.contains(&vec!["audience".into(), "maju:nostr-identity".into()]));
        assert!(tags.contains(&vec!["action".into(), "bind_nostr_identity".into(),]));
        assert!(tags.contains(&vec!["protocol".into(), "maju-nostr-identity".into(),]));
