...
pub(super) async fn fetch_channels(
    ...
    let (member_chain_result, open_meta_result, hidden_dms) = tokio::join!(
        ...
        async {
            let events = query_relay(
                state,
                &[serde_json::json!({
                    "kinds": [maju_core_pkg::kind::KIND_DM_VISIBILITY],
                    "#p": [&my_pubkey],
                    "limit": 1,
                })],
            )
    ...
    #[cfg(debug_assertions)]
    {
        ...
        eprintln!(
            "maju-desktop: get_channels profile channels={} phase1(member_chain+open_meta+hidden_dm)={:?} phase2(member_counts+last_msg)={:?} total={:?}",
            channels.len(),
            t_phase1,
            total - t_phase1,
            total,
