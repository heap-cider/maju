...
pub(in crate::commands) fn propagate_membership_best_effort(
    ...
    if let Err(e) = result {
        eprintln!("maju-desktop: team-membership-propagate: {e}");
    }
}
...
/// rosters:
///
/// - **Added** (`current` but not `previous`): bind the persona's instances to
///   this team, replacing any stale binding. Maju permits one team per
///   definition, so the newest valid local or inbound roster is authoritative.
...
/// change both sets are empty and no instance is re-pointed. `create`
...
fn apply_team_membership_delta(
    ...
    for record in records.iter_mut() {
        ...
        if added.contains(&persona_id) && record.team_id.as_deref() != Some(team_id) {
...
mod tests {
    ...
    /// An explicit add is authoritative and binds an unbound instance.
    #[test]
    fn added_persona_binds_to_the_edited_team() {
        let mut records = vec![instance('a', "duncan", None)];
        assert!(apply_team_membership_delta(
            &mut records,
            "team-a",
