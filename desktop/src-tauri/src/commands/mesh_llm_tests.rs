...
#[test]
fn maju_mesh_join_uses_the_same_live_member_from_every_other_node() {
    let targets = vec![
    ...
    assert_eq!(
        maju_mesh_join_targets(targets.clone(), "member-b")
            .into_iter()
            .next()
    ...
    assert_eq!(
        maju_mesh_join_targets(targets, "member-c")
            .into_iter()
            .next()
            .map(|target| target.endpoint_addr),
        Some("addr-a".to_string())
...
#[test]
fn maju_mesh_bootstrap_member_does_not_dial_itself() {
    let targets = vec![
        reported_target("member-b", "model-b", "addr-b"),
        reported_target("member-a", "model-a", "addr-a"),
    ];
    ...
    assert_eq!(
        maju_mesh_join_targets(targets, "MEMBER-A")
            .into_iter()
            .next(),
...
#[test]
fn maju_mesh_join_ignores_targets_without_a_validated_reporter() {
    let targets = vec![
...
#[test]
fn maju_mesh_join_keeps_other_device_with_the_same_member_key() {
    ...
    assert_eq!(
        maju_mesh_join_targets(vec![self_target, other_device], "owner-self")
            .into_iter()
            .next()
            .map(|target| target.endpoint_addr),
...
#[test]
fn maju_mesh_name_is_stable_and_does_not_expose_the_relay() {
    let first = maju_mesh_name_for_relay("WSS://EXAMPLE.COM/");
    let second = maju_mesh_name_for_relay("wss://example.com:443/some/path?ignored=yes");
    let other_relay = maju_mesh_name_for_relay("wss://other.example.com");

    assert_eq!(first, second);
    assert_ne!(first, other_relay);
    assert!(first.starts_with("maju-community-"));
...
fn sharing_config_keeps_the_community_where_sharing_was_enabled() {
    let request = mesh_llm::StartMeshNodeRequest {
        ...
        max_vram_gb: Some(24),
        join_token: None,
        mesh_name: Some("maju-community-test".to_string()),
...
/// Maju shared compute as a client"). That forbade exactly what a user should be
...
///   cargo test -p maju-desktop --features mesh-llm \
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            runtime.block_on(async {
                ...
                let hosted_model = std::env::var("MAJU_MESH_TEST_MODEL")
                ...
                let inference = loop {
                    let response = http
                        ...
                        .json(&serde_json::json!({
                            ...
                            "messages": [{
                                "role": "user",
                                "content": "Reply with exactly MAJU_SINGLE_SHARE_OK and nothing else."
                            }],
                            "max_tokens": 512,
                            "temperature": 0
                        }))
