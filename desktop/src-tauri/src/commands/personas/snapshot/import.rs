//! Import-side helpers for `maju-agent-snapshot v1`.
//!
...
/// Decode a `maju-agent-snapshot v1` manifest from raw bytes.
///
...
/// Import a `maju-agent-snapshot v1` file as a brand-new agent.
///
/// Phase sequence:
...
pub async fn confirm_agent_snapshot_import(
    ...
    let (agent_keys, private_key_nsec, pubkey, auth_tag, owner_pubkey_hex) = {
        ...
        // NIP-OA auth tag: bridge nostr 0.37 → 0.36 (maju-sdk) via hex round-trip.
    ...
    if memory_total > 0 {
        ...
        for (idx, entry) in snapshot.memory.entries.iter().enumerate() {
            let body = if entry.slug == maju_core_pkg::engram::CORE_SLUG {
                maju_core_pkg::engram::Body::Core {
                    profile: entry.body.clone(),
                }
            } else {
                maju_core_pkg::engram::Body::Memory {
            ...
            match maju_core_pkg::engram::build_event(&agent_keys, &owner_pubkey, &body, created_at)
            {
                Ok(event) => {
...
fn retain_agent_pending(app: &AppHandle, state: &AppState, record: &ManagedAgentRecord) {
    ...
    if let Err(e) = result {
        eprintln!("maju-desktop: snapshot-import retain-agent: {e}");
    }
}
