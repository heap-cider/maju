...
//! agent's `maju_agent_snapshot` manifest through the existing snapshot
//! encoder so the card IS an importable `.agent.png`.
//!
//! Boundary rules (agreed with Wren, maju-agent-trading-cards thread):
...
/// The Maju card frame template — Tyler's gold-honeycomb base. Generation
...
pub fn list_agent_cards(app: AppHandle) -> Result<Vec<ArchivedCardMeta>, String> {
    ...
    for entry in entries.flatten() {
        ...
        let Ok(meta) = serde_json::from_str::<ArchivedCardMeta>(&content) else {
            ...
            eprintln!(
                "maju-desktop: card-archive: skipping malformed sidecar {}",
                path.display()
            );
            continue;
        };
...
pub(crate) fn build_card_instructions(
    ...
    format!(
        r#"You are designing one premium collectible trading card for the Maju agent "{agent_name}".

Input image 1 is the official Maju card frame template (gold honeycomb border, dark interior, name banner top, hex badge top-right, text box lower third). Input image 2 is the agent's avatar — study its exact art style: medium, pixel grid if any, palette, shading, background motifs.
...
pub async fn mint_agent_card(
    ...
    if lock_keys.is_some() {
        ...
        if json_len > maju_core_pkg::engram::NIP44_PLAINTEXT_MAX {
            ...
            return Err(format!(
                "Agent manifest is too large to lock ({json_len} bytes; the encrypted \
                 format caps at {}). {hint}",
                maju_core_pkg::engram::NIP44_PLAINTEXT_MAX
            ));
        }
    }
    ...
    if let Err(e) = archive_minted_card(&app, &id, &display_name, &minted, &final_bytes) {
        eprintln!("maju-desktop: card-archive: failed to archive minted card: {e}");
    }
