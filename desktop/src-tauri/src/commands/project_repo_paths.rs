...
fn default_repos_root_candidates_for(
    ...
    if !is_demo_build {
        candidates.extend(
            home.map(|home| home.join(".maju").join("REPOS"))
...
mod tests {
    ...
    fn production_keeps_the_legacy_repo_fallback() {
        ...
        assert_eq!(
            default_repos_root_candidates_for(
                Some(home.join(".maju-dev")),
                Some(home.clone()),
                false,
            ),
            vec![home.join(".maju-dev/REPOS"), home.join(".maju/REPOS")]
        );
    }
    ...
    fn named_demos_only_search_their_selected_nest() {
        ...
        for slug in ["workstream-board", "second-demo"] {
            let nest = home.join(format!(".maju-demo-{slug}"));
            assert_eq!(
