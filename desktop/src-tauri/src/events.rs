...
//!
//! Mirrors the maju-sdk builder patterns but uses nostr 0.37 API
...
/// Maximum content size — matches maju-sdk (64 KiB).
const MAX_CONTENT_BYTES: usize = 64 * 1024;

/// Maximum mention count — matches maju-sdk.
const MAX_MENTIONS: usize = 50;

/// Maximum emoji length in characters — matches maju-sdk.
...
pub fn build_create_channel(
    ...
    let name = maju_sdk_pkg::canonical_channel_name(name);
...
pub fn build_update_channel(
    ...
    let name = name.map(maju_sdk_pkg::canonical_channel_name);
...
pub fn build_message_edit(
    ...
    if let Some(mention_refs) = edit_tags.mention_refs {
        ...
        tags.push(tag(vec!["maju:mention-snapshot"])?);
    }
...
mod tests {
    ...
    fn edit_emits_full_mention_reference_snapshot() {
        ...
        assert!(
            tags.iter().any(|tag| tag == &["maju:mention-snapshot"]),
    ...
    fn partial_edit_omits_mention_snapshot() {
        ...
        assert!(!tags
            .iter()
            .any(|tag| tag.first().map(String::as_str) == Some("maju:mention-snapshot")));
    }
