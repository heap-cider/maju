...
//! projection here so the native menu can remain useful while Maju is hidden.
...
const TRAY_ID: &str = "maju-tray";
const OPEN_MAJU_ID: &str = "tray-open-maju";
...
fn preview_activities() -> Option<Vec<TrayAgentActivity>> {
    if !cfg!(debug_assertions) || std::env::var("MAJU_TRAY_MENU_DEMO").ok().as_deref() != Some("1")
    {
        return None;
    }
...
/// Builds the standalone Maju bee as a transparent, macOS template image.
///
...
pub(crate) fn show_main_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    if let Err(error) = window.unminimize() {
        eprintln!("maju-desktop: failed to restore main window from tray: {error}");
        return;
    }
    if let Err(error) = window.show() {
        eprintln!("maju-desktop: failed to show main window from tray: {error}");
        return;
    }
    if let Err(error) = window.set_focus() {
        eprintln!("maju-desktop: failed to focus main window from tray: {error}");
    }
}
...
fn queue_tray_action<R: Runtime>(app: &AppHandle<R>, mut action: TrayAction) {
    ...
    let Ok(mut queue) = state.action_queue.lock() else {
        eprintln!("maju-desktop: tray action queue is unavailable");
        return;
    };
    ...
    if let Err(error) = app.emit("tray-action-available", ()) {
        eprintln!("maju-desktop: failed to notify frontend of tray action: {error}");
    }
}
...
fn handle_menu_event<R: Runtime>(app: &AppHandle<R>, id: &str) {
    match id {
        OPEN_MAJU_ID => show_main_window(app),
        NEW_CHANNEL_ID => {
...
/// Installs the persistent Maju tray icon with the initial empty activity menu.
pub fn init<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    if let Err(error) = apply_activity_presentation(&tray, activities, recent_activities) {
        eprintln!("maju-desktop: failed to apply tray menu presentation: {error}");
    }
    Ok(())
}
...
pub fn take_tray_actions<R: Runtime>(app: AppHandle<R>) -> Result<Vec<TrayAction>, String> {
    ...
    let mut queue = state
        .action_queue
        .lock()
        .map_err(|_| "Maju tray action queue is unavailable".to_string())?;
...
pub fn update_tray_agent_activity<R: Runtime>(
    ...
    let mut activity_items = state
        .activity_items
        .lock()
        .map_err(|_| "Maju tray menu state is unavailable".to_string())?;
            ...
            })
    {
        ...
        let tray = app
            .tray_by_id(TRAY_ID)
            .ok_or_else(|| "Maju tray icon is not available".to_string())?;
