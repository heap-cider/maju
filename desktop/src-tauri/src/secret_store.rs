...
//!
//! The store is deliberately NOT on any env-read path. `MAJU_PRIVATE_KEY`
...
//
// Two concurrent Maju processes (e.g. the signed DMG build and an unsigned dev
// build via `just staging`) share the same OS keychain blob because the
// service name `"maju-desktop"` is a constant — it does not key off the bundle
...
// per-user path `/tmp/maju-keychain-<uid>-<service>.lock` on Unix — a path
...
///
/// The path is `/tmp/maju-keychain-<uid>-<service>.lock` on Unix — a
...
impl SecretStore {
    ...
    ///
    /// Only one service name (`"maju-desktop"`) is used in practice. If a
...
impl SecretStore {
    ...
    {
        // Acquire the interprocess advisory lock first. All Maju processes
        // using the same service name contend on the same lockfile at
        // /tmp/maju-keychain-<uid>-<service>.lock (a deterministic per-user
...
mod tests {
    ...
    fn probe_returns_present_when_key_in_cache() {
        ...
        let store = SecretStore::with_cache("maju-test-cache-hit", Some(map));
    ...
    fn load_returns_value_when_key_in_cache() {
        let mut map = HashMap::new();
        map.insert("identity".to_string(), "nsec1test".to_string());
        let store = SecretStore::with_cache("maju-test-load-cache-hit", Some(map));
    ...
    fn test_stale_warm_cache_add_observes_prior_write() {
        ...
        let svc = "maju-test-race-stale-cache";
    ...
    fn test_concurrent_adds_neither_key_dropped() {
        ...
        let svc = "maju-test-race-concurrent-add";
    ...
    fn test_blob_lockfile_path_is_in_tmp_with_uid() {
        ...
        let path = blob_lockfile_path("maju-desktop");
        #[cfg(unix)]
        {
            let uid = unsafe { libc::getuid() };
            assert!(
            ...
            assert!(
                name.contains("maju-keychain"),
                "lockfile name must contain 'maju-keychain'"
            );
        }
        #[cfg(not(unix))]
        {
            assert!(
                path.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.contains("maju-keychain")),
                "lockfile name must contain 'maju-keychain'"
            );
        }
    }
    ...
    fn test_blob_lock_acquire_and_release() {
        ...
        let guard = acquire_blob_lock("maju-test-lock-smoke");
        assert!(
            guard.is_ok(),
        ...
        drop(guard);
        let guard2 = acquire_blob_lock("maju-test-lock-smoke");
        assert!(
            guard2.is_ok(),
    ...
    fn mutate_blob_does_not_advance_cache_on_write_failure() {
        ...
        //   cargo test -p maju-desktop -- --ignored mutate_blob_does_not_advance
        //
        ...
        let store = SecretStore::with_cache("maju-test-cow-write-fail", Some(map));
    ...
    //   cargo test -p maju-desktop -- --ignored blob_
    //
    ...
    fn blob_stores_and_retrieves_multiple_keys() {
        let store = SecretStore::keyring("maju-test-blob-multi");
    ...
    fn blob_probe_present_absent_unreachable() {
        let store = SecretStore::keyring("maju-test-blob-probe");
    ...
    fn blob_delete_removes_key_not_others() {
        let store = SecretStore::keyring("maju-test-blob-delete");
    ...
    fn blob_migration_from_per_key_entry() {
        let svc = "maju-test-blob-migration";
        let key = "identity";
    ...
    fn delete_all_with_legacy_cleanup_removes_per_key_identity() {
        let svc = "maju-test-delete-all-legacy";
        let key = "identity";
