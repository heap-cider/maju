...
//! # Exit-code fidelity
//!
//! [#2892](https://github.com/block/maju/pull/2892) established that an install
...
///
/// `slug` names the downloaded script (`maju-install-<slug>.ps1`) so concurrent
...
macro_rules! windows_install_command {
    ...
    ($slug:literal, $url:literal, $env_prefix:literal) => {
        concat!(
            ...
            "$installer=Join-Path $env:TEMP 'maju-install-",
            $slug,
            ".ps1'; ",
            "Invoke-RestMethod ",
            $url,
...
mod tests {
    ...
    fn test_every_windows_install_command_downloads_to_a_file_then_executes_it() {
        ...
        for (id, command) in commands {
            ...
            assert!(
                command.contains(&format!("maju-install-{id}.ps1")),
