...
impl BoundedChild {
    ...
    fn terminate_timed_out(&mut self) {
        #[cfg(unix)]
        {
            // The group may already be gone (ESRCH).
...
#[cfg(unix)]
fn set_nonblocking<F: std::os::fd::AsFd>(f: &F) -> bool {
    use nix::fcntl::{fcntl, FcntlArg, OFlag};
    let Ok(flags) = fcntl(f, FcntlArg::F_GETFL) else {
        return false;
    };
    fcntl(
        f,
...
mod tests {
    ...
    //
    //   cargo test -p maju-desktop --lib bounded_command -- --ignored --nocapture
    //
    ...
    fn pid_alive(pid: u32) -> bool {
        crosswin::windows::process::Process::open(
            pid,
            crosswin::windows::process::ProcessAccess::AllAccess,
        )
        .is_ok_and(|process| process.is_running())
    }
