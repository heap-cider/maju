...
/// `minimal` (Maju-only) is invalid — skipped as absent at every tier.
...
/// maju-agent's accepted persisted thinking-effort values — a validation-only
/// contract, NOT a canonicalization one. Unlike Goose, maju-agent keeps `xhigh`
/// and `max` as *distinct* efforts, so these values are validated (invalid →
/// skip as absent) but never aliased or collapsed.
///
/// Source of truth: `parse_thinking_effort`, `crates/maju-agent/src/config.rs`
...
/// emitted as `MAJU_AGENT_THINKING_EFFORT=off`, which the parser rejects at
/// config init (child exits 2).
pub(crate) static MAJU_AGENT_EFFORT_VALUES: &[&str] =
...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → Maju creates a symlink at `<nest>/<dir>/maju-cli`
    /// pointing to the canonical `.agents/skills/maju-cli`. `None` → this
    ...
    /// catalog (maju-agent); see `getProviderEffortConfig()` in TS for that
    /// path. Contract-less does NOT mean keyless: maju-agent still has a native
    /// `thinking_env_var`, and Claude/Codex route the canonical through
    /// `MAJU_ACP_EFFORT_LEVEL` for ACP startup even with `thinking_env_var: None`.
    ///
    ...
    /// (maju-agent: `parse_thinking_effort`'s accepted set). Used only for
    ...
    /// any string over the `MAJU_ACP_EFFORT_LEVEL` transport.
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, maju-agent).
