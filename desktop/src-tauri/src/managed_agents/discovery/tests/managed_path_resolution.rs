...
fn cheap_discovery_never_spawns_login_shell_even_when_cold() {
    ...
    fs::write(
        ...
        r#"{
            ...
            "command": "maju-absent-command-xyzzy",
            "args": []
        }"#,
    )
    .unwrap();
...
/// dirs + cache, so `maju-agent` reported "not installed" at every cold launch.
...
fn cheap_path_resolves_workspace_sidecar_without_cache() {
    ...
    let dir = std::env::temp_dir().join(format!("maju-sidecar-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("maju-agent");
