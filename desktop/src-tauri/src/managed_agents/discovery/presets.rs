...
pub(super) const PRESET_HARNESSES: &[PresetHarness] = &[
    PresetHarness {
        ...
        install_hint: "Maju talks to Pi through the pi-acp adapter. Install Pi with `npm install -g --ignore-scripts @earendil-works/pi-coding-agent`, then install the adapter with `npm install -g pi-acp`.",
    ...
    PresetHarness {
        ...
        install_hint: "Maju talks to Devin through the official Devin CLI's ACP mode (devin acp).",
        underlying_cli: None,
    },
    PresetHarness {
        id: "cursor",
