...
//! local Claude Code agents. `MAJU_ACP_MODEL` is removed from the spawned
...
/// (or remove it if `None`) and strip `MAJU_ACP_MODEL` from the spawned env.
///
/// Must be called after `descriptor.env` is written so that any user-supplied
/// `ANTHROPIC_MODEL` is overridden by the Maju-resolved value.
pub fn apply_claude_model_env(command: &mut std::process::Command, effective_model: Option<&str>) {
    // Remove MAJU_ACP_MODEL — the catalog-switch path is for live ACP switches
