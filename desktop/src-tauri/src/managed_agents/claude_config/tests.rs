...
/// A1: MAJU_ACP_MODEL must NOT be present in the spawned-child env after
/// `apply_claude_model_env`, even if it was set before (dual-authority defect).
/// ANTHROPIC_MODEL must be set to the resolved model.
#[test]
fn a1_maju_acp_model_absent_anthropic_model_present_after_env_apply() {
    let mut cmd = std::process::Command::new("true");
    // Simulate descriptor.env writing MAJU_ACP_MODEL (the pre-A1 path).
    cmd.env("MAJU_ACP_MODEL", "claude-opus-4");
    apply_claude_model_env(&mut cmd, Some("claude-opus-4"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();

    // MAJU_ACP_MODEL must be removed. Command::get_envs returns None for
    // explicitly-removed keys.
    let maju_acp = env_map.get(std::ffi::OsStr::new("MAJU_ACP_MODEL"));
    assert!(
        maju_acp.is_none() || maju_acp.unwrap().is_none(),
        "MAJU_ACP_MODEL must be absent (or explicitly removed) after A1 policy"
    );
...
fn a1_anthropic_model_removed_when_no_effective_model() {
    ...
    cmd.env("MAJU_ACP_MODEL", "claude-3-5-sonnet");
    ...
    assert!(
        maju_acp.is_none() || maju_acp.unwrap().is_none(),
        "MAJU_ACP_MODEL must always be absent after A1 policy"
    );
}
