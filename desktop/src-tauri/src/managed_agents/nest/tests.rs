...
fn nest_dir_is_under_home() {
    if let Some(dir) = nest_dir() {
        // Accepts both .maju (prod) and .maju-dev (dev) depending on
        // whether init_nest_dir was called before this test ran.
        let name = dir.file_name().and_then(|n| n.to_str()).unwrap_or("");
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir must end with .maju or .maju-dev, got {dir:?}"
        );
    }
}

#[test]
fn init_nest_dir_prod_sets_maju() {
    ...
    if let Some(d) = dir {
        ...
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir suffix must be .maju or .maju-dev, got {d:?}"
        );
    }
}
...
fn nest_skill_contains_safe_mention_workflow() {
    assert!(MAJU_CLI_SKILL_MD.contains("--mention <hex-or-npub>"));
    assert!(MAJU_CLI_SKILL_MD.contains("every presentation-only name that should notify"));
    assert!(MAJU_CLI_SKILL_MD
        .contains("permits unresolved or ambiguous `@Name` text as presentation-only"));
    assert!(MAJU_CLI_SKILL_MD.contains("signed event's `mention_pubkeys`"));
    assert!(MAJU_CLI_SKILL_MD.contains("no follow-up verification command is needed"));
    assert!(MAJU_CLI_SKILL_MD.contains("Add membership separately only when authorized"));
    assert!(MAJU_CLI_SKILL_MD.contains("never changes membership automatically"));
}

#[test]
fn ensure_nest_creates_all_dirs_and_agents_md() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join(".maju");
...
fn ensure_nest_skill_dir_has_700_permissions() {
    ...
    // Symlinks (e.g. .goose/skills/maju-cli) are skipped by the chmod loop.
    for dir in [
        ".agents",
        ".agents/skills",
        ".agents/skills/maju-cli",
        ".goose",
        ".goose/skills",
        ".claude",
        ".claude/skills",
...
fn ensure_nest_migrates_old_skill_dir() {
    ...
    let _ = fs::remove_file(root.join(".claude/skills/maju-cli"));
    let _ = fs::remove_dir_all(root.join(".agents/skills/maju-cli"));
    let old_skill_dir = root.join(".claude/skills/maju-cli");
    ...
    let _ = fs::remove_file(root.join(".agents/skills/maju-cli/.skill-version"));
    ...
    let new_skill = root.join(".agents/skills/maju-cli/SKILL.md");
    ...
    let old_path = root.join(".claude/skills/maju-cli");
    assert!(
        old_path
            .symlink_metadata()
            .unwrap()
...
fn ensure_skill_symlinks_are_idempotent() {
    ...
    for dir in [".goose/skills", ".claude/skills", ".codex/skills"] {
        ...
        assert_eq!(
            ...
            "symlink at {dir}/maju-cli should use relative target"
        );
    }
}
...
fn ensure_skill_symlinks_skips_existing_path_during_initial_pass() {
    ...
    // migrates pre-existing real directories at .claude/skills/maju-cli to
    ...
    let real_dir = root.join(".claude/skills/maju-cli");
    ...
    assert!(
        real_dir
            ...
            .file_type()
            .is_symlink(),
        ".claude/skills/maju-cli should be migrated to a symlink"
    );
    // The canonical path now holds the migrated content.
    let canonical = root.join(".agents/skills/maju-cli/SKILL.md");
    assert_eq!(
...
fn ensure_skill_symlinks_skip_dangling_symlink() {
    ...
    let dangling = codex_skills.join("maju-cli");
...
#[test]
fn cli_link_name_prod_is_maju() {
    assert_eq!(cli_link_name(false), "maju");
}

#[test]
fn cli_link_name_dev_is_maju_dev() {
    assert_eq!(cli_link_name(true), "maju-dev");
}
...
fn ensure_cli_symlink_creates_symlink_prod() {
    ...
    fs::write(exe_parent.join("maju"), "binary").unwrap();

    let local_bin = tmp.path().join("local_bin");
    fs::create_dir_all(&local_bin).unwrap();

    // Prod link name is "maju"; simulate the symlink creation path.
    let link = local_bin.join(cli_link_name(false));
    std::os::unix::fs::symlink(exe_parent.join("maju"), &link).unwrap();
    assert!(link.symlink_metadata().unwrap().file_type().is_symlink());
    assert_eq!(fs::read_link(&link).unwrap(), exe_parent.join("maju"));
}
...
fn ensure_cli_symlink_creates_symlink_dev() {
    ...
    // Dev link must be "maju-dev", never "maju".
    assert_eq!(cli_link_name(true), "maju-dev");
    ...
    assert!(!local_bin.join("maju").exists());
}
...
fn ensure_cli_symlink_does_not_clobber_regular_file_dev() {
    ...
    fs::create_dir_all(&local_bin).unwrap();
    let link = local_bin.join(cli_link_name(true));
    fs::write(&link, "user-installed maju-dev binary").unwrap();

    // Regular files at the dev path are also preserved.
    assert!(link.symlink_metadata().unwrap().file_type().is_file());
    assert_eq!(
        fs::read_to_string(&link).unwrap(),
        "user-installed maju-dev binary"
    );
}
...
fn test_upsert_managed_section_with_markers() {
    ...
    assert!(result.contains("<!-- BEGIN MAJU MANAGED"));
    assert!(result.contains("<!-- END MAJU MANAGED -->"));
...
fn test_upsert_managed_section_without_markers() {
    ...
    let begin_pos = result.find("<!-- BEGIN MAJU MANAGED").unwrap();
...
fn test_upsert_marker_in_code_block() {
    ...
    fs::write(
        &file,
        "# Header\n\n    <!-- BEGIN MAJU MANAGED — some indented marker -->\n\nReal content here\n",
    )
    .unwrap();
    ...
    assert!(
        result.contains("    <!-- BEGIN MAJU MANAGED — some indented marker -->"),
    ...
    let begin_pos = result
        .find("<!-- BEGIN MAJU MANAGED — regenerated")
...
fn refresh_skill_md_writes_version_file() {
    ...
    let root = tmp.path().join(".maju");
    ensure_nest_at(&root).unwrap();
    let version = fs::read_to_string(root.join(".agents/skills/maju-cli/.skill-version")).unwrap();
...
fn refresh_skill_overwrites_on_version_bump() {
    ...
    let skill_md = root.join(".agents/skills/maju-cli/SKILL.md");
