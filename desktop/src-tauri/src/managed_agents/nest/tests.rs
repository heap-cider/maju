...
fn cli_link_name_prod_follows_build_identity() {
    let expected = crate::build_identity::demo_slug()
        .map(|slug| format!("maju-demo-{slug}"))
        .unwrap_or_else(|| "maju".to_string());
...
fn cli_link_name_dev_follows_build_identity() {
    let expected = crate::build_identity::demo_slug()
        .map(|slug| format!("maju-demo-{slug}"))
        .unwrap_or_else(|| "maju-dev".to_string());
...
fn ensure_cli_symlink_creates_symlink_dev() {
    ...
    // Dev and demo links must never overwrite production's "maju".
    assert_ne!(cli_link_name(true), "maju");
