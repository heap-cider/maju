...
pub(crate) const DISPLAY_NAME_ENV_VAR: &str = "MAJU_ACP_DISPLAY_NAME";
...
/// place.
pub(crate) const REPLAY_FLOOR_ENV_VAR: &str = "MAJU_ACP_REPLAY_FLOOR";
...
/// wins over any user-supplied `MAJU_ACP_REPLAY_FLOOR` entry — the same
...
/// filter that already mentions `maju_acp`, append `maju_acp=info` to any other
/// non-empty filter, and default to `maju_acp=info` when unset.
