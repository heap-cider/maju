...
pub(crate) fn sweep_orphaned_agent_processes(app: &AppHandle, skip_pids: &[u32]) {
    ...
    let targets: Vec<i32> = legacy_entries
        ...
        .filter(|pid| {
            ...
            // time — they are Maju-owned by construction; no name gate needed.
            // Kill live processes; dead ones fall through to receipt cleanup.
            (process_is_running(*pid) && process_has_maju_marker(*pid, &instance_id))
    ...
    for (pubkey, pid) in &legacy_entries {
        ...
        if !process_is_running(*pid) || !process_has_maju_marker(*pid, &instance_id) {
    ...
    for (_, receipt) in &receipt_entries {
        ...
        if !process_is_running(receipt.pid) || !process_has_maju_marker(receipt.pid, &instance_id) {
...
//
// The `MAJU_MANAGED_AGENT` env marker is the SOLE authoritative ownership
...
// On Windows no `/proc`-based sweep runs, so `process_has_maju_marker`
// always returns `false`.

/// Enumerate all processes on the system owned by the current user and kill any
/// agent binary stamped with *this* instance's `MAJU_MANAGED_AGENT` marker
...
/// while leaving another live Maju instance's agents untouched.
#[cfg(target_os = "macos")]
pub(crate) fn sweep_system_agent_processes(instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        // MAJU_MANAGED_AGENT env marker is the authoritative ownership proof.
        if !process_has_maju_marker(upid, instance_id) {
            continue;
        }
    ...
    if !orphans.is_empty() {
        eprintln!(
            "maju-desktop: system sweep found {} orphaned agent process(es), cleaning up",
            orphans.len()
        );
...
pub(crate) fn sweep_system_agent_processes_with_grace(
    ...
    if !confirmed.is_empty() {
        eprintln!(
            "maju-desktop: periodic sweep confirmed {} orphaned agent process(es), cleaning up",
            confirmed.len()
        );
