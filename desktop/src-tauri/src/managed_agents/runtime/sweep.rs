...
//! or that predates `MAJU_MANAGED_AGENT` injection. This sweep derives the
//! expected `maju-acp` path from the running executable and kills any process
...
///    intermediates run in their own process groups (e.g. maju-acp ->
///    node shim -> codex-acp).
#[cfg(target_os = "macos")]
...
///
/// On Linux, `read_link("/proc/<pid>/exe")` returns `…/maju-acp (deleted)`
...
/// `extract_maju_marker_value` / `process_has_maju_marker` which must skip
...
/// binary name (`maju-acp`) proceed to the full exe-path fetch.  This mirrors
...
/// processes but at most a handful of `maju-acp` instances).
#[cfg(target_os = "macos")]
...
fn collect_process_snapshots(harness_name: &str) -> Vec<ProcessSnapshot> {
    ...
    for entry in entries.flatten() {
        ...
        // "maju-acp" is 8 chars so it's always preserved).
...
/// Derive the expected path of the `maju-acp` harness binary next to the
/// current executable. Returns `None` if `current_exe()` fails or has no
/// parent directory.
///
/// In a `.app` bundle: `.../Contents/MacOS/maju-acp`.
/// In a dev checkout: `<target-dir>/debug/maju-acp` or similar.
...
/// scopes by `MAJU_MANAGED_AGENT` instance ID rather than exe path.
#[cfg(unix)]
pub fn expected_harness_exe_path() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    let dir = exe.parent()?;
    let raw = dir.join("maju-acp");
...
#[cfg(unix)]
const HARNESS_BINARY_NAME: &str = "maju-acp";

// ── sweep_untracked_bundle_harnesses ─────────────────────────────────────

/// Sweep and kill harness processes that share this bundle's exact `maju-acp`
/// executable path but are not in `skip_pids`.
///
/// Complements the env-var-based `sweep_system_agent_processes`: this sweep
/// catches orphans that predate the `MAJU_MANAGED_AGENT` env var injection
...
/// where possible) equals `<this bundle>/maju-acp` are candidates. Dev builds
...
pub(crate) fn sweep_untracked_bundle_harnesses(skip_pids: &[u32]) {
    ...
    eprintln!(
        "maju-desktop: sweep_untracked_bundle_harnesses: reaping {} stale harness process(es) {:?} (exe: {})",
        to_kill.len(),
        to_kill,
        harness_exe.display(),
    );
...
mod tests {
    ...
    fn strip_deleted_suffix_removes_kernel_suffix() {
        ...
        let p = PathBuf::from("/Applications/Maju.app/Contents/MacOS/maju-acp (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/Applications/Maju.app/Contents/MacOS/maju-acp")
        );
    }

    #[test]
    fn strip_deleted_suffix_leaves_normal_path_unchanged() {
        let p = PathBuf::from("/Applications/Maju.app/Contents/MacOS/maju-acp");
    ...
    fn strip_deleted_suffix_does_not_strip_partial_match() {
        ...
        let p = PathBuf::from("/some/path/maju-acp(deleted)");
        assert_eq!(strip_deleted_suffix(p.clone()), p,);
    }

    // ── select_untracked_bundle_harnesses ────────────────────────────────

    const BUNDLE_HARNESS: &str = "/Applications/Maju.app/Contents/MacOS/maju-acp";
    const DEV_HARNESS: &str = "/Users/dev/maju/.worktrees/main/target/debug/maju-acp";
    ...
    fn child_of_tracked_parent_not_directly_targeted() {
        ...
        let snapshots = vec![snap(1004, "/Applications/Maju.app/Contents/MacOS/goose")];
        let result =
            select_untracked_bundle_harnesses(&snapshots, &PathBuf::from(BUNDLE_HARNESS), &[]);
        assert!(result.is_empty());
    }
    ...
    fn deleted_suffix_stripped_path_matches_expected() {
        ...
        let raw = PathBuf::from("/Applications/Maju.app/Contents/MacOS/maju-acp (deleted)");
