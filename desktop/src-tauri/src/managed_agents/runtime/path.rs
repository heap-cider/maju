...
/// with only Maju's managed Node dirs — no `curl`, `sh`, or `tar`, which
...
///   1. `managed` — Maju-controlled dirs (highest precedence, e.g. managed Node/npm bins)
...
///   2. Maju-managed npm prefix bin dir — app-private ACP adapter shims
///   3. Maju-managed Node.js bin dir — app-private Node/npm runtime
...
/// guards against, which left managed agents unable to find `maju`. Returns
...
pub(in crate::managed_agents) fn build_augmented_path(
    ...
    if has_local_context {
        if let Some(managed_npm_bin) = crate::managed_agents::maju_managed_npm_bin_dir() {
            managed.push(managed_npm_bin);
        }
        if let Some(managed_node_bin) = crate::managed_agents::maju_managed_node_bin_dir() {
...
mod tests {
    ...
    fn splits_colon_delimited_shell_path() {
        ...
        let result = build_augmented_path(
            ...
            Some(PathBuf::from("/Applications/Maju.app/Contents/MacOS")),
        ...
        assert!(
            result.contains(":/Applications/Maju.app/Contents/MacOS:"),
            "{result}"
        );
        assert!(
    ...
    fn nvm_bin_inserted_after_local_bin_before_exe_parent() {
        ...
        let nvm = result
            ...
            .unwrap();
        let exe = result
            .find("/Applications/Maju.app/Contents/MacOS")
            .unwrap();
    ...
    /// the inherited process PATH — otherwise the child gets only Maju-managed
...
mod compose_tests {
    ...
    fn managed_entries_appear_first() {
        let managed = vec![p("/maju/node/bin"), p("/maju/npm/bin")];
        let login = vec![p("/usr/local/bin"), p("/usr/bin")];
        let result = compose_path_entries(managed, login, vec![], false);
        assert_eq!(result[0], p("/maju/node/bin"), "managed[0] must be first");
        assert_eq!(result[1], p("/maju/npm/bin"), "managed[1] must be second");
        assert_eq!(
            result[2],
            p("/usr/local/bin"),
