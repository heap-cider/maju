...
pub fn kill_stale_tracked_processes(
    ...
    kill_stale_tracked_processes_with(
        records,
        runtimes,
        |pid| process_has_maju_marker(pid, instance_id),
        terminate_process,
    )
}
...
/// `MAJU_MANAGED_AGENT` marker; `kill(pid)` performs the termination.
pub(crate) fn kill_stale_tracked_processes_with(
    ...
    for record in records.iter_mut() {
        ...
        if !runtimes.keys().any(|key| key.pubkey == record.pubkey) {
            ...
            // binary names not in KNOWN_AGENT_BINARIES. MAJU_MANAGED_AGENT is the
