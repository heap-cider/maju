...
/// Binary names for the Maju desktop/Tauri process. Used by dead-instance
...
const DESKTOP_BINARY_NAMES: &[&str] = &[
    ...
    // AppImage shim's real executable name, `maju-desktop.bin`.
    "maju-desktop.bi",
];

/// Check if a process name matches a known Maju desktop binary.
...
/// JSON as `"identifier":"xyz.block.maju.app.dev"` and in environment entries
...
/// `xyz.block.maju.app` from matching inside `xyz.block.maju.app.dev`.
...
/// Extract the `MAJU_MANAGED_AGENT` value from a process's environment.
/// Returns `None` if the process doesn't have the marker or can't be read.
#[cfg(target_os = "macos")]
fn extract_maju_marker_value(pid: u32) -> Option<String> {
    let prefix = b"MAJU_MANAGED_AGENT=";
...
#[cfg(not(unix))]
fn extract_maju_marker_value(_pid: u32) -> Option<String> {
    None
}

/// Check if a Maju desktop process is still alive for the given instance ID.
/// Scans all user-owned processes named "Maju" or "maju-desktop" and checks
...
/// Reap agent processes belonging to dead Maju desktop instances.
///
/// Scans all user processes for `MAJU_MANAGED_AGENT=*`, groups them by
/// instance ID, and for each foreign instance (≠ `our_instance_id`) checks
/// whether a Maju desktop binary is still alive for that instance. If not,
...
pub(crate) fn reap_dead_instance_agents(our_instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        // arbitrary binary names and MAJU_MANAGED_AGENT is the authoritative
        // ownership proof.
        let Some(agent_instance_id) = extract_maju_marker_value(upid) else {
            continue;
        };
    ...
    for (instance_id, agent_pids) in &foreign_agents {
        ...
        eprintln!(
            "maju-desktop: reaping {} orphaned agent(s) from dead instance '{instance_id}'",
            agent_pids.len()
        );
