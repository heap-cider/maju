...
/// Binary name fragments for all known agent/harness processes that Maju
...
pub(crate) const KNOWN_AGENT_BINARIES: &[&str] = &[
    ...
    "codex_acp",
    "goose",
    // maju-dev-mcp's multicall personalities (rg, tree, maju,
...
/// carry `MAJU_MANAGED_AGENT` in its environment (checked by the caller via
/// `process_has_maju_marker()`). This avoids sweeping unrelated node processes.
#[cfg(any(unix, test))]
...
/// Callers must additionally verify `MAJU_MANAGED_AGENT` ownership.
#[cfg(any(unix, test))]
...
pub(crate) fn process_belongs_to_us(pid: u32) -> bool {
    ...
    // the caller's `process_has_maju_marker()` check decides true ownership.
...
/// The value stamped into the `MAJU_MANAGED_AGENT` env var of every agent we
/// spawn, identifying *which* desktop instance owns it. We use the app's bundle
/// identifier (`xyz.block.maju.app` for release, `xyz.block.maju.app.dev`
...
/// build's agents, and vice versa). This is what lets two Majus coexist on
...
/// Build the full `MAJU_MANAGED_AGENT=<instance-id>` env entry we match
/// against when scanning processes. Kept here so the spawn stamp and the sweep
/// matcher can never drift apart.
#[cfg(any(unix, test))]
pub(super) fn maju_marker_entry(instance_id: &str) -> Vec<u8> {
    format!("MAJU_MANAGED_AGENT={instance_id}").into_bytes()
}

/// Check if a running process is one of *our* managed agents: it must carry
/// `MAJU_MANAGED_AGENT=<instance_id>` in its environment, where `instance_id`
/// is this desktop instance's id. A process stamped with a *different* instance
/// id belongs to another live Maju app and must never be reaped here.
#[cfg(target_os = "macos")]
pub(crate) fn process_has_maju_marker(pid: u32, instance_id: &str) -> bool {
    let marker = maju_marker_entry(instance_id);
...
#[cfg(not(unix))]
pub(crate) fn process_has_maju_marker(_pid: u32, _instance_id: &str) -> bool {
    false
}
...
/// and signal the groups. An orphaned grandchild (e.g. `goose` or `maju-dev-mcp`)
...
pub(super) fn resolve_pgids_and_kill(candidate_pids: &[i32]) {
    ...
    if pgids.is_empty() && candidate_groups > 0 {
        eprintln!(
            "maju-desktop: orphan sweep: skipped all {candidate_groups} candidate group(s) (live foreign group leader or candidate already exited); nothing signalled"
        );
    }
...
pub(crate) fn valid_agent_runtime_receipt_with(
    ...
    canonical == receipt.key
        ...
        // Maju-owned by construction. Marker-only ownership: custom-harness
