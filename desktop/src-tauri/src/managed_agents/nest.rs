//! Maju Nest — persistent agent workspace at `~/.maju`.
//!
//! Creates a shared knowledge directory on first launch so every
//! Maju-spawned agent starts with orientation (AGENTS.md) and a
...
/// Default SKILL.md content for the maju-cli skill.
/// Written to ~/.maju/.agents/skills/maju-cli/SKILL.md on first init.
const MAJU_CLI_SKILL_MD: &str = include_str!("nest_skill.md");
...
/// when the Tauri app-data directory name starts with `"xyz.block.maju.app.dev"`.
...
/// Returns `true` iff `path` ends with the dev-nest directory name (`.maju-dev`).
///
...
/// Returns `true` when the running binary is using the dev nest (`~/.maju-dev`).
///
...
/// directory starts with `"xyz.block.maju.app.dev"`.
///
/// Returns `false` when:
/// - The nest is the production nest (`~/.maju`, signed DMG).
...
/// Creates the Maju nest at `~/.maju` if it doesn't already exist.
///
...
/// Creates a Maju nest at the given `root` path.
///
/// - Creates the root directory and all subdirectories.
/// - Writes `AGENTS.md` only if it doesn't already exist.
/// - Writes `.agents/skills/maju-cli/SKILL.md` only if it doesn't already exist.
/// - Creates harness-specific symlinks pointing to the canonical
///   `.agents/skills/maju-cli` directory for each known provider.
...
pub fn ensure_nest_at(root: &Path) -> Result<(), String> {
    ...
    // Write maju-cli skill to the harness-agnostic .agents path.
        ...
        .open(&skill_md)
    {
        Ok(mut file) => {
            use std::io::Write;
            file.write_all(MAJU_CLI_SKILL_MD.as_bytes())
    ...
    // Migration of the old .claude/skills/maju-cli real dir is handled in
...
///
/// Dev builds (`is_dev = true`) use `"maju-dev"` so that a running DMG and a
/// concurrent dev build each own a separate link and never clobber each other —
/// the same isolation that separates `~/.maju` (prod) from `~/.maju-dev` (dev).
...
/// Ensures `~/.local/bin/maju` (prod) or `~/.local/bin/maju-dev` (dev) is a
...
/// `~/.maju` and `~/.maju-dev` nests (see [`NEST_DIR_DEV`]).
///
/// On every boot: replaces any existing symlink unconditionally (the `maju` /
/// `maju-dev` name is our namespace), creates a new one if absent, and leaves
...
pub fn ensure_cli_symlink(exe_parent: &Path, is_dev: bool) -> Result<(), String> {
    let maju_bin = exe_parent.join("maju");
    if !maju_bin.exists() {
...
///
/// Preserves everything from the `<!-- BEGIN MAJU MANAGED` marker onward
...
fn refresh_skill_md_if_stale(root: &Path) -> Result<(), String> {
    let agents_skill_dir = root.join(".agents/skills/maju-cli");
    let version_path = agents_skill_dir.join(".skill-version");
    if read_version_file(&version_path) >= NEST_SKILL_VERSION {
        return Ok(());
    }

    // Migration: if .claude/skills/maju-cli exists as a real directory
    ...
    let skill_content = if old_is_real_dir {
        ...
        fs::read_to_string(old_skill_dir.join("SKILL.md"))
            .unwrap_or_else(|_| MAJU_CLI_SKILL_MD.to_string())
    } else {
        MAJU_CLI_SKILL_MD.to_string()
    };
    ...
    // Create/replace the .claude/skills/maju-cli symlink.
    #[cfg(unix)]
    {
        ...
        let symlink_path = root.join(".claude/skills/maju-cli");
        ...
        create_symlink(
            std::path::Path::new("../../.agents/skills/maju-cli"),
            &symlink_path,
        )
...
pub fn try_regenerate_nest(app: &AppHandle) {
    if let Err(error) = regenerate_nest_context(app) {
        eprintln!("maju-desktop: nest context regeneration failed: {error}");
    }
}
