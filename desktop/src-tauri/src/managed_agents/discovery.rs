...
mod windows_install;
#[cfg(all(test, unix))]
pub(crate) use login_shell::login_shell_candidates;
mod catalog;
...
/// is not on PATH on a stock Windows install; maju-agent ships with the app.)
...
///
/// Consults the Maju-managed shim dir (a filesystem stat, never a spawn) and
...
pub fn resolve_command_cached(command: &str) -> Option<PathBuf> {
    ...
    // Bundled sidecars (e.g. `maju-agent`) ship next to the app executable, so
    ...
    // until a forced discovery warmed the resolve cache, so `maju-agent` (which
...
pub fn missing_command_message(command: &str, role: &str) -> String {
    ...
    format!(
        "{role} `{command}` was not found. Make sure it is installed and on your PATH. Antivirus software can quarantine bundled binaries — if that happened, restore the file or reinstall Maju. (Source builds: see TESTING.md.)"
    )
}
