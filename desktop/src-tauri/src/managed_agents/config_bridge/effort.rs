...
//! would apply over the runtime env configuration and
//! the sanitized per-tier env inputs, in the CLEAR authority order:
//!
//! ```text
//! record native(valid) > record legacy(valid)
...
//! sole authority and a user-supplied `MAJU_ACP_EFFORT_LEVEL` is transport, not
//! a tier). The **emission** key ([`EffortLaunch::key`]) is
//! `thinking_env_var.unwrap_or(MAJU_ACP_EFFORT_LEVEL)`: Goose emits
//! `GOOSE_THINKING_EFFORT`, maju-agent emits `MAJU_AGENT_THINKING_EFFORT`,
//! Claude/Codex/keyless-ACP and any unknown/custom runtime emit the retained
//! ACP-startup sentinel `MAJU_ACP_EFFORT_LEVEL`.
//!
...
pub(crate) const ACP_STARTUP_EFFORT_KEY: &str = "MAJU_ACP_EFFORT_LEVEL";
...
impl EffortLaunch {
    ...
    /// case-sensitive layer merge (e.g. a lower-tier `MAJU_ACP_EFFORT_LEVEL`
    /// plus a higher-tier `maju_acp_effort_level`); the carry selects the LAST
...
///   2. Legacy key (`MAJU_AGENT_THINKING_EFFORT`) — only when the native key
...
///   → `None` (skip as absent).
/// - `contract` absent but `accepted` present (maju-agent): validation-only —
...
///   `MAJU_AGENT_THINKING_EFFORT=off`, which crashes the child at config init.
/// - both absent (Claude/Codex, unknown/custom): raw passthrough — the value
///   rides `MAJU_ACP_EFFORT_LEVEL` to an adapter that accepts any string.
...
/// `build_maju_agent_provider_defaults` (which writes raw baked env) and
...
pub(crate) fn effort_launch_projection(
    ...
    // a user-set case variant (e.g. `maju_acp_effort_level`) is never intentional
    // config, and leaving one to shadow the emitted `MAJU_ACP_EFFORT_LEVEL` on
    ...
    // Value gate: Goose canonicalizes through its alias contract; maju-agent
...
/// and by `effort_cmd_tests` (test seam). Deleting `build_maju_agent_provider_defaults`
...
pub(crate) fn apply_spawn_effort_env(
    ...
    crate::managed_agents::agent_env::build_maju_agent_provider_defaults(cmd);
