...
pub(super) const MAJU_AGENT_KEY: &str = "MAJU_AGENT_THINKING_EFFORT";
pub(super) const ACP_KEY: &str = "MAJU_ACP_EFFORT_LEVEL";
...
fn maju_agent() -> &'static KnownAcpRuntime {
    known_acp_runtime_exact("maju-agent").expect("maju-agent runtime in catalog")
}
...
fn goose_emits_only_goose_key() {
    let mut r = record();
    r.env_vars.insert(MAJU_AGENT_KEY.into(), "high".into());
...
#[test]
fn maju_agent_passes_raw_contract_less_value_under_native_key() {
    let mut r = record();
    // maju-agent has no static normalization contract: a per-model value that
    // Goose would reject (e.g. "minimal") passes through raw.
    r.env_vars.insert(MAJU_AGENT_KEY.into(), "minimal".into());
    let launch = project_record_only(&r, Some(maju_agent()));
    assert_eq!(launch.value.as_deref(), Some("minimal"));
    assert_eq!(launch.key, MAJU_AGENT_KEY);
}
...
#[test]
fn decisive_record_native_outranks_a_different_valid_legacy_alias() {
    // The mixed-authority pin Thufir/Will require: a valid record-native env
    // key and a DIFFERENT valid canonical legacy_alias must resolve to the
    ...
    assert_eq!(
        ...
        Some("low"),
        "record-native env outranks the canonical legacy_alias"
    );
}

#[test]
fn canonical_legacy_alias_wins_when_no_record_native() {
    // No record-native key present: the legacy_alias is the next tier and wins over
...
fn record_legacy_alias_wins_over_persona_for_goose() {
    // Record legacy `MAJU_AGENT_THINKING_EFFORT` outranks persona for a runtime
    // whose native key differs from the legacy key.
    let mut r = record();
    r.persona_id = Some("p".into());
    r.env_vars = env(&[(MAJU_AGENT_KEY, "max")]);
    let personas = vec![persona("p", env(&[(GOOSE_KEY, "low")]))];
    let launch = effort_launch_projection(
        &r,
        Some(goose()),
...
#[test]
fn goose_alias_legacy_alias_xhigh_normalizes_to_max() {
    let mut r = record();
    r.env_vars.insert(MAJU_AGENT_KEY.into(), "xhigh".into());
    let launch = project_record_only(&r, Some(goose()));
    assert_eq!(launch.value.as_deref(), Some("max"));
}

#[test]
fn invalid_goose_legacy_alias_skips_and_falls_through_to_persona() {
...
fn suppress_covers_all_native_legacy_and_sentinel_keys() {
    ...
    assert!(
        keys.contains(&MAJU_AGENT_KEY),
        "maju-agent native + legacy key suppressed"
    );
...
fn apply_strips_every_foreign_effort_key_then_emits_one() {
    ...
    assert_eq!(launch_env.get(MAJU_AGENT_KEY), None);
    assert_eq!(
...
#[test]
fn maju_agent_generic_legacy_alias_does_not_leak_acp_sentinel() {
    let mut r = record();
    r.env_vars = env(&[(ACP_KEY, "high")]);
    r.env_vars.insert(MAJU_AGENT_KEY.into(), "medium".into());
    let launch = project_record_only(&r, Some(maju_agent()));
    ...
    assert_eq!(
        ...
        None,
        "ACP sentinel stripped for maju-agent"
    );
    assert_eq!(
        launch_env.get(MAJU_AGENT_KEY).map(String::as_str),
        Some("medium")
    );
}
...
fn unknown_runtime_does_not_suppress_user_effort_env() {
    ...
    // `[MAJU_ACP_EFFORT_LEVEL]` — no foreign effort key is stripped.
    let mut r = record();
...
#[test]
fn unknown_runtime_keeps_user_acp_sentinel_when_no_legacy_alias() {
    // Custom adapter with hand-set sentinel and no legacy_alias: sentinel carries through.
    let mut r = record();
    ...
    assert_eq!(
        ...
        Some("low"),
        "hand-set sentinel survives on an unknown runtime with no legacy_alias"
    );
}

#[test]
fn unknown_runtime_collapses_mixed_case_sentinel_to_canonical_when_no_legacy_alias() {
    // Carl P2 (no-legacy_alias): a hand-set mixed-case sentinel on a custom runtime
    ...
    let mut r = record();
    r.env_vars = env(&[("maju_acp_effort_level", "low")]);
    let launch = project_record_only(&r, None);
    assert_eq!(launch.value, None);
    let mut launch_env = r.env_vars.clone();
    launch.apply(&mut launch_env);
    ...
    assert_eq!(
        launch_env.get("maju_acp_effort_level"),
        None,
...
#[test]
fn unknown_runtime_no_legacy_alias_multi_variant_preserves_windows_effective_value() {
    ...
    let mut r = record();
    r.env_vars = env(&[(ACP_KEY, "high"), ("maju_acp_effort_level", "low")]);
    let launch = project_record_only(&r, None);
    assert_eq!(launch.value, None);
    assert!(launch.preserve_passthrough);
    let mut launch_env = r.env_vars.clone();
    ...
    assert_eq!(launch_env.get("maju_acp_effort_level"), None);
...
#[test]
fn goose_off_legacy_alias_skips_for_maju_agent_destination() {
    // Regression: canonical legacy_alias `off` is valid Goose but NOT a maju-agent
    // effort. Switching a record with effort_level=off to maju-agent must NOT
    // emit MAJU_AGENT_THINKING_EFFORT=off — parse_thinking_effort rejects it and
    // the child exits 2. Invalid → skip as absent → no key emitted.
    let mut r = record();
    r.env_vars.insert(MAJU_AGENT_KEY.into(), "off".into());
    let launch = project_record_only(&r, Some(maju_agent()));
    assert_eq!(
        launch.value, None,
        "foreign canonical `off` skipped for maju-agent's vocabulary"
    );

    let mut launch_env = BTreeMap::new();
    launch.apply(&mut launch_env);
    assert_eq!(
        launch_env.get(MAJU_AGENT_KEY),
        None,
...
#[test]
fn maju_agent_minimal_legacy_alias_skips_for_goose_destination() {
    // The reverse: `minimal` is a valid maju-agent effort but invalid Goose, so
...
#[test]
fn maju_agent_accepts_its_own_distinct_efforts() {
    // maju-agent keeps xhigh and max distinct (no Goose-style xhigh→max
    // collapse): both are valid and pass through unchanged.
    for v in ["xhigh", "max", "none", "minimal"] {
        let mut r = record();
        r.env_vars.insert(MAJU_AGENT_KEY.into(), v.into());
        let launch = project_record_only(&r, Some(maju_agent()));
        assert_eq!(
            launch.value.as_deref(),
            Some(v),
            "maju-agent accepts `{v}` verbatim (no alias collapse)"
        );
    }
}
...
fn mixed_case_native_key_is_read_and_wins() {
    ...
    assert_eq!(
        launch.value.as_deref(),
        Some("low"),
        "mixed-case record-native key is read and outranks the legacy_alias"
    );
}
...
#[test]
fn native_acp_options_remain_exact_and_do_not_create_a_legacy_sentinel() {
    let mut r = record();
    r.env_vars.insert(
        "MAJU_ACP_CONFIG_OPTIONS".into(),
        r#"{"reasoning_effort":"ultra","service_tier":"fast"}"#.into(),
    );
    for runtime in [Some(claude()), None] {
        let launch = project_record_only(&r, runtime);
        let mut launch_env = r.env_vars.clone();
        launch.apply(&mut launch_env);
        assert_eq!(launch_env, r.env_vars);
        assert!(!launch_env.contains_key(ACP_KEY));
    }
}
