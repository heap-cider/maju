...
/// [`effort`] to translate old saves. maju-agent's native key equals this
/// constant, so no aliasing applies there.
pub(crate) const LEGACY_THINKING_EFFORT_KEY: &str = "MAJU_AGENT_THINKING_EFFORT";
...
/// so env vars we would otherwise require are not needed from Maju.
