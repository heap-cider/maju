...
/// Baked `MAJU_AGENT_THINKING_EFFORT` collision: legacy alias stripped.
#[test]
fn strip_removes_baked_maju_agent_native_key_collision() {
    let mut cmd = std::process::Command::new("echo");
    cmd.env(MAJU_AGENT_KEY, "medium");
    strip_effort_keys_from_command(&mut cmd);
    let removed = cmd
        .get_envs()
        .any(|(key, value)| key == MAJU_AGENT_KEY && value.is_none());
    assert!(
        removed,
        "MAJU_AGENT_THINKING_EFFORT must be registered for removal after strip"
    );
}
...
// Deletion proofs:
//   - remove `build_maju_agent_provider_defaults` inside → baked keys leak;
...
fn production_sequence_goose_inherited_collision_resolved_in_child() {
    ...
    cmd.env(MAJU_AGENT_KEY, "legacy-medium");
    cmd.env("MY_AGENT_CONFIG", "keep-me");

    let mut r = record();
    r.env_vars.insert(GOOSE_KEY.into(), "high".into());
    ...
    assert!(
        !child_env.contains(MAJU_AGENT_KEY),
        "legacy maju-agent key must not reach child; env:\n{child_env}"
    );
    assert!(
    ...
    let effort_key_count = [GOOSE_KEY, MAJU_AGENT_KEY, ACP_KEY]
        .iter()
