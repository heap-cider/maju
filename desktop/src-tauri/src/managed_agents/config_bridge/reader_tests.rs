...
fn config_file_only_system_prompt_surfaces_as_config_file_origin() {
    ...
    // system_prompt — the UI writes back via MAJU_ACP_SYSTEM_PROMPT.
    let record = test_record();
    ...
    assert!(matches!(
        ...
        ConfigWriteMechanism::RespawnWithEnvVar { ref env_key }
            if env_key == "MAJU_ACP_SYSTEM_PROMPT"
    ));
...
fn explicit_record_model_not_retagged_when_already_present() {
    let mut record = test_record();
    // Record already has its own model — origin stays MajuExplicit.
    record.model = Some("explicit-model".to_string());
    let runtime = test_runtime();
...
fn maju_agent_runtime() -> &'static KnownAcpRuntime {
    &KnownAcpRuntime {
        ...
        max_rounds_env_var: Some("MAJU_AGENT_MAX_ROUNDS"),
        required_normalized_fields: &["model", "provider"],
        login_hint: None,
        auth_probe_args: None,
    }
...
fn maju_agent_rt() -> &'static KnownAcpRuntime {
    crate::managed_agents::discovery::known_acp_runtime_exact("maju-agent")
        .expect("maju-agent must be in catalog")
}

/// AC-1: no record effort, global env has effort → GlobalDefault.
/// Real-world case: global-agent-config has MAJU_AGENT_THINKING_EFFORT=high,
...
fn global_effort_surfaces_as_global_default_when_record_has_none() {
    ...
    let tiers = global_env_tiers("MAJU_AGENT_THINKING_EFFORT", "high");
...
fn persona_effort_shadows_global_and_tags_persona_default() {
    let record = test_record();
    let runtime = maju_agent_rt();
    let tiers = persona_and_global_env_tiers("MAJU_AGENT_THINKING_EFFORT", "medium", "high");
...
/// AC-3: record-level effort wins over persona and global, stays MajuExplicit.
#[test]
fn record_effort_outranks_persona_and_global_keeps_maju_explicit() {
    let mut record = test_record();
    record.env_vars.insert(
        "MAJU_AGENT_THINKING_EFFORT".to_string(),
        "xhigh".to_string(),
