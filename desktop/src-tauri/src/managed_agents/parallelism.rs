...
//
//   * local spawn  — `MAJU_ACP_AGENTS` in the child environment
//   * remote deploy — `launch.policy_env["MAJU_ACP_AGENTS"]` + legacy field
...
///
/// Each maju-acp worker spawned by the Desktop is a client of the single
...
/// Return the value to emit as `MAJU_ACP_AGENTS` for a spawn command.
///
...
/// a decimal string ready for `command.env("MAJU_ACP_AGENTS", …)`.
///
...
mod tests {
    ...
    fn policy_table() {
        ...
        assert_eq!(super::harness_max_parallelism("maju-agent"), None);
        ...
        assert_eq!(super::effective_parallelism("maju-agent", 32), 32);
    }
    ...
    /// Legacy OpenClaw record (parallelism 10, above cap): MAJU_ACP_AGENTS must be "5".
    #[test]
    fn acp_agents_value_openclaw_above_cap_is_capped() {
        assert_eq!(
            super::acp_agents_value("openclaw", 10),
            "5",
            "MAJU_ACP_AGENTS for openclaw with parallelism 10 must be \"5\""
        );
