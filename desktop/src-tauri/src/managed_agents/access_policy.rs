...
//!
//! `MAJU_BUILD_AGENT_ACCESS_OWNER_ONLY` marks a build whose managed agents may
...
//!   `MAJU_ACP_RESPOND_TO` to `owner-only` and pins the independent
//!   `MAJU_ACP_ALLOWED_RESPOND_TO=owner-only` guard on every start, whatever
//!   the record says.
...
//! `crates/maju-acp/src/lib.rs`). That is the intended boundary, not an
//! oversight: an owner's own agents are inside their trust boundary, and Maju's
...
/// Release packaging sets `MAJU_BUILD_AGENT_ACCESS_OWNER_ONLY`; OSS/custom
/// builds do not.
pub(crate) fn owner_only_access_build() -> bool {
    option_env!("MAJU_DESKTOP_BUILD_AGENT_ACCESS_OWNER_ONLY").is_some()
}
...
pub(crate) fn build_respond_to_env_with_policy(
    ...
    let mut set = vec![("MAJU_ACP_RESPOND_TO", respond_to.as_str().to_string())];
    ...
    if enforced_owner_only {
        ...
        remove.push("MAJU_ACP_ALLOWED_RESPOND_TO");
    }
...
mod tests {
    ...
    fn owner_only_access_enforcement_clamps_local_and_provider() {
        for (label, backend) in [
            ...
            assert_eq!(
                gate_set.get("MAJU_ACP_RESPOND_TO").map(String::as_str),
                Some("owner-only"),
