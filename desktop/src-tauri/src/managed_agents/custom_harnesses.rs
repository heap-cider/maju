...
pub(crate) struct HarnessDefinition {
    ...
    /// first and LOSES on conflict with Maju-injected vars — `MAJU_MANAGED_AGENT`
...
fn validate_harness_definition(def: &HarnessDefinition) -> Result<(), String> {
    ...
    // `MAJU_ACP_AGENT_ARGS` env transport (clap `value_delimiter = ','` on the
    // maju-acp side), so a literal comma inside one argument would silently
    ...
    // reserved-key bypass exploit (MAJU_AUTH_TAG=x forgery shape), rejects
...
// spawn time, causing silent fallback to maju-agent.
//
...
// registry, the caller gets a typed error — never a silent maju-agent fallback.
...
mod tests {
    ...
    fn validate_rejects_malformed_key_with_equals_sign() {
        // MAJU_AUTH_TAG=x is the documented reserved-key bypass shape:
        // the key contains '=' so Command::env would produce
        // `MAJU_AUTH_TAG=x=forged` in the child env.
        let mut env = BTreeMap::new();
        env.insert("MAJU_AUTH_TAG=x".to_string(), "forged".to_string());
        let def = HarnessDefinition {
        ...
        assert!(
            err.contains("MAJU_AUTH_TAG"),
            "error must name the offending key: {err}"
        );
    }

    #[test]
    fn validate_rejects_reserved_key_maju_managed_agent() {
        // MAJU_MANAGED_AGENT and MAJU_MANAGED_AGENT_START_NONCE are the
        // ownership markers — supplying them in a definition must be rejected.
        let mut env = BTreeMap::new();
        env.insert(
            "MAJU_MANAGED_AGENT".to_string(),
    ...
    fn validate_rejects_reserved_key_case_insensitive() {
        // MAJU_PRIVATE_KEY in any casing must be blocked.
        let mut env = BTreeMap::new();
        env.insert("maju_private_key".to_string(), "secret".to_string());
        let def = HarnessDefinition {
    ...
    /// validation boundary — the comma-delimited `MAJU_ACP_AGENT_ARGS`
