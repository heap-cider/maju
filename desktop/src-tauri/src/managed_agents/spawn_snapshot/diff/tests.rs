...
fn base() -> SpawnConfigSnapshot {
    SpawnConfigSnapshot {
        ...
        env: BTreeMap::from([
            ...
            ("MAJU_LOG".to_string(), "info".to_string()),
        ]),
...
fn env_key_removal_is_removed_without_a_payload() {
    let mut after = base();
    after.env.remove("MAJU_LOG");
    assert_eq!(
        change_at(&diff(&base(), &after), "env.MAJU_LOG"),
...
fn allowlisted_env_key_shows_plain_value() {
    // MAJU_AGENT_THINKING_EFFORT is on the safe-to-reveal allowlist — the user
    // must be able to see actual enum values like "medium → high".
    let mut before = base();
    before
        .env
        .insert("MAJU_AGENT_THINKING_EFFORT".into(), "medium".into());
    let mut after = before.clone();
    after
        .env
        .insert("MAJU_AGENT_THINKING_EFFORT".into(), "high".into());
    assert_eq!(
        change_at(&diff(&before, &after), "env.MAJU_AGENT_THINKING_EFFORT"),
        &RestartChange::Value {
...
fn allowlisted_env_key_is_case_insensitive() {
    ...
    before
        .env
        .insert("maju_agent_provider".into(), "anthropic".into());
    let mut after = before.clone();
    after
        .env
        .insert("maju_agent_provider".into(), "openai".into());
    assert_eq!(
        change_at(&diff(&before, &after), "env.maju_agent_provider"),
        &RestartChange::Value {
