...
pub(crate) async fn flush_pending_events_at(
    ...
    for row in pending {
        ...
        // ±900s from server time (`crates/maju-relay/src/handlers/ingest.rs`
        ...
        } else if maju_core_pkg::kind::is_identity_archive_request_kind(current.kind) {
...
///
/// Uses the captured community owner, even if the active community changes.
