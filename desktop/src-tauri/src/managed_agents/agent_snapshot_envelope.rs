...
//!
//! A locked card carries the same `maju_agent_snapshot` tEXt chunk as a plain
...
//!
//! Wire contract (agreed with Wren, maju-agent-trading-cards thread):
...
use maju_core_pkg::engram::NIP44_PLAINTEXT_MAX;
...
/// `maju-agent-snapshot` so detection never guesses.
pub const LOCKED_FORMAT: &str = "maju-agent-snapshot-encrypted";
...
/// Typed outer envelope stored (base64 JSON) in the `maju_agent_snapshot`
...
///
/// - `maju-agent-snapshot` → full plain-manifest decode + validation.
/// - `maju-agent-snapshot-encrypted` → size caps, typed envelope parse,
...
mod tests {
    ...
    fn unknown_format_version_scheme_rejected() {
        ...
        let unknown = serde_json::json!({"format": "maju-agent-snapshot-v9", "version": 1});
