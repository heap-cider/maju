...
// `crates/maju-relay/src/handlers/ingest.rs`), and an over-ceiling event is
...
pub fn member_key_for(record: &AgentDefinition) -> String {
    ...
    hasher.update(b"maju:team-catalog:member-key:v1\0");
...
fn validate_member(member: &TeamCatalogMember) -> Result<(), String> {
    ...
    // (`MAJU_ACP_SYSTEM_PROMPT`), so invisible/bidi controls could make what
...
pub fn validate_team_catalog_content(content: &TeamCatalogContent) -> Result<(), String> {
    ...
    if let Some(instructions) = &content.instructions {
        ...
        // (`MAJU_ACP_TEAM_INSTRUCTIONS`), so they are executable-definition
