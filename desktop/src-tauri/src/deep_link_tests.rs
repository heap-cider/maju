...
fn demo_entity_transport_produces_the_frontend_golden_contract() {
    ...
    let scheme = "maju-demo-board-1234567812345678";
    for canonical in golden["links"].as_object().unwrap().values() {
        let canonical = canonical.as_str().unwrap();
        let transport = Url::parse(&canonical.replacen("maju:", &format!("{scheme}:"), 1)).unwrap();
        ...
        assert!(canonical_entity_deep_link(&transport, "maju").is_none());
        assert!(
            canonical_entity_deep_link(&transport, "maju-demo-other-8765432187654321").is_none()
        );
        assert!(canonical_entity_deep_link(&Url::parse(canonical).unwrap(), scheme).is_none());
        assert_eq!(
            canonical_entity_deep_link(&Url::parse(canonical).unwrap(), "maju").as_deref(),
            Some(canonical)
        );
    }
