...
pub async fn spawn_media_proxy(http_client: reqwest::Client, app_handle: tauri::AppHandle) -> u16 {
    ...
    eprintln!("maju-desktop: media proxy listening on 127.0.0.1:{port}");
    port
}
...
/// This handler routes `maju-media://localhost/{path}` through reqwest, which
/// runs in the Tauri process and goes through the VPN.
pub async fn handle_maju_media(
    app: &tauri::AppHandle,
