...
pub fn run() {
    ...
    app.run(move |app_handle, event| match event {
        ...
        RunEvent::WindowEvent {
            ...
            // Keep the webview alive so Maju can be reopened from its tray menu.
            api.prevent_close();
            if let Some(window) = app_handle.get_webview_window("main") {
                if let Err(error) = window.hide() {
                    eprintln!("maju-desktop: failed to hide main window: {error}");
                }
            }
        }
        RunEvent::WindowEvent {
            ...
            if is_active_huddle_window {
                if let Err(error) = app_handle.emit("huddle-companion-returned", ()) {
                    eprintln!("maju-desktop: failed to restore huddle drawer: {error}");
                }
            }
        }
