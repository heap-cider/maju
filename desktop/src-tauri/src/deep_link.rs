...
impl PendingNavigationDeepLinks {
    fn lock(&self) -> std::sync::MutexGuard<'_, VecDeque<PendingNavigationDeepLink>> {
        self.0.lock().unwrap_or_else(|poisoned| {
            eprintln!("maju-desktop: recovering poisoned pending navigation deep-link queue");
            poisoned.into_inner()
        })
    }
...
pub(crate) fn install_deep_link_handlers(app: &mut tauri::App) {
    ...
    match app.deep_link().get_current() {
        ...
        Ok(None) => {}
        Err(error) => eprintln!("maju-desktop: failed to read launch deep link: {error}"),
    }
}
...
/// Hosts of the `maju://` git-entity links built by
/// `desktop/src/shared/lib/entityLink.ts` and `crates/maju-cli/src/links.rs`.
...
/// Validate a `maju://repo|project|pr|issue?…` link and return it verbatim
...
///
/// Workspace tabs addressable by `maju://repo|project` links — mirrors
...
/// - `maju://repo|project|pr|issue?…` — emits `deep-link-entity` to the frontend
    ...
    match url.host_str() {
        ...
        Some("channel") => {
            let Some(payload) = parse_channel_deep_link(&url) else {
                eprintln!("maju-desktop: channel deep link missing/invalid channel: {url_str}");
                return;
            };
            activate_main_window(app);
        ...
        Some("repo" | "project" | "pr" | "issue") => {
            // `maju://repo|project?owner=<pubkey>&d=<dtag>` and
            // `maju://pr|issue?id=<eventId>&owner=<pubkey>&d=<dtag>` — the
            ...
            if parse_entity_deep_link(&url).is_none() {
                eprintln!("maju-desktop: malformed entity deep link: {url_str}");
                return;
            }
            activate_main_window(app);
