...
async fn reconcile_maju_mesh_join(app: &AppHandle) -> Result<(), String> {
    ...
    let targets =
        crate::commands::mesh_llm::resolve_maju_mesh_join_targets_at(&state, &relay_url).await?;
...
enum RosterReconcileAction {
    ...
    Keep,
    /// Restart Maju so MeshLLM is rebuilt with a freshly resolved roster.
    ///
    /// MeshLLM's native listeners are process-owned in practice: stopping and
    /// starting the embedded runtime in one process can terminate Maju or race
...
async fn reconcile_roster(
    ...
    drop(guard);
    eprintln!(
        "maju-mesh: membership roster changed; restarting Maju to rebuild MeshLLM with the fresh community allowlist"
    );
    app.request_restart();
    Ok(())
}
