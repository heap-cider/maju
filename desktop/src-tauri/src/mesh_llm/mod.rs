...
pub use coordinator::{start_coordinator, MeshCoordinator, KIND_MAJU_MESH_MEMBER_STATUS};
...
const MESH_STATUS_KIND: u64 = KIND_MAJU_MESH_MEMBER_STATUS as u64;
const MESH_API_PORT_ENV: &str = "MAJU_MESH_API_PORT";
const MESH_CONSOLE_PORT_ENV: &str = "MAJU_MESH_CONSOLE_PORT";
...
const MESH_IROH_RELAYS_ENV: &str = "MAJU_MESH_IROH_RELAYS";
...
/// Maju defines client readiness by the OpenAI ingress agents consume on
...
/// because the optional management API is delayed; Maju's ingress watchdog is
...
pub struct MeshServeTarget {
    ...
    /// Maju member that signed the discovery note containing this target.
    /// Populated after signature/membership validation; never trusted from the
    /// note payload itself.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub reporter_pubkey: Option<String>,
    /// Per-runtime MeshLLM owner identity verified by the signed Maju status.
    /// Distinguishes two devices logged into the same Maju member account.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct StartMeshNodeRequest {
    ...
    /// Stable, relay-scoped mesh name injected by the Maju backend. It is not
...
enum DesktopMeshHandle {
    ...
    /// available. Keep that wait off the agent-save path while Maju supervises
    ...
    pub async fn start(mut request: StartMeshNodeRequest) -> anyhow::Result<Self> {
        ...
        let handle = match request.mode {
            MeshNodeMode::Serve => {
                ...
                // the Maju relay is the source of the roster; possession of a
            ...
            Ok(Ok(ready)) => {
                for token in queued_join_tokens {
                    if let Err(error) = ready.join_token(token).await {
                        eprintln!(
                            "maju-mesh: failed to apply a dial request queued during startup: {error:#}"
                        );
                    }
                }
    ...
    pub async fn stop(self) -> anyhow::Result<()> {
        match self.handle.into_inner() {
            ...
            DesktopMeshHandle::Starting { task, .. } => {
                ...
                // Abort only the Maju-side waiter; normal callers never replace
...
pub fn models_from_status_payload(payload: Option<&serde_json::Value>) -> Vec<MeshModelOption> {
    ...
    if let Some(payload) = payload {
        ...
        // `runtime.models`. Maju-authored status reports use `models`. Do not
