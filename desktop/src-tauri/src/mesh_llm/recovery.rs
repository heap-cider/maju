...
pub(crate) const MESH_REARM_ERROR_SENTINEL: &str = "[maju-mesh-rearm] ";
...
pub(crate) async fn recover_stale_mesh_runtime(
    ...
    let (candidate_id, startup_in_progress, candidate_mode) =
        match state.mesh_llm_runtime.lock().await.as_ref() {
            ...
            None => {
                ...
                // A cancelled SDK startup can outlive its Maju-side task briefly
    ...
    if startup_in_progress && consecutive < DEAD_PROBE_EVICT_THRESHOLD {
        eprintln!(
            "maju-mesh: ingress is not live on the first startup probe; allowing the supervised SDK task one watchdog interval"
        );
        return MeshRuntimeRecovery::Debouncing;
    }
    if !should_evict_after_probe(urgency, probe, consecutive) {
        eprintln!(
            "maju-mesh: ingress probe {probe:?} ({consecutive}/{DEAD_PROBE_EVICT_THRESHOLD}); debouncing"
        );
        return MeshRuntimeRecovery::Debouncing;
    }
    ...
    match tokio::time::timeout(STALE_STOP_TIMEOUT, stale.stop()).await {
        Ok(Ok(())) => {}
        Ok(Err(error)) => eprintln!("maju-mesh: stale runtime stop failed: {error:#}"),
        Err(_) => eprintln!(
            "maju-mesh: stale runtime stop exceeded {}s; waiting for port release",
    ...
    if wait_for_ingress_release().await {
        ...
        eprintln!(
            "maju-mesh: old runtime still owns the local ingress; deferring replacement to avoid a port-conflict loop"
        );
    ...
    match recovery {
        ...
        MeshRuntimeRecovery::RestartRequired => {
            if runtime_mode == Some(crate::mesh_llm::MeshNodeMode::Serve) {
                eprintln!(
                    "maju-mesh: serving ingress failed; restarting Maju to restore Share Compute without changing roles"
                );
                app.request_restart();
                return Ok(());
            }
            ...
            eprintln!(
                "maju-mesh: supervised client startup lost its ingress before the SDK exposed a shutdown handle; restarting Maju"
            );
            app.request_restart();
            return Ok(());
        }
        ...
        .await
        {
            Ok(()) => {
                if let Err(error) = clear_mesh_last_error_if_set(app, &record.pubkey) {
                    eprintln!("maju-mesh: failed to clear recovery error: {error}");
                }
            }
            Err(error) => {
                let message = format!(
                    "{MESH_REARM_ERROR_SENTINEL}Maju shared compute offline — failed to re-arm local ingress for this agent: {error}"
                );
                if let Err(persist_error) = persist_mesh_last_error(app, &record.pubkey, &message) {
                    eprintln!("maju-mesh: failed to persist recovery error: {persist_error}");
                }
