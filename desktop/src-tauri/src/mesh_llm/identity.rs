...
//! Maju/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in Maju or standalone.
...
impl OwnerIdentity {
    /// Sign a Maju-to-MeshLLM ownership binding. The member's Nostr signature
