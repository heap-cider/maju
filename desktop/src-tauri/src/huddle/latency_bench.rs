...
//!
//! Run:
//!   MAJU_BENCH_WAV=<48k f32 mono wav> cargo test --release -p maju-desktop \
...
#[test]
#[ignore = "ad-hoc latency baseline; needs models in ~/.maju/models and an audio output device"]
fn baseline_stt_fake_llm_tts_first_audio() {
    let home = dirs::home_dir().expect("home");
    let stt_dir = home.join(".maju/models/parakeet-tdt-ctc-110m-en");
    let tts_dir = home.join(".maju/models/pocket-tts");
    assert!(
    ...
    let wav_path = std::env::var("MAJU_BENCH_WAV").expect("set MAJU_BENCH_WAV");
    ...
    let llm_delay_ms: u64 = std::env::var("MAJU_BENCH_LLM_MS")
        .ok()
    ...
    let turns: usize = std::env::var("MAJU_BENCH_TURNS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(6);
