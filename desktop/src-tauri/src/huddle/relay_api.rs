...
fn build_audio_auth_event(
    ...
    if let Some(auth_tag_json) = auth_tag_json {
        ...
        maju_sdk_pkg::nip_oa::verify_auth_tag(auth_tag_json, &compat_pubkey)
            .map_err(|e| format!("agent auth tag verification failed: {e}"))?;
        let compat_tag = maju_sdk_pkg::nip_oa::parse_auth_tag(auth_tag_json)
...
pub(crate) async fn connect_audio_relay(
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            eprintln!("maju-desktop: audio relay pipeline exited: {e}");
        }
...
fn queue_tts_broadcast_packet(
    ...
    for chunk in samples_48k.chunks(960) {
        if queue.len() >= TTS_BROADCAST_MAX_FRAMES {
            eprintln!("maju-desktop: tts broadcast status=dropped reason=queue_duration_limit");
            break;
        }
...
pub(crate) async fn connect_tts_audio_publisher(
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            eprintln!("maju-desktop: tts broadcast status=disconnected error={error}");
        }
        ...
        loop {
            ...
            for chunk in samples.chunks(FRAME_SAMPLES) {
                ...
                let n = match encode_result {
                    Ok(n) => n,
                    Err(e) => {
                        eprintln!("maju-desktop: opus encode error: {e}");
                        continue;
                    }
                };
                if n > 0 {
...
pub(crate) async fn fetch_channel_members_with_roles(
    ...
    let events = query_relay(state, std::slice::from_ref(&filter))
        .await
        .map_err(|e| {
            eprintln!("maju-desktop: fetch channel members failed: {e}");
            e
        })?;
