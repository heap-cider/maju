...
//!
//! `MAJU_TTS_STREAMING=1` streams PCM deltas out of Pocket as they are
//! generated instead of waiting for the full first-chunk synthesis.
//! `MAJU_TTS_EMIT_FRAMES` tunes the delta size in Flow LM frames (80 ms of
...
/// when `MAJU_TTS_STREAMING=1`, `None` for the production batch path.
pub(super) fn streaming_emit_frames() -> Option<usize> {
    std::env::var("MAJU_TTS_STREAMING")
        .is_ok_and(|v| v == "1")
        .then(|| {
            std::env::var("MAJU_TTS_EMIT_FRAMES")
                .ok()
...
pub(super) fn synthesize_streaming(
    ...
    match stream_result {
        ...
        Ok(false) => {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=cancelled reason=stream_callback route_id={route_id}"
            );
            *first_append = true;
            Some("cancelled")
        }
        Err(_) => {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=failed reason=inference route_id={route_id}"
            );
            Some("failed")
        }
    }
