...
Only messages sent with `maju messages send` to this huddle channel are spoken aloud, in the order sent; everything else you produce is silent.
...
mod tests {
    ...
    fn voice_mode_guidelines_pin_spoken_reply_as_first_tool_call() {
        ...
        assert!(guidelines.contains("Only messages sent with `maju messages send`"));
