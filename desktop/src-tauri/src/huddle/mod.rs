    ...
    if needs_restart {
        ...
        if let Some(eph_id) = eph_id {
            ...
            if let Err(e) = maybe_start_stt_pipeline(&state, &eph_id).await {
                eprintln!("maju-desktop: STT pipeline restart on mode switch failed: {e}");
            }
        }
    }
    ...
    let result: Result<(Vec<String>, String), String> = async {
            ...
            events::build_huddle_guidelines(&ephemeral_channel_id, &guidelines)
        {
            if let Err(e) = submit_event(guidelines_builder, &state).await {
                eprintln!("maju-desktop: huddle guidelines (kind:48106) failed: {e}");
            }
        }
        ...
        for pubkey in &member_pubkeys {
            ...
            match submit_event(add_builder, &state).await {
                ...
                Err(e) => {
                    eprintln!("maju-desktop: huddle add_member failed for {pubkey}: {e}");
            ...
            if channel_was_created {
                if let Ok(archive_builder) = events::build_archive(ephemeral_uuid) {
                    if let Err(ae) = submit_event(archive_builder, &state).await {
                        eprintln!(
                            "maju-desktop: rollback archive of {ephemeral_channel_id} failed: {ae}"
                        );
                    }
                }
            }
    ...
    if !parent_channel_id.is_empty() && !ephemeral_channel_id.is_empty() {
            ...
            events::build_huddle_ended(parent_channel_id, ephemeral_channel_id)
        {
            if let Err(e) = submit_event(ended_builder, state).await {
                eprintln!("maju-desktop: huddle_ended event failed: {e}");
            }
        }
    }
        ...
        if let Ok(uuid) = parse_channel_uuid(ephemeral_channel_id) {
            if let Ok(archive_builder) = events::build_archive(uuid) {
                if let Err(e) = submit_event(archive_builder, state).await {
                    eprintln!("maju-desktop: archive ephemeral channel failed: {e}");
                }
            }
        }
    }
...
async fn remove_huddle_agents(ephemeral_channel_id: &str, state: &AppState) {
    ...
    let agent_pubkeys = match fetch_channel_members(ephemeral_channel_id, Some("bot"), state).await
    {
        Ok(pubkeys) => pubkeys,
        Err(e) => {
            eprintln!("maju-desktop: fetch huddle agents for cleanup failed: {e}");
            return;
        }
    };
    ...
    for pubkey in agent_pubkeys {
        ...
        if let Err(e) = submit_event(remove_builder, state).await {
            eprintln!("maju-desktop: remove huddle agent {pubkey} failed: {e}");
        }
    }
}
...
pub async fn leave_huddle(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    if !parent_channel_id.is_empty() && !ephemeral_channel_id.is_empty() {
        ...
        if humans_remaining <= 1 {
            ...
            eprintln!("maju-desktop: last human left huddle — auto-ending");
            emit_end_and_archive(&parent_channel_id, &ephemeral_channel_id, &state).await;
        } else {
            ...
            if let Ok(eph_uuid) = parse_channel_uuid(&ephemeral_channel_id) {
                if let Ok(leave_builder) = events::build_leave(eph_uuid) {
                    if let Err(e) = submit_event(leave_builder, &state).await {
                        eprintln!("maju-desktop: huddle leave ephemeral channel failed: {e}");
                    }
                }
            }
        }
...
pub async fn speak_agent_message(
    ...
    eprintln!("maju-desktop: tts stage=invoke status=started route_id={route_id}");
    ...
    if !state.huddle()?.tts_enabled {
        eprintln!(
            "maju-desktop: tts stage=invoke status=no_op reason=disabled route_id={route_id}"
        );
        return Ok(());
    }
    ...
    let Some(voice_reference) =
        ...
        eprintln!(
            "maju-desktop: tts stage=invoke status=no_op reason=agent_disabled route_id={route_id}"
        );
        return Ok(());
    };
    ...
    let needs_pipeline = {
        ...
        match classify_agent_tts_runtime(hs.tts_enabled, &hs.phase, hs.tts_pipeline.is_some()) {
            ...
            AgentTtsRuntimeGate::Inactive => {
                eprintln!(
                    "maju-desktop: tts stage=invoke status=failed reason=inactive_huddle route_id={route_id}"
                );
                return Err(
    ...
    if needs_pipeline {
        maybe_start_tts_pipeline(&state).await.inspect_err(|_| {
            eprintln!(
                "maju-desktop: tts stage=invoke status=failed reason=startup_failed route_id={route_id}"
            );
        })?;
        await_inflight_tts_start(&state).await.inspect_err(|_| {
            eprintln!(
                "maju-desktop: tts stage=invoke status=failed reason=startup_timeout route_id={route_id}"
            );
        })?;
    }
    ...
    let pipeline = {
        ...
        if !agent_is_present {
            eprintln!(
                "maju-desktop: tts stage=queue status=dropped reason=speaker_removed route_id={route_id}"
            );
            return Ok(());
        }
    ...
    let Some(pipeline) = pipeline else {
        eprintln!(
            "maju-desktop: tts stage=invoke status=failed reason=unavailable route_id={route_id}"
        );
    ...
    match agent_tts_publisher::ensure(&app, &state, &pipeline, &speaker_pubkey).await {
        Ok(true) => eprintln!(
            "maju-desktop: tts broadcast status=ready route_id={route_id}"
        ),
        Ok(false) => eprintln!(
            "maju-desktop: tts broadcast status=unavailable reason=agent_identity_not_local route_id={route_id}"
        ),
        Err(error) => eprintln!(
            "maju-desktop: tts broadcast status=unavailable reason=publisher_setup_failed route_id={route_id} error={error}"
        ),
    }
    ...
    .await
    .inspect(|_| eprintln!("maju-desktop: tts stage=queue status=accepted route_id={route_id}"))
    .inspect_err(|_| {
        eprintln!("maju-desktop: tts stage=queue status=failed reason=closed route_id={route_id}")
    })
}
