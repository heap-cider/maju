...
fn tts_worker(
    ...
    let append_audio = |prepared: PreparedModelAudio,
        ...
        if !speaker_is_current {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=cancelled reason=speaker_removed route_id={route_id}"
            );
            return false;
        }
    ...
    loop {
        ...
        if !queued_speaker_is_current(&speaker_generations, &queued_text) {
            eprintln!(
                "maju-desktop: tts stage=queue status=dropped reason=speaker_removed route_id={}",
