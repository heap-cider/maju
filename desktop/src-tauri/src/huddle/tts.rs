...
impl TtsPipeline {
    ...
    pub fn speak(&self, text: String) -> Result<(), String> {
        self.text_tx
            ...
            .map_err(|e| {
                eprintln!("maju-desktop: TTS queue saturated, dropping message: {e}");
    ...
    pub fn select_voice(&self, voice: &str) -> Option<tokio::sync::oneshot::Receiver<()>> {
        ...
        if acknowledged.is_some() {
            eprintln!("maju-desktop: tts stage=cancellation reason=voice_switch route_id=0");
        }
        acknowledged
    }
    ...
    pub fn shutdown(&self) {
        eprintln!("maju-desktop: tts stage=cancellation reason=shutdown route_id=0");
...
fn tts_worker(
    ...
    let engine = match load_text_to_speech(&model_dir_str) {
        ...
        Err(e) => {
            ...
            eprintln!("maju-desktop: tts stage=startup status=failed reason=engine_load");
    ...
    let mut style = match load_voice_style(&fallback_path) {
        ...
        Err(e) => {
            ...
            eprintln!("maju-desktop: tts stage=startup status=failed reason=fallback_voice_style");
            let _ = startup_tx.send(Err(error));
            return;
        }
    };
    ...
    // and discard the output so the first real utterance runs at warm-session speed.
    {
        match engine.synth_chunk("warmup", "en", &style, SYNTH_STEPS) {
            Ok(_) => eprintln!("maju-desktop: tts stage=warmup status=ready"),
            Err(_) => eprintln!(
                "maju-desktop: tts stage=warmup status=failed reason=inference first_utterance_may_be_slow=true"
            ),
        }
    }
    ...
    let sink_handle = match super::audio_output::open_output_sink_by_name(output_device.as_deref())
    {
        ...
        Err(e) => {
            ...
            eprintln!("maju-desktop: tts stage=startup status=failed reason=output_open");
            let _ = startup_tx.send(Err(error));
            return;
        }
    };
    ...
    // the first TTS message to be truncated after a few words.
    {
        ...
        while !player.empty() {
            if std::time::Instant::now() >= deadline {
                eprintln!("maju-desktop: tts stage=startup status=failed reason=output_prime");
                let _ = startup_tx.send(Err(
    ...
    eprintln!("maju-desktop: tts stage=startup status=ready");
    ...
    if let Err(ref e) = monitor {
        ...
        eprintln!("maju-desktop: TTS barge-in monitor failed to spawn: {e}");
    }
    ...
    let append_audio = |prepared: PreparedModelAudio, route_id: u64| {
            ...
            || shutdown.load(Ordering::Acquire)
        {
            ...
            eprintln!(
                "maju-desktop: tts stage=synthesis status=cancelled reason={reason} route_id={route_id}"
            );
            return false;
        }
        player.append(SamplesBuffer::new(channels, rate, prepared.buffer));
        eprintln!(
            "maju-desktop: tts stage=player status=append_accepted route_id={route_id} chunk_index={} sample_count={}",
    ...
    loop {
            ...
            None => match text_rx.recv_timeout(RECV_TIMEOUT) {
                ...
                Err(mpsc::RecvTimeoutError::Timeout) => {
                    ...
                    if player.empty() && !first_append {
                        tts_active.store(false, Ordering::Release);
                        eprintln!(
                            "maju-desktop: tts stage=player status=drained route_id={last_route_id}"
                        );
                        first_append = true;
                    }
                    continue;
        ...
        if queued_text.generation < voice_generation.load(Ordering::Acquire) {
            eprintln!(
                "maju-desktop: tts stage=queue status=dropped reason=voice_switch route_id={}",
        ...
        eprintln!("maju-desktop: tts stage=synthesis status=started route_id={route_id}");
        ...
        if !reconcile_selected_voice(&model_dir, &selected_voice, &mut voice_name, &mut style) {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=failed reason=voice_unavailable route_id={route_id}"
            );
            continue;
        }
        ...
        if player.empty() && !first_append {
            tts_active.store(false, Ordering::Release);
            eprintln!("maju-desktop: tts stage=player status=drained route_id={last_route_id}");
            first_append = true;
        }
        ...
        if text.is_empty() {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=empty reason=preprocess route_id={route_id}"
            );
            continue;
        }
        ...
        if chunks.is_empty() {
            eprintln!(
                "maju-desktop: tts stage=synthesis status=empty reason=no_chunks route_id={route_id}"
            );
            continue;
        }
        ...
        'playback_chunks: for chunk in &chunks {
            ...
            let model_chunks = match engine.split_text_into_chunks(text) {
                ...
                Err(_) => {
                    eprintln!(
                        "maju-desktop: tts stage=synthesis status=failed reason=chunking route_id={route_id}"
                    );
            ...
            for model_chunk in &model_chunks {
                    ...
                    || shutdown.load(Ordering::Acquire)
                {
                    ...
                    // never reaches the player, while keeping maju-voice's
                ...
                match synthesis {
                    ...
                    Ok(_) => {
                        eprintln!(
                            "maju-desktop: tts stage=synthesis status=empty route_id={route_id} chunk_index={chunk_index}"
                        );
                    }
                    Err(_) => {
                        eprintln!(
                            "maju-desktop: tts stage=synthesis status=failed reason=inference route_id={route_id} chunk_index={chunk_index}"
                        );
                        synthesis_outcome = "failed";
                        break;
                    }
        ...
        if synthesis_outcome == "completed" && appended_audio {
            eprintln!("maju-desktop: tts stage=synthesis status=completed route_id={route_id}");
        }
...
fn handle_cancel_or_shutdown(
    ...
    if shutdown.load(Ordering::Acquire) {
        eprintln!(
            "maju-desktop: tts stage=cancellation reason=shutdown route_id={}",
    ...
    if cancel.load(Ordering::Acquire) || voice_cancel.load(Ordering::Acquire) {
        ...
        eprintln!(
            "maju-desktop: tts stage=cancellation reason={} route_id={}",
