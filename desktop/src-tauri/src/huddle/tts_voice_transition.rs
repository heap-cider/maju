...
pub(super) fn reconcile_selected_voice(
    ...
    match load_voice_style(&requested_path) {
        ...
        Err(_) => {
            eprintln!("maju-desktop: tts stage=voice_switch status=fallback reason=voice_style");
            ...
            match load_voice_style(&fallback_path) {
                ...
                Err(_) => {
                    eprintln!(
                        "maju-desktop: tts stage=voice_switch status=failed reason=fallback_voice_style"
                    );
                    false
                }
            }
...
pub(super) fn reconcile_queued_voice(
    ...
    match load_voice_style(&voice_path(model_dir, requested_voice)) {
        ...
        Err(_) => {
            eprintln!(
                "maju-desktop: tts stage=agent_voice_switch status=fallback reason=voice_style"
            );
...
fn log_cancelled_route(route_id: u64, reason: &str) {
    eprintln!("maju-desktop: tts stage=queue status=dropped reason={reason} route_id={route_id}");
}
