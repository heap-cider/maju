...
//!
//! Built on `maju-ws-client`, which owns the wire format and the NIP-42
...
//! into it, because `maju-cli` and `maju-test-client` consume that crate and do
...
use maju_ws_client_pkg::{NostrWsConnection, RelayMessage};
...
impl RelaySession {
    ...
    pub(crate) async fn set_subscriptions(&self, subscriptions: Vec<Subscription>) {
        ...
        for id in violations {
            eprintln!(
                "maju-desktop: native_relay_client: subscription {id} changed filter under a \
...
async fn run_session(
    ...
    loop {
        ...
        match NostrWsConnection::connect_authenticated(&relay_url, &keys, auth_tag.as_ref()).await {
            ...
            Err(error) => {
                eprintln!("maju-desktop: native_relay_client: connect failed: {error}");
            }
        }
            ...
            message = conn.next_event(READ_TIMEOUT) => {
                match message {
                    ...
                    Ok(RelayMessage::Closed { subscription_id, message }) => {
                        ...
                        eprintln!(
                            "maju-desktop: native_relay_client: relay closed {subscription_id}: {message}"
                        );
                    }
                    ...
                    Err(error) => {
                        if !is_read_timeout(&error) {
                            eprintln!("maju-desktop: native_relay_client: read failed: {error}");
                            return;
                        }
                    }
                }
...
fn is_read_timeout(error: &maju_ws_client_pkg::WsClientError) -> bool {
    matches!(error, maju_ws_client_pkg::WsClientError::Timeout)
}
...
mod relay_backed_tests {
    ...
    /// and I cannot.
    ///
    /// `#[ignore]`d because it needs a relay on `MAJU_TEST_RELAY_URL`. Run:
    ///
    /// ```text
    /// ./scripts/start-isolated-test-relay.sh          # ws://localhost:3030
    /// MAJU_TEST_RELAY_URL=ws://localhost:3030 \
    ///   cargo test -p maju-desktop -- --ignored archive_sync_session
    /// ```
    #[tokio::test]
    #[ignore = "requires a local relay (set MAJU_TEST_RELAY_URL)"]
    async fn archive_sync_session_receives_live_events_from_a_real_relay() {
        let Ok(relay_url) = std::env::var("MAJU_TEST_RELAY_URL") else {
            panic!("set MAJU_TEST_RELAY_URL to a running relay");
        };
