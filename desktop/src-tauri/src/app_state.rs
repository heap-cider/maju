...
fn recover_from_keyring(
    ...
    eprintln!(
        "maju-desktop: corrupt nsec in keyring ({error}), looking for a recovery path before clearing"
    );
    ...
    if migration_marker_path(data_dir).exists() {
        let ephemeral = Keys::generate();
        eprintln!(
            "maju-desktop: identity lost — keyring value failed to parse and no valid identity.key \
