...
/// (e.g. `xyz.block.maju.app` or `xyz.block.maju.app.dev`).
...
pub(crate) struct ResetContext<'a> {
    ...
    /// Nest dir (`~/.maju` or `~/.maju-dev`) scoped to this build's variant,
...
pub(crate) fn run_boot_reset(app_data_dir: &Path) -> ResetOutcome {
    ...
    let demo_config_dir = match crate::build_identity::demo_config_home() {
        Ok(dir) => dir,
        Err(error) => {
            eprintln!("maju-desktop reset: {error}");
            return ResetOutcome {
                completed: false,
                failed: true,
            };
...
pub(crate) fn run_boot_reset_with_keychain(ctx: ResetContext<'_>) -> ResetOutcome {
    ...
    if ctx.is_demo && ctx.demo_config_dir.is_none() {
        eprintln!("maju-desktop reset: cannot resolve demo credential directory");
        return ResetOutcome {
            completed: false,
            failed: true,
        };
    ...
    let demo_config_removed =
        ctx.demo_config_dir
            ...
            .is_none_or(|path| match std::fs::remove_dir_all(path) {
                ...
                Err(error) => {
                    eprintln!(
                        "maju-desktop reset: remove demo config {}: {error}",
                        path.display()
                    );
                    false
                }
    ...
    if let Some(ref home) = ctx.home_dir {
        if !ctx.is_demo {
            ...
            let _ = std::fs::remove_dir_all(home.join(".config").join("maju-agent"));
        }
    ...
    if let Err(e) = ctx.keychain.delete_all_with_legacy() {
        eprintln!("maju-desktop reset: keychain delete: {e}");
    ...
    {
        eprintln!(
            "maju-desktop reset: verification failed (keychain_wiped={keychain_ok}, \
    ...
    if let Err(e) = delete_sentinel(app_data_dir) {
        eprintln!("maju-desktop reset: delete sentinel: {e}");
...
mod tests {
    ...
    fn test_dev_build_wipes_dev_nest_not_prod() {
        ...
        // Create both nests.
        let dev_nest = tmp.path().join(".maju-dev");
        let prod_nest = tmp.path().join(".maju");
    ...
    fn test_completed_dev_reset_suppresses_dev_repos_dir_migration() {
        ...
        // Seed prod ~/.maju/.repos-dir so the migration has something to copy.
        let home = tmp.path().join("home");
        let prod_nest = home.join(".maju");
        std::fs::create_dir_all(&prod_nest).unwrap();
        ...
        let dev_nest_2 = tmp.path().join(".maju-dev-control");
        ...
        let dev_nest_3 = tmp.path().join(".maju-dev-prod");
    ...
    fn test_crash_retry_cleans_prior_deterministic_trash() {
        ...
        let app_data = app_support.join("xyz.block.maju.app");
        std::fs::create_dir_all(&app_data).unwrap();
        write_sentinel(&app_data).unwrap();
        ...
        let trash_app_dir = app_support.join("xyz.block.maju.app.reset-trash");
    ...
    fn test_keychain_fail_restores_all_then_retry_cleans() {
        ...
        let trash_app = app_support.join("xyz.block.maju.app.reset-trash");
