...
type E2eConfig = {
  ...
  mock?: {
     ...
     *  Releasable early via `__MAJU_E2E_RELEASE_HUDDLE_AGENT_SYNCS__()`. */
     ...
     *  Releasable early via `__MAJU_E2E_RELEASE_MANAGED_AGENT_STARTS__()`. */
     ...
     *  `__MAJU_E2E_RELEASE_THREAD_REPLIES__()` is called. Unlike
    ...
    // Pending community deep links (maju://join / maju://connect / maju://add-community) seeded into
 ...
 * every member's own set (d=`maju:custom-emoji`). We serve TWO member-authored
 * sets from distinct pubkeys so the e2e exercises the union/collapse path, not
 * a single relay-owned set. `:maju:` is the stable shortcode exercised by
  ...
  return [
    createMockEvent(
      ...
      [
        ...
        ["emoji", "maju", "https://example.com/e2e/maju.png"],
    ...
    createMockEvent(
      ...
      [
        ...
        // member B claims :maju: with a DIFFERENT url — unionCustomEmoji must
        // collapse it to one deterministic winner, never expose two URLs.
        ["emoji", "maju", "https://example.com/e2e/maju-b.png"],
...
async function writeClipboardFlavors({
  ...
  window.__MAJU_E2E_LAST_CLIPBOARD__ = { html: html ?? null, text };
  if (
    html &&
...
declare global {
  interface Window {
    __MAJU_E2E__?: E2eConfig;
    /** Last payload written through the native clipboard command. */
    __MAJU_E2E_LAST_CLIPBOARD__?: { html: string | null; text: string };
    __MAJU_E2E_COMMANDS__?: string[];
    __MAJU_E2E_COMMAND_PAYLOADS__?: Array<{
      command: string;
      payload: unknown;
    }>;
    ...
    __MAJU_E2E_RELEASE_MEDIA_PROXY__?: () => number;
    /** Release mock send events that were stored but withheld from live subscribers. */
    __MAJU_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__?: () => number;
    __MAJU_E2E_EMIT_MEDIA_UPLOAD_PHASE__?: (input: {
      id: string;
      phase: string;
    }) => Promise<void>;
    __MAJU_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__?: (input: {
      id: string;
      sent: number;
      total: number;
    }) => Promise<void>;
    __MAJU_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__?: (payload: {
      pubkey: string | null;
      level: number;
    }) => Promise<void>;
    __MAJU_E2E_WEBVIEW_ZOOM__?: number;
    ...
    __MAJU_E2E_HAS_MOCK_GLOBAL_KIND_SUBSCRIPTION__?: (kind: number) => boolean;
    __MAJU_E2E_SET_MOCK_USER_STATUS__?: (input: {
      text: string;
      emoji?: string;
      expiresAt?: number;
      createdAt?: number;
    ...
    __MAJU_E2E_EMIT_MOCK_PRESENCE__?: (input: {
      pubkey: string;
      status: PresenceStatus;
    }) => void;
    ...
    __MAJU_E2E_PREPEND_MOCK_HISTORY__?: (input: {
      channelName: string;
      count: number;
      startIndex?: number;
      lineCount?: number;
    ...
    __MAJU_E2E_EMIT_MOCK_TYPING__?: (input: {
      channelName: string;
      createdAt?: number;
      pubkey?: string;
      threadHeadId?: string;
    ...
    __MAJU_E2E_EMIT_TAURI_EVENT__?: (
      event: string,
      payload: unknown,
    ) => Promise<void>;
    __MAJU_E2E_SET_MOCK_HUDDLE_SNAPSHOT__?: (input: {
      members: MockHuddleMemberSeed[];
      transcriptionEnabled: boolean;
    }) => Promise<void>;
    /**
     ...
     */
    __MAJU_E2E_REPLACE_MOCK_TEAM_CATALOG_HEAD__?: (event: RelayEvent) => void;
    __MAJU_E2E_PUSH_MOCK_FEED_ITEM__?: (item: RawFeedItem) => RawFeedItem;
    /** Replace an existing feed item by id (or push if not found) and fire the updated event. */
    __MAJU_E2E_REPLACE_MOCK_FEED_ITEM__?: (
      oldId: string,
      item: RawFeedItem,
    ) => RawFeedItem;
     ...
     */
    __MAJU_E2E_EXTRA_PROJECT_EVENTS__?: Array<{
      id: string;
      kind: number;
      pubkey: string;
      created_at: number;
    ...
    __MAJU_E2E_PROJECT_MERGE_ERROR__?: {
      code: string;
      message: string;
      recovery: {
    ...
    __MAJU_E2E_PROJECT_OWNER_OVERRIDE__?: string;
    /** Project history kinds rejected with CLOSED for aggregate-query tests. */
    __MAJU_E2E_REJECT_PROJECT_QUERY_KINDS__?: number[];
    /** Captured aggregate project-history filters for request-count assertions. */
    __MAJU_E2E_PROJECT_QUERY_FILTERS__?: MockFilter[];
    /** Optional local repository snapshot returned for project branch tests. */
    __MAJU_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__?: unknown;
    /** Optional bounded file contents returned by repository content reads. */
    __MAJU_E2E_PROJECT_REPO_FILE_CONTENTS__?: Record<string, string | null>;
    __MAJU_E2E_PROJECT_REPO_SYNC_STATUS__?: {
    ...
    __MAJU_E2E_SET_RELAY_CONNECTION_STATE__?: (state: ConnectionState) => void;
    __MAJU_E2E_GET_RELAY_CONNECTION_STATE__?: () => ConnectionState;
    /** Queue deterministic mock AUTH outcomes, consumed in order. */
    __MAJU_E2E_QUEUE_AUTH_RESPONSES__?: (
      responses: Array<{ success: boolean; message: string }>,
    ) => void;
    /** Inject CLOSED into every active mock live subscription. */
    __MAJU_E2E_CLOSE_LIVE_SUBSCRIPTIONS__?: (reason: string) => number;
    /** Queue CLOSED responses for channel history REQs. */
    __MAJU_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__?: (reasons: string[]) => void;
    ...
    __MAJU_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__?: (
      unavailable: boolean,
    ) => void;
    __MAJU_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => number[];
    __MAJU_E2E_ACTIVATE_RELAY_RATE_LIMIT__?: (seconds: number) => void;
    __MAJU_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => void;
    __MAJU_E2E_SET_MESH__?: (mesh: {
      admitted?: boolean;
    ...
    __MAJU_E2E_RUN_MODEL_SWITCH__?: (input: {
      agentPubkey: string;
      channelIds: string[];
      modelId: string;
      requestId: string;
    ...
    __MAJU_E2E_MOUNT_AGENT_SESSION_PANEL__?: (input: {
      agentPubkey: string;
      channelId: string;
      canInterruptTurn?: boolean;
    }) => Promise<void>;
    /** Resolve decoded test-only observer controls back through the mock relay. */
    __MAJU_E2E_OBSERVER_CONTROLS__?: Array<{
      agentPubkey: string;
      payload: unknown;
    }>;
    __MAJU_E2E_SEED_ACTIVE_TURNS__?: (input: {
      agentPubkey: string;
      channelId: string;
      turnId: string;
      kind?: "turn_started" | "turn_completed";
    }) => void;
    __MAJU_E2E_SEED_OBSERVER_EVENTS__?: (input: {
      agentPubkey: string;
      events: Array<{
        seq: number;
        timestamp: string;
    ...
    __MAJU_E2E_SEED_MOCK_REMINDERS__?: (reminders: RelayEvent[]) => void;
    ...
    __MAJU_E2E_MD_PARSE_COUNT__?: () => number;
    /**
     ...
     * __MAJU_E2E_INVOKE_MOCK_COMMAND__. Call after the mutation to make the
     ...
     * follow up with __MAJU_E2E_INVALIDATE_CHANNELS__ to flush the cache.
     *
     * Only the listed fields are writeable; omitted fields are left unchanged.
     */
    __MAJU_E2E_MUTATE_CHANNEL__?: (opts: {
      channelId: string;
     ...
     * are held in a queue and not resolved until `__MAJU_E2E_RELEASE_GET_EVENT__()`
     ...
     */
    __MAJU_E2E_DEFER_GET_EVENT__?: string | null;
     ...
     *  requests released. */
    __MAJU_E2E_RELEASE_GET_EVENT__?: () => number;
    /** Count of `get_event` invocations for the current defer-target ID since
     *  the last time `__MAJU_E2E_DEFER_GET_EVENT__` was set. */
    __MAJU_E2E_GET_EVENT_CALL_COUNT__?: number;
    ...
    __MAJU_E2E_RELEASE_MANAGED_AGENT_STARTS__?: () => number;
...
// in e2e (instead of the `maju-media://` fallback). The reaction guard
...
// reacting with it exercises the proxy rewrite (unlike the `:maju:` fixture,
...
const E2E_IDENTITY_OVERRIDE_STORAGE_KEY = "maju:e2e-identity-override.v1";
...
// When `window.__MAJU_E2E_DEFER_GET_EVENT__` is set to a target event ID,
// `handleGetEvent` holds calls for that ID in this queue.  All other event IDs
// continue to resolve immediately.
// `window.__MAJU_E2E_RELEASE_GET_EVENT__()` flushes the queue and returns the
...
// Toggled at runtime by `__MAJU_E2E_HOLD_USERS_BATCH__(hold)` rather than fixed
...
// via `__MAJU_E2E_RELEASE_MANAGED_AGENT_STARTS__()`: a spec that holds a
...
// releasable early via `__MAJU_E2E_RELEASE_HUDDLE_AGENT_SYNCS__()`. The hold
...
function buildMockConfigSurface(pubkey: string): {
  ...
  // panel witnesses distinct provenance labels in one frame: "Set in Maju",
  ...
  const majuAgentSurface = {
    ...gooseSurface,
    runtimeId: "maju-agent",
    runtimeLabel: "Maju Agent",
    advanced: [],
    extensions: [],
    sources: {
  ...
  switch (pubkey) {
    ...
    case PUBKEY_MAJU_AGENT:
      return majuAgentSurface;
    default:
      return gooseSurface;
  }
}
...
function buildSeededManagedAgent(seed: MockManagedAgentSeed): MockManagedAgent {
  ...
  // is incorrect: maju-agent ships with no default args.
  const DEFAULT_RUNTIME_COMMAND: Record<
    string,
    { command: string; args: string[] }
  > = {
    goose: { command: "goose", args: ["acp"] },
    "maju-agent": { command: "maju-agent", args: [] },
  ...
  return {
    ...
    log_lines: [
      `maju-acp starting: relay=${DEFAULT_RELAY_WS_URL} agent_pubkey=${seed.pubkey} parallelism=1`,
...
const mockChannels: MockChannel[] = [
  ...
  createMockChannel({
    ...
    channel_type: "stream",
    visibility: "open",
    description: "Project home for the Maju community platform.",
    topic: null,
    purpose: null,
...
//
// This entire module (e2eBridge.ts) is loaded only when `window.__MAJU_E2E__`
...
const MOCK_HUDDLE_STORAGE_KEY = "maju.e2e.mock-huddle.v1";
...
function handleUpdateWorkflow(args: {
  ...
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__MAJU_E2E__?.mock?.workflowUpdateError;
...
function handleDeleteWorkflow(args: { workflowId: string }) {
  const configuredError = window.__MAJU_E2E__?.mock?.workflowDeleteError;
  if (configuredError) throw new Error(configuredError);
...
function handleTriggerWorkflow(args: { workflowId: string }) {
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__MAJU_E2E__?.mock?.workflowTriggerError;
  if (configuredError) throw new Error(configuredError);
...
function assertExpectedRelayScope(
  ...
  try {
    const activeId = window.localStorage.getItem("maju-active-community-id");
...
/**
 * Mirror the relay's channel-window row set (maju-db `thread.rs`, NIP-CW
...
const MOCK_PROJECT_SEEDS = [
  {
    ...
    name: "maju",
    description:
      "Relay, desktop, and mobile clients for the Maju community platform.",
    cloneUrl: `${DEFAULT_RELAY_HTTP_URL}/git/${MOCK_IDENTITY_PUBKEY}/maju`,
    webUrl: null,
...
const MOCK_PROJECT_BRANCHES_KEY = "maju-e2e-project-branches";
...
function buildMockProjectEvents(): RelayEvent[] {
  ...
  if (window.__MAJU_E2E_EMPTY_PROJECTS__) {
    return [];
  }
  ...
  for (const [projectIndex, seed] of MOCK_PROJECT_SEEDS.entries()) {
    const owner =
      projectIndex === 0
        ? (window.__MAJU_E2E_PROJECT_OWNER_OVERRIDE__ ?? seed.owner)
        : seed.owner;
  ...
  if (!window.__MAJU_E2E_REPOSITORY_ONLY_PROJECTS__) {
    const projectOwner =
      window.__MAJU_E2E_PROJECT_OWNER_OVERRIDE__ ?? MOCK_PROJECT_SEEDS[0].owner;
    events.push(
      createMockEvent(
        KIND_PROJECT_ANNOUNCEMENT,
        "",
        [
          ["d", "maju"],
          ["name", "maju"],
          ["description", "The complete Maju community platform."],
          ["a", `${KIND_REPO_ANNOUNCEMENT}:${projectOwner}:maju`],
          ...
          ["maju-related-channel", "9dae0116-799b-5071-a0a8-fdd30a91a35d"],
        ],
        projectOwner,
        now,
        "project-maju".padEnd(64, "0"),
      ),
    );
  }
...
function getMockProjectEventStore(): RelayEvent[] {
  if (!mockProjectEventStore) {
    ...
    const extras = window.__MAJU_E2E_EXTRA_PROJECT_EVENTS__;
...
async function handleDiscoverAcpRuntimes(
  ...
  const defaultCatalog: RawAcpRuntimeCatalogEntry[] = [
    ...
    },
    {
      ...
      install_hint: "Ships with the Maju desktop app.",
...
async function handleDiscoverManagedAgentPrereqs(
  ...
  return {
    acp: {
      command:
        configuredPrereqs?.acp?.command ?? args.input?.acpCommand ?? "maju-acp",
      resolved_path:
        configuredPrereqs?.acp?.resolvedPath ??
        "/Users/wesb/dev/maju/target/debug/maju-acp",
...
async function handleCreateManagedAgent(
  ...
  const pubkey = crypto
    ...
    .padEnd(64, "0")
    .slice(0, 64);
  const agentCommand = args.input.agentCommand ?? "maju-agent";
  const agentArgs =
  ...
  const managedAgent: MockManagedAgent = {
    ...
    acp_command: args.input.acpCommand ?? "maju-acp",
    agent_command: agentCommand,
    agent_args: agentArgs,
    ...
    log_lines: [
      `maju-acp starting: relay=${args.input.relayUrl ?? DEFAULT_RELAY_WS_URL} agent_pubkey=${pubkey} parallelism=${mintParallelism}`,
...
async function handleStartManagedAgent(
  ...
  if (isRelayMeshManagedAgent(agent)) {
    ...
    if (!hasLiveTarget) {
      throw new Error(
        "Maju shared compute cannot start because no live member is serving this model.",
      );
    }
  }
...
async function handleEditMessage(
  ...
  const extraTags = [
    ...
    ...(mentionTags ?? []),
    ...(mentionTags ? [["maju:mention-snapshot"]] : []),
    ...(args.suppressLinkPreviews ? [["link-preview", "none"]] : []),
  ];
...
async function handleGetEvent(
  ...
  // Defer/release seam: when __MAJU_E2E_DEFER_GET_EVENT__ is set to this
  // event's ID, hold this call in the queue until __MAJU_E2E_RELEASE_GET_EVENT__()
  ...
  if (
    window.__MAJU_E2E_DEFER_GET_EVENT__ &&
    window.__MAJU_E2E_DEFER_GET_EVENT__ === args.eventId
  ) {
    // Increment the count only for calls that are actually deferred.
    window.__MAJU_E2E_GET_EVENT_CALL_COUNT__ =
      (window.__MAJU_E2E_GET_EVENT_CALL_COUNT__ ?? 0) + 1;
...
function sendToMockSocket(args: {
  ...
  if (type === "REQ") {
    ...
    if (
      ...
      window.__MAJU_E2E_PROJECT_QUERY_FILTERS__ ??= [];
      window.__MAJU_E2E_PROJECT_QUERY_FILTERS__.push(filter);
      const rejectedKinds =
      ...
      if (
        window.__MAJU_E2E_DEFER_FULL_PROJECT_QUERIES__ &&
        !filter.authors &&
        !filter["#a"] &&
        !filter["#maju-channel"] &&
        !filter["#d"] &&
  ...
  if (type === "EVENT") {
    ...
    if (event.kind === KIND_AGENT_OBSERVER_FRAME) {
      ...
      if (frame === "control") {
        ...
        window.__MAJU_E2E_OBSERVER_CONTROLS__?.push({
    ...
    if (isMockProjectScopedEvent(event)) {
      ...
      if (
        ...
        window.__MAJU_E2E_UNSUPPORTED_PROJECT_ANNOUNCEMENTS__
      ) {
        sendWsText(socket.handler, [
          "OK",
          event.id,
      ...
      const rejectionIndex =
        window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(event.kind) ??
        -1;
      if (rejectionIndex >= 0) {
        window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
          rejectionIndex,
          1,
        );
        sendWsText(socket.handler, [
      ...
      const acceptedProjectEvents =
        window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__ ?? [];
      acceptedProjectEvents.push({
        content: event.content,
        kind: event.kind,
        tags: event.tags,
      });
      window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
      const failedAckIndex =
        window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(event.kind) ??
        -1;
      if (failedAckIndex >= 0) {
        window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
          failedAckIndex,
          1,
        );
        sendWsText(socket.handler, [
...
export function maybeInstallE2eTauriMocks() {
  ...
  window.__MAJU_E2E_LINK_PREVIEW_UPLOAD_STARTS__ = 0;
  window.__MAJU_E2E_MEDIA_FETCH_STATE__ = { active: 0, peak: 0 };
  window.__MAJU_E2E_RELEASE_LINK_PREVIEW_METADATA__ = () => {
    const queued = deferredLinkPreviewMetadataQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__MAJU_E2E_RELEASE_LINK_PREVIEW_UPLOADS__ = () => {
    const queued = deferredLinkPreviewUploadQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__MAJU_E2E_RELEASE_THREAD_REPLIES__ = () => {
    const queued = deferredThreadRepliesQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__MAJU_E2E_THREAD_REPLIES_PENDING__ = () =>
    deferredThreadRepliesQueue.length;
  window.__MAJU_E2E_HOLD_USERS_BATCH__ = (hold: boolean) => {
    holdUsersBatch = hold;
  ...
  window.__MAJU_E2E_USERS_BATCH_PENDING__ = () => heldUsersBatchReleases.length;
  ...
  window.__MAJU_E2E_OBSERVER_CONTROLS__ = [];
  window.__MAJU_E2E_RUN_MODEL_SWITCH__ = async ({
    agentPubkey,
    channelIds,
    modelId,
    requestId,
  ...
  window.__MAJU_E2E_MOUNT_AGENT_SESSION_PANEL__ = async ({
    agentPubkey,
    channelId,
  ...
  window.__MAJU_E2E_RELEASE_MEDIA_PROXY__ = () => {
    mockMediaProxyPort = MOCK_MEDIA_PROXY_PORT;
    resetMediaCaches();
    ensureRelayOriginFetch();
    return mockMediaProxyPort;
  };
  window.__MAJU_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__ = (payload) =>
    emit("huddle-tts-speaker-level", payload);
  window.__MAJU_E2E_SIGNED_EVENTS__ = [];
  window.__MAJU_E2E_WEBVIEW_ZOOM__ = 1;
  window.__MAJU_E2E_EMIT_MEDIA_UPLOAD_PHASE__ = async (input) => {
    await emit("media-upload-phase", input);
  };
  window.__MAJU_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__ = async (input) => {
    await emit("media-upload-progress", input);
  };
  window.__MAJU_E2E_SET_MOCK_HUDDLE_SNAPSHOT__ = async ({
    members,
  ...
  window.__MAJU_E2E_SET_MOCK_USER_STATUS__ = ({
    text,
    emoji,
    expiresAt,
    createdAt,
  ...
  window.__MAJU_E2E_PREPEND_MOCK_HISTORY__ = prependMockHistory;
  window.__MAJU_E2E_EMIT_MOCK_TYPING__ = ({
    channelName,
    createdAt,
    pubkey,
    threadHeadId,
  ...
  window.__MAJU_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__ = ({ channelName, kind }) => {
    const channel = mockChannels.find(
      (candidate) => candidate.name === channelName,
    );
    if (!channel) {
  ...
  window.__MAJU_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__ = ({
    ownerPubkey,
    kind,
  }) => hasMockOwnerKindSubscription(ownerPubkey, kind);
  window.__MAJU_E2E_HAS_MOCK_GLOBAL_KIND_SUBSCRIPTION__ = (kind) =>
    hasMockLiveSubscription(GLOBAL_MOCK_SUBSCRIPTION, kind);
  window.__MAJU_E2E_EMIT_MOCK_PRESENCE__ = ({ pubkey, status }) => {
  ...
  window.__MAJU_E2E_REPLACE_MOCK_TEAM_CATALOG_HEAD__ = (event) => {
    const dTag = event.tags.find((tag) => tag[0] === "d")?.[1];
    const existingIndex = mockTeamCatalogEvents.findIndex(
      (candidate) =>
        candidate.pubkey.toLowerCase() === event.pubkey.toLowerCase() &&
  ...
  window.__MAJU_E2E_PUSH_MOCK_FEED_ITEM__ = (item) => {
    const category = item.category === "mention" ? "mentions" : item.category;
    mockFeedOverrides[category].unshift(item);
    window.dispatchEvent(new CustomEvent("maju:e2e-home-feed-updated"));
    return item;
  };
  window.__MAJU_E2E_REPLACE_MOCK_FEED_ITEM__ = (oldId, item) => {
    const category = item.category === "mention" ? "mentions" : item.category;
  ...
  window.__MAJU_E2E_MD_PARSE_COUNT__ = getMarkdownParseCount;
  window.__MAJU_E2E_ACTIVATE_TIMEOUT__ = (expiresAtMs: number) => {
  ...
  window.__MAJU_E2E_INVALIDATE_CHANNELS__ = async () => {
    await window.__MAJU_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["channels"],
    });
  };
  window.__MAJU_E2E_MUTATE_CHANNEL__ = ({
    channelId,
    channelType,
    description,
    removeMemberPubkey,
  ...
  window.__MAJU_E2E_GET_EVENT_CALL_COUNT__ = 0;
  window.__MAJU_E2E_DEFER_GET_EVENT__ = null;
  deferredGetEventQueue = [];
  heldManagedAgentStartReleases = [];
  window.__MAJU_E2E_RELEASE_MANAGED_AGENT_STARTS__ = () => {
    const held = heldManagedAgentStartReleases.splice(0);
    for (const release of held) release();
    return held.length;
  };
  heldHuddleAgentSyncReleases = [];
  window.__MAJU_E2E_RELEASE_HUDDLE_AGENT_SYNCS__ = () => {
  ...
  window.__MAJU_E2E_CHANNELS_READ_PENDING__ = 0;
  window.__MAJU_E2E_DEFER_NEXT_CHANNELS_READ__ = () => {
    if (deferredChannelsReadResolve) {
      throw new Error("a channel read is already deferred");
    }
    deferNextChannelsRead = true;
  };
  window.__MAJU_E2E_RELEASE_CHANNELS_READ__ = () => {
    deferNextChannelsRead = false;
  ...
  window.__MAJU_E2E_RELEASE_GET_EVENT__ = () => {
  ...
  window.__MAJU_E2E_EMIT_MOCK_READ_STATE__ = ({
    clientId,
    contexts,
    createdAt,
    slotId,
  ...
  window.__MAJU_E2E_SET_RELAY_CONNECTION_STATE__ = (state) => {
  ...
  window.__MAJU_E2E_GET_RELAY_CONNECTION_STATE__ = () =>
    relayClient.getConnectionState();
  window.__MAJU_E2E_QUEUE_AUTH_RESPONSES__ = (responses) => {
    mockAuthResponses.push(...responses);
  };
  window.__MAJU_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__ = (reasons) => {
    mockChannelHistoryCloses.push(...reasons);
  };
  window.__MAJU_E2E_CLOSE_LIVE_SUBSCRIPTIONS__ = (reason) => {
    let closed = 0;
    for (const socket of mockSockets.values()) {
  ...
  window.__MAJU_E2E_SEED_MOCK_REMINDERS__ = (reminders) => {
  ...
  window.__MAJU_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__ = (unavailable) => {
    mockWebsocketUnavailable = unavailable;
    if (unavailable) relayWebsocketConnectAttemptStarts.length = 0;
  };
  window.__MAJU_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__ = () => [
    ...relayWebsocketConnectAttemptStarts,
  ];
  window.__MAJU_E2E_ACTIVATE_RELAY_RATE_LIMIT__ = (seconds) => {
    activateRateLimit(seconds);
  };
  window.__MAJU_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__ = () => {
    relayWebsocketConnectAttemptStarts.length = 0;
  };
  window.__MAJU_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__ = () => {
  ...
  window.__MAJU_E2E_SET_MESH__ = (mesh) => {
  ...
  window.__MAJU_E2E_SEED_ACTIVE_TURNS__ = ({
    agentPubkey,
    channelId,
    turnId,
    kind = "turn_started",
  ...
  window.__MAJU_E2E_SEED_OBSERVER_EVENTS__ = ({ agentPubkey, events }) => {
  ...
  const handleMockCommand = async (
    ...
    window.__MAJU_E2E_COMMANDS__?.push(command);
    const loggedPayload = (() => {
    ...
    window.__MAJU_E2E_COMMAND_PAYLOADS__?.push({
      command,
      payload: loggedPayload,
    });
    window.__MAJU_E2E_COMMAND_LOG__?.push({ command, payload });
    ...
    switch (command) {
      ...
      case "check_builderlab_community_name":
        return {
          available: true,
          normalized_host: `${(payload as { name?: string })?.name ?? "community"}.communities.maju.xyz`,
        };
      case "create_builderlab_community": {
        ...
        return {
          community: activeConfig?.mock?.builderlabCreatedCommunity ?? {
            id: `hosted-${name}`,
            name,
            normalized_host: `${name}.communities.maju.xyz`,
          },
        };
      }
          ...
          entries: [
            {
              ...
              size: "3.5GB",
              sizeGb: 3.5,
              description: "Maju-curated local agent model",
              fit: "comfortable",
              installed: true,
              recommended: true,
              curated: true,
      ...
      case "sign_out":
        ...
        // assert invocation via __MAJU_E2E_COMMANDS__ and the pending UI.
        return;
      ...
      case "get_project_repo_snapshot":
        ...
        return {
          ...
          files: [
            ...
            {
              path: "crates/maju-relay/src/api/git/transport.rs",
              kind: "blob",
              size: 33120,
              preview_content:
                "// Smart HTTP git transport\n// Handles upload-pack and receive-pack for Maju git repos.\n",
            },
          ],
        };
      case "get_project_local_repo_snapshot":
        return window.__MAJU_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__ ?? null;
      ...
      case "get_project_local_repo_file_content": {
        ...
        return path
          ? (window.__MAJU_E2E_PROJECT_REPO_FILE_CONTENTS__?.[path] ?? null)
          : null;
      }
      ...
      case "clone_project_repository": {
        ...
        const path = "/tmp/maju/REPOS/maju";
      ...
      case "publish_project_owner_announcement": {
        ...
        window.__MAJU_E2E_SIGNED_EVENTS__?.push({
          content: event.content,
          createdAt: event.created_at,
          kind: event.kind,
          tags: event.tags,
        ...
        if (
          ...
          const rejectionIndex =
            window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(
              event.kind,
            ) ?? -1;
          if (rejectionIndex >= 0) {
            window.__MAJU_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
            ...
            window.__MAJU_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
            const failedAckIndex =
              window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(
                event.kind,
              ) ?? -1;
            if (failedAckIndex >= 0) {
              window.__MAJU_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
      ...
      case "sign_project_pull_request_status": {
        ...
        window.__MAJU_E2E_SIGNED_EVENTS__?.push(event);
        getMockProjectEventStore().push(event);
        return null;
      }
      ...
      case "merge_project_pull_request": {
        ...
        if (window.__MAJU_E2E_PROJECT_MERGE_ERROR__) {
          throw window.__MAJU_E2E_PROJECT_MERGE_ERROR__;
        }
      ...
      case "open_project_merge_recovery_terminal": {
        ...
        return {
          path: "/tmp/maju/REPOS/maju",
          cloned: false,
          recoveryRef: `refs/maju/merge-recovery/${input.expectedCommit}`,
          targetRef: `refs/maju/merge-recovery-target/${"f".repeat(40)}`,
        };
      }
      ...
      case "get_channels": {
        ...
        if (deferred) {
          await new Promise<void>((resolve) => {
            ...
            window.__MAJU_E2E_CHANNELS_READ_PENDING__ = 1;
          });
        }
        return channels;
      }
      ...
      case "export_agent_snapshot":
        ...
        // Specs assert invocation via __MAJU_E2E_COMMANDS__.
        return true;
      ...
      case "start_managed_agent":
        ...
        return handleStartManagedAgent(
          ...
          window.__MAJU_E2E_COMMANDS__?.push("start_managed_agent:settled");
        });
      ...
      case "discover_agent_models": {
        ...
        if (provider === "relay-mesh") {
          ...
          if (mockMeshState.models.length === 0) {
            throw new Error(
              "no Maju shared compute serving members are available",
            );
          }
        }
        const models =
      ...
      case "upload_media_bytes": {
        ...
        if (input.filename?.startsWith("link-preview-")) {
          window.__MAJU_E2E_LINK_PREVIEW_UPLOAD_STARTS__ =
            (window.__MAJU_E2E_LINK_PREVIEW_UPLOAD_STARTS__ ?? 0) + 1;
        }
      ...
      case "fetch_media_bytes": {
        ...
        if (!window.__MAJU_E2E_MEDIA_FETCH_STATE__) {
          window.__MAJU_E2E_MEDIA_FETCH_STATE__ = { active: 0, peak: 0 };
        }
        const stats = window.__MAJU_E2E_MEDIA_FETCH_STATE__;
        stats.active += 1;
        stats.peak = Math.max(stats.peak, stats.active);
        try {
          if (window.__MAJU_E2E_HOLD_MEDIA_FETCHES__) {
      ...
      case "save_agent_card":
        ...
        // command was invoked via `__MAJU_E2E_COMMANDS__`, not the dialog.
        return true;
      ...
      case "plugin:webview|set_webview_zoom":
        window.__MAJU_E2E_WEBVIEW_ZOOM__ = (payload as { value: number }).value;
        return;
      case "start_pairing": {
      ...
      case "list_save_subscriptions": {
        ...
        if (!win.__MAJU_E2E_IPC_COUNTERS__) {
          win.__MAJU_E2E_IPC_COUNTERS__ = {};
        }
        const ipcCounters = win.__MAJU_E2E_IPC_COUNTERS__ as Record<
          string,
          number
        >;
      ...
      case "channel_head_cache_load": {
        ...
        const key = `maju-e2e-channel-head:${args.scope.pubkey.toLowerCase()}:${args.scope.relayUrl.toLowerCase().replace(/\/$/, "")}`;
  ...
  window.__MAJU_E2E_INVOKE_MOCK_COMMAND__ = (command, payload) =>
    handleMockCommand(command, payload ?? null);
  window.__MAJU_E2E_EMIT_TAURI_EVENT__ = (event, payload) =>
    emit(event, payload);
