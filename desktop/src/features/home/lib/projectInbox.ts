...
/** Returns the canonical Maju Git repository and root event for an Inbox row. */
...
/** Whether a feed event belongs to a Maju Git pull request or issue thread. */
export function isProjectInboxItem(item: FeedItem) {
  return getProjectInboxReference(item) !== null;
}

/** Resolves an Inbox event to the current canonical Maju Git work item. */
