...
import { resetChannelSnapshotCache } from "@/features/channels/channelSnapshot";
    ...
    async function init() {
      if (!activeCommunity) {
        ...
        try {
          ...
          // selection even when MAJU_RELAY_URL is overridden at runtime.
          if (
            !suppressAutoConnect &&
            (isSharedIdentity ||
