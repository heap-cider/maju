...
function buildProjectPatchTemplate({
  ...
  // (d, name, description, maju-channel, maju-visibility, relay hints embedded
  ...
  return {
    ...
    tags: [
      ...repository.eventTags
        .filter((tag) => tag[0] !== "maju-channel")
        .map((tag) => [...tag]),
      ["maju-channel", normalizedChannelId],
    ],
  };
}
...
export function buildAddedRepositoryEventTemplatesFromHead({
  ...
  repositoryTags.push(["maju-channel", normalizedAccessChannelId]);
