...
export type Project = {
   ...
   * `maju-related-channel` tags. Client convention: NIP-MP treats the tag as
...
/** Repeatable project tag naming an extra stream besides `maju-channel`. */
export const PROJECT_RELATED_CHANNEL_TAG = "maju-related-channel";
...
export function eventToRepository(
  ...
  const channel = getTag(event, "maju-channel");
  return {
    id: `${owner}:${dtag}`,
    dtag,
...
export function eventToExplicitProject(
  ...
  const rawVisibility = getTag(event, "maju-visibility");
  const visibility =
 ...
 * including Maju's NIP-OA owner delegation for agent-authored events.
 */
