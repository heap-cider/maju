...
function projectEvent(repositoryTags, overrides = {}) {
  return {
    ...
    tags: [
      ...
      ["maju-channel", "11111111-1111-4111-8111-111111111111"],
      ...repositoryTags,
    ],
    ...overrides,
  };
...
test("buildProjectReadModels keeps extra related channel ids", () => {
  ...
  const projects = buildProjectReadModels({
    projectEvents: [
      projectEvent([
        ["maju-related-channel", relatedA],
        ["maju-related-channel", home],
        ["maju-related-channel", relatedB],
        ["maju-related-channel", relatedA],
      ]),
    ],
    repositoryEvents: [],
...
test("selectProjectRepository resolves a non-primary repository when repositoryId is derived from a 30617 projectId", () => {
  ...
  const project = {
    ...
    status: "active",
    projectAddress: `30621:${OWNER}:maju`,
