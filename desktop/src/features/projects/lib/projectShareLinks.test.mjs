...
test("projectShareLink carries the active workspace tab for both link kinds", () => {
  assert.equal(
    ...
    `maju://project?owner=${OWNER}&d=pollinator&tab=prs`,
  );
  // Legacy projects share as maju://repo and keep the tab too.
  assert.equal(
    projectShareLink({ projectAddress: REPO_ADDRESS }, "issues"),
    `maju://repo?owner=${OWNER}&d=flappy-bee&tab=issues`,
  );
});
...
test("projectShareLink links legacy projects as their backing repository", () => {
  assert.equal(
    ...
    `maju://repo?owner=${OWNER}&d=flappy-bee`,
  );
});
...
test("issue and pull request links carry the event id and repo coordinate", () => {
  assert.equal(
    ...
    `maju://issue?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
  assert.equal(
    pullRequestShareLink({ id: EVENT_ID, repoAddress: REPO_ADDRESS }),
    `maju://pr?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
});
