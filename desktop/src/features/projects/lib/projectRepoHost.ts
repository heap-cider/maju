...
export function projectRepoHost(
  ...
  try {
    ...
    const isMajuPath = /^\/git\/[0-9a-f]{64}\/[^/]+\/?$/i.test(clone.pathname);

    if (clone.origin === relay.origin && isMajuPath) {
      return { kind: "maju" };
    }
...
/**
 * Human-readable location of a repository's git data — "github.com/heap-cider/maju"
 * for external repos (host + path, `.git` stripped), or "owner/repo" for
 * Maju-hosted ones (the relay host and full owner pubkey carry no signal;
...
export function repositoryDisplayPath(
  ...
  if (projectRepoHost(cloneUrl, relayOrigin).kind === "maju") {
