...
test("query builders emit the tokens FTS needs, nothing else", () => {
  ...
  assert.equal(
    repositoryDiscussionQuery({ owner: OWNER, dtag: "maju-world" }),
    `${OWNER} maju-world`,
  );
});
...
test("discussionSnippet strips entity links and coordinates", () => {
  assert.equal(
    discussionSnippet(
      `Can someone review maju://pr?id=${EVENT_ID}&owner=${OWNER}&d=maju before Friday?`,
    ),
  ...
  assert.equal(
    discussionSnippet(`maju://repo?owner=${OWNER}&d=maju`),
