...
| Action | Command |
| --- | --- |
| Inspect the repository | \`maju repos get --owner {{REPO_OWNER_HEX}} --id {{REPO_SLUG}}\` |
| Create a task | \`maju issues create --channel {{CHANNEL_UUID}} --title "..." --content -\` |
| Claim or assign a task | \`maju issues assign --issue <id> --repo-owner {{REPO_OWNER_HEX}} --repo-id {{REPO_SLUG}} --assignee <hex>\` |
| Track task state | \`maju issues status --issue <id> --repo-owner {{REPO_OWNER_HEX}} --repo-id {{REPO_SLUG}} --status open|resolved|closed|draft\` |
| Open a review | \`maju pr open --repo-owner {{REPO_OWNER_HEX}} --repo-id {{REPO_SLUG}} --subject "..." --body-file - --commit <tip> --clone {{REPO_CLONE_URL}} --branch-name <branch> --channel {{CHANNEL_UUID}}\` |
| Update a review | \`maju pr update --repo-owner {{REPO_OWNER_HEX}} --repo-id {{REPO_SLUG}} --pr <id> --pr-author <hex> --commit <tip> --clone {{REPO_CLONE_URL}}\` |
| Mark a review merged or closed | \`maju pr status --pr <id> --repo-owner {{REPO_OWNER_HEX}} --repo-id {{REPO_SLUG}} --status merged|closed\` |
| Share files or artifacts | \`maju upload file --file <path>\` |
| Update this living document | \`maju canvas set --channel {{CHANNEL_UUID}} --content -\` |
...
4. **Ship:** Open a review and post the returned Maju link verbatim so it renders as a card. Mark the issue resolved when merged.
