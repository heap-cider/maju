...
test("projectRepoHost recognizes a canonical repository on the relay", () => {
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/maju`, ORIGIN), {
    kind: "maju",
  });
});
...
test("projectRepoHost identifies an external repository by host", () => {
  assert.deepEqual(
    projectRepoHost("https://github.com/heap-cider/maju.git", ORIGIN),
...
test("projectRepoHost fails closed while either URL is unresolved", () => {
  ...
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/maju`, null), {
    kind: "unresolved",
  });
