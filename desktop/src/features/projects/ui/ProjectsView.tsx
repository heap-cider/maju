...
export function ProjectsView() {
  ...
  const [repositoryScope, setRepositoryScope] =
    React.useState<ProjectsRepositoryScope>(() => {
      ...
      return filter === "projects" &&
        (storedScope === "maju" || storedScope === "linked")
        ? "all"
        : storedScope;
    });
  ...
  // One blobless clone per primary Maju repository, only while the overview
  // header is visible.
  const snapshotProjects = React.useMemo(
    () =>
      filter === "all"
        ? projects.filter(
            (project) =>
              projectRepoHostForProject(project, relayOrigin).kind === "maju",
          )
        : [],
        ...
        >
          ...
          <div className="flex min-h-0 min-w-0 flex-1">
            <div className="relative min-h-0 min-w-0 flex-1">
              ...
              />
              <div
                className="maju-content-scrollbar h-full min-h-0 min-w-0 overflow-x-hidden overflow-y-scroll"
                onScroll={handleContentScroll}
              >
                <div className="px-4 pb-4">
                  <div className="w-full space-y-3">
                    <ProjectsOverviewToolbarRow
                      detached={overviewDetached}
                      filter={filter}
