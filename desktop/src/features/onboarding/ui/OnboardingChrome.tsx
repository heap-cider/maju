 ...
 * light-blue label (`--maju-onboarding-cta-label`, available on any
 * `.maju-onboarding-neutral-theme` subtree).
 */
export const ONBOARDING_PRIMARY_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--maju-onboarding-cta-label)]`;

/**
 * Primary-CTA styling for the landing screen only: the shared pill with the
 * chartreuse label (`--maju-welcome-chartreuse`). The blue label is reserved
 * for the CTAs inside the steps.
 */
export const ONBOARDING_LANDING_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--maju-welcome-chartreuse)]`;

/**
 * Icon-control styling for onboarding surfaces that sit on the textured card:
 * olive backup ink (`--maju-onboarding-backup-ink`) with a plain
...
export const ONBOARDING_INK_ICON_CLASS =
  "text-[color:var(--maju-onboarding-backup-ink)] hover:bg-transparent hover:text-foreground";

/**
 * Shared onboarding chrome shown on every page after the landing screen: a
 * static Maju mark pinned to the top-left, and a centered pagination track that
  ...
  return (
    ...
    >
      <span className="block w-11" data-testid="onboarding-logo">
        <MajuMark className="h-auto w-full" />
      </span>
      <div
