 ...
 * Provider path: the preferred Maju Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "maju-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "maju-agent");
  ...
  if (preferredRuntime.id !== "maju-agent" && preferredRuntime.id !== "goose") {
    return { ready: false };
  }
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "maju-agent" };
    }
  }
