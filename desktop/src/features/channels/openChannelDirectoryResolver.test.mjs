...
beforeEach(() => {
  ipc.reset();
  localStorage.clear();
  localStorage.setItem("maju-communities", JSON.stringify([COMMUNITY]));
  localStorage.setItem("maju-active-community-id", COMMUNITY.id);
});
...
test("markdown message links resolve private destinations without a directory scan", async () => {
  ...
  const link = `maju://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector(
        "span[data-message-link][data-maju-link]",
      ),
      null,
...
test("authored-label channel and message links respect the private-destination gate", async () => {
  ...
  //   - maju://channel/<uuid> and maju://channel/<uuid>/<event-id> reach the
  //     gate via ChannelDeepLinkAnchor's authored branch, and
  //   - the canonical maju://message?channel=&id= form (produced by
  ...
  const channelMessageLink = `maju://channel/${channelId}/${messageId}`;
  const canonicalMessageLink = `maju://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector("span[data-maju-link]"),
      null,
