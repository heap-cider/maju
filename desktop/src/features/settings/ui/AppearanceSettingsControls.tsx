...
/** Maju navigation can use either its production tint or a stronger tab. */
...
/**
 * Build the sample thumbnail as an SVG data URL from the Maju gradient
 * tokens. Data-URL images cannot resolve CSS variables, so the token values
 * are read from the live stylesheet and baked in per render — if the Maju
 * gradient ever changes in `theme.css`, this preview follows automatically.
 */
function majuGradientSampleImage(isDark: boolean): string {
  ...
  const top = isDark
    ? readToken("--maju-gradient-dark-top", "#4a4616")
    : readToken("--maju-gradient-light-top", "#e6e6b6");
  const bottom = isDark
    ? readToken("--maju-gradient-dark-bottom", "#0a1423")
    : readToken("--maju-gradient-light-bottom", "#c4d0da");
...
function LinkPreviewSample({ style }: { style: LinkPreviewStyle }) {
  ...
  const preview = React.useMemo<ResolvedLinkPreview>(
    () => ({
      ...
      imageDataUrl: majuGradientSampleImage(isDark),
    }),
    [isDark],
  );
  return (
...
function ThreadLayoutDiagram({ mode }: { mode: ThreadViewMode }) {
  ...
  // Maju gradient tokens directly and follows theme.css automatically.
  const gradientTop = isDark
    ? "var(--maju-gradient-dark-top, #4a4616)"
    : "var(--maju-gradient-light-top, #e6e6b6)";
  const gradientBottom = isDark
    ? "var(--maju-gradient-dark-bottom, #0a1423)"
    : "var(--maju-gradient-light-bottom, #c4d0da)";
    ...
    <SettingsOptionRow className="items-start">
      <div className="min-w-0">
        ...
        <p
          ...
          data-settings-subcopy
        >
          Choose the highlight color used throughout Maju.
        </p>
      </div>
      <div
