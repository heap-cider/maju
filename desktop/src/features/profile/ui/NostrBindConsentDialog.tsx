...
const COPY_SUCCESS_MESSAGE =
  "Signed response copied. Paste it into the Maju admin console.";
const PREVIEW_COPY_SUCCESS_MESSAGE = "Preview response copied.";
const COPY_FAILURE_MESSAGE = "Maju couldn't access the clipboard. Try again.";
...
function appendCallbackStatus(callbackUrl: string): string {
  ...
  url.searchParams.set("maju_bind", "signed");
  return url.toString();
}
    ...
    const unlistenPromise = listenForNostrBindDeepLinks((nextPayload) => {
      ...
      getIdentity()
        ...
        .catch((error) => {
          ...
          setIdentity(null);
          setError("Could not load the current Maju identity.");
        });
    });
      ...
      <DialogPrimitive.Portal>
        {payload ? (
          <DialogPrimitive.Content
            ...
            className="maju-onboarding-neutral-theme maju-startup-shell fixed inset-0 z-50 flex overflow-y-auto bg-background px-4 py-12 text-foreground outline-hidden"
            ...
            <div className="m-auto flex w-full max-w-[500px] flex-col items-center text-center">
              ...
              {signedResponse ? (
                ...
                >
                  ...
                  >
                    {payload.returnMode === "browser_fragment_v1"
                      ? "Maju opened your browser to finish verification."
                      : "Copy the response below, then paste it into the Maju website to finish verification."}
                  </DialogPrimitive.Description>
