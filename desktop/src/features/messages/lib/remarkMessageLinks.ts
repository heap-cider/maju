/**
 * Remark plugin that detects bare `maju://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `maju://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `maju://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](maju://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:maju|maju):\/\/message\?[^\s<>"')\]]+/g;
