...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `maju://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `maju://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `maju://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects maju:// with wrong host", () => {
  const r = parseMessageLink(`maju://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`maju://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`maju://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy maju://message links", () => {
  const r = parseMessageLink(`maju://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches maju://message and legacy maju://message", () => {
  assert.equal(
    isMessageLink(`maju://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`maju://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("maju://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("maju://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `maju://message?channel=${CHANNEL}&id=${MESSAGE}`;
