...
const PROJECT_HREF = `maju://project?owner=${OWNER}&d=maju-world`;
...
test("rejects malformed message links", () => {
  assert.equal(
    resolveComposerMessageLinkAttributes(
      `maju://message?channel=${CHANNEL_ID}`,
      () => "general",
    ),
    null,
  );
...
const EXACT_LINK_PASTE_ACCEPTED_CASES = [
  ...
  [
    ...
    `maju://message?channel=${CHANNEL_ID}&id=${CHANNEL_MESSAGE_ID}`,
  ],
...
test("exact link paste canonicalizes Maju links", () => {
  assert.deepEqual(
    exactLinkPaste(
      `MAJU://channel/${CHANNEL_ID.toUpperCase()}/${CHANNEL_MESSAGE_ID.toUpperCase()}`,
    ),
    {
      href: `maju://message?channel=${CHANNEL_ID}&id=${CHANNEL_MESSAGE_ID}`,
    },
...
for (const input of [
  ...
  "not a url",
  `See ${HREF}`,
  `maju://channel/${CHANNEL_ID}/not-a-message-id`,
]) {
...
test("paste handler canonicalizes Maju links over selected text", () => {
...
test("composer node renders channel and entity chip presentations", () => {
  ...
  assert.equal(renderedChipLabel(repo), "maju-world");

  const project = render(PROJECT_HREF);
  assert.equal(project[1]["data-maju-link-kind"], "project");
  assert.match(project[1].class, /inline-chip-icon-project/);
  assert.equal(renderedChipLabel(project), "maju-world");
  ...
  assert.equal(renderedChipLabel(issue), "maju-world");
  ...
  assert.equal(renderedChipLabel(pullRequest), "maju-world");
});
