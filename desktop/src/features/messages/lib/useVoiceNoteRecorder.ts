...
export function useVoiceNoteRecorder() {
  ...
  const start = React.useCallback(async () => {
    ...
    try {
          ...
          if (!session.cancelled && blob.size > 0) {
            try {
              ...
              if (
                ...
                setError("Maju could not prepare this voice note for upload.");
              }
            }
          }
          releaseSessionAudio(session);
      ...
      setError(
        denied
          ? "Allow Maju to access your microphone to record a voice note."
          : "Maju could not start the voice recorder.",
      );
    }
  }, [status]);
