...
const EMOJI_TAGS = [["emoji", "maju", "https://relay/maju.png"]];
...
test("derives custom emoji and emoji-only flag from event tags", () => {
  ...
  assert.equal(isEmojiOnlyMessage(":maju:", customEmoji), true);
  assert.equal(isEmojiOnlyMessage("hi :maju:", customEmoji), false);
});

test("messages without tags get no custom emoji and are never emoji-only", () => {
  const customEmoji = undefined;
  assert.equal(isEmojiOnlyMessage(":maju:", customEmoji), false);
});
