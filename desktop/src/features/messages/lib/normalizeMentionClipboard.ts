...
/**
 * Detect whether clipboard HTML contains Maju mention / channel-link
...
/**
 * Normalize clipboard HTML that contains Maju mention / channel-link
...
export function normalizeMentionClipboardContent(
  ...
  for (const el of Array.from(
    ...
    // the sigil unless the source already carries it (Maju's own copy
    // handlers write it back before the HTML reaches the clipboard) —
    // unless the chip is a fragment. Maju's copy handlers decline a
