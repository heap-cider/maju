...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "maju_custom_emoji";
  const TOKEN_TYPE = "maju_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:maju:` and
    // `http://x:y:maju:` must NOT turn the inner `:maju:` into an image;
