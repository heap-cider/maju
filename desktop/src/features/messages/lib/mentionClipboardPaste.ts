...
import { getMajuCopyKind } from "./mentionClipboard";
...
/**
 * Paste clipboard HTML that carries Maju mention markers.
 *
 ...
 * - `rich` (or legacy Maju HTML with no marker) — keep the HTML path, with
...
export function handleMentionClipboardPaste({
  ...
  if (getMajuCopyKind(html) === "markdown" && text) {
    preventDefault();
