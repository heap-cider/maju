...
test("rule fires for a boundary :shortcode: (start of string)", () => {
  const rule = captureRule(["maju"]);
  const { matched, advanced } = runRule(rule, ":maju:", 0);
  assert.equal(matched, true);
  assert.equal(advanced, ":maju:".length);
});

test("rule fires for a :shortcode: preceded by whitespace", () => {
  const rule = captureRule(["maju"]);
  // pos points at the `:` after the space.
  const { matched } = runRule(rule, "hi :maju:", 3);
  assert.equal(matched, true);
});

test("rule does NOT fire when the colon is glued to a word char (not:maju:)", () => {
  const rule = captureRule(["maju"]);
  // pos points at the `:` immediately after `not`.
  const { matched } = runRule(rule, "not:maju:", 3);
  assert.equal(matched, false);
});

test("rule does NOT fire inside a URL-like sequence (http://x:y:maju:)", () => {
  const rule = captureRule(["maju"]);
  const src = "http://x:y:maju:";
  // pos points at the `:` immediately after `y` (a word char).
  const { matched } = runRule(rule, src, src.indexOf(":maju:"));
  assert.equal(matched, false);
});
...
test("rule fires after punctuation boundary (e.g. parenthesis)", () => {
  ...
  const { matched } = runRule(rule, "(:maju:)", 1);
  assert.equal(matched, true);
});
