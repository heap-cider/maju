 ...
 *
 * Every Maju copy writes two flavors:
 ...
 *    carrying its pubkey. Pasting back into a Maju composer harvests those
 ...
 * which content path to use — see `MajuCopyKind`.
 */

/** Marks clipboard HTML that Maju produced, and what its plain flavor holds. */
export const MAJU_COPY_ATTRIBUTE = "data-maju-copy";
 ...
 *   chip picks up in transit: Maju's copy handlers write the sigil into the
...
/**
 * What the `text/plain` flavor of a Maju copy contains.
 *
 ...
 */
export type MajuCopyKind = "markdown" | "rich";
...
export function buildMentionClipboardHtml({
  ...
  return `<span ${MAJU_COPY_ATTRIBUTE}="${kind}">${parts.join("")}</span>`;
}

/** The Maju copy marker on clipboard HTML, or `null` for foreign HTML. */
export function getMajuCopyKind(html: string): MajuCopyKind | null {
  const match = html.match(
    new RegExp(
      `\\b${MAJU_COPY_ATTRIBUTE}\\s*=\\s*["'](markdown|rich)["']`,
      "i",
    ),
  );
  return (match?.[1] as MajuCopyKind | undefined) ?? null;
}
...
/**
 * Recover the `label → pubkey` records a Maju copy embedded in its HTML.
 *
...
/**
 * Narrow copied records to the pairs trusted Maju state vouches for.
 *
 ...
 * outlives a paste the user could not see; and trusted Maju state has to
