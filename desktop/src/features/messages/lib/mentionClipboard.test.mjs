...
test("rich copies declare their own flavor", () => {
  ...
  assert.match(html, /data-maju-copy="rich"/);
});

// ── getMajuCopyKind ───────────────────────────────────────────────────

test("reads the copy marker, and only Maju's", () => {
  assert.equal(
    getMajuCopyKind('<span data-maju-copy="markdown">hi</span>'),
    "markdown",
  );
  assert.equal(getMajuCopyKind('<div data-maju-copy="rich">hi</div>'), "rich");
  assert.equal(getMajuCopyKind("<p>hi</p>"), null);
  assert.equal(getMajuCopyKind('<span data-maju-copy="other">hi</span>'), null);
});

// ── parseMentionClipboardRecords ──────────────────────────────────────

test("recovers records from a Maju copy", () => {
