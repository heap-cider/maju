/**
 * `maju://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `maju://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "maju:";
...
/**
 * Build a `maju://message` URL for a given channel + message.
 *
...
/**
 * Parse a `maju://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("maju://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("maju://message?") || href === "maju://message";
}
 ...
 * CommonMark autolinks (`<maju://message?...>`) and explicitly labeled links
