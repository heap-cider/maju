...
test("edit mention snapshot replaces original references, including removals", () => {
  ...
  assert.deepEqual(
    replacement.filter((tag) => tag[0] === "maju:mention-snapshot"),
    [["maju:mention-snapshot"]],
  );

  const removed = applyEditTagOverlay(original, [["maju:mention-snapshot"]]);
  assert.deepEqual(
    removed.filter((tag) => tag[0] === "mention"),
    [],
  );
  assert.deepEqual(
    removed.filter((tag) => tag[0] === "maju:mention-snapshot"),
    [["maju:mention-snapshot"]],
  );
});
