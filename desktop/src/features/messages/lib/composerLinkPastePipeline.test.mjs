...
async function mountComposerEditor() {
  ...
  // what teaches linkify the `maju` protocol. Paste before that lands and a
  // `maju://` assertion passes for the wrong reason.
  await waitFor(() =>
  ...
  assert.equal(
    ...
    CHANNEL_HREF,
    "expected Link's onCreate to register the maju protocol with linkify",
  );
  return editor;
}
...
test("mixed selection paste of a Maju link becomes a chip, not a part-link", async () => {
  const editor = await mountComposerEditor();
  seedDocument(
    editor,
    paragraph(text("ordinary")),
