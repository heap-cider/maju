...
test("maju://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`maju://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message maju:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("maju://channel?foo=bar", handlers);
  assert.deepEqual(external, ["maju://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed maju://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with maju://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("maju://message?nope=1", handlers);
  assert.deepEqual(external, ["maju://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
