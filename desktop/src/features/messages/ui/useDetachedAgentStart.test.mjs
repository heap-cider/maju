...
beforeEach(async () => {
  ...
  window.localStorage.setItem("maju-active-community-id", "community-a");
});
...
const OTHER_AGENT_RECORD = { pubkey: OTHER_AGENT, name: "maju" };
...
test("a wake is refused when no community is active to scope it", async () => {
  window.localStorage.setItem("maju-communities", JSON.stringify([]));
  window.localStorage.removeItem("maju-active-community-id");
  const { act, rendered } = await renderDetachedStart();
...
test("a blank relay URL is refused rather than sent as no assertion", async () => {
  ...
  window.localStorage.setItem("maju-active-community-id", "community-blank");
  const { act, rendered } = await renderDetachedStart();
