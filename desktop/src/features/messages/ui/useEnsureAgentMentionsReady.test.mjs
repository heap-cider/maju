...
test("a representative on another device receives the mention without a duplicate wake", async () => {
  for (const backend of [{ type: "local" }, { type: "provider" }]) {
    const agent = managedAgent({ backend });
    let reads = 0;
    ...
    assert.deepEqual(result.agentsToWake, []);
    assert.deepEqual(result.errors, []);
    assert.equal(result.wroteRelayState, false);
