...
function ComposerLinkPreviewCard({
  ...
  // External cards are send-ready only once their snapshot tag exists. Maju
  ...
  const done = snapshotTagReady || isMajuEntityPreview(preview);
  let path = "";
  try {
...
export function useComposerLinkPreviews(content: string, enabled = true) {
  ...
  // ready -> not-ready -> ready (maju:// links never snapshot, so they never
  ...
  // before resolution even starts. maju:// links never snapshot, so ignore them.
  const hasUnresolvedLiveCandidates =
    !suppressed &&
    liveCandidatesRef.current.some(
      (href) =>
        !href.startsWith("maju://") &&
        !readyTags[href] &&
