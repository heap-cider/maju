...
//
// A same-relay `/git/<owner>/<repo>` clone URL is a Maju repository entity: the
// renderer normalizes it onto `maju://repo` and shows it as an inline chip, not
...
test("composer input classifies a same-relay clone URL as a Maju entity", async () => {
  const { updateComposerLinkPreviewInput } = await import(
    "./useComposerLinkPreviews.tsx"
  );
  const empty = {
...
test("composer never fetches a snapshot for a same-relay clone URL", async () => {
  ...
  try {
    ...
    assert.equal(
      fetchCalls,
      0,
      "a Maju repository entity must not enter external snapshot fetching",
    );
