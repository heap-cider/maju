 ...
 *   maju-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/maju-agent/src/types.rs`.
 *   maju-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped maju-agent prefix or the maju-acp v0 wrap.
  if (
