...
import { NUMERIC_KIND_MIN } from "../ui/majuAgentModelTuningFields.tsx";
...
test("Goose exposes provider, model, and its real effort application key", () => {
  ...
  // global tier is native-only, so the legacy MAJU_AGENT_THINKING_EFFORT in the
...
for (const scope of ["global", "onboarding"]) {
  test(`effort persists to the runtime native key at ${scope} scope`, () => {
    ...
    const maju = deriveAgentConfigFieldModel({
      config,
      runtime: runtime("maju-agent", {
        thinkingEnvVar: "MAJU_AGENT_THINKING_EFFORT",
      }),
    ...
    const majuEffort = field(maju, "effort");
    assert.deepEqual(majuEffort.currentPersistence, {
      kind: "envVar",
      key: "MAJU_AGENT_THINKING_EFFORT",
    });
    assert.equal(majuEffort.value, "high");
  });
}

// Per-agent scopes (definition/instance) intentionally keep effort on the
// generic legacy MAJU_AGENT_THINKING_EFFORT row until PR 2.7 migrates Goose —
...
test("maju-agent derives three numeric descriptors from catalog fields", () => {
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("maju-agent", {
      modelEnvVar: "MAJU_AGENT_MODEL",
      providerEnvVar: "MAJU_AGENT_PROVIDER",
      thinkingEnvVar: "MAJU_AGENT_THINKING_EFFORT",
      maxTokensEnvVar: "MAJU_AGENT_MAX_OUTPUT_TOKENS",
      contextLimitEnvVar: "MAJU_AGENT_MAX_CONTEXT_TOKENS",
      maxRoundsEnvVar: "MAJU_AGENT_MAX_ROUNDS",
    }),
    scope: "global",
  });
  ...
  assert.deepEqual(maxOutput.currentPersistence, {
    kind: "envVar",
    key: "MAJU_AGENT_MAX_OUTPUT_TOKENS",
  });
...
test("numeric descriptor value is read from env_vars when set", () => {
  const cfgWithTuning = {
    env_vars: {
      ...
      MAJU_AGENT_MAX_CONTEXT_TOKENS: "100000",
      MAJU_AGENT_MAX_ROUNDS: "25",
    },
    model: "test-model",
    preferred_runtime: null,
    provider: "anthropic",
...
// key (MAJU_AGENT_THINKING_EFFORT) must NOT appear in the output — it must
...
test("structuredEnvKeys_global_includes_effort_key_and_numeric_keys", () => {
  ...
  const majuAgentModel = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("maju-agent", {
      modelEnvVar: "MAJU_AGENT_MODEL",
      providerEnvVar: "MAJU_AGENT_PROVIDER",
  ...
  const renderedDescriptors = majuAgentModel.fields.filter(
    (f) => f.render === "control",
  );
  const keys = structuredEnvKeys(renderedDescriptors);

  assert.ok(
    keys.includes("MAJU_AGENT_THINKING_EFFORT"),
    "effort key must be hidden on global (effort control renders)",
  );
  assert.ok(
    keys.includes("MAJU_AGENT_MAX_OUTPUT_TOKENS"),
    "maxOutputTokens key must be hidden on global",
  );
  assert.ok(
    keys.includes("MAJU_AGENT_MAX_CONTEXT_TOKENS"),
    "contextLimit key must be hidden on global",
  );
  assert.ok(
    keys.includes("MAJU_AGENT_MAX_ROUNDS"),
    "maxRounds key must be hidden on global",
  );
});

test("structuredEnvKeys_per_agent_maju_agent_includes_effort_and_numeric_keys", () => {
  // Per-agent maju-agent renders effort + all 3 numeric descriptors.
  const majuAgentModel = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("maju-agent", {
      thinkingEnvVar: "MAJU_AGENT_THINKING_EFFORT",
  ...
  assert.ok(keys.includes("MAJU_AGENT_THINKING_EFFORT"), "effort key present");
  assert.ok(keys.includes("MAJU_AGENT_MAX_OUTPUT_TOKENS"), "maxTokens present");
  assert.ok(
    keys.includes("MAJU_AGENT_MAX_CONTEXT_TOKENS"),
    "contextLimit present",
  );
  assert.ok(keys.includes("MAJU_AGENT_MAX_ROUNDS"), "maxRounds present");
});
...
test("structuredEnvKeys_per_agent_goose_excludes_effort_key_discriminating_invariant", () => {
  ...
  // (MAJU_AGENT_THINKING_EFFORT) must NOT appear in the output — any saved
...
test("deriveNumericDescriptors_matches_deriveAgentConfigFieldModel_numeric_subset", () => {
  ...
  const runtimeEntry = runtime("maju-agent", {
    maxTokensEnvVar: "MAJU_AGENT_MAX_OUTPUT_TOKENS",
    contextLimitEnvVar: "MAJU_AGENT_MAX_CONTEXT_TOKENS",
    maxRoundsEnvVar: "MAJU_AGENT_MAX_ROUNDS",
  });
...
//
// max output tokens and context limit must have min=1 (maju-agent rejects 0).
...
// to useEffortAutoClear with maju-agent provider/model vocab → "off" not in
...
test("Goose_optionSource_is_harnessNative_not_legacyProviderModelCatalog", () => {
  ...
  // maju-agent provider/model catalog, deleting valid Goose values on mount.
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("goose", { thinkingEnvVar: "GOOSE_THINKING_EFFORT" }),
    scope: "global",
  ...
  assert.equal(
    ...
    "harnessNative",
    'Goose global optionSource must be "harnessNative" — "legacyProviderModelCatalog" routes to maju-agent vocab and deletes valid `off` on mount',
  );
});
...
test("Goose_global_off_value_is_preserved_by_harnessNative_optionSource", () => {
  ...
  // auto-clear hook should be a no-op (no maju-agent vocab gate).
  const savedConfig = {
    ...config,
  ...
  assert.equal(
    effortField.value,
    "off",
    "saved GOOSE_THINKING_EFFORT=off must survive round-trip through field model (not deleted by maju-agent vocab check)",
  );
});
...
test("Goose_onboarding_optionSource_is_harnessNative", () => {
  ...
  // at both global and onboarding, so both must guard against maju-agent vocab.
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("goose", { thinkingEnvVar: "GOOSE_THINKING_EFFORT" }),
    scope: "onboarding",
...
test("maju_agent_optionSource_unchanged_still_majuAgentCatalog", () => {
  // Ensure the fix did not accidentally change maju-agent's optionSource.
  // maju-agent's effort MUST go through the provider/model catalog for the
  ...
  assert.equal(
    field(model, "effort").optionSource,
    "majuAgentCatalog",
    "maju-agent optionSource must remain majuAgentCatalog",
  );
});
