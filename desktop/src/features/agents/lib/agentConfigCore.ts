...
import { MAJU_AGENT_THINKING_EFFORT } from "../ui/majuAgentConfig";
...
export function deriveAgentConfigFieldModel({
  ...
  if (runtime?.thinkingEnvVar) {
    ...
    //     projection ignores. For maju-agent this IS MAJU_AGENT_THINKING_EFFORT.
    //   - definition/instance: still the generic legacy MAJU_AGENT_THINKING_EFFORT
    ...
    fields.push({
      kind: "effort",
      optionSource:
        runtime.id === "maju-agent" ? "majuAgentCatalog" : "harnessNative",
      currentPersistence: {
        kind: "envVar",
        key: persistenceKey,
      },
 ...
 * - Per-agent maju-agent: effort key + 3 numeric keys
 * - Per-agent Goose: 2 numeric keys only — Goose effort (MAJU_AGENT_THINKING_EFFORT)
