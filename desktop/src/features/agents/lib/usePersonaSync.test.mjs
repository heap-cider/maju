...
// Upstream fixtures describe relay payloads; attach their actual transport
// origin at the same boundary Maju uses. Tests for a mismatched socket replace
// these seams explicitly and still exercise the production rejection path.
beforeEach(() => {
  mock.method(relayClient, "fetchEventsWithOrigin", async (...args) => ({
    events: await relayClient.fetchEvents(...args),
    relayUrl: "wss://relay.example",
  }));
  mock.method(relayClient, "subscribeLiveWithOrigin", (filter, listener) =>
    relayClient.subscribeLive(filter, (event) =>
      listener(event, "wss://relay.example"),
    ),
  );
});
