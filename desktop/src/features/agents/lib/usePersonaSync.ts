...
// `max_limit` (1000; `crates/maju-db` `DEFAULT_MAX_PAGE_LIMIT`), so a single
...
class PersonaHistoryScopeChangedError extends Error {
  constructor() {
    super("owner history arrived from a different community");
    this.name = "PersonaHistoryScopeChangedError";
  }
}
...
async function fetchOwnerHistoryToExhaustion(
  ...
  for (;;) {
    const received = await relayClient.fetchEventsWithOrigin({
      kinds: PERSONA_SYNC_KINDS,
      authors: [pubkey],
      limit: PERSONA_HISTORY_PAGE_LIMIT,
      ...(until === undefined ? {} : { until }),
    });
    if (!sameRelayScope(received.relayUrl, relayUrl)) {
      throw new PersonaHistoryScopeChangedError();
    }
    const page = received.events;
...
export function startPersonaSync(
  ...
  const reconcile = (event: RelayEvent, arrivalRelayUrl = relayUrl) => {
  ...
  const runBackfill = async () => {
    for (let attempt = 0; attempt < BACKFILL_MAX_ATTEMPTS; attempt += 1) {
      try {
        const events = await fetchOwnerHistoryToExhaustion(pubkey, relayUrl);
        ...
        const transient =
          ...
          !(error instanceof PersonaHistoryScopeChangedError);
  ...
  void relayClient
    .subscribeLiveWithOrigin(
      ...
      (event, arrivalRelayUrl) => {
        if (sameRelayScope(relayUrl, arrivalRelayUrl)) onLiveEvent(event);
      },
    )
    .then((dispose) => {
      if (onCancelled()) {
