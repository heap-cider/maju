...
14. **Native ACP selections are Save-gated definition data.** The model,
    reasoning, and advanced controls use the session-advertised configOptions.
    Keep their scalar values in MAJU_ACP_CONFIG_OPTIONS and save the complete
    draft through the normal definition/instance update boundary. Cancel must
    not persist a selection. Refresh dependent options after changing models,
    and do not add a second device-local effort column beside that map.
