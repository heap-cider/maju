...
export function EffortSelectField({
  ...
  // maju-agent master list. This ensures runtime-native values like Goose's
  // "off" appear as options even though they are not in the maju-agent list.
  ...
  // values while keeping maju-agent unavailable options visible when the
  ...
  const allValues = [
    ...
    ...MAJU_AGENT_THINKING_EFFORT_VALUES.filter((v) => !effortValidSet.has(v)),
  ];
 ...
 *
 * - `maxOutputTokens` / `contextLimit`: minimum 1 — the maju-agent runtime
 *   rejects 0 for these fields (crates/maju-agent/src/config.rs:921-928).
