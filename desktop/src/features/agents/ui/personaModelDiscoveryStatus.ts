...
function isEmptySharedComputeError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("no maju shared compute serving members") ||
    normalized.includes("no live maju shared compute models") ||
...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "Maju is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of Maju cannot use shared compute. Update Maju or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "Maju received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "Maju couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }

  // Spec-reserved auth error text (agent-client-protocol ErrorCode::AuthRequired),
  // surfaced verbatim through maju-acp's stderr — generic across conformant
  ...
  if (
    ...
    message.includes("MAJU_AGENT_PROVIDER is required")
  ) {
    return null;
  }
...
function formatDatabricksAuthStatus(
  ...
  if (message.includes("Databricks sign-in is required")) {
    return {
      message:
        "Databricks sign-in is required. Open the model picker to sign in, or run `maju-agent auth databricks` in a terminal.",
      tone: "muted",
    };
  }
  ...
  if (
    ...
    return {
      message:
        "Databricks sign-in didn't complete. Open the model picker to retry, or run `maju-agent auth databricks` in a terminal.",
      tone: "warning",
    };
  }
