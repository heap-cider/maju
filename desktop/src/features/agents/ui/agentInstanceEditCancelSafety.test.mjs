/**
 * Exercise the real dialog: native ACP selections remain drafts until Save.
 * Cancel writes nothing, and access plus native settings share one locked IPC
 * update. All dismissal paths remain blocked until that save has settled.
 */
...
function installEffortIpc({ deferUpdate = false, failUpdate = false } = {}) {
  ...
  set("discover_agent_models", ({ input }) =>
    Promise.resolve({
      agentName: input.agentCommand,
      agentVersion: "1.0",
      models: [{ id: "test-model", name: "Test model" }],
      agentDefaultModel: "test-model",
      selectedModel: input.model ?? null,
      supportsSwitching: true,
      configOptions:
        input.agentCommand === "goose"
          ? [
              {
...
async function selectEffort(label) {
  const trigger = await screen.findByTestId("acp-thought-level-select");
  assert.ok(
    trigger,
...
function savedNativeEffort(call) {
  const raw = call.args.input?.envVars?.MAJU_ACP_CONFIG_OPTIONS;
  return raw ? JSON.parse(raw).reasoning_effort : undefined;
}

function effortCalls() {
  return ipcCalls.filter(
    (call) =>
      call.cmd === "update_managed_agent" &&
      savedNativeEffort(call) !== undefined,
  );
}
...
test("effort Save includes the native option map inside update_managed_agent payload", async () => {
  ...
  assert.equal(
    savedNativeEffort({ args: updateArgs }),
    "high",
    "the native option must be in the locked update payload",
  );
  ...
  assert.equal(
    Object.hasOwn(updateArgs.input, "effortLevel"),
    false,
    "no independent effort column may be sent",
  );
...
test("access and effort changed together both appear in the locked update_managed_agent call", async () => {
  ...
  assert.equal(
    savedNativeEffort(updates[0]),
    "high",
    "the native option must be in the same locked update as the access change",
  );
});
...
test("runtime switch clears touched effort — no effortLevel dispatched after switching runtimes", async () => {
  ...
  assert.equal(
    dom.window.document.querySelector(
      '[data-testid="acp-thought-level-select"]',
    ),
    null,
