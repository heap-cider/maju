...
// env vars (e.g. MAJU_AGENT_MAX_ROUNDS) that are owned by first-class controls
...
test("buildRecord_hidden_tuning_key_unchanged_when_generic_row_edited", () => {
  // Structured field set MAJU_AGENT_MAX_ROUNDS to "50"; it lives in value
  ...
  const value = { MAJU_AGENT_MAX_ROUNDS: "50", MY_VAR: "old" };
...
test("buildRecord_runtime_switch_new_hiddenKeys_then_generic_edit", () => {
  ...
  //
  // Before switch: agent is maju-agent with MAJU_AGENT_MAX_ROUNDS = "50" stored
  // in value (set via the numeric tuning control). After switching to Goose,
  // the maju-agent key is no longer hidden — it becomes a visible generic row.
  // The test verifies:
  //   (a) After the switch, the old maju-agent key appears as a generic row
  ...
  const majuAgentRuntime = {
    id: "maju-agent",
    label: "Maju Agent",
    avatarUrl: "",
    availability: "available",
    command: "maju-agent",
    binaryPath: "maju-agent",
    defaultArgs: [],
    mcpCommand: null,
    ...
    contextLimitEnvVar: "MAJU_AGENT_CONTEXT_LIMIT",
  ...
  const majuDescriptors = deriveNumericDescriptors(majuAgentRuntime);
  const gooseDescriptors = deriveNumericDescriptors(gooseRuntime);
  const majuHiddenKeys = structuredEnvKeys(majuDescriptors);
  const gooseHiddenKeys = structuredEnvKeys(gooseDescriptors);

  // Sanity-check that MAJU_AGENT_MAX_ROUNDS is hidden under maju-agent but not
  // under Goose — that contrast is what makes it become a generic row.
  assert.ok(
    majuHiddenKeys.includes("MAJU_AGENT_MAX_ROUNDS"),
    "MAJU_AGENT_MAX_ROUNDS must be hidden under maju-agent descriptors",
  );
  assert.equal(
    gooseHiddenKeys.includes("MAJU_AGENT_MAX_ROUNDS"),
    false,
    "MAJU_AGENT_MAX_ROUNDS must not be hidden under Goose descriptors",
  );

  // Pre-switch value: maju-agent max-rounds was set, GOOSE_MAX_TOKENS was
  ...
  const valueBeforeSwitch = {
    MAJU_AGENT_MAX_ROUNDS: "50",
    GOOSE_MAX_TOKENS: "16384",
    USER_VAR: "original",
  };

  // After the switch to Goose, toRows is reproj with the new (Goose) hidden
  // set. MAJU_AGENT_MAX_ROUNDS is no longer hidden → appears as a generic row.
  ...
  assert.ok(
    rowsAfterSwitch.some((r) => r.key === "MAJU_AGENT_MAX_ROUNDS"),
...
test("filterBakedGenericRows_numeric_baked_key_excluded_and_placeholder_shown", () => {
  ...
  //
  // The global baked env contains MAJU_AGENT_MAX_OUTPUT_TOKENS = "4096"
  ...
  const numericDescriptors = deriveNumericDescriptors(majuAgentRuntime);
  const numericStructuredKeys = structuredEnvKeys(numericDescriptors);

  assert.ok(
    numericStructuredKeys.includes("MAJU_AGENT_MAX_OUTPUT_TOKENS"),
    "numeric key must appear in structured keys via production helpers",
  );

  // Simulate the baked env: MAJU_AGENT_MAX_OUTPUT_TOKENS is baked, plus a
  // non-structured baked var.
  const bakedEnv = [
    { key: "MAJU_AGENT_MAX_OUTPUT_TOKENS", value: "4096" },
  ...
  assert.equal(
    genericRows.some((r) => r.key === "MAJU_AGENT_MAX_OUTPUT_TOKENS"),
    false,
...
test("buildRecord_clearing_structured_field_allows_placeholder_to_return", () => {
  ...
  //
  // Step 1: value has MAJU_AGENT_MAX_ROUNDS = "50" (user set it via the
  //         structured field). MAJU_AGENT_MAX_ROUNDS is in hiddenKeys.
  ...
  // After the clear, value no longer contains MAJU_AGENT_MAX_ROUNDS.
  ...
  assert.equal(
    "MAJU_AGENT_MAX_ROUNDS" in record,
    false,
