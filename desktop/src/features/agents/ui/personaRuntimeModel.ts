 ...
 * persona runtime string (e.g. `maju-agent`). A catalog lookup then returns
 ...
 *
 * - maju-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * maju-agent/Anthropic). Persisting the local (empty) provider + credential-
 ...
 *
 * MODEL follows the same transition rule. maju-agent/goose readiness requires a
