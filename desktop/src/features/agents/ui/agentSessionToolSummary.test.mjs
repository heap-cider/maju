...
test("buildCompactToolSummary formats Maju send_message preview", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "send_message",
      majuToolName: "send_message",
      title: "Send Message",
...
test("buildCompactToolSummary treats maju messages send commands as messages", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "maju-dev-mcp__shell",
      args: {
        command:
          'maju --format compact messages send --channel channel-1 --content "@Ned are you working"',
      },
    }),
  );
...
test("buildCompactToolSummary returns null preview for piped stdin sends", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      ...
      args: {
        command:
          'echo "hello from stdin" | ./target/release/maju messages send --channel channel-1 --content -',
      },
    }),
  );
...
test("buildCompactToolSummary formats view_image thumbnail source", () => {
  ...
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "maju-dev-mcp__view_image",
      args: { source },
    }),
  );
...
test("buildCompactToolSummary promotes non-send maju CLI commands to relay ops", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: {
        command: "maju channels get --channel channel-1",
      },
    }),
  );
  ...
  assert.equal(summary.presentation, "inline");
  assert.equal(summary.shellContent, "maju channels get --channel channel-1");
});

test("buildCompactToolSummary exposes shellContent for shell-sourced maju CLI reads", () => {
  const command =
    "sleep 45; maju messages thread --channel channel-uuid --event abc | tail -n 20";
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: { command },
...
test("buildCompactToolSummary derives structured actions for native Maju MCP tools", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "get_channel",
      majuToolName: "get_channel",
      args: {
