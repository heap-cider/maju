...
export function selectionOnRuntimeChange(
  ...
  if (params.previousRuntime !== params.nextRuntime) {
    ...
    if (envVars[MAJU_ACP_CONFIG_OPTIONS] !== undefined) {
      // Preserve an explicit clear for account-device definition sync. Native
      // option ids and values belong to the previously selected engine.
      envVars = { ...envVars, [MAJU_ACP_CONFIG_OPTIONS]: "{}" };
    }
