...
test("localMode_majuAgent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("maju-agent"),
    true,
    "maju-agent must support LLM provider selection",
  );
});
...
test("localMode_majuAgent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects maju-agent but leaves provider empty.
  // Rust readiness requires MAJU_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_majuAgent_emptyModel_notSatisfied", () => {
  // Scenario: maju-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires MAJU_AGENT_MODEL — empty = NotReady.
...
// ── Gate: maju-agent / anthropic with missing key → markers shown ─────────

test("localMode_majuAgent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects maju-agent/anthropic + fills model, but hasn't
...
test("localMode_majuAgent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("maju-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for maju-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("maju-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Maju env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("maju-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("maju-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("maju-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("maju-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("maju-agent", "relay-mesh"),
    "Use agent defaults (Maju shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    ...
    {
      key: "MAJU_AGENT_MODEL",
      value: "goose-claude-opus-4-8",
      masked: false,
    },
    { key: "MAJU_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "MAJU_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "MAJU_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "maju-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  const bakedEnv = [
    { key: "MAJU_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    { key: "MAJU_AGENT_MODEL", value: "build-model", masked: false },
    { key: "MAJU_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];

  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "MAJU_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "MAJU_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "MAJU_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("maju-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("maju-agent", "");
...
// include ANTHROPIC_API_KEY for explicit maju-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("maju-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "maju-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("maju-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("maju-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("maju-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "maju-agent + databricks must list DATABRICKS_HOST as required",
  );
