 ...
 * so AgentConfigFields used maju-agent vocab (no "off"), clearing the value.
 ...
 *     reverts effortValidForRenderer to maju-agent vocab (no "off") → the custom
...
test("AgentConfigFields (useCustomSelect): Goose off renders 'Off' in custom trigger (mount)", async () => {
  ...
  // uses maju-agent vocab (no "off"); the custom trigger shows the inherit
