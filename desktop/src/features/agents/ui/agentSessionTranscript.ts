...
function upsertTool(
  ...
  const existing = d.itemsById.get(id);
  const canonicalMajuToolName =
    majuToolName ?? findMajuToolName(toolName, true);
  if (existing?.type === "tool") {
    const updatedTitle = !isGenericToolTitle(title) ? title : existing.title;
    let updatedToolName = existing.toolName;
    let updatedMajuToolName = existing.majuToolName;
    if (canonicalMajuToolName) {
      updatedMajuToolName = canonicalMajuToolName;
      updatedToolName = canonicalMajuToolName;
    } else if (!existing.majuToolName && !isGenericToolTitle(toolName)) {
    ...
    const descriptor = classifyTool({
      ...
      majuToolName: updatedMajuToolName,
      args: updatedArgs,
  ...
  const resolvedToolName = canonicalMajuToolName ?? toolName;
  const descriptor = classifyTool({
    title,
