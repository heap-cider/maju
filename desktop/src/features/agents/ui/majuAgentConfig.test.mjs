...
import {
  ...
  MAJU_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isMajuAgentRuntime,
} from "./majuAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("MAJU_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...MAJU_AGENT_THINKING_EFFORT_VALUES],
...
test("MAJU_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(MAJU_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, MAJU_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected MAJU_AGENT_* names", () => {
  assert.equal(MAJU_AGENT_THINKING_EFFORT, "MAJU_AGENT_THINKING_EFFORT");
  assert.equal(MAJU_AGENT_MAX_OUTPUT_TOKENS, "MAJU_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(MAJU_AGENT_MAX_CONTEXT_TOKENS, "MAJU_AGENT_MAX_CONTEXT_TOKENS");
  assert.equal(MAJU_AGENT_MAX_ROUNDS, "MAJU_AGENT_MAX_ROUNDS");
});
...
test("isMajuAgentRuntime returns true only for maju-agent id", () => {
  assert.equal(isMajuAgentRuntime("maju-agent"), true);
});

test("isMajuAgentRuntime returns false for other runtimes", () => {
  assert.equal(isMajuAgentRuntime("goose"), false);
  assert.equal(isMajuAgentRuntime("custom"), false);
  assert.equal(isMajuAgentRuntime(""), false);
  assert.equal(isMajuAgentRuntime("maju-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, MAJU_AGENT_THINKING_EFFORT, "high");
  assert.equal(result[MAJU_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [MAJU_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, MAJU_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, MAJU_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact MAJU_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  ...
  assert.equal(result[MAJU_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [MAJU_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, MAJU_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, MAJU_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact MAJU_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    MAJU_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[MAJU_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [MAJU_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, MAJU_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, MAJU_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact MAJU_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, MAJU_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[MAJU_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [MAJU_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, MAJU_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, MAJU_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [MAJU_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, MAJU_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[MAJU_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[MAJU_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      MAJU_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(MAJU_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[MAJU_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isMajuAgent` derivation in CreateAgentRuntimeFields.
...
test("isMajuAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves maju-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is maju-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "maju-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "maju-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isMajuAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isMajuAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'maju-agent' MUST trigger model-tuning fields",
  );
});

test("isMajuAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned maju-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "maju-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "maju-agent";

  assert.equal(isMajuAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isMajuAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isMajuAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-maju-agent", () => {
  // E.g. user switches from maju-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isMajuAgentRuntime(prospectiveRuntimeId), false);
});

test("isMajuAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in MajuAgentModelTuningFields and AgentDefaultsSettingsCard
