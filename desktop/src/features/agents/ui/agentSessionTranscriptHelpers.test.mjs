...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Maju event", there is no event content to surface, so
  // userText is empty and the title falls through to "Maju event".
  ...
  assert.equal(result.userTitle, "Maju event");
...
test("parsePromptText yields a null pubkey when From has no hex", () => {
  const text = ["[Maju event: note]", "From: Someone", "Content: hi"].join(
    "\n",
  );
...
test("parsePromptText defaults the title to 'Maju event' when no kind is present", () => {
  const text = ["[Maju event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "Maju event");
});
...
test("parsePromptText splits a tagged standing prefix from the dynamic turn", () => {
  ...
  assert.deepEqual(
    ...
    ["Base", "Agent Instructions", "Context", "Maju event: @mention"],
  );
});
...
test("extractToolIdentity ignores Maju tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit Maju tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections preserves a Windows workspace path", () => {
  ...
  assert.deepEqual(sections, [
    ...
    {
      title: "Workspace",
      body: "Current working directory: C:\\Users\\me\\maju",
    },
    { title: "Agent Instructions", body: "persona text" },
  ]);
});
...
test("parseSystemPromptSections pins the full Base+System+Core+Canvas harness shape", () => {
  ...
  const framed = [
    ...
    "Fetch current content with: maju canvas get --channel 94a444a4-c0a3-5966-ab05-530c6ddc2301",
  ].join("\n");
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are an assistant." },
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in Maju.",
    "",
    "[System]",
  ...
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are a helpful AI assistant running in Maju." },
    {
      title: "System",
