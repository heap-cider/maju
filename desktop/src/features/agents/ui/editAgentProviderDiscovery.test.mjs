...
test("editAgent_providerFieldVisible_forMajuAgent", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("maju-agent"),
    true,
    "maju-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "maju-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "maju-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
//
// Switching to maju-agent runtime (which supports LLM provider selection)
// must make the provider field visible, enabling live discovery.

test("editAgent_runtimeSwitch_toMajuAgentEnablesProvider", () => {
  // Simulate: user switches from "claude" to "maju-agent"
  const previousRuntime = "claude";
  const nextRuntime = "maju-agent";
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "maju-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/maju-agent";
  const catalog = [
    { id: "maju-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "maju-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "maju-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives maju-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "maju-agent";
  const catalogRuntime = {
    id: "maju-agent",
    command: "/usr/local/bin/maju-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "maju-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "maju-agent", command: null, defaultArgs: [] };
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting maju-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the maju-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/maju-agent";
  ...
  const selectedRuntimeId = "maju-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks maju-agent (inheritHarness→false, selectedRuntimeId='maju-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'maju-agent')
  ...
  const runtimes = [
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
    { id: "maju-agent", command: "/usr/local/bin/maju-agent", defaultArgs: [] },
  ];
  const selectedRuntimeId = "maju-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was maju-agent

  // llmProviderFieldVisible is driven by the live dropdown (maju-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows maju-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to maju-agent
  ...
  const selectedRuntimeId = "maju-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is maju-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedMajuAgentProvider_preservedOnNameOnlySave", () => {
  // Inherited maju-agent persona with databricks_v2 snapshot.
  // User makes a name-only edit (never touches runtime or provider).
  // The catalog-arrival effect correctly derived selectedRuntimeId="maju-agent".

  const inheritHarness = true; // agentCommandOverride == null → inheriting
  const agentCommand = "/usr/local/bin/maju-agent"; // inherited maju-agent command
  const runtimes = [
    { id: "maju-agent", command: "/usr/local/bin/maju-agent", defaultArgs: [] },
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
  ];
  const selectedRuntimeId = "maju-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows maju-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited maju-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → maju-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited maju-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited maju-agent must omit provider (not send null to clear it)",
  );
});

test("editAgent_inheritedMajuAgentProvider_clearsWhenUserSwitchesToInheritedClaude", () => {
  // An agent inheriting maju-agent with databricks_v2, but the persona was
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited maju-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "maju-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but maju-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "maju-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "maju-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null maju-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited maju-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "maju-agent", command: "maju-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "maju-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capableMajuAgentLoadedCatalog_preservedOnNoOpSave", () => {
  // Confirm loaded-catalog inherited maju-agent still preserves provider.
  // This is Finding D's good path — must not regress with the tri-state change.

  const inheritHarness = true;
  const agentCommand = "maju-agent";
  const runtimes = [
    { id: "maju-agent", command: "maju-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  ...
  assert.equal(capability, "capable", "loaded maju-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited maju-agent (loaded catalog) must omit provider",
  );
});
...
//
// Problem: maju-agent stores agentCommand="maju-agent" (short name) while the
// catalog entry has command="/Applications/Maju.app/.../maju-agent" (resolved path).
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "maju-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "maju-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "maju-agent",
      command: "/Applications/Maju.app/Contents/MacOS/maju-agent", // resolved path
  ...
  assert.equal(
    selectedRuntimeId,
    "maju-agent",
    "short-name agentCommand must resolve to maju-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/maju-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "maju-agent",
      command: "/usr/local/bin/maju-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "maju-agent" via id-fallback,
  ...
  const agentCommand = "maju-agent"; // short name
  const runtimes = [
    {
      id: "maju-agent",
      command: "/Applications/Maju.app/Contents/MacOS/maju-agent",
      availability: "available",
      defaultArgs: [],
    },
  ];
  ...
  assert.equal(
    selectedRuntimeId,
    "maju-agent",
    "id-fallback must resolve selectedRuntimeId to maju-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    "/Applications/Maju.app/Contents/MacOS/maju-agent",
    "discoveryAgentCommand must be the resolved path from the catalog entry",
  );
  assert.equal(
    canDiscoverModelOptions,
    true,
    "discovery gate must pass for inherited maju-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: maju-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("maju-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: maju-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("maju-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: maju-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("maju-agent", "databricks");
...
test("requiredCredentialEnvKeys: maju-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("maju-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: maju-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("maju-agent", "");
...
test("blockSave_majuAgentAnthropicMissingKey_blocked", () => {
  // Will's exact case: maju-agent / anthropic / opus / no ANTHROPIC_API_KEY
  const requiredKeys = requiredCredentialEnvKeys("maju-agent", "anthropic");
...
test("blockSave_majuAgentAnthropicKeyProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("maju-agent", "anthropic");
...
test("blockSave_majuAgentAnthropicEmptyStringKey_blocked", () => {
...
test("blockSave_majuAgentDatabricksMissingHost_blocked", () => {
  const requiredKeys = requiredCredentialEnvKeys("maju-agent", "databricks");
  const envVars = {};
  assert.equal(
    hasRequiredEnvKeyMissing(requiredKeys, envVars),
    true,
...
test("blockSave_majuAgentDatabricksHostProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("maju-agent", "databricks");
...
//   FALSE-ALLOW: claude pin → inherit maju-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is maju-agent/anthropic, key absent)
//   FALSE-BLOCK: maju-agent pin → inherit claude persona
//     → must NOT be blocked (claude has no dialog-fixable credential requirement)

test("blockSave_inheritTransition_claudePin_toMajuAgentPersona_missingKey_blocked", () => {
  ...
  // from persona" where the persona uses maju-agent/anthropic.
  // prospectiveRuntimeId resolves to "maju-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (maju-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "maju-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (maju-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting maju-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});

test("blockSave_inheritTransition_majuAgentPin_toClaudePersona_notBlocked", () => {
  // Scenario: agent is pinned to maju-agent/anthropic. The user checks
