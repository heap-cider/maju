...
before(async () => {
  ...
  dom.window.localStorage.setItem("maju-follow-system", "false");
...
test("first-open splash waits for the first terminal frame", async () => {
  ...
  assert.equal(
    view.container.querySelector(".maju-terminal-welcome"),
    null,
  ...
  await waitFor(() =>
    assert.ok(view.container.querySelector(".maju-terminal-welcome")),
  );
  view.unmount();
});
...
test("opening a tab keeps terminal ownership while its attachment is pending", async () => {
  ...
  attachResolver = () => {};
  fireEvent.click(view.getByLabelText("New Maju Term tab"));
  await waitFor(() => assert.equal(typeof attachResolver, "function"));
  assert.equal(
    substrate.dataset.terminalOwner,
    "terminal",
    "an attaching session must not force the substrate back to Maju",
  );
...
// accumulates pixels into whole cells and `maju-terminal` already proves which
