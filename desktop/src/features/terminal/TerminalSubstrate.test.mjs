...
function gridDraws(view) {
  const grid = view.container.querySelector(
    ".maju-terminal-viewport > canvas:not(.maju-terminal-welcome)",
  );
...
function fixture(overrides = {}) {
  ...
  const renderTree = (nextProps) =>
    createElement(
      ...
      null,
      createElement("div", {
        className: "maju-huddle-app-surface",
        tabIndex: -1,
      }),
...
async function ready(view) {
  await waitFor(() =>
    assert.ok(view.container.querySelector(".maju-terminal-substrate")),
  );
}
...
test("mounted IME paths neither toggle nor emit preedit text", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".maju-terminal-substrate");
  toggleChord(true);
  assert.equal(substrate.dataset.terminalOwner, "maju");
...
test("tab actions restore terminal input focus", async () => {
  ...
  const actions = [
    ...
    ["new", view.getByLabelText("New Maju Term tab")],
  ];
...
test("drag resize batches visual updates and commits state only on release", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".maju-terminal-substrate");
  const handle = view.getByLabelText("Resize Maju Term");
  ...
  assert.equal(substrate.dataset.terminalResizing, "true");
  assert.equal(window.localStorage.getItem("maju-terminal-dock-height"), null);
  ...
  assert.equal(window.localStorage.getItem("maju-terminal-dock-height"), "380");
});
...
test("unmount cancels a queued drag update", async () => {
  ...
  await ready(view);
  const handle = view.getByLabelText("Resize Maju Term");
  const previousHeight = handle.closest(".maju-terminal-substrate").style
    .height;
  ...
  assert.equal(window.localStorage.getItem("maju-terminal-dock-height"), null);
  assert.equal(
    handle.closest(".maju-terminal-substrate").style.height,
    previousHeight,
  );
});
...
async function expectWelcome(view, present) {
  ...
  await waitFor(() =>
    assert.equal(
      view.container.querySelector(".maju-terminal-welcome") !== null,
      present,
    ),
  );
}
...
test("canvas failure atomically restores Maju ownership", async () => {
  ...
  await waitFor(() => assert.equal(substrate.dataset.terminalOwner, "maju"));
  toggleChord();
  await waitFor(() =>
    assert.equal(substrate.dataset.terminalOwner, "terminal"),
  );
...
test("cursor blink runs only while the terminal owns input and resets on input", async () => {
  ...
  try {
    ...
    await ready(subject.view);
    assert.equal(callbacks.size, 0, "Maju ownership must pause blinking");
...
test("reduced motion keeps the terminal cursor solid", async () => {
  ...
  try {
    ...
    await waitFor(() =>
      assert.equal(
        subject.view.container.querySelector(".maju-terminal-substrate").dataset
          .terminalOwner,
        "terminal",
      ),
    );
...
test("tab chords stay inert while Maju owns input", async () => {
  const subject = tabFixture();
  await ready(subject.view);
  // Deliberately not revealed: owner is "maju".
  const spawn = press({ code: "KeyT", metaKey: true });
  ...
  assert.equal(
    ...
    false,
    "Maju-mode ⌘T belongs to the rest of the app",
  );
});
