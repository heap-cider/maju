...
export function TerminalSubstrate({
  ...
  const [owner, setOwner] = React.useState<"maju" | "terminal">("maju");
  ...
  const [dockHeight, setDockHeight] = React.useState(() => {
    const stored = Number.parseInt(
      window.localStorage.getItem("maju-terminal-dock-height") ?? "",
      10,
    );
  ...
  const terminalStyle = terminalPalette
    ? ({
        "--maju-terminal-background": terminalPalette.background,
        "--maju-terminal-foreground": terminalPalette.foreground,
      } as React.CSSProperties)
    : undefined;

  const forceMajuFallback = React.useEffectEvent(() => {
   ...
   * ownership: in Maju mode these keys belong to the rest of the app.
   */
  ...
  React.useLayoutEffect(() => {
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      ...
      if (onToggle) onToggle();
        ...
        setOwner((current) => {
          const next = current === "terminal" ? "maju" : "terminal";
          if (next === "terminal") {
  ...
  return (
    <section
      aria-label="Maju Term"
      className="maju-terminal-substrate"
    ...
    >
      ...
      <div className="maju-terminal-contract-bar">
        <div className="maju-terminal-tabs" role="tablist">
          {sessions.map((session, index) => (
            <div
              className={cn(
                "maju-terminal-tab",
                session.active && "maju-terminal-tab-active",
              )}
              key={session.id}
              role="presentation"
            >
              ...
              <button
                ...
                className="maju-terminal-tab-select"
                disabled={session.closing}
                onClick={() => runTabAction(() => onSelectSession(session.id))}
                role="tab"
                type="button"
              >
                <span className="maju-terminal-designator maju-terminal-tab-title">
                  {session.title === "SHELL" ? (
                    <>
                      <ChevronRight />
                      <span>{index + 1}</span>
                ...
                {session.closing ? (
                  <span className="maju-terminal-tab-title">Closing…</span>
                ) : null}
              </button>
            </div>
          ))}
          <button
            aria-label="New Maju Term tab"
            className="maju-terminal-new-tab"
        ...
        <div className="maju-terminal-readout">
          <button
            aria-label={
              mode === "maximized" ? "Restore Maju Term" : "Maximize Maju Term"
            }
            className="maju-terminal-window-action"
            onClick={() =>
      ...
      <div className="maju-terminal-viewport px-5 pt-2">
        <canvas ref={canvasRef} />
        <div
          aria-hidden="true"
          className="maju-terminal-selection-layer"
          onCopy={(event) => {
        ...
        {welcomeVisible && banner ? (
          <canvas className="maju-terminal-welcome" ref={bannerCanvasRef} />
        ) : null}
        <textarea
          aria-label="Terminal input"
          autoCapitalize="off"
          autoComplete="off"
          className="maju-terminal-input"
          onCompositionEnd={() => {
      ...
      <div aria-live="polite" className="sr-only">
        {owner === "terminal" ? "Maju Term mode" : "Maju mode"}
      </div>
    </section>
  );
}
