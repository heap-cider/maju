...
function resetDevWebviewStateFromUrl() {
  ...
  // WebKit groups every Maju binary under one disk directory, but storage is
...
function configureDevE2eBridgeFromUrl() {
  ...
  e2eWindow.__MAJU_E2E__ ??= { mode: "mock" };
  ...
  window.localStorage.setItem("maju-active-community-id", E2E_COMMUNITY_ID);
    ...
    <React.StrictMode>
      <CommunitiesProvider>
        <CommunityOnboardingProvider enabled={huddleWindowChannelId() === null}>
          <ThemeProvider defaultTheme="maju">
...
async function installE2eBridgeIfConfigured() {
  ...
  if (
    ...
    !(window as E2eWindow).__MAJU_E2E__
  ) {
    return;
  }
