...
function AnimatedCountSlot({
  ...
  if (direction === 0 || previous === current) {
    return (
      <span
        className={cn(
          "maju-animated-count__slot",
          isDigit && "maju-animated-count__slot--digit",
        )}
      >
  ...
  if (!isDigit) {
    return (
      <span
        className="maju-animated-count__symbol"
  ...
  return (
    <span
      className="maju-animated-count__slot maju-animated-count__slot--digit"
      data-direction={direction > 0 ? "up" : "down"}
    >
      <span
        className="maju-animated-count__reel"
        key={`${version}-${previous}-${current}`}
      >
        {direction > 0 ? (
          <>
...
export function AnimatedCount({ className, value }: AnimatedCountProps) {
  ...
  return (
    <span className={cn("maju-animated-count", className)}>
      <span className="sr-only">{transition.current}</span>
      <span aria-hidden className="maju-animated-count__motion">
        {slots.map((slot) => (
          <AnimatedCountSlot
      ...
      <span aria-hidden className="maju-animated-count__static">
        {transition.current}
      </span>
    </span>
  );
