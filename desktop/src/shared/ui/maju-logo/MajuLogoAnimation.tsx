...
import "./maju-logo-animation.css";
...
export type MajuLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function MajuLogoAnimation({
  ariaLabel = "Maju logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `maju-logo-cutouts-${idSuffix}`;
  const textureId = `maju-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "maju-logo",
    fullScreen && "maju-logo--screen",
    !fullScreen && "maju-logo--compact",
    showBackground && "maju-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
