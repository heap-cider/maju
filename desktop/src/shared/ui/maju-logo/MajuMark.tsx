...
/**
 * The finished Maju mark as a plain static SVG — no SMIL, no scripting, no
 * animation machinery. Geometry matches the final keyframe of the
 * MajuLogoAnimation morph (v8 variant), rendered in `currentColor`, so it
 * paints complete on the very first frame regardless of animation support.
 */
export function MajuMark({ className }: { className?: string }) {
  const maskId = `maju-mark-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;

  return (
    <svg
      aria-hidden="true"
      className={["maju-mark", className].filter(Boolean).join(" ")}
      viewBox="0 0 466 309"
      fill="currentColor"
    >
      <defs>
