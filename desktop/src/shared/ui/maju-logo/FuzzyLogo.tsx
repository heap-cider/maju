...
import MajuLogoAnimation, {
  type MajuLogoAnimationProps,
} from "./MajuLogoAnimation";
...
export type FuzzyLogoProps = {
  ...
  pulse?: boolean;
  reverse?: boolean;
  variant?: MajuLogoAnimationProps["variant"];
};

/**
 * The fuzzy Maju mark. v8 ships a built-in animated texture (looping fractal-noise
...
export function FuzzyLogo({
  ...
  return (
    <MajuLogoAnimation
      ariaLabel={ariaLabel}
      className={cn(
        pulse && !fuzz && !hasRestWindow && "maju-logo--pulse",
        className,
      )}
      fullScreen={false}
      loop={loop}
