...
export const POOF_TRIGGER_CLASS = "maju-poof-trigger";
export const POOF_ORIGIN_CLASS = "maju-poof-origin";
export const POOF_POINTER_ORIGIN_CLASS = "maju-poof-pointer-origin";
...
export function PoofBurstProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <div aria-hidden="true" className="maju-poof-layer">
        {bursts.map((burst) => (
          <div
            className="maju-poof-burst"
            key={burst.id}
            style={
              {
                "--maju-poof-size": `${burst.size}px`,
                "--maju-poof-x": `${burst.x}px`,
                "--maju-poof-y": `${burst.y}px`,
              } as PoofStyle
            }
          >
            {POOF_FRAMES.map((frame, index) => (
              <img
                alt=""
                className={`maju-poof-frame maju-poof-frame-${index + 1}`}
                decoding="async"
                draggable={false}
