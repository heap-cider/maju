 ...
 *
 * `maju://pr|issue|repo` entity links (and relay clone URLs, which normalize
 * onto `maju://repo`) are the exception and are rendered recipient-side:
...
export function mergeMessageLinkPreviews(
  ...
  return candidates.flatMap((candidate) => {
    const preview = isMajuEntityPreview(candidate)
...
export function useMessageLinkPreviews({
  ...
  const entityLinkPreviews = React.useMemo(
    () => linkPreviewCandidates.filter(isMajuEntityPreview),
