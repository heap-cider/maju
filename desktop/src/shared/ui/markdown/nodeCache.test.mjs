...
test("customEmoji is keyed by value, not identity", () => {
  clearMarkdownNodeCache();
  const emoji = [{ shortcode: "maju", url: "https://relay/maju.png" }];
  ...
  const second = renderCachedMarkdown({
    ...BASE,
    content: "hi :maju:",
    customEmoji: [{ shortcode: "maju", url: "https://relay/maju.png" }],
  });
  assert.equal(first, second);
  ...
  const third = renderCachedMarkdown({
    ...BASE,
    content: "hi :maju:",
    customEmoji: [{ shortcode: "maju", url: "https://relay/other.png" }],
  });
