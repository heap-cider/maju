...
import { MajuInlineLink, MajuLinkChip } from "./MajuLinkChip";
...
function EntityMetadataTooltip({
  ...
  React.useEffect(() => {
    let cancelled = false;
    void loadMajuEntityMetadata(href).then((value) => {
...
/**
 * Navigate to the project detail view for a `maju://pr|issue|repo` link.
...
/**
 * In-app open handlers for `maju://` entity preview cards, keyed by href.
...
/**
 * Resolve an anchor href to a canonical `maju://` entity link, accepting
 * both the deep-link scheme directly and HTTPS relay clone URLs (which the
 * preview parser normalizes onto `maju://repo` only when the URL origin
...
/**
 * Render an inline anchor for a Maju entity link (`maju://pr|issue|repo` or
...
export function renderEntityLinkAnchor({
  ...
  const chip = (metadata?: LinkPreviewMetadata | null) => {
    ...
    return (
      <MajuLinkChip
        data-maju-link-kind={parsed.value.type}
        href={href}
