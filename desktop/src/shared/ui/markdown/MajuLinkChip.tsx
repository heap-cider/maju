...
function useMajuLinkContextMenu({
  href,
  interactive,
  onOpenLink,
}: {
  ...
  const contextMenu =
    position && href ? (
      <MediaContextMenu
        dataAttributes={["data-maju-link-context-menu"]}
        items={[
          {
            label: "Open link",
            onSelect: () => {
...
export function MajuLinkChip({
  ...
  const { contextMenu, onContextMenuCapture } = useMajuLinkContextMenu({
    href,
    interactive,
    onOpenLink,
  });
...
export function MajuInlineLink({
  children,
  href,
  interactive,
  onOpenLink,
