...
const ENTITY_HREF = `maju://pr?id=${EVENT_ID}&owner=${OWNER}&d=maju-world`;
...
test("relay-resolved Maju entities beat conflicting sender snapshots in content order", () => {
  ...
  const snapshots = parseLinkPreviewSnapshots(
    [
      snapshotTag(ENTITY_HREF, "Forged sender title", "Definitely Real Maju"),
