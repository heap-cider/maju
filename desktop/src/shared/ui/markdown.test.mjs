...
// which would break copy → paste → click for `maju://message?…` links and
// `maju://pr|issue|repo?…` entity links end-to-end. We pass a custom
// `urlTransform` (`majuDeepLinkUrlTransform`) that preserves valid Maju
...
function majuDeepLinkUrlTransform(value, key) {
...
function renderMarkdown(content) {
  return renderToStaticMarkup(
    React.createElement(
      ReactMarkdown,
      { urlTransform: majuDeepLinkUrlTransform },
      content,
    ),
  );
}

test("messageLinkUrlTransform: preserves maju://message href", () => {
  const html = renderMarkdown(
    "Click [here](maju://message?channel=abc&id=xyz)",
  );
  // HTML-encoded `&` in attributes is fine — the browser decodes back to `&`.
  assert.match(html, /href="maju:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves maju://message autolink href", () => {
  const html = renderMarkdown("<maju://message?channel=abc&id=xyz>");
  assert.match(html, /href="maju:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves maju://message href with thread", () => {
  const html = renderMarkdown(
    "[link](maju://message?channel=c1&id=m1&thread=t1)",
  );
  assert.match(html, /href="maju:\/\/message\?[^"]*thread=t1"/);
});

test("messageLinkUrlTransform: preserves maju://channel href", () => {
  const html = renderMarkdown(
    "Click [here](maju://channel/580ca78b-9dae-46f3-8854-bd671853ba32)",
  );
  assert.match(
    html,
    /href="maju:\/\/channel\/580ca78b-9dae-46f3-8854-bd671853ba32"/,
  );
});

test("messageLinkUrlTransform: rejects malformed maju://channel href", () => {
  const html = renderMarkdown(
    "Click [here](maju://channel/580ca78b-9dae-46f3-8854-bd671853ba32?extra=true)",
  );
...
test("messageLinkUrlTransform: preserves legacy maju://message href", () => {
  const html = renderMarkdown(
    "Click [here](maju://message?channel=abc&id=xyz)",
  );
  assert.match(html, /href="maju:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
...
test("messageLinkUrlTransform: leaves non-entity maju:// schemes to default", () => {
  // `maju://connect?relay=…` is handled by a different code path (Tauri
  ...
  const html = renderMarkdown(
    "[connect](maju://connect?relay=wss://relay.example)",
  );
  assert.match(html, /href=""/);
});

test("majuDeepLinkUrlTransform: preserves maju://pr entity link href", () => {
  const prLink = `maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`;
  const html = renderMarkdown(`[My PR](${prLink})`);
  // The href must survive — our transform preserves valid entity links.
  assert.match(html, /href="maju:\/\/pr\?/);
  assert.doesNotMatch(html, /href=""/);
});

test("majuDeepLinkUrlTransform: preserves maju://pr autolink href", () => {
  const prLink = `maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`;
...
test("majuDeepLinkUrlTransform: preserves maju://issue entity link href", () => {
  const issueLink = `maju://issue?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`;
  const html = renderMarkdown(`[Issue title](${issueLink})`);
  assert.match(html, /href="maju:\/\/issue\?/);
  assert.doesNotMatch(html, /href=""/);
});

test("majuDeepLinkUrlTransform: preserves maju://repo entity link href", () => {
  const repoLink = `maju://repo?owner=${OWNER_HEX}&d=maju-world`;
  const html = renderMarkdown(`[My repo](${repoLink})`);
  assert.match(html, /href="maju:\/\/repo\?/);
  assert.doesNotMatch(html, /href=""/);
});

test("majuDeepLinkUrlTransform: preserves maju://project autolink href", () => {
  const projectLink = `maju://project?owner=${OWNER_HEX}&d=onboarding`;
  const html = renderMarkdown(`<${projectLink}>`);
  assert.match(html, /href="maju:\/\/project\?/);
  assert.doesNotMatch(html, /href=""/);
});

test("majuDeepLinkUrlTransform: strips malformed maju://pr (unknown param)", () => {
  // Strict parser rejects unknown params — transform falls back to default sanitizer.
  const html = renderMarkdown(
    `[link](maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world&extra=ignored)`,
  );
  assert.match(html, /href=""/);
});
...
test("renderEntityLinkAnchor_directEntityLink_returnsAnchorRegardlessOfOrigin", () => {
  // A direct maju://pr link always resolves in-app — it does not require origin.
  const prLink = `maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`;
  const el = renderEntityLinkAnchor({
  ...
  assert.notEqual(
    el,
    null,
    "direct maju://pr link must produce an entity anchor regardless of origin",
  );
});
...
// `remark-gfm`'s autolinker only covers http(s)://, so bare `maju://message`
...
test("remarkMessageLinks: bare maju://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("maju://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
  assert.equal(para.children[0].value, "maju://message?channel=c&id=m");
  assert.equal(para.children[0].data.hName, "message-link");
});

test("remarkMessageLinks: legacy bare maju://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("maju://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
...
test("remarkMessageLinks: mid-sentence URL splits surrounding text", () => {
  const tree = runPlugin(
    paragraph(text("see maju://message?channel=c&id=m here")),
  );
...
test("remarkMessageLinks: two URLs in one text node both replaced", () => {
  const tree = runPlugin(
    paragraph(
      text(
        "first maju://message?channel=a&id=1 then maju://message?channel=b&id=2 done",
      ),
    ),
  );
  const kids = tree.children[0].children;
  ...
  assert.equal(links[0].value, "maju://message?channel=a&id=1");
  assert.equal(links[1].value, "maju://message?channel=b&id=2");
});

test("remarkMessageLinks: trailing sentence punctuation stays outside URL", () => {
  for (const punctuation of [".", ",", ";", ":", "!", "?"]) {
    const tree = runPlugin(
      paragraph(text(`see maju://message?channel=c&id=m${punctuation}`)),
    );
    const kids = tree.children[0].children;
    ...
    assert.equal(kids[1].value, "maju://message?channel=c&id=m", punctuation);
...
test("remarkMessageLinks: URL inside parens keeps closing paren outside", () => {
  const tree = runPlugin(
    paragraph(text("see (maju://message?channel=c&id=m) for details")),
  );
  const kids = tree.children[0].children;
...
test("remarkMessageLinks: non-message maju:// URLs are not matched", () => {
  const original = "maju://connect?relay=wss://x.example";
...
test("remarkMessageLinks: text inside inlineCode is left alone", () => {
    ...
    children: [
      {
        type: "paragraph",
        children: [
          { type: "inlineCode", value: "maju://message?channel=c&id=m" },
        ],
      },
    ],
  };
  ...
  assert.equal(kids[0].value, "maju://message?channel=c&id=m");
});
...
function nudgeBody(agentPubkey) {
  return [
    ...
    "",
    "Open Edit Agent in the Maju app to set these.",
    "",
    "```maju:config-nudge",
    JSON.stringify({
      agent_name: "Fizz",
...
test("bare Maju permalinks render cohesive icon-prefixed chips", () => {
  const channelId = "580ca78b-9dae-46f3-8854-bd671853ba32";
  const messageLink = `maju://message?channel=${channelId}&id=${EVENT_HEX}`;
  const compatibilityMessageLink = `maju://channel/${channelId}/${EVENT_HEX}`;
  const channelLink = `maju://channel/${channelId}`;
  const links = [
    messageLink,
    compatibilityMessageLink,
    channelLink,
    `maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`,
    `maju://issue?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`,
    `maju://repo?owner=${OWNER_HEX}&d=maju-world`,
  ];
  ...
  assert.equal((html.match(/data-maju-link=""/g) ?? []).length, 6);
  assert.equal(
    (
      html.match(
  ...
  assert.equal((visibleText.match(/maju-world/g) ?? []).length, 3);
...
test("inline issue and pull-request chips show the repository name without the event hash", () => {
  ...
  assert.equal(issueText, "maju-world");
  ...
  assert.match(issueHtml, /data-maju-link-kind="issue"/);
  assert.match(issueHtml, /inline-chip-icon-issue/);
  assert.match(
    issueHtml,
    /aria-label="Open issue c3b589fa in repository maju-world"/,
  );
  ...
  assert.equal(pullRequestText, "maju-world");
...
test("inline entity chip labels are bounded without truncating their accessible names", () => {
  ...
  const html = renderToStaticMarkup(
    renderEntityLinkAnchor({
      children: null,
      href: `maju://repo?owner=${OWNER_HEX}&d=${longRepository}`,
      onOpenEntityLink: () => {},
      relayOrigin: null,
    }),
  );
...
test("inline message chips omit fetched metadata and the event hash", () => {
  ...
  const markdown = renderCachedMarkdown({
    components: createMarkdownComponents(true, false),
    content: `maju://message?channel=${channelId}&id=${EVENT_HEX}`,
...
test("authored Maju permalink labels remain ordinary links", () => {
  const channelId = "580ca78b-9dae-46f3-8854-bd671853ba32";
  const links = [
    `[the message](maju://message?channel=${channelId}&id=${EVENT_HEX})`,
    `[the compatibility message](maju://channel/${channelId}/${EVENT_HEX})`,
    `[**design discussion**](maju://channel/${channelId})`,
    `[the issue](maju://issue?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world)`,
  ];
  const markdown = renderCachedMarkdown({
    components: createMarkdownComponents(true, false),
    content: links.join(" "),
    variant: "authored-maju-link-integration-test",
  });
  const html = renderToStaticMarkup(
    React.createElement(
      QueryClientProvider,
  ...
  assert.equal((html.match(/data-maju-link=""/g) ?? []).length, 0);
...
test("bare Maju permalinks shorten unavailable channel identifiers", () => {
  const channelId = "580ca78b-9dae-46f3-8854-bd671853ba32";
  const markdown = renderCachedMarkdown({
    components: createMarkdownComponents(true, false),
    content: [
      `maju://message?channel=${channelId}&id=${EVENT_HEX}`,
      `maju://channel/${channelId}`,
    ].join(" "),
    variant: "unknown-channel-maju-link-integration-test",
  });
  const html = renderToStaticMarkup(
    React.createElement(
      MarkdownRuntimeContext.Provider,
...
test("renderEntityLinkAnchor renders Maju entity links as chips", () => {
  const prLink = `maju://pr?id=${EVENT_HEX}&owner=${OWNER_HEX}&d=maju-world`;
  const el = renderEntityLinkAnchor({
    children: "PR · abc123",
    href: prLink,
  ...
  const html = renderToStaticMarkup(el);
  assert.match(html, /data-maju-link=""/);
  assert.match(html, /<span/);
  assert.match(html, /role="button"/);
  assert.match(html, /tabindex="0"/);
  assert.match(html, /data-maju-link-kind="pr"/);
  assert.match(html, /wrapping-inline-chip/);
  assert.match(html, /inline-chip-leading-fragment[^>]*>maju-</);
