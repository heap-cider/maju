...
export const THEME_STORAGE_KEY = "maju-theme";
const CACHE_KEY = "maju-theme-cache";
export const ACCENT_STORAGE_KEY = "maju-accent-color";
export const NEUTRAL_ACCENT = "neutral";
const FOLLOW_SYSTEM_KEY = "maju-follow-system";
...
const MAJU_VIBRANCY_MATERIAL = "sidebar";
...
function applyAccentColor(value: string) {
  ...
  if (value === NEUTRAL_ACCENT) {
    ...
    root.style.setProperty("--maju-selected-accent", foreground);
    root.style.setProperty(
      "--maju-video-review-accent",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
    root.style.setProperty(
      "--maju-video-review-accent-foreground",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
  ...
  root.style.setProperty("--maju-selected-accent", accentHsl);
  root.style.setProperty("--maju-video-review-accent", accentHsl);
  root.style.setProperty(
    "--maju-video-review-accent-foreground",
...
/**
 * The Maju themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a Maju theme is
 ...
 */
export function isMajuTheme(themeName: string): boolean {
  return themeName === "maju" || themeName === "maju-dark";
}

/**
 * Resolve the accent to actually apply for a theme: Maju themes are pinned to
...
function resolveEffectiveAccent(
  ...
  return isMajuTheme(themeName) ? NEUTRAL_ACCENT : accentColor;
}

/**
 * Toggle the opaque Maju sidebar-gradient marker. This is always safe to apply
 * synchronously: `data-maju-sidebar` paints solid gradient colors, so it never
 * makes the window see-through. The *translucent* treatment (transparent
 * root/body) is handled separately via {@link setMajuTranslucent} because it
 * must be sequenced against the native vibrancy layer — see
 * {@link applyMajuVibrancy}.
 */
function applyMajuSidebar(themeName: string) {
  const root = document.documentElement;
  if (isMajuTheme(themeName)) {
    root.setAttribute("data-maju-sidebar", "");
    // Keep the concrete Maju variant on the root as well as the generic
    ...
    root.setAttribute("data-maju-theme", themeName);
  } else {
    root.removeAttribute("data-maju-sidebar");
    root.removeAttribute("data-maju-theme");
    // Leaving Maju: drop translucency synchronously here too. Going *opaque*
    // never shows desktop/prior content through, so there's no ordering risk
    // on the way out — only on the way in.
    setMajuTranslucent(false);
  }
}
 ...
 * transparent root/body themselves are driven by the `data-maju-translucent`
 ...
 * underneath" flicker). {@link applyMajuVibrancy} owns that sequencing.
 */
function setMajuTranslucent(enabled: boolean) {
  const root = document.documentElement;
  if (enabled) {
    root.setAttribute("data-maju-translucent", "");
  } else {
    root.removeAttribute("data-maju-translucent");
  }
}

/**
 * Monotonic token identifying the most recent vibrancy request. Because
 * {@link applyMajuVibrancy} awaits the native `set_window_vibrancy` IPC, a rapid
 * Maju → non-Maju toggle can fire two overlapping calls whose awaits resolve out
 ...
 * translucency — otherwise the earlier Maju call could re-add
 * `data-maju-translucent` after the later non-Maju call already cleared it,
 * leaving the window transparent under a non-Maju theme.
 */
let majuVibrancyRequest = 0;

/**
 * Whether the native vibrancy layer is confirmed installed for a Maju theme.
 ...
 */
let majuVibrancyReady = false;

/** The native layer does not need rebuilding when Maju only changes mode. */
let majuVibrancyEnabled = false;

/**
 * Enable the CSS translucency treatment, but only once BOTH prerequisites for
 * the current request are in place:
 *
 *  1. the native vibrancy layer is installed ({@link majuVibrancyReady}), and
 *  2. the Maju sidebar marker + gradient vars are applied (`data-maju-sidebar`,
 *     set synchronously by {@link applyMajuSidebar} inside {@link applyTheme}).
 *
 * Translucency clears the body/sidebar surfaces so the vibrancy layer shows
 * through; enabling it before the Maju gradient vars are installed would flash a
 * transparent/unstyled sidebar. `applyTheme` (theme vars) and
 * `applyMajuVibrancy` (native layer) are independent async effects that can win
 ...
 */
function maybeEnableMajuTranslucent(themeName: string, requestToken: number) {
  if (requestToken !== majuVibrancyRequest) return;
  if (!isMajuTheme(themeName) || !isMacPlatform()) return;
  if (!majuVibrancyReady) return;
  if (!document.documentElement.hasAttribute("data-maju-sidebar")) return;
  setMajuTranslucent(true);
}
 ...
 * behind it:
 *
 * - Entering Maju (macOS): install the vibrancy layer first (await the IPC),
 ...
 * - Leaving Maju: translucency was already removed synchronously in
 *   `applyMajuSidebar` (safe — opaque never shows through), so here we just
 *   clear the native layer.
 *
 * On non-macOS `set_window_vibrancy` is a no-op and translucency stays off, so
 * these platforms fall back to the opaque Maju gradient.
 *
 * Overlapping calls are guarded by {@link majuVibrancyRequest} so a stale async
 * continuation can't re-enable translucency after a newer theme superseded it.
 */
async function applyMajuVibrancy(themeName: string) {
  const maju = isMajuTheme(themeName);
  const requestToken = ++majuVibrancyRequest;

  // Maju Light and Maju Dark use the same native material. Rebuilding the
  ...
  if (maju && majuVibrancyEnabled && majuVibrancyReady) {
    maybeEnableMajuTranslucent(themeName, requestToken);
    return;
  }
  ...
  try {
    await invokeTauri<void>("set_window_vibrancy", {
      enabled: maju,
      material: MAJU_VIBRANCY_MATERIAL,
    });
    ...
    if (requestToken !== majuVibrancyRequest) return;
    majuVibrancyEnabled = maju;
    // Native layer is installed. Record readiness and try to enable translucency
    // — but only if `applyMajuSidebar` has already installed the Maju gradient
    // vars. If that effect hasn't landed yet (the IPC won the race), it will
    // call maybeEnableMajuTranslucent itself once the marker is applied.
    if (maju && isMacPlatform()) {
      majuVibrancyReady = true;
      maybeEnableMajuTranslucent(themeName, requestToken);
    }
  } catch (error) {
...
function applyCachedVars(): string | null {
  try {
    ...
    applyMajuSidebar(themeName);

    const accent = getStorageItem(ACCENT_STORAGE_KEY) ?? DEFAULT_ACCENT;
    // Pin Maju themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached Maju theme + non-neutral stored accent flashes the
...
async function applyTheme(name: SyntaxThemeName): Promise<{
  ...
  root.classList.remove("light", "dark");
  root.classList.add(isDark ? "dark" : "light");
  applyMajuSidebar(name);
  // The Maju gradient vars are now installed. If the vibrancy layer already
  // resolved for the current request (the IPC won the race against this theme
  // load), enable translucency now — otherwise applyMajuVibrancy does it. This
  // is the second half of the two-effect handshake; the token guards against a
  // superseding theme switch.
  maybeEnableMajuTranslucent(name, majuVibrancyRequest);
  ...
  // new theme for a frame — the flicker seen when switching to Maju. Maju
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the Maju
  ...
  useEffect(() => {
    if (!isValidThemeName(effectiveTheme)) return;
    void applyMajuVibrancy(effectiveTheme);
  }, [effectiveTheme]);
  ...
  // changes. applyTheme already applies the (Maju-neutral-aware) accent in the
