...
test("scales fixed line-height utilities with the typography rem", () => {
  assert.deepEqual(config.theme.extend.lineHeight, {
    3: "calc(var(--maju-type-rem) * 0.75)",
    4: "var(--maju-type-rem)",
    5: "calc(var(--maju-type-rem) * 1.25)",
    6: "calc(var(--maju-type-rem) * 1.5)",
    7: "calc(var(--maju-type-rem) * 1.75)",
    8: "calc(var(--maju-type-rem) * 2)",
...
test("derives the typography rem from the real root so zoom scales layout too", () => {
  ...
  assert.match(
    typographyCss,
    /--maju-type-rem:\s*calc\(1rem \* var\(--maju-type-scale\)\);/,
  );
  assert.doesNotMatch(typographyCss, /--maju-type-rem:\s*[\d.]+px/);
});

test("maps the font size attribute to the 13 / 14 / 15px type contract", () => {
  assert.match(typographyCss, /:root\s*\{[^}]*--maju-type-scale:\s*1;/s);
  assert.match(
    typographyCss,
    /:root\[data-font-size="smaller"\]\s*\{\s*--maju-type-scale:\s*calc\(13 \/ 14\);/,
  );
  assert.match(
    typographyCss,
    /:root\[data-font-size="larger"\]\s*\{\s*--maju-type-scale:\s*calc\(15 \/ 14\);/,
  );
});
