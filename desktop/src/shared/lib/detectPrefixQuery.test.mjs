...
const CHANNELS = ["maju-bugs", "maju dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#maju", CHANNELS), {
    query: "maju",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #maju", CHANNELS), {
    query: "maju",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#maju", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#maju de", CHANNELS), {
    query: "maju de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#maju de` — `#` preceded by `x`, no boundary → no query even though
  // "maju de" is a prefix of a known channel.
  assert.equal(at("#", "x#maju de", CHANNELS), null);
});
