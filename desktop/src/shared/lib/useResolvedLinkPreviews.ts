...
/**
 * Maju entity titles come from relay events, so they are community-scoped —
...
/**
 * Resolve a Maju PR/issue card title from the relay event's `subject` tag
 ...
 */
async function fetchMajuEntityTitle(href: string): Promise<string | null> {
...
function resolveTitle(preview: SupportedLinkPreview): Promise<string | null> {
  return preview.href.startsWith("maju://")
    ? fetchMajuEntityTitle(preview.href)
