...
export async function fetchMajuEntityMetadata(
  href: string,
...
export function shouldResolveTitle(preview: SupportedLinkPreview): boolean {
  ...
  return parsed.ok && preview.title === majuEntityFallbackTitle(parsed.value);
}
...
export function resolveLinkPreview(
  ...
  if (metadata === undefined) {
    return {
      ...preview,
      imageState: isMajuEntityPreview(preview) ? "none" : "pending",
    };
  }
  if (metadata === null) {
...
export function isMajuEntityPreview(preview: SupportedLinkPreview): boolean {
  return (
    preview.kind === "maju-pull-request" ||
    preview.kind === "maju-issue" ||
    preview.kind === "maju-repository" ||
    preview.kind === "maju-project"
  );
}

/**
 * Recipient-side `maju://` entity cards must render even when the relay
 ...
 * `<dtag> #<id8>` for PRs/issues — see `majuEntityFallbackTitle`). Re-adds
...
export function withEntityFallbacks(
  ...
  return previews.flatMap((preview) => {
    ...
    return isMajuEntityPreview(preview)
...
export function useResolvedLinkPreviews(
  ...
  React.useEffect(() => {
    ...
    if (refetchNewNegatives) {
      ...
      // maju:// entity links resolve off the relay, not this cache — skip them.
    ...
    for (const preview of previews) {
      const loader = preview.href.startsWith("maju://")
