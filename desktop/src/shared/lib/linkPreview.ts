...
// Maju relay hosts differ per community, so relay git URLs are recognized by
...
const SUPPORTED_URL_RE =
  /(^|[\s([{<>"'])((?:https?:\/\/)?(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^\s<>"'\]]+|https?:\/\/[^\s<>"'\]]+\/git\/[a-f0-9]{64}\/[^\s<>"'\]]+|maju:\/\/(?:pr|issue|repo)\?[^\s<>"'\]]+)/gi;
const MARKDOWN_SUPPORTED_LINK_RE =
  /!?\[([^\]\n]+)\]\(((?:https?:\/\/)?(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^)\s<>"']+|https?:\/\/[^)\s<>"']+\/git\/[a-f0-9]{64}\/[^)\s<>"']+|maju:\/\/(?:pr|issue|repo)\?[^)\s<>"']+)\)/gi;
...
function parseMajuEntityPreview(href: string): SupportedLinkPreview | null {
  ...
  const title = majuEntityFallbackTitle(link);
  if (link.type === "pr") {
    return {
...
const MAJU_GIT_PATH_RE =
  /^\/git\/([a-f0-9]{64})\/([a-zA-Z0-9._-]+?)(?:\.git)?\/?$/;

/**
 * Recognize a Maju relay git URL (`{relay-origin}/git/<owner-pubkey>/<repo>`,
 * the clone URL shape agents paste when announcing work). The preview href
 * is normalized to the canonical `maju://repo` deep link: the raw git
 * transport endpoint is not a browsable page, and the maju:// href gives the
 ...
 * active Maju relay — an arbitrary external URL sharing the path shape must
...
function parseMajuGitLink(
  ...
  const match = MAJU_GIT_PATH_RE.exec(parsed.pathname);
...
export function parseSupportedLinkPreview(
  ...
  if (isEntityLink(candidate)) {
    return parseMajuEntityPreview(candidate);
  }
  ...
  return (
    parseMajuGitLink(parsed, activeRelayOrigin ?? null) ??
