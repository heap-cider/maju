 ...
 *
 * Only URLs hosted on the Maju relay are rewritten. External Blossom URLs
 ...
 * (the Maju relay), resulting in 404s.
 */
...
/**
 * Cached relay origin (e.g. "https://maju-oss.stage.blox.sqprod.co"),
 ...
 * (`normalize_host` in maju-core), but the saved community relay URL keeps
...
/**
 * The relay origin (e.g. `https://maju-oss.stage.blox.sqprod.co`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the Maju relay, rewrite it
 * to go through the local streaming proxy. External Blossom URLs and
 * non-Blossom URLs are returned unchanged.
 *
 * Falls back to maju-media:// if the proxy port isn't available yet.
 */
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.maju.xyz).
  if (cachedRelayOrigin) {
  ...
  return `maju-media://localhost/media/${m[1]}`;
}
