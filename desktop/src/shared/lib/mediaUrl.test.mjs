...
test("rewriteRelayUrl: matches relay origin case-insensitively (uppercase saved community URL)", async () => {
    ...
    __TAURI_INTERNALS__: {
      invoke(command) {
        ...
        if (command === "get_relay_http_url") {
          ...
          // emits lowercased media URLs (normalize_host in maju-core).
          return Promise.resolve("https://PENDING-SEED.communities.maju.xyz");
        }
        return Promise.reject(new Error(`Unexpected command: ${command}`));
      },
    },
  ...
  try {
    ...
    const relayMediaUrl = `https://pending-seed.communities.maju.xyz/media/${HASH}.png`;
    assert.equal(
