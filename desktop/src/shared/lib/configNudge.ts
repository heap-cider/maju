/**
 * Utilities for extracting and parsing the `maju:config-nudge` sentinel that
 * `maju-acp`'s setup-listener appends to its kind:9 nudge body.
 *
...
/**
 * The structured payload embedded in the `maju:config-nudge` sentinel block.
...
const FENCE_OPEN = "```maju:config-nudge";
const FENCE_CLOSE = "```";
...
/**
 * Strip the `maju:config-nudge` sentinel block (and any preceding blank line)
