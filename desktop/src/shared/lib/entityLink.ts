 ...
 *
 * Formats:
 *   maju://repo?owner=<owner-pubkey>&d=<repo-dtag>
 ...
 * builder in `crates/maju-cli/src/links.rs` emits the same format — the two
...
export function buildRepoLink(input: { owner: string; dtag: string }): string {
  ...
  return `maju://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}
