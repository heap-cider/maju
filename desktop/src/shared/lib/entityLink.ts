/**
 * `maju://` deep links for Maju-hosted git entities, mirroring
 * `features/messages/lib/messageLink.ts` for `maju://message`.
 *
 * Formats:
 *   maju://repo?owner=<owner-pubkey>&d=<repo-dtag>[&tab=<tab>][&commit=<git-hash>]
 *   maju://project?owner=<owner-pubkey>&d=<project-dtag>[&tab=<tab>]
 *   maju://pr?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *   maju://issue?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *
 ...
 * overview. The CLI builder in `crates/maju-cli/src/links.rs` emits the
 * same format — the two must stay compatible (see the golden-format tests
 * on both sides).
 */

const ENTITY_LINK_SCHEME = "maju:";
...
export function buildRepoLink(input: {
  ...
  return `maju://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildCommitLink(input: {
  ...
  return `maju://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}&tab=commits&commit=${input.commitHash.toLowerCase()}`;
}
...
export function buildProjectLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `maju://project?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildPullRequestLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `maju://pr?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}
...
export function buildIssueLink(input: {
  ...
  checkEventId(input.id);
  checkCoordinate(input.owner, input.dtag);
  return `maju://issue?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/**
 * Cheap pre-check used by the markdown renderer and preview extraction
 * before parsing. `maju://message` is intentionally excluded — it has its
 * own pill rendering path.
 */
export function isEntityLink(href: string | undefined | null): boolean {
  if (!href) return false;
  return (
    href.startsWith("maju://pr?") ||
    href.startsWith("maju://issue?") ||
    href.startsWith("maju://repo?") ||
    href.startsWith("maju://project?")
  );
}

/**
 * Parse a `maju://pr|issue|repo?…` URL. Returns a discriminated result so
