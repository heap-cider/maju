...
test("metadata cache keys deduplicate URL fragments", () => {
  assert.equal(
    __linkPreviewMetadataTest.metadataCacheKey(
      "https://github.com/heap-cider/maju/pull/3834#issuecomment-1",
    ),
    "https://github.com/heap-cider/maju/pull/3834",
  );
});
...
test("Maju project metadata resolves from the 30621 announcement", async () => {
  const owner = "cd".repeat(32);
  const result = await fetchMajuEntityMetadata(
    `maju://project?owner=${owner}&d=pollinator`,
    async () => [
      relayEvent({
        id: "01".repeat(32),
        kind: 30621,
...
test("Maju project metadata declines a missing or invalid announcement", async () => {
  const owner = "cd".repeat(32);
  assert.equal(
    await fetchMajuEntityMetadata(
      `maju://project?owner=${owner}&d=pollinator`,
      async () => [],
    ),
    null,
