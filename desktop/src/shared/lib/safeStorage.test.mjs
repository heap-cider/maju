...
test("getStorageItem returns the stored value when storage is healthy", () => {
  const restore = patchLocalStorage(
    createThrowingStorage({ "maju-theme": "maju" }),
  );
  try {
    assert.equal(getStorageItem("maju-theme"), "maju");
...
test("getStorageItem returns null when the key is absent", () => {
  ...
  try {
    assert.equal(getStorageItem("maju-theme"), null);
  } finally {
    restore();
  }
});
...
test("setStorageItem returns true on a healthy write", () => {
  ...
  try {
    assert.equal(setStorageItem("maju-theme", "maju-dark"), true);
    assert.equal(storage.values.get("maju-theme"), "maju-dark");
  } finally {
    restore();
  }
});
