export const MAJU_RELEASES_URL = "https://github.com/heap-cider/maju/releases";
const MAJU_RELEASES_API_URL =
  "https://api.github.com/repos/heap-cider/maju/releases?per_page=10";
const CACHE_KEY = "maju.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type MajuDownloadPlatform = {
...
): MajuDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): MajuDownloadPlatform["architecture"] {
...
export async function detectMajuDownloadPlatform(
  navigatorValue: Navigator,
): Promise<MajuDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: MajuDownloadPlatform): RegExp | undefined {
...
export function selectMajuDownloadUrl(
  releases: GitHubRelease[],
  platform: MajuDownloadPlatform,
): string | undefined {
...
export async function resolveMajuDownloadUrlForPlatform(
  platform: MajuDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(MAJU_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return MAJU_RELEASES_URL;
    const url = selectMajuDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return MAJU_RELEASES_URL;
    try {
    ...
    return MAJU_RELEASES_URL;
  }
}

export async function resolveMajuDownloadUrl(): Promise<string> {
  return resolveMajuDownloadUrlForPlatform(
    await detectMajuDownloadPlatform(navigator),
  );
}
