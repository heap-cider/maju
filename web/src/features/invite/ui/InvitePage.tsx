import majuAppIcon from "@/assets/app-icon@3x.png";
import { claimInviteInBrowser } from "@/features/invite/invite-api";
import {
  MAJU_RELEASES_URL,
  type MajuDownloadPlatform,
  detectMajuDownloadPlatform,
  resolveMajuDownloadUrlForPlatform,
} from "@/shared/lib/maju-download";
...
export function InvitePage({ code }: { code: string }) {
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(MAJU_RELEASES_URL);
  ...
  React.useEffect(() => {
    let active = true;
    detectMajuDownloadPlatform(navigator).then(async (platform) => {
      if (!active) return;
      if (
      ...
      const url = await resolveMajuDownloadUrlForPlatform(platform);
  ...
  const openInvite = async () => {
    ...
    try {
      ...
      window.location.href = `maju://join?${query.toString()}`;
    } finally {
      setOpening(false);
    }
  };
    ...
    >
      <div className="w-full max-w-xl space-y-4">
        <div className="flex w-full flex-col items-center rounded-3xl bg-white px-6 py-10 sm:px-12 sm:py-12">
          ...
          >
            <img alt="Maju" className="h-full w-full" src={majuAppIcon} />
          </div>
          ...
          <div className="mt-9 w-full max-w-md space-y-2">
            ...
            {policy === null ? (
              ...
              >
                <a
                  href={`maju://join?relay=${encodeURIComponent(relay)}&code=${encodeURIComponent(code)}`}
                >
                  Accept invite in Maju
                </a>
              </Button>
            ) : (
              <Button
