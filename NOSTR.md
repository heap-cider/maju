# Using Third-Party Nostr Clients with Maju

Maju is a Nostr relay that speaks NIP-29 (relay-based groups) natively. Third-party Nostr clients connect directly to `maju-relay` using NIP-29 and NIP-42 authentication. The old NIP-28 compatibility proxy has been removed.

## Community scope

Maju treats the relay URL/domain as authoritative for the community. Today's
...
export MAJU_PUBKEY_ALLOWLIST=true
...
PGPASSWORD=maju_dev psql -h localhost -U maju -d maju -c \
...
| **Edits (kind:40003)** | ⚠️ | Works on the wire but Maju-only — no standard NIP-29 client renders these |
| **Rich content (kind:40002)** | ⚠️ | Works on the wire but Maju-only — no standard NIP-29 client renders these |
...
When `MAJU_PUBKEY_ALLOWLIST=true`, NIP-42 connections that authenticate with only a pubkey
...
| Kind | Tags | Content |
|------|------|---------|
| **39000** | `d=<uuid>`, `name`, `closed` (always); `about` (if description non-empty); `private` (if applicable); `hidden` (DM channels only) | Group metadata. **Note:** `closed` is always emitted per NIP-29 convention (Maju channels require explicit membership), but open channels are still readable/writable by non-members at runtime. The tag reflects the membership model, not access enforcement. |
...
|--------|----------|:--------:|-------|
| **MajuTestClient** | Rust (repo) | Automated E2E | Full NIP-29 flow: discovery (39000/39001/39002), kind:9 send/receive, reactions, deletions, h-tag enforcement |
...
When `MAJU_REQUIRE_RELAY_MEMBERSHIP=true`, every authenticated connection is checked against the
...
Use `maju-admin` — the operator CLI shipped in the relay image — to manage relay membership.
...
Or invoke `maju-admin` directly inside the container:

```bash
docker compose exec relay maju-admin add-member --pubkey npub1abc...
docker compose exec relay maju-admin add-member --pubkey npub1abc... --role admin
docker compose exec relay maju-admin remove-member --pubkey npub1abc...
docker compose exec relay maju-admin list-members
```
...
| `MAJU_RELAY_PRIVATE_KEY` | Hex private key — required to sign kind:13534 events |
...
|----------|:--------:|---------|-------------|
| `MAJU_PUBKEY_ALLOWLIST` | ❌ | `false` | Enable pubkey allowlist for NIP-42 pubkey-only auth |
| `MAJU_RELAY_PRIVATE_KEY` | ❌ | random | Hex secret key for relay signing (discovery events, system messages) |
| `MAJU_REQUIRE_AUTH_TOKEN` | ❌ | `false` | Require authenticated NIP-42 for all connections |
...
- [`docs/nips/`](docs/nips/) — Maju's own NIP extension documents.
