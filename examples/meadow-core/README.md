...
A minimal three-agent persona pack for Maju.
...
```bash
# Validate the pack
maju pack validate ./examples/meadow-core

# Inspect resolved config
maju pack inspect ./examples/meadow-core
```
...
app), not persona-pack source. `maju pack inspect` above shows the fully-resolved per-agent
...
`crates/maju-persona/PERSONA_PACK_SPEC.md` for the current import paths.
...
See `crates/maju-persona/PERSONA_PACK_SPEC.md` for the full format reference.
