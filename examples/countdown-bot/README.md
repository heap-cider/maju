...
A tiny non-AI Maju bot example.

The bot is deliberately boring and algorithmic: it listens to one Maju channel
...
It demonstrates that Maju participants do not have to be LLM agents. Any
...
members list and in Maju's mention autocomplete.
...
```bash
MAJU_RELAY_URL=ws://localhost:3000 \
MAJU_CHANNEL_ID=<channel-uuid> \
MAJU_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
MAJU_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Maju agents
...
MAJU_CHANNEL_ID=<channel-uuid> \
MAJU_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
MAJU_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
MAJU_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
MAJU_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
MAJU_BOT_AUTH_MODE=owner-attested \
# plus MAJU_RELAY_URL, MAJU_CHANNEL_ID, MAJU_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `MAJU_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `MAJU_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Maju UI adds that tag when the bot is selected from
