...
const BOT_ABOUT: &str =
    "A tiny non-AI Maju reference bot that replies to !countdown and countdown-style !fib.";
...
impl Config {
    fn from_env() -> Result<Self> {
        let relay_url =
            std::env::var("MAJU_RELAY_URL").unwrap_or_else(|_| DEFAULT_RELAY_URL.to_string());
        let channel_id = required_env("MAJU_CHANNEL_ID")?;
        let bot_keys = Keys::parse(&required_env("MAJU_BOT_PRIVATE_KEY")?)
            .context("MAJU_BOT_PRIVATE_KEY must be an nsec or hex private key")?;

        let auth_mode =
            std::env::var("MAJU_BOT_AUTH_MODE").unwrap_or_else(|_| "standalone".to_string());
        let owner_auth_tag = match auth_mode.as_str() {
            "standalone" => None,
            "owner-attested" => {
                let tag_json = match std::env::var("MAJU_AUTH_TAG") {
                    Ok(value) if !value.trim().is_empty() => value,
                    _ => {
                        let owner_keys = Keys::parse(&required_env("MAJU_OWNER_PRIVATE_KEY")?)
                            .context("MAJU_OWNER_PRIVATE_KEY must be an nsec or hex private key")?;
                        maju_sdk::nip_oa::compute_auth_tag(&owner_keys, &bot_keys.public_key(), "")?
                    }
                };

                let owner = maju_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                    .context("MAJU_AUTH_TAG is not valid for MAJU_BOT_PRIVATE_KEY")?;
                eprintln!("owner-attested auth tag verified; owner={}", owner.to_hex());
                Some(maju_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!("MAJU_BOT_AUTH_MODE must be 'standalone' or 'owner-attested', got {other:?}")
            }
        };
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = maju_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = maju_sdk::builders::build_message(
