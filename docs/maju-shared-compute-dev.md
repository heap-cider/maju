# Maju shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Maju Desktop → maju-acp → maju-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `heap-cider/maju` repository root on the mesh-enabled branch.
...
`~/.maju-dev`, and local Docker volumes; it preserves the installed Maju app's
data, production keyring, and `~/.maju`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Maju**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Maju**.
...
Stop only stale Maju/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Maju window to open and for the terminal to
...
Maju may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `maju-agent`. No API key is required.
...
**maju-agent** as the runtime, **Maju shared compute** as the LLM provider,
...
- no current Maju membership snapshot is available (admission fails closed).

## Security boundary

Maju publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Maju membership controls which node identities are
...
`MAJU_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
...
MeshLLM—not the Maju relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Maju-side checks above.
