...
- Slice 1 — `maju://pr|issue|repo` deep links: `entityLink.ts`
...
the `maju://project` scheme (waiting on NIP-MP landing), and the follow-ups
in slice 4.
...
```
maju://repo?owner=<pubkey-hex>&d=<repo-dtag>
maju://project?owner=<pubkey-hex>&d=<project-dtag>
...
(sibling to `useMessageDeepLinks.ts`). This makes links pasted outside Maju
