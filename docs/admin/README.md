...
Maju can expose a private, deployment-wide moderation dashboard from the existing
relay process. It shows open moderation reports and recent product feedback.

Configure `MAJU_ADMIN_HOST` to activate the dashboard. A private ingress limits
access to the operator VPN or approved source IPs.

Required configuration:

```text
MAJU_ADMIN_HOST=admin.example.com
MAJU_ADMIN_WEB_DIR=/srv/maju/admin-web
```
...
`MAJU_ADMIN_HOST` activates the dashboard in the default `nip98` mode. Configure
the mode with `MAJU_ADMIN_AUTH`, which accepts `nip98` (default when unset) or
`disabled`.

`MAJU_ADMIN_TOKEN` is no longer recognised: token authentication was removed.
...
### NIP-98 mode (`MAJU_ADMIN_AUTH=nip98`, default)
...
When `MAJU_ADMIN_HOST` is set, the relay advertises the admin API origin in its
...
### Disabled mode (`MAJU_ADMIN_AUTH=disabled`)
...
```
MAJU_ADMIN_AUTH=disabled — the admin API is unauthenticated; the operator has
...
`MAJU_ADMIN_AUTH` accepts exactly `nip98` or `disabled`. Any other non-empty
value is a startup error (typo-proofing). `MAJU_ADMIN_TOKEN` is no longer
recognised and its presence is ignored with a startup warning regardless of the
other variables:

| Combination | Result |
|---|---|
| `MAJU_ADMIN_TOKEN` set (with or without `MAJU_ADMIN_HOST`) | warn + ignore |
| `MAJU_ADMIN_AUTH=nip98` with a malformed `RELAY_OWNER_PUBKEY` | startup error |
| `MAJU_ADMIN_AUTH` junk value | startup error |
| `MAJU_ADMIN_AUTH` without `MAJU_ADMIN_HOST` | warn + ignore |
...
**Upgrading from a pre-auth release (Maju prior to the introduction of this
`MAJU_ADMIN_HOST` requirement):** any deployed relay with `MAJU_ADMIN_HOST` set
boots in `nip98` mode after upgrade unless `MAJU_ADMIN_AUTH=disabled` is set.
Choose the mode that fits your deployment:

- **Nostr principal model (default):** set `MAJU_ADMIN_AUTH=nip98` (or leave it
  ...
  `MAJU_ADMIN_AUTH=nip98` and add the operator pubkey(s) — never split these
...
- **Network-layer mode (e.g. Block's `bb-public` behind WARP+Okta):** set
  `MAJU_ADMIN_AUTH=disabled` in your deploy config, then roll the new version.

**Upgrading from a token-mode release:** token authentication was removed.
Remove `MAJU_ADMIN_TOKEN` from the environment — leaving it set is ignored with
a startup warning — and adopt `nip98` (populate `RELAY_OPERATOR_PUBKEYS`) or
`disabled` before rolling the new version.

**Upgrading from the previous `MAJU_ADMIN_INSECURE_NO_AUTH=true` variable:**
replace it with `MAJU_ADMIN_AUTH=disabled`. Behavior is identical; the old
variable is no longer recognised.

Relays without `MAJU_ADMIN_HOST` are completely unaffected, except that a
lingering `MAJU_ADMIN_TOKEN` now logs a startup warning and must be removed.
...
defaults to `MAJU_ADMIN_AUTH=disabled`, so the dashboard renders without a
