...
This document specifies the protocol by which Maju Desktop delegates the
...
1. **The provider protocol** — a zero-registration plugin contract between the
   desktop and any executable named `maju-backend-<id>`: discovery, the `info`
...
3. **The Kubernetes binding** — the first conforming provider,
   `maju-backend-kubernetes`, which realizes the contract as a bare Pod
...
launcher among many**. What makes a process a live Maju agent is a keypair,
a NIP-OA auth tag, and a relay URL, handed as environment to the `maju-acp`
...
- **Agent conversational behavior.** What the agent does with events is
  governed by the ACP harness (`maju-acp`) and the NIPs it implements
...
- **Desktop** `D` — the Maju Desktop app. Holds the agent's identity (nsec in
  the OS keyring), its configuration record, and the only UI. Trusted.
- **Provider** `P` — an executable `maju-backend-<id>` on `D`'s machine.
...
- **Substrate** `S` — the remote compute environment `P` deploys into (a
  ...
  `D` never talks to `S`.
- **Agent** `A` — a `maju-acp` harness process (plus the ACP agent under it)
  running on `S`, holding the nsec it was given, connected to the relay.
- **Relay** `R` — the Maju relay. The *only* channel that connects `D` to a
...
harness spawns — `goose`, `claude-agent-acp`, `codex-acp`, `maju-agent`, or
...
1. **The agent/harness contract — binds every launcher.** A live Maju agent
   is a `maju-acp` process holding a keypair, a NIP-OA auth tag (or resolved
   ...
   restart** (I5). A bash script that exports `MAJU_PRIVATE_KEY`,
   `MAJU_RELAY_URL`, `MAJU_AUTH_TAG` and execs the harness is a conforming
...
- **(I3) Presence is the status.** `D` derives a remote agent's live state
  ...
  (`PRESENCE_TTL_SECS`, `maju-pubsub/src/presence.rs:16`; the vision's
  ...
  presence-suppression knob, `MAJU_ACP_NO_PRESENCE`, MUST join
...
`maju-backend-<id>`. The suffix after the prefix is the provider id and MUST
...
`maju-backend-kubernetes` will ask. (At `28ae6cd21` discovery records only
...
`MAJU_PRIVATE_KEY`, `NOSTR_PRIVATE_KEY`, `MAJU_AUTH_TAG`, `MAJU_RELAY_URL`,
...
payload fields (`private_key_nsec` → `MAJU_PRIVATE_KEY`/`NOSTR_PRIVATE_KEY`,
`auth_tag` → `MAJU_AUTH_TAG`, `relay_url` → `MAJU_RELAY_URL`); reading
...
POSIX-shaped names before merge, because a key like `MAJU_AUTH_TAG=x`
...
"launch": {
  ...
  "policy_env":   {str: str},   // overridable behavior defaults (tier 1, below):
                                ...
                                // MAJU_ACP_RELAY_OBSERVER, MAJU_ACP_LAZY_POOL=true,
                                // MAJU_ACP_SESSION_TITLE (resolved),
                                // MAJU_ACP_TEAM_INSTRUCTIONS, MAJU_ACP_MODEL,
                                // MCP_HOOK_SERVERS=* (mcp_hooks runtimes only)
  "owner_pubkey": str | null    // resolved workspace owner (hex) — legacy
                                // MAJU_ACP_AGENT_OWNER fallback, non-secret
}
```
...
Claude; `MAJU_AGENT_MODEL`/`MAJU_AGENT_PROVIDER` for maju-agent). A fixed
`provider → MAJU_AGENT_PROVIDER` mapping is wrong for three of the four
...
- `MAJU_ACP_TEAM_INSTRUCTIONS` — the only truly non-reconstructible policy
  value: `effective_team_instructions` (`spawn_hash.rs:41-52`) needs the
  desktop's `TeamRecord` store, which no pod can reach.
- `MAJU_ACP_SESSION_TITLE` — sent **resolved** (`resolve_session_title`,
...
- `MAJU_ACP_LAZY_POOL=true` — a **deliberate pick, not a transcription**:
...
- `MCP_HOOK_SERVERS=*` when the resolved runtime has `mcp_hooks`
  (`runtime.rs:594-598`; maju-agent only at `28ae6cd21`) — gates the
  `_Stop`/`_PostCompact` hook tools.
- `MAJU_ACP_SYSTEM_PROMPT`, `MAJU_ACP_IDLE_TIMEOUT`,
  `MAJU_ACP_MAX_TURN_DURATION`, `MAJU_ACP_AGENTS` — resolved by the desktop
  from the record's `system_prompt` / `idle_timeout_seconds` /
  `max_turn_duration_seconds` / `parallelism` (each omitted when null,
  matching the local spawn's conditional emission). `MAJU_ACP_AGENTS` is
  ...
  are **tier-1 control-plane** keys: `MAJU_ACP_AGENTS` is in
...
`MAJU_ACP_DEDUP` and `MAJU_ACP_MULTIPLE_EVENT_HANDLING` are **deliberately
...
imply a divergence that does not exist. `MAJU_MANAGED_AGENT` is likewise
...
1. **Overridable behavior defaults** — `launch.policy_env`. Most keys here
   ...
   **Exception — `MAJU_ACP_AGENTS`:** this key IS reserved
...
3. **Authoritative** — unoverridable at every layer, written last and
   ...
   `MAJU_ACP_AGENT_OWNER`, the inactivity bound, `MAJU_ACP_MCP_COMMAND`,
   and `MAJU_MANAGED_AGENT_START_NONCE`. For the nonce, the provider MUST
   set it to the attempt's **generation token** (§K8s Secrets): the harness
   stamps it into every observer lifecycle frame (`maju-acp/lib.rs:1501`),
...
- the harness and agent binaries: `launch.command` is a *name*, resolved
  against the image's own `PATH` (`MAJU_ACP_AGENT_COMMAND`), and
  `MAJU_ACP_MCP_COMMAND=maju-dev-mcp` likewise;
...
- `MAJU_ACP_SETUP_PAYLOAD` is desktop-computed readiness state and MUST NOT
  appear in a remote pod.

**Owner resolution (normative):** the provider MUST have either a non-null
`auth_tag` (→ `MAJU_AUTH_TAG`) or a non-null `launch.owner_pubkey`
(→ `MAJU_ACP_AGENT_OWNER`) before any mutation; if both are null it MUST
refuse the deploy. Without an owner the harness cannot match `!shutdown`
(`maju-acp/src/lib.rs: resolve_agent_owner`, main-loop owner check) and the
agent answers its own stop command conversationally — §Stop would be
describing a mechanism that does not work. `MAJU_ACP_AGENT_OWNER` is a
reserved key, so this value can only arrive as authoritative launch data,
never through user env.

**Maju shared compute (relay-mesh) is non-deployable, and this is forced,
...
**Auto-repair is fenced to Maju-authored, positively identified residue
...
management marker — `app.kubernetes.io/managed-by: maju-backend-kubernetes`
...
is substrate-operator residue survives with one qualifier: *Maju-authored*
...
`maju.heap-cider.github.io/create-intent`, written at pod create — the same shape as
...
```
--exit-after-inactivity <secs>   /   MAJU_ACP_EXIT_AFTER_INACTIVITY
```
...
- **Mechanism**: on expiry of the bound, the harness fires the same shutdown
  ...
  **The expiry check MUST NOT depend on pool readiness.** This is a design
  ...
  `MAJU_ACP_LAZY_POOL=true` (§Launch data) into a deadlock in I5's single
...
- **Reserved keys**: `MAJU_ACP_EXIT_AFTER_INACTIVITY` MUST join
  ...
  door. `MAJU_ACP_NO_PRESENCE` (`config.rs:378`) MUST join in the same
...
## The Kubernetes Binding (`maju-backend-kubernetes`)

The first conforming provider: a Rust crate in `heap-cider/maju`, distributed as a
...
generated `maju-agents-<rand6>` as the `namespace` field's schema *default*
...
`ghcr.io/heap-cider/maju-sprig`: Alpine base + `bash` (required by the dev-MCP
shell tool) + `git` + CA certificates + the static musl `sprig` multicall
binary with its personality links (`maju-acp`, `maju-agent`, `maju-dev-mcp`,
`rg`, `tree`, `maju`, `git-credential-nostr`, `git-sign-nostr`) + a baked
...
`ghcr.io/heap-cider/maju-sprig@sha256:<that-digest>` — a `sha-<git-sha>` *tag*
...
**An image override MUST contain the runtime ABI** — the `maju-acp`
entrypoint and everything §Entrypoint and launch ABI requires — not merely
alternate-harness dependencies. A conforming custom image is "maju-sprig
...
exec maju-acp   # exec, not a call — maju-acp must be PID 1
```

`bash -c "setup && maju-acp"` (no `exec`) is non-conforming: bash becomes
...
| source | env var |
|---|---|
| `relay_url` | `MAJU_RELAY_URL` |
| `private_key_nsec` | `MAJU_PRIVATE_KEY` and `NOSTR_PRIVATE_KEY` (the git helpers read the latter) |
| `auth_tag` | `MAJU_AUTH_TAG` (omitted when null; then `launch.owner_pubkey` → `MAJU_ACP_AGENT_OWNER` is REQUIRED — §Launch data owner rule) |
| `launch.command` | `MAJU_ACP_AGENT_COMMAND` — the *name*, resolved against the image's own PATH; never a forwarded host path |
| `launch.args` | `MAJU_ACP_AGENT_ARGS`, comma-joined |
| `launch.env`, `launch.policy_env` | verbatim, at their precedence tiers |
| generation token (§K8s Secrets) | `MAJU_MANAGED_AGENT_START_NONCE` — the lifecycle-frame correlator and the Secret generation are one identity (§Launch data tier 3) |
| `system_prompt`, `idle_timeout_seconds`, `max_turn_duration_seconds`, `parallelism` | **not mapped by the provider** — the desktop resolves these into `launch.policy_env` (`MAJU_ACP_SYSTEM_PROMPT`, `MAJU_ACP_IDLE_TIMEOUT`, `MAJU_ACP_MAX_TURN_DURATION`, `MAJU_ACP_AGENTS`). `MAJU_ACP_AGENTS` carries the **effective** parallelism (`min(record.parallelism, harness_cap)`), is reserved (`env_vars.rs:RESERVED_ENV_KEYS`), and cannot be overridden by user env. The remaining knobs are tier-1 by local fact (written before user env); a provider that mapped the top-level copies after `launch.env` would silently defeat local overrides. The top-level fields remain as display/bookkeeping inputs only |
| `turn_timeout_seconds` | not mapped — deprecated upstream and ignored; the local spawn also does not emit it |
| `respond_to` | `MAJU_ACP_RESPOND_TO` |
| `respond_to_allowlist` | `MAJU_ACP_RESPOND_TO_ALLOWLIST`, comma-joined |
| — | `MAJU_ACP_MCP_COMMAND=maju-dev-mcp` (image-local; the dev-MCP requirement) |
| `provider_config.inactivity_seconds` | `MAJU_ACP_EXIT_AFTER_INACTIVITY` (schema default 7200; the I5 opt-in, §Auto-Stop — the config field and this env var are one knob, not two) |
...
(per-runtime vars, `provider_locked` suppression, `MAJU_ACP_MODEL`) arrives
...
**Encoding honesty note.** `MAJU_ACP_AGENT_ARGS` is comma-delimited by the
...
- **Naming/labeling — the exact contract** (63-char label-value limit; a hex
  ...
  - pod name: `maju-agent-<first-12-hex-of-pubkey>` — also the returned
    `agent_id`
  - label `maju.heap-cider.github.io/agent-pubkey: <first-32-hex>` — the selector key
    ...
    not decorative
  - label `app.kubernetes.io/managed-by: maju-backend-kubernetes` and label
    `maju.heap-cider.github.io/binding-version: <schema-version>` — the **management
  ...
  - annotation `maju.heap-cider.github.io/agent-pubkey-full: <full-64-hex>` —
  ...
  - annotation `maju.heap-cider.github.io/create-intent: <sha256-of-intent-template>` — the
    ...
    never-started pods
  - Secret name: `maju-agent-<first-12-hex>-<gen>`, where `<gen>` is a random
...
(`maju-agent-<first-12-hex>-<gen>`, §Pod shape). The rationale is a
...
`MAJU_MANAGED_AGENT_START_NONCE`, so lifecycle frames and pod logs share a
...
2. It does not suppress the harness's promises on a remote agent:
   presence stays enabled (`MAJU_ACP_NO_PRESENCE` never set — remotely,
   presence is the only signal, I3), and the inactivity knob
   (`MAJU_ACP_EXIT_AFTER_INACTIVITY`) carries the owner's *deliberate*
...
3. **Deploy payload bypasses the launch resolver** (the prerequisite this
   ...
   policy (`MAJU_ACP_RELAY_OBSERVER`, runtime `default_env` such as
   ...
   (`MAJU_ACP_DEDUP`/`MAJU_ACP_MULTIPLE_EVENT_HANDLING` are *not* on this
...
4. **The I5 reaper does not exist, and its natural home is a trap**
   (harness code prerequisite). `MAJU_ACP_EXIT_AFTER_INACTIVITY` appears
...
| Presence publish / offline-on-exit | `crates/maju-acp/src/lib.rs` (`publish_presence`, shutdown path) |
| `!shutdown` owner check | `crates/maju-acp/src/lib.rs` (main loop) |
| Graceful shutdown path (budget enforcement *to be added* — Known Defect 7) | `crates/maju-acp/src/lib.rs` (pool shutdown, then drain / reap / presence / relay close) |
| Clean-exit exit-code contract | *to be added*: `crates/maju-acp` distinguished exit codes + pinning test (Known Defect 6; gates `OnFailure`) |
| Auto-stop flag | *to be added*: `crates/maju-acp/src/config.rs` + a pool-independent timer (NOT the `pool_ready`-gated maintenance tick — Known Defect 4) + `RESERVED_ENV_KEYS` entry |
| Kubernetes binding | *to be added*: `crates/maju-backend-kubernetes` |
...
- **G. Remote override semantics** — the spec keeps local semantics: user
  env continues to beat Maju behavior defaults remotely (three-tier
...
- **I. Never-started escape hatch** — the create-intent fingerprint
  ...
  to Maju-authored, positively identified residue** (§Deploy State Machine
...
Remote agents extend Maju's managed-agent model across a deliberately thin
