# Multi-Tenant Maju Relay: A Formal Specification
...
Today a Maju relay *process* is the security boundary: one `DATABASE_URL`, one
...
4. **Unauthenticated global surface.** The NIP-11 relay information document at
   ...
   (`crates/maju-relay/src/nip11.rs:122`) takes only static inputs and
...
**O.WS — WebSocket transport** (`crates/maju-relay/src/protocol.rs:180-215`). The
...
- **`O.REST.BODY`** — JSON response: row content, projection results, and audit
  entries (`crates/maju-audit/src/service.rs:get_entries`) must all be B-labeled.
...
- **L7 — Error label.** A finite, statically-declared alphabet `Σ_err` governs the
  *authenticated, tenant-scoped* WS error surface: every `O.WS.OK.message`,
  ...
  property). Today `RelayError::Database(#[from] maju_db::DbError)` (`error.rs:11`)
...
- **(P3)** *NIP-98 mint freshness.* A NIP-98 mint event (kind:27235) is accepted
  ...
  within ±60s of server time (`maju-auth/src/nip98.rs:77-83`,
  `TIMESTAMP_TOLERANCE_SECS = 60`) **and** a seen-set keyed on event id
  (`maju-relay/src/api/bridge.rs::check_nip98_replay`), whose cache TTL (120s,
...
- **P-SIG / A_HASH** are the standard Nostr crypto assumptions; admitted by using
  the audited libraries the rest of Maju uses.
- **P3** is admitted by the NIP-98 handler enforcing *both* timestamp-range
  ...
  2. **Per-pod scope.** The seen-set is `Arc<AppState>`-scoped, not cross-pod, so
     ...
     `deploy/charts/maju/examples/argocd-app.yaml:27` and
     `deploy/charts/maju/examples/flux-helmrelease.yaml:35`) are
...
- **I1 / I4** — every DB entry point takes `TenantContext` and `SET LOCAL
  ...
  (`crates/maju-db/src/channel.rs:545-560`, which unions every open channel in the
...
- **S3 / S4** — the relay keypair becomes a per-community signing key
  ...
  global audit chain (`crates/maju-audit/src/service.rs`) becomes N per-community
...
- **P3 / S2** — the NIP-98 mint freshness obligation the Tamarin model abstracts
  ...
  `crates/maju-auth/src/nip98.rs:77-83` and the event-id seen-set
  `check_nip98_replay` in `crates/maju-relay/src/api/bridge.rs:76-94`, called
...
- **C2.4** — the NIP-11 builder `RelayInfo::build`
  (`crates/maju-relay/src/nip11.rs:122`) must keep its relay-static-only signature
...
- **Redis / subscription refinement** — Redis pub/sub keys, presence keys, typing
  ...
  `maju:{community}:channel:{channel_id}`,
  `maju:{community}:presence:{pubkey}`, and
  `maju:{community}:typing:{channel_id}`. The current unprefixed keys are
...
#### G1 — establishment (`crates/maju-relay/src/handlers/req.rs:79-204`)
...
#### G2 — delivery (`crates/maju-relay/src/handlers/event.rs:59-113`)
