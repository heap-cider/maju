...
Maju-agent supports lifecycle hooks — MCP tools that the agent calls at
...
| `MAJU_AGENT_HOOK_TIMEOUT_MS` | 2500 | Per-hook call timeout in milliseconds |
| `MAJU_AGENT_STOP_MAX_REJECTIONS` | 3 | Per-prompt `_Stop` budget (0 = disable) |
...
`maju-agent` has one in-process objection at the `_Stop` gate that is **not** an
MCP hook and exposes no hook tool: the reply guard
(`MAJU_AGENT_REQUIRE_REPLY=1`), which reminds the model to publish when a turn is
about to end with nothing posted to Maju. There is no `_ReplyGuard` tool to
implement and no server to allowlist — the env var and the recognition contract
are documented in
[crates/maju-agent/README.md](../crates/maju-agent/README.md#reply-guard).

It is mentioned here only because it shares this lifecycle point and this
budget: its reminders count against `MAJU_AGENT_STOP_MAX_REJECTIONS` like any
