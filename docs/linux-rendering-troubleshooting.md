...
| Blank window on startup, no crash output | dmabuf renderer incompatibility (NVIDIA or AppImage) | `WEBKIT_DISABLE_DMABUF_RENDERER=1 ./Maju.AppImage` or `--safe-rendering` |
...
**Conflict detection:** If you set a WebKit variable in your environment and also pass `--safe-rendering`, Maju will refuse to start and print exactly which variable conflicts. Unset the conflicting variable or drop the flag.
...
**Workaround (verified by reporter):** Set these three variables before launching Maju:
