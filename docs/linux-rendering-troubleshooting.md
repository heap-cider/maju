...
**Affected distributions:** Fedora 40+ and any distro shipping Google's Noto Color Emoji in COLRv1 format (`Noto-COLRv1.ttf`). Issues [#2548](https://github.com/heap-cider/maju/issues/2548), [#2982](https://github.com/heap-cider/maju/issues/2982).

**Symptom:** Maju starts, the window appears briefly (or stays blank), then the process aborts with output like:
...
**Fix:** Upgrade to the latest AppImage (v0.5.2+). The build container was bumped to `ubuntu:24.04` ([#3602](https://github.com/heap-cider/maju/pull/3602)), which ships FreeType 2.13.2. The compiled layout now matches every crash-affected host (FreeType ≥ 2.13), eliminating the ABI mismatch.
...
**Workaround (before upgrading):** Add a fontconfig override that removes color-format fonts from Maju's view:

```bash
mkdir -p ~/.config/maju-fontconfig
cat > ~/.config/maju-fontconfig/fonts.conf <<'XML'
<?xml version="1.0"?>
...
XML
FONTCONFIG_FILE=~/.config/maju-fontconfig/fonts.conf ./Maju_*.AppImage
```

**Native packages (`deb`/`rpm`):** The COLRv1 crash ([#2548](https://github.com/heap-cider/maju/issues/2548), [#2982](https://github.com/heap-cider/maju/issues/2982)) is AppImage-only — native packages use the system WebKit, which has a consistent FreeType ABI, and are not affected.

---

## Blank window on startup (no crash): dmabuf renderer

**Affected hardware:** NVIDIA GPUs (proprietary and nouveau drivers) and AppImage installs on any GPU. Issue [#2338](https://github.com/heap-cider/maju/issues/2338).

**Symptom:** Maju launches without any crash or assertion output, but the window is blank or invisible. The process is running (`ps aux | grep maju`), but nothing renders.
...
**Fix:** Maju sets `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1` automatically before WebKit initializes when it detects an NVIDIA GPU (`/sys/class/drm` vendor ID `0x10de`) or when running as an AppImage. That keeps SharedMemory in WebKit's transport set while skipping the hardware dmabuf path. Debian/Ubuntu's NVIDIA dmabuf patch can bypass this, so the crash can persist there. `WEBKIT_DMABUF_RENDERER_FORCE_SHM` exists in WebKitGTK ≥ 2.44 (absent at 2.42); on older system WebKit the export is a silent no-op.
...
```bash
./Maju_*.AppImage --safe-rendering
# or for a native install:
maju-desktop --safe-rendering
```
...
**Conflict detection:** If you set a WebKit variable in your environment and also pass `--safe-rendering`, Maju will refuse to start and print exactly which variable conflicts. Unset the conflicting variable or drop the flag. Operators who previously exported `WEBKIT_DISABLE_DMABUF_RENDERER=0` to override the old heuristic can keep that — the module still treats that assignment as a user takeover.

---

## AMD RDNA4 / transparent window

**Affected hardware:** AMD RDNA4 GPUs (RX 9000 series) with the `radv` driver. Issue [#2643](https://github.com/heap-cider/maju/issues/2643).

**Symptom:** The Maju window is transparent or renders with graphical corruption on AMD RDNA4 hardware.

**Workaround (recommended; FORCE_SHM swap not re-verified on RDNA4):** Set these three variables before launching Maju. The reporter originally verified a three-var set that used `WEBKIT_DISABLE_DMABUF_RENDERER=1`; that var is the [#3654](https://github.com/block/buzz/issues/3654) crash on current WebKitGTK, so this recipe swaps in `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1` instead. Please re-confirm on RDNA4 if you can.
...
A dedicated fix for RDNA4 detection is being tracked in [#2643](https://github.com/heap-cider/maju/issues/2643).
...
1. Run Maju from a terminal and capture the output:
   ```bash
   ./Maju_*.AppImage 2>&1 | tee maju-crash.log
   ```
...
4. File a [new issue](https://github.com/heap-cider/maju/issues/new) with your distro, GPU, driver version, and the terminal output.
