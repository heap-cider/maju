...
- **`at+jwt` access token**: a Maju-resource access token whose protected
  `typ` is exactly `at+jwt` and whose `aud` contains the configured Maju
...
- **dedicated Maju assertion**: a separately minted assertion whose protected
