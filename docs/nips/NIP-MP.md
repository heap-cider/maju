...
Maju renders one card per `kind:30617`, so "the platform" — a relay, a desktop app, and a mobile app — appears as three unrelated repositories. Real work spans repositories; the model does not.

[VISION_PROJECTS.md](../../VISION_PROJECTS.md) sets the bar as "standard kinds as substrate, custom kinds only where genuinely novel," and every other forge concept in Maju clears it: repositories, patches, issues, statuses, and ref state are all standard NIP-34 kinds. Multi-repository grouping is the one semantic that cannot be:
...
| This repository (`crates/maju-core/src/kind.rs`) | full range | `30620` is `KIND_WORKFLOW_DEF`, `30622` is `KIND_DM_VISIBILITY` (NIP-DV). `30621` is the one free number between them. |

Both external registries are advisory, not authoritative allocators: neither reserves numbers, and an unregistered kind may still be in use by an unpublished client. A future upstream assignment of `30621` would be a collision Maju absorbs the same way it already does for its other custom kinds — the number is Maju-specific, and interoperability rests on the member `kind:30617` events, which remain standard.
...
```jsonc
{
  ...
  "tags": [
    ...
    ["a", "30617:<owner-a-pubkey-hex>:maju"],
    ["a", "30617:<owner-b-pubkey-hex>:maju-infra"],
    ["maju-channel", "<channel-uuid>"],
    ["maju-visibility", "listed"]
  ]
}
```
...
| `maju-channel` | 0 or 1 | UUID of the channel this project's discussion lives in. Metadata only — see [Authority](#authority). At most 256 bytes. |
| `maju-visibility` | 0 or 1 | `listed` (default) or `unlisted`. Feeds [listing eligibility](#listing-eligibility). At most 256 bytes. |
...
Ingest bounds metadata cardinality and length; it interprets no metadata value. `maju-channel` and `maju-visibility` are opaque strings to a relay, exactly as they are on `kind:30617`. Interpretation is a client concern, and every client MUST resolve it the same way:

- `name` absent → clients display the `d` value.
- `maju-visibility` absent or holding any value other than `listed` or `unlisted` → treated as `listed`. An unrecognized token MUST NOT hide a project: a typo in a metadata field is not a privacy signal, and treating it as one would make a project vanish for reasons its author cannot see.
- `maju-channel` absent, or naming a channel the viewer cannot resolve or read → the project renders without a channel link. It MUST NOT be dropped from the collection, and the unresolvable value MUST NOT be surfaced as a broken link.
...
Maju-hosted repositories cannot currently produce such a coordinate: their `d` values are validated as `[a-zA-Z0-9._-]{1,64}` (`crates/maju-relay/src/handlers/side_effects.rs`, `crates/maju-sdk/src/builders.rs`). The tolerance is for the repositories this NIP does not control — NIP-34 announcements from other clients, and any future relaxation of Maju's own rule — and it matches how Maju already parses coordinates in NIP-09 deletion handling, so a project coordinate and a deletion coordinate can never disagree about where a repository's `d` value begins.
...
`maju-channel` on a project is **metadata only**. Git push policy reads the `maju-channel` of the repository's own `kind:30617` (`crates/maju-relay/src/api/git/policy.rs`); a project neither overrides that binding nor supplies one to a member that lacks it. A project's channel binding therefore cannot widen or narrow push access to anything.
...
**Who may delete.** The project signer always may. On the Maju relay, so may the signer's registered NIP-OA owner: `validate_standard_deletion_event` resolves the deletion's effective author and accepts it when that actor is the target pubkey's registered owner (`crates/maju-relay/src/handlers/side_effects.rs`). This is a **Maju relay extension to NIP-09**, applied uniformly to every kind rather than specially to projects — it is what lets a human clean up events published by an agent they own. Vanilla NIP-09 relays accept only the signer, so a project deleted through the owner path on Maju will still be live on a relay that lacks the extension.
...
2. **`d-empty`** — the `d` value is non-empty. Same collapse hazard. Its length is bounded by the relay's existing generic `d`-tag limit (`maju_db::event::D_TAG_MAX_LEN`, 1024 bytes); this NIP adds no second bound.
3. **`member-cap`** — at most 64 member `a` tags, counting **every** `a` tag rather than distinct coordinates. Counting distinct coordinates would leave parse volume bounded only by the relay frame limit (512 KiB by default, `crates/maju-relay/src/config.rs`), since a duplicate-heavy event could carry thousands of tags naming one coordinate. The cap is inclusive: 64 is accepted, 65 is not.
...
7. **`metadata-cardinality`** — at most one each of `name`, `description`, `maju-channel`, `maju-visibility`. Duplicates would make the effective value reader-dependent.
8. **`metadata-length`** — `name` at most 256 bytes; `description` at most 2048 bytes; `maju-channel` at most 256 bytes; `maju-visibility` at most 256 bytes. The two `maju-` bounds are generous by design: neither value has a semantic length, and the bound exists only so an unbounded string cannot ride into storage on a tag ingest does not interpret.
...
Three checks land in the Maju validator together with the fixture wiring that exercises them: the `maju-channel` and `maju-visibility` bounds in rule 8, and rule 4's arity. The validator bounds `name` and `description` today, and reads element 1 of each member `a` tag while ignoring any element past it.
...
**Routing.** `kind:30621` is global-only, like every other NIP-34 kind in Maju: it is addressed by `(pubkey, kind, d)` and is never channel-scoped. A stray `h` tag MUST NOT scope it to a channel — the `maju-channel` tag is a metadata reference, not a routing directive.
...
**Deletion** follows NIP-09 with two Maju-wide behaviors that are not project-specific:
...
- its `maju-visibility` is `unlisted`, or
...
- **NIP-01**: Supplies the addressable-event class, the `a` tag grammar, addressing, replacement, and the owner-only editing model. Owner-only editing is not enforcement code in Maju — it is what NIP-01 replacement already means.
- **NIP-09**: Supplies container deletion, which deletes the container only. Maju extends it in two ways that are not project-specific: an agent's registered NIP-OA owner may also delete, and a tombstone applies only at or before its own `created_at` ([Deletion](#deletion)).
- **NIP-29**: Supplies the channel a project's `maju-channel` names. The reference is metadata; project state is never channel-scoped.
