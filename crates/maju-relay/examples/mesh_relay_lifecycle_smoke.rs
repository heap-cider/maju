//! Relay-driven mesh lifecycle smoke — the full Maju join story, CI-shaped.
//!
...
//!      maju-relay (kind:13534 roster via maju-admin); a third is not.
...
//! cargo build --profile ci -p maju-admin
//! MAJU_ADMIN_BIN=target/ci/maju-admin \
//!   cargo run --profile ci -p maju-relay --example mesh_relay_lifecycle_smoke
//! ```
...
/// (`KIND_MAJU_MESH_MEMBER_STATUS` in the desktop coordinator).
...
const STATUS_K_TAG: &str = "maju-mesh-status";
...
fn member_endpoint_binding_bytes(member_pubkey: &str, endpoint_tokens: &[String]) -> Vec<u8> {
    ...
    format!(
        "maju-mesh-owner-endpoint-binding-v1:{}:{}",
        member_pubkey.trim().to_ascii_lowercase(),
...
async fn role_serve() -> anyhow::Result<()> {
    ...
    let keys = Keys::parse(&env("MAJU_MEMBER_NSEC")?)?;
    ...
    let mut relay = MajuTestClient::connect(&relay_ws_url(), &keys)
        .await
    ...
    let cfg = serve::EmbeddedServeConfig::builder()
        ...
        // auto-discover. The Maju relay is the only discovery surface.
        .publish(false)
        .auto_join(false)
...
async fn role_stranger() -> anyhow::Result<()> {
    ...
    match MajuTestClient::connect(&relay_ws_url(), &keys).await {
        Err(error) => {
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let admin =
        std::env::var("MAJU_ADMIN_BIN").unwrap_or_else(|_| "target/ci/maju-admin".to_string());
    anyhow::ensure!(
        std::path::Path::new(&admin).exists(),
        "maju-admin binary not found at {admin} (set MAJU_ADMIN_BIN)"
    );

    let scratch = std::env::temp_dir().join(format!("maju-mesh-lifecycle-{}", std::process::id()));
    ...
    // MEMBERSHIP: A and B become relay members via maju-admin (publishes the
    ...
    for (label, keys) in [("A", &member_a), ("B", &member_b)] {
        let status = Command::new(&admin)
            ...
            .status()?;
        anyhow::ensure!(status.success(), "maju-admin add-member {label} failed");
        eprintln!(
    ...
    let mut serve_child = Command::new(&exe)
        ...
        .env("MAJU_MEMBER_NSEC", secret_hex(&member_a))
    ...
    let mut client_child = Command::new(&exe)
        ...
        .env("MAJU_MEMBER_NSEC", secret_hex(&member_b))
    ...
    let mut stranger_child = Command::new(&exe)
        ...
        .env("MAJU_MEMBER_NSEC", secret_hex(&stranger))
