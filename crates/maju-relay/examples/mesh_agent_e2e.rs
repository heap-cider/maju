...
//!   P4 agentic tool use     — agent + maju-dev-mcp writes a file on disk.
//!
...
//! `maju-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when maju-agent's default 32768
    // budget met a 32k-context model). 150k output: passes maju-agent's own
    ...
    // P4: agentic tool use via maju-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text MAJU_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("maju-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, "mesh", None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("MAJU_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real maju-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("maju-agent") {
        Ok(agent) => agent,
    ...
    command
        ...
        // model, key, and chat API. Not MAJU_AGENT_REQUIRE_REPLY, which needs
        // Maju's publish tools to mean anything.
        .env("MAJU_AGENT_PROVIDER", "openai")
        .env("MAJU_AGENT_MODEL", model)
        .env("OPENAI_COMPAT_BASE_URL", base)
        .env("OPENAI_COMPAT_MODEL", model)
        .env("OPENAI_COMPAT_API_KEY", "maju-mesh-local")
        .env("OPENAI_COMPAT_API", "chat")
        .env("MAJU_AGENT_MAX_OUTPUT_TOKENS", "4096")
        // No MAJU_AGENT_THINKING_EFFORT: apply_relay_mesh_env() deliberately
    ...
    if let Some(value) = max_output_tokens {
        command.env("MAJU_AGENT_MAX_OUTPUT_TOKENS", value);
    }
