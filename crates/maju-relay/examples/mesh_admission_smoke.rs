...
//!
//! Proves the security claim of Maju's membership-derived mesh admission:
...
//!      same builder calls Maju desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Maju runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("maju-mesh-admission-{}", std::process::id()));
