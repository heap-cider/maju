...
use maju_relay::lifecycle::StartupPhase;
...
impl RelayProcess {
    fn spawn(environment: &[(&str, &str)]) -> Self {
        let scratch_dir =
            std::env::temp_dir().join(format!("maju-boot-lifecycle-{}", uuid::Uuid::new_v4()));
        let mut command = Command::new(env!("CARGO_BIN_EXE_maju-relay"));
        command
            .env_clear()
            .env("RUST_BACKTRACE", "0")
            .env("RUST_LOG", "maju_relay=info")
            .env("MAJU_GIT_REPO_PATH", scratch_dir.join("repos"))
        ...
        let mut child = command.spawn().expect("spawn maju-relay child process");
    ...
    fn wait(mut self, timeout: Duration) -> Output {
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                let _ = child.kill();
                let _ = child.wait();
                panic!("maju-relay child exceeded {timeout:?}");
            }
...
fn wait_for_relay_metrics(process: &mut RelayProcess, port: u16) -> String {
    ...
    loop {
        ...
        if let Ok(response) = scrape_metrics(port) {
            if response.contains("maju_audit_enabled") {
                return response;
            }
        }
        assert!(
...
fn lifecycle_events(output: &Output) -> Vec<Value> {
    let mut events: Vec<Value> = output
        ...
        .filter(|event| event["event_name"] == "maju_process_lifecycle")
        .collect();
...
fn invalid_config_terminalizes_at_main_even_with_logs_disabled() {
    let output = run_relay(&[
        ("RUST_LOG", "off"),
        ("MAJU_BIND_ADDR", "not-a-socket-address"),
    ]);
...
fn config_filesystem_failure_has_a_bounded_terminal() {
    let output = run_relay(&[
        ("RUST_LOG", "off"),
        ("MAJU_GIT_REPO_PATH", "/dev/null/not-a-directory"),
    ]);
    assert!(!output.status.success());
    let events = lifecycle_events(&output);
    assert_accounting(&events);
...
fn invalid_config_value_has_the_same_bounded_terminal() {
    let output = run_relay(&[("RUST_LOG", "off"), ("MAJU_DRAIN_JITTER_MS", "bogus")]);
    assert!(!output.status.success());
    let events = lifecycle_events(&output);
    assert_accounting(&events);
    assert_terminal(&events, "config_load", "failed", Some("config_invalid"));
...
fn invalid_key_uses_a_bounded_reason_without_leaking_the_value() {
    ...
    let output = run_relay(&[("MAJU_RELAY_PRIVATE_KEY", secret)]);
    assert!(!output.status.success());
    let events = lifecycle_events(&output);
    assert_accounting(&events);
...
fn occupied_metrics_port_has_a_typed_bind_terminal() {
    ...
    let output = run_relay(&[
        ("MAJU_RELAY_PRIVATE_KEY", VALID_RELAY_PRIVATE_KEY),
        ("MAJU_METRICS_PORT", &port),
    ]);
    assert!(!output.status.success());
    let events = lifecycle_events(&output);
    assert_accounting(&events);
...
fn otlp_build_failure_is_degraded_without_leaking_endpoint_credentials() {
    ...
    let mut process = RelayProcess::spawn(&[
        ...
        ("RUST_LOG", "maju_relay=warn"),
        ("MAJU_RELAY_PRIVATE_KEY", VALID_RELAY_PRIVATE_KEY),
        ("MAJU_METRICS_PORT", &port_value),
