...
pub enum ConfigError {
    /// The `MAJU_BIND_ADDR` environment variable could not be parsed as a socket address.
    #[error("invalid MAJU_BIND_ADDR: {0}")]
...
///
/// Configured by `MAJU_ADMIN_AUTH`: unset/empty/`nip98` → `Nip98` (fail-secure
...
pub enum AdminAuth {
    ...
    /// Selected by `MAJU_ADMIN_AUTH=disabled`.
    ...
    /// time from config + DB. Selected by `MAJU_ADMIN_AUTH=nip98` or by leaving
    /// `MAJU_ADMIN_AUTH` unset (fail-secure default). Read-write per resolved
...
pub struct Config {
    ...
    /// Replica read budget `B` in milliseconds (`MAJU_REPLICA_READ_MAX_AGE_MS`).
    /// `0` (the default) disables bounded-staleness replica routing; see
    /// [`maju_db::DbConfig::replica_read_max_age_ms`].
    ...
    /// shutdown (`MAJU_DRAIN_JITTER_MS`). Each live connection is closed after
    ...
    ///
    /// The `maju-db` default of 20 was sized for a handful of pods against
    ...
    /// (`MAJU_DB_READ_POOL_SIZE`). Defaults to `db_pool_size`. Sized
    ...
    /// `MAJU_HUDDLE_AUDIO_AVAILABLE=false` until the out-of-relay media/SFU
    /// service lands.
    pub huddle_audio_available: bool,

    /// Inter-relay mesh configuration (`MAJU_MESH`, `MAJU_MESH_BIND_ADDR`).
    /// Opt-in: mesh forms only when `MAJU_MESH=on` is explicit. The default
    ...
    pub mesh: maju_relay_mesh::MeshConfig,

    /// Testbed-only reliable-stream echo consumer (`MAJU_MESH_DEMO_ECHO`).
    ...
    /// Same strict opt-in as `MAJU_MESH`; default off means inbound reliable
    ...
    ///
    /// Default: `false`. Set via `MAJU_ALLOW_NIP_OA_AUTH=true`.
    ...
    pub media: maju_media::MediaConfig,
    ...
    /// Set `MAJU_AUDIT_ENABLED=false` for deployments that do not require it.
    ...
    /// Example: `MAJU_EPHEMERAL_TTL_OVERRIDE=60` → all ephemeral channels expire
    ...
    /// An absent setting selects the canonical Maju gateway. An explicitly
...
fn parse_push_gateway_delivery_url(raw: &str) -> Result<url::Url, ConfigError> {
    let url = url::Url::parse(raw.trim()).map_err(|e| {
        ConfigError::InvalidValue(format!(
            "MAJU_PUSH_GATEWAY_DELIVERY_URL is not a valid URL: {e}"
        ))
    })?;
    if url.scheme() != "https"
        || url.host().is_none()
    ...
    {
        return Err(ConfigError::InvalidValue(
            "MAJU_PUSH_GATEWAY_DELIVERY_URL must be an exact HTTPS /v1/deliveries/apns URL without credentials, query, or fragment"
                .to_string(),
        ));
    }
    Ok(url)
...
fn ensure_git_repo_path(
    ...
    ensure_git_path("MAJU_GIT_REPO_PATH", raw)
}
...
///
/// `MAJU_REQUIRE_MEDIA_GET_AUTH` was the real flag; `MAJU_REQUIRE_MEDIA_READ_AUTH`
...
const INERT_MEDIA_READ_AUTH_VARS: [&str; 2] = [
    "MAJU_REQUIRE_MEDIA_GET_AUTH",
    "MAJU_REQUIRE_MEDIA_READ_AUTH",
];
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        let bind_addr_raw =
            std::env::var("MAJU_BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:3000".to_string());
        ...
        if std::env::var("MAJU_REPLICA_HEAD_MAX_AGE_SECS").is_ok() {
            return Err(ConfigError::InvalidValue(
                "MAJU_REPLICA_HEAD_MAX_AGE_SECS was renamed to MAJU_REPLICA_READ_MAX_AGE_MS \
        ...
        let replica_read_max_age_ms = match std::env::var("MAJU_REPLICA_READ_MAX_AGE_MS") {
            Ok(raw) => raw.trim().parse::<u64>().map_err(|_| {
                ConfigError::InvalidValue(
                    "MAJU_REPLICA_READ_MAX_AGE_MS must be a non-negative integer".to_string(),
                )
            })?,
            Err(_) => 0,
        };
        ...
        let drain_jitter_ms = match std::env::var("MAJU_DRAIN_JITTER_MS") {
            ...
            Ok(raw) => raw
                ...
                .map_err(|_| {
                    ConfigError::InvalidValue(
                        "MAJU_DRAIN_JITTER_MS must be a non-negative integer".to_string(),
                    )
                })?
        ...
        let redis_pool_size = std::env::var("MAJU_REDIS_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(16);

        let db_pool_size = std::env::var("MAJU_DB_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(50);

        let db_read_pool_size = std::env::var("MAJU_DB_READ_POOL_SIZE")
            .ok()
            .and_then(|v| v.parse::<u32>().ok())
            .filter(|&v| v > 0);
        ...
        let pairing_relay_url = std::env::var("MAJU_PAIRING_RELAY_URL")
            .ok()
            ...
            .map(|value| {
                let parsed = url::Url::parse(&value).map_err(|e| {
                    ConfigError::InvalidValue(format!(
                        "MAJU_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL: {e}"
                    ))
                })?;
                if !matches!(parsed.scheme(), "ws" | "wss") || parsed.host_str().is_none() {
                    return Err(ConfigError::InvalidValue(
                        "MAJU_PAIRING_RELAY_URL must be a valid ws:// or wss:// URL".to_string(),
                    ));
                }
                Ok(value)
            })
        ...
        let max_connections = std::env::var("MAJU_MAX_CONNECTIONS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10_000);

        let max_concurrent_handlers = std::env::var("MAJU_MAX_CONCURRENT_HANDLERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1024);

        let send_buffer_size = std::env::var("MAJU_SEND_BUFFER")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1_000);

        let max_frame_bytes = std::env::var("MAJU_MAX_FRAME_BYTES")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&v| v > 0)
            .unwrap_or(DEFAULT_MAX_FRAME_BYTES);

        let slow_client_grace_limit = std::env::var("MAJU_SLOW_CLIENT_GRACE_LIMIT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15);

        let require_auth_token = std::env::var("MAJU_REQUIRE_AUTH_TOKEN")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let pubkey_allowlist_enabled = std::env::var("MAJU_PUBKEY_ALLOWLIST")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let require_relay_membership = std::env::var("MAJU_REQUIRE_RELAY_MEMBERSHIP")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
        ...
        let huddle_audio_available = std::env::var("MAJU_HUDDLE_AUDIO_AVAILABLE")
            .map(|v| !(v == "false" || v == "0"))
            .unwrap_or(true);
        ...
        // `MAJU_MESH=on`; anything else (absent, `off`, other values) keeps
        // exact single-instance behavior.
        let mesh_enabled = std::env::var("MAJU_MESH")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);
        let mesh_bind_addr = std::env::var("MAJU_MESH_BIND_ADDR")
            .map(|raw| {
                raw.parse::<SocketAddr>().map_err(|e| {
                    ConfigError::InvalidValue(format!("invalid MAJU_MESH_BIND_ADDR: {e}"))
                })
            })
            .unwrap_or_else(|_| Ok("0.0.0.0:3478".parse().expect("static default parses")))?;
        let mesh = maju_relay_mesh::MeshConfig {
            enabled: mesh_enabled,
            bind_addr: mesh_bind_addr,
            registry_refresh: std::time::Duration::from_secs(15),
        };

        // Demo echo opt-in: same strict pattern as MAJU_MESH — explicit
        // `on`/`true`/`1` only, anything else (absent, `off`, typos) is off.
        let mesh_demo_echo = std::env::var("MAJU_MESH_DEMO_ECHO")
            .map(|v| v.eq_ignore_ascii_case("on") || v == "true" || v == "1")
            .unwrap_or(false);

        let allow_nip_oa_auth = std::env::var("MAJU_ALLOW_NIP_OA_AUTH")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);

        let klipy = std::env::var("MAJU_KLIPY_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .map(|api_key| KlipyConfig { api_key });

        // Note: intentionally not prefixed with MAJU_ — this is a relay-identity
        ...
        // Note: intentionally not prefixed with MAJU_ — same relay-identity
        ...
        let auth = maju_auth::AuthConfig {
            rate_limits: rate_limit_config_from_env()?,
        };

        if !require_auth_token {
            warn!(
                "MAJU_REQUIRE_AUTH_TOKEN is false — REST API requests bypass token auth. \
                 WebSocket protocol auth is unaffected. Set to true for production."
            );
        }

        let cors_origins = std::env::var("MAJU_CORS_ORIGINS")
            .unwrap_or_default()
            .split(',')
        ...
        let relay_private_key = std::env::var("MAJU_RELAY_PRIVATE_KEY").ok();

        let uds_path = std::env::var("MAJU_UDS_PATH")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty());

        let health_port = std::env::var("MAJU_HEALTH_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(8080);

        let metrics_port = std::env::var("MAJU_METRICS_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(9102);

        let s3_addressing_style = match std::env::var("MAJU_S3_ADDRESSING_STYLE") {
            Ok(value) => value.parse().map_err(ConfigError::InvalidValue)?,
            Err(std::env::VarError::NotPresent) => maju_media::config::S3AddressingStyle::default(),
            Err(std::env::VarError::NotUnicode(_)) => {
                return Err(ConfigError::InvalidValue(
                    "MAJU_S3_ADDRESSING_STYLE must be valid Unicode and one of 'path' or 'virtual'"
                        .to_string(),
                ));
            }
        };
        let media = maju_media::MediaConfig {
            s3_endpoint: std::env::var("MAJU_S3_ENDPOINT")
                .unwrap_or_else(|_| "http://localhost:9000".to_string()),
            s3_access_key: std::env::var("MAJU_S3_ACCESS_KEY")
                .unwrap_or_else(|_| "maju_dev".to_string()),
            s3_secret_key: std::env::var("MAJU_S3_SECRET_KEY")
                .unwrap_or_else(|_| "maju_dev_secret".to_string()),
            s3_bucket: std::env::var("MAJU_S3_BUCKET").unwrap_or_else(|_| "maju-media".to_string()),
            s3_region: std::env::var("MAJU_S3_REGION")
                .or_else(|_| std::env::var("AWS_REGION"))
                .unwrap_or_else(|_| "us-east-1".to_string()),
            s3_addressing_style,
            max_image_bytes: std::env::var("MAJU_MAX_IMAGE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(50 * 1024 * 1024),
            max_gif_bytes: std::env::var("MAJU_MAX_GIF_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(10 * 1024 * 1024),
            max_video_bytes: std::env::var("MAJU_MAX_VIDEO_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(500 * 1024 * 1024),
            max_file_bytes: std::env::var("MAJU_MAX_FILE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(100 * 1024 * 1024),
            public_base_url: std::env::var("MAJU_MEDIA_BASE_URL")
            ...
            upload_records_enabled: std::env::var("MAJU_MEDIA_UPLOAD_RECORDS")
                .map(|v| v == "true" || v == "1")
                .unwrap_or(false),
            upload_ip_header: std::env::var("MAJU_MEDIA_UPLOAD_IP_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
            upload_port_header: std::env::var("MAJU_MEDIA_UPLOAD_PORT_HEADER")
                .ok()
                .map(|s| s.trim().to_lowercase())
                .filter(|s| !s.is_empty()),
        };
        let media_max_concurrent_uploads: usize =
            std::env::var("MAJU_MEDIA_MAX_CONCURRENT_UPLOADS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(8);
        let media_max_concurrent_uploads_per_pubkey: u32 =
            std::env::var("MAJU_MEDIA_MAX_CONCURRENT_UPLOADS_PER_PUBKEY")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|&v| v > 0)
                .unwrap_or(2)
                .min(u32::try_from(media_max_concurrent_uploads).unwrap_or(u32::MAX));
        let media_uploads_per_minute: u32 = std::env::var("MAJU_MEDIA_UPLOADS_PER_MINUTE")
            .ok()
            .and_then(|v| v.parse().ok())
            .filter(|&v| v > 0)
            .unwrap_or(30);
        ...
        let ephemeral_ttl_override = std::env::var("MAJU_EPHEMERAL_TTL_OVERRIDE")
            .ok()
        ...
        if let Some(ttl) = ephemeral_ttl_override {
            warn!(
                "MAJU_EPHEMERAL_TTL_OVERRIDE={ttl}s — all ephemeral channels will use \
        ...
        let git_repo_path = ensure_git_repo_path(
            std::env::var("MAJU_GIT_REPO_PATH").unwrap_or_else(|_| "./repos".to_string()),
        )?;
        let git_pack_cache_path = ensure_git_path(
            "MAJU_GIT_PACK_CACHE_PATH",
            std::env::var("MAJU_GIT_PACK_CACHE_PATH")
                .map(std::path::PathBuf::from)
                .unwrap_or_else(|_| git_repo_path.join(".pack-cache")),
        )?;
        let git_max_pack_bytes: u64 = std::env::var("MAJU_GIT_MAX_PACK_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(500 * 1024 * 1024); // 500 MB
        let git_max_repo_bytes: u64 = std::env::var("MAJU_GIT_MAX_REPO_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_pack_bytes.saturating_mul(2)); // 1 GB at defaults
        let git_pack_cache_max_bytes: u64 = std::env::var("MAJU_GIT_PACK_CACHE_MAX_BYTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or_else(|| git_max_repo_bytes.saturating_mul(5)); // 5 GB at defaults
        let git_pack_cache_max_concurrent_populations: usize =
            std::env::var("MAJU_GIT_PACK_CACHE_MAX_CONCURRENT_POPULATIONS")
                .ok()
                .and_then(|v| v.parse().ok())
                .filter(|value| *value > 0)
                .unwrap_or(2);
        let git_max_repos_per_pubkey: u32 = std::env::var("MAJU_GIT_MAX_REPOS_PER_PUBKEY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(100);
        let git_max_concurrent_ops: usize = std::env::var("MAJU_GIT_MAX_CONCURRENT_OPS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(20);
        let git_hook_hmac_secret: String = std::env::var("MAJU_GIT_HOOK_HMAC_SECRET")
            .unwrap_or_else(|_| {
                // Generate a random secret if not configured (dev mode).
                let secret: [u8; 32] = rand::random();
                hex::encode(secret)
            });
        let push_enabled = parse_bool("MAJU_PUSH_ENABLED", false)?;
        let push_executor_key_id =
            std::env::var("MAJU_PUSH_EXECUTOR_KEY_ID").unwrap_or_else(|_| "relay-v1".to_string());
        if push_executor_key_id.is_empty() || push_executor_key_id.len() > 64 {
            return Err(ConfigError::InvalidValue(
                "MAJU_PUSH_EXECUTOR_KEY_ID must contain 1..=64 bytes".to_string(),
            ));
        }
        let push_gateway_delivery_url = match std::env::var("MAJU_PUSH_GATEWAY_DELIVERY_URL") {
            Ok(raw) if raw.trim().is_empty() && push_enabled => {
                return Err(ConfigError::InvalidValue(
                    "MAJU_PUSH_GATEWAY_DELIVERY_URL must not be empty when MAJU_PUSH_ENABLED=true"
                        .to_string(),
                ));
            }
        ...
        let push_gateway_timeout_millis = match std::env::var("MAJU_PUSH_GATEWAY_TIMEOUT_MS") {
            Ok(raw) => raw
                .parse::<u64>()
                .ok()
                ...
                .ok_or_else(|| {
                    ConfigError::InvalidValue(
                        "MAJU_PUSH_GATEWAY_TIMEOUT_MS must be an integer in 100..=10000"
                            .to_string(),
                    )
                })?,
            Err(_) => 2_000,
        ...
        let terms_markdown = read_policy_markdown("MAJU_TERMS_OF_SERVICE_MARKDOWN")?;
        let privacy_markdown = read_policy_markdown("MAJU_PRIVACY_POLICY_MARKDOWN")?;
        let age_attestation_required = parse_optional_bool("MAJU_AGE_ATTESTATION_REQUIRED")?;
        let audit_enabled = parse_bool("MAJU_AUDIT_ENABLED", true)?;
        ...
        let admin = match std::env::var("MAJU_ADMIN_HOST")
            .ok()
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
        {
            None => {
                if std::env::var_os("MAJU_ADMIN_TOKEN").is_some() {
                    tracing::warn!(
                        "MAJU_ADMIN_TOKEN is set but token authentication was removed — \
                         the value is ignored; the admin API now supports only \
                         MAJU_ADMIN_AUTH=nip98 (default) or disabled; remove \
                         MAJU_ADMIN_TOKEN from the environment"
                    );
                }
                if std::env::var_os("MAJU_ADMIN_AUTH").is_some() {
                    tracing::warn!(
                        "MAJU_ADMIN_AUTH is set without MAJU_ADMIN_HOST — \
            ...
            Some(host) => {
                if host.contains(['/', '\\', '@']) {
                    return Err(ConfigError::InvalidValue(
                        "MAJU_ADMIN_HOST must be an exact authority".to_string(),
                    ));
                }
                ...
                if !host.starts_with('[') && host.matches(':').count() > 1 {
                    return Err(ConfigError::InvalidValue(format!(
                        "MAJU_ADMIN_HOST={host} looks like a bare IPv6 literal; \
                ...
                // mixed-case MAJU_ADMIN_HOST round-trips correctly through desktop
                ...
                if !is_bare_authority {
                    return Err(ConfigError::InvalidValue(format!(
                        "MAJU_ADMIN_HOST={host} is not a valid URL authority; \
                ...
                // Parse MAJU_ADMIN_AUTH. Accepted values: "nip98" (default when
                // unset or empty) and "disabled". Any other value is a startup
                // error (typo-proofing). Token authentication was removed —
                // MAJU_ADMIN_TOKEN in the environment is ignored with a startup
                ...
                let auth = match std::env::var("MAJU_ADMIN_AUTH")
                    .ok()
                    .as_deref()
                    .map(str::trim)
                {
                    ...
                    Some("disabled") => {
                        tracing::warn!(
                            "MAJU_ADMIN_AUTH=disabled — the admin API is \
                    ...
                    Some(other) => {
                        return Err(ConfigError::InvalidValue(format!(
                            "MAJU_ADMIN_AUTH must be \"nip98\" or \"disabled\"; got \"{other}\""
                        )))
                    }
                };

                let web_dir = std::env::var("MAJU_ADMIN_WEB_DIR")
                    .ok()
                ...
                if let Some(ref dir) = web_dir {
                    if !dir.join("index.html").is_file() {
                        return Err(ConfigError::InvalidValue(format!(
                            "MAJU_ADMIN_WEB_DIR={} does not contain index.html",
                            dir.display()
                        )));
                    }
                }
        ...
        let web_dir = std::env::var("MAJU_WEB_DIR")
            .ok()
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .map(std::path::PathBuf::from);
        let serve_git_web_gui = std::env::var("MAJU_SERVE_GIT_WEB_GUI")
        ...
        if let Some(ref dir) = web_dir {
            if !dir.join("index.html").is_file() {
                return Err(ConfigError::InvalidValue(format!(
                    "MAJU_WEB_DIR={} does not contain index.html",
                    dir.display()
                )));
            }
            tracing::info!("MAJU_WEB_DIR={} — serving web UI from relay", dir.display());
        }
        ...
        // only fires when someone sets MAJU_GIT_HOOK_HMAC_SECRET to a weak value.
        if std::env::var("MAJU_GIT_HOOK_HMAC_SECRET").is_ok() && git_hook_hmac_secret.len() < 32 {
            return Err(ConfigError::InvalidValue(
                "MAJU_GIT_HOOK_HMAC_SECRET must be at least 32 characters (16 bytes hex)"
                    .to_string(),
            ));
        }
...
mod tests {
    ...
    fn inert_media_read_auth_vars_are_reported_even_when_false() {
        let found = inert_env_vars(
            ...
            env_of(&[("MAJU_REQUIRE_MEDIA_GET_AUTH", "false")]),
        );

        assert_eq!(found, vec!["MAJU_REQUIRE_MEDIA_GET_AUTH"]);
    }

    /// `MAJU_REQUIRE_MEDIA_READ_AUTH` was advertised in `.env.example` as an
    ...
    fn inert_media_read_auth_vars_include_the_documented_alias() {
        let found = inert_env_vars(
            &INERT_MEDIA_READ_AUTH_VARS,
            env_of(&[
                ("MAJU_REQUIRE_MEDIA_GET_AUTH", "true"),
                ("MAJU_REQUIRE_MEDIA_READ_AUTH", "false"),
            ]),
        );
    ...
    fn inert_media_read_auth_vars_stay_quiet_when_unset() {
        let found = inert_env_vars(
            &INERT_MEDIA_READ_AUTH_VARS,
            env_of(&[("MAJU_REQUIRE_RELAY_MEMBERSHIP", "true")]),
        );
    ...
    fn defaults_are_valid() {
        ...
        assert_eq!(
            ...
            maju_media::config::S3AddressingStyle::Path,
    ...
    fn config_with_admin_env(values: &[(&str, Option<&str>)]) -> Result<Config, ConfigError> {
        const KEYS: [&str; 3] = ["MAJU_ADMIN_HOST", "MAJU_ADMIN_TOKEN", "MAJU_ADMIN_AUTH"];
    ...
    /// warning fired. The `MAJU_ADMIN_TOKEN` warning is the sole behavioral
    ...
    /// Assert `captured` contains a WARN naming the removal of `MAJU_ADMIN_TOKEN`
    ...
    fn assert_admin_token_removal_warning(captured: &str) {
        ...
        for needle in ["MAJU_ADMIN_TOKEN", "removed", "ignored"] {
            assert!(
    ...
    /// `MAJU_ADMIN_TOKEN` is now ignored with a startup warning and never
    ...
    fn admin_token_set_is_ignored_and_warns_at_startup() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // Token authentication was removed. A lingering MAJU_ADMIN_TOKEN with a
        ...
        for (auth, expected) in [
            ...
            let (config, logs) = config_with_admin_env_capturing_logs(&[
                ("MAJU_ADMIN_HOST", Some("admin.example")),
                ("MAJU_ADMIN_TOKEN", Some(SOME_ADMIN_TOKEN)),
                ("MAJU_ADMIN_AUTH", auth),
            ]);
            let admin = config
                .unwrap_or_else(|e| {
                    panic!("MAJU_ADMIN_TOKEN with auth={auth:?} must be ignored: {e:?}")
                })
                .admin
            ...
            assert_eq!(
                ...
                "MAJU_ADMIN_TOKEN must not change auth mode for auth={auth:?}"
            );
    ...
    fn admin_surface_defaults_to_nip98_when_auth_unset() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let admin = config_with_admin_env(&[("MAJU_ADMIN_HOST", Some("admin.example"))])
            .expect("config with an admin host and no MAJU_ADMIN_AUTH")
            .admin
            .expect("admin surface is configured");
        assert_eq!(admin.host, "admin.example");
        assert!(
            matches!(admin.auth, crate::config::AdminAuth::Nip98),
            "unset MAJU_ADMIN_AUTH must default to nip98 (fail-secure)"
        );
    }
    ...
    fn admin_host_bare_ipv6_literal_fails_closed() {
        ...
        for host in ["::1", "::1:3000", "fe80::1", "2001:db8::1"] {
            let result = config_with_admin_env(&[("MAJU_ADMIN_HOST", Some(host))]);
            assert!(
                matches!(
                    result,
                    Err(ConfigError::InvalidValue(ref message))
                        if message.contains("MAJU_ADMIN_HOST") && message.contains("bracket")
                ),
        ...
        for host in [
            ...
            assert!(
                matches!(
                    result,
                    Err(ConfigError::InvalidValue(ref message))
                        if message.contains("MAJU_ADMIN_HOST") && message.contains("valid URL authority")
                ),
    ...
    fn admin_host_bracketed_ipv6_literal_is_accepted() {
        ...
        for host in ["[::1]", "[::1]:3000", "[2001:db8::1]:8443"] {
            let admin = config_with_admin_env(&[("MAJU_ADMIN_HOST", Some(host))])
    ...
    fn admin_host_mixed_case_is_normalized_to_lowercase() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // Hostnames are case-insensitive (RFC 4343). A mixed-case MAJU_ADMIN_HOST
        ...
        for (input, expected) in [
            ...
            let admin = config_with_admin_env(&[("MAJU_ADMIN_HOST", Some(input))])
    ...
    fn admin_token_without_a_host_is_ignored_and_warns() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // Even without MAJU_ADMIN_HOST, a lingering MAJU_ADMIN_TOKEN is ignored
        ...
        let (config, logs) = config_with_admin_env_capturing_logs(&[
            ("MAJU_ADMIN_HOST", None),
            ("MAJU_ADMIN_TOKEN", Some(SOME_ADMIN_TOKEN)),
        ]);
        let admin = config
            .expect("MAJU_ADMIN_TOKEN without a host is ignored, not a startup error")
            .admin;
        assert!(
            admin.is_none(),
    ...
    fn disabled_mode_activates_without_a_token() {
        ...
        let admin = config_with_admin_env(&[
            ("MAJU_ADMIN_HOST", Some("admin.example")),
            ("MAJU_ADMIN_TOKEN", None),
            ("MAJU_ADMIN_AUTH", Some("disabled")),
        ])
    ...
    fn admin_auth_junk_values_all_fail_closed() {
        ...
        for junk in [
            ...
            let result = config_with_admin_env(&[
                ("MAJU_ADMIN_HOST", Some("admin.example")),
                ("MAJU_ADMIN_TOKEN", None),
                ("MAJU_ADMIN_AUTH", Some(junk)),
            ]);
            assert!(
                matches!(
                    result,
                    Err(ConfigError::InvalidValue(ref message))
                        if message.contains("MAJU_ADMIN_AUTH")
                ),
    ...
    fn admin_auth_empty_string_defaults_to_nip98() {
        // An empty value (e.g. `MAJU_ADMIN_AUTH=`) is treated as unset → nip98,
        // the fail-secure default.
        let _guard = ENV_MUTEX.lock().unwrap();
        let admin = config_with_admin_env(&[
            ("MAJU_ADMIN_HOST", Some("admin.example")),
            ("MAJU_ADMIN_TOKEN", None),
            ("MAJU_ADMIN_AUTH", Some("")),
        ])
        .expect("empty MAJU_ADMIN_AUTH defaults to nip98")
        .admin
        .expect("admin surface is configured");
    ...
    fn nip98_mode_parses_and_succeeds_without_pubkeys_env() {
        ...
        let admin = config_with_admin_env(&[
            ("MAJU_ADMIN_HOST", Some("admin.example")),
            ("MAJU_ADMIN_AUTH", Some("nip98")),
        ])
        .expect(
            "nip98 mode succeeds without MAJU_ADMIN_PUBKEYS (role resolution is at request time)",
        )
        .admin
        .expect("admin surface is configured");
        assert!(matches!(admin.auth, crate::config::AdminAuth::Nip98));
    ...
    fn s3_addressing_style_env_accepts_virtual_and_rejects_invalid_values() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_S3_ADDRESSING_STYLE");

        std::env::set_var("MAJU_S3_ADDRESSING_STYLE", "virtual");
        let configured = Config::from_env()
            .expect("virtual style config")
            .media
            .s3_addressing_style;

        std::env::set_var("MAJU_S3_ADDRESSING_STYLE", "auto");
        let invalid = Config::from_env();

        if let Some(value) = previous {
            std::env::set_var("MAJU_S3_ADDRESSING_STYLE", value);
        } else {
            std::env::remove_var("MAJU_S3_ADDRESSING_STYLE");
        }

        assert_eq!(configured, maju_media::config::S3AddressingStyle::Virtual);
        assert!(matches!(
            invalid,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_S3_ADDRESSING_STYLE must be 'path' or 'virtual'")
        ));
    }
    ...
    fn redis_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_REDIS_POOL_SIZE");

        std::env::set_var("MAJU_REDIS_POOL_SIZE", "32");
        let overridden = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("MAJU_REDIS_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").redis_pool_size;

        std::env::set_var("MAJU_REDIS_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").redis_pool_size;

        if let Some(value) = previous {
            std::env::set_var("MAJU_REDIS_POOL_SIZE", value);
        } else {
            std::env::remove_var("MAJU_REDIS_POOL_SIZE");
        }
    ...
    fn db_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_DB_POOL_SIZE");

        std::env::set_var("MAJU_DB_POOL_SIZE", "80");
        let overridden = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("MAJU_DB_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_pool_size;

        std::env::set_var("MAJU_DB_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_pool_size;

        if let Some(value) = previous {
            std::env::set_var("MAJU_DB_POOL_SIZE", value);
        } else {
            std::env::remove_var("MAJU_DB_POOL_SIZE");
        }
    ...
    fn db_read_pool_size_env_override_and_invalid_fallback() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_DB_READ_POOL_SIZE");

        std::env::remove_var("MAJU_DB_READ_POOL_SIZE");
        let unset = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("MAJU_DB_READ_POOL_SIZE", "40");
        let overridden = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("MAJU_DB_READ_POOL_SIZE", "0");
        let zero = Config::from_env().expect("config").db_read_pool_size;

        std::env::set_var("MAJU_DB_READ_POOL_SIZE", "not-a-number");
        let junk = Config::from_env().expect("config").db_read_pool_size;

        if let Some(value) = previous {
            std::env::set_var("MAJU_DB_READ_POOL_SIZE", value);
        } else {
            std::env::remove_var("MAJU_DB_READ_POOL_SIZE");
        }
    ...
    fn read_database_url_unset_or_blank_is_none() {
        ...
        std::env::set_var("READ_DATABASE_URL", "postgres://maju:pw@replica:5432/maju"); // sadscan:disable np.postgres.1
        ...
        assert_eq!(
            set.as_deref(),
            Some("postgres://maju:pw@replica:5432/maju") // sadscan:disable np.postgres.1
        );
    }
    ...
    fn replica_read_max_age_defaults_off_and_rejects_junk() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_REPLICA_READ_MAX_AGE_MS");
        let previous_old = std::env::var_os("MAJU_REPLICA_HEAD_MAX_AGE_SECS");
        std::env::remove_var("MAJU_REPLICA_HEAD_MAX_AGE_SECS");

        std::env::remove_var("MAJU_REPLICA_READ_MAX_AGE_MS");
        let unset = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("MAJU_REPLICA_READ_MAX_AGE_MS", "1000");
        let set = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("MAJU_REPLICA_READ_MAX_AGE_MS", "0");
        let zero = Config::from_env().expect("config").replica_read_max_age_ms;

        std::env::set_var("MAJU_REPLICA_READ_MAX_AGE_MS", "soon");
        ...
        std::env::set_var("MAJU_REPLICA_READ_MAX_AGE_MS", "1000");
        std::env::set_var("MAJU_REPLICA_HEAD_MAX_AGE_SECS", "5");
        let old_name = Config::from_env();

        std::env::remove_var("MAJU_REPLICA_HEAD_MAX_AGE_SECS");
        if let Some(value) = previous {
            std::env::set_var("MAJU_REPLICA_READ_MAX_AGE_MS", value);
        } else {
            std::env::remove_var("MAJU_REPLICA_READ_MAX_AGE_MS");
        }
        if let Some(value) = previous_old {
            std::env::set_var("MAJU_REPLICA_HEAD_MAX_AGE_SECS", value);
        }
        ...
        match old_name {
            Err(ConfigError::InvalidValue(message)) => assert!(
                message.contains("MAJU_REPLICA_READ_MAX_AGE_MS"),
    ...
    fn drain_jitter_defaults_off_and_rejects_junk() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_DRAIN_JITTER_MS");

        std::env::remove_var("MAJU_DRAIN_JITTER_MS");
        let unset = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("MAJU_DRAIN_JITTER_MS", "20000");
        let set = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("MAJU_DRAIN_JITTER_MS", "60000");
        let capped = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("MAJU_DRAIN_JITTER_MS", "0");
        let zero = Config::from_env().expect("config").drain_jitter_ms;

        std::env::set_var("MAJU_DRAIN_JITTER_MS", "soon");
        let junk = Config::from_env();

        std::env::set_var("MAJU_DRAIN_JITTER_MS", "");
        ...
        if let Some(value) = previous {
            std::env::set_var("MAJU_DRAIN_JITTER_MS", value);
        } else {
            std::env::remove_var("MAJU_DRAIN_JITTER_MS");
        }
    ...
    fn audit_logging_defaults_on_and_accepts_explicit_off() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_AUDIT_ENABLED");
        std::env::remove_var("MAJU_AUDIT_ENABLED");
        assert!(parse_bool("MAJU_AUDIT_ENABLED", true).unwrap());
        std::env::set_var("MAJU_AUDIT_ENABLED", "false");
        assert!(!parse_bool("MAJU_AUDIT_ENABLED", true).unwrap());
        if let Some(value) = previous {
            std::env::set_var("MAJU_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("MAJU_AUDIT_ENABLED");
        }
    }
    ...
    fn audit_logging_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_AUDIT_ENABLED");
        std::env::set_var("MAJU_AUDIT_ENABLED", "sometimes");
        let result = parse_bool("MAJU_AUDIT_ENABLED", true);
        if let Some(value) = previous {
            std::env::set_var("MAJU_AUDIT_ENABLED", value);
        } else {
            std::env::remove_var("MAJU_AUDIT_ENABLED");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_AUDIT_ENABLED")
        ));
    }
    ...
    fn join_policy_age_attestation_rejects_invalid_boolean() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_AGE_ATTESTATION_REQUIRED");
        std::env::set_var("MAJU_AGE_ATTESTATION_REQUIRED", "sometimes");
        let result = parse_optional_bool("MAJU_AGE_ATTESTATION_REQUIRED");
        if let Some(value) = previous {
            std::env::set_var("MAJU_AGE_ATTESTATION_REQUIRED", value);
        } else {
            std::env::remove_var("MAJU_AGE_ATTESTATION_REQUIRED");
        }
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_AGE_ATTESTATION_REQUIRED")
        ));
    }
    ...
    fn rate_limits_can_be_overridden() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN", "1001");
        std::env::set_var("MAJU_RATE_LIMIT_GIF_SEARCHES_PER_MIN", "1004");
        std::env::set_var("MAJU_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN", "1002");
        std::env::set_var("MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "1003");

        let config = Config::from_env().expect("config");

        std::env::remove_var("MAJU_RATE_LIMIT_HUMAN_MESSAGES_PER_MIN");
        std::env::remove_var("MAJU_RATE_LIMIT_GIF_SEARCHES_PER_MIN");
        std::env::remove_var("MAJU_RATE_LIMIT_HUMAN_API_CALLS_PER_MIN");
        std::env::remove_var("MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC");
    ...
    fn rate_limit_overrides_reject_zero() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC", "0");
        let result = Config::from_env();
        std::env::remove_var("MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC");
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_RATE_LIMIT_HUMAN_WS_EVENTS_PER_SEC")
        ));
    }
    ...
    fn relay_operator_pubkeys_parse_dedupe_and_normalize() {
        ...
        std::env::set_var(
            "RELAY_OPERATOR_API_ORIGIN",
            "http://maju.mesh.bb-production.com",
        );
        let config = Config::from_env().expect("config");
    ...
    fn relay_operator_api_origin_rejects_paths() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("RELAY_OPERATOR_API_ORIGIN", "https://maju.example/operator");
        let result = Config::from_env();
        std::env::remove_var("RELAY_OPERATOR_API_ORIGIN");
    ...
    #[test]
    fn push_is_opt_in_and_gateway_defaults_to_maju() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous_enabled = std::env::var_os("MAJU_PUSH_ENABLED");
        let previous = std::env::var_os("MAJU_PUSH_GATEWAY_DELIVERY_URL");
        std::env::remove_var("MAJU_PUSH_ENABLED");
        std::env::remove_var("MAJU_PUSH_GATEWAY_DELIVERY_URL");
        let config = Config::from_env().expect("default config");
        ...
        std::env::set_var("MAJU_PUSH_ENABLED", "true");
        ...
        std::env::set_var("MAJU_PUSH_GATEWAY_DELIVERY_URL", "");
        let result = Config::from_env();
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
        ...
        std::env::set_var("MAJU_PUSH_ENABLED", "false");
        ...
        if let Some(value) = previous_enabled {
            std::env::set_var("MAJU_PUSH_ENABLED", value);
        } else {
            std::env::remove_var("MAJU_PUSH_ENABLED");
        }
        if let Some(value) = previous {
            std::env::set_var("MAJU_PUSH_GATEWAY_DELIVERY_URL", value);
        } else {
            std::env::remove_var("MAJU_PUSH_GATEWAY_DELIVERY_URL");
        }
    }
    ...
    fn invalid_push_enabled_value_is_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let previous = std::env::var_os("MAJU_PUSH_ENABLED");
        std::env::set_var("MAJU_PUSH_ENABLED", "sometimes");
        let result = Config::from_env();
        if let Some(value) = previous {
            std::env::set_var("MAJU_PUSH_ENABLED", value);
        } else {
        ...
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_PUSH_ENABLED")
        ));
    }
    ...
    fn invalid_push_gateway_timeout_is_not_silently_defaulted() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_PUSH_GATEWAY_TIMEOUT_MS", "99");
        let result = Config::from_env();
        std::env::remove_var("MAJU_PUSH_GATEWAY_TIMEOUT_MS");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_PUSH_GATEWAY_TIMEOUT_MS")
        ));
    }
    ...
    fn invalid_push_executor_key_id_is_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_PUSH_EXECUTOR_KEY_ID", "");
        let result = Config::from_env();
        std::env::remove_var("MAJU_PUSH_EXECUTOR_KEY_ID");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref message))
                if message.contains("MAJU_PUSH_EXECUTOR_KEY_ID")
        ));
    }
    ...
    fn huddle_audio_available_can_be_disabled_for_horizontal_scaling() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_HUDDLE_AUDIO_AVAILABLE", "false");
        let config = Config::from_env().expect("config");
        std::env::remove_var("MAJU_HUDDLE_AUDIO_AVAILABLE");
        assert!(
            !config.huddle_audio_available,
            "MAJU_HUDDLE_AUDIO_AVAILABLE=false must disable huddle audio (multi-pod deployments)"
        );
    }
    ...
    fn pairing_relay_url_accepts_websocket_urls_and_rejects_http() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_PAIRING_RELAY_URL", "wss://pairing.maju.xyz");
        let config = Config::from_env().expect("config");
        assert_eq!(
            config.pairing_relay_url.as_deref(),
            Some("wss://pairing.maju.xyz")
        );

        std::env::set_var("MAJU_PAIRING_RELAY_URL", "https://pairing.maju.xyz");
        let result = Config::from_env();
        std::env::remove_var("MAJU_PAIRING_RELAY_URL");
        assert!(matches!(
            result,
            Err(ConfigError::InvalidValue(ref msg)) if msg.contains("MAJU_PAIRING_RELAY_URL")
        ));
    }
    ...
    fn max_frame_bytes_can_be_configured() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("MAJU_MAX_FRAME_BYTES", "262144");
        let config = Config::from_env().expect("config");
        std::env::remove_var("MAJU_MAX_FRAME_BYTES");
    ...
    fn git_repo_path_is_created_if_missing() {
        ...
        std::env::set_var("MAJU_GIT_REPO_PATH", &nested);
        let result = Config::from_env();
        std::env::remove_var("MAJU_GIT_REPO_PATH");
    ...
    fn git_repo_path_unwritable_returns_error() {
        ...
        assert!(
            matches!(result, Err(ConfigError::InvalidValue(ref msg)) if msg.contains("MAJU_GIT_REPO_PATH")),
            "expected InvalidValue mentioning MAJU_GIT_REPO_PATH, got {result:?}"
        );
    }
}
