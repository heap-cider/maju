...
//! [`maju_core::kind::is_moderation_command_kind`], list them in
...
//!
//! Lane ownership: L6 (Quinn) — plus `maju-cli` `moderation` command group.
...
use maju_db::moderation::NewAction;
...
pub(crate) async fn handle_resolve(
    ...
    let (target_pubkey, target_event_id) = match &report.target {
        maju_db::moderation::ReportTarget::Pubkey(p) => (Some(p.as_slice()), None),
        maju_db::moderation::ReportTarget::Event(e) => (None, Some(e.as_slice())),
        maju_db::moderation::ReportTarget::Blob(_) => (None, None),
    };
...
mod tests {
    ...
    fn resolve_audit_actions_are_allowed_by_db_check_vocabulary() {
        for action in ["dismiss", "escalate", "delete", "kick", "ban", "timeout"] {
            ...
            assert!(
                maju_db::moderation::MODERATION_ACTION_CHECK_VOCAB.contains(&audit_action),
