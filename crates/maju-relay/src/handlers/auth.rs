...
/// the JSON-array string that [`maju_sdk::nip_oa::verify_auth_tag`] expects.
///
...
pub async fn handle_auth(event: nostr::Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("maju_auth_attempts_total", "method" => "nip42").increment(1);
        ...
        .await
    {
        Ok(mut auth_ctx) => {
            ...
            // extracted from the self-proving auth tag with no DB round-trip.
            {
                ...
                if let Some((metric_reason, deny_reason)) = denial {
                    ...
                    metrics::counter!("maju_auth_failures_total", "reason" => metric_reason)
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
            ...
            if state.config.pubkey_allowlist_enabled
                && auth_ctx.auth_method == maju_auth::AuthMethod::Nip42
            {
                ...
                if !allowed {
                    ...
                    metrics::counter!("maju_auth_failures_total", "reason" => "allowlist_denied")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    metrics::counter!("maju_auth_failures_total", "reason" => "not_relay_member")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
        ...
        Err(e) => {
            ...
            metrics::counter!("maju_auth_failures_total", "reason" => "nip42_invalid").increment(1);
            *conn.auth_state.write().await = AuthState::Failed;
            conn.send(RelayMessage::ok(
                &event_id_hex,
                false,
