...
use maju_db::channel::{MemberRecord, MemberRole};
...
/// The per-fire authority gate in `maju-workflow` is the fail-closed backstop;
...
async fn evict_conn_channel_subscriptions(
    ...
    for update in removed {
        state
            .pubsub
            .release_topic(tenant, maju_pubsub::EventTopic::Channel(channel_id))
            .await;
        if update.removed {
...
///
/// Maju accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    state: &Arc<AppState>,
    community_id: maju_core::CommunityId,
    members: &[maju_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
    ...
    match kind {
        9000 => {
            ...
            let requested_role = match role_str {
                Some(ref s) => match s.parse::<maju_db::channel::MemberRole>() {
                    Ok(r) => Some(r),
            ...
            let actor_role: Option<maju_db::channel::MemberRole> = members
                .iter()
                ...
                .filter(|(m, role)| m.role != role.as_str())
            {
                ...
                if target.role == "owner"
                    && role != maju_db::channel::MemberRole::Owner
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !maju_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
...
async fn emit_addressable_discovery_event(
    ...
    let min_ts = {
        let existing = state
            .db
            .query_events_for_event_write(&maju_db::event::EventQuery {
                kinds: Some(vec![kind as i32]),
                channel_id: Some(channel_id),
                limit: Some(1),
                ..maju_db::event::EventQuery::for_community(tenant.community())
            })
            .await
            .unwrap_or_default();
        existing
...
async fn store_group_members_event(
    ...
    state: &Arc<AppState>,
    channel_id: Uuid,
    member_snapshot: &mut maju_db::channel::LockedMemberSnapshot,
) -> anyhow::Result<Option<maju_core::StoredEvent>> {
...
async fn dispatch_group_members_event(
    tenant: &TenantContext,
    state: &Arc<AppState>,
    stored: Option<maju_core::StoredEvent>,
    relay_pubkey_hex: &str,
) {
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // Maju channels always require explicit membership
...
async fn handle_create_group(
    ...
    let channel_type: maju_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == maju_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    state
        ...
        .add_member(
            ...
            channel_id,
            &actor_bytes,
            maju_db::channel::MemberRole::Member,
            None,
        )
        .await?;
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        maju_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        maju_core::kind::KIND_WORKFLOW_DEF => {
...
pub(crate) async fn handle_git_repo_announcement_inner(
    ...
    use maju_db::git_repo::ReserveOutcome;
    ...
    let serving_write = maju_deletion::acquire_serving_write(
        &state.db,
        tenant.community(),
...
/// carrying the announcer's pubkey in the `p` tag (maju extension).
...
pub async fn reconcile_nip43_membership_snapshots_with_purpose(
    ...
    for community in communities {
        let community_id = maju_core::CommunityId::from_uuid(community.id);
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("maju_nip43_membership_reconciliation_failures_total")
                    .increment(1);
    ...
    metrics::counter!("maju_nip43_membership_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub async fn publish_nip43_membership_list(
    ...
    metrics::counter!("maju_nip43_membership_publications_total", "result" => "attempted")
        .increment(1);
    let result = publish_nip43_membership_list_inner(tenant, state).await;
    metrics::histogram!("maju_nip43_membership_publication_seconds")
        .record(started_at.elapsed().as_secs_f64());
    metrics::counter!(
        "maju_nip43_membership_publications_total",
...
pub async fn reconcile_large_channel_member_snapshots(
    ...
    for candidate in candidates {
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("maju_channel_roster_reconciliation_failures_total").increment(1);
                warn!(
    ...
    metrics::counter!("maju_channel_roster_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub(crate) mod publish_test_hooks {
    use maju_core::tenant::CommunityId;
