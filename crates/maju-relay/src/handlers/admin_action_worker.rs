...
use maju_db::relay_admin_actions::StrandedActionClaim;
...
pub(crate) async fn recover_one(state: &Arc<AppState>, claim: StrandedActionClaim) {
    ...
    let community_id = maju_core::CommunityId::from_uuid(rec.report_community_id);
    let tenant = match state.db.lookup_community_host(community_id).await {
        Ok(Some(host)) => maju_core::tenant::TenantContext::resolved(community_id, host),
        Ok(None) => {
            warn!(
                action_id = %action_id,
