...
use maju_db::admin_moderation::AdminReportDetail;
use maju_db::relay_admin_actions::{AdminActionRecord, ClaimResult};
...
impl From<maju_db::DbError> for ResolutionError {
    fn from(e: maju_db::DbError) -> Self {
    ...
    loop {
        ...
        if rec.step_marker.is_none() {
            ...
            let lease_token = if let Some(token) = held_lease {
                ...
                match lease {
                    maju_db::relay_admin_actions::LeaseResult::Acquired(token) => token,
                    maju_db::relay_admin_actions::LeaseResult::Contended => {
                    ...
                    maju_db::relay_admin_actions::LeaseResult::NotLeasable => {
...
async fn run_atomic_mutation(
    ...
    let raw: anyhow::Result<bool> = match ctx.action {
        ...
        "kick" => {
            ...
            {
                maju_db::relay_admin_actions::KickWithMarkerResult::Removed => Ok(true),
                maju_db::relay_admin_actions::KickWithMarkerResult::AlreadyMarked => Ok(false),
                maju_db::relay_admin_actions::KickWithMarkerResult::AlreadyGone => Err(
...
mod tests {
    use super::*;
    use maju_db::admin_moderation::{AdminReport, AdminReportedMessage};
