...
//!    participant-hash-idempotent DM model (`maju-db/src/dm.rs`).
...
use maju_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            maju_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
