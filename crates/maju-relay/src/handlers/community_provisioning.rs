...
use maju_core::tenant::{normalize_host, TenantContext};
use url::{Host, Url};
...
/// present, the owner is (re)bootstrapped via [`maju_db::Db::bootstrap_owner`]
...
pub async fn provision_community(
    ...
    if request.create_only {
        ...
        {
            maju_db::CreateCommunityWithOwnerResult::Created(record) => record,
            maju_db::CreateCommunityWithOwnerResult::HostExists => {
                return Err("community already exists".to_string());
            }
            maju_db::CreateCommunityWithOwnerResult::LimitReached => {
                return Err(
...
mod tests {
    ...
    fn host_valid_bare_domain() {
        assert!(validate_host("acme.communities.maju.xyz").is_ok());
    }
