...
use maju_db::relay_admin_actions::OutboxRecord;
...
async fn resolve_tenant(
    ...
    task_type: &str,
) -> Result<maju_core::tenant::TenantContext, String> {
    let host = state
        .db
    ...
    Ok(maju_core::tenant::TenantContext::resolved(
        community_id,
        host,
    ))
}
...
async fn deliver_tombstone(state: &Arc<AppState>, row: &OutboxRecord) -> Result<(), String> {
    ...
    let community_id = maju_core::CommunityId::from_uuid(community_uuid);
