...
async fn persist_command_event(
    ...
    use maju_db::replaceable::{ParameterizedReplacePrecondition, ParameterizedReplaceStatus};
    ...
    let d_tag = maju_db::event::extract_d_tag(event);
    ...
    let (_, was_inserted) =
        maju_db::event::insert_event_in_transaction(&mut tx, tenant.community(), event, channel_id)
            .await
...
async fn handle_workflow_def(
    ...
    let (def, definition_json_str) = maju_workflow::WorkflowEngine::parse_yaml(&event.content)
    ...
    let webhook_secret = if matches!(def.trigger, maju_workflow::TriggerDef::Webhook) {
...
async fn resume_workflow_after_approval(
    engine: Arc<maju_workflow::WorkflowEngine>,
    db: maju_db::Db,
    ...
    let def: maju_workflow::WorkflowDef = match serde_json::from_value(workflow.definition.clone())
    {
        Ok(d) => d,
        Err(e) => {
...
mod postgres_tests {
    ...
    async fn persistence_test_context() -> (maju_db::Db, TenantContext) {
        let url = std::env::var("MAJU_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| "postgres://maju:maju_dev@localhost:5432/maju".to_string()); // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn workflow_persistence_replays_legacy_malformed_revision_before_validation() {
        ...
        let (_, was_inserted) = maju_db::event::insert_event_in_transaction(
            &mut tx,
            tenant.community(),
            &legacy,
