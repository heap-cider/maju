...
use maju_db::{Db, DbError, DbReadinessOutcome};
use tokio::time::Instant;

const READINESS_TIMEOUT: Duration = Duration::from_secs(2);

/// Closed label set exported by `maju_readiness_checks_total{reason}`.
#[cfg(test)]
...
fn classify_deletion_catalog_result(result: maju_db::Result<()>) -> DeletionCatalogOutcome {
    match result {
...
fn record_overall_state(ready: bool) {
    metrics::gauge!("maju_readiness_state", "check" => "overall").set(if ready {
        1.0
    } else {
        0.0
    });
...
fn record_dependency_state(dependency: &'static str, ready: bool) {
    metrics::gauge!("maju_readiness_state", "check" => dependency).set(if ready {
        1.0
    } else {
        0.0
    });
...
mod tests {
    ...
    fn gauge_value(
        ...
        let value = exact_metric(snapshot, "maju_readiness_state", &[("check", check)])
    ...
    fn shutdown_dominates_an_in_flight_success_without_resurrecting_gauges() {
        ...
        assert_eq!(gauge_value(&snapshot, "overall"), 0.0);
        assert!(
            exact_metric(&snapshot, "maju_readiness_state", &[("check", "postgres")]).is_none()
        );
        assert!(matches!(
            exact_metric(
                &snapshot,
