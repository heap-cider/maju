...
use maju_auth::{generate_challenge, AuthContext, LimitType};
...
async fn handle_active_connection(
    ...
    metrics::gauge!("maju_ws_connections_active").increment(1.0);
        ...
        tokio::select! {
            _ = tokio::time::sleep(AUTH_TIMEOUT) => {
                ...
                if !authenticated {
                    ...
                    metrics::counter!("maju_ws_auth_timeouts_total").increment(1);
    ...
    for removed in state.sub_registry.remove_connection(conn.conn_id) {
        if removed.scope.is_global() {
            state
                .pubsub
                .release_topic(&conn.tenant, maju_pubsub::EventTopic::Global)
                .await;
        }
        for &channel_id in removed.scope.channel_ids() {
            state
                .pubsub
                .release_topic(&conn.tenant, maju_pubsub::EventTopic::Channel(channel_id))
                .await;
        }
    }
    ...
    metrics::gauge!("maju_ws_connections_active").decrement(1.0);
    ...
    loop {
        ...
        tokio::select! {
            ...
            Some(msg) = data_rx.recv() => {
                ...
                metrics::histogram!("maju_ws_send_batch_size").record(batched as f64);
            }
        }
    }
}
...
fn send_admission_result(
    ...
    match result {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("maju_admission_rejections_total", "transport" => "websocket", "reason" => "quota").increment(1);
            conn.send(request_rejection_message(
                sub_id,
        ...
        Err(crate::admission::AdmissionError::Unavailable) => {
            metrics::counter!("maju_admission_rejections_total", "transport" => "websocket", "reason" => "unavailable").increment(1);
            conn.send(request_rejection_message(
                sub_id,
