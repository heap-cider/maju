...
/// enforces membership (`MAJU_REQUIRE_RELAY_MEMBERSHIP=true`) AND has a
...
pub struct GifDescriptor {
    /// Provider identifier understood by Maju clients.
    pub provider: String,
...
///
/// `max_limit` is [`maju_db::DEFAULT_MAX_PAGE_LIMIT`], the same constant the
...
fn relay_limitation(max_message_length: usize) -> RelayLimitation {
    ...
    RelayLimitation {
        ...
        max_filters: Some(10),
        max_limit: Some(maju_db::DEFAULT_MAX_PAGE_LIMIT as u32),
...
impl RelayInfo {
    ...
    /// kinds 39000/39001/39002, which Maju signs with `state.relay_keypair`
    ...
    /// `build` advertises the provider-agnostic `maju-gif` extension and the
    ...
    pub fn build(
        ...
        let gif = gif_provider.map(|provider| {
            supported_extensions.push("maju-gif".to_string());
            GifDescriptor {
        ...
        Self {
            name: "Maju Relay".to_string(),
            description: "Maju — private team communication relay".to_string(),
            ...
            push: None,
            software: "https://github.com/heap-cider/maju".to_string(),
...
fn push_descriptor(
    ...
    Some(serde_json::json!({
        ...
        "app_profiles": [
            {"id": "maju-ios-production", "transport": "apns"},
            {"id": "maju-ios-sandbox", "transport": "apns"}
        ],
...
mod tests {
    ...
    #[test]
    fn build_advertises_maju_repository_url() {
        let info = RelayInfo::build(None, None, false, DEFAULT_MAX_FRAME_BYTES, None, None);
        assert_eq!(info.software, "https://github.com/heap-cider/maju");
    }
    ...
    fn gif_descriptor_and_extension_are_config_gated_and_credential_free() {
        ...
        assert!(json["supported_extensions"]
            .as_array()
            .expect("extensions")
            .contains(&serde_json::json!("maju-gif")));
    ...
    fn auth_required_is_advertised_true() {
        ...
        // `AuthState::Authenticated` (see `crates/maju-relay/src/handlers/`),
