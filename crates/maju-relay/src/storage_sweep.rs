...
//! classifier + pure fold live in `maju_media::bucket_index`). Summary:
//!
...
use maju_media::{BucketSnapshot, SweepError};
...
impl StorageSweepConfig {
    /// Reads `MAJU_STORAGE_SWEEP_INTERVAL_SECS` (default 3600, floor 60),
    /// `MAJU_STORAGE_SWEEP_TIMEOUT_SECS` (default 120),
    /// `MAJU_STORAGE_SWEEP_MAX_OBJECTS` (default 1_000_000), and the
    /// `MAJU_STORAGE_METRICS` kill switch (`off` ⇒ disabled, anything else
    /// including unset ⇒ enabled).
    pub fn from_env() -> Self {
        let interval_secs = std::env::var("MAJU_STORAGE_SWEEP_INTERVAL_SECS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(3600)
            .max(60);
        let timeout_secs = std::env::var("MAJU_STORAGE_SWEEP_TIMEOUT_SECS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(120);
        let max_objects = std::env::var("MAJU_STORAGE_SWEEP_MAX_OBJECTS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(1_000_000);
        let enabled = std::env::var("MAJU_STORAGE_METRICS")
            .ok()
...
impl StorageEmittedKey {
    fn set(&self, value: f64) {
        match self {
            Self::Bytes(host) => {
                metrics::gauge!("maju_community_storage_bytes", "community" => host.clone())
                    .set(value);
            }
            Self::Objects(host) => {
                metrics::gauge!("maju_community_storage_objects", "community" => host.clone())
                    .set(value);
            }
        }
    }
...
/// row) and rolls into `maju_storage_unmapped_community_bytes` instead of a
...
/// `maju_storage_unmapped_community_bytes` gauge.
pub async fn emit_storage_metrics(
    ...
    metrics::gauge!("maju_storage_sweep_ok").set(if ok { 1.0 } else { 0.0 });
    ...
    metrics::gauge!("maju_storage_sweep_failures").set(state.failures_total as f64);
    if let Some(attempt) = state.last_attempt {
        metrics::gauge!("maju_storage_sweep_duration_seconds").set(attempt.duration.as_secs_f64());
    }
    ...
    metrics::gauge!("maju_storage_sweep_age_seconds")
        .set(cached.completed_at.elapsed().as_secs_f64());

    let snapshot = &cached.data;
    metrics::gauge!("maju_total_storage_bytes", "kind" => "physical")
        .set(snapshot.physical_bytes as f64);
    metrics::gauge!("maju_total_storage_objects", "kind" => "physical")
        .set(snapshot.physical_objects as f64);
    metrics::gauge!("maju_total_storage_bytes", "kind" => "logical")
        .set(snapshot.logical_bytes as f64);
    metrics::gauge!("maju_total_storage_objects", "kind" => "logical")
        .set(snapshot.logical_objects as f64);

    metrics::gauge!("maju_storage_orphan_blob_bytes").set(snapshot.orphan_blob_bytes as f64);
    metrics::gauge!("maju_storage_orphan_blobs").set(snapshot.orphan_blob_count as f64);
    metrics::gauge!("maju_storage_orphan_sidecars").set(snapshot.orphan_sidecar_count as f64);
    metrics::gauge!("maju_storage_multi_variant_shas").set(snapshot.multi_variant_shas as f64);
    metrics::gauge!("maju_storage_multi_variant_bytes").set(snapshot.multi_variant_bytes as f64);
    metrics::gauge!("maju_storage_unknown_key_bytes").set(snapshot.unknown_key_bytes as f64);
    metrics::gauge!("maju_storage_unknown_key_objects").set(snapshot.unknown_key_objects as f64);
    ...
    metrics::gauge!("maju_storage_unmapped_community_bytes").set(unmapped_bytes as f64);
...
mod tests {
    ...
    use maju_media::CommunityStorage;
    ...
    fn config_defaults_and_floors_apply_when_env_absent() {
        ...
        for key in [
            "MAJU_STORAGE_SWEEP_INTERVAL_SECS",
            "MAJU_STORAGE_SWEEP_TIMEOUT_SECS",
            "MAJU_STORAGE_SWEEP_MAX_OBJECTS",
            "MAJU_STORAGE_METRICS",
        ] {
    ...
    // lock (`maju_db::UsageMetricsLeader`, see `crates/maju-db/src/lib.rs`)
    ...
    async fn a_completed_but_unharvested_sweep_never_emits_its_snapshot() {
        ...
        assert_eq!(
            values.get("maju_storage_sweep_ok"),
            Some(&0.0),
            "unharvested attempt must not register as a success"
        );
        assert!(!values.contains_key("maju_total_storage_bytes"));
        assert!(!values.contains_key("maju_community_storage_bytes"));
        assert!(!values.contains_key("maju_storage_sweep_age_seconds"));
    }
    ...
    async fn stalled_sweep_across_several_ticks_emits_health_only_then_once_on_completion() {
        ...
        for _ in 0..3 {
            ...
            let values = gauge_snapshot(&recorder);
            assert_eq!(values.get("maju_storage_sweep_ok"), Some(&0.0));
            assert!(!values.contains_key("maju_total_storage_bytes"));
        }
        ...
        let values = gauge_snapshot(&recorder);
        assert_eq!(values.get("maju_storage_sweep_ok"), Some(&1.0));
        assert_eq!(values.get("maju_total_storage_bytes"), Some(&500.0));
    }
    ...
    async fn cold_cache_emits_only_health_gauges() {
        ...
        let values = gauge_snapshot(&recorder);
        assert_eq!(values.get("maju_storage_sweep_ok"), Some(&0.0));
        assert_eq!(values.get("maju_storage_sweep_failures"), Some(&0.0));
        assert!(!values.contains_key("maju_total_storage_bytes"));
        assert!(!values.contains_key("maju_storage_sweep_age_seconds"));
    }
    ...
    async fn warm_cache_emits_community_and_unmapped_totals_with_scope_gating() {
        ...
        let values = gauge_snapshot(&recorder);
        assert_eq!(values.get("maju_storage_sweep_ok"), Some(&1.0));
        assert_eq!(values.get("maju_total_storage_bytes"), Some(&137.0));
        assert_eq!(
            values.get("maju_storage_unmapped_community_bytes"),
            Some(&30.0)
        );
        assert_eq!(values.get("maju_community_storage_bytes"), Some(&100.0));
    }
    ...
    async fn stale_per_community_series_are_zeroed_on_disappearance() {
        ...
        });
        {
            ...
            assert_eq!(
                labeled.get(&(
                    "maju_community_storage_bytes".to_string(),
                    "host.a".to_string()
                )),
                Some(&10.0),
        ...
        assert_eq!(
            labeled.get(&(
                "maju_community_storage_objects".to_string(),
                "host.a".to_string()
            )),
            Some(&0.0),
