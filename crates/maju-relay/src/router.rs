...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let api_router = Router::new()
        ...
        // Mesh demo echo probe — testbed-only; 404 unless MAJU_MESH=on and
        // MAJU_MESH_DEMO_ECHO=on (see api::mesh_demo).
...
fn build_cors_layer(cors_origins: &[String]) -> CorsLayer {
    ...
    if origins.is_empty() {
        tracing::error!(
            "MAJU_CORS_ORIGINS set but no valid origins could be parsed — \
...
mod tests {
    ...
    async fn readiness_state(evaluator: Arc<dyn readiness::ReadinessEvaluator>) -> Arc<AppState> {
        let mut config = crate::config::Config::from_env().expect("default config loads");
        config.require_relay_membership = false;
        config.database_url = "postgres://maju:maju_dev@127.0.0.1:1/maju".to_string();
        config.redis_url = "redis://127.0.0.1:1".to_string();
        let pool = sqlx::PgPool::connect_lazy(&config.database_url).expect("lazy pg pool");
        let db = maju_db::Db::from_pool(pool.clone());
        let redis_pool = deadpool_redis::Config::from_url(&config.redis_url)
    ...
    fn readiness_metric_lines(rendered: &str) -> Vec<&str> {
        rendered
            .lines()
            .filter(|line| line.starts_with("maju_readiness"))
            .collect()
    }
    ...
    fn production_readiness_routes_export_the_frozen_health_only_contract() {
        ...
        metrics::with_local_recorder(&recorder, || {
            ...
            runtime.block_on(async {
                ...
                assert!(first_scrape.contains("# TYPE maju_readiness_checks_total counter"));
                assert!(first_scrape
                    .contains("# TYPE maju_readiness_dependency_checks_total counter"));
                assert!(first_scrape
                    .contains("# TYPE maju_readiness_check_duration_seconds histogram"));
                assert!(first_scrape.contains("# TYPE maju_readiness_state gauge"));
                assert_eq!(
                    metric_value(
                        &first_scrape,
                        "maju_readiness_checks_total{reason=\"ready\"}"
                    ),
                    1.0
                );
                assert_eq!(
                    metric_value(
                        &first_scrape,
                        "maju_readiness_dependency_checks_total{dependency=\"postgres\",outcome=\"success\"}"
                    ),
                    1.0
                );
                assert_eq!(
                    metric_value(
                        &first_scrape,
                        "maju_readiness_state{check=\"overall\"}"
                    ),
                    1.0
                );
                for bucket in ["2", "2.5", "+Inf"] {
                    assert!(first_scrape.contains(&format!(
                        "maju_readiness_check_duration_seconds_bucket{{check=\"overall\",le=\"{bucket}\"}}"
                    )));
                }
                assert!(!first_scrape.contains("result="));
                assert!(!first_scrape
                    .lines()
                    .filter(|line| line.starts_with("maju_readiness_check_duration_seconds"))
                ...
                let histogram_counts_before = ["overall", "postgres", "redis", "deletion_catalog"]
                    .map(|check| {
                        metric_value(
                            &before_shutdown,
                            &format!(
                                "maju_readiness_check_duration_seconds_count{{check=\"{check}\"}}"
                            ),
                        )
                    });
                ...
                assert_eq!(
                    metric_value(
                        &final_scrape,
                        "maju_readiness_checks_total{reason=\"shutting_down\"}"
                    ),
                    1.0
                );
                assert_eq!(
                ...
                let exported_reasons = final_scrape
                    .lines()
                    .filter(|line| line.starts_with("maju_readiness_checks_total{"))
                    .count();
    ...
    fn real_health_route_generation_fence_covers_both_completion_orders() {
        ...
        assert_eq!(
            metric_value(&success_scrape, "maju_readiness_state{check=\"overall\"}"),
            1.0
        );
        assert_eq!(
            metric_value(&success_scrape, "maju_readiness_state{check=\"redis\"}"),
            1.0
        );
        ...
        assert_eq!(
            metric_value(&failure_scrape, "maju_readiness_state{check=\"overall\"}"),
            0.0
        );
        assert_eq!(
            metric_value(&failure_scrape, "maju_readiness_state{check=\"redis\"}"),
            0.0
        );
        for scrape in [&success_scrape, &failure_scrape] {
            assert_eq!(
                metric_value(scrape, "maju_readiness_checks_total{reason=\"ready\"}"),
                1.0
            );
            assert_eq!(
                metric_value(
                    scrape,
                    "maju_readiness_checks_total{reason=\"redis_pool_timeout\"}"
                ),
                1.0
            );
        }
    ...
    fn real_health_route_shutdown_fence_dominates_an_in_flight_success() {
        ...
        metrics::with_local_recorder(&recorder, || {
            runtime.block_on(async {
                ...
                assert_eq!(
                    metric_value(&scrape, "maju_readiness_state{check=\"overall\"}"),
                    0.0
                );
                assert!(scrape
                    .lines()
                    .all(|line| !line.starts_with("maju_readiness_state{check=\"postgres\"}")));
                assert_eq!(
                    metric_value(
                        &scrape,
                        "maju_readiness_checks_total{reason=\"shutting_down\"}"
    ...
    fn status_payload_exposes_source_and_build_identity() {
        ...
        assert_eq!(payload["service"], "maju-relay");
