...
pub fn require_mutation_principal(
    ...
    principal
        .ok_or_else(|| ApiError::forbidden_with_message("mutations require MAJU_ADMIN_AUTH=nip98"))
}
...
async fn authorize_nip98(
    ...
    let pubkey =
        maju_auth::verify_nip98_event(&event_json, &url, method, raw_body).map_err(|_| unauth())?;
...
mod tests {
    ...
    /// `maju_auth::verify_nip98_event`, which checks the `method` tag.
    ///
