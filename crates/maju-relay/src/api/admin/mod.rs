...
) -> Result<Json<Vec<maju_db::admin_moderation::AdminReport>>, ApiError> {
    authorize(
        &state,
        &headers,
        uri.path_and_query()
...
async fn report_detail(
    ...
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<maju_db::admin_moderation::AdminReportDetail>, ApiError> {
    authorize(
        &state,
        &headers,
        uri.path_and_query()
...
async fn feedback_detail(
    ...
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<maju_db::admin_moderation::AdminFeedback>, ApiError> {
    authorize(
        &state,
        &headers,
        uri.path_and_query()
...
async fn feedback_attachment(
    ...
    let response = crate::api::media::serve_feedback_attachment(&state, &tenant, &sha256, &headers)
        ...
        .map_err(|error| match error {
            maju_media::MediaError::NotFound => ApiError::not_found(),
            _ => ApiError::internal(),
        })?;
    tracing::info!(
        feedback_id = %feedback.id,
...
async fn reopen_report(
    ...
    use maju_db::relay_admin_actions::ReopenResult;
...
async fn cancel_report(
    ...
    let record = state
        ...
        .await?
        .ok_or_else(ApiError::internal)?;
    let dto = maju_db::admin_moderation::AdminActionDto::from_record(&record);
...
async fn upsert_operator(
    ...
    state
        ...
        .await
        .map_err(|error| match error {
            maju_db::DbError::LastOperator => ApiError::conflict(
...
fn attachment_url_matches(url: &str, community_host: &str, sha256: &str) -> bool {
    ...
    matches!(url.scheme(), "http" | "https")
        && maju_core::tenant::normalize_host(&authority)
            == maju_core::tenant::normalize_host(community_host)
        && url_hash == sha256
...
mod postgres_tests {
    ...
    fn database_url() -> String {
        std::env::var("MAJU_TEST_DATABASE_URL").unwrap_or_else(|_| {
            "postgres://maju:maju_dev@localhost:5432/maju".to_string() // sadscan:disable np.postgres.1 -- local test-only credentials
        })
    }
    ...
    async fn disabled_mode_state() -> Arc<crate::state::AppState> {
        ...
        let db = maju_db::Db::from_pool(pool.clone());
        let redis_pool = deadpool_redis::Config::from_url(&config.redis_url)
            .create_pool(Some(deadpool_redis::Runtime::Tokio1))
            .expect("redis pool");
        let pubsub = Arc::new(
            maju_pubsub::PubSubManager::new(&config.redis_url, redis_pool.clone())
                .await
                .expect("pubsub manager"),
        );
        let audit = maju_audit::AuditService::new(pool.clone());
        let auth = maju_auth::AuthService::new(config.auth.clone());
        let search = maju_search::SearchService::new(pool.clone());
        let workflow_engine = Arc::new(maju_workflow::WorkflowEngine::new(
            db.clone(),
            maju_workflow::WorkflowConfig::default(),
        ));
        let media_storage = maju_media::MediaStorage::new(&config.media).expect("media storage");
    ...
    impl maju_auth::Nip98ReplayGuard for AlwaysFreshReplayGuard {
        fn try_mark_in_scope<'a>(
            ...
            Box<dyn std::future::Future<Output = Result<bool, maju_auth::AuthError>> + Send + 'a>,
        > {
    ...
    impl maju_auth::Nip98ReplayGuard for TrackingReplayGuard {
        fn try_mark_in_scope<'a>(
            &'a self,
            _scope: &'a str,
    ...
    async fn nip98_state_with_replay(
        pubkeys: Vec<String>,
        replay: Arc<dyn maju_auth::Nip98ReplayGuard>,
    ...
    // agents probed live — not just the `maju-auth` unit layer.
    ...
    //
    // They delegate to the DB-layer tests in maju_db::relay_admin_actions::tests
    ...
    async fn racing_moderators_one_succeeds_one_gets_409() {
        // Covered by maju_db relay_admin_actions::tests::racing_moderators_exactly_one_claim_one_conflict
        // Run: cargo test -p maju-db relay_admin_actions::tests::racing -- --ignored
        //
        ...
        let cid = maju_core::CommunityId::from_uuid(community_id);
        ...
        let claim = |request_id: uuid::Uuid,
            ...
            maju_db::relay_admin_actions::claim_report(
                &pool,
                cid,
                report_id,
                request_id,
        ...
        let claimed_count = outcomes
            .iter()
            .filter(|r| matches!(r, maju_db::relay_admin_actions::ClaimResult::Claimed(_)))
            .count();
        let conflict_count = outcomes
            .iter()
            .filter(|r| matches!(r, maju_db::relay_admin_actions::ClaimResult::NotOpen(_)))
            .count();
    ...
    async fn same_request_id_retry_returns_existing_action() {
        ...
        let first = maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id,
            request_id,
        ...
        let first_id = match first {
            maju_db::relay_admin_actions::ClaimResult::Claimed(a) => a.id,
            other => panic!("expected Claimed, got {other:?}"),
        };

        let second = maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id,
            request_id,
        ...
        let second_id = match second {
            maju_db::relay_admin_actions::ClaimResult::AlreadyClaimed(a) => a.id,
    ...
    async fn community_9044_against_processing_report_fails_cleanly() {
        ...
        let _ = maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id,
            uuid::Uuid::new_v4(),
        ...
        let result = maju_db::relay_admin_actions::resolve_report_decision_atomic(
            &pool,
            cid,
            report_id,
            "dismissed",
    ...
    async fn cancel_after_mutation_success_is_rejected() {
        ...
        let action_id = match maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id,
            uuid::Uuid::new_v4(),
        ...
        let _ = maju_db::relay_admin_actions::begin_enforcing(&pool, action_id)
            .await
            .expect("begin_enforcing");
        let _ = maju_db::relay_admin_actions::commit_mutation_step(&pool, action_id)
            .await
            .expect("commit_mutation_step");

        let cancelled = maju_db::relay_admin_actions::cancel_action(
            &pool,
            action_id,
            cid,
            report_id,
    ...
    async fn cancel_route_returns_open_and_embeds_the_cancelled_action_dto() {
        ...
        maju_db::relay_admin_actions::begin_enforcing(&pool, action_id)
            .await
            .expect("begin_enforcing");
        let lease_until = chrono::Utc::now() + chrono::Duration::seconds(60);
        let lease_token =
            match maju_db::relay_admin_actions::acquire_action_lease(&pool, action_id, lease_until)
                .await
                .expect("acquire_action_lease")
            {
                maju_db::relay_admin_actions::LeaseResult::Acquired(t) => t,
                other => panic!("expected Acquired, got {other:?}"),
            };
        assert!(
            maju_db::relay_admin_actions::record_failure(&pool, action_id, lease_token, "boom")
                .await
        ...
        let seed_failed_action = |report_id: Uuid| {
            ...
            async move {
                ...
                maju_db::relay_admin_actions::begin_enforcing(&pool, action_id)
                    ...
                    .expect("begin_enforcing");
                let lease_until = chrono::Utc::now() + chrono::Duration::seconds(60);
                let lease_token = match maju_db::relay_admin_actions::acquire_action_lease(
                    &pool,
                    action_id,
                    lease_until,
                )
    ...
    async fn worker_crash_redrive_converges_to_exactly_one_enforcement() {
        ...
        let reloaded = maju_db::relay_admin_actions::get_action(&pool, action_id)
            .await
            .expect("get_action")
        ...
        let finalized = maju_db::relay_admin_actions::finalize_success(
            &pool,
            action_id,
            cid,
            report_id,
        ...
        let second_finalize = maju_db::relay_admin_actions::finalize_success(
            &pool,
            action_id,
            cid,
            report_id,
        ...
        let outbox_rows = maju_db::relay_admin_actions::list_pending_outbox(&pool, action_id)
            .await
            .expect("list outbox");
    ...
    fn e2e_tenant(community_id: uuid::Uuid, host: &str) -> maju_core::tenant::TenantContext {
        maju_core::tenant::TenantContext::resolved(
            maju_core::CommunityId::from_uuid(community_id),
            host.to_string(),
        )
    }
    ...
    async fn delete_then_crash_before_tombstone_redrive() {
        ...
        let committed = maju_db::relay_admin_actions::execute_delete_with_marker(
            &pool,
            action_id,
            lease_token,
            cid,
        ...
        let rec = maju_db::relay_admin_actions::get_action(&pool, action_id)
            .await
            .expect("get_action")
            .expect("action exists");
        ...
        let batch = maju_db::relay_admin_actions::claim_stranded_action_batch(
            &pool,
            "e2e-crash-worker",
        ...
        let final_rec = maju_db::relay_admin_actions::get_action(&pool, action_id)
            .await
        ...
        let batch2 = maju_db::relay_admin_actions::claim_stranded_action_batch(
            &pool,
            "e2e-crash-worker-2",
            chrono::Utc::now() + chrono::Duration::seconds(120),
            1000,
    ...
    async fn kick_retry_after_this_action_removed_member() {
        ...
        let action_id1 = match maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id1,
            uuid::Uuid::new_v4(),
        ...
        let _ = maju_db::relay_admin_actions::begin_enforcing(&pool, action_id1)
            .await
            .expect("begin_enforcing1");

        // Acquire lease for action_id1 (required by execute_kick_with_marker).
        let lease_until = chrono::Utc::now() + chrono::Duration::seconds(60);
        let lease_token1 = match maju_db::relay_admin_actions::acquire_action_lease(
            &pool,
            action_id1,
            lease_until,
        )
        ...
        let r1 = maju_db::relay_admin_actions::execute_kick_with_marker(
            &pool,
            action_id1,
            lease_token1,
            cid,
        ...
        assert!(
            matches!(
                r1,
                maju_db::relay_admin_actions::KickWithMarkerResult::Removed
            ),
        ...
        let rec1 = maju_db::relay_admin_actions::get_action(&pool, action_id1)
            .await
            .expect("get_action1")
            .expect("exists");
        let state = state_from_pool(pool.clone()).await;
        ...
        let final_rec1 = maju_db::relay_admin_actions::get_action(&pool, action_id1)
            .await
        ...
        let action_id2 = match maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id2,
            uuid::Uuid::new_v4(),
        ...
        let _ = maju_db::relay_admin_actions::begin_enforcing(&pool, action_id2)
            .await
            .expect("begin_enforcing2");

        // Acquire lease for action_id2.
        let lease_token2 = match maju_db::relay_admin_actions::acquire_action_lease(
            &pool,
            action_id2,
            lease_until,
        )
        ...
        let r2 = maju_db::relay_admin_actions::execute_kick_with_marker(
            &pool,
            action_id2,
            lease_token2,
            cid,
        ...
        assert!(
            matches!(
                r2,
                maju_db::relay_admin_actions::KickWithMarkerResult::AlreadyGone
            ),
        ...
        let rec2 = maju_db::relay_admin_actions::get_action(&pool, action_id2)
            .await
            .expect("get_action2")
            .expect("exists");
        assert!(
    ...
    async fn same_request_id_retry_with_changed_body_uses_persisted_claim() {
        ...
        let claimed = match maju_db::relay_admin_actions::claim_report(
            &pool,
            cid,
            report_id,
            request_id,
        ...
        .expect("first claim")
        {
            maju_db::relay_admin_actions::ClaimResult::Claimed(a) => a,
            other => panic!("expected Claimed, got {other:?}"),
        };
    ...
    async fn worker_redrive_of_event_kick_converges_after_event_purged_mid_flight() {
        ...
        let committed = maju_db::relay_admin_actions::execute_kick_with_marker(
            &pool,
            action_id,
            lease_token,
            cid,
    ...
    async fn delivery_failure_leaves_report_resolved_with_retryable_delivery_state() {
        ...
        let _ = maju_db::relay_admin_actions::execute_ban_with_marker(
            &pool,
            action_id,
            lease_token,
            cid,
        ...
        for attempt in 1..=maju_db::relay_admin_actions::OUTBOX_MAX_ATTEMPTS {
            ...
            if attempt < maju_db::relay_admin_actions::OUTBOX_MAX_ATTEMPTS {
                assert_eq!(
                    row_state, "pending",
                ...
                assert_eq!(
                    ...
                    maju_db::relay_admin_actions::OUTBOX_MAX_ATTEMPTS
                );
            }
        }
    ...
    async fn lease_expiry_action_takeover_by_worker() {
        ...
        let host = state
            ...
            .expect("lookup")
            .expect("host");
        let tenant = maju_core::tenant::TenantContext::resolved(cid, host);
        ...
        let after_phase1 = maju_db::relay_admin_actions::get_action(&pool, action_id)
            .await
    ...
    async fn stale_action_lease_token_rejected_at_mutation() {
        ...
        let valid_token = match maju_db::relay_admin_actions::acquire_action_lease(
            &pool,
            action_id,
        ...
        let stale_result = maju_db::relay_admin_actions::execute_ban_with_marker(
            &pool,
            action_id,
            stale_token,
            cid,
        ...
        let valid_result = maju_db::relay_admin_actions::execute_ban_with_marker(
            &pool,
            action_id,
            valid_token,
            cid,
        ...
        let rec2 = maju_db::relay_admin_actions::get_action(&pool, action_id)
            .await
    ...
    async fn stale_outbox_claim_token_rejected_on_completion() {
        ...
        let delivered = maju_db::relay_admin_actions::mark_outbox_delivered(
            &pool,
            outbox_id,
            valid_claim_token,
        )
        ...
        let stale_fail = maju_db::relay_admin_actions::fail_outbox_row(
            &pool,
            outbox_id,
            stale_claim_token,
            "stale error",
        ...
        let stale_delivered = maju_db::relay_admin_actions::mark_outbox_delivered(
            &pool,
            outbox_id,
            stale_claim_token,
        )
    ...
    async fn failed_system_message_insert_not_marked_delivered() {
        ...
        //     `maju.created_at_floor` GUC session-locally (not database-globally).
        ...
        .await
        .expect("execute_ban");
        let _ = maju_db::relay_admin_actions::finalize_success(
            &pool,
            action_id,
            cid,
            report_id,
        ...
        // Build a dedicated pool whose after_connect sets maju.created_at_floor = 5
        ...
        let floor_pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    sqlx::query("SELECT set_config('maju.created_at_floor', '5', false)")
                        .execute(conn)
                        .await?;
                    Ok(())
                })
    ...
    async fn reporter_notice_duplicate_delivery_persists_exactly_one() {
        ...
        let _ = maju_db::relay_admin_actions::begin_enforcing(&pool, action_id)
            .await
            .expect("begin_enforcing");
        let lt = match maju_db::relay_admin_actions::acquire_action_lease(
            &pool,
            action_id,
            chrono::Utc::now() + chrono::Duration::seconds(60),
        )
        ...
        let record_a: maju_db::relay_admin_actions::OutboxRecord = {
        ...
        let record_b = maju_db::relay_admin_actions::OutboxRecord {
            id: record_a.id,
