...
use base64::Engine;
use maju_audit::{AuditAction, NewAuditEntry};
use maju_core::tenant::TenantContext;
use maju_media::{BlobDescriptor, MediaError, UploadAttribution, UploadNetworkInfo};
...
fn should_stream_as_video(sniff: &[u8]) -> bool {
    infer::get(sniff).is_some_and(|kind| kind.mime_type() == "video/mp4")
        || maju_media::looks_like_iso_bmff(sniff)
}
...
impl FromRequestParts<Arc<AppState>> for AuthenticatedUpload {
    ...
    async fn from_request_parts(
        ...
        maju_media::auth::verify_blossom_auth_event(&auth_event, Some(tenant.host()), 3600)?;
        ...
        if upload_rate_limited(state, tenant.community(), &auth_event.pubkey) {
            metrics::counter!("maju_media_upload_rejections_total", "reason" => "rate_limit")
                .increment(1);
        ...
        let upload_permit = acquire_upload_permit(state, tenant.community(), &auth_event.pubkey)
            .inspect_err(|_| {
                metrics::counter!("maju_media_upload_rejections_total", "reason" => "concurrency")
                    .increment(1);
            })?;
...
/// (`MAJU_MEDIA_UPLOAD_RECORDS`). Returns `None` when the feature is off —
...
///   (`MAJU_MEDIA_UPLOAD_IP_HEADER`) and validated as a public IP —
...
async fn upload_attribution(
    ...
    let ip = header_value(&cfg.upload_ip_header).and_then(maju_media::parse_public_ip);
    let port = ip.and(header_value(&cfg.upload_port_header).and_then(maju_media::parse_port));
...
fn serving_write_error(error: anyhow::Error) -> MediaError {
    if maju_deletion::ServingWriteGuard::acquisition_is_fenced(&error) {
...
pub async fn upload_blob(
    ...
    let serving_write =
        maju_deletion::acquire_serving_write(&state.db, auth.tenant.community(), "media_upload")
            .await
            .map_err(serving_write_error)?;

    if auth.route_mode == UploadRouteMode::LegacyMedia {
        metrics::counter!("maju_media_legacy_upload_route_total").increment(1);
    }
    ...
    let mut descriptor = serving_write
        .protect(async {
            Ok(if should_stream_as_video(&sniff) {
                ...
                maju_media::process_video_upload(
                    &state.media_storage,
                    &state.config.media,
                    &auth.tenant,
                    &auth.auth_event,
                ...
                if is_image {
                    ...
                    maju_media::process_file_upload(
                        &state.media_storage,
                        &state.config.media,
                        &auth.tenant,
                        &auth.auth_event,
        ...
        .await
        .map_err(|error| {
            if maju_deletion::ServingWriteGuard::is_lease_lost(&error) {
...
async fn authenticate_media_read(
    ...
    maju_media::auth::verify_blossom_get_auth(&auth_event, sha256, Some(tenant.host()), 3600)?;
...
pub(crate) async fn serve_blob_for_tenant(
    ...
    let disposition = if maju_media::serve_inline(&content_type) {
        "inline"
    } else {
        "attachment"
    };
...
async fn resolve_s3_key(
    storage: &maju_media::MediaStorage,
    tenant: &TenantContext,
    sha256_ext: &str,
...
mod tests {
    ...
    fn serving_write_error_taxonomy_separates_fence_from_backend_failure() {
        let fenced = anyhow::Error::from(maju_db::DbError::AccessDenied("fenced".to_string()));
        assert!(matches!(
            serving_write_error(fenced),
            MediaError::CommunityWriteFenced
        ));
        let backend = anyhow::Error::from(maju_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
        assert!(matches!(
    ...
    async fn upload_rate_limiter_is_scoped_by_community() {
        ...
        let community_a = maju_core::CommunityId::from_uuid(Uuid::from_u128(0xAAAA));
        let community_b = maju_core::CommunityId::from_uuid(Uuid::from_u128(0xBBBB));
