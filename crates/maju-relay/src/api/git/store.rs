...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `maju-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`maju_media::MediaStorage::new`]:
    ...
    pub fn new(
        ...
        bucket_name: &str,
        region: &str,
        addressing_style: maju_media::config::S3AddressingStyle,
        ...
        let bucket = match addressing_style {
            maju_media::config::S3AddressingStyle::Path => bucket.with_path_style(),
            maju_media::config::S3AddressingStyle::Virtual => bucket,
        };
        Ok(Self {
...
mod tests {
    ...
    fn constructor_applies_both_addressing_styles() {
        for (style, expected_url, path_style) in [
            (
                maju_media::config::S3AddressingStyle::Path,
                "https://storage.example/maju-git",
                true,
            ),
            (
                maju_media::config::S3AddressingStyle::Virtual,
                "https://maju-git.storage.example",
                false,
            ),
        ] {
            let store = GitStore::new(
    ...
    fn partial_static_keys_are_rejected() {
        for (access, secret) in [("maju_dev", ""), ("", "maju_dev_secret")] {
...
mod probe {
    ...
    //!
    //! Run manually:
    //!   MAJU_GIT_S3_PROBE=1 cargo test -p maju-relay --lib \
    //!     api::git::store::probe -- --nocapture --test-threads=1
    //!
    //! Pre-req: `docker compose up minio` and the `maju-git` bucket exists.

    use super::*;

    fn probe_enabled() -> bool {
        std::env::var("MAJU_GIT_S3_PROBE").as_deref() == Ok("1")
    }
    ...
    fn store() -> GitStore {
        ...
        let endpoint =
            std::env::var("MAJU_S3_ENDPOINT").unwrap_or_else(|_| "http://localhost:9000".into());
        let access_key = std::env::var("MAJU_S3_ACCESS_KEY").unwrap_or_else(|_| "maju_dev".into());
        let secret_key =
            std::env::var("MAJU_S3_SECRET_KEY").unwrap_or_else(|_| "maju_dev_secret".into());
        let bucket = std::env::var("MAJU_S3_BUCKET").unwrap_or_else(|_| "maju-git".into());
        let region = std::env::var("MAJU_S3_REGION").unwrap_or_else(|_| "us-east-1".into());
        let addressing_style = std::env::var("MAJU_S3_ADDRESSING_STYLE")
            .unwrap_or_else(|_| "path".into())
            .parse()
            .expect("MAJU_S3_ADDRESSING_STYLE must be path or virtual");
        GitStore::new(
            &endpoint,
            &access_key,
            &secret_key,
    ...
    async fn probe_412_surfacing() {
        if !probe_enabled() {
            eprintln!("skipping: set MAJU_GIT_S3_PROBE=1 to run against live MinIO");
            return;
        }
        let st = store();
