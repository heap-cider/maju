//! Smart HTTP git transport for Maju.
//!
...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| {
                Response::builder()
                    ...
                    .header(
                        "WWW-Authenticate",
                        format!("Nostr realm=\"maju\", method=\"{method}\""),
                    )
        ...
        let pubkey =
            maju_auth::nip98::verify_nip98_event(&event_json, &expected_url, &event_method, None)
                .map_err(|e| {
...
async fn git_restriction_state(
    ...
    community: maju_core::CommunityId,
    pubkey: &nostr::PublicKey,
) -> Result<maju_db::moderation::RestrictionState, Response> {
...
fn enforce_git_ban(restriction: &maju_db::moderation::RestrictionState) -> Result<(), StatusCode> {
...
fn enforce_git_ban_cascade(
    agent: &maju_db::moderation::RestrictionState,
    owner: Option<&maju_db::moderation::RestrictionState>,
) -> Result<(), StatusCode> {
...
fn acquire_git_permit(
    ...
    Arc::clone(&state.git_semaphore)
        ...
        .map_err(|_| {
            metrics::counter!(
                "maju_git_semaphore_rejections_total",
                "operation" => operation
            )
            .increment(1);
            Response::builder()
...
///    resolve;
/// 2. its `maju-channel` tag → channel UUID;
/// 3. [`maju_db::Db::get_member_role`] for the caller — a read is allowed
///    only on `Ok(Some(role))` with a role the relay recognizes.
///
/// Fail-closed: missing/deleted announcement, invalid owner, missing or
/// malformed `maju-channel` binding, non-member, unknown role, and every DB
...
/// `maju-channel` tag, and the repo then 404s forever with no explanation
        ...
        RepoBinding::NotBound => {
            ...
            if repo_event.event.pubkey == *caller {
                ...
                return Err((
                    ...
                    format!(
                        "run: maju repos bind --id {repo_name} --channel <channel-uuid> — repository {repo_name:?} has no channel binding, so the relay cannot authorize access"
                    ),
                )
                    .into_response());
            }
            warn!(repo = %repo_name, "git read gate: missing maju-channel binding (deny)");
            return Err(denied());
        }
        RepoBinding::Broken => {
            warn!(repo = %repo_name, "git read gate: malformed maju-channel binding (deny)");
            return Err(denied());
        }
    };
...
fn read_role_allows(role: Option<&str>) -> bool {
    match role {
        Some(r) => r.parse::<maju_core::channel::MemberRole>().is_ok(),
        None => false,
    }
}
...
/// 0000
/// <pkt><head-oid> HEAD\0<caps> symref=HEAD:<head-ref> object-format=<fmt> agent=maju-git\n
...
pub async fn receive_pack(
    ...
    let mut hook_env = vec![
        ("MAJU_HOOK_URL", hook_url),
        (
            "MAJU_HOOK_SECRET",
            state.config.git_hook_hmac_secret.clone(),
        ),
        ("MAJU_REPO_ID", repo_name.to_string()),
        ("MAJU_REPO_OWNER", params.owner.clone()),
        (
            "MAJU_COMMUNITY_ID",
            auth.tenant.community().as_uuid().to_string(),
        ),
        ("MAJU_PUSHER_PUBKEY", pusher_hex.clone()),
    ];
    ...
    S: futures_util::Stream<Item = Result<bytes::Bytes, std::io::Error>>,
{
    ...
    fn poll_next(
        ...
        if self.deadline.as_mut().poll(cx).is_ready() {
            self.finished = true;
            metrics::counter!("maju_git_upload_pack_timeouts_total").increment(1);
...
impl<S> Drop for TimedByteStream<S> {
    fn drop(&mut self) {
        metrics::histogram!("maju_git_upload_pack_stream_seconds")
            .record(self.started_at.elapsed().as_secs_f64());
        metrics::histogram!("maju_git_upload_pack_stream_bytes").record(self.streamed_bytes as f64);
    }
}
...
async fn finalize_push_inner(
    ...
    let serving_write = match maju_deletion::acquire_serving_write(
        &state.db,
        ...
        match build_ref_state_event(&inputs, &state.relay_keypair) {
            Ok(event) => {
                ...
                let insert_result = if hooks.fail_ref_state_insert {
                    Err(maju_db::DbError::InvalidData(
...
mod track_c_tests {
    ...
    fn receive_pack_config_allows_deleting_current_branch() {
        ...
        assert_git_success(
            run_test_git(source.as_path(), &["config", "user.name", "Maju Test"], &[]),
            "configure user name",
        );
        assert_git_success(
            run_test_git(
                source.as_path(),
                &["config", "user.email", "maju-test@example.com"],
                &[],
            ),
    ...
    async fn finalize_test_state() -> (Arc<AppState>, sqlx::PgPool) {
        ...
        config.database_url = std::env::var("MAJU_TEST_DATABASE_URL")
    ...
    async fn approved_deletion(
        ...
        maju_db::deletion::DeletionRequest,
        maju_db::deletion::ClaimedDeletion,
    ) {
        ...
        let inventory = FrozenInventory {
            ...
            storage: StorageManifest {
                version: 4,
                prefixes: maju_media::tenant_prefixes(*request.community_id.as_uuid())
                    .into_iter()
    ...
    async fn repo_announcement_holds_serving_lease_until_pointer_is_seeded() {
        ...
        assert!(matches!(
            error,
            maju_db::DbError::ServingWritesNotDrained { .. }
        ));
        ...
        assert_eq!(
            state
                ...
                .stage,
            maju_db::deletion::DeletionStage::Fenced
        );
        drop(state);
        pool.close().await;
    }
    ...
    async fn finalize_push_holds_serving_lease_through_post_cas_publication() {
        ...
        let mut query = maju_db::event::EventQuery::for_community(community);
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = maju_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            maju_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
...
mod sec005_postgres_tests {
    ...
    fn durable_ban_denies_git_even_with_otherwise_valid_auth() {
        let restriction = maju_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
    ...
    fn restriction(banned: bool) -> maju_db::moderation::RestrictionState {
        maju_db::moderation::RestrictionState {
            banned,
            muted_until: None,
        }
    }
    ...
    async fn setup_db() -> maju_db::Db {
        let url = std::env::var("MAJU_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_string());
        let pool = sqlx::PgPool::connect(&url).await.expect("connect test DB");
        maju_db::Db::from_pool(pool)
    }

    /// How the fixture's kind:30617 binds (or fails to bind) a channel.
    enum Binding {
        /// `maju-channel` tag carrying the fixture channel's UUID.
        Channel,
        /// No `maju-channel` tag at all.
        Missing,
        /// `maju-channel` tag whose value is not a UUID.
        Malformed,
        /// `maju-channel` tag carrying a well-formed UUID that names no
    ...
    async fn setup_repo(binding: Binding) -> RepoFixture {
        ...
        db.create_channel_with_id(
            ...
            maju_db::channel::ChannelType::Stream,
            maju_db::channel::ChannelVisibility::Open,
            None,
            &creator_pk,
            None,
        )
        ...
        db.add_member(
            ...
            channel,
            &member_pk,
            maju_core::channel::MemberRole::Member,
            Some(&creator_pk),
        )
        .await
        .expect("member");
        ...
        match binding {
            Binding::Channel => {
                tags.push(Tag::parse(["maju-channel", &channel.to_string()]).unwrap());
            }
            Binding::Missing => {}
            Binding::Malformed => {
                tags.push(Tag::parse(["maju-channel", "not-a-uuid"]).unwrap());
            }
            Binding::UnknownChannel => {
                tags.push(Tag::parse(["maju-channel", &uuid::Uuid::new_v4().to_string()]).unwrap());
            }
        }
    ...
    async fn read_gate_denies_missing_or_malformed_binding_and_absent_repo() {
        // Missing maju-channel tag → deny even for a channel member, with
        ...
        // Malformed maju-channel tag → deny with the generic body EVEN FOR
    ...
    async fn read_gate_gives_author_of_unbound_repo_remediation_body() {
        ...
        assert!(
            body.starts_with(&format!("run: maju repos bind --id {}", f.repo)),
    ...
    async fn setup_ban_community() -> (maju_db::Db, maju_core::CommunityId, Vec<u8>) {
        let db = setup_db().await;
    ...
    async fn ban_gate_cascades_to_a_banned_nip_oa_owner() {
        ...
        let auth_tag = maju_sdk::nip_oa::compute_auth_tag(&owner, &agent.public_key(), "kind=9")
            .expect("auth tag");
    ...
    async fn ban_gate_fails_closed_with_503_when_the_store_is_unreachable() {
        ...
        let community = maju_core::CommunityId::from_uuid(uuid::Uuid::new_v4());
        let (status, body) = denial_parts(
