...
//!   ]
//!
//! Maju extension: a `p` tag carrying the pusher's pubkey (or the repo
//! owner on creation events) so subscribers can filter by author of state
//! transition. Not part of NIP-34 but consistent with the rest of maju's
...
pub struct RefStateInputs<'a> {
    ...
    /// Pubkey to include in the `p` tag (maju extension). On push, this is
...
pub fn build_ref_state_event(
    ...
    // p-tag: maju extension (pusher or owner pubkey).
