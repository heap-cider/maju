...
//! 4. Otherwise resolves the pusher's channel role via maju-channel binding
//! 5. Promotes Bot → Member (bots in a channel push as members)
//! 6. Calls `maju_core::git_perms::evaluate_push()`
...
use maju_core::channel::MemberRole;
use maju_core::git_perms::{
...
pub async fn hook_policy_check(
    ...
    let community = maju_core::CommunityId::from_uuid(community_uuid);
    ...
    let rules = match parse_protection_tags(&tags) {
        Ok(parsed) => {
            ...
            for unknown in &parsed.unknown_rules {
                warn!(repo = %req.repo_id, rule = %unknown, "unknown maju-protect rule (skipped)");
            }
            parsed.rules
        }
        Err(e) => {
    ...
    let channel_id = match crate::api::git::binding::resolve_repo_binding(&repo_event.event) {
        ...
        crate::api::git::binding::RepoBinding::Broken => {
            warn!(repo = %req.repo_id, "hook callback: broken maju-channel binding");
    ...
    let role = if is_repo_owner || is_managed_agent_owner {
        ...
        match channel_id {
            None => {
                warn!(repo = %req.repo_id, "hook callback: no maju-channel binding");
                // Declared cross-component contract — see the const docs in
                // maju-core::git_perms for who consumes the token and why
...
mod postgres_tests {
    ...
    fn no_channel_binding_body_satisfies_old_and_new_matchers() {
        assert!(
            GIT_NO_CHANNEL_BINDING_BODY.starts_with(&format!(
                "{}: ",
                maju_core::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN
            )),
    ...
    fn bash_hmac_matches_rust_hmac() {
        ...
        let bash_script = format!(
            r#"
export LC_ALL=C
MAJU_REPO_ID="{repo_id}"
MAJU_REPO_OWNER="{repo_owner}"
MAJU_COMMUNITY_ID="{community_id}"
MAJU_PUSHER_PUBKEY="{pusher}"
MAJU_HOOK_SECRET="{secret}"
TIMESTAMP="{timestamp}"
...
REPO_ID_LEN=${{#MAJU_REPO_ID}}
HMAC_INPUT="${{REPO_ID_LEN}}:${{MAJU_REPO_ID}}|${{MAJU_REPO_OWNER}}|${{MAJU_COMMUNITY_ID}}|${{MAJU_PUSHER_PUBKEY}}|"
...
# Compute HMAC-SHA256
printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$MAJU_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //'
"#,
            repo_id = repo_id,
...
MAJU_REPO_ID="{repo_id}"
REPO_ID_LEN=${{#MAJU_REPO_ID}}
HMAC_INPUT="${{REPO_ID_LEN}}:${{MAJU_REPO_ID}}|{owner}|{community_id}|{pusher}|"
sort "$HMAC_FILE" | while IFS=' ' read -r ref_name old_oid new_oid is_anc; do
    REF_LEN=${{#ref_name}}
    printf '%s%s%s:%s%s' "$old_oid" "$new_oid" "$REF_LEN" "$ref_name" "$is_anc"
done > "$HMAC_FILE.concat"
    ...
    async fn push_gate_denies_owner_through_broken_binding() {
        ...
        let response = owner_push_response(
            ...
            vec![
                ...
                Tag::parse(["maju-channel", &uuid::Uuid::new_v4().to_string()]).unwrap(),
            ],
        )
        .await;
        ...
        assert!(
            !body.contains(maju_core::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN),
