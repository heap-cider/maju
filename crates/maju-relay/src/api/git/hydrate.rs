...
pub async fn hydrate_for_read(
    ...
    let outcome = match &result {
        Ok(Some(repo)) => {
            metrics::histogram!("maju_git_hydrate_bytes").record(repo.hydrated_bytes() as f64);
            metrics::histogram!("maju_git_hydrate_packs").record(repo.hydrated_packs() as f64);
            "success"
        }
        Ok(None) => "missing",
    ...
    metrics::counter!("maju_git_hydrations_total", "outcome" => outcome).increment(1);
    metrics::histogram!("maju_git_hydrate_seconds", "outcome" => outcome)
