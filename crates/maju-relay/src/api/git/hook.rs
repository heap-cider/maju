...
/// - `MAJU_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `MAJU_HOOK_SECRET` — per-push HMAC secret
/// - `MAJU_REPO_ID` — repo identifier (d-tag)
/// - `MAJU_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `MAJU_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Maju pre-receive hook — FAIL-CLOSED
...
: "${MAJU_REPO_ID:?error: MAJU_REPO_ID not set}"
: "${MAJU_REPO_OWNER:?error: MAJU_REPO_OWNER not set}"
: "${MAJU_COMMUNITY_ID:?error: MAJU_COMMUNITY_ID not set}"
: "${MAJU_PUSHER_PUBKEY:?error: MAJU_PUSHER_PUBKEY not set}"
: "${MAJU_HOOK_URL:?error: MAJU_HOOK_URL not set}"
: "${MAJU_HOOK_SECRET:?error: MAJU_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$MAJU_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$MAJU_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${MAJU_REPO_OWNER}\",\"community_id\":\"${MAJU_COMMUNITY_ID}\",\"pusher_pubkey\":\"${MAJU_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
