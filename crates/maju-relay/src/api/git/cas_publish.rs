...
fn record_compaction(
    ...
    metrics::counter!("maju_git_pack_compactions_total", "outcome" => outcome).increment(1);
    metrics::histogram!("maju_git_pack_compaction_seconds", "outcome" => outcome)
        .record(started_at.elapsed().as_secs_f64());
    metrics::histogram!("maju_git_pack_compaction_packs_before").record(packs_before as f64);
    if let Some(packs_after) = packs_after {
        metrics::histogram!("maju_git_pack_compaction_packs_after").record(packs_after as f64);
    }
    if let Some(compacted_bytes) = compacted_bytes {
        metrics::histogram!("maju_git_pack_compaction_bytes").record(compacted_bytes as f64);
    }
}
...
async fn cas_publish_inner(
    ...
    let m_after = if let Some(manifest) = compacted_manifest {
        ...
        if pack_bytes.is_some() && packs_before >= MAX_MANIFEST_PACKS {
            ...
            metrics::counter!("maju_git_pack_compaction_required_failures_total").increment(1);
            return Err(error);
        }
...
mod tests {
    ...
    fn live_store() -> GitStore {
        let endpoint = std::env::var("MAJU_GIT_S3_ENDPOINT")
            .or_else(|_| std::env::var("MAJU_S3_ENDPOINT"))
            .unwrap_or_else(|_| "http://localhost:9000".into());
        let access_key = std::env::var("MAJU_GIT_S3_ACCESS_KEY")
            .or_else(|_| std::env::var("MAJU_S3_ACCESS_KEY"))
            .unwrap_or_else(|_| "maju_dev".into());
        let secret_key = std::env::var("MAJU_GIT_S3_SECRET_KEY")
            .or_else(|_| std::env::var("MAJU_S3_SECRET_KEY"))
            .unwrap_or_else(|_| "maju_dev_secret".into());
        let bucket = std::env::var("MAJU_GIT_S3_BUCKET")
            .or_else(|_| std::env::var("MAJU_S3_BUCKET"))
            .unwrap_or_else(|_| "maju-media".into());
        let region = std::env::var("MAJU_GIT_S3_REGION")
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            maju_core::CommunityId::from_uuid(uuid::Uuid::from_u128(1)),
            "git.example",
        )
    }
