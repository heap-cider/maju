...
//!
//! The `maju-channel` tag on a kind:30617 announcement IS the git ACL: the
...
//!
//! Only the *first* `maju-channel` tag is considered, and it must carry a
...
//! who can append a second `maju-channel` tag would pick the channel.
//!
...
pub enum RepoBinding {
    /// No `maju-channel` tag at all. The announcement author (the only
    /// identity that can rebind — 30617 is keyed by `(author, d)`) may be
    /// offered remediation; everyone else gets the generic denial.
    NotBound,
    /// First `maju-channel` tag carries a valid UUID.
    Bound(Uuid),
    /// First `maju-channel` tag exists but its value is not a UUID.
