...
use maju_auth::{LimitType, Nip98ReplayGuard, DEFAULT_REPLAY_TTL_SECS};
...
pub(crate) async fn enforce_http_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("maju_admission_rejections_total", "transport" => "http", "reason" => "quota").increment(1);
            Err(api_error(
        ...
        Err(crate::admission::AdmissionError::Unavailable) => {
            metrics::counter!("maju_admission_rejections_total", "transport" => "http", "reason" => "unavailable").increment(1);
            Err(api_error(
...
pub(crate) fn verify_bridge_auth_with_options(
        ...
        .and_then(|s| s.strip_prefix("Nostr "))
    {
        ...
        let pubkey = maju_auth::verify_nip98_event(&event_json, url, method, body)
...
fn extract_maju_channel(raw: &Value) -> Option<&str> {
    raw.get("#maju-channel")
...
fn extract_search_mode(raw: &Value) -> maju_search::SearchMode {
    match raw
        .get("search_mode")
        .or_else(|| raw.get("searchMode"))
        .and_then(Value::as_str)
    {
        Some("prefix") => maju_search::SearchMode::Prefix,
        _ => maju_search::SearchMode::FullText,
    }
}
...
const WINDOW_AUX_KINDS: [u32; 4] = [
    maju_core::kind::KIND_DELETION,
    maju_core::kind::KIND_REACTION,
    maju_core::kind::KIND_NIP29_DELETE_EVENT,
    maju_core::kind::KIND_STREAM_MESSAGE_EDIT,
];
...
/// (`maju_db::DEFAULT_MAX_PAGE_LIMIT`) so each page is one full query.
const AUX_PAGE_LIMIT: i64 = maju_db::DEFAULT_MAX_PAGE_LIMIT;
...
fn build_aux_query(
    community: maju_core::CommunityId,
    target_ids: Vec<String>,
    kinds: &[u32],
) -> maju_db::EventQuery {
    let mut query = maju_db::EventQuery::for_community(community);
...
enum AuxReader<'a> {
    Session(&'a mut maju_db::ReadSession),
    Routed(&'a maju_db::Db, &'static str),
    #[cfg(test)]
    Fake(&'a mut (dyn FnMut(&maju_db::EventQuery) -> Vec<maju_core::StoredEvent> + Send)),
}

impl AuxReader<'_> {
    async fn fetch(
        &mut self,
        query: &maju_db::EventQuery,
    ) -> maju_db::Result<Vec<maju_core::StoredEvent>> {
        match self {
...
async fn handle_channel_window_filter(
    state: &AppState,
    tenant: &maju_core::TenantContext,
    raw: &Value,
    filter: &nostr::Filter,
    accessible_channels: &[uuid::Uuid],
    events: &mut Vec<Value>,
) -> Result<(), (StatusCode, Json<Value>)> {
    use maju_core::kind::{KIND_THREAD_SUMMARY, KIND_WINDOW_BOUNDS};
...
fn event_in_accessible_channel(se: &maju_core::StoredEvent, accessible: &[uuid::Uuid]) -> bool {
    match se.channel_id {
...
async fn submit_event_authed(
    ...
    let kind_u32 = maju_core::kind::event_kind_u32(&event);
    let auth = IngestAuth::Http {
        pubkey,
        scopes: maju_auth::Scope::all_known(), // Pure Nostr: full scopes, channel access via membership
...
async fn query_events_authed(
    ...
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        ...
        for feed_type in &feed_types {
            ...
            for se in type_events {
                ...
                if !maju_core::filter::reader_authorized_for_event(&se.event, &authed_pubkey_hex) {
                    continue;
                }
            ...
            for hop_kinds in [&WINDOW_AUX_KINDS[..], &WINDOW_AUX_DELETE_KINDS[..]] {
                ...
                for se in aux_events {
                    if !seen_aux.insert(se.event.id)
                        ...
                        || !maju_core::filter::reader_authorized_for_event(
                            &se.event,
                            &authed_pubkey_hex,
                        )
                    {
    ...
    let mut catchall_queries: Vec<(usize, maju_db::EventQuery)> = Vec::new();
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        if handled.contains(&idx) {
            continue;
        }
        ...
        if let Some(channel) = extract_maju_channel(raw) {
            query.custom_tag = Some(("maju-channel".into(), channel.into()));
        }
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    ...
                    if !maju_core::filter::filters_match(std::slice::from_ref(filter), &se) {
                        continue;
                    }
                    ...
                    .await
                {
                    Ok(stored_events) => {
                        if crate::handlers::req::count_fallback_exceeded(stored_events.len()) {
                            metrics::counter!("maju_count_fallback_rejections_total").increment(1);
                            return Err(api_error(
                                StatusCode::BAD_REQUEST,
...
fn search_hit_accepted(
    filter: &nostr::Filter,
    stored: &maju_core::StoredEvent,
    accessible_channels: &[uuid::Uuid],
    reader_pubkey_hex: &str,
) -> bool {
    if !maju_core::filter::filters_match(std::slice::from_ref(filter), stored) {
        return false;
    }
    ...
    if !maju_core::filter::reader_authorized_for_event(&stored.event, reader_pubkey_hex) {
        return false;
    }
    true
}
    ...
    for (raw, filter) in raw_filters.iter().zip(filters) {
        ...
        let filter_channel_scope =
            if let Some(vs) = filter.generic_tags.get(&h_tag).filter(|vs| !vs.is_empty()) {
                ...
                maju_search::ChannelScope::Channels(valid)
            } else {
                channel_scope.clone()
            };
        ...
        let search_query = maju_search::SearchQuery {
            community: tenant.community(),
            q: search_text,
        ...
        let event_map: std::collections::HashMap<[u8; 32], &maju_core::StoredEvent> = stored_events
            .iter()
...
pub async fn workflow_webhook(
    ...
    let def: maju_workflow::WorkflowDef = serde_json::from_value(workflow.definition.clone())
        .map_err(|e| super::internal_error(&format!("corrupt workflow definition: {e}")))?;

    if !matches!(def.trigger, maju_workflow::TriggerDef::Webhook) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    let mut trigger_ctx = maju_workflow::executor::TriggerContext {
        channel_id: workflow
            .channel_id
            .map(|ch| ch.to_string())
            .unwrap_or_default(),
    ...
    if !workflow.enabled || workflow.status != maju_db::workflow::WorkflowStatus::Active {
    ...
    tokio::spawn(async move {
        let def: maju_workflow::WorkflowDef = match serde_json::from_value(def_value) {
            Ok(d) => d,
            Err(e) => {
        ...
        let result = maju_workflow::executor::execute_from_step(
            &engine,
            community_id,
            run_id,
            &def,
...
async fn synthesize_presence(
    pubsub: &maju_pubsub::PubSubManager,
    relay_keypair: &nostr::Keys,
    tenant: &maju_core::tenant::TenantContext,
    filters: &[nostr::Filter],
) -> Option<Result<Vec<Value>, (StatusCode, Json<Value>)>> {
    use maju_core::kind::{KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE};
...
fn report_json(r: &maju_db::moderation::ReportRecord) -> Value {
    let (target_kind, target) = match &r.target {
        maju_db::moderation::ReportTarget::Event(id) => ("event", hex::encode(id)),
        maju_db::moderation::ReportTarget::Pubkey(pk) => ("pubkey", hex::encode(pk)),
        maju_db::moderation::ReportTarget::Blob(sha) => ("blob", hex::encode(sha)),
    };
    serde_json::json!({
        "id": r.id,
...
fn action_json(a: &maju_db::moderation::ActionRecord) -> Value {
    serde_json::json!({
        "id": a.id,
...
fn ban_json(b: &maju_db::moderation::BanRecord) -> Value {
    serde_json::json!({
...
mod postgres_tests {
    ...
    fn fresh_tenant(host: &str) -> TenantContext {
        TenantContext::resolved(
            maju_core::CommunityId::from_uuid(uuid::Uuid::new_v4()),
            host,
        )
    }
    ...
    async fn synthesize_presence_surfaces_redis_failure_as_error_response() {
        use maju_core::kind::KIND_PRESENCE_SNAPSHOT;
        ...
        let pubsub = maju_pubsub::PubSubManager::new("redis://127.0.0.1:1", dead_pool)
            .await
    ...
    fn aux_event(keys: &Keys, created_at: u64, content: &str) -> maju_core::StoredEvent {
        let ev = EventBuilder::new(Kind::Custom(7), content)
            .custom_created_at(nostr::Timestamp::from(created_at))
            .sign_with_keys(keys)
            .unwrap();
        maju_core::StoredEvent::new(ev, None)
    }
    ...
    async fn query_all_pages_drains_past_the_page_clamp() {
        ...
        let mut fetch = |q: &maju_db::EventQuery| {
    ...
    async fn query_all_pages_stops_at_one_short_page() {
        ...
        let query = build_aux_query(tenant.community(), vec!["root".into()], &WINDOW_AUX_KINDS);
        let mut calls = 0;
        let mut fetch = |_q: &maju_db::EventQuery| {
            calls += 1;
            Vec::new()
        };
    ...
    fn bridge_search_mode_extension_defaults_to_full_text() {
        assert_eq!(
            ...
            maju_search::SearchMode::FullText
        );
        assert_eq!(
    ...
    async fn nip98_replay_guard_rejects_cross_pod_replay_on_bridge_path() {
        let pool = redis_pool();
        let pod_a = maju_pubsub::RedisNip98ReplayGuard::new(pool.clone());
        let pod_b = maju_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    async fn nip98_replay_guard_rejects_same_pod_same_community_replay() {
        let pool = redis_pool();
        let pod = maju_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    /// `maju-auth/src/nip98_replay.rs:70-73`).
    ///
    ...
    /// `maju_auth::nip98::tests::make_nip98_event` so the bridge tests don't
    /// reach into maju-auth's test scope.
    ...
    /// `maju moderation reports` / `audit` 401'd on a NIP-98 URL mismatch in
    ...
    fn verify_nip42_rejects_event_signed_for_wrong_communitys_host() {
        ...
        assert!(
            matches!(err, maju_auth::AuthError::RelayUrlMismatch),
    ...
    fn engram_envelope(agent: &Keys, owner_hex: &str) -> maju_core::StoredEvent {
    ...
    #[test]
    fn extract_maju_channel_requires_one_string_value() {
        assert_eq!(
            extract_maju_channel(&serde_json::json!({"#maju-channel": ["channel-a"]})),
            Some("channel-a")
        );
        assert_eq!(
            extract_maju_channel(&serde_json::json!({"#maju-channel": ["channel-a", "channel-b"]})),
            None
        );
        assert_eq!(
            extract_maju_channel(&serde_json::json!({"#maju-channel": [42]})),
            None
        );
    }
    ...
    fn extract_page_offset_sizes_pages_from_clamped_limit() {
        let clamped = maju_db::DEFAULT_MAX_PAGE_LIMIT;
    ...
    fn event_accessible_no_channel() {
        ...
        let ev = EventBuilder::new(Kind::Custom(1), "test")
            .sign_with_keys(&keys)
            .unwrap();
        let se = maju_core::StoredEvent::new(ev, None);
    ...
    fn event_accessible_matching_channel() {
        ...
        let mut se = maju_core::StoredEvent::new(ev, None);
    ...
    fn search_hit_rejects_dm_visibility_for_kindless_ids_third_party() {
        ...
        let ev = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_DM_VISIBILITY as u16), "")
            .tags([d_tag, p_tag])
            .sign_with_keys(&relay)
            .expect("sign snapshot");
        let stored = maju_core::StoredEvent::new(ev.clone(), None);
    ...
    // increments maju_events_rejected_total{transport="http"}.  They are
    ...
    async fn bridge_handler_test_state() -> Option<Arc<crate::state::AppState>> {
        ...
        let media_storage = maju_media::MediaStorage::new(&config.media).ok()?;
    ...
    /// Collect maju_events_rejected_total with (transport, reason) labels from
    ...
    fn http_reject_counts(
        ...
        snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "maju_events_rejected_total")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("maju_events_rejected_total must be a counter");
                };
    ...
    /// increment maju_events_rejected_total{transport="http",reason="invalid"}.
    ///
    ...
    fn submit_event_relay_only_kind_increments_http_transport_counter() {
        ...
        let relay_only_event = EventBuilder::new(
            Kind::Custom(maju_core::kind::KIND_NIP43_MEMBERSHIP_LIST as u16),
            "",
        )
