...
//! GIF bytes are never downloaded, cached, or stored by Maju.
//!
...
async fn enforce_search_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("maju_gif_search_rejections_total", "reason" => "quota").increment(1);
            Err(api_error(
...
mod tests {
    ...
    async fn unconfigured_test_state() -> Arc<AppState> {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            .connect_lazy("postgres://maju:maju_dev@127.0.0.1:1/maju") // sadscan:disable np.postgres.1
        ...
        let media_storage =
            maju_media::MediaStorage::new(&config.media).expect("test media storage config");
        let (state, _audit_shutdown) = AppState::new(
            config,
            db,
            redis_pool,
            None::<maju_audit::AuditService>,
            pubsub,
            auth,
            search,
            workflow_engine,
