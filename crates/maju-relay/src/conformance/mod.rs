...
//! from `maju-conformance`. See `crates/maju-conformance/src/lib.rs` for
...
///   [`maju_db::Maju::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, maju_core::CommunityId>,
...
/// [`maju_db::Maju::communities_of_channels`] over the distinct
/// channel ids in `rows`.
