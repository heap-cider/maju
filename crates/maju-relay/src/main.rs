...
async fn main() -> anyhow::Result<()> {
    ...
    let relay_keypair = if let Some(hex) = &config.relay_private_key {
        nostr::Keys::parse(hex)
            .map_err(|e| anyhow::anyhow!("invalid MAJU_RELAY_PRIVATE_KEY: {e}"))?
        ...
        tracing::warn!(
            ...
            "Using hardcoded dev relay keypair (MAJU_REQUIRE_AUTH_TOKEN=false). \
             Set MAJU_RELAY_PRIVATE_KEY for production."
        );
        keys
    } else {
        panic!(
            "MAJU_RELAY_PRIVATE_KEY must be set when MAJU_REQUIRE_AUTH_TOKEN=true. \
    ...
    if config.require_relay_membership {
        match maju_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(&state).await
        {
        ...
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                match maju_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(
                    &reconcile_state,
                )
                .await
                {
...
mod tests {
    ...
    use super::{
        dropped_in_memory_keys, idle_timeout_secs, maju_auto_migrate_enabled,
