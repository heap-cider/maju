...
fn log_env_filter(rust_log: Option<&str>) -> EnvFilter {
    EnvFilter::new(rust_log.unwrap_or("maju_relay=info"))
}
use uuid::Uuid;

use maju_audit::AuditService;
use maju_auth::AuthService;
use maju_core::CommunityId;
use maju_db::{Db, DbConfig};
use maju_pubsub::PubSubManager;
use maju_search::SearchService;

use maju_relay::config::{Config, MAX_DRAIN_JITTER_MS};
use maju_relay::lifecycle::{BootTracker, LifecycleReason, StartupPhase};
use maju_relay::metrics as relay_metrics;
use maju_relay::router::{build_health_router, build_router};
use maju_relay::state::AppState;
use maju_relay::storage_sweep;
use maju_relay::telemetry;
use maju_workflow::WorkflowEngine;
use tokio_util::sync::CancellationToken;

fn maju_auto_migrate_enabled(value: Option<&str>) -> bool {
...
fn relay_keypair_from_config(relay_private_key: Option<&str>) -> anyhow::Result<nostr::Keys> {
    let hex = relay_private_key.ok_or_else(|| {
        anyhow::anyhow!(
            "MAJU_RELAY_PRIVATE_KEY must be set. Run `just bootstrap` for local \
             development or configure a stable 32-byte hex private key."
        )
    })?;
    nostr::Keys::parse(hex).map_err(|e| anyhow::anyhow!("invalid MAJU_RELAY_PRIVATE_KEY: {e}"))
}
...
///
/// Fleet-wide totals (`maju_total_*`) always emit regardless of mode.
///
/// Set via `MAJU_USAGE_METRICS_PER_COMMUNITY`:
...
impl EmissionScope {
    fn from_env() -> Self {
        let raw = std::env::var("MAJU_USAGE_METRICS_PER_COMMUNITY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase();
        match raw.as_str() {
            ...
            other => {
                warn!(
                    value = other,
                    "MAJU_USAGE_METRICS_PER_COMMUNITY: unknown value — defaulting to all"
                );
                EmissionScope::All
            }
        }
...
async fn run_relay_main(boot: BootTracker) -> anyhow::Result<()> {
    ...
    //
    // Build a single shared Resource (service.name=maju-relay by default, overridable
    ...
    let otel_layer = match &tracer_init {
        telemetry::TracerInit::Enabled(p) => {
            ...
            Some(tracing_opentelemetry::layer().with_tracer(p.tracer("maju-relay")))
        }
        _ => None,
    };
    ...
    tracing_subscriber::registry()
        ...
        .with(otel_layer.map(|layer| {
            layer.with_filter(telemetry::otel_env_filter(
                std::env::var("MAJU_OTEL_FILTER").ok().as_deref(),
            ))
        }))
    ...
    boot.finish();
    metrics::gauge!("maju_audit_enabled").set(if config.audit_enabled { 1.0 } else { 0.0 });
    metrics::gauge!("maju_push_enabled").set(if config.push_enabled { 1.0 } else { 0.0 });
    info!(
    ...
    let auto_migrate =
        maju_auto_migrate_enabled(std::env::var("MAJU_AUTO_MIGRATE").ok().as_deref());
    if auto_migrate {
        ...
        info!("Skipping database migrations because MAJU_AUTO_MIGRATE is not enabled");
    }
    ...
    // MAJU_AUTO_MIGRATE off and migration 0021 unapplied can never open the
    ...
    if config.require_relay_membership && config.relay_owner_pubkey.is_none() {
        error!(
            "MAJU_REQUIRE_RELAY_MEMBERSHIP=true but RELAY_OWNER_PUBKEY is not set or invalid. \
             Set RELAY_OWNER_PUBKEY to a valid 64-char hex pubkey."
        );
        return Err(anyhow::anyhow!(
            "RELAY_OWNER_PUBKEY required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true"
        ));
    }
    ...
    if config.require_relay_membership && config.relay_private_key.is_none() {
        return Err(anyhow::anyhow!(
            "MAJU_RELAY_PRIVATE_KEY is required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true. \
    ...
    let deployment_community = {
        let host = maju_relay::tenant::relay_url_authority(&config.relay_url);
        if host.is_empty() {
            if config.require_relay_membership {
                return Err(anyhow::anyhow!(
                    "Cannot derive a community host from MAJU_RELAY_URL ({:?}); a resolvable host is required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true",
                    config.relay_url
                ));
            }
            error!(
            ...
            match db.ensure_configured_community_for_bootstrap(&host).await {
                ...
                Err(e) => {
                    if config.require_relay_membership {
                        ...
                        return Err(anyhow::anyhow!(
                            "Failed to ensure deployment community (required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                        ));
                    }
        ...
        match db.backfill_from_allowlist(community).await {
            ...
            Err(e) => {
                if config.require_relay_membership {
                    ...
                    return Err(anyhow::anyhow!(
                        "Failed to backfill pubkey_allowlist (required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                    ));
                } else {
        ...
        match db.bootstrap_owner(community, owner_pubkey).await {
            ...
            Err(e) => {
                if config.require_relay_membership {
                    ...
                    return Err(anyhow::anyhow!(
                        "Failed to bootstrap relay owner (required when MAJU_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                    ));
                } else {
                    error!(
    ...
    let audit = if config.audit_enabled {
        ...
        info!("Audit logging disabled by MAJU_AUDIT_ENABLED");
        None
    };
    ...
    let workflow_config = maju_workflow::WorkflowConfig::default();
    ...
    let media_storage = maju_media::MediaStorage::new(&config.media)
    ...
    // Inter-relay mesh (MAJU_MESH seam). `boot_mesh` returns None when the
    ...
    if let Some(handle) = maju_relay::mesh_boot::boot_mesh(
        &state.config,
    ...
    .await?
    {
        ...
        // MAJU_MESH_DEMO_ECHO) before peers can route traffic here.
    ...
    if std::env::var("MAJU_GIT_CONFORMANCE_PROBE")
        .map(|v| v != "false")
        .unwrap_or(true)
    {
        let race_width = std::env::var("MAJU_GIT_PROBE_WRITERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(32);
        let race_rounds = std::env::var("MAJU_GIT_PROBE_ROUNDS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3);
        let cfg = maju_relay::api::git::store::ProbeConfig {
            race_width,
            race_rounds,
        };
        tracing::info!(
    ...
    match maju_relay::handlers::side_effects::reconcile_large_channel_member_snapshots(&state).await
    {
    ...
    if config.require_relay_membership {
        match maju_relay::handlers::side_effects::reconcile_nip43_membership_snapshots_with_purpose(
            &state,
            maju_relay::handlers::side_effects::Nip43ReconciliationPurpose::Bootstrap,
        )
        .await
        {
        ...
        let interval_secs = std::env::var("MAJU_NIP43_RECONCILE_INTERVAL_SECS")
            .ok()
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match maju_relay::handlers::side_effects::reconcile_nip43_membership_snapshots_with_purpose(
                    &reconcile_state,
                    maju_relay::handlers::side_effects::Nip43ReconciliationPurpose::Maintenance,
                )
                .await
                {
                    Ok(count) if count > 0 => {
    ...
    // Only runs when MAJU_RECONCILE_CHANNELS=true (dev/CI environments).
    // Production relays create channels through the event pipeline and don't need this.
    if std::env::var("MAJU_RECONCILE_CHANNELS").is_ok() {
        let reconcile_state = Arc::clone(&state);
        tokio::spawn(async move {
            ...
            let tenant = match maju_relay::tenant::bind_deployment_community(
                &reconcile_state.db,
            ...
            for attempt in 0..24u32 {
                ...
                match maju_relay::handlers::side_effects::reconcile_channel_events(
                    &tenant,
                    &reconcile_state,
                )
                .await
    ...
    let action_sink = Arc::new(maju_relay::workflow_sink::RelayActionSink::new(&state));
    ...
    // together with the workflow engine in a future multi-pod coordination pass.
    {
        ...
        let reaper_interval_secs: u64 = std::env::var("MAJU_REAPER_INTERVAL_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(60);
        tokio::spawn(async move {
            ...
            loop {
                ...
                for channel in &expired {
                    ...
                    let tenant = maju_core::tenant::TenantContext::resolved(
                        channel.community_id,
                        channel.host.clone(),
                    );
                    let channel_id = channel.channel_id;
                    // Emit a system message so members see why the channel was archived.
                    if let Err(e) = maju_relay::handlers::side_effects::emit_system_message(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    ...
                    if let Err(e) = maju_relay::handlers::side_effects::emit_group_discovery_events(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
                    ...
                    maju_relay::handlers::side_effects::evict_all_channel_subscriptions(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
    ...
    if state.config.push_enabled {
        tokio::spawn(maju_relay::push_runtime::run_matcher(Arc::clone(&state)));
        tokio::spawn(maju_relay::push_runtime::run_delivery_worker(Arc::clone(
            &state,
        )));
        info!("NIP-PL push matcher and delivery worker started");
    } else {
        info!("NIP-PL push disabled by MAJU_PUSH_ENABLED");
    }
    ...
    // run the worker concurrently without double-delivery.
    {
        ...
        tokio::spawn(
            async move { maju_relay::handlers::admin_outbox_worker::run(outbox_state).await },
        );
    ...
    // step_marker state without re-running the mutation.
    {
        ...
        tokio::spawn(
            async move { maju_relay::handlers::admin_action_worker::run(action_state).await },
        );
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for reminder in due {
                    ...
                    let reminder_tenant = maju_core::tenant::TenantContext::resolved(
    ...
    // (published by other relay instances) and fan out to local WS subscribers.
    {
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    Ok(channel_event) => {
                        maju_relay::handlers::event::fan_out_pubsub_event(
                            &state_for_sub,
                            channel_event,
                        )
                        .await;
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        metrics::counter!("maju_multinode_fanout_lag_total").increment(n);
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        metrics::counter!("maju_cache_invalidation_lag_total").increment(n);
    ...
    // so missed archive commands still converge without a global DB scan.
    {
        ...
        let interval_secs = std::env::var("MAJU_COMMUNITY_REVALIDATE_INTERVAL_SECS")
            .ok()
            .and_then(|value| value.parse::<u64>().ok())
            .unwrap_or(30)
            .clamp(1, 300);
            ...
            loop {
                match rx.recv().await {
                    Ok(scoped) => match scoped.command {
                        maju_pubsub::conn_control::ConnControl::DisconnectCommunity => {
                            state_for_conn_ctrl
                                .community_connections
                                .disconnect_community(scoped.community_id);
                        }
                        maju_pubsub::conn_control::ConnControl::DisconnectPubkey {
                            pubkey,
                            event_id,
                            reason,
                        } => {
    ...
    // Pool metrics: periodic background task polling DB + Redis pool stats.
    {
        ...
        let interval_secs = std::env::var("MAJU_POOL_METRICS_INTERVAL_SECS")
            .ok()
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                metrics::gauge!("maju_db_pool_size").set(db_stats.size as f64);
                metrics::gauge!("maju_db_pool_idle").set(db_stats.idle as f64);
                metrics::gauge!("maju_db_pool_active").set(active as f64);
                metrics::gauge!("maju_db_pool_max").set(db_stats.max as f64);
                ...
                if let Some(read_stats) = pool_state.db.read_pool_stats() {
                    ...
                    metrics::gauge!("maju_db_read_pool_size").set(read_stats.size as f64);
                    metrics::gauge!("maju_db_read_pool_idle").set(read_stats.idle as f64);
                    metrics::gauge!("maju_db_read_pool_active").set(read_active as f64);
                    metrics::gauge!("maju_db_read_pool_max").set(read_stats.max as f64);
                    ...
                    match pool_state.db.fence().verified_through() {
                        Some(fence_ts) => {
                            ...
                            metrics::gauge!("maju_db_replica_fence_open").set(1.0);
                            metrics::gauge!("maju_db_replica_fence_lag_seconds").set(lag as f64);
                        }
                        None => {
                            metrics::gauge!("maju_db_replica_fence_open").set(0.0);
                        }
                    }
                    ...
                    if let Some(age) = pool_state.db.fence().heartbeat_age() {
                        metrics::gauge!("maju_db_replica_heartbeat_age_seconds")
                            .set(age.as_secs_f64());
                    }
                }

                let rs = pool_state.redis_pool.status();
                metrics::gauge!("maju_redis_pool_available").set(rs.available as f64);
                metrics::gauge!("maju_redis_pool_size").set(rs.size as f64);
                metrics::gauge!("maju_redis_pool_max").set(rs.max_size as f64);
                metrics::gauge!("maju_redis_pool_waiting").set(rs.waiting as f64);

                let deletion_store = pool_state.db.deletion_store();
                match deletion_store.reap_expired_serving_write_leases(1000).await {
                    Ok(reaped) => metrics::counter!("maju_deletion_serving_leases_reaped_total")
                        .increment(reaped),
                    Err(error) => tracing::warn!(%error, "serving-lease reaper failed"),
                }
                match deletion_store.serving_lease_stats().await {
                    Ok(stats) => {
                        metrics::gauge!("maju_deletion_serving_leases_active")
                            .set(stats.active as f64);
                        metrics::gauge!("maju_deletion_serving_leases_expired")
                            .set(stats.expired as f64);
                        metrics::gauge!("maju_deletion_serving_leases_dead_tuples")
    ...
    //   In-memory:  each pod exports its partition → dashboard uses sum()
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                metrics::gauge!("maju_usage_poller_is_leader").set(if leader.is_some() {
                    1.0
                } else {
                    0.0
                });
...
mod env_filter_tests {
    ...
    use maju_relay::telemetry::otel_env_filter;
    ...
    fn unset_enables_datastore_only_for_otel_filter() {
        ...
        tracing::subscriber::with_default(logs, || {
            assert!(!tracing::enabled!(target: "maju_datastore", tracing::Level::INFO));
            assert!(tracing::enabled!(target: "maju_relay", tracing::Level::INFO));
        });
    ...
    fn explicit_datastore_off_is_preserved_alone() {
        assert_eq!(
            otel_env_filter(Some("maju_datastore=off")).to_string(),
            "maju_datastore=off"
        );
    }
    ...
    fn explicit_datastore_debug_is_preserved_alone() {
        assert_eq!(
            otel_env_filter(Some("maju_datastore=debug")).to_string(),
            "maju_datastore=debug"
        );
    }
    ...
    fn log_and_otel_filters_are_configured_independently() {
        ...
        assert_eq!(
            otel_env_filter(Some("maju_relay=debug")).to_string(),
            "maju_relay=debug"
        );
    }
}
...
/// ```text
/// ┌─────────────────────────────────────────────────────────┐
/// │  Listener 1: TCP MAJU_BIND_ADDR:3000  (app router)   │
/// │  Listener 2: UDS MAJU_UDS_PATH        (app, optional)│
...
/// (capped at [`maju_relay::config::MAX_DRAIN_JITTER_MS`] = 20s) plus the
...
/// chart's `terminationGracePeriodSeconds: 60` (`deploy/charts/maju/values.yaml`),
/// which leaves headroom but assumes no `preStop` hook adds further delay.
/// With jitter off (`MAJU_DRAIN_JITTER_MS=0`, the default) sockets close
...
async fn serve(
    ...
    info!(addr = %config.bind_addr, "maju-relay TCP listening");
    ...
    if let Some(ref uds_path) = config.uds_path {
        ...
        match std::fs::symlink_metadata(uds_path) {
            ...
            Ok(_) => {
                return Err(anyhow::anyhow!(
                    "MAJU_UDS_PATH {uds_path} exists but is not a socket"
                ));
            }
            Err(_) => {}
        }
        ...
        info!(path = %uds_path, "maju-relay UDS listening");
    ...
    if config.uds_path.is_some() {
        tracing::warn!("MAJU_UDS_PATH set but UDS not supported on this platform");
    }
...
fn reminder_to_event(reminder: &maju_db::event::DueReminder) -> nostr::Event {
...
fn usage_metrics_interval_secs() -> u64 {
    std::env::var("MAJU_USAGE_METRICS_INTERVAL_SECS")
        .ok()
...
fn usage_metrics_idle_timeout_secs(interval_secs: u64) -> u64 {
    let configured = std::env::var("MAJU_USAGE_METRICS_IDLE_TIMEOUT_SECS")
        .ok()
        .and_then(|value| value.parse().ok());
    ...
    fn set(&self, value: f64) {
        match self {
            Self::WsConnections(community) => {
                metrics::gauge!("maju_community_ws_connections", "community" => community.clone())
                    .set(value);
            }
            Self::UsersOnline(community) => {
                metrics::gauge!("maju_community_users_online_pod", "community" => community.clone())
                    .set(value);
            }
            Self::Subscriptions(community) => {
                metrics::gauge!("maju_community_subscriptions", "community" => community.clone())
                    .set(value);
            }
        }
    }
...
fn refresh_legacy_active_gauge_recency() {
    metrics::gauge!("maju_ws_connections_active").increment(0.0);
    metrics::gauge!("maju_subscriptions_active").increment(0.0);
}
...
fn emit_in_memory_usage_metrics(
    ...
    metrics::gauge!("maju_total_ws_connections").set(total_connections as f64);
    metrics::gauge!("maju_total_users_online_pod").set(users_online.values().sum::<u64>() as f64);
    metrics::gauge!("maju_total_subscriptions").set(total_subscriptions as f64);
...
async fn run_usage_metrics_tick(
    state: &AppState,
    emission_scope: &EmissionScope,
    leader: &mut Option<maju_db::UsageMetricsLeader>,
...
async fn run_storage_sweep_tick(
    ...
    storage_sweep::maybe_spawn_sweep(
        ...
        config.timeout,
        async move {
            maju_media::fold_bucket_listing(max_objects, move |token| {
...
async fn emit_db_usage_metrics(
    ...
    // this tick. Fleet-wide totals (maju_total_*) always emit regardless.
    ...
    // maju_communities_total (no tag — fleet-wide count)
    metrics::gauge!("maju_communities_total").set(community_total as f64);

    // maju_community_users{community, type:human|agent}
    ...
    // rather than keeping the last nonzero value until process restart.
    {
        ...
        metrics::gauge!("maju_total_users", "type" => "human").set(total_human as f64);
        metrics::gauge!("maju_total_users", "type" => "agent").set(total_agent as f64);
        ...
        for (&id, community) in host_map {
            ...
            metrics::gauge!("maju_community_users", "community" => community.clone(), "type" => "human")
                .set(human as f64);
            metrics::gauge!("maju_community_users", "community" => community.clone(), "type" => "agent")
                .set(agent as f64);
        }
    }

    // maju_community_channels{community, type}
    ...
    // drops to zero emits 0 rather than retaining its last nonzero value.
    {
        ...
        for &ct in CHANNEL_TYPES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("maju_total_channels", "type" => ct).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // maju_community_messages{community}
    // Emit 0 for communities with no messages so dashboards don't stale-read.
    {
        ...
        metrics::gauge!("maju_total_messages").set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            metrics::gauge!("maju_community_messages", "community" => community.clone())
                .set(count as f64);
        }
    }

    // maju_community_relay_members{community, role}
    ...
    // CHECK constraint over {'owner', 'admin', 'member'}.
    {
        ...
        for &role in RELAY_ROLES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("maju_total_relay_members", "role" => role).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // maju_community_workflows{community, status}
    ...
    // DB enum: {'active', 'disabled', 'archived'}.
    {
        ...
        for &status in WORKFLOW_STATUSES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("maju_total_workflows", "status" => status).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // maju_community_git_repos{community}
    // Emit 0 for communities with no repos.
    {
        ...
        // Fleet total (always emitted).
        let total: i64 = rows.values().sum();
        metrics::gauge!("maju_total_git_repos").set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            let count = rows.get(&id).copied().unwrap_or(0);
            metrics::gauge!("maju_community_git_repos", "community" => community.clone())
                .set(count as f64);
        }
    }
    ...
    for (data, label) in [
        ...
        metrics::gauge!("maju_total_active_users", "window" => label, "type" => "human")
            .set(total_human as f64);
        metrics::gauge!("maju_total_active_users", "window" => label, "type" => "agent")
            .set(total_agent as f64);
        metrics::gauge!("maju_total_active_users", "window" => label, "type" => "unknown")
    ...
    for (data, label) in [(active_channels_1d, "1d"), (active_channels_7d, "7d")] {
        ...
        // Fleet total (always emitted).
        let total: i64 = rows.values().sum();
        metrics::gauge!("maju_total_active_channels", "window" => label).set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            let count = rows.get(&id).copied().unwrap_or(0);
            metrics::gauge!(
                "maju_community_active_channels",
                "community" => community.clone(),
                "window" => label
            )
            .set(count as f64);
...
mod tests {
    ...
    use super::{
        connect_audit_pool, dropped_in_memory_keys, idle_timeout_secs, maju_auto_migrate_enabled,
    ...
    #[test]
    fn maju_auto_migrate_is_opt_in() {
        assert!(!maju_auto_migrate_enabled(None));
        assert!(!maju_auto_migrate_enabled(Some("")));
        assert!(!maju_auto_migrate_enabled(Some("false")));
        assert!(!maju_auto_migrate_enabled(Some("0")));
        assert!(!maju_auto_migrate_enabled(Some("no")));

        assert!(maju_auto_migrate_enabled(Some("true")));
        assert!(maju_auto_migrate_enabled(Some("TRUE")));
        assert!(maju_auto_migrate_enabled(Some(" 1 ")));
        assert!(maju_auto_migrate_enabled(Some("yes")));
        assert!(maju_auto_migrate_enabled(Some("on")));
    }
    ...
    fn test_legacy_gauge_recency_refresh_preserves_lifecycle_deltas() {
        ...
        metrics::with_local_recorder(&recorder, || {
            let connections = metrics::gauge!("maju_ws_connections_active");
            let subscriptions = metrics::gauge!("maju_subscriptions_active");
        ...
        assert_eq!(values.get("maju_ws_connections_active"), Some(&0.0));
        assert_eq!(values.get("maju_subscriptions_active"), Some(&2.0));
    }
