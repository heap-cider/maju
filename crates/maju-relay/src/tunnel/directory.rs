...
use maju_relay_mesh::{FencedHeader, MeshError, Profile, RuntimeId};
use redis::Script;
use uuid::Uuid;
...
impl SessionDirectory {
    ...
    pub fn with_db(pool: deadpool_redis::Pool, db: maju_db::Db) -> Self {
        Self {
            pool,
    ...
    async fn begin_serving_write(
        &self,
        community_id: CommunityId,
    ) -> Result<Option<maju_deletion::ServingWriteGuard>, DirectoryError> {
        match &self.db {
            Some(db) => maju_deletion::acquire_serving_write(db, community_id, "session_directory")
                .await
                .map(Some)
...
impl SessionKeys {
    fn new(community_id: CommunityId, session_id: Uuid) -> Self {
        let base = format!("maju:{}:tunnel:{}", community_id, session_id);
        Self {
...
mod tests {
    ...
    fn key_shape_is_community_scoped_and_separates_counter() {
        ...
        assert_eq!(
            keys.lease,
            format!("maju:{}:tunnel:{}:lease", community(), session())
        );
        assert_eq!(
            keys.generation,
            format!("maju:{}:tunnel:{}:generation", community(), session())
        );
