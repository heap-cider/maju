...
use maju_core::filter::{filters_match, reader_authorized_for_event};
...
async fn process_match_batch(state: &AppState, batch: maju_db::push::ClaimedMatchBatch) {
    ...
    let context = match load_match_context(state, &batch).await {
        ...
        Err(e) => {
            ...
            metrics::counter!("maju_push_match_jobs_total", "result" => "context_error")
    ...
    let mut wakes: Vec<maju_db::push::WakeRequest> = Vec::new();
    for job in &batch.jobs {
        let match_queue_seconds = Utc::now()
            ...
            .max(0) as f64
            / 1_000.0;
        metrics::histogram!("maju_push_match_queue_seconds").record(match_queue_seconds);
        let event_id = job.event.event.id.as_bytes().to_vec();
        match match_job(job, &context) {
            Ok(job_wakes) if job_wakes.is_empty() => {
                metrics::counter!("maju_push_match_jobs_total", "result" => "unmatched")
                    .increment(1);
                completed.push(event_id);
            }
            Ok(job_wakes) => {
                metrics::counter!("maju_push_match_jobs_total", "result" => "matched").increment(1);
                pending.push((event_id, job.attempt));
                wakes.extend(job_wakes);
            }
            Err(e) => {
                metrics::counter!("maju_push_match_jobs_total", "result" => "error").increment(1);
                warn!(event_id=%job.event.event.id, attempt=job.attempt, "push match failed: {e}");
                if job.attempt >= maju_db::push::MAX_MATCH_ATTEMPTS {
        ...
        Ok(outcomes) => {
            for outcome in outcomes {
                let result = match outcome {
                    maju_db::push::EnqueueWakeOutcome::Enqueued(_) => "enqueued",
                    maju_db::push::EnqueueWakeOutcome::Duplicate(_) => "duplicate",
                    maju_db::push::EnqueueWakeOutcome::InactiveLease => "inactive_lease",
                };
                metrics::counter!("maju_push_wakes_total", "result" => result).increment(1);
            }
            completed.extend(pending.into_iter().map(|(event_id, _)| event_id));
        }
        Err(e) => {
            metrics::counter!("maju_push_wake_enqueue_errors_total").increment(1);
            warn!(%community, "push wake batch enqueue failed: {e}");
            for (event_id, attempt) in pending {
                if attempt >= maju_db::push::MAX_MATCH_ATTEMPTS {
                    completed.push(event_id);
                } else {
                    retry.push(event_id);
                }
...
fn match_job(
    job: &maju_db::push::BatchedMatch,
    context: &MatchContext,
) -> anyhow::Result<Vec<maju_db::push::WakeRequest>> {
    ...
    for lease in &context.leases {
        ...
        wakes.push(maju_db::push::WakeRequest {
...
fn push_filter_authorized_for_event(
    ...
    if maju_core::kind::event_kind_u32(event) != maju_core::kind::KIND_GIFT_WRAP {
        return true;
    }
        ...
        match state.db.usage_community_hosts().await {
            Ok(communities) => {
                for community in communities {
                    let community = maju_core::CommunityId::from_uuid(community.id);
                    let until = Utc::now() + TimeDelta::seconds(CLAIM_SECS);
...
async fn deliver_one(
    state: &AppState,
    http: &reqwest::Client,
    claimed: maju_db::push::ClaimedWake,
) {
    let outcome = match state
        ...
        .await
    {
        Ok(maju_db::push::RevalidateWakeOutcome::Deliver(wake)) => wake,
        Ok(maju_db::push::RevalidateWakeOutcome::Suppressed) => {
            let _ = state
                .db
    ...
    if outcome.attempt == 1 {
        let wake_queue_seconds = Utc::now()
            ...
            .max(0) as f64
            / 1_000.0;
        metrics::histogram!("maju_push_wake_queue_seconds").record(wake_queue_seconds);
    }
    ...
    metrics::counter!("maju_push_gateway_requests_total").increment(1);
    let gateway_started = Instant::now();
    let protected = serving_write
        .protect(send_gateway_request(http, url, body, auth))
        .await;
    metrics::histogram!("maju_push_gateway_request_seconds")
...
fn record_delivery(outcome: &'static str) {
    metrics::counter!("maju_push_deliveries_total", "outcome" => outcome).increment(1);
}

async fn retry_or_fail(
    state: &AppState,
    wake: &maju_db::push::ClaimedWake,
    delay: i64,
) -> &'static str {
