...
use chrono::Utc;
use maju_core::kind::KIND_STREAM_MESSAGE;
use maju_core::tenant::CommunityId;
use maju_workflow::action_sink::{ActionSink, ActionSinkError};
...
fn append_workflow_mention_tags(
    ...
    for mentioned in rendered_mentions {
        ...
        if authored_mentions.contains(&mentioned) {
            tags.push(
                Tag::parse(["maju:workflow-mention", &mentioned]).map_err(|e| {
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = maju_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    maju_db::DbError::ChannelNotFound(_) | maju_db::DbError::NotFound(_) => {
            ...
            //    - `maju:workflow` tag prevents recursive workflow triggering
            //    - `maju:workflow-owner` lets harnesses apply the owner's
            ...
            //    - one `maju:workflow-mention` tag only when the same target was
            ...
            let mut tags = vec![
                ...
                Tag::parse(["maju:workflow", "true"])
                    .map_err(|e| ActionSinkError::EventBuild(format!("workflow tag: {e}")))?,
                Tag::parse(["maju:workflow-owner", &author_pubkey_hex])
            ...
            // the `root` + `reply` pair — matching `maju_sdk::builders::thread_tags`
            ...
            let thread_meta = Some(match &thread_meta_owned {
                ...
                None => maju_db::event::ThreadMetadataParams {
...
mod tests {
    ...
    fn trigger_injected_rendered_mention_gets_no_authority() {
        ...
        assert!(
            tags.iter()
                .all(|tag| tag.as_slice() != ["maju:workflow-mention", agent.as_str()]),
    ...
    fn explicit_owner_mention_keeps_single_legacy_owner_tag() {
        ...
        let owner_workflow_mentions = tags
            .iter()
            .filter(|tag| tag.as_slice() == ["maju:workflow-mention", owner.as_str()])
            .count();
...
mod postgres_tests {
    ...
    //!   `cargo test -p maju-relay --lib workflow_sink -- --ignored`
    use super::*;
    use maju_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use maju_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
    ...
    async fn execute_send_message_workflow(
        ...
        let trigger_ctx = maju_workflow::executor::TriggerContext {
        ...
        let stored_definition: maju_workflow::WorkflowDef =
            serde_json::from_value(stored_workflow.definition).expect("parse stored definition");
        let result = maju_workflow::executor::execute_run(
    ...
    async fn workflow_send_message_binds_authority_to_authored_mentions() {
        ...
        let tag_values = |stored: &maju_core::StoredEvent, name: &str| -> Vec<String> {
            stored
                .event
                .tags
                .iter()
        ...
        assert!(
            tag_values(&injected, "maju:workflow-mention").is_empty(),
