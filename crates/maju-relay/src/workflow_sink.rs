...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            //    - `maju:workflow-owner` tag names the workflow owner explicitly,
            ...
            let thread_meta = Some(maju_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    async fn workflow_send_message_p_tags_mentioned_member() {
        ...
        let owner_tag = stored
            ...
            .tags
            .iter()
            .find(|t| t.as_slice().first().map(|s| s.as_str()) == Some("maju:workflow-owner"))
            .and_then(|t| t.as_slice().get(1).map(|s| s.as_str()));
        assert_eq!(
            owner_tag,
            Some(author_hex.as_str()),
            "workflow owner must be named explicitly via maju:workflow-owner \
