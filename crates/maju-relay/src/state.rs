...
use maju_auth::{AuthService, Nip98ReplayGuard};
use maju_core::tenant::TenantContext;
use maju_core::CommunityId;
use maju_db::Db;
use maju_media::MediaStorage;
use maju_pubsub::cache_invalidation::CacheInvalidation;
use maju_pubsub::conn_control::ConnControl;
use maju_pubsub::rate_limiter::RedisRateLimiter;
use maju_pubsub::{PubSubManager, RedisNip98ReplayGuard};
...
pub(crate) async fn run_registered_community_connection<Check, CheckFuture, Run, RunFuture>(
    ...
    CheckFuture: Future<Output = Result<bool, maju_db::DbError>>,
...
) -> (usize, Vec<(CommunityId, maju_db::DbError)>)
where
...
impl ConnectionManager {
    ...
    /// (`MAJU_DRAIN_JITTER_MS` unset or `0`). It is synchronous and returns as
    ...
    ///
    /// This is the jittered drain, used only when `MAJU_DRAIN_JITTER_MS > 0`.
            ...
            match conn.tx.try_send(msg) {
                ...
                Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                    ...
                    if count >= conn.grace_limit {
                        ...
                        metrics::counter!("maju_ws_backpressure_disconnects_total").increment(1);
                        conn.cancel.cancel();
                    } else {
...
pub struct AppState {
    ...
    pub audit_tx: Option<mpsc::Sender<maju_audit::NewAuditEntry>>,
    ...
    /// See `crates/maju-conformance/` and `crate::conformance` for the
    /// schema, emitter helpers, and the independent checker.
    pub tracer: Arc<dyn maju_conformance::Tracer>,
...
impl AppState {
    ...
    pub fn new(
        ...
        let (audit_tx, mut audit_rx) = mpsc::channel::<maju_audit::NewAuditEntry>(1000);
        ...
        let state = Self {
            ...
            // `crates/maju-test-client` once those land).
    ...
    pub async fn is_member_cached(
        ...
        channel_id: Uuid,
        pubkey: &[u8],
    ) -> Result<bool, maju_db::DbError> {
        let key = (community_id, channel_id, pubkey.to_vec());
        if let Some(cached) = self.membership_cache.get(&key) {
            metrics::counter!("maju_membership_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("maju_membership_cache_misses_total").increment(1);
    ...
    pub async fn disconnect_community_clusterwide(
        &self,
        tenant: &TenantContext,
    ) -> Result<usize, maju_pubsub::PubSubError> {
        let closed = self
    ...
    pub async fn get_accessible_channel_ids_cached(
        ...
        community_id: CommunityId,
        pubkey: &[u8],
    ) -> Result<Vec<Uuid>, maju_db::DbError> {
        let key = (community_id, pubkey.to_vec());
        if let Some(cached) = self.accessible_channels_cache.get(&key) {
            metrics::counter!("maju_accessible_channels_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("maju_accessible_channels_cache_misses_total").increment(1);
        let result = self
            .db
    ...
    pub async fn channel_visibility_cached(
        ...
        community_id: CommunityId,
        channel_id: Uuid,
        prefetched: Option<&maju_db::channel::ChannelRecord>,
    ) -> Result<String, maju_db::DbError> {
...
async fn log_audit_entry(audit: &maju_audit::AuditService, entry: maju_audit::NewAuditEntry) {
    ...
    loop {
        match audit.log(entry.clone()).await {
            Ok(_) => {
                metrics::histogram!("maju_audit_log_seconds").record(t.elapsed().as_secs_f64());
                return;
            }
            Err(maju_audit::AuditError::Database(sqlx::Error::Database(database_error)))
                if database_error.code().as_deref() == Some("55P03") =>
            {
                retries += 1;
                metrics::counter!("maju_audit_log_lock_retries_total").increment(1);
                tracing::warn!(
                    retries,
                    retry_delay_ms,
            ...
            Err(error) => {
                metrics::counter!("maju_audit_log_errors_total").increment(1);
...
pub(crate) mod tests {
    ...
    async fn audit_worker_retries_lock_timeout_until_original_entry_is_appended_once() {
        ...
        let entry = maju_audit::NewAuditEntry {
        ...
        // Mirrors maju_audit::service::AUDIT_LOCK_NAMESPACE.
        let lock_key = format!("maju_audit:{community_id}");
    ...
    fn shared_counter_between_direct_and_fanout() {
        ...
        let conn = ConnectionState {
            conn_id,
            tenant: maju_core::tenant::TenantContext::resolved(
                maju_core::tenant::CommunityId::from_uuid(Uuid::nil()),
    ...
    async fn disconnect_pubkey_is_fenced_to_the_banning_community() {
        ...
        let community_a = maju_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xa));
        let community_b = maju_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xb));
    ...
    async fn drain_all_sends_restart_close_and_cancels_every_conn() {
        ...
        let (mut ctrl_a, cancel_a) = register(maju_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xa),
        ));
        let (mut ctrl_b, cancel_b) = register(maju_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xb),
        ));
