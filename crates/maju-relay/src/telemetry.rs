...
//! - `OTEL_SERVICE_NAME` (default: `maju-relay`; read explicitly — not via SDK detector)
...
/// `MAJU_OTEL_FILTER` to override the default targets.
pub fn otel_env_filter(configured: Option<&str>) -> EnvFilter {
    EnvFilter::new(configured.unwrap_or("maju_relay=info,maju_datastore=info"))
}
...
/// 3. Hard-coded fallback `maju-relay`.
///
...
/// `maju-relay` default.  We therefore read `OTEL_SERVICE_NAME` explicitly
...
pub fn service_resource() -> Resource {
    // Honor OTEL_SERVICE_NAME when set+non-empty; otherwise use maju-relay.
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "maju-relay".to_string());
...
mod tests {
    ...
    fn test_service_resource_default_when_env_unset() {
        ...
        assert_eq!(
            ...
            Some("maju-relay"),
            "expected maju-relay fallback when OTEL_SERVICE_NAME is unset"
        );
    }
    ...
    fn test_service_resource_empty_string_falls_back_to_default() {
        ...
        assert_eq!(
            service_name_from(&r).as_deref(),
            Some("maju-relay"),
            "expected maju-relay fallback when OTEL_SERVICE_NAME is empty"
        );
    }
