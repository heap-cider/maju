...
/// Blossom kind:24242 verbs Maju currently accepts.
...
/// [`maju_core::tenant::normalize_host`] rule so the comparison agrees with how
...
mod tests {
    ...
    fn build_get_auth(keys: &Keys, tags: Vec<Tag>) -> nostr::Event {
        EventBuilder::new(Kind::from(24242), "Get maju-media")
            .tags(tags)
            .sign_with_keys(keys)
            .unwrap()
    }
    ...
    fn test_server_tag_enforcement() {
        ...
        assert!(matches!(
            verify_blossom_upload_auth(&event, &sha256, Some("maju.example.com"), 600),
            Err(MediaError::ServerMismatch)
        ));
