...
pub fn validate_file_content(
    ...
    match infer::get(bytes) {
        Some(kind) => {
            ...
            // audio is rejected until Maju has an explicit sanitizer and
...
/// Returns true when a raw tEXt chunk payload is a Maju snapshot manifest:
...
fn validate_png_metadata_free(bytes: &[u8]) -> Result<(), MediaError> {
    ...
    while i < bytes.len() {
        ...
        let end = i
            ...
            .ok_or(MediaError::InvalidImage)?;
        if &kind == b"tEXt" {
            // Maju agent/team snapshot manifests ride in a single tEXt chunk
...
mod tests {
    ...
    fn test_png_snapshot_text_chunks_are_allowed() {
        ...
        let config = test_config();
        for keyword in [b"maju_agent_snapshot".as_slice(), b"maju_team_snapshot"] {
    ...
    fn test_png_snapshot_text_chunk_rejected_when_duplicated_or_spoofed() {
        ...
        let mut payload = b"maju_agent_snapshot".to_vec();
        payload.push(0);
        ...
        // stay forbidden.
        for payload in [
            b"maju_agent_snapshotX\0data".as_slice(),
            b"maju_agent_snapshot_extra\0data",
            b"maju_agent_snapshot", // no separator at all
