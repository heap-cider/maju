...
2. Copy the encoded files into both fixture directories:
   ...
   cp uikit-encoded.png crates/maju-media/tests/fixtures/ios/
   cp uikit-encoded.jpg crates/maju-media/tests/fixtures/ios/
   ```
...
5. Run `cargo test -p maju-media test_ios_uikit` to verify that UIKit's encoded output is rejected and the matching sanitizer output is accepted by the relay contract.
