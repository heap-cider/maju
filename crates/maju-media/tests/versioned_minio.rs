...
//!
//! Run it against the docker-compose MinIO (creds `maju_dev`/`maju_dev_secret`):
//!
//! ```bash
//! docker compose up -d minio minio-init
//! cargo test -p maju-media --test versioned_minio -- --ignored --nocapture
//! ```
//!
//! The test creates and removes its own bucket. The MinIO container name is
//! overridable with `MAJU_MINIO_CONTAINER`; credentials/endpoint/region/addressing
//! use the same `MAJU_S3_*` env vars as `static_creds_minio`.

use std::process::Command;

use maju_media::config::MediaConfig;
use maju_media::storage::{MediaStorage, ObjectVersionKind, ObjectVersionRef};
...
fn minio_config(bucket: String) -> MediaConfig {
    MediaConfig {
        s3_endpoint: env_or("MAJU_S3_ENDPOINT", "http://localhost:9000"),
        s3_access_key: env_or("MAJU_S3_ACCESS_KEY", "maju_dev"),
        s3_secret_key: env_or("MAJU_S3_SECRET_KEY", "maju_dev_secret"),
        s3_bucket: bucket,
        s3_region: env_or("MAJU_S3_REGION", "us-east-1"),
        s3_addressing_style: env_or("MAJU_S3_ADDRESSING_STYLE", "path")
            .parse()
...
fn run_mc(args: &[String]) -> Result<(), String> {
    let container = env_or("MAJU_MINIO_CONTAINER", "maju-minio");
...
async fn list_all_versions(
    ...
    prefix: &str,
    max_keys: usize,
) -> Vec<maju_media::storage::ObjectVersionEntry> {
...
fn refs_from(entries: &[maju_media::storage::ObjectVersionEntry]) -> Vec<ObjectVersionRef> {
    entries
        .iter()
...
async fn never_versioned_bucket_lists_null_versions_and_exact_delete_empties_listing() {
    let bucket = format!("maju-media-never-versioned-{}", std::process::id());
...
async fn versioned_bucket_exact_version_delete_reaches_final_list_versions_emptiness() {
    let bucket = format!("maju-media-versioned-{}", std::process::id());
    let bucket_path = format!("local/{bucket}");
    let config = minio_config(bucket.clone());
    mc_alias(&config.s3_access_key, &config.s3_secret_key).expect("configure mc alias");
    run_mc(&[
