...
use maju_media::{is_tenant_owned_key, tenant_prefixes, MediaStorage};
use serde::Serialize;
...
async fn connect_services_with_store(store: DeletionStore) -> Result<Services> {
    let media_config = maju_media::MediaConfig {
        ...
        s3_access_key: required_env("MAJU_S3_ACCESS_KEY")?,
        s3_secret_key: required_env("MAJU_S3_SECRET_KEY")?,
