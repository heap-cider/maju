...
pub struct ServingWriteGuard {
    store: DeletionStore,
    lease: maju_db::deletion::ServingWriteLease,
...
impl ServingWriteGuard {
    ...
    pub fn acquisition_is_fenced(error: &anyhow::Error) -> bool {
        matches!(
            error.downcast_ref::<maju_db::DbError>(),
            Some(maju_db::DbError::AccessDenied(_))
        )
    }
    ...
    pub fn lease(&self) -> &maju_db::deletion::ServingWriteLease {
        &self.lease
    }
...
fn is_permanent_error(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        cause.is::<PermanentSource>()
            || matches!(
                cause.downcast_ref::<maju_db::DbError>(),
                Some(maju_db::DbError::DeletionSafety(_))
            )
            || cause
...
async fn run_with_services(command: Command, services: Services) -> Result<i32> {
    match command {
        ...
        Command::Sweep => {
            ...
            let outcome =
                maju_media::sweep_bucket_taxonomy(cap, SWEEP_UNKNOWN_KEY_SAMPLE, move |token| {
...
fn resolve_submit_host(host: Option<&str>, relay_url: Option<&str>) -> Result<String> {
    ...
    let host = maju_core::tenant::relay_url_authority(relay_url);
    if host.is_empty() {
        anyhow::bail!(
...
async fn connect_store() -> Result<DeletionStore> {
    ...
    let db = Db::new(
        &DbConfig {
            database_url,
            max_connections: env_parse("MAJU_DB_POOL_SIZE", 20),
            ..DbConfig::default()
        }
...
fn resolve_s3_region(maju_region: Option<String>, aws_region: Option<String>) -> String {
    maju_region
...
async fn connect_services_with_store(store: DeletionStore) -> Result<Services> {
    ...
    let media_config = maju_media::MediaConfig {
        s3_endpoint: required_env("MAJU_S3_ENDPOINT")?,
        s3_access_key,
        s3_secret_key,
        s3_bucket: required_env("MAJU_S3_BUCKET")?,
        s3_region: s3_region_from_env(),
        s3_addressing_style: std::env::var("MAJU_S3_ADDRESSING_STYLE")
...
fn s3_key_pair_from(get_env: impl Fn(&str) -> Option<String>) -> (String, String) {
    (
        optional_env_from(&get_env, "MAJU_S3_ACCESS_KEY"),
        optional_env_from(&get_env, "MAJU_S3_SECRET_KEY"),
    )
}
...
fn validate_storage_ownership(request: &DeletionRequest, manifest: &StorageManifest) -> Result<()> {
    maju_db::deletion::validate_storage_manifest(manifest)?;
...
fn manifest_chunk_deleted_detail(
    prefix: &str,
    key_count: usize,
    outcome: &maju_media::BulkDeleteOutcome,
) -> serde_json::Value {
    serde_json::json!({
        "prefix": prefix,
        "keys": key_count,
...
async fn delete_manifest_chunk_with<F, Fut>(
    chunk: &maju_db::deletion::ManifestKeyChunk,
    storage_version: i32,
    delete: F,
...
async fn record_stage_failure(
    ...
    match result {
        Ok(()) => Ok(true),
        Err(error) if maju_db::deletion::is_stale_deletion_lease(&error) => Ok(false),
    ...
    match request.stage {
        DeletionStage::Approved => {
            ...
            match services.store.fence(&token).await {
                Ok(_) => {}
                Err(maju_db::DbError::ServingWritesNotDrained {
                    active_count,
                    operations,
                    ..
                }) => {
...
async fn publish_disconnect_community(
    ...
    let channel = format!("maju:{community}:conn-control");
...
async fn purge_redis_namespace(
    ...
    let mut connection = pool.get().await?;
    let pattern = format!("maju:{community}:*");
    let mut cursor = 0u64;
...
fn sweep_object_cap() -> u64 {
    std::env::var("MAJU_DELETION_SWEEP_MAX_OBJECTS")
        .ok()
        .and_then(|value| value.parse().ok())
...
fn manifest_chunk_keys() -> usize {
    std::env::var("MAJU_DELETION_MANIFEST_CHUNK_KEYS")
        .ok()
...
fn default_executor_id() -> String {
    let hostname = std::env::var("HOSTNAME").unwrap_or_else(|_| "maju-admin".to_string());
...
mod postgres_tests {
    ...
    fn empty_storage_manifest(community: maju_core::CommunityId) -> StorageManifest {
        StorageManifest {
            version: 4,
    ...
    async fn claimed_test_deletion(prefix: &str) -> (Db, Services, ClaimedDeletion) {
        let database_url = std::env::var("MAJU_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .expect("MAJU_TEST_DATABASE_URL or DATABASE_URL is required");
        ...
        let services = Services {
            store,
            media: Arc::new(
                MediaStorage::new(&maju_media::MediaConfig {
                    ...
                    s3_addressing_style: maju_media::S3AddressingStyle::Path,
                    max_image_bytes: 1,
                    max_gif_bytes: 1,
                    max_video_bytes: 1,
                    max_file_bytes: 1,
    ...
    fn deletion_s3_key_pair_trims_static_and_preserves_partial_pairs() {
        assert_eq!(
            ...
            ("maju_dev".to_string(), "maju_dev_secret".to_string())
        );

        for (env, expected) in [
            (
                &[("MAJU_S3_ACCESS_KEY", " maju_dev ")][..],
                ("maju_dev".to_string(), String::new()),
            ),
            (
                &[("MAJU_S3_SECRET_KEY", " maju_dev_secret ")][..],
                (String::new(), "maju_dev_secret".to_string()),
            ),
            (
                &[
                    ("MAJU_S3_ACCESS_KEY", " maju_dev "),
    ...
    fn deletion_test_media_storage() -> Arc<MediaStorage> {
        let endpoint = std::env::var("MAJU_TEST_S3_ENDPOINT")
            .or_else(|_| std::env::var("MAJU_S3_ENDPOINT"))
            .expect("MAJU_TEST_S3_ENDPOINT or MAJU_S3_ENDPOINT is required");
        let access_key = std::env::var("MAJU_TEST_S3_ACCESS_KEY")
            .or_else(|_| std::env::var("MAJU_S3_ACCESS_KEY"))
            .expect("MAJU_TEST_S3_ACCESS_KEY or MAJU_S3_ACCESS_KEY is required");
        let secret_key = std::env::var("MAJU_TEST_S3_SECRET_KEY")
            .or_else(|_| std::env::var("MAJU_S3_SECRET_KEY"))
            .expect("MAJU_TEST_S3_SECRET_KEY or MAJU_S3_SECRET_KEY is required");
        let bucket = std::env::var("MAJU_TEST_S3_BUCKET")
            .or_else(|_| std::env::var("MAJU_S3_BUCKET"))
            .expect("MAJU_TEST_S3_BUCKET or MAJU_S3_BUCKET is required");
        Arc::new(
            MediaStorage::new(&maju_media::MediaConfig {
                ...
                s3_bucket: bucket,
                s3_region: std::env::var("MAJU_TEST_S3_REGION")
                    .or_else(|_| std::env::var("MAJU_S3_REGION"))
    ...
    async fn frozen_inventory_digest_and_storage_ownership_fail_closed() {
        ...
        let foreign_manifest =
            empty_storage_manifest(maju_core::CommunityId::from_uuid(Uuid::new_v4()));
    ...
    fn legacy_v4_manifest_chunk_decodes_bare_keys_for_resume_delete() {
        let chunk = maju_db::deletion::ManifestKeyChunk {
            chunk_no: 3,
    ...
    fn permanent_failures_are_typed_not_string_classified() {
        ...
        let db_permanent = anyhow::Error::from(maju_db::DbError::DeletionSafety(
            "typed catalog drift".to_string(),
        ));
        let db_transient = anyhow::Error::from(maju_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
    ...
    fn deletion_configuration_requires_every_destructive_dependency() {
        let variable = format!("MAJU_DELETION_REQUIRED_TEST_{}", Uuid::new_v4().simple());
    ...
    fn redis_absence_requires_terminal_cursor_and_all_pages_empty() {
        ...
        assert!(!scan_proves_absence(&[
            (9, Vec::new()),
            (0, vec!["maju:tenant:late".to_string()]),
        ]));
