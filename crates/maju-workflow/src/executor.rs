...
pub async fn dispatch_action(
    ...
    let serving_write =
        maju_deletion::acquire_serving_write(&engine.db, community_id, "workflow_action")
            .await
            .map_err(|error| {
...
// is_private_ip is provided by maju_core::network::is_private_ip
...
async fn check_ssrf(host: &str, port: u16) -> Result<std::net::IpAddr, WorkflowError> {
    ...
    for ip in &addrs {
        if maju_core::network::is_private_ip(ip) {
...
async fn add_reaction_impl(message_id: &str, emoji: &str) -> Result<JsonValue, WorkflowError> {
    let base_url =
        std::env::var("MAJU_RELAY_BASE_URL").unwrap_or_else(|_| "http://localhost:3000".to_owned());
    ...
    if let Ok(token) = std::env::var("MAJU_API_TOKEN") {
        req = req.header("Authorization", format!("Bearer {token}"));
    } else if let Ok(pubkey) = std::env::var("MAJU_RELAY_PUBKEY") {
...
pub async fn execute_run(
    ...
    engine
        ...
        .update_workflow_run(
            community_id,
            run_id,
            maju_db::workflow::RunStatus::Running,
            0,
            &serde_json::json!([]),
            None,
        )
...
mod tests {
    ...
    fn trigger_context_get_field_webhook_field() {
        let mut ctx = make_trigger();
        ctx.webhook_fields
            .insert("repo".to_owned(), "maju".to_owned());
        assert_eq!(ctx.get_field("repo"), Some("maju"));
    }
