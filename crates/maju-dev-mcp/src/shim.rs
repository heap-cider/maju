...
///
/// Shell children receive `path_env`, `git_env`, and `MAJU_PRIVATE_KEY` (for
/// the maju CLI). `NOSTR_PRIVATE_KEY` is removed from the process env after
...
impl Shim {
    pub fn install() -> std::io::Result<Self> {
        let dir = tempfile::Builder::new().prefix("maju-dev-mcp-").tempdir()?;
...
fn write_keyfile(shim_dir: &Path, raw: &str) -> Option<KeyInfo> {
    ...
    let keys = match nostr::Keys::parse(raw) {
        ...
        Err(e) => {
            eprintln!(
                "maju-dev-mcp: warning: NOSTR_PRIVATE_KEY is set but invalid ({e}); \
    ...
    if write_keyfile_atomic(&keyfile, raw.as_bytes()).is_err() {
        eprintln!(
            "maju-dev-mcp: warning: failed to write nostr keyfile; git auth/signing disabled"
        );
        return None;
    }
    let keyfile_path = match keyfile.to_str() {
        ...
        None => {
            eprintln!(
                "maju-dev-mcp: warning: tempdir path is not valid UTF-8; git auth/signing disabled"
            );
            return None;
        }
    };
...
/// Format: `<hex_pubkey>@<relay_host>` (e.g., `ab12...cd@relay.maju.dev`).
/// Falls back to `<hex_pubkey>@maju` if no relay URL is configured.
fn derive_git_email(pubkey_hex: &str) -> String {
    let host = std::env::var("MAJU_RELAY_URL")
        .ok()
        .and_then(|url| {
...
///
/// Deliberately distinct from `MAJU_ACP_SESSION_TITLE`, which is per-session UI
...
const DISPLAY_NAME_ENV_VAR: &str = "MAJU_ACP_DISPLAY_NAME";
...
/// Normalize a Maju display name into a git author name, or `None` to fall
/// back to the npub.
///
...
/// via maju-agent (which clears env), the base is always 0 — composition only
...
fn build_git_env(info: &KeyInfo) -> Vec<(String, String)> {
    ...
    let entries: Vec<(&str, String)> = vec![
        // Identity — Maju display name (npub fallback), NIP-05-style email
        ("user.name", user_name),
        ("user.email", email),
        // Nostr credential helper is additive — it silently declines non-Maju
        ...
        // Required: Maju relay verifies NIP-98 against the full repo-root URL.
...
mod git_user_name_tests {
    ...
    fn test_build_git_env_uses_display_name_and_leaves_email_on_the_pubkey() {
        ...
        std::env::remove_var("MAJU_RELAY_URL");
        ...
        assert_eq!(
            ...
            Some(format!("{PUBKEY_HEX}@maju").as_str())
        );
        assert_eq!(
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_unset() {
        ...
        // MAJU_ACP_DISPLAY_NAME lands on the Desktop side.
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_is_unusable() {
        ...
        for raw in ["<>", "\u{200B}"] {
            std::env::set_var("MAJU_ACP_DISPLAY_NAME", raw);
            let env = build_git_env(&key_info());
            assert_eq!(
