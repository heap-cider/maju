...
impl DevMcp {
    ...
    #[tool(
        name = "shell",
        description = "Run a shell command (bash by default; set `MAJU_SHELL` to use cmd, PowerShell, or another shell). Ephemeral process per call. Output tail-truncated to ~8KB for the LLM; full output (first 10MB) saved to artifact file. timeout_ms defaults to 120000 (2 min) if omitted; capped at 1,200,000 (20 min). For long-running commands (git push with hooks, cargo build, test suites), use 300000+. On PATH: rg (prefer over grep; flags: -n -i -l -g <glob> -C <n> --files), tree (flags: -d <depth>; shows line counts), and maju (Maju relay CLI — run maju --help for commands)."
    )]
    async fn shell(
        &self,
...
async fn async_main(cmd: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // maju CLI needs tokio (async HTTP client).
    if cmd == "maju" {
