...
fn server_authority(url: &reqwest::Url) -> Option<String> {
    let authority = maju_core::tenant::relay_url_authority(url.as_str());
...
/// media and `MAJU_PRIVATE_KEY` is available; `None` otherwise.
///
...
fn relay_media_get_auth(url: &reqwest::Url) -> Option<String> {
    let relay = std::env::var("MAJU_RELAY_URL").ok()?;
    let relay = reqwest::Url::parse(&relay).ok()?;
    if !is_relay_media_url(url, &relay) {
        return None;
    }
    let key = std::env::var("MAJU_PRIVATE_KEY").ok()?;
    let keys = match nostr::Keys::parse(&key) {
        Ok(k) => k,
        Err(e) => {
            tracing::warn!("MAJU_PRIVATE_KEY invalid; fetching relay media unauthenticated: {e}");
            return None;
        }
    };
...
/// `MAJU_RELAY_URL` + `MAJU_PRIVATE_KEY` are configured.
async fn fetch_url(url: &str) -> Result<Vec<u8>, ErrorData> {
    ...
    if auth.is_some() {
        // Do not forward Maju auth headers to a redirect target. reqwest strips
        ...
        if matches!(status.as_u16(), 401 | 403) && !authed {
            return Err(invalid_params(format!(
                "fetch {url} returned HTTP {status} — this relay requires authenticated media \
                 reads; set MAJU_PRIVATE_KEY (and MAJU_RELAY_URL) to a member identity"
            )));
        }
        return Err(invalid_params(format!(
