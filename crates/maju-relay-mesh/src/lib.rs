//! maju-relay-mesh — the inter-relay QUIC mesh.
//!
...
//! mesh-free: when `MAJU_MESH=off` or no peers exist, the relay never
...
// seams below; huddle fan-out lives in maju-relay's audio module (Dawn).
...
pub struct MeshConfig {
    /// `MAJU_MESH` — `on` (default when replicas can exist) | `off` kill
    /// switch. When off, the relay must behave exactly like single-instance.
    pub enabled: bool,
    /// UDP bind for the iroh endpoint (`MAJU_MESH_BIND_ADDR`, default
...
pub enum MeshError {
    ...
    #[error("mesh is disabled (MAJU_MESH=off)")]
    Disabled,
    #[error("transport: {0}")]
    Transport(String),
    #[error("redis: {0}")]
