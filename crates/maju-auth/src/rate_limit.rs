...
//! `maju-relay` / `maju-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `maju-relay` / `maju-pubsub`.
...
/// key is community-prefixed (`maju:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `maju:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `maju:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("maju:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("maju:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(ip_rate_limit_key(&ip), "maju:ratelimit:ip:192.168.1.1:conn");
    }
