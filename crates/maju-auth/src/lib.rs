#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `maju-auth` — Authentication and authorization for the Maju relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("maju-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("maju-test-key:{username}");
