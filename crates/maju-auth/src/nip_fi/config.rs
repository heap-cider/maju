...
//!
//! Maju ships the generic OSS contract only: issuer URLs and audiences are
...
impl TransportContractId {
    ...
    pub fn core_client_attached() -> Self {
        ...
        hasher.update(b"maju:nip-fi:transport-contract:v1\0");
...
pub enum TokenClass {
    ...
    /// A dedicated Maju assertion: protected `typ` is exactly `nip-fi+jwt`.
    DedicatedNipFi,
}
...
fn derive_assertion_policy_id(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"maju:nip-fi:assertion-policy:v1\0");
