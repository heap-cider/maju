...
/// this private supertrait, so no external `maju_auth` consumer can implement
...
/// outside `maju_auth`. That closes the authority-construction seam: an
...
///
/// ```compile_fail
/// use maju_auth::{AssertionKeySet, IssuerKeySource};
/// struct Forge;
