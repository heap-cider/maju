...
//!
//! ```ignore
//! let pubkey = maju_auth::verify_nip98_event(json, url, method, body)?;
//! if !replay.try_mark(&ctx, &event_id, maju_auth::DEFAULT_REPLAY_TTL_SECS).await? {
...
///
/// The production implementation lives in `maju-pubsub` (Redis `SET NX EX`).
...
/// `maju:{community}:nip98:{event_id_hex}`.
///
...
pub fn nip98_replay_key_for_scope(scope: &str, event_id: &EventId) -> String {
    format!("maju:{scope}:nip98:{}", event_id.to_hex())
}
...
mod tests {
    ...
    fn key_includes_community_prefix() {
        ...
        let expected_prefix = format!("maju:{}:nip98:", ctx.community());
        assert!(
