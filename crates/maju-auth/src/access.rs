...
//!
//! Defines [`ChannelAccessChecker`] so `maju-auth` can enforce access
//! without depending on `maju-db` directly.
...
///
/// Implemented by the database layer (`maju-db`) in production. The `maju-auth`
