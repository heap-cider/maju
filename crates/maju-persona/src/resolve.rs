...
fn runtime_env_vars(persona: &LoadedPersona) -> Vec<(String, String)> {
    ...
    if let Some(model_str) = &persona.model {
        ...
        match runtime {
            Some("maju-agent") => {
                vars.push(("MAJU_AGENT_MODEL".to_owned(), model_id.to_owned()));
                if let Some(p) = provider {
                    vars.push(("MAJU_AGENT_PROVIDER".to_owned(), p.to_owned()));
                }
            }
            _ => {
                if let Some(p) = provider {
...
mod tests {
    ...
    #[test]
    fn runtime_env_vars_maju_agent_emits_maju_agent_vars() {
        let mut lp = stub_persona(Some("databricks:goose-claude-4-6-opus"), None, None);
        lp.runtime = Some("maju-agent".to_owned());
        let vars = runtime_env_vars(&lp);
        let map: HashMap<&str, &str> = vars.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
        assert_eq!(map["MAJU_AGENT_MODEL"], "goose-claude-4-6-opus");
        assert_eq!(map["MAJU_AGENT_PROVIDER"], "databricks");
    ...
    fn runtime_env_vars_goose_emits_goose_vars() {
        ...
        assert!(!map.contains_key("MAJU_AGENT_MODEL"));
    }
    ...
    #[test]
    fn runtime_env_vars_maju_agent_model_without_provider() {
        let mut lp = stub_persona(Some("gpt-4o"), None, None);
        lp.runtime = Some("maju-agent".to_owned());
        let vars = runtime_env_vars(&lp);
        let map: HashMap<&str, &str> = vars.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
        assert_eq!(map["MAJU_AGENT_MODEL"], "gpt-4o");
        assert!(!map.contains_key("MAJU_AGENT_PROVIDER"));
    }
