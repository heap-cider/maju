...
//! (OPS metadata) and tells Maju where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the Maju runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "maju")]
    pub maju: Option<String>,
}
...
/// validator issues advisory warnings for Maju-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().maju.as_deref(), Some(">=0.9.0"));
