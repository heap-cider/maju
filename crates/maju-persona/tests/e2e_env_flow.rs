...
//! - Maju-agent personas emit MAJU_AGENT_MODEL, MAJU_AGENT_PROVIDER
...
use maju_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_maju_agent_persona_emits_maju_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("MAJU_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit MAJU_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("MAJU_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit MAJU_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "maju-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "maju-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Maju-agent persona
    fs::write(
        root.join("agents/maju-bot.persona.md"),
        r#"---
name: "maju-bot"
display_name: "Maju Bot"
description: "A maju-agent runtime bot"
runtime: "maju-agent"
model: "openai:gpt-4o"
---
You are a maju bot.
"#,
    ...
    let maju = pack
        .personas
        .iter()
        .find(|p| p.name == "maju-bot")
        .expect("maju-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("MAJU_AGENT_MODEL"),
        "goose persona must not emit MAJU_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("MAJU_AGENT_PROVIDER"),
        "goose persona must not emit MAJU_AGENT_PROVIDER"
    );

    // Maju-agent persona gets MAJU_AGENT_* env vars
    let maju_env: std::collections::HashMap<_, _> = maju
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(maju_env.get("MAJU_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(maju_env.get("MAJU_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !maju_env.contains_key("GOOSE_MODEL"),
        "maju-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !maju_env.contains_key("GOOSE_PROVIDER"),
        "maju-agent persona must not emit GOOSE_PROVIDER"
    );
}
