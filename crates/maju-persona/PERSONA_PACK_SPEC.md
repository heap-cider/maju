...
for deployment in Maju. It is a **superset of the [Open Plugin Spec](https://open-plugin-spec.org)**
...
1. **Portable** — zip file or git repo; no Maju tooling required to inspect
...
4. **Harness-honest** — explicit about what the agent runtime does vs. what maju-acp does
...
and Maju-specific extensions live alongside the OPS fields at the top level. Since the Open
...
```json
{
  ...
  "version": "1.2.0",
  "description": "A four-agent security review team for Maju.",
  "author": "Meadow Engineering",
  "license": "MIT",
  "homepage": "https://github.com/example/meadow-security-team",
  "keywords": ["security", "code-review", "maju"],
  "engines": {
    "maju": ">=0.9.0"
  },
  "personas": [
...
- **Maju**: reads both OPS fields and the Maju-specific fields; `personas` is authoritative.
- **Version negotiation**: `engines.maju` specifies minimum required Maju version; maju-acp
  rejects packs requiring a newer version.
- **Extension mechanism**: Maju-specific fields sit at the top level of `plugin.json` alongside
  OPS fields. No OPS core field is overloaded.
- **`defaults`**: ignored entirely by OPS consumers. maju-acp resolves it at deploy time before
...
└── my-pack-1.2.0.majupack.sha256  # Checksum (required for zip distribution)
```
...
# === Behavioral Config (Maju-specific) ===
subscribe:
  - "#security-reviews"
  - "#code-reviews"
triggers:
...
| `display_name` | string | ✅ | Human-readable name shown in Maju UI. |
...
maju-acp assembles the agent's context from two distinct prompt layers before sending each
...
```
[Base]
<platform orientation — injected by maju-acp>
...
The `[Base]` layer is compiled into maju-acp and is **identical for every agent**. It covers:

| Content | Purpose |
|---------|---------|
| Platform identity | Tells the agent it is running inside Maju and what that means |
...
Pack authors do not write or configure the `[Base]` layer — it is maintained by the Maju team
and updated in maju-acp releases.

**Disabling or customizing the base layer**: Set `MAJU_ACP_NO_BASE_PROMPT` to omit the `[Base]`
section entirely. To replace the compiled-in default with custom content, set
`MAJU_ACP_BASE_PROMPT_FILE` to a file path — maju-acp reads it at startup and uses it instead.
...
- That the agent is running inside Maju (covered by `[Base]`)
...
> **Note**: `.agents/skills/` is maju-acp's canonical skill location. The other paths shown
...
$AGENT_CWD/.agents/skills/<skill-name>/SKILL.md   ← maju-acp uses this one
```
...
maju-acp process. maju-acp determines what value to pass as `NewSessionRequest.cwd` in this
order:

1. The `AGENT_CWD` environment variable on the maju-acp process, if set.
2. `std::env::current_dir()` as a fallback.
3. If both fail, maju-acp logs an error and **refuses to start**.
...
is **not overwritten**. This allows operators to pin custom skill versions. maju-acp **must log
...
maju-acp lists available skills in the user message prefix so the agent knows what's available.
...
by the agent runtime. `maju pack validate` warns on skill name mismatches but does not yet
...
maju-acp merges them and passes the result via the ACP protocol — no filesystem placement required.
...
When implemented, all `env` values will be scanned for `${VAR_NAME}`. maju-acp will resolve
from the process environment **before** passing to the agent runtime. Unresolved variables will
cause a startup error.

### Delivery

maju-acp passes the merged config via `NewSessionRequest.mcp_servers`. **No `.mcp.json` is written to the agent's working directory.**
...
in the pack. maju-acp appends it to the persona prompt in the user message prefix.

maju-acp appends `instructions.md` to the persona prompt in the user message prefix (see
...
Hooks are shell commands fired by maju-acp at agent lifecycle points. **Agent runtimes typically have no hook system**
...
Hooks run as the maju-acp user; working directory is `$AGENT_CWD`; agent env vars are available.
...
that must be implemented directly in maju-acp's dispatch loop, not via a hook.
...
`on_stop` fires on normal exit and on handled errors. It **will not fire** if maju-acp crashes
...
**Hooks are NOT agent runtime features.** They are implemented entirely in maju-acp. Bypassing
maju-acp means no hooks fire.
...
Maju conversations. These are all Maju-specific — the agent runtime has no awareness of them. They sit
...
In this spec, **"deploy time"** means when maju-acp loads the pack and constructs per-persona
session configurations — typically at maju-acp process startup. For git-based packs, this occurs
each time maju-acp starts and reads the installed pack directory.

When maju-acp resolves the effective configuration for a persona, it applies this order (highest
wins):

```
1. Operator env vars           — e.g. GOOSE_MODEL, GOOSE_PROVIDER (agent-runtime-specific)
                                 already set in the parent process environment
2. Desktop UI per-agent        — overrides set in the Maju desktop app per-agent settings
...
5. Built-in defaults           — maju-acp's hardcoded fallback values
```

maju-acp resolves levels 3–5 at deploy time (when the pack is loaded and sessions are
constructed). Levels 1–2 are applied at runtime and are outside the pack's control.

**Level 1 — Operator env vars**: If the operator has already set env vars for model, provider, temperature, or context limit in the parent process environment, maju-acp MUST NOT override them with pack/persona values. maju-acp only injects env vars for fields that are NOT already set in the parent environment. This ensures operators can always override pack configuration.

**Implementation**: when constructing the child process environment, maju-acp checks
...
| `temperature` | float | `0.7` | Provider-dependent (typically 0.0–2.0). maju-acp passes through without range validation; `maju pack validate` checks type only (must be a number), not range. | Passed as env var to agent runtime. |
...
**Unknown keys** in `defaults` (in `plugin.json`) are **validation warnings** in `maju pack
...
maju-acp logs a `WARN` and ignores unknown manifest keys, remaining fail-soft:
...
The `#` prefix in `subscribe` entries is a **display convention only**. Channel names in the Maju
relay are stored and queried **without** the `#` prefix. maju-acp strips the leading `#` before
making any relay API calls. `"#security-reviews"` and `"security-reviews"` are equivalent in this
field.

### Env Var Mapping

maju-acp resolves pack defaults and per-persona overrides (precedence levels 3–5) into a single
...
maju-acp translates persona behavioral config fields to agent configuration via environment
...
If `model` is omitted from both the persona frontmatter and `defaults`, maju-acp does not set
`GOOSE_PROVIDER` or `GOOSE_MODEL`, and the agent runtime uses its configured operator default.

> **Implementation note**: `AcpClient::spawn` accepts per-persona env vars via the `extra_env` parameter. maju-acp checks `std::env::var(key)` before injecting each var — if the parent environment already has the key set, injection is skipped (operator precedence, level 1).
...
> Field Schema table lists maju-acp's built-in fallbacks (precedence level 5). If `defaults` is
...
All fields are consumed entirely by maju-acp. None are passed to the agent runtime directly — they are projected as env vars or used by the harness's subscription/dispatch logic.
...
A pack is distributed as a `.majupack` file (zip archive):

```bash
maju pack validate ./my-pack
maju pack ./my-pack --output my-pack-1.2.0.majupack
maju install ./my-pack-1.2.0.majupack
maju install https://example.com/releases/my-pack-1.2.0.majupack
```

#### Pack Integrity (Required)

Zip packs **must** ship with `<pack-name>-<version>.majupack.sha256` containing `sha256sum`
output (`<hex-digest>  <filename>`). maju-acp **must** verify before installation and refuse on
...
```json
{
  "com.example.meadow-security-team": {
    "source": "https://example.com/releases/my-pack-1.2.0.majupack",
    "sha256": "a3f1c2d4e5b6...",
    "version": "1.2.0",
...
```bash
maju install github:example/meadow-security-team
maju install github:example/meadow-security-team@v1.2.0
maju install git+https://gitlab.example.com/team/pack.git
```
...
A Maju-hosted registry and in-app browser for discovering and installing packs. API-compatible
with OPS registries. Details TBD.

### Installed Pack Location

Installed packs live at `~/.maju/packs/<pack-id>/`. maju-acp reads packs from this location
at agent startup.

### Desktop App Import

The Maju desktop app's **Agents** page does not import persona-pack `.zip` archives or
...
- **Agent teams section → Import**: Accepts `.team.json` or `.team.png` (a `maju-team-snapshot
...
> `maju pack validate` / `maju pack inspect` (Section 11). A snapshot is captured *from* an
...
> `maju pack inspect`'s resolved config as reference.
...
|-----------|----------------|-----------|-------------------|
| Skills | Copy at deploy time (planned) | maju-acp will copy `skills/` → `$AGENT_CWD/.agents/skills/` | ✅ Yes (only one) |
| MCP servers | ACP protocol | `NewSessionRequest.mcp_servers` | ❌ No |
| Persona prompt | User message prefix | `[System]` block prepended to user message text by maju-acp | ❌ No |
| Pack instructions | User message prefix | Appended to `[System]` block in user message text | ❌ No |
| Lifecycle hooks | Harness internal | maju-acp fires shell commands directly | ❌ No |
| Model/provider | Child process env vars | Agent-runtime-specific env vars (e.g. `GOOSE_PROVIDER`, `GOOSE_MODEL`) | ❌ No |
| Behavioral config | Harness internal | maju-acp subscription + dispatch logic | ❌ No |
| Pack defaults (`defaults`) | Harness internal | Resolved at deploy time by maju-acp into per-persona effective config; never passed to the agent runtime directly | ❌ No |

> **Pack defaults are resolved at deploy time**, not at runtime. When maju-acp loads a pack and
...
maju-acp's `format_prompt()` in `queue.rs` prepends a `[System]` block to the **user message
text** before sending it to the agent runtime. This is a **maju-acp feature, not an agent runtime
...
harness-side interpolation is implemented, maju-acp will resolve them from the process
...
- **Phase 1 (zip)**: Packs **must** ship with `<pack-name>-<version>.majupack.sha256` containing
  `sha256sum` output (`<hex-digest>  <filename>`). maju-acp **must** verify before installation
  and refuse on mismatch.
- **Phase 2 (git)**: `pack.lock` pins the resolved commit SHA; maju-acp verifies on install.
- **Phase 3 (registry)**: Registry signatures TBD.

### Hook Execution

Hooks run with maju-acp's privileges — significant attack surface. Only install packs from
trusted sources. Review all hook commands before installing. Consider sandboxing maju-acp
(container, restricted user) for untrusted packs. maju-acp should display hook commands before
...
### From V6 (maju-namespaced) Format

Field mapping from V6 `.persona.md` to current `.persona.md`:

| V6 location | Current location |
|---|---|
| `maju.model` | `model` (top-level frontmatter) |
| `maju.temperature` | `temperature` (top-level frontmatter) |
| `maju.max_context_tokens` | `max_context_tokens` (top-level frontmatter) |
| `maju.subscribe` | `subscribe` (top-level frontmatter) |
| `maju.respond_to` | `triggers` (top-level frontmatter) |
| `maju.thread_replies` | `thread_replies` (top-level frontmatter) |
| `maju.broadcast_replies` | `broadcast_replies` (top-level frontmatter) |
| `plugin.json` → `maju.defaults` | `plugin.json` → `defaults` (top-level) |
| `plugin.json` → `maju.personas` | `plugin.json` → `personas` (top-level) |
| `plugin.json` → `maju.pack_instructions` | `plugin.json` → `pack_instructions` (top-level) |
| `plugin.json` → `maju.mcp_config` | `plugin.json` → `mcp_config` (top-level) |
| `plugin.json` → `maju.hooks_config` | `plugin.json` → `hooks_config` (top-level) |
...
5. Run `maju pack validate ./my-pack`

### Backward Compatibility

The V6 namespaced `maju:` block format is not supported. Only the current flat top-level fields format is accepted. The `respond_to` key is accepted as a legacy alias for `triggers` in both persona frontmatter and `plugin.json` defaults.
...
1. **`session/set_model` as env var alternative**: The ACP runtime implements `on_set_model()` (ACP
   unstable feature). maju-acp could call `session/set_model` after `session/new` to set the
...
2. **`CONTEXT_FILE_NAMES` env var**: The goose agent runtime supports this env var to control which filenames are
   scanned for hints. Should maju-acp set this to include pack-specific filenames? Deferred
   pending use case.
...
`maju pack init` scaffolding; hot reload of skills/instructions; skill marketplace; pack dependencies; agent-to-agent handoff within a pack.
...
| PF-2 | `maju pack validate` CLI: **Implemented.** Schema-validates `plugin.json`; checks `.persona.md` required identity fields; validates behavioral config fields; warns on unknown keys and skill name mismatches. Remaining: verify `skills:` and `hooks:` paths exist; error on `SKILL.md` missing `name:` or `description:`. | `maju-cli` / `maju-admin` |
| PF-3 | Skill collision warning: emit `WARN` when a pack skill is skipped because a skill with the same load key already exists in `.agents/skills/`. | maju-acp skill copy logic |
| PF-4 | `$AGENT_CWD` resolution: determine `NewSessionRequest.cwd` from (1) `AGENT_CWD` env var, (2) `std::env::current_dir()`, (3) error and refuse to start. | maju-acp startup / session init |
| PF-5 | Skill parse failure warning: emit `WARN` when `parse_skill_content` returns `None` (missing `name:`, missing `description:`, or malformed frontmatter). Currently the agent runtime silently skips. maju-acp should pre-validate during skill copy. | maju-acp skill copy logic |
| PF-6 | Per-subprocess env var injection: **Implemented.** `AcpClient::spawn` accepts `extra_env: &[(String, String)]` injected via `Command::env()`. maju-acp checks `std::env::var(key)` before injecting — operator env vars take precedence (level 1). | `maju-acp/src/acp.rs` `AcpClient::spawn()` |
