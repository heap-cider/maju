...
NIP-98 credential helper for git — signs HTTP auth events with your Nostr key so git can push/pull from Maju's git server without passwords.
...
When a Maju git server returns `HTTP 401` with a
...
| `method hint` | Server's `WWW-Authenticate` header is missing `method="..."` | Upgrade the Maju server |
