//! git-credential-nostr — NIP-98 git credential helper for Maju.
//!
...
/// Load the NIP-OA owner attestation injected by Maju Desktop/ACP.
///
...
fn load_auth_tag() -> Result<Option<Tag>, String> {
    let raw = std::env::var("MAJU_AUTH_TAG")
        .ok()
...
fn parse_method(wwwauth: &str) -> Option<HttpMethod> {
    ...
    // Handles variations: `Nostr method="GET", realm="maju"` and
    // `Nostr method="GET",realm="maju"` (with or without space after comma).
...
pub fn run() -> i32 {
    ...
    // No Nostr challenge from the server — this isn't a Maju remote.
    // Exit silently so git falls through to the next credential helper.
    // This check comes FIRST so non-Maju remotes never hit validation errors.
