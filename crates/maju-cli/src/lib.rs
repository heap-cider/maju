...
use client::MajuClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the Maju CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = maju_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from maju-acp/maju-dev-mcp)
    ...
    // maju-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[command(
    name = "maju",
    about = "Maju CLI — interact with a Maju relay",
    long_about = "\
Maju CLI — interact with a Maju relay

Configuration (flags override env vars):
  MAJU_RELAY_URL     Relay base URL        [default: http://localhost:3000]
  MAJU_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  MAJU_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides MAJU_RELAY_URL env var.
    #[arg(long, env = "MAJU_RELAY_URL", default_value = "http://localhost:3000")]
    relay: String,

    /// Nostr private key (hex or nsec). This is the CLI's identity.
    #[arg(long, env = "MAJU_PRIVATE_KEY", hide_env_values = true)]
    private_key: Option<String>,

    /// NIP-OA auth tag JSON (owner attestation). Injected into every signed event.
    #[arg(long, env = "MAJU_AUTH_TAG", hide_env_values = true)]
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's Maju Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's Maju Desktop
    DraftUpdate {
...
running under MAJU_AUTH_TAG signs as itself, so it can only ever satisfy \
the self path (target == signer) — not the owner-of-agent path for another \
identity.\n\n\
Examples:\n  \
maju agents archive <PUBKEY> --reason retired\n  \
maju agents archive <PUBKEY> --reason bot-rebuilt --replaced-by <NEW_PUBKEY>"
    )]
    Archive {
...
Examples:\n  \
maju agents unarchive <PUBKEY> --reason returned"
    )]
    Unarchive {
        /// Target identity pubkey (hex)
        target_pubkey: String,
...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  maju messages send --channel <UUID> --content \"hello\"\n  maju messages send --channel <UUID> --content \"@alice check this\"\n  echo \"hello from stdin\" | maju messages send --channel <UUID> --content -"
    )]
    Send {
        /// Channel UUID (from 'maju channels list')
        #[arg(long)]
        channel: String,
    ...
    #[command(
        after_help = "Examples:\n  maju messages get --channel <UUID>\n  maju messages get --channel <UUID> --limit 50 --kinds 1,1984"
    )]
    Get {
        /// Channel UUID
        #[arg(long)]
    ...
    /// Get the containing thread for a message or Maju message link
    #[command(
        after_help = "Examples:\n  maju messages thread --channel <UUID> --event <EVENT_ID>\n  maju messages thread --link 'maju://message?channel=<UUID>&id=<EVENT_ID>&thread=<ROOT_ID>'"
    )]
    Thread {
        ...
        #[arg(long, required_unless_present = "link", conflicts_with = "link")]
        event: Option<String>,
        /// Canonical maju://message deep link; uses the configured relay and identity
    ...
    #[command(
        after_help = "Examples:\n  maju messages search --query checkout\n  maju messages search --author npub1... --since 1783497600\n  maju messages search --author Aaron --query checkout --limit 20"
    )]
    Search {
...
pub enum ChannelsCmd {
    ...
    #[command(
        after_help = "Examples:\n  maju channels list\n  maju channels list --visibility open"
    )]
    List {
    ...
    #[command(
        after_help = "Examples:\n  maju channels search --query composer\n  maju channels search --query maju-chat-composer --exact\n  maju channels search --query design --include-archived"
    )]
    Search {
    ...
    #[command(
        after_help = "Examples:\n  maju channels create --name general --type stream --visibility open\n  maju channels create --name design --type forum --visibility open --description \"Design discussions\"\n  maju channels create --name standup --type stream --visibility open --ttl 3600  # ephemeral, archived after 1h idle\n  maju channels create --name project-x --template \"Maju Team\"  # type/visibility/canvas/roster from the template; explicit flags override"
    )]
    Create {
        /// Channel name
        #[arg(long)]
    ...
    #[command(
        after_help = "Examples:\n  maju channels update --channel <uuid> --name general\n  maju channels update --channel <uuid> --visibility open\n  maju channels update --channel <uuid> --visibility private"
    )]
    Update {
        /// Channel UUID
        #[arg(long)]
...
pub enum GifsCmd {
    ...
    /// `maju messages send --content` to share a GIF.
    Search {
...
pub enum WorkflowsCmd {
    ...
    #[command(
        after_help = "Examples:\n  maju workflows trigger --workflow <UUID>\n  maju workflows trigger --workflow <UUID> --inputs '{\"key\":\"value\"}'"
    )]
    Trigger {
        /// Workflow UUID
        #[arg(long)]
...
pub enum NotesCmd {
    ...
    #[command(
        after_help = "Examples:\n  echo '# Hello' | maju notes set --name hello --title 'Hello' --content -\n  maju notes set --name hello --tag onboarding --content - < draft.md"
    )]
    Set {
...
pub enum ReposCmd {
    ...
    Create {
        ...
        relays: Vec<String>,
        /// Channel UUID to bind the repo to. The `maju-channel` tag is the
        /// git ACL: without it the relay 404s every clone/fetch/push until
        /// the author runs `maju repos bind` (issue #3527).
        #[arg(long)]
        channel: Option<String>,
    },
    ...
    ///
    /// The `maju-channel` tag on the announcement is the git ACL: the relay
...
pub enum ProjectsCmd {
    ...
    /// Draft a project-linked channel for owner review in Maju Desktop
...
pub enum PatchesCmd {
    ...
    #[command(
        after_help = "Examples:\n  git format-patch -1 HEAD --stdout | maju patches send --repo-owner <hex> --repo-id myrepo --patch-file - --root\n  maju patches send --repo-owner <hex> --repo-id myrepo --patch-file 0001-fix.patch --reply-to <prev-patch-id>"
    )]
    Send {
...
pub enum PrCmd {
    ...
    #[command(
        after_help = "Examples:\n  maju pr open --repo-owner <hex> --repo-id myrepo --subject 'Fix bug' --body-file - --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo --branch-name fix-bug\n  maju pr update --repo-owner <hex> --repo-id myrepo --pr <event> --pr-author <hex> --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo"
    )]
    Open {
        /// Repo owner pubkey (64-char hex)
        #[arg(long)]
...
pub enum IssuesCmd {
    ...
    Create {
        ...
        /// `MAJU_GIT_ORIGIN_CHANNEL_ID`) names a project home.
        #[arg(long)]
        repo_owner: Option<String>,
        ...
        /// this project when none exists. Defaults to `MAJU_GIT_ORIGIN_CHANNEL_ID`.
        #[arg(long)]
        channel: Option<String>,
        /// Issue title
...
/// Subcommands for `maju mem`.
#[derive(Subcommand)]
pub enum MemCmd {
    /// List non-tombstoned memory entries
    Ls {
        /// Owner pubkey (hex). Overrides MAJU_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
    ...
    Patch {
        ...
        /// against. Hashes the exact UTF-8 bytes returned by `maju mem get`,
        /// not normalized lines. Run `maju mem hash <slug>` to capture this
        /// before editing.
        #[arg(long)]
...
/// `MAJU_RELAY_URL` — moderation commands are community-global and carry no
...
pub enum ModerationCmd {
    ...
    #[command(
        after_help = "Examples:\n  maju moderation reports\n  maju moderation reports --status open --limit 20"
    )]
    Reports {
    ...
    #[command(
        after_help = "Examples:\n  maju moderation resolve --report <REPORT_EVENT_ID> --status dismissed --action dismiss\n  maju moderation resolve --report <REPORT_EVENT_ID> --status resolved --action ban --reason \"rule 3\""
    )]
    Resolve {
    ...
    #[command(
        after_help = "Examples:\n  maju moderation ban --pubkey <HEX>\n  maju moderation ban --pubkey <HEX> --expires-in 604800 --reason \"repeated spam\""
    )]
    Ban {
    ...
    #[command(
        after_help = "Examples:\n  maju moderation timeout --pubkey <HEX> --expires-in 3600\n  maju moderation timeout --pubkey <HEX> --expires-at 1783500000 --reason \"cool off\""
    )]
    Timeout {
        /// Target member pubkey (hex)
        #[arg(long)]
...
/// Normalize hand-authored `MAJU_AUTH_TAG` input to strict JSON.
///
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("MAJU_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid MAJU_PRIVATE_KEY: {e}")))?;

    // NIP-OA: parse and verify the auth tag if provided.
    //
    // `MAJU_AUTH_TAG` is hand-authored configuration, so the unquoted raw
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = maju_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("MAJU_AUTH_TAG is malformed: {e}")))?;
            maju_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "MAJU_AUTH_TAG verification failed for pubkey {}: {e}",
                    keys.public_key().to_hex()
                ))
            })?;
            // Canonical wire form derives from the parsed-and-verified tag
            // (same shape as maju-acp's RestClient), never from raw input.
            let canonical = serde_json::to_string(tag.as_slice())
                .map_err(|e| CliError::Auth(format!("MAJU_AUTH_TAG serialization failed: {e}")))?;
            (Some(tag), Some(canonical))
        }
        _ => (None, None),
    };

    let client = MajuClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
...
mod tests {
    ...
    fn messages_thread_accepts_link_or_explicit_identifiers() {
        ...
        let link = format!("maju://message?channel={channel}&id={event}");

        assert!(
            Cli::try_parse_from(["maju", "messages", "thread", "--link", link.as_str(),]).is_ok()
        );
    ...
    fn messages_thread_rejects_partial_or_mixed_targets() {
        ...
        assert!(Cli::try_parse_from(["maju", "messages", "thread"]).is_err());
        assert!(Cli::try_parse_from(["maju", "messages", "thread", "--channel", channel]).is_err());
        assert!(Cli::try_parse_from([
            "maju",
            "messages",
            "thread",
    ...
    fn set_status_clear_rejects_text_and_emoji() {
        for extra in [["--text", "busy"], ["--emoji", "🎶"]] {
            let args = ["maju", "users", "set-status", "--clear"]
                .into_iter()
                .chain(extra);
            assert!(
    ...
    fn set_status_requires_text_or_clear() {
        assert!(Cli::try_parse_from(["maju", "users", "set-status"]).is_err());
        assert!(
            Cli::try_parse_from(["maju", "users", "set-status", "--emoji", "🎶"]).is_err(),
            "--emoji alone must not imply a status"
        );
        assert!(Cli::try_parse_from(["maju", "users", "set-status", "--clear"]).is_ok());
    }
    ...
    #[test]
    fn projects_create_accepts_repositories_without_optional_metadata() {
        assert!(Cli::try_parse_from([
    ...
    fn projects_update_no_mutation_is_rejected_by_clap() {
        ...
        assert!(
            Cli::try_parse_from(["maju", "projects", "update", "my-slug"]).is_err(),
