...
fn relay_server_tag(relay_url: &str) -> Option<String> {
    let authority = maju_core::tenant::relay_url_authority(relay_url);
...
pub struct MajuClient {
    http: reqwest::Client,
    relay_url: String, // base URL, no trailing slash, e.g. "https://relay.maju.place"
    keys: Keys,
...
impl MajuClient {
    ...
    ///
    /// - `MAJU_CONNECT_TIMEOUT_SECS` — TCP connect timeout (default 15 s)
    /// - `MAJU_TIMEOUT_SECS` — per-request total timeout (default 30 s)
    ///
    ...
    pub fn new(
        ...
        let http = reqwest::Client::builder()
            .timeout(env_duration_secs("MAJU_TIMEOUT_SECS", 30))
            .connect_timeout(env_duration_secs("MAJU_CONNECT_TIMEOUT_SECS", 15))
            .build()
    ...
    ///
    /// Used by `maju gifs search` and `maju gifs share` to reach the relay's
                ...
                Ok(resp) => {
                    ...
                    if !resp_was_success(status) {
                        let message = serde_json::from_str::<serde_json::Value>(&body_text)
                            ...
                            .unwrap_or(body_text);
                        let message = if status == 403 && std::env::var("MAJU_AUTH_TAG").is_ok() {
                            format!(
                                "{message} (MAJU_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                            )
                        } else {
                            message
                        };
    ...
    /// `maju_ws_client::publish_event` which handles connect, NIP-42 auth,
    ...
    pub async fn publish_ephemeral_event(&self, event: nostr::Event) -> Result<String, CliError> {
        ...
        // See maju_ws_client::{AUTH_CHALLENGE_TIMEOUT_SECS, AUTH_OK_TIMEOUT_SECS,
        // PUBLISH_OK_TIMEOUT_SECS} for the inner ceilings.
        let ok =
            maju_ws_client::publish_event(&ws_url, event, &self.keys, self.auth_tag.as_ref(), 75)
                .await
                .map_err(|e| CliError::Other(e.to_string()))?;
    ...
    async fn handle_response(&self, resp: reqwest::Response) -> Result<String, CliError> {
        if !resp.status().is_success() {
            ...
            let message = serde_json::from_str::<serde_json::Value>(&body)
                ...
                .unwrap_or(body);
            if status == 403 && std::env::var("MAJU_AUTH_TAG").is_ok() {
                let message = format!(
                    "{message} (MAJU_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                );
                return Err(CliError::Relay {
...
/// MAJU_RELAY_URL may be ws/wss (copied from MCP config).
...
mod retry_tests {
    ...
    fn env_duration_secs_parsing() {
        ...
        const KEY: &str = "MAJU_CLI_TEST_DURATION_SECS";
...
/// through `MajuClient` to verify behavioural properties — not implementation details.
#[cfg(test)]
mod retry_policy_tests {
    ...
    fn test_client(base_url: &str) -> MajuClient {
        let keys = Keys::generate();
        MajuClient::new(base_url.to_string(), keys, None, None).unwrap()
    }
...
mod tests {
    ...
    fn with_auth_tag_omits_header_when_not_configured() {
        let keys = Keys::generate();
        let client = MajuClient::new("https://test.relay".into(), keys, None, None).unwrap();
