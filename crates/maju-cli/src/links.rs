...
/// A validated `maju://message` deep link.
...
/// Parse a `maju://message?channel=<uuid>&id=<event>[&thread=<root>]` link.
///
...
pub fn parse_message_link(input: &str) -> Result<MessageLink, CliError> {
    let url = url::Url::parse(input.trim())
        .map_err(|_| CliError::Usage("invalid Maju message link".into()))?;
        ...
        || url.fragment().is_some()
    {
        return Err(CliError::Usage(
            "expected a maju://message link without credentials or a fragment".into(),
        ));
    }
    ...
    for (key, value) in url.query_pairs() {
        let slot = match key.as_ref() {
            ...
            _ => {
                return Err(CliError::Usage(
                    "Maju message link contains an unsupported query parameter".into(),
                ))
            }
        };
        if slot.replace(value.into_owned()).is_some() {
            return Err(CliError::Usage(format!(
                "Maju message link contains more than one {key} parameter"
            )));
        }
    }
    ...
    let channel = channel
        ...
        .ok_or_else(|| CliError::Usage("Maju message link is missing channel".into()))?;
    let message = message
        .filter(|value| !value.is_empty())
        .ok_or_else(|| CliError::Usage("Maju message link is missing id".into()))?;
    if thread.as_deref() == Some("") {
        return Err(CliError::Usage(
            "Maju message link contains an empty thread parameter".into(),
        ));
    }

    let channel_id = uuid::Uuid::parse_str(&channel)
        .map_err(|_| CliError::Usage("Maju message link contains an invalid channel UUID".into()))?
        .to_string();
...
fn canonical_event_id(value: &str, parameter: &str) -> Result<String, CliError> {
    if value.len() != 64 || !value.chars().all(|character| character.is_ascii_hexdigit()) {
        return Err(CliError::Usage(format!(
            "Maju message link contains an invalid {parameter} event ID"
        )));
    }
    Ok(value.to_ascii_lowercase())
}

/// Whether a d-tag can be expressed in a `maju://` link.
///
...
/// Build a `maju://repo` link for a repository announcement (kind 30617).
pub fn repo_link(owner: &str, repo_id: &str) -> String {
    format!("maju://repo?owner={owner}&d={repo_id}")
}

/// Build a `maju://project` link for a project announcement (kind 30621).
pub fn project_link(owner: &str, project_id: &str) -> String {
    format!("maju://project?owner={owner}&d={project_id}")
}

/// Build a `maju://pr` link for a pull request event (kind 1618).
pub fn pull_request_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("maju://pr?id={event_id}&owner={owner}&d={repo_id}")
}

/// Build a `maju://issue` link for an issue event (kind 1621).
pub fn issue_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("maju://issue?id={event_id}&owner={owner}&d={repo_id}")
}
...
mod tests {
    ...
    fn parses_message_link_with_thread_root() {
        let parsed = parse_message_link(&format!(
            "maju://message?channel={CHANNEL}&id={MESSAGE}&thread={THREAD}"
        ))
        .unwrap();
    ...
    fn parses_message_link_without_thread_root() {
        let parsed =
            parse_message_link(&format!("maju://message?channel={CHANNEL}&id={MESSAGE}")).unwrap();
    ...
    fn rejects_message_link_that_could_change_connection_context() {
        for link in [
            format!("maju://message?channel={CHANNEL}&id={MESSAGE}&relay=other"),
            format!("maju://user:secret@message?channel={CHANNEL}&id={MESSAGE}"),
            format!("maju://message?channel={CHANNEL}&id={MESSAGE}#fragment"),
        ] {
    ...
    fn rejects_duplicate_or_malformed_message_link_identifiers() {
        for link in [
            format!("maju://message?channel={CHANNEL}&channel={CHANNEL}&id={MESSAGE}"),
            format!("maju://message?channel=not-a-uuid&id={MESSAGE}"),
            format!("maju://message?channel={CHANNEL}&id=not-an-event"),
            format!("maju://message?channel={CHANNEL}&id={MESSAGE}&thread="),
        ] {
            assert!(parse_message_link(&link).is_err(), "accepted {link}");
        }
    }
