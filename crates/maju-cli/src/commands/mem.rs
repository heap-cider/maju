//! `maju mem` — agent-side engram management (NIP-AE).
//!
//! Subcommands:
//! - `maju mem ls`                   — list non-tombstoned memories
//! - `maju mem get <slug>`            — print the value to stdout
//! - `maju mem hash <slug>`           — print sha256(value) hex
//! - `maju mem set <slug> <value|-> ` — write a value (use `-` for stdin)
//! - `maju mem patch <slug>`          — apply a unified diff to the current value
//! - `maju mem rm <slug>`             — publish a tombstone
//!
//! By default, the caller's `MAJU_PRIVATE_KEY` is the agent's nsec. The
//! agent's owner pubkey is resolved from `MAJU_AUTH_TAG` (NIP-OA attestation)
...
fn resolve_owner(client: &MajuClient, owner_flag: Option<&str>) -> Result<PublicKey, CliError> {
    ...
    let tag = client.auth_tag_owner_hex().ok_or_else(|| {
        CliError::Usage(
            "owner pubkey required (set MAJU_AUTH_TAG with a NIP-OA attestation or pass --owner)"
                .into(),
        )
    })?;
...
/// owner from `--owner` / MAJU_AUTH_TAG. Owner-side recovery passes
...
async fn submit_engram(client: &MajuClient, event: nostr::Event) -> Result<(), CliError> {
...
/// `maju mem ls` — list non-tombstoned memory entries.
pub async fn cmd_ls(
    client: &MajuClient,
    owner_flag: Option<&str>,
    agent_flag: Option<&str>,
...
/// `maju mem get <slug>` — print value (memory) or profile (core) to stdout.
///
...
pub async fn cmd_get(
    ...
    match body {
        ...
        Some(Body::Memory { value: Some(v), .. }) => {
            // Raw stdout, no trailing newline — round-trips with `maju mem set foo -`.
            std::io::stdout()
...
/// `maju mem set <slug> <value|->` — write a value or core profile.
///
        ...
        if buf.is_empty() && !allow_empty {
            return Err(CliError::Usage(
                "refusing to write empty value from stdin (an upstream pipeline step likely \
                 failed). Pass --allow-empty to confirm, or use `maju mem rm <slug>` to \
                 tombstone."
                    .into(),
            ));
        }
...
/// `maju mem hash <slug>` — print sha256(value) in hex to stdout.
///
...
/// then pass it to `maju mem patch --base-hash <hex>` to make the edit
...
pub async fn cmd_patch(
    ...
    match (base_hash, no_base_hash) {
        ...
        (None, false) => {
            return Err(CliError::Usage(
                "missing --base-hash <hex> (run `maju mem hash <slug>` to get it). \
    ...
    if new_value.is_empty() && !allow_empty {
        return Err(CliError::Usage(
            "refusing to write empty value (patch result is empty). \
             Pass --allow-empty to confirm, or use `maju mem rm <slug>` to tombstone."
                .into(),
        ));
    }
...
/// `maju mem rm <slug>` — publish a tombstone (`value: null`).
///
...
pub async fn cmd_rm(
    ...
    if slug == engram::CORE_SLUG {
        return Err(CliError::Usage(
            "core cannot be tombstoned; overwrite it with `maju mem set core ''` instead".into(),
        ));
    }
    let owner = resolve_owner(client, owner_flag)?;
...
pub async fn dispatch(cmd: crate::MemCmd, client: &MajuClient) -> Result<(), CliError> {
    use crate::MemCmd;
    match cmd {
...
mod tests {
    ...
    fn test_client(keys: nostr::Keys) -> MajuClient {
        MajuClient::new("http://127.0.0.1:9".into(), keys, None, None).unwrap()
    }
