...
/// A single contact entry (CLI-local, not from maju-sdk).
...
pub async fn cmd_publish_note(
    ...
    let builder = maju_sdk::build_note(content, reply_id)
...
pub async fn cmd_set_contact_list(
    ...
    let builder = maju_sdk::build_contact_list(&contacts)
        .map_err(|e| CliError::Other(format!("build error: {e}")))?;
...
pub async fn cmd_get_event(client: &MajuClient, event_id: &str) -> Result<(), CliError> {
...
pub async fn cmd_get_contact_list(client: &MajuClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
    let filter = serde_json::json!({
        "kinds": [3],
        "authors": [pubkey],
...
pub async fn dispatch(cmd: crate::SocialCmd, client: &MajuClient) -> Result<(), CliError> {
    use crate::SocialCmd;
    match cmd {
