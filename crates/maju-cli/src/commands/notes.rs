//! `maju notes` — NIP-23 long-form editable notes (kind:30023).
//!
...
pub async fn fetch_own_note(client: &MajuClient, slug: &str) -> Result<Option<Event>, CliError> {
...
pub async fn fetch_by_slug(client: &MajuClient, slug: &str) -> Result<Vec<Event>, CliError> {
    let filter = serde_json::json!({
        "kinds": [KIND_LONG_FORM],
        "#d": [slug],
        "limit": 50,
...
pub async fn resolve_author(client: &MajuClient, author_flag: &str) -> Result<PublicKey, CliError> {
    if author_flag == "me" {
...
pub async fn cmd_rm(client: &MajuClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::NotesCmd, client: &MajuClient) -> Result<(), CliError> {
    use crate::NotesCmd;
    match cmd {
        NotesCmd::Set {
            name,
