...
use maju_sdk::{GitAppliedPatchRef, GitPatchMeta, GitRepoCoord, GitStatus, GitStatusMeta};
...
pub async fn cmd_send_patch(
    ...
    let builder =
        with_git_provenance(maju_sdk::build_git_patch(&repo, &content, &meta).map_err(sdk_err)?)?;
...
pub async fn cmd_get_patch(client: &MajuClient, event: &str) -> Result<(), CliError> {
...
/// status kind either way. Shared by `maju issues status`.
...
pub async fn dispatch(cmd: crate::PatchesCmd, client: &MajuClient) -> Result<(), CliError> {
