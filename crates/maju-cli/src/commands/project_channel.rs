...
//! (or creates) a `kind:30617` bound to the same `maju-channel` rather than
//! asking the caller to invent a second project.

use maju_core::kind::KIND_GIT_REPO_ANNOUNCEMENT;
use nostr::Event;
...
fn project_is_unlisted(event: &Event) -> bool {
    event.tags.iter().any(|tag| {
        matches!(tag.as_slice(), [name, value, ..] if name == "maju-visibility" && value == "unlisted")
    })
}
...
fn repo_from_announcement(event: &Event, channel: &str) -> Option<ChannelProjectRepo> {
    if event.kind.as_u16() != KIND_GIT_REPO_ANNOUNCEMENT as u16
        ...
        || first_tag_value(event, "maju-channel") != Some(channel)
    {
        return None;
    }
...
async fn fetch_channel_repos(client: &MajuClient, channel: &str) -> Result<Vec<Event>, CliError> {
...
pub(crate) fn require_repo_channel_binding(event: &Event, channel: &str) -> Result<(), CliError> {
    match first_tag_value(event, "maju-channel") {
...
mod tests {
    ...
    fn authoritative_project_requires_repo_owner_consent() {
        ...
        let repo = signed_event(
            &owner,
            30617,
            vec![tag(&["d", "game"]), tag(&["maju-channel", channel])],
        );
    ...
    fn existing_repo_must_bind_requested_channel() {
        ...
        let matching = signed_event(
            &owner,
            30617,
            vec![tag(&["d", "game"]), tag(&["maju-channel", requested])],
        );
        ...
        let foreign = signed_event(
            &owner,
            30617,
            vec![tag(&["d", "game"]), tag(&["maju-channel", other])],
        );
        assert!(matches!(
