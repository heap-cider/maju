//! `maju projects` commands — NIP-MP kind:30621 write path.
//!
...
use maju_core::kind::KIND_PROJECT;
use maju_sdk::{
...
async fn cmd_add_channel_draft(
    ...
    let owner_hex = client
        .auth_tag_owner_hex()
        .ok_or_else(|| CliError::Auth("project channel requests require MAJU_AUTH_TAG".into()))?;
    ...
    if let Some(object) = output.as_object_mut() {
        ...
        object.insert(
            "message".into(),
            "Project channel draft sent to Maju Desktop for owner review. The channel is not created until the owner approves it."
                .into(),
        );
    }
    println!("{output}");
...
// ── Maju repo-ID grammar (bare --repo shorthand) ─────────────────────────────

/// Pattern for a Maju-hosted repo identifier (bare `--repo` shorthand).
...
/// Fetch listed kind:30621 heads whose `maju-channel` is `channel`.
fn project_tags_match_channel<'a>(tags: impl IntoIterator<Item = &'a Tag>, channel: &str) -> bool {
    tags.into_iter()
        .any(|tag| tag_name(tag) == Some("maju-channel") && tag_value(tag) == Some(channel))
}
...
fn project_is_unlisted(event: &Event) -> bool {
    event.tags.iter().any(|tag| {
        matches!(
            tag.as_slice(),
            [name, value, ..] if name == "maju-visibility" && value == "unlisted"
        )
    })
}
...
async fn fetch_own_project(client: &MajuClient, slug: &str) -> Result<Option<Event>, CliError> {
...
pub async fn cmd_create(
    ...
    if fetch_own_project(client, slug).await?.is_some() {
        return Err(CliError::Conflict(format!(
            "project {slug:?} already exists; use 'maju projects update' to modify it"
        )));
    }
...
/// `maju projects get`
pub async fn cmd_get(client: &MajuClient, slug: &str, owner: Option<&str>) -> Result<(), CliError> {
    validate_project_slug(slug)?;
...
pub async fn cmd_update(
    ...
    if !has_mutation {
        return Err(CliError::Usage(
            "maju projects update requires at least one of: \
    ...
    let singleton_fields = ["name", "description", "maju-channel", "maju-visibility"];
            ...
            if let Some(field) = tag_name(t) {
                if singleton_fields.contains(&field) {
                    let clear = match field {
                        ...
                        "maju-channel" => clear_channel || channel.is_some(),
                        "maju-visibility" => clear_visibility || visibility.is_some(),
                        _ => false,
                    };
                    return !clear;
                }
    ...
    if let Some(ch) = channel {
        tags.push(make_tag(&["maju-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(make_tag(&["maju-visibility", vis])?);
    }
...
pub async fn cmd_delete(client: &MajuClient, slug: &str) -> Result<(), CliError> {
    validate_project_slug(slug)?;
...
/// Does NOT impose the Maju repo-ID grammar — project slugs are more permissive.
...
/// Validate a `maju-visibility` value at the writer level.
...
pub async fn dispatch(cmd: crate::ProjectsCmd, client: &MajuClient) -> Result<(), CliError> {
...
#[cfg(test)]
mod tests {
    use maju_sdk::{validate_project_envelope, PROJECT_MEMBER_CAP};
    use nostr::Tag;
    ...
    async fn run_default_repo_create_race(
        ...
        let server = tokio::spawn(async move {
            while let Ok((mut socket, _)) = listener.accept().await {
                ...
                let (status, body) = if request.starts_with("POST /query ") {
                    ...
                    if kind == maju_core::kind::KIND_GIT_REPO_ANNOUNCEMENT as u16 {
                        (
                            "200 OK",
                            serde_json::json!({
        ...
        let client = crate::client::MajuClient::new(base_url, keys, None, None).unwrap();
    ...
    async fn create_does_not_publish_project_after_default_repo_loses_to_foreign_home() {
        ...
        assert_eq!(
            posted_kinds,
            vec![maju_core::kind::KIND_GIT_REPO_ANNOUNCEMENT as u16],
    ...
    async fn create_is_idempotent_when_dominated_default_repo_winner_matches_home() {
        ...
        assert_eq!(
            posted_kinds,
            vec![
                maju_core::kind::KIND_GIT_REPO_ANNOUNCEMENT as u16,
                maju_core::kind::KIND_PROJECT as u16,
            ]
        );
    }
    ...
    async fn project_lookup_scopes_the_production_query_before_the_global_bound() {
        ...
        let client =
            crate::client::MajuClient::new(base_url, nostr::Keys::generate(), None, None).unwrap();
        ...
        assert_eq!(body[0]["#maju-channel"], serde_json::json!([channel]));
    ...
    async fn channel_scoping_prevents_unrelated_heads_from_consuming_the_bound() {
        ...
        let server = tokio::spawn(async move {
            ...
            let response_body = if filter[0]["#maju-channel"] == serde_json::json!([channel]) {
    ...
    fn project_channel_matching_ignores_unrelated_claims() {
        ...
        let tags = make_head_tags(&[
            make_test_tag(&["maju-channel", "22222222-2222-4222-8222-222222222222"]),
            make_test_tag(&["name", "Unrelated"]),
        ]);
        assert!(!project_tags_match_channel(tags.iter(), expected));

        let tags = make_head_tags(&[make_test_tag(&["maju-channel", expected])]);
        assert!(project_tags_match_channel(tags.iter(), expected));
    }
    ...
    fn expand_repo_coord_rejects_uppercase_owner() {
        let upper = "30617:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:maju";
    ...
    fn rebuild_project_preserves_hinted_member_tags() {
        ...
        let coord = format!("30617:{OWNER_HEX}:maju");
    ...
    fn apply_update_tags(
        ...
        if let Some(ch) = channel {
            tags.push(make_test_tag(&["maju-channel", ch]));
        }
        if let Some(vis) = visibility {
            tags.push(make_test_tag(&["maju-visibility", vis]));
        }
        tags
    }
    ...
    fn update_clear_visibility_drops_tag() {
        let head = make_head_tags(&[make_test_tag(&["maju-visibility", "unlisted"])]);
        let result = apply_update_tags(head, None, false, None, false, None, false, None, true);
        assert!(!result
            .iter()
            .any(|t| tag_name(t) == Some("maju-visibility")));
    }

    #[test]
    fn update_exactly_one_singleton_after_replace() {
        // Start with a maju-channel; replace with a new one; must have exactly one.
        let uuid1 = "3580ca9b-47b4-4af9-b22a-1068778f26c6";
        let uuid2 = "00000000-0000-0000-0000-000000000000";
        let head = make_head_tags(&[make_test_tag(&["maju-channel", uuid1])]);
        let result = apply_update_tags(
            head,
            None,
            false,
        ...
        let channels: Vec<_> = result
            .iter()
            .filter(|t| tag_name(t) == Some("maju-channel"))
            .collect();
    ...
    fn project_head_at(created_at: u64) -> Event {
        ...
        let tags = vec![
            make_test_tag(&["d", "platform"]),
            make_test_tag(&["a", &format!("30617:{OWNER_HEX}:maju")]),
        ];
    ...
    async fn empty_update_returns_usage_error_before_any_network_call() {
        ...
        let client = crate::client::MajuClient::new("http://127.0.0.1:9".into(), keys, None, None)
    ...
    fn discard_client() -> crate::client::MajuClient {
        let keys = nostr::Keys::generate();
        crate::client::MajuClient::new("http://127.0.0.1:9".into(), keys, None, None)
            .expect("client construction")
    }
    ...
    // (step 7: maju already present → exit=5). No relay mock is available
