...
//!
//! `maju gifs search` / `maju gifs share` hit the relay-relative endpoints
...
const REQUIRED_EXTENSION: &str = "maju-gif";
...
pub(crate) fn parse_gif_descriptor_info(
    ...
    // Gate 1: `supported_extensions` must contain `"maju-gif"`.
    let extensions = info
    ...
    let gif = info.get("gif").ok_or_else(|| {
        CliError::Other("relay advertises maju-gif but has no \"gif\" descriptor".to_string())
    })?;
...
/// - the relay does not advertise `maju-gif`
...
/// Normalized GIF entry emitted by `maju gifs search`.
///
/// `cdn_url` is the URL to embed directly in a `maju messages send --content`
...
/// `maju gifs search [--query <q>] [--locale <l>]`
///
...
/// `maju messages send --content` argument.
...
pub async fn cmd_share(client: &MajuClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::GifsCmd, client: &MajuClient) -> Result<(), CliError> {
    match cmd {
...
mod tests {
    ...
    fn alt_nip11() -> &'static str {
        ...
        r#"{"supported_extensions":["maju-gif"],"gif":{"provider":"klipy","search":"/x/search-alt","share":"/x/share-alt"}}"#
    }
    ...
    fn test_client_with_tag(base_url: &str) -> (MajuClient, String) {
        let keys = Keys::generate();
