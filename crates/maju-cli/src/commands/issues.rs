...
use maju_sdk::{GitIssueMeta, GitRepoCoord, GitStatusMeta};
use nostr::Timestamp;
...
pub async fn cmd_create_issue(
    ...
    let builder = with_git_provenance(
        maju_sdk::build_git_issue(&repo, subject, &body, &meta).map_err(sdk_err)?,
    )?;
    ...
    match (owner, id) {
        ...
        (None, None) => {
            ...
            let Some(channel) = channel else {
                return Err(CliError::Usage(
                    "provide --repo-owner and --repo-id, or --channel (or set MAJU_GIT_ORIGIN_CHANNEL_ID)".into(),
                ));
            };
...
async fn publish_issue_assignment_operation(
    ...
    let builder = match (operation, is_self_service) {
        (IssueAssignmentOperation::Assign, true) => {
            maju_sdk::build_git_issue_assignment_with_prior(
                &repo,
                issue,
                assignees,
                &content,
        ...
        (IssueAssignmentOperation::Unassign, true) => {
            maju_sdk::build_git_issue_unassignment_with_prior(
                &repo,
                issue,
                assignees,
                &content,
        ...
        (IssueAssignmentOperation::Assign, false) => {
            maju_sdk::build_git_issue_assignment(&repo, issue, assignees, &content)
        }
        (IssueAssignmentOperation::Unassign, false) => {
            maju_sdk::build_git_issue_unassignment(&repo, issue, assignees, &content)
        }
    }
...
pub async fn cmd_get_issue(client: &MajuClient, event: &str) -> Result<(), CliError> {
...
pub async fn cmd_issue_status(
    ...
    // Mirrors `maju patches status`: default a `p` tag to the repo owner
    ...
    let builder =
        with_git_provenance(maju_sdk::build_git_status(status, &body, &meta).map_err(sdk_err)?)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
    Ok(())
...
pub async fn dispatch(cmd: crate::IssuesCmd, client: &MajuClient) -> Result<(), CliError> {
    use crate::IssuesCmd;
    match cmd {
        IssuesCmd::Create {
            repo_owner,
