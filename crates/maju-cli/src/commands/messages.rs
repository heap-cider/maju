use maju_sdk::{DeleteMessageOptions, DiffMeta, ThreadRef, VoteDirection};
use nostr::PublicKey;
use uuid::Uuid;

use crate::client::{normalize_events, normalize_write_response, MajuClient};
use crate::error::CliError;
use crate::validate::{
    infer_language, parse_event_id, parse_uuid, read_or_stdin, truncate_diff,
    validate_content_size, validate_hex64, validate_uuid, MAX_DIFF_BYTES,
};
use maju_sdk::mentions::{
...
///
/// Delegates marker parsing and collapse to [`maju_core::nip10`] (shared with
...
fn find_root_from_tags(tags: &serde_json::Value) -> Option<String> {
    let parts: Vec<Vec<String>> = tags
        ...
        .collect();
    maju_core::nip10::parse_thread_markers_from_parts(parts.iter().map(Vec::as_slice))
        .resolve()
        .map(|(root, _)| root)
}
...
async fn fetch_event(client: &MajuClient, event_id: &str) -> Result<serde_json::Value, CliError> {
...
async fn resolve_channel_id(client: &MajuClient, event_id: &str) -> Result<Uuid, CliError> {
...
pub fn resolve_thread_target(
    ...
    if expected_root_id.is_some_and(|expected| expected != root_event_id) {
        return Err(CliError::Usage(
            "Maju message link thread root does not match the selected message".into(),
        ));
    }
    Ok(root_event_id)
}
...
async fn resolve_author(client: &MajuClient, author: &str) -> Result<String, CliError> {
...
pub async fn cmd_send_message(
    ...
    if !missing.is_empty() {
        return Err(CliError::Usage(
            serde_json::json!({
                ...
                "add_member_command": format!("maju channels add-member --channel {} --pubkey <pubkey> --role <member|bot>", p.channel_id),
            })
            .to_string(),
        ));
    }
    ...
    let builder = match p.kind {
        Some(45001) => {
            maju_sdk::build_forum_post(channel_uuid, &final_content, &mention_refs, &media_tags)
...
pub async fn cmd_send_diff_message(client: &MajuClient, p: SendDiffParams) -> Result<(), CliError> {
    ...
    let builder =
        maju_sdk::build_diff_message(channel_uuid, &diff, &diff_meta, thread_ref.as_ref())
...
pub async fn cmd_delete_message(
    ...
    let builder = maju_sdk::build_delete_message_with_options(
        channel_uuid,
        target_eid,
...
pub async fn cmd_edit_message(
    ...
    let builder = maju_sdk::build_edit(channel_uuid, target_eid, content)
...
pub async fn cmd_vote_on_post(
    ...
    let builder = maju_sdk::build_vote(channel_uuid, target_eid, vote_dir)
...
mod tests {
    use super::{
        ...
        thread_ref_from_parent_tags, CliError, MajuClient, Uuid,
    };
    use maju_sdk::mentions::{
    ...
    async fn malformed_channel_is_rejected_before_thread_fetch() {
        let client =
            MajuClient::new("http://127.0.0.1:1".into(), Keys::generate(), None, None).unwrap();
    ...
    async fn cmd_send_message_attaches_emoji_tags_for_known_shortcodes() {
        ...
        let client = MajuClient::new(url, Keys::generate(), None, None).unwrap();
