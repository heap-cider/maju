use maju_core::kind::KIND_IA_ARCHIVED_LIST;
use maju_sdk::builders::{build_archive_identity_request, build_unarchive_identity_request};
use nostr::PublicKey;
use serde_json::json;
...
pub async fn dispatch(command: AgentsCmd, client: &MajuClient) -> Result<(), CliError> {
    match command {
        AgentsCmd::DraftCreate {
            ...
            if let Some(obj) = output.as_object_mut() {
                ...
                obj.insert(
                    "message".into(),
                    "Draft sent to Maju Desktop for owner review. Nothing changes until the owner saves it."
                        .into(),
                );
            }
            println!("{output}");
...
/// Require `MAJU_AUTH_TAG` and parse the owner pubkey from it. Used only by
/// the `draft-create` and `draft-update` paths.
fn require_owner(client: &MajuClient) -> Result<PublicKey, CliError> {
    let hex = client
        .auth_tag_owner_hex()
        .ok_or_else(|| CliError::Auth("agent draft requests require MAJU_AUTH_TAG".into()))?;
...
/// failure cases without a live `MajuClient` or async runtime.
fn extract_auth(
...
pub(crate) async fn fetch_archived_snapshot(client: &MajuClient) -> Result<Vec<String>, CliError> {
...
/// `maju agents archived`: read path over [`fetch_archived_snapshot`] for
/// direct invocation — a trust failure (state 3) is fatal here so a
/// verification command can never look like success.
async fn cmd_archived(client: &MajuClient) -> Result<(), CliError> {
...
mod tests {
    ...
    fn test_client(base_url: &str) -> crate::client::MajuClient {
        let keys = nostr::Keys::generate();
        crate::client::MajuClient::new(base_url.to_string(), keys, None, None).unwrap()
    }
    ...
    fn archive_admin_flag_is_parsed() {
        use crate::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["maju", "agents", "archive", &hex64('a'), "--admin"])
    ...
    fn unarchive_admin_flag_is_parsed() {
        use crate::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["maju", "agents", "unarchive", &hex64('a'), "--admin"])
