//! Desktop-local channel template loading for `maju channels create --template`.
//!
...
//! a crate, since maju-cli and desktop-tauri are independent crates and the
...
const PROD_BUNDLE_IDENTIFIER: &str = "xyz.block.maju.app";
...
/// `<platform-data-dir>/xyz.block.maju.app/templates/channel-templates.json`.
...
fn load_templates(path: &Path) -> Result<Vec<ChannelTemplateRecord>, CliError> {
    if !path.exists() {
        return Err(CliError::NotFound(format!(
            "no channel templates store found at {} (create a template in Maju Desktop first, \
...
mod tests {
    ...
    fn resolve_templates_path_defaults_to_prod_bundle() {
        ...
        assert!(path.ends_with("xyz.block.maju.app/templates/channel-templates.json"));
    }

    #[test]
    fn find_template_matches_case_insensitive() {
        let f = write_store(r#"[{"id":"t1","name":"Maju Team","createdAt":"x","updatedAt":"x"}]"#);
        let t = find_template(f.path(), "maju team").expect("found");
        assert_eq!(t.name, "Maju Team");
    ...
    fn load_templates_parses_full_roster() {
        let f = write_store(
            r##"[{
                "id":"t1","name":"Maju Team","channel_type":"forum","visibility":"private",
