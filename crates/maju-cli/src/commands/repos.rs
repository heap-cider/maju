...
fn protection_pattern(tag: &Tag) -> Option<&str> {
    let values = tag.as_slice();
    (values.first().map(String::as_str) == Some("maju-protect"))
        .then(|| values.get(1).map(String::as_str))
        .flatten()
}
...
fn build_protection_tag(
    ...
    let mut values = vec!["maju-protect".to_string(), ref_pattern.to_string()];
...
enum RepoChange {
    ...
    /// `maju-channel` tag with exactly one carrying the validated UUID.
    BindChannel(String),
}
...
fn build_updated_repo_announcement(
    ...
    let (removed_pattern, removed_channel, replacement) = match change {
        ...
        RepoChange::BindChannel(channel) => {
            ...
            let tag = Tag::parse(["maju-channel", channel.as_str()]).map_err(tag_error)?;
            (None, true, Some(tag))
        }
    };
    ...
    let mut tags: Vec<Tag> = existing
        ...
        .filter(|tag| {
            ...
            if removed_channel && has_tag_name(tag, "maju-channel") {
                return false;
            }
    ...
    maju_sdk::build_repo_announcement_with_tags(repo_id, &existing.content, tags)
...
fn protection_rules_json(event: &Event) -> Result<serde_json::Value, CliError> {
    ...
    let protections: Vec<serde_json::Value> = event
        ...
        .filter_map(|tag| {
            let values = tag.as_slice();
            (values.first().map(String::as_str) == Some("maju-protect")).then(|| {
                serde_json::json!({
...
async fn submit_repo_update(client: &MajuClient, builder: EventBuilder) -> Result<(), CliError> {
...
/// `maju-channel` binding when requested.
///
/// Pure (no I/O) so the emitted tags are unit-testable. Exactly one
/// validated `maju-channel` tag is appended — the tag is the git ACL
...
fn build_create_announcement(
    ...
    let mut builder = maju_sdk::build_repo_announcement(
        repo_id,
        name,
        description,
        &clone_refs,
    ...
    if let Some(channel) = channel {
        crate::validate::validate_uuid(channel)?;
        builder = builder.tag(Tag::parse(["maju-channel", channel]).map_err(tag_error)?);
    }
    Ok(builder)
}
...
pub async fn cmd_create_repo(
    ...
    // `link` renders as a rich preview card in Maju Desktop when included in
...
async fn current_repo(client: &MajuClient, repo_id: &str) -> Result<Event, CliError> {
    validate_repo_id(repo_id)?;
...
async fn cmd_protect_list(client: &MajuClient, repo_id: &str) -> Result<(), CliError> {
...
/// the caller's own kind:30617 with exactly one `maju-channel` tag; all
...
/// latency.
async fn cmd_bind_repo(client: &MajuClient, repo_id: &str, channel: &str) -> Result<(), CliError> {
    let event = current_repo(client, repo_id).await?;
    let builder =
...
pub async fn dispatch(cmd: crate::ReposCmd, client: &MajuClient) -> Result<(), CliError> {
...
mod tests {
    ...
    fn protection_update_preserves_metadata_and_replaces_only_matching_pattern() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["name", "Demo"]),
                tag(&["maju-channel", "channel-id"]),
                tag(&["future-metadata", "preserve-me"]),
                tag(&["auth", &"a".repeat(64), "kind=30617", &"b".repeat(128)]),
                tag(&["maju-protect", "refs/heads/main", "push:member"]),
                tag(&["maju-protect", "refs/tags/*", "no-delete"]),
            ],
            "repository content",
            100,
        );
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["maju-channel", "channel-id"]));
        assert!(updated
            .tags
            .iter()
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| { tag.as_slice() == ["maju-protect", "refs/tags/*", "no-delete"] }));
        assert_eq!(
            updated
                .tags
                .iter()
    ...
    fn protection_remove_preserves_other_patterns() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["maju-protect", "refs/heads/main", "no-delete"]),
                tag(&["maju-protect", "refs/heads/release", "push:owner"]),
            ],
            "",
            10,
        );
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| { tag.as_slice() == ["maju-protect", "refs/heads/release", "push:owner"] }));
    }
    ...
    fn protection_update_rejects_malformed_existing_rules() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["maju-protect", "refs/heads/main"]),
            ],
            "",
            10,
        );
    ...
    fn bind_channel_replaces_duplicates_and_preserves_everything_else() {
        ...
        let existing = signed_repo(
            vec![
                ...
                tag(&["maju-channel", "old-and-broken"]),
                tag(&["maju-channel", &uuid::Uuid::new_v4().to_string()]),
                tag(&["auth", &"a".repeat(64), "kind=30617", &"b".repeat(128)]),
                tag(&["maju-protect", "refs/heads/main", "push:admin"]),
                tag(&["future-metadata", "preserve-me"]),
            ],
            "repository content",
            100,
        ...
        let bindings: Vec<_> = updated
            .tags
            .iter()
            .filter(|tag| tag.as_slice().first().map(String::as_str) == Some("maju-channel"))
            .collect();
        assert_eq!(bindings.len(), 1);
        assert_eq!(bindings[0].as_slice(), ["maju-channel", channel.as_str()]);
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["maju-protect", "refs/heads/main", "push:admin"]));
        assert!(updated
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["future-metadata", "preserve-me"]));
    ...
    fn bind_channel_adds_binding_to_unbound_repo() {
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["maju-channel", channel.as_str()]));
    }
    ...
    /// `maju-channel` tag so the primary create command stops producing
    ...
    fn create_with_channel_emits_exactly_one_binding_tag() {
        ...
        let bindings: Vec<_> = event
            ...
            .filter(|tag| tag.as_slice().first().map(String::as_str) == Some("maju-channel"))
            .collect();
        assert_eq!(bindings.len(), 1, "exactly one maju-channel tag");
        assert_eq!(bindings[0].as_slice(), ["maju-channel", channel.as_str()]);
    ...
    fn create_without_channel_emits_no_binding_tag() {
        ...
        assert!(
            !event
                .tags
                .iter()
                .any(|tag| tag.as_slice().first().map(String::as_str) == Some("maju-channel")),
