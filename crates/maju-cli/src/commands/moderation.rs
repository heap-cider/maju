//! `maju moderation` — community moderation queue, enforcement, and audit.
//!
...
async fn cmd_ban(
    ...
    let builder = maju_sdk::build_moderation_ban(pubkey, expiry, reason)
...
async fn cmd_unban(client: &MajuClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
    let builder = maju_sdk::build_moderation_unban(pubkey)
...
async fn cmd_timeout(
    ...
    let builder = maju_sdk::build_moderation_timeout(pubkey, expiry, reason)
...
async fn cmd_untimeout(client: &MajuClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
    let builder = maju_sdk::build_moderation_untimeout(pubkey)
...
async fn cmd_resolve(
    ...
    let builder = maju_sdk::build_moderation_resolve_report(report, status, action, reason)
...
async fn cmd_restricted(client: &MajuClient) -> Result<(), CliError> {
    let resp = client.get_authed("/moderation/restricted").await?;
    println!("{resp}");
    Ok(())
}

async fn cmd_audit(client: &MajuClient, limit: i64) -> Result<(), CliError> {
    let resp = client
