...
const CUSTOM_EMOJI_SET_D_TAG: &str = maju_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// - Shortcode is canonicalized via `maju_sdk::normalize_custom_emoji_shortcode`
...
fn emoji_tags_of(event: &serde_json::Value) -> Vec<EmojiEntry> {
    ...
    for tag in tags {
        ...
        let shortcode = match maju_sdk::normalize_custom_emoji_shortcode(raw_shortcode) {
            Ok(s) => s,
            Err(_) => continue,
        };
...
/// own kind:30030 set (d=`maju:custom-emoji`).
async fn cmd_list(client: &MajuClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [maju_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &MajuClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &MajuClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = maju_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &MajuClient, shortcode: &str, url: &str) -> Result<(), CliError> {
...
async fn cmd_rm(client: &MajuClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = maju_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &MajuClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
...
mod tests {
    ...
    // a real `MajuClient` against an axum fake `/query` server.  They verify
    ...
    fn test_client(base_url: &str) -> MajuClient {
        MajuClient::new(base_url.to_string(), Keys::generate(), None, None).unwrap()
    }
