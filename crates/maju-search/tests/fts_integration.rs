...
//!
//! Run with a local PG: `MAJU_TEST_DATABASE_URL=postgres://maju:maju_dev@localhost:5432/maju cargo test -p maju-search --tests -- --include-ignored`
//!
...
use maju_search::{ChannelScope, SearchQuery, SearchService};
...
async fn setup() -> (PgPool, String) {
    let url = std::env::var("MAJU_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string());
...
async fn teardown(pool: PgPool, schema: &str) {
    ...
    let admin_pool = PgPoolOptions::new()
        .max_connections(1)
        .connect(
            &std::env::var("MAJU_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string()),
        )
        .await
...
async fn search_finds_event_in_same_community() {
    ...
    insert_event(
        ...
        pk,
        9,
        "hello wonderland — maju everyone",
        None,
        1700000000,
    )
    .await;
...
async fn kind0_search_by_display_name_works_without_flattening() {
    ...
    insert_event(
        ...
        pk,
        0,
        r#"{"display_name":"Alice Wonderland","name":"alice","nip05":"alice@maju.app","about":"hello"}"#,
        None,
        1700000000,
    )
    .await;
    ...
    let svc = SearchService::new(pool.clone());
    for q in ["wonderland", "alice", "alice@maju.app"] {
        let r = svc
            .search(&SearchQuery {
                community: c,
                q: q.to_string(),
...
async fn prefix_mode_matches_final_token_prefix_without_changing_full_text() {
    ...
    let prefix = svc
        .search(&SearchQuery {
            ...
            page: 1,
            per_page: 10,
            mode: maju_search::SearchMode::Prefix,
        })
        .await
...
async fn p_gated_persistent_kinds_have_storage_null_tsvector() {
    ...
    let persistent: Vec<u32> = P_GATED_KINDS
        .iter()
        .copied()
        .filter(|&k| !maju_core::kind::is_ephemeral(k))
        .collect();
    assert!(
        !persistent.is_empty(),
