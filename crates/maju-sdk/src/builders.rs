...
///
/// Shortcodes are case-insensitive in Maju's relay-global set; lowercase
...
/// Build a Maju-native delete event (kind 9005).
...
/// Build a Maju-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "maju:custom-emoji";
...
/// "maju:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(maju_core::channel::canonical_channel_name)
...
/// Maximum byte length of a project `maju-channel` tag value.
pub const PROJECT_CHANNEL_MAX: usize = 256;
/// Maximum byte length of a project `maju-visibility` tag value.
...
///
/// Rules enforced (matches relay `maju-db` ingest logic):
...
/// 7. Singleton metadata: each of `name`, `description`, `maju-channel`,
///    `maju-visibility` appears at most once.
/// 8. Metadata byte lengths: `name` ≤256, `description` ≤2048,
///    `maju-channel` ≤256, `maju-visibility` ≤256.
...
pub fn build_project(
    ...
    if let Some(ch) = channel {
        uuid::Uuid::parse_str(ch).map_err(|_| {
            SdkError::InvalidInput(format!("maju-channel must be a valid UUID (got {ch:?})"))
        })?;
    }
    ...
    if let Some(vis) = visibility {
        if vis != "listed" && vis != "unlisted" {
            return Err(SdkError::InvalidInput(format!(
                "maju-visibility must be 'listed' or 'unlisted' (got {vis:?})"
            )));
        }
    }
    ...
    if let Some(ch) = channel {
        tags.push(tag(&["maju-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(tag(&["maju-visibility", vis])?);
    }
...
mod tests {
    ...
    fn agent_observer_frame_happy_path() {
        ...
        let recipient = keys();
        let agent = keys();
        let encrypted = maju_core::observer::encrypt_observer_payload(
            &sender,
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["maju-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "maju-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | maju patches send
    ...
    fn build_project_emitted_envelope_has_correct_shape() {
        ...
        // name, description, maju-channel, maju-visibility present.
        ...
        let ch_tags: Vec<_> = all_tags.iter().filter(|t| t[0] == "maju-channel").collect();
        assert_eq!(ch_tags.len(), 1);
        assert_eq!(ch_tags[0][1], VALID_UUID);

        let vis_tags: Vec<_> = all_tags
            .iter()
            .filter(|t| t[0] == "maju-visibility")
            .collect();
        ...
        assert_eq!(a_tags[0][1], format!("30617:{OWNER64}:maju"));
    }
