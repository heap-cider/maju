...
fn database_metrics_and_slow_logs_exclude_sensitive_or_unbounded_fields() {
    ...
    let datastore_macro = include_str!("../../maju-datastore-tracing/src/lib.rs");
...
fn p0_pool_acquisitions_use_typed_operation_pairs_without_other() {
    ...
    assert!(!observability.contains("maju_db_pool_acquire_timeouts_total"));
    ...
    let side_effects = include_str!("../../maju-relay/src/handlers/side_effects.rs");
    ...
    let ingest = include_str!("../../maju-relay/src/handlers/ingest.rs");
    assert!(ingest.contains(".huddle_started_link_exists_for_event_write("));
    let audio = include_str!("../../maju-relay/src/audio/handler.rs");
    assert!(audio.contains(".huddle_started_link_exists("));

    let workflow_sink = include_str!("../../maju-relay/src/workflow_sink.rs");
    assert!(workflow_sink.contains(".get_members_for_event_write("));
    assert!(workflow_sink.contains(".get_users_bulk_for_event_write("));

    for write_caller in [
        include_str!("../../maju-relay/src/handlers/side_effects.rs"),
        include_str!("../../maju-relay/src/handlers/ingest.rs"),
        include_str!("../../maju-relay/src/handlers/command_executor.rs"),
        workflow_sink,
    ] {
    ...
    let relay_main = include_str!("../../maju-relay/src/main.rs");
    ...
    let relay_api = include_str!("../../maju-relay/src/api/mod.rs");
