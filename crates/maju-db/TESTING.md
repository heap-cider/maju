# PostgreSQL-backed tests in maju-db
...
`MAJU_TEST_SCHEMA_MODE=migration`.

Helpers that normally run migrations honor `MAJU_TEST_SCHEMA_MODE=desired` in
...
Set `MAJU_POSTGRES_ADMIN_URL` to a PostgreSQL maintenance database owned by a
