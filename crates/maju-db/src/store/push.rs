...
const PUSH_GATE_LOCK_NAMESPACE: &str = "maju_push_gate:";
...
pub struct BatchedMatch {
    ...
    pub event: maju_core::StoredEvent,
    /// Attempt number, starting at one for the first claim.
    pub attempt: i32,
}
...
where
    ...
    Fut: std::future::Future<Output = Result<Vec<maju_core::StoredEvent>>>,
{
...
mod postgres_tests {
    ...
    async fn setup_pool() -> PgPool {
        let database_url = std::env::var("MAJU_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://maju:maju_dev@localhost:5432/maju".into()); // sadscan:disable np.postgres.1 -- local test-only credentials
