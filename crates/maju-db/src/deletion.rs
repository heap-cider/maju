...
/// the instant it is checked. The value is arbitrary (ASCII `majudel1`) but
...
impl DeletionStore {
    ...
    ///
    /// Probe the deployed catalog rather than SQLx's migration ledger. Maju also
    ...
    pub async fn guard_transaction_with_serving_lease(
        ...
        sqlx::query(
            "SELECT set_config('maju.serving_write_community', $1, true), \
                    set_config('maju.serving_write_lease_id', $2, true), \
                    set_config('maju.serving_write_owner', $3, true), \
                    set_config('maju.serving_write_generation', $4, true), \
                    set_config('maju.serving_write_fence_generation', $5, true)",
        )
...
async fn set_executor_gucs(
    ...
    sqlx::query(
        "SELECT set_config('maju.deletion_executor_community', $1, true), \
                set_config('maju.deletion_fence_generation', $2, true)",
    )
...
mod postgres_tests {
    ...
    async fn preclaim_setup_failure_is_durable_without_a_lease() {
        ...
        assert_eq!(
            ...
            Some("MAJU_S3_ENDPOINT is required")
        );
        assert_eq!(
    ...
    async fn destructive_stages_serialize_with_migrations_and_fail_closed_on_new_scoped_tables() {
        ...
        let probe_db = format!("maju_lock_probe_{}", Uuid::new_v4().simple());
    ...
    async fn desired_state_schema_bootstrap_progresses_beyond_fencing() {
        ...
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("maju_desired_state_{}", Uuid::new_v4().simple());
        sqlx::query(AssertSqlSafe(format!("CREATE DATABASE {probe_db}")))
            .execute(&admin)
            .await
            .expect("create probe database");
