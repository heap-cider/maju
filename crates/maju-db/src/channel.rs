...
// Re-export the canonical enum definitions from maju-core.
// These live in core (zero I/O deps) so the SDK can share them
// without pulling in sqlx/tokio.
pub use maju_core::channel::{ChannelType, ChannelVisibility, MemberRole};
...
/// `maju_channel_ttl:`.
const CHANNEL_MEMBERSHIP_LOCK_NAMESPACE: &str = "maju_channel_membership:";
...
impl LockedMemberSnapshot {
    ...
    pub async fn replace_member_event(
        ...
        channel_id: Uuid,
        event: &nostr::Event,
    ) -> Result<(maju_core::StoredEvent, bool)> {
        ...
        let kind = maju_core::kind::event_kind_i32(event);
        if kind != 39002 {
            return Err(DbError::InvalidData(
        ...
        if let Some((existing_ts, existing_id)) = existing {
            ...
            {
                return Ok((
                    maju_core::StoredEvent::with_received_at(
                        event.clone(),
                        Utc::now(),
                        Some(channel_id),
                        false,
...
pub async fn update_channel(
    ...
    if let Some(name) = updates.name.as_mut() {
        *name = maju_core::channel::canonical_channel_name(name).to_owned();
        if name.is_empty() {
            return Err(DbError::InvalidData("channel name is required".into()));
        }
    }
