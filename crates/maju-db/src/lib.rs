#![deny(unsafe_code)]
#![warn(missing_docs)]
//! maju-db — Postgres event store for Maju.
//!
//! ## Design invariants
...
pub mod partition;
/// Maju product-feedback sidecar persistence.
...
use maju_core::{CommunityId, StoredEvent};
            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `maju_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("maju_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
enum RouteDecision {
    ...
    /// failure emits exactly one `maju_db_route_decision` event per request
...
impl RoutePredicate {
    ...
    ///
    /// `routing_enabled` is whether `MAJU_REPLICA_READ_MAX_AGE_MS` is set
...
/// Map the configured read budget (`MAJU_REPLICA_READ_MAX_AGE_MS`) to the
...
pub struct DbConfig {
    ...
    /// `MAJU_DB_READ_POOL_SIZE`). `None` inherits [`Self::max_connections`].
    ...
    /// `MAJU_REPLICA_READ_MAX_AGE_MS`). `0` disables bounded-staleness
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://maju:maju_dev@localhost:5432/maju".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// `maju.created_at_floor` GUC — this is what makes the replica fence
        ...
        let options = PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('maju.created_at_floor', $1, false)")
    ...
    /// the migration decision. On a relay with `MAJU_AUTO_MIGRATE` off, the
    ...
    > {
        ...
        .await
        {
            ...
            // `maju_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `maju_db_read_pool_active` / `_max`
            ...
            // sample (MAJU_POOL_METRICS_INTERVAL_SECS, default 10s) while
    ...
    /// the writer's: `maju_db_read_pool_active / maju_db_read_pool_max` is
    /// the operator's utilisation signal for tuning `MAJU_DB_READ_POOL_SIZE`,
    ...
    /// request/connection error. maju-db only reads the durable host map.
    ...
    /// fully-formed [`maju_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `maju-relay`:
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if kind_u32 == maju_core::kind::KIND_AUTH {
            return Err(DbError::AuthEventRejected);
        }
        if maju_core::kind::is_ephemeral(kind_u32) {
    ...
    /// seam is gated on `MAJU_REPLICA_READ_MAX_AGE_MS` (default off): when
    ...
    ///   `MAJU_REPLICA_HEAD_MAX_AGE_SECS` until it has, proven by a
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = maju_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![maju_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = maju_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    /// ordering watermark. Maju mesh status coordinates also hard-delete their
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == maju_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
        ...
        let is_maju_mesh_status = kind_i32 == maju_core::kind::KIND_BOOKMARK_SET as i32
            && d_tag.starts_with("maju-mesh-member-status:")
            && event.tags.iter().any(|tag| {
                let parts = tag.as_slice();
                parts.len() == 2 && parts[0] == "k" && parts[1] == "maju-mesh-status"
            });
        let hard_delete_superseded = is_nip_rs || is_maju_mesh_status;
        ...
        if existing.is_some() {
            if is_nip_rs {
                ...
                sqlx::query("SELECT set_config('maju.nip_rs_hard_delete', 'on', true)")
                    .execute(&mut *tx)
                    .await?;
            }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://maju:maju_dev@localhost:5432/maju";
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign new");
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let d_tag = "maju-mesh-member-status:owner-test";
        let tags = vec![
            Tag::parse(["d", d_tag]).expect("d tag"),
            Tag::parse(["k", "maju-mesh-status"]).expect("k tag"),
        ];
        let base = Timestamp::now().as_secs();
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(maju_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let kind = maju_core::kind::KIND_PROJECT as i32;
        let d_tag = "stale-tombstone-project";
        let pubkey = keys.public_key().to_bytes().to_vec();
        let base = Timestamp::now().as_secs();

        let version = |content: &str, offset: u64| {
            EventBuilder::new(Kind::Custom(maju_core::kind::KIND_PROJECT as u16), content)
    ...
    async fn duplicate_nip_rs_discriminator_tags_keep_legacy_retention() {
        ...
        for (case, tags) in [
            ...
            let old = EventBuilder::new(
                Kind::Custom(maju_core::kind::KIND_READ_STATE as u16),
                format!("{case}-old"),
            )
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('maju.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
            ...
            let leaked: Option<String> = sqlx::query_scalar(
                "SELECT NULLIF(current_setting('maju.nip_rs_hard_delete', true), '')",
            )
            .fetch_one(&mut *conn)
            .await
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(maju_core::kind::KIND_READ_STATE as i32)
    ...
    async fn test_usage_metrics_lock_has_single_owner_and_releases_on_drop() {
        ...
        // races any live maju-relay on the same database (see #3619).
        ...
        // Same key as production (`maju-relay` USAGE_METRICS_LOCK_KEY) — safe here
    ...
    async fn create_community_with_owner_enforces_per_owner_limit() {
        ...
        // Fill this owner's community allowance to the configured default.
    ...
    /// `routing_enabled = false` (MAJU_REPLICA_READ_MAX_AGE_MS unset)
    ...
    /// ceiling, not the writer's — `maju_db_read_pool_active / _max` is the
    ...
    /// directly, because `maju_db_route_decision` is emitted by `route_read`
    ...
    async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
        ...
        let reasons: std::collections::HashMap<(String, String), u64> = snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "maju_db_route_decision")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("maju_db_route_decision must be a counter");
                };
    ...
    async fn lazy_reader_pool_still_spawns_fence_probe() {
        ...
        // `maju.created_at_floor` GUC — `spawn_fence_probe` verifies the
    ...
    async fn created_at_floor_guard_aborts_old_channel_rows_at_commit() {
        ...
        let insert_raw = |ev: nostr::Event, channel_id: Option<Uuid>| {
            ...
            async move {
                ...
                sqlx::query("SELECT set_config('maju.created_at_floor', $1, true)")
    ...
    fn writer_pool_safety_hook_is_single_and_composed() {
        ...
        assert!(connect_pool.contains("maju.created_at_floor"));
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let effective: String = sqlx::query_scalar("SHOW maju.created_at_floor")
            .fetch_one(&db.pool)
            .await
    ...
    /// `MAJU_AUTO_MIGRATE` off: an armed GUC with no enforcing trigger must
    ...
    async fn fence_probe_refuses_to_start_without_verified_floor_guard() {
        ...
        // Sabotage B: trigger dropped entirely (the MAJU_AUTO_MIGRATE=off /
