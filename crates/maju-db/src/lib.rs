...
//! This crate intentionally keeps database runtime and Maju domain persistence
...
mod tests {
    ...
    async fn parameterized_replacement_in_existing_transaction_honors_revision_and_rollback() {
        ...
        .bind(community.as_uuid())
        .bind(maju_core::kind::KIND_PROJECT as i32)
        .bind(keys.public_key().to_bytes())
        .bind(&d_tag)
        .fetch_one(&db.pool)
        .await
        ...
        let missing = EventBuilder::new(
            Kind::Custom(maju_core::kind::KIND_PROJECT as u16),
            "missing",
        )
        .tags(vec![
    ...
    async fn parameterized_replacement_rolls_back_when_mention_indexing_fails() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_PROJECT as u16), "old")
            .tags(tags())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old project");
        let new = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_PROJECT as u16), "new")
            .tags(tags())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
