...
const TEST_DB_URL: &str = "postgres://maju:maju_dev@localhost:5432/maju"; // sadscan:disable np.postgres.1 -- local test-only credentials
...
async fn setup_db() -> Db {
    ...
    if std::env::var("MAJU_TEST_SCHEMA_MODE").as_deref() == Ok("migration") {
...
async fn begin_transaction_compatibility_alias_is_preserved() {
    ...
    let counters = snapshotter
        ...
        .filter_map(|(key, _, _, value)| {
            ...
            if ![
                ...
                "maju_db_pool_acquisitions_total",
            ]
            .contains(&name)
            {
                return None;
    ...
    let expected = [
        (
            (
                "maju_db_pool_acquire_attempts_total".to_owned(),
                [
        ...
        (
            (
                "maju_db_pool_acquisitions_total".to_owned(),
                [
                    ("outcome".to_owned(), "error".to_owned()),
                    ("pool_role".to_owned(), "writer".to_owned()),
                ]
...
async fn push_gateway_profile_migration_converges_brownfield_authority() {
    ...
    sqlx::query(
        "INSERT INTO push_gateway_installations(\
         ...
         VALUES($1, $2, $3, 0, 'maju-ios-production', $4, $5, 1, $6)",
    )
    ...
    let sandbox = sqlx::query(
        "INSERT INTO push_gateway_installations(\
         id, app_attest_key_id, app_attest_public_key, assertion_counter, app_profile, \
         token_ciphertext, token_fingerprint, endpoint_epoch, expires_at) \
         VALUES($1, $2, $3, 0, 'maju-ios-sandbox', $4, $5, 1, $6)",
    )
    .bind(Uuid::new_v4())
    .bind(vec![10_u8])
    .bind(vec![11_u8; 33])
...
fn session_timeout_env_overlay_zero_passthrough_and_invalid_fallback() {
    ...
    let keys = [
        "MAJU_DB_LOCK_TIMEOUT_MS",
        "MAJU_DB_IDLE_TXN_TIMEOUT_MS",
        "MAJU_DB_STATEMENT_TIMEOUT_MS",
    ];
    ...
    std::env::set_var("MAJU_DB_LOCK_TIMEOUT_MS", "2000");
    std::env::set_var("MAJU_DB_IDLE_TXN_TIMEOUT_MS", "30000");
    std::env::set_var("MAJU_DB_STATEMENT_TIMEOUT_MS", "10000");
