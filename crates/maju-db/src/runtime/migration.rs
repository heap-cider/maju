//! Embedded SQLx migrations for Maju.
//!
...
/// Run all pending Maju database migrations.
///
...
mod postgres_tests {
    ...
    /// URL. Only the shapes this test emits (`MAJU_TEST_DATABASE_URL` /
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(mesh_retention.contains("maju-mesh-member-status:%"));
        assert!(mesh_retention.contains("maju-mesh-status"));
        assert!(mesh_retention
            .contains("CREATE TRIGGER trg_events_purge_soft_deleted_maju_mesh_status"));
        assert!(!migrations[0]
            .sql
            .as_str()
            .contains("purge_soft_deleted_maju_mesh_status"));
        ...
        assert!(push_gate.contains("'maju_push_gate:' || NEW.community_id::text"));
        ...
        assert!(ttl_shared.contains("'maju_channel_ttl:' || NEW.community_id::text"));
        ...
        assert!(roster_fence.contains("'maju_channel_membership:'"));
        ...
        // OSS Maju is stateless for identity (spec v2, PR #7214); the durable
    ...
    fn migration_execution_cannot_bypass_schema_destruction_lock() {
        ...
        // exemption honest.
        let push_gateway_exception = crates_dir.join("maju-push-gateway/src/postgres.rs");
        let push_gateway_migrations = crates_dir.join("maju-push-gateway/migrations");
        for entry in
            std::fs::read_dir(&push_gateway_migrations).expect("read push gateway migrations")
        {
            ...
            assert!(
                ...
                "{} defines community-scoped data; its migrator would bypass the \
                 schema/destruction lock and must move under maju-db migrations",
                path.display()
            );
        }
        let mut files = Vec::new();
            ...
            if *path == this_file {
                ...
                assert_eq!(
                    ...
                    "{} embeds or runs a SQLx migrator outside the schema/destruction \
                     ...
                     maju_db migration::run_migrations",
                    path.display()
                );
            }
        }
    ...
    async fn cancelled_migration_cannot_expose_shared_lock_while_ddl_backend_lives() {
        ...
        let base_url = std::env::var("MAJU_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_owned());
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("maju_lock_cancel_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn admin_schema_parity_between_desired_state_and_migrations() {
        ...
        let desired_db = format!("maju_admin_desired_{}", uuid::Uuid::new_v4().simple());
        let migrated_db = format!("maju_admin_migrated_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn run_migrations_applies_consolidated_initial_schema_on_fresh_database() {
        ...
        sqlx::query(
            "SELECT set_config('maju.deletion_executor_community', $1, true), \
                    set_config('maju.deletion_fence_generation', '1', true)",
        )
