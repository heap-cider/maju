...
pub struct DbConfig {
    ...
    /// `MAJU_DB_LOCK_TIMEOUT_MS`). `0` disables the timeout.
    pub lock_timeout_ms: u64,
    /// Session `idle_in_transaction_session_timeout` in milliseconds for
    /// writer connections (env `MAJU_DB_IDLE_TXN_TIMEOUT_MS`). `0` disables.
    pub idle_txn_timeout_ms: u64,
    /// Session `statement_timeout` in milliseconds for writer connections
    /// (env `MAJU_DB_STATEMENT_TIMEOUT_MS`). `0` disables it and is the
...
impl DbConfig {
    /// Overlay writer session timeouts from the shared `MAJU_DB_*_TIMEOUT_MS`
    ...
    ///
    /// This belongs in `maju-db` so relay, admin, deletion, and audit writers
    ...
    pub fn with_session_timeouts_from_env(mut self) -> Self {
        ...
        if let Some(value) = parse("MAJU_DB_LOCK_TIMEOUT_MS") {
            self.lock_timeout_ms = value;
        }
        if let Some(value) = parse("MAJU_DB_IDLE_TXN_TIMEOUT_MS") {
            self.idle_txn_timeout_ms = value;
        }
        if let Some(value) = parse("MAJU_DB_STATEMENT_TIMEOUT_MS") {
