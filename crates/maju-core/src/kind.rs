//! Maju V2 kind number registry.
//!
//! This module is the authoritative source for Maju kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Maju today.
...
/// Maju custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Maju command kinds (Pure Nostr plan)
...
/// `crates/maju-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `maju-db`.
///
...
/// Maju product feedback submission. Accepted at ingest, sidecarred to the
...
// Maju community moderation commands (mod-signed, processed like 9030-series:
...
/// Maju: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — Maju requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Maju workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Maju command kind that requires transactional execution.
...
/// Safe: all Maju kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Maju kind constants fit in nostr's u16-backed Kind.
