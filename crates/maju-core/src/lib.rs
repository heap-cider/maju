#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `maju-core` — zero-I/O foundation types for the Maju relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other Maju crates depend on this one.
...
pub mod invite;
/// Maju kind number registry — custom event type constants.
pub mod kind;
