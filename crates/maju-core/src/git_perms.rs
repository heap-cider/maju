...
//!
//! This module defines the core data types for the Maju git permission system.
//! The permission model: channel role = repo role; `maju-protect` tags on
...
/// announcement with no `maju-channel` binding.
///
...
/// Maximum number of `maju-protect` tags per repo.
...
/// A single protection rule parsed from a `maju-protect` tag on kind:30617.
///
/// Format: `["maju-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `maju-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "maju-protect tag needs pattern + at least one rule"),
...
/// Parse a single `maju-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["maju-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("maju-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `maju-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `maju-protect` tag, returning unknown rules for logging.
...
/// Parse all `maju-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `maju-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("maju-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `maju-protect` tag matches.
