...
/// Subscribes to `maju:*:conn-control` and forwards scoped commands to the
...
mod tests {
    ...
    fn conn_control_channel_is_community_scoped() {
        ...
        assert_eq!(
            ...
            format!("maju:{}:conn-control", a.community())
        );
    ...
    fn parse_rejects_foreign_channels() {
        assert_eq!(
            parse_conn_control_channel("maju:not-a-uuid:conn-control"),
            None
        );
        assert_eq!(parse_conn_control_channel("maju:*:cache-invalidate"), None);
        let a = ctx(0x1234, "a.example");
