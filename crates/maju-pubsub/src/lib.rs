#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `maju-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE maju:{community}:channel:{id} / maju:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: maju_core::CommunityId,
...
/// Central pub/sub manager for a Maju relay instance.
...
impl PubSubManager {
    ...
    /// `maju:{community}:channel:{id}` / `maju:{community}:global`, and
