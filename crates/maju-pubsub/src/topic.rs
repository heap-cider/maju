...
/// Redis key prefix for Maju-scoped pub/sub topics and keys.
pub const MAJU_PREFIX: &str = "maju";
    ...
    pub fn redis_channel(&self) -> String {
        match self.topic {
            EventTopic::Channel(channel_id) => {
                format!("{MAJU_PREFIX}:{}:channel:{channel_id}", self.community_id)
            }
            EventTopic::Global => format!("{MAJU_PREFIX}:{}:global", self.community_id),
        }
    }
...
mod tests {
    ...
    fn channel_key_includes_community_and_channel() {
        ...
        assert_eq!(
            ...
            format!("maju:{}:channel:{channel_id}", ctx.community())
        );
    }
    ...
    fn global_key_includes_community() {
        ...
        assert_eq!(global_key(&ctx), format!("maju:{}:global", ctx.community()));
    }
    ...
    fn parses_channel_topic() {
        ...
        let channel_id = Uuid::from_u128(0xbbbb);
        let raw = format!("maju:{community_id}:channel:{channel_id}");
    ...
    fn parses_global_topic() {
        let community_id = CommunityId::from_uuid(Uuid::from_u128(0xaaaa));
        let raw = format!("maju:{community_id}:global");
    ...
    fn rejects_malformed_or_wrong_prefix_topics() {
        for raw in [
            "",
            "not-maju:00000000-0000-0000-0000-00000000aaaa:global",
            "maju:not-a-uuid:global",
            "maju:00000000-0000-0000-0000-00000000aaaa",
            "maju:00000000-0000-0000-0000-00000000aaaa:global:extra",
            "maju:00000000-0000-0000-0000-00000000aaaa:channel",
            "maju:00000000-0000-0000-0000-00000000aaaa:channel:not-a-uuid",
            "maju:00000000-0000-0000-0000-00000000aaaa:presence:abc",
        ] {
            assert!(
