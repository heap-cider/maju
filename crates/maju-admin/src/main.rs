...
//! Maju instance administration CLI.
//!
...
use maju_core::kind::KIND_NIP43_MEMBERSHIP_LIST;
use maju_core::tenant::{relay_url_authority, TenantContext};
use maju_db::{Db, DbConfig};
use maju_pubsub::{EventTopic, PubSubManager};
use nostr::{EventBuilder, Keys, Kind, Tag};
use tracing::warn;

#[derive(Parser)]
#[command(name = "maju-admin", about = "Maju instance administration")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Migrate,
    /// Inspect deployment-wide Maju product feedback.
    ProductFeedback {
        #[command(subcommand)]
    ...
    ReconcileChannels {
        ...
        /// MAJU_RELAY_PRIVATE_KEY env var. If neither is set, generates
...
async fn main() {
    ...
    // Redis TLS connection without this. Mirrors maju-relay's main().
...
async fn run(cli: Cli) -> Result<i32> {
    match cli.command {
        Command::GenerateKey => {
            ...
            println!("\nSet MAJU_PRIVATE_KEY to the secret key to use this identity.");
            Ok(0)
        }
        Command::Migrate => {
...
async fn cmd_remove_member(pubkey_arg: String, role_filter: Option<String>) -> Result<i32> {
    ...
    let tenant = resolve_admin_tenant(&db).await?;
    use maju_db::relay_members::RemoveResult;
...
async fn publish_membership_list_with_bump(
    ...
    tracing::info!(
        ...
        ts,
        "NIP-43 membership list published by maju-admin"
    );
    Ok(())
}
...
///
/// `MAJU_RELAY_PRIVATE_KEY` is required — the CLI signs kind:13534 events.
async fn connect_member_services() -> Result<(Db, Arc<PubSubManager>, Keys)> {
    let db = connect_db().await?;

    let relay_keypair = {
        let hex = std::env::var("MAJU_RELAY_PRIVATE_KEY").map_err(|_| {
            anyhow::anyhow!(
                "MAJU_RELAY_PRIVATE_KEY is required for add-member/remove-member.\n\
                 The relay must have a stable signing key to publish kind:13534 events."
            )
        })?;
        Keys::parse(&hex).map_err(|e| anyhow::anyhow!("invalid MAJU_RELAY_PRIVATE_KEY: {e}"))?
    };
...
///
/// `maju-admin` runs inside the relay container (`compose exec relay
/// maju-admin …`), so it shares the relay's `RELAY_URL` and resolves the same
...
async fn resolve_admin_tenant(db: &Db) -> Result<TenantContext> {
    ...
    // resolution do (`maju_core::tenant::relay_url_authority`): host plus an
    ...
    // the helper keeps maju-admin byte-identical to the community startup seeds.
    ...
    let record = db.lookup_community_by_host(&host).await?.ok_or_else(|| {
        anyhow::anyhow!(
            "RELAY_URL host '{host}' is not mapped to a community.\n\
             maju-admin operates on the configured relay's community; ensure the \
...
async fn reconcile_channels(
    ...
    use maju_core::kind::KIND_NIP29_GROUP_ADMINS;
    ...
    let configured_relay_key =
        relay_key_arg.or_else(|| std::env::var("MAJU_RELAY_PRIVATE_KEY").ok());
    if channel_arg.is_some() && configured_relay_key.is_none() {
        return Err(anyhow::anyhow!(
            "--channel requires --relay-key or MAJU_RELAY_PRIVATE_KEY"
        ));
    }
    let relay_keys = match configured_relay_key {
        ...
        None => {
            ...
            eprintln!("Pass --relay-key or set MAJU_RELAY_PRIVATE_KEY for production use.");
            k
        }
    };
