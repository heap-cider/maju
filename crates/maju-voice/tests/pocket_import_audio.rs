...
fn evidence_dir() -> PathBuf {
    std::env::var_os("MAJU_VOICE_EVIDENCE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/maju-voice-evidence")
        })
}
...
#[test]
#[ignore = "requires MAJU_POCKET_MODEL_DIR and runs the installed Pocket ONNX model"]
fn objective_import_synthesis_delete_and_mary_fallback() {
    let model_dir = required_path("MAJU_POCKET_MODEL_DIR");
