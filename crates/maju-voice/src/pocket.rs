//! April 2026 Pocket TTS engine for Maju Desktop.
//!
...
//! Maju selects the upstream three-graph INT8 variant while retaining the
...
/// sessions via `MAJU_TTS_THREADS`. Default preserves production's 1.
fn tts_num_threads() -> usize {
    std::env::var("MAJU_TTS_THREADS")
        .ok()
...
/// Load Maju Desktop's pinned April INT8 model.
...
mod tests {
    ...
    #[test]
    #[ignore = "requires MAJU_POCKET_TEST_MODEL_DIR"]
    fn production_api_emits_non_silent_april_int8_pcm() {
        let dir = std::env::var("MAJU_POCKET_TEST_MODEL_DIR")
            .expect("set MAJU_POCKET_TEST_MODEL_DIR to an April INT8 model directory");
