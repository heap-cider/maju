...
impl PocketVoiceLibrary {
    ...
    pub fn load(&self) -> Result<Vec<ImportedVoice>, String> {
        ...
        if registry.version > REGISTRY_VERSION {
            return Err(format!(
                "imported voice registry version {} is newer than this Maju build supports",
                registry.version
            ));
        }
        Ok(registry
...
mod tests {
    ...
    #[test]
    #[ignore = "requires MAJU_VOICE_IMPORT_TEST_DIR with common-format fixtures"]
    fn imports_common_audio_format_fixtures() {
        let fixtures =
            PathBuf::from(std::env::var("MAJU_VOICE_IMPORT_TEST_DIR").expect("fixture directory"));
        let temp = tempfile::tempdir().expect("temp voice workspace");
        let library = PocketVoiceLibrary::new(temp.path().join("library"));
