//! `maju-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! maju-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! maju-pair target [--relay wss://relay.example.com]
...
use maju_core::kind::KIND_PAIRING;
use maju_core::pairing::session::PairingSession;
use maju_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(maju_core::pairing::types::AbortReason::SasMismatch)?
        {
