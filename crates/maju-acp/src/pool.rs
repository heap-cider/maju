...
use crate::config::{
    compose_scoped_session_title, DedupMode, PermissionMode, ACP_CONFIG_OPTIONS_ENV,
};
use crate::observer;
...
pub struct ChannelDeliveryState {
    ...
    /// Maju event IDs already delivered to this ACP session, either as trigger
...
pub struct PromptContext {
    /// Native ACP choices captured from the harness environment at startup.
    /// Shared by every worker; model-specific validation happens after switching.
    pub acp_config_options: BTreeMap<String, serde_json::Value>,
    ...
    /// `--no-memory` / `MAJU_ACP_NO_MEMORY`.
    pub memory_enabled: bool,
    /// Harness identity string for NIP-AM `harness` field. Derived from the
    /// configured `agent_command` at startup (e.g. `"goose"`, `"maju-agent"`).
...
async fn create_session_and_apply_model(
    ...
    // Apply the configured model against the fresh session/new response. A
    // successful adapter response becomes the only downstream session snapshot,
    // so model-dependent options never fall back to the pre-switch catalog.
    let mut session_config_raw = resp.raw.clone();
    let switch_succeeded = if let Some(ref desired) = agent.desired_model {
        // A busy-path switch gets its terminal success only after this deferred
        // apply completes. Consume the flag for every terminal outcome so it
        // cannot fire again on a later session.
            ...
            Some(method) => {
                match apply_model_switch(&mut agent.acp, &resp.session_id, desired, &method).await?
                {
                    ModelSwitchOutcome::Applied(switch_result) => {
                        ...
                        session_config_raw = switch_result;
                        if pending_ack {
                            agent.acp.observe(
                                "control_result",
                                serde_json::json!({
    ...
    // Apply the single persisted MAJU_ACP_CONFIG_OPTIONS map after model
    // selection, against the option catalog for the model that is actually
    // running. The process environment is immutable, so edits made while this
    // worker is alive take effect only after its required restart.
    apply_configured_session_options(
        &mut agent.acp,
        &resp.session_id,
        &mut session_config_raw,
        &ctx.acp_config_options,
    )
    .await?;

    // Cache the effective post-switch, post-option snapshot. This is one view of
    // the running session, not separate configured and effective effort values.
    agent.acp.observe(
...
fn mcp_servers_with_git_origin(
    ...
    let origin = match (channel_id, channel_type) {
        (Some(channel_id), Some("stream")) => Some(EnvVar {
            name: "MAJU_GIT_ORIGIN_CHANNEL_ID".into(),
            value: channel_id.to_string(),
        }),
        (Some(_), _) => agent_name
            .filter(|name| !name.trim().is_empty())
            .map(|name| EnvVar {
                name: "MAJU_GIT_ORIGIN_AGENT_NAME".into(),
...
/// Read the single native-option map before constructing the shared prompt context.
pub(crate) fn configured_session_options() -> BTreeMap<String, serde_json::Value> {
...
async fn apply_configured_session_options(
    ...
    configured_options: &BTreeMap<String, serde_json::Value>,
) -> Result<(), AcpError> {
    for (config_id, value) in configured_options {
        ...
        let result = tokio::time::timeout(
            ...
            acp.session_set_config_option_value(session_id, config_id, value.clone()),
        )
        .await;
        match result {
            Ok(Ok(update)) => {
                merge_session_config_update(session_config, &update);
                patch_config_option_current_value(session_config, config_id, value);
            }
            Ok(Err(
                error @ (AcpError::Io(_)
...
/// Update the captured effective value after an adapter accepts a persisted
/// option. Some adapters return only `{ "ok": true }` instead of a refreshed
/// configOptions array, so merging the RPC response alone would leave the
/// Desktop cache showing the pre-apply default.
...
/// default permission mode (`"default"`), which still works through per-tool
/// auto-approval in `handle_permission_request`.
///
...
pub async fn run_prompt_task(
    ...
    //
    // Operator opt-out: `--no-memory` / `MAJU_ACP_NO_MEMORY` skips the fetch.
            ...
            if let Some(sid) = agent.state.sessions.get(scope) {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        // Seed a zero usage baseline: maju-acp spawned this session
    ...
    // (`prompt[0].text.startsWith("/")`) fires; the wrapped Maju context
                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(maju_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(maju_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(maju_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
async fn fetch_channel_info_once(channel_id: Uuid, rest: &RestClient) -> Option<PromptChannelInfo> {
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            maju_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
pub(crate) async fn fetch_project_home_for_channel(
    ...
    let filters = [
        serde_json::json!({
            "kinds": [maju_core::kind::KIND_PROJECT],
            "#maju-channel": [channel],
        }),
        serde_json::json!({
            "kinds": [maju_core::kind::KIND_GIT_REPO_ANNOUNCEMENT],
            "#maju-channel": [channel],
        }),
    ];
    ...
    let (projects, repos): (Vec<_>, Vec<_>) = events.into_iter().partition(|event| {
        event.get("kind").and_then(serde_json::Value::as_u64)
            == Some(maju_core::kind::KIND_PROJECT as u64)
    });
...
async fn fetch_huddle_instructions(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            maju_core::kind::KIND_HUDDLE_GUIDELINES as u16,
        ))
        .author(*owner)
...
fn huddle_instructions_from_query_response(
    ...
    if event.pubkey != *owner
        || event.kind.as_u16() as u32 != maju_core::kind::KIND_HUDDLE_GUIDELINES
        || !event
            .tags
            .iter()
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let h_tag = SingleLetterTag::lowercase(Alphabet::H);
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(maju_core::kind::KIND_CANVAS as u16))
        .custom_tags(h_tag, [channel_id.to_string()])
        .limit(1);
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(maju_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
    ...
    crate::prompt_framing::semantic_section(
        ...
        &format!(
            "Canvas revision (event ID): {event_id}\n\
             ...
             Fetch current content with: maju canvas get --channel {channel_uuid}"
        ),
    )
}
...
/// IDs are compared in Maju's canonical 64-character lowercase hex form: relay
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(maju_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(maju_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn delivery_receipt_line(channel_id: Uuid, event_ids: &HashSet<String>) -> String {
    ...
    format!(
        "turn delivered Maju events for channel {channel_id}: {}",
        event_ids.join(",")
    )
}
...
fn acp_stop_to_core(r: &StopReason) -> maju_core::agent_turn_metric::StopReason {
    use maju_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<maju_core::agent_turn_metric::TokenCounts>,
    Option<maju_core::agent_turn_metric::TokenCounts>,
) {
    use maju_core::agent_turn_metric::TokenCounts;
    ...
    let cumulative_counts = Some(TokenCounts {
        ...
        // reported this field (e.g. goose or older maju-agent sessions).
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<maju_core::agent_turn_metric::StopReason>,
) {
    use maju_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match maju_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(maju_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `maju_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match maju_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(maju_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder = match maju_sdk::build_message(
        channel_id,
        content,
        thread_ref.as_ref(),
        &[],
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match maju_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn delivery_receipt_line_sorts_event_ids() {
        ...
        assert_eq!(
            ...
            format!("turn delivered Maju events for channel {channel_id}: alpha,beta")
        );
    }
    ...
    fn public_session_forwards_channel_origin_to_mcp() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "MAJU_GIT_ORIGIN_CHANNEL_ID" && entry.value == channel_id.to_string()
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "MAJU_GIT_ORIGIN_AGENT_NAME"));
    }
    ...
    fn private_session_forwards_agent_name_without_channel_id() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "MAJU_GIT_ORIGIN_AGENT_NAME" && entry.value == "Builder"
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "MAJU_GIT_ORIGIN_CHANNEL_ID"));
    }
    ...
    fn goose_uses_system_prompt_only_after_custom_method_succeeds() {
        ...
        assert!(has_system_prompt_support(2, "maju-agent", None));
        ...
        assert_eq!(
            session_new_system_prompt(false, 2, "maju-agent", Some("instructions")),
    ...
    fn test_workspace_section_preserves_windows_cwd() {
        assert_eq!(
            workspace_section(r"C:\Users\me\maju"),
            "<workspace>\nCurrent working directory: C:\\Users\\me\\maju\n</workspace>"
        );
    }
    ...
    async fn run_prompt_task_commits_standing_context_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "maju-acp-standing-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn channel_prompt_commits_delivery_state_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "maju-acp-channel-delivery-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn merged_cancel_prompt_commits_and_deduplicates_all_rendered_event_ids() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "maju-acp-merged-delivery-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  count=$((count + 1))
    ...
    async fn late_successful_steer_ack_excludes_event_from_next_channel_wire_prompt() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "maju-acp-late-steer-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    /// A maju-agent-commanded context must not panic — verifies the harness
    /// field flows through encrypt/sign without error.
    #[tokio::test]
    async fn test_publish_agent_turn_metric_maju_agent_harness_name() {
        let agent_keys = nostr::Keys::generate();
        let owner_keys = nostr::Keys::generate();
        let mut ctx = make_prompt_context_with_owner(&agent_keys, owner_keys.public_key());
        ctx.harness_name = "maju-agent".to_string();
        let usage = crate::usage::TurnUsage {
            session_id: "sess-ba".to_string(),
            turn_seq: 1,
            delta_reliable: false, // first turn from maju-agent
    ...
    fn test_build_turn_metric_counts_cache_read_tokens_thread_through() {
        // Wire-parse a maju-agent payload with cache, run it through the tracker,
    ...
    fn make_prompt_context_impl(
        ...
        PromptContext {
            acp_config_options: BTreeMap::new(),
            mcp_servers: vec![],
    ...
    fn huddle_instructions_require_owner_signature_and_channel() {
        ...
        let event = |keys: &Keys, channel_id: Uuid| {
            ...
            serde_json::to_value(
                EventBuilder::new(
                    Kind::Custom(maju_core::kind::KIND_HUDDLE_GUIDELINES as u16),
                    "reply immediately",
                )
                .tags([h_tag])
                .sign_with_keys(keys)
    ...
    fn test_render_canvas_section_produces_exact_shape() {
        ...
        assert_eq!(
            ...
            "<channel-canvas>\n\
             ...
             Fetch current content with: maju canvas get --channel 00f1ccaf-1506-4dd7-9a0e-fa67e9e486ae\n\
             </channel-canvas>"
        );
    }
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(maju_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_happy_path() {
        ...
        assert!(section.contains("maju canvas get --channel"));
    ...
    fn test_canvas_section_from_query_response_string_timestamp_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(Kind::Custom(maju_core::kind::KIND_CANVAS as u16), "content")
                .tags([h_tag])
                .sign_with_keys(&keys)
                .expect("sign"),
        )
    ...
    fn test_canvas_section_from_query_response_tampered_event_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(
                Kind::Custom(maju_core::kind::KIND_CANVAS as u16),
                "original",
            )
            .tags([h_tag])
            .sign_with_keys(&keys)
    ...
    async fn expired_absence_refreshes_to_project_without_restart() {
        ...
        let responses = [
            json!([{
                "kind": 30621,
                "pubkey": owner,
                "tags": [["d", "app"], ["maju-channel", channel], ["a", coordinate]]
            }]),
            json!([{
                "kind": 30617,
                "pubkey": "a".repeat(64),
                "tags": [["d", "app"], ["maju-channel", id.to_string()]]
            }]),
        ];
    ...
    async fn indeterminate_project_context_never_reaches_acp_prompt_boundary() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "maju-acp-indeterminate-project-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
  ...
  printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'
    ...
    async fn resolve_finds_authoritative_project_beyond_first_bridge_page() {
        ...
        let first_page: Vec<_> = (0..500)
            .map(|index| {
                json!({
                    ...
                    "kind": 30621,
                    "pubkey": "b".repeat(64),
                    "tags": [["d", format!("decoy-{index}")], ["maju-channel", channel]]
                })
            })
            .collect();
        let responses = [
        ...
        let server = tokio::spawn(async move {
            while let Ok((mut socket, _)) = listener.accept().await {
                ...
                if index > 0 {
                    assert!(request.contains("#maju-channel"));
                }
                let body = responses[index].to_string();
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{}",
    ...
    async fn test_new_session_channel_context_qualifies_a_normal_channel() {
        ...
        let id = Uuid::new_v4();
        let response = channel_metadata_response(id, &[["name", "maju-dev"], ["t", "stream"]]);
        ...
        assert_eq!(title_channel.as_deref(), Some("maju-dev"));
        ...
        assert_eq!(again.as_deref(), Some("maju-dev"));
        assert_eq!(
            requests.load(Ordering::SeqCst),
            4,
...
mod startup_effort_tests {
    ...
    /// (id 1). Tests supply native choices through the production prompt context.
    fn effort_agent(acp: AcpClient) -> OwnedAgent {
        OwnedAgent {
            index: 0,
            acp,
            state: SessionState::default(),
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn test_applied_effort_patches_captured_current_value_to_high() {
        ...
        let mut agent = effort_agent(acp);
...
exit 0"#
    ...
    fn test_patch_config_option_current_value_matches_by_id_key() {
        ...
        let mut opts = serde_json::json!({"configOptions": [
            { "id": "effort", "category": "thought_level", "currentValue": "low" }
        ]});
        patch_config_option_current_value(&mut opts, "effort", &json!("high"));
        assert_eq!(opts["configOptions"][0]["currentValue"], "high");
    }
...
#[cfg(test)]
mod native_config_option_tests {
    use super::*;
    use serde_json::json;
    #[test]
    ...
    #[test]
    fn accepted_config_option_patches_the_single_effective_snapshot() {
        ...
        patch_config_option_current_value(&mut session, "reasoningEffort", &json!("high"));

        assert_eq!(session["configOptions"][0]["currentValue"], "high");
    }
...
mod model_switch_tests {
    ...
    /// A protocol-v2 agent with a live `desired_model` override.
    /// `model_overridden` is set so the capture's `modelOverridden`
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn test_applied_switch_refreshes_capabilities_from_post_switch_snapshot() {
        ...
        assert!(
            caps.config_options_raw
                .iter()
                .any(|option| option["category"] == "model" && option["currentValue"] == "model-b"),
            "the cached model selector must reflect the target model after switching"
        );
        let cap = capture(&obs);
        assert_eq!(
