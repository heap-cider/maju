//! Event queue state machine for maju-acp.
//!
...
/// to [`maju_core::nip10`] so ACP anchoring reads ancestry exactly as relay
...
/// checking whether the **first** prompt content block starts with `/`. Maju
...
pub fn extract_slash_command(content: &str, known_names: &[&str]) -> Option<String> {
    ...
    loop {
        if rest.starts_with("nostr:npub1") || rest.starts_with("nostr:nprofile1") {
            ...
            // token of the characters Maju allows in plain @mentions.
            let name_len = names
                .iter()
                .find_map(|name| {
...
fn append_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: For ordinary replies in this turn, use `--reply-to {event_id}` \
         on `maju messages send` so the conversation stays threaded. \
...
fn append_new_thread_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: This is a new top-level message. For ordinary replies in \
         this turn, use `--reply-to {event_id}` on `maju messages send` — the \
...
fn append_project_home(s: &mut String, channel_info: Option<&PromptChannelInfo>, channel_id: Uuid) {
    ...
    s.push_str(&format!(
        "\nThis channel is that project's home. Tasks, repositories, and files created here belong to this project. Do not run `maju projects create`. Create a repository with `maju repos create --id <id> --name \"…\" --channel {channel_id}`. Create tasks with `maju issues create --channel {channel_id} --subject \"…\" --content \"…\"`."
    ));
}
...
fn format_context_hints(
    ...
    if is_dm {
        ...
        let ctx_hint = if complete_conversation_context && is_reply {
            ...
            "Thread context included below. Use `maju messages thread --channel <UUID> --event <ID>` for full history if truncated."
        } else if has_conversation_context {
            "Conversation context included below. Use `maju messages get --channel <UUID>` for full history if truncated."
        } else if conversation_context_had_delivered_events && is_reply {
            "Earlier thread context was already delivered in this session. Use `maju messages thread --channel <UUID> --event <ID>` to re-read the reply chain."
        } else if conversation_context_had_delivered_events {
            "Earlier conversation context was already delivered in this session. Use `maju messages get --channel <UUID>` to re-read it."
        } else if is_reply {
            "Use `maju messages thread --channel <UUID> --event <ID>` to fetch the reply chain."
        } else {
            "Use `maju messages get --channel <UUID>` for conversation context."
        };
        let mut s = format!(
            "Scope: dm\n\
        ...
        .or(thread_tags.root_event_id.as_deref())
    {
        let ctx_hint = if complete_conversation_context {
            ...
            "Earlier thread context was already delivered in this session. Use `maju messages thread --channel <UUID> --event <ID>` to re-read it."
        } else {
            "Use `maju messages thread --channel <UUID> --event <ID>` to fetch thread context."
        };
        ...
        append_project_home(&mut s, channel_info, channel_id);
        s.push_str(
            "\nHint: Use `maju messages get --channel <UUID>` for recent messages if needed.",
        );
        if let Some(event_id) = reply_anchor {
            append_new_thread_reply_instruction(&mut s, event_id);
        }
...
/// 3. `<maju-event>` / `<maju-events>` — the triggering event(s)
///
...
mod tests {
    ...
    fn test_format_prompt_single() {
        ...
        assert!(prompt.contains("<maju-event type=\"@mention\">\n"));
    ...
    fn test_format_prompt_batch() {
        ...
        assert!(prompt.contains("<context>"));
        assert!(prompt.contains("<maju-events count=\"3\">"));
                ...
                for (event, is_reply) in [(&top, false), (&reply, true)] {
                    ...
                    for modern in [false, true] {
                        ...
                        if is_dm {
                            ...
                            assert!(prompt.contains("maju messages thread"));
                            assert!(!prompt.contains("maju messages get"));
                        } else {
    ...
    fn test_thread_context_retrieval_hint_only_when_needed() {
        ...
        assert!(!complete_prompt.contains("maju messages thread"));
        ...
        .join("\n\n");
        assert!(prompt_with_prior_delivery.contains("maju messages thread"));
        ...
        assert!(truncated_prompt.contains("maju messages thread"));
        ...
        assert!(missing_root_prompt.contains("maju messages thread"));
    }
    ...
    fn test_thread_context_retrieval_hint_requires_batch_coverage() {
        ...
        assert!(mixed_prompt.contains("maju messages thread"));
        ...
        assert!(!same_thread_prompt.contains("maju messages thread"));
    }
    ...
    fn test_format_prompt_dm_non_reply_hints_get_messages() {
        ...
        assert!(prompt.contains("Scope: dm"));
        assert!(
            prompt.contains("maju messages get"),
            "DM non-reply hint should mention `maju messages get`"
        );
        assert!(
            !prompt.contains("maju messages thread"),
            "DM non-reply should NOT mention `maju messages thread`"
        );
    }
    ...
    fn test_reply_instruction_allows_explicit_root_post_requests() {
        ...
        assert!(
            !prompt.contains("on EVERY `maju messages send` call"),
    ...
    fn test_append_project_home_names_the_project_and_blocks_duplicates() {
        ...
        assert!(
            s.contains("do not run `maju projects create`")
                || s.contains("Do not run `maju projects create`")
        );
        assert!(s.contains("maju issues create --channel 11111111-1111-4111-8111-111111111111"));
        assert_eq!(
            s.lines()
