...
//! model, or credentials), it spawns `maju-acp` with a setup-mode payload
...
///
/// maju-acp is a separate crate and must NOT depend on desktop types —
...
pub(crate) const SETUP_PAYLOAD_ENV_VAR: &str = "MAJU_ACP_SETUP_PAYLOAD";
...
/// (`managed_agents/readiness.rs`). Desktop serializes it; `maju-acp`
...
impl SetupPayload {
    ...
    /// 2. A fenced `maju:config-nudge` sentinel block containing the structured
    ...
    fn nudge_body(&self) -> String {
        let prose = if self.requirements.is_empty() {
            ...
            let footer = if has_doctor_requirement {
                ...
                // Mixed: some Maju-managed fields, some external config.
                "Open Edit Agent in the Maju app for the Maju-managed fields; fix the external CLI config files manually and restart the agent.".to_string()
            } else {
                // All Maju-managed — original footer unchanged.
                "Open Edit Agent in the Maju app to set these.".to_string()
            };
...
pub(crate) async fn run_setup_listener(config: Config, payload: SetupPayload) -> Result<()> {
    tracing::info!(
        ...
        "maju-acp entering setup mode"
    );

    let pubkey_hex = config.keys.public_key().to_hex();

    // Parse MAJU_AUTH_TAG for relay membership / NIP-OA.
    ...
    loop {
        let Some(maju_event) = relay.next_event().await else {
        ...
        {
            handle_setup_membership(&mut relay, &maju_event, &config, &rules, &channel_ids).await;
            continue;
        }
...
async fn nudge_authorized_event(
    ...
    let (maju_event, effective_author) = authorized_event.into_parts();

    // Apply channel/kind filter rules.
    let filter_matched =
        filter::match_event(&maju_event.event, maju_event.channel_id, rules, pubkey_hex)
            .await
            .is_some();

    if !should_nudge_for_event(maju_event.event.id, filter_matched, nudged_event_ids) {
        return false;
    }
...
async fn handle_setup_membership(
    ...
    maju_event: &crate::relay::MajuEvent,
    config: &Config,
    rules: &[filter::SubscriptionRule],
...
async fn publish_setup_nudge(
    ...
    let event_builder = maju_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    fn setup_payload_deserializes_git_bash_requirement() {
        let payload: SetupPayload = serde_json::from_str(
            r#"{"agent_name":"Maju Agent","agent_pubkey":"test","requirements":[{"surface":"git_bash"}]}"#,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn nudge_body_git_bash_copy_points_to_agent_runtimes() {
        let payload = SetupPayload {
            agent_name: "Maju Agent".to_string(),
            agent_pubkey: "test".to_string(),
    ...
    fn nudge_body_mixed_requirements_uses_split_footer() {
        // Mixed list: one Maju-managed env key + one external config invalid.
    ...
    #[test]
    fn nudge_body_all_maju_managed_retains_original_footer() {
        // Pure Maju-managed requirements → original "Open Edit Agent" footer unchanged.
        let payload = SetupPayload {
            agent_name: "Fizz".to_string(),
            agent_pubkey: "test".to_string(),
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("Open Edit Agent in the Maju app to set these."),
    ...
    fn nudge_body_contains_sentinel_block() {
        // The body must end with a ```maju:config-nudge fence so the desktop
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("```maju:config-nudge\n"),
    ...
    fn nudge_body_sentinel_round_trips_payload() {
        ...
        let fence_open = "```maju:config-nudge\n";
        let fence_close = "\n```";
        let start = body
            .rfind(fence_open)
    ...
    //
    // These tests prove the desktop→maju-acp→sentinel path preserves the
    // `availability` field. They simulate what actually happens at runtime:
    // desktop serializes a `cli_login` JSON blob → maju-acp parses it via
