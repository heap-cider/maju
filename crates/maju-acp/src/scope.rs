...
///
/// Selected via `--session-policy` / `MAJU_ACP_SESSION_POLICY`. Defaults to
...
impl SessionScope {
    ...
    ///
    /// Thread roots are resolved with [`parse_thread_tags`], i.e. Maju's shared
    /// [`maju_core::nip10`] canonical-root rules — a malformed marker id is
