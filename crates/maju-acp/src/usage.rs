...
//! turn.  Both goose and maju-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because maju-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — maju-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — maju-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
    pub context_limit: u64,
    /// Session-cumulative inclusive input tokens.
    ///
    /// `None` when maju-agent omitted the field — this happens when the
    ...
    /// emitted by maju-agent when every turn in the session so far supplied a
    ...
    #[serde(default)]
    pub pricing_identity: Option<maju_core::agent_turn_metric::PricingIdentity>,
}
...
pub struct TurnUsage {
    ...
    /// Session-cumulative genuine provider total tokens as reported by maju-agent;
    ...
    /// Session-cumulative cache-read input tokens as reported by maju-agent.
    ...
    /// Session-cumulative cache-write tokens as reported by maju-agent.
...
pub(crate) struct UsageTracker {
    ...
    pending_identity: Option<Option<maju_core::agent_turn_metric::PricingIdentity>>,
...
impl UsageTracker {
    ...
    /// Seed a zero baseline for a session that maju-acp just spawned.
    ///
    /// When maju-acp creates a session itself via `session/new`, the session's
...
mod tests {
    ...
    /// The camelCase key maju-agent actually puts on the wire must land on the
    ...
    fn notification_deserializes_without_used_and_context_limit() {
        // maju-agent emits usage_update without used/contextLimit.
        let raw = serde_json::json!({
            "sessionId": "maju-sess",
            "update": {
                "sessionUpdate": "usage_update",
    ...
    #[test]
    fn maju_agent_notification_flows_through_tracker() {
        // End-to-end: a maju-agent-shaped usage_update (no used/contextLimit)
    ...
    #[test]
    fn maju_agent_payload_no_context_fields_processes_correctly() {
    ...
    fn pool_omitted_cache_field_publishes_no_cache_read_tokens_in_kind44200() {
        // End-to-end: a maju-agent or goose payload that omits the cache field
    ...
    fn pool_reported_cache_field_publishes_nonzero_cache_read_tokens_in_kind44200() {
        // End-to-end: a maju-agent payload with a nonzero cache count must
    ...
    fn make_pricing_identity_payload(
        ...
        UsageUpdatePayload {
            ...
            pricing_identity: Some(maju_core::agent_turn_metric::PricingIdentity {
    ...
    /// Old harnesses (goose, older maju-agent) that do not emit `pricingIdentity`
    ...
    // The ACP tracker has a DISTINCT multi-notification path: maju-agent sends
