                    ...
                    match maju_event {
                        Some(maju_event) => {
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if maju_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(maju_event.channel_id);
                                            tracing::info!(
                                                channel_id = %maju_event.channel_id,
                                                invalidated,
                            ...
                            // it never revokes same-owner team bots.
                            {
                                ...
                                // author (the `maju:workflow-owner` tag — the
                                ...
                                let author = match workflow_attributed_author(
                                    ...
                                    Some(attributed) => {
                                        tracing::debug!(
                                            channel_id = %maju_event.channel_id,
                                            relay_author = %maju_event.event.pubkey.to_hex(),
                                            attributed_author = %attributed,
                                            "relay-signed workflow message — gating on attributed author"
                                        );
                                        attributed
                                    }
                                    None => maju_event.event.pubkey.to_hex(),
                                };
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(maju_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %maju_event.channel_id,
                                        author = %maju_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&maju_event.event, maju_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %maju_event.channel_id, kind = maju_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            // Capture author pubkey before queue.push() moves
                            // maju_event.event (needed for mode gate below).
                            let author_hex = maju_event.event.pubkey.to_hex();
                            ...
                            if accepted && queue.is_channel_in_flight(maju_event.channel_id) {
...
/// `workflow.owner_pubkey` relay-side) via the explicit `maju:workflow-owner`
...
/// 4. **exactly one** tag exactly equal to `["maju:workflow", "true"]` — no
///    duplicates, no extra fields, no other value;
/// 5. **exactly one** tag exactly equal to `["maju:workflow-owner", <pubkey>]`
...
fn workflow_attributed_author(event: &nostr::Event, relay_self: Option<&str>) -> Option<String> {
    ...
    // 4. Exactly one marker tag, exactly ["maju:workflow", "true"]. Collect
    ...
    let markers: Vec<&[String]> = event
        ...
        .iter()
        .map(|t| t.as_slice())
        .filter(|s| s.first().map(|k| k.as_str()) == Some("maju:workflow"))
        .collect();
    if markers.len() != 1 || markers[0] != ["maju:workflow", "true"] {
        return None;
    }

    // 5. Exactly one owner tag, exactly ["maju:workflow-owner", <pubkey>].
    ...
    let owners: Vec<&[String]> = event
        ...
        .iter()
        .map(|t| t.as_slice())
        .filter(|s| s.first().map(|k| k.as_str()) == Some("maju:workflow-owner"))
        .collect();
...
mod workflow_attributed_author_tests {
    ...
    fn workflow_tags(owner_hex: &str, mention_hex: &str) -> Vec<Tag> {
        vec![
            ...
            Tag::parse(["maju:workflow", "true"]).unwrap(),
            Tag::parse(["maju:workflow-owner", owner_hex]).unwrap(),
    ...
    fn relay_signed_workflow_message_attributes_to_workflow_owner_tag() {
        ...
        assert_eq!(
            ...
            Some(owner),
            "a relay-signed maju:workflow event must attribute to the \
             maju:workflow-owner tag, not any mentioned agent"
        );
    }

    #[test]
    fn attribution_ignores_p_tags_entirely() {
        // Only the explicit maju:workflow-owner tag attributes; p tags
        ...
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "without a maju:workflow-owner tag there is no attributed author, \
    ...
    fn malformed_owner_tag_value_attributes_to_no_one() {
        ...
        let event = make_event(
            ...
            vec![
                Tag::parse(["maju:workflow", "true"]).unwrap(),
                Tag::parse(["maju:workflow-owner", "not-a-pubkey"]).unwrap(),
            ],
        );
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "a maju:workflow-owner value that is not 64-hex must be rejected"
        );
    }
    ...
    fn non_relay_author_gets_no_exemption_even_with_workflow_tag() {
        // A member forging the maju:workflow tag on their own event must not
        ...
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "a maju:workflow tag on a non-relay-signed event must be ignored"
        );
    }
    ...
    fn relay_signed_message_without_workflow_tag_gets_no_exemption() {
        ...
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "relay-signed events without the maju:workflow tag keep the plain author gate"
        );
    }
    ...
    fn workflow_message_without_owner_tag_attributes_to_no_one() {
        let relay = Keys::generate();
        let event = make_event(&relay, vec![Tag::parse(["maju:workflow", "true"]).unwrap()]);
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "a workflow message with no maju:workflow-owner tag has no attributed \
    ...
    fn duplicate_marker_tags_disqualify() {
        ...
        let event = make_event(
            ...
            vec![
                Tag::parse(["maju:workflow", "true"]).unwrap(),
                Tag::parse(["maju:workflow", "true"]).unwrap(),
                Tag::parse(["maju:workflow-owner", &owner]).unwrap(),
            ],
        );
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
            None,
            "more than one maju:workflow marker tag must fail closed"
        );
    }
    ...
    fn marker_value_mismatch_disqualifies() {
        ...
        let owner = Keys::generate().public_key().to_hex();
        for bad_marker in [
            Tag::parse(["maju:workflow", "false"]).unwrap(),
            Tag::parse(["maju:workflow"]).unwrap(),
            Tag::parse(["maju:workflow", "true", "extra"]).unwrap(),
        ] {
            let event = make_event(
                &relay,
                vec![
            ...
            assert_eq!(
                workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
                None,
                "marker tag {:?} is not exactly [\"maju:workflow\", \"true\"] and must fail closed",
                bad_marker.as_slice()
            );
        }
    }
        ...
        for second_owner in [&owner, &other] {
            let event = make_event(
                ...
                vec![
                    Tag::parse(["maju:workflow", "true"]).unwrap(),
                    Tag::parse(["maju:workflow-owner", &owner]).unwrap(),
                    Tag::parse(["maju:workflow-owner", second_owner]).unwrap(),
                ],
            );
            assert_eq!(
                workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
                None,
                "duplicate maju:workflow-owner tags must fail closed"
            );
        }
    }
    ...
    fn owner_tag_with_extra_fields_disqualifies() {
        ...
        let event = make_event(
            ...
            vec![
                Tag::parse(["maju:workflow", "true"]).unwrap(),
                Tag::parse(["maju:workflow-owner", &owner, "extra"]).unwrap(),
            ],
        );
        assert_eq!(
            workflow_attributed_author(&event, Some(&relay.public_key().to_hex())),
