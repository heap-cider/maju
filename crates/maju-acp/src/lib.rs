...
/// name (e.g., `maju-acp --verbose models`) are not supported.
...
/// Timeout for `maju-acp authenticate`. Browser-based vendor auth can require
...
async fn publish_presence(
    ...
    use maju_core::kind::KIND_PRESENCE_UPDATE;
...
///
/// Priority:
/// 1. `MAJU_AUTH_TAG` env var — NIP-OA attestation signed by the owner.
///    Verified against the agent's own pubkey to extract the owner pubkey.
/// 2. `--agent-owner` CLI flag / `MAJU_ACP_AGENT_OWNER` env var.
fn resolve_agent_owner(config: &Config) -> Option<String> {
    // Try MAJU_AUTH_TAG first (NIP-OA attestation).
    if let Ok(auth_tag) = std::env::var("MAJU_AUTH_TAG") {
        if !auth_tag.is_empty() {
            let agent_pk = config.keys.public_key();
            match maju_sdk::nip_oa::verify_auth_tag(&auth_tag, &agent_pk) {
                Ok(owner_pk) => {
                    let owner_hex = owner_pk.to_hex().to_ascii_lowercase();
                    tracing::info!("owner resolved from MAJU_AUTH_TAG: {owner_hex}");
                    return Some(owner_hex);
                }
                Err(e) => {
                    tracing::warn!("MAJU_AUTH_TAG verification failed: {e} — falling back");
                }
            }
        }
    }
...
///
/// `maju:workflow-owner` alone is not authority: any ordinary event author can
...
/// must also have exactly one canonical `maju:workflow-mention` tag; legacy `p`
...
fn verified_workflow_owner(
    ...
    let markers: Vec<&[String]> = event
        ...
        .filter(|values| values.first().map(String::as_str) == Some("maju:workflow"))
        .collect();
    if markers.as_slice() != [["maju:workflow", "true"]] {
        return None;
    }
    ...
    let owners: Vec<&[String]> = event
        ...
        .iter()
        .map(|tag| tag.as_slice())
        .filter(|values| values.first().map(String::as_str) == Some("maju:workflow-owner"))
        .collect();
    ...
    let workflow_mentions: Vec<&[String]> = event
        ...
        .iter()
        .map(|tag| tag.as_slice())
        .filter(|values| values.first().map(String::as_str) == Some("maju:workflow-mention"))
        .collect();
...
mod inbound_author_gate {
    ...
    pub(crate) struct AuthorizedListenerEvent {
        maju_event: relay::MajuEvent,
        effective_author: String,
    }

    impl AuthorizedListenerEvent {
        pub(crate) fn into_parts(self) -> (relay::MajuEvent, String) {
            (self.maju_event, self.effective_author)
        }
    }
    ...
    impl InboundAuthorGate {
        ...
        pub(crate) async fn evaluate_listener_event(
            ...
            if refresh_needed(self.refreshed_generation, maju_event.connection_generation) {
                let (relay_self, completed) =
                    refresh_relay_self(rest_client, self.relay_self.take(), "listener").await;
                self.relay_self = relay_self;
                if completed {
                    self.refreshed_generation = Some(maju_event.connection_generation);
                }
            }
            let is_dm = is_dm_channel(maju_event.channel_id, channel_info).await;
        ...
        pub(crate) async fn authorize_listener_event(
            ...
            if !decision.allowed {
                tracing::debug!(
                    channel_id = %maju_event.channel_id,
                    raw_author = %maju_event.event.pubkey.to_hex(),
...
impl AuthorizedNormalListenerEvent {
    async fn match_subscription(
        ...
        let (maju_event, effective_author) = self.0.into_parts();
        let matched = filter::match_event(
            &maju_event.event,
            maju_event.channel_id,
            rules,
            agent_pubkey_hex,
        )
        .await?;
...
impl NormalListenerIngress {
    fn push(
        ...
        let event_id_hex = maju_event.event.id.to_hex();
        let event_for_steer = maju_event.event.clone();
        let prompt_tag_for_steer = prompt_tag.clone();
        let channel_id = maju_event.channel_id;
        let accepted = queue.push(QueuedEvent {
            channel_id,
            scope: session_scope.clone(),
            event: maju_event.event,
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match maju_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in maju-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing maju-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (maju-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match maju_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = maju_core::verify_event(&event) {
...
/// replay floor (`--replay-floor` / `MAJU_ACP_REPLAY_FLOOR`).
///
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("maju_acp=info")),
        )
        .compact()
        .init();
    ...
    // model, or provider), it spawns maju-acp with MAJU_ACP_SETUP_PAYLOAD set.
    ...
    {
        tracing::info!("maju-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("maju-acp starting: {}", config.summary());
    ...
    // PI_ACP_PI_COMMAND is Maju-owned. Strip stale/user-provided copies from
    // every adapter before optionally installing Maju's generated Pi launcher.
    config
        .persona_env_vars
    ...
    if let Some(prepared) = pi_launch_override.as_ref() {
        ...
        tracing::info!(
            ...
            "configured Pi to consume Maju standing context and managed skills through native CLI flags"
        );
    }
    ...
    // timestamp as a replay floor (`--replay-floor` / `MAJU_ACP_REPLAY_FLOOR`):
    ...
    // Parse MAJU_AUTH_TAG into a nostr::Tag for NIP-OA relay membership delegation.
    let relay_auth_tag: Option<nostr::Tag> = std::env::var("MAJU_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
        .and_then(|s| maju_sdk::nip_oa::parse_auth_tag(&s).ok());
    ...
    // Priority: MAJU_AUTH_TAG (NIP-OA attestation) → --agent-owner flag.
            ...
            RespondTo::OwnerOnly => {
                tracing::warn!(
                    "respond-to=owner-only but no owner is set — all events will be \
                     dropped. Set MAJU_AUTH_TAG or --agent-owner, or use --respond-to=anyone."
                );
            }
            RespondTo::Allowlist => {
                tracing::warn!(
    ...
    let runtime_start_nonce = std::env::var("MAJU_MANAGED_AGENT_START_NONCE").unwrap_or_default();
    ...
    let ctx = Arc::new(PromptContext {
        acp_config_options: pool::configured_session_options(),
    ...
    if !config.memory_enabled {
        tracing::info!(
            target: "engram::core",
            "NIP-AE core memory injection disabled (re-enable by removing --no-memory / MAJU_ACP_NO_MEMORY)"
        );
    }
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                maju_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match maju_event {
                        Some(maju_event) => {
                            let kind_u32 = maju_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = maju_event.channel_id;
                                let ts = maju_event.event.created_at.as_secs();
                                let eid = maju_event.event.id.to_hex();
                            ...
                            if config.ignore_self && maju_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %maju_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if maju_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %maju_event.channel_id,
                                            sender = %maju_event.event.pubkey.to_hex(),
                            ...
                            if is_cancel {
                                let from_owner = owner_cache.get().is_some_and(|owner| {
                                    maju_event.event.pubkey.to_hex() == *owner
                                });
                                if from_owner {
                                    ...
                                    let scope = scope::SessionScope::derive(
                                        ...
                                        maju_event.channel_id,
                                        is_dm_channel(maju_event.channel_id, &ctx.channel_info)
                                            .await,
                                        &maju_event.event,
                                    );
                            ...
                            let session_scope = scope::SessionScope::derive(
                                config.session_policy,
                                ingress.maju_event.channel_id,
                                is_dm_channel(
                                    ingress.maju_event.channel_id,
                                    &ctx.channel_info,
                                )
                                .await,
                                &ingress.maju_event.event,
                            );
                            tracing::debug!(
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Maju event: …]`
...
fn handle_prompt_result(
    ...
    // which live in maju-agent's session state and are what `llm: (model) …`
...
mod agent_draft_prompt_tests {
    ...
    fn shared_base_prompt_teaches_portable_agent_drafts() {
        ...
        assert!(prompt.contains("maju agents draft-create"));
    ...
    fn shared_base_prompt_teaches_real_newlines_for_multiline_messages() {
        ...
        assert!(prompt.contains("maju messages send ... --content -"));
    }
    ...
    fn shared_base_prompt_teaches_not_to_duplicate_projects() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.contains("do **not** run `maju projects create`"));
        assert!(prompt.contains("maju issues create --channel"));
        assert!(prompt.contains("is not a Maju repository"));
    }

    #[test]
    fn shared_base_prompt_teaches_single_command_mentions_and_preflight() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.contains("use the person's **exact display name as shown in Maju**"));
        ...
        assert!(prompt
            .contains("add them explicitly with `maju channels add-member` only when authorized"));
...
fn default_heartbeat_prompt() -> String {
    ...
    format!(
        "[System: Heartbeat]\nTime: {now}\n\n\
         ...
         Your tasks:\n\
         1. Run `maju feed get --types needs_action` to check for pending workflow approvals or\n\
            high-priority requests addressed to you.\n\
         2. Run `maju feed get --types mentions` to check for unanswered @mentions.\n\
         3. If you find actionable items, address them using the appropriate CLI commands\n\
            (e.g., `maju workflows approve --token <UUID>`, `maju messages send`,\n\
            `maju messages send --reply-to <event-id>`).\n\
         4. If there are no pending actions or mentions, end your turn immediately.\n\n\
         Do not run `maju channels list` or `maju messages search` unless you have a specific reason.\n\
...
/// `maju-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `maju-acp authenticate` — invoke one adapter-owned auth method.
...
fn build_mcp_servers(config: &Config) -> Vec<McpServer> {
    ...
    vec![McpServer {
        ...
        env: {
            let mut env = vec![
                EnvVar {
                    name: "MAJU_RELAY_URL".into(),
                    value: config.relay_url.clone(),
                },
                EnvVar {
                    name: "MAJU_PRIVATE_KEY".into(),
            ...
            // Forward MAJU_AUTH_TAG (NIP-OA owner attestation credential)
            // so the MCP server can attach it to every signed event.
            if let Ok(auth_tag) = std::env::var("MAJU_AUTH_TAG") {
                if !auth_tag.is_empty() {
                    env.push(EnvVar {
                        name: "MAJU_AUTH_TAG".into(),
                        value: auth_tag,
                    });
                }
            }
            ...
            if let Ok(display_name) = std::env::var("MAJU_ACP_DISPLAY_NAME") {
                if !display_name.is_empty() {
                    env.push(EnvVar {
                        name: "MAJU_ACP_DISPLAY_NAME".into(),
                        value: display_name,
                    });
                }
            }
...
mod workflow_owner_tests {
    ...
    fn workflow_event(
        ...
        if let Some(owner) = owner {
            tags.push(Tag::parse(["maju:workflow-owner", owner]).expect("workflow owner tag"));
        }
    ...
    fn trusted_relay_workflow_uses_owner_for_explicit_target() {
        ...
        let event = workflow_event(
            ...
            Some(&owner),
            &[&["maju:workflow", "true"]],
            &[&["maju:workflow-mention", agent.as_str()]],
    ...
    fn multiple_explicit_targets_each_use_owner() {
        ...
        let event = workflow_event(
            ...
            &[&["maju:workflow", "true"]],
            &[
                &["maju:workflow-mention", agent_a.as_str()],
                &["maju:workflow-mention", agent_b.as_str()],
            ],
    ...
    fn owner_as_explicit_target_uses_owner_without_duplicate_p_tag() {
        ...
        let event = workflow_event(
            ...
            Some(&owner),
            &[&["maju:workflow", "true"]],
            &[&["maju:workflow-mention", owner.as_str()]],
            &[owner.as_str()],
        );
    ...
    fn p_tag_without_matching_explicit_target_keeps_relay_signer() {
        ...
        let event = workflow_event(
            ...
            Some(&owner),
            &[&["maju:workflow", "true"]],
            &[&["maju:workflow-mention", other.as_str()]],
    ...
    fn forged_or_tampered_workflow_keeps_raw_signer() {
        ...
        let owner = Keys::generate().public_key().to_hex();
        let agent = Keys::generate().public_key().to_hex();
        let mentions = [&["maju:workflow-mention", agent.as_str()][..]];
    ...
    fn malformed_or_ambiguous_metadata_fails_closed() {
        ...
        let valid_mentions = [&["maju:workflow-mention", agent.as_str()][..]];
        ...
        for event in [
            ...
            workflow_event(
                &relay,
                Some(&owner),
                &[&["maju:workflow", "true"], &["maju:workflow", "true"]],
                &valid_mentions,
                &[agent.as_str()],
            ),
            workflow_event(
            ...
            workflow_event(
                ...
                Some(&owner),
                &[&["maju:workflow", "true"]],
                &[&["maju:workflow-mention", agent.as_str(), "extra"]],
                &[agent.as_str()],
            ),
            workflow_event(
                &relay,
            ...
            workflow_event(
                ...
                Some(&owner),
                &[&["maju:workflow", "true"]],
                &[&["maju:workflow-mention", "not-a-pubkey"]],
                &[agent.as_str()],
            ),
        ] {
            assert_eq!(
        ...
        tags.push(Tag::parse(["maju:workflow-owner", owner.as_str()]).expect("duplicate owner"));
        let duplicate_owner =
            EventBuilder::new(Kind::Custom(KIND_STREAM_MESSAGE as u16), "scheduled prompt")
                .tags(tags)
    ...
    fn wrong_kind_or_missing_relay_identity_fails_closed() {
        ...
        let wrong_kind = EventBuilder::new(Kind::TextNote, "scheduled prompt")
            .tags([
                Tag::parse(["maju:workflow", "true"]).expect("marker"),
                Tag::parse(["maju:workflow-owner", owner.as_str()]).expect("owner"),
                Tag::parse(["maju:workflow-mention", agent.as_str()]).expect("workflow mention"),
            ])
            .sign_with_keys(&relay)
            .expect("signed event");
        assert_eq!(
...
mod author_gate_tests {
    ...
    pub(super) fn relay_signed_workflow_dispatch(
        ...
        nostr::EventBuilder::new(nostr::Kind::Custom(KIND_STREAM_MESSAGE as u16), "dispatch")
            .tags([
                nostr::Tag::parse(["maju:workflow", "true"]).expect("workflow marker"),
                nostr::Tag::parse(["maju:workflow-owner", owner]).expect("workflow owner tag"),
                nostr::Tag::parse(["maju:workflow-mention", agent]).expect("workflow mention tag"),
    ...
    async fn test_connected_gate_wakes_owner_only_agent_for_relay_signed_workflow() {
        ...
        let maju_event = relay::MajuEvent {
    ...
    async fn test_generation_refresh_retries_after_nip11_failure() {
        ...
        let stale_old_event = relay::MajuEvent {
            connection_generation: 2,
            channel_id,
    ...
    async fn test_combined_gate_accepts_explicit_trusted_workflow_target_only() {
        ...
        let event =
            nostr::EventBuilder::new(nostr::Kind::Custom(KIND_STREAM_MESSAGE as u16), "dispatch")
                .tags([
                    nostr::Tag::parse(["maju:workflow", "true"]).expect("workflow marker"),
                    nostr::Tag::parse(["maju:workflow-owner", workflow_owner.as_str()])
                        .expect("workflow owner tag"),
                    nostr::Tag::parse(["maju:workflow-mention", agent.as_str()])
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./maju-acp.toml"),
    ...
    fn session_new_mcp_server_has_required_fields() {
        ...
        assert!(
            names.contains(&"MAJU_RELAY_URL"),
            "missing MAJU_RELAY_URL; got {names:?}"
        );
        assert!(
            names.contains(&"MAJU_PRIVATE_KEY"),
            "missing MAJU_PRIVATE_KEY; got {names:?}"
        );
    }

    #[test]
    fn session_new_mcp_server_forwards_maju_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("MAJU_AUTH_TAG", "test-attestation-tag");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("MAJU_AUTH_TAG");

        let server = &servers[0];
        let auth_tag_env = server.env.iter().find(|e| e.name == "MAJU_AUTH_TAG");
        assert!(
            auth_tag_env.is_some(),
            "MAJU_AUTH_TAG should be forwarded when set"
        );
        assert_eq!(auth_tag_env.unwrap().value, "test-attestation-tag");
    }

    #[test]
    fn session_new_mcp_server_skips_empty_maju_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("MAJU_AUTH_TAG", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("MAJU_AUTH_TAG");

        let server = &servers[0];
        let has_auth_tag = server.env.iter().any(|e| e.name == "MAJU_AUTH_TAG");
        assert!(!has_auth_tag, "empty MAJU_AUTH_TAG should not be forwarded");
    }

    #[test]
    fn test_display_name_set_is_forwarded_to_mcp_server() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("MAJU_ACP_DISPLAY_NAME", "Duncan");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("MAJU_ACP_DISPLAY_NAME");

        let entry = servers[0]
            .env
            .iter()
            .find(|e| e.name == "MAJU_ACP_DISPLAY_NAME");
        assert_eq!(
    ...
    fn test_display_name_unset_omits_the_key_entirely() {
        ...
        assert!(
            !servers[0]
                .env
                .iter()
                .any(|e| e.name == "MAJU_ACP_DISPLAY_NAME"),
    ...
    fn test_display_name_empty_omits_the_key_entirely() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("MAJU_ACP_DISPLAY_NAME", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("MAJU_ACP_DISPLAY_NAME");
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Maju event: …]…<huge>"}].
        ...
        // [Maju event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Maju event: @mention]"))
            .unwrap();
        assert!(
