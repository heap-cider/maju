//! Configuration for the maju-acp harness.
//!
...
/// while running long sub-tools (e.g. a maju-agent running another agent, or
...
/// CLI args for `maju-acp models` — query available models from an agent.
///
...
pub struct AuthAgentArgs {
    /// Agent binary to spawn (e.g. "goose", "claude-agent-acp", "codex-acp").
    #[arg(long, env = "MAJU_ACP_AGENT_COMMAND", default_value = "goose")]
    pub agent_command: String,

    /// Arguments passed to the agent binary.
    #[arg(
        long,
        env = "MAJU_ACP_AGENT_ARGS",
        default_value = "acp",
        value_delimiter = ','
    )]
...
/// CLI args for `maju-acp auth-methods` — query adapter-advertised login methods.
#[derive(Debug, Parser)]
#[command(
    name = "maju-acp auth-methods",
...
/// CLI args for `maju-acp authenticate` — start an adapter-owned login flow.
#[derive(Debug, Parser)]
#[command(
    name = "maju-acp authenticate",
...
#[command(
    name = "maju-acp",
    about = "ACP harness that bridges Maju events to AI agents"
)]
pub struct CliArgs {
    #[arg(long, env = "MAJU_RELAY_URL", default_value = "ws://localhost:3000")]
    ...
    #[arg(long, env = "MAJU_ACP_AGENT_OWNER")]
    ...
    #[arg(long, env = "MAJU_ACP_MCP_COMMAND", default_value = "")]
    pub mcp_command: String,

    /// Idle timeout: max seconds of silence before killing a turn.
    /// Resets on any agent stdout activity.
    #[arg(long, env = "MAJU_ACP_IDLE_TIMEOUT")]
    pub idle_timeout: Option<u64>,

    /// Absolute wall-clock cap per turn (safety valve).
    #[arg(long, env = "MAJU_ACP_MAX_TURN_DURATION", default_value_t = DEFAULT_MAX_TURN_DURATION_SECS)]
    pub max_turn_duration: u64,

    /// Deprecated: alias for --idle-timeout. If both set, --idle-timeout wins.
    #[arg(long, env = "MAJU_ACP_TURN_TIMEOUT", hide = true)]
    pub turn_timeout: Option<u64>,

    #[arg(
        long,
        env = "MAJU_ACP_SYSTEM_PROMPT",
    ...
    #[arg(
        long,
        env = "MAJU_ACP_HEARTBEAT_PROMPT_FILE",
        conflicts_with = "heartbeat_prompt"
    )]
    pub heartbeat_prompt_file: Option<PathBuf>,

    #[arg(long, env = "MAJU_ACP_INITIAL_MESSAGE")]
    pub initial_message: Option<String>,

    #[arg(
        long,
        env = "MAJU_ACP_SUBSCRIBE",
        default_value = "mentions",
        value_enum
    )]
    pub subscribe: SubscribeMode,

    #[arg(long, env = "MAJU_ACP_KINDS", value_delimiter = ',')]
    pub kinds: Option<Vec<u32>>,

    #[arg(long, env = "MAJU_ACP_CHANNELS", value_delimiter = ',')]
    pub channels: Option<Vec<String>>,

    #[arg(long, env = "MAJU_ACP_NO_MENTION_FILTER")]
    pub no_mention_filter: bool,

    #[arg(long, env = "MAJU_ACP_CONFIG", default_value = "./maju-acp.toml")]
    pub config: PathBuf,

    #[arg(long, env = "MAJU_ACP_DEDUP", default_value = "queue", value_enum)]
    pub dedup: DedupMode,
    ...
    #[arg(
        long,
        env = "MAJU_ACP_MULTIPLE_EVENT_HANDLING",
        default_value = "steer",
        value_enum
    )]
    pub multiple_event_handling: MultipleEventHandling,

    #[arg(long, env = "MAJU_ACP_NO_IGNORE_SELF")]
    ...
    /// when the relay confirms no core engram exists). The `maju mem` CLI
    ...
    /// Pass `--no-memory` / `MAJU_ACP_NO_MEMORY=true` to disable.
    #[arg(
        long,
        env = "MAJU_ACP_MEMORY",
    ...
    #[arg(long, env = "MAJU_ACP_NO_MEMORY", conflicts_with = "memory")]
    pub no_memory: bool,
    ...
    /// Use `maju-acp models` to discover available model IDs.
    #[arg(long, env = "MAJU_ACP_MODEL")]
    pub model: Option<String>,
    ...
    #[arg(long, env = "MAJU_ACP_SESSION_TITLE")]
    ...
    #[arg(long, env = "MAJU_ACP_RESPOND_TO_ALLOWLIST", value_delimiter = ',')]
    ...
    /// Example: `MAJU_ACP_ALLOWED_RESPOND_TO=owner-only,allowlist`
    ...
    #[arg(long, env = "MAJU_ACP_RELAY_OBSERVER", default_value_t = false)]
    pub relay_observer: bool,

    /// Connect and subscribe before starting the ACP/LLM subprocess pool.
    #[arg(long, env = "MAJU_ACP_LAZY_POOL", default_value_t = false)]
    pub lazy_pool: bool,
    ...
    #[arg(long, env = "MAJU_ACP_IDLE_POOL_SLEEP", default_value_t = 0)]
    ...
    #[arg(long, env = "MAJU_ACP_REPLAY_FLOOR")]
...
pub struct Config {
    ...
    /// `--no-memory` / `MAJU_ACP_NO_MEMORY` opt-out.
    ...
    /// Per-persona env vars to inject at agent spawn time (e.g., GOOSE_PROVIDER, GOOSE_MODEL, MAJU_AGENT_MODEL).
    ...
    /// (`--replay-floor` / `MAJU_ACP_REPLAY_FLOOR`), set by a publish-first
...
///
/// Maju is the only guard here: Codex's own `normalize_thread_name` merely
...
/// Per-runtime environment defaults applied when Maju owns the agent process.
///
...
///
/// Codex sandboxes MCP subprocesses (including `maju-cli`) behind a Seatbelt sandbox
/// that blocks all outbound network by default. Without this env var, `maju-cli`
...
pub fn propagate_legacy_env_vars() {
    for (legacy, canonical) in [
        ("MAJU_ACP_PRIVATE_KEY", "MAJU_PRIVATE_KEY"),
        ("MAJU_ACP_API_TOKEN", "MAJU_API_TOKEN"),
    ] {
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let idle_timeout_secs = {
            let raw = match (args.idle_timeout, args.turn_timeout) {
                (Some(idle), Some(_turn)) => {
                    tracing::warn!(
                        "--turn-timeout / MAJU_ACP_TURN_TIMEOUT is deprecated and ignored \
                         when --idle-timeout / MAJU_ACP_IDLE_TIMEOUT is also set"
                    );
                    idle
                }
                (Some(idle), None) => idle,
                (None, Some(turn)) => {
                    tracing::warn!(
                        "--turn-timeout / MAJU_ACP_TURN_TIMEOUT is deprecated; \
                         use --idle-timeout / MAJU_ACP_IDLE_TIMEOUT instead"
                    );
                    turn
                }
            ...
            for s in &raw {
                RespondTo::from_str(s.trim(), true).map_err(|_| {
                    ConfigError::ConfigFile(format!(
                        "invalid value in MAJU_ACP_ALLOWED_RESPOND_TO: '{s}' \
        ...
        // opens the Seatbelt network sandbox for maju-cli (an MCP subprocess). No-op
...
mod tests {
    ...
    fn test_config(mode: SubscribeMode) -> Config {
        Config {
            ...
            config_path: PathBuf::from("./maju-acp.toml"),
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&maju_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&maju_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&maju_core::kind::KIND_STREAM_REMINDER));
        }
    }
    ...
    fn default_agent_env_recognizes_hermes_identities() {
        ...
        for command in ["goose", "codex-acp", "claude-agent-acp", "maju-agent", ""] {
            assert!(
    ...
    fn codex_network_env_non_codex_agent_returns_none() {
        ...
        assert!(codex_network_env("maju-agent", "wss://relay.example.com").is_none());
    }
    ...
    fn test_load_rules_valid_toml() {
        let dir = std::env::temp_dir().join("maju-acp-test-valid");
...
"#,
    ...
    fn test_load_rules_empty_name_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-empty-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_duplicate_name_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-dup-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_invalid_filter_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-bad-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn test_load_rules_channel_scope_typo_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-scope-typo");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_too_many_rules_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-too-many");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_load_rules_filter_too_long_rejected() {
        let dir = std::env::temp_dir().join("maju-acp-test-long-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn lazy_pool_defaults_off() {
        let key = "0".repeat(64);
        assert!(!CliArgs::parse_from(["maju-acp", "--private-key", &key]).lazy_pool);
    }

    #[test]
    fn idle_pool_sleep_defaults_disabled_and_accepts_cli_value() {
        let key = "0".repeat(64);
        let default = CliArgs::parse_from(["maju-acp", "--private-key", &key]);
    ...
    fn lazy_pool_cli_flag_enables_deferred_startup() {
        let key = "0".repeat(64);
        let args = CliArgs::try_parse_from(["maju-acp", "--private-key", &key, "--lazy-pool=true"]);
        assert!(args.is_err(), "bool flags do not take an explicit value");
        assert!(CliArgs::parse_from(["maju-acp", "--private-key", &key, "--lazy-pool"]).lazy_pool);
    }
    ...
    fn test_permission_mode_value_enum_camel_case_aliases() {
        ...
        // (e.g. MAJU_ACP_PERMISSION_MODE=bypassPermissions). The #[value(alias)]
    ...
    fn test_session_policy_default_is_channel() {
        ...
        let args = CliArgs::parse_from(["maju-acp", "--private-key", &"0".repeat(64)]);
    ...
    fn test_session_policy_env_var_parses() {
        // The env fallback (`MAJU_ACP_SESSION_POLICY`) must resolve to the same
    ...
    fn budget_ordering_invariant_shell_cap_plus_headroom_fits_within_idle_timeout() {
        ...
        //   maju-dev-mcp MAX_TIMEOUT_MS (1 200 000 ms = 1 200s)
        //   ≤ maju-agent MAJU_AGENT_TOOL_TIMEOUT_SECS default (1 260s)
        //   < maju-acp DEFAULT_IDLE_TIMEOUT_SECS (1 500s)
        //
        // The idle deadline must strictly outlast the agent tool timeout so a
        // legitimately long-running tool call is killed by maju-agent first (at
        ...
        const SHELL_CAP_MS: u64 = 1_200_000; // maju-dev-mcp MAX_TIMEOUT_MS
        const SHELL_CAP_SECS: u64 = SHELL_CAP_MS / 1_000;
        const AGENT_TOOL_TIMEOUT_SECS: u64 = 1_260; // maju-agent MAJU_AGENT_TOOL_TIMEOUT_SECS default
    ...
    // --- MAJU_ACP_ALLOWED_RESPOND_TO gate ---
    ...
    fn allowed_respond_to_rejects_invalid_mode_string() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("invalid value in MAJU_ACP_ALLOWED_RESPOND_TO"),
    ...
    //
    // These tests exercise the actual wiring: MAJU_ACP_ALLOWED_RESPOND_TO in the
    ...
    fn compose_session_title_qualifies_the_agent_name_with_the_channel() {
        assert_eq!(
            compose_session_title("Fizz", Some("maju-dev")),
            "Fizz · #maju-dev"
        );
    }
    ...
    fn compose_session_title_drops_the_channel_when_the_agent_name_fills_the_cap() {
        ...
        assert_eq!(compose_session_title(&agent, Some("maju-dev")), agent);
    }
    ...
    fn scoped_session_title_keeps_short_root_even_when_names_fill_the_cap() {
        ...
        assert_eq!(
            compose_scoped_session_title("Fizz", Some("maju-dev"), Some(&root)),
            "Fizz · #maju-dev · abcdef01"
        );
        assert_eq!(
        ...
        assert_eq!(
            compose_scoped_session_title("Fizz", Some("maju-dev"), Some("abc")),
            "Fizz · #maju-dev · abc"
        );
        ...
        assert_eq!(
            compose_scoped_session_title("Fizz", Some("maju-dev"), None),
            "Fizz · #maju-dev"
        );
