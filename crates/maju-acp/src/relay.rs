//! Harness-side Maju relay client.
//!
//! Connects to the Maju relay via NIP-01 WebSocket, authenticates via NIP-42,
...
//! - Forwards `MajuEvent`s through an `mpsc` channel
...
/// Override with `MAJU_ACP_EVENT_BUFFER` env var at startup.
...
fn event_channel_capacity() -> usize {
    std::env::var("MAJU_ACP_EVENT_BUFFER")
        .ok()
...
impl RestClient {
    ...
    /// that expose the document through Maju's explicit alias.
    ...
    /// lookup needs `#maju-channel`, which this path serializes verbatim.
...
///
/// Connects to the Maju relay, authenticates via NIP-42, and streams
...
pub struct HarnessRelay {
    ...
    event_rx: mpsc::Receiver<Option<MajuEvent>>,
...
impl HarnessRelay {
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                maju_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
    ...
    pub async fn next_event(&mut self) -> Option<MajuEvent> {
...
async fn run_background_task(
    ...
    event_tx: mpsc::Sender<Option<MajuEvent>>,
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    let event = match tokio::task::spawn_blocking(move || {
                        maju_core::verify_event(&event).map(|()| event)
                    })
                    .await
                    {
                        Ok(Ok(event)) => event,
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(maju_event)) {
                            Ok(()) => {
...
/// sends (`crates/maju-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    ///
    /// This is the maju-acp side of the relay's rejection-correlation change:
    ...
    async fn rate_limited_ok_arms_gate_and_reparks_refused_observer_frame() {
        let (mut client, _server) = test_ws_pair().await;
        let (event_tx, _event_rx) = mpsc::channel::<Option<MajuEvent>>(4);
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        maju_sdk::build_agent_observer_frame(
