...
pub struct AcpClient {
    ...
    ///
    /// Both goose and maju-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/maju-agent emit `activeRunId: null` at end of turn).
    ...
    /// Usage tracker for goose/maju-agent's cumulative notification format.
...
///
/// Returns `Some(json_string)` when `has_generated_codex_config` is true (Maju injected a
...
pub(crate) fn build_codex_config_env(
    ...
    // Without an explicit Maju-generated overlay signal, skip the merge entirely.
...
/// `_meta.goose.activeRunId`. Emitted by goose and maju-agent only.
...
/// `outcome` value meaning the steer was applied to the turn Maju is waiting
/// on, which therefore keeps running.
const STEER_OUTCOME_INJECTED: &str = "injected";

/// `outcome` value meaning the turn Maju was steering had already finished, so
...
fn build_client_capabilities() -> serde_json::Value {
    serde_json::json!({
        // Signal to ACP adapters that Maju can hand users to terminal-native
        // auth flows. Adapters decide which auth methods to expose; Maju does
...
impl AcpClient {
    ...
    ///   (ACP protocol v2, maju-agent, goose unused).
    ...
    /// `["/cmd args", "<maju context>"]` instead of one wrapped block.
    ...
    ///
    /// Both goose and maju-agent emit `session_info_update`; other agents
    ...
    /// Consume per-turn usage for NIP-AM publishing. Goose/maju-agent is an
    ...
    /// Notify the usage tracker that maju-acp just spawned a new session.
    ///
    ...
    /// Must be called only when maju-acp created the session via `session/new`;
                            ...
                            if let Some((steer_id, _, _)) = pending_steer.as_ref() {
                                if *id == serde_json::json!(*steer_id) {
                                    ...
                                    let ack = if let Some(error) = msg.get("error") {
                                        ...
                                        // turn Maju awaits is still running —
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and maju-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and maju-agent both
...
///
/// - **`Field`** — bare `systemPrompt` field (ACP protocol v2, maju-agent).
...
mod tests {
    ...
    async fn spawn_named_and_read_child_env(
        ...
        let dir = std::env::temp_dir().join(format!("maju-acp-env-probe-{}", uuid::Uuid::new_v4()));
    ...
    /// Maju-owned Hermes processes get the configured-MCP isolation default,
    ...
    async fn goose_system_prompt_request_uses_set_contract() {
        ...
        assert_eq!(received["params"]["key"], "maju");
    ...
    async fn session_new_full_sends_session_title_in_meta_when_some() {
        ...
        let resp = client
            .session_new_full("/tmp", vec![], None, Some("Fizz · #maju-dev"))
            .await
            .expect("session_new_full should succeed");
    ...
    /// `SessionInfoUpdate` schema), matching what goose and maju-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("maju-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, maju-agent, any older adapter) must
    ...
    /// Maju maps `SteerAck::Success` to `queue.remove_event`, so decoding
    ...
    /// the turn Maju was steering had already ended and the adapter began a
    ...
    fn build_codex_config_env_generated_only_single_entry_with_signal_true_merges_with_parent() {
        // No persona: Maju injects one CODEX_CONFIG; signal=true.
    ...
    fn build_codex_config_env_persona_only_signal_false_returns_none() {
        // Persona set CODEX_CONFIG; Maju did not inject a generated overlay (signal=false).
