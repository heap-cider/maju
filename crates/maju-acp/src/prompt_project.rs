...
///
/// A project's own `maju-channel` is presentation metadata and cannot establish
/// authority. A candidate is accepted only when one of its `a` members resolves
/// to a live `kind:30617` whose first `maju-channel` is `channel_id` and whose
...
pub fn pick_authoritative_project_home(
    ...
    let mut matches = project_events.iter().filter_map(|event| {
        if event_is_unlisted(event) || !event_has_tag_value(event, "maju-channel", channel_id) {
            return None;
        }
    ...
    events
        ...
        .filter_map(|event| {
            if event.get("kind").and_then(Value::as_u64) != Some(30617)
                ...
                || first_tag_value(event, "maju-channel") != Some(channel_id)
            {
                return None;
            }
...
fn event_is_unlisted(event: &Value) -> bool {
    event_has_tag_value(event, "maju-visibility", "unlisted")
}
...
mod tests {
    ...
    fn project(owner: &str, slug: &str, repo: &str) -> Value {
        json!({"pubkey": owner, "kind": 30621, "tags": [
            ["d", slug], ["name", slug], ["maju-channel", CHANNEL_ID], ["a", repo]
        ]})
    }

    fn repo(owner: &str, id: &str, channel: &str, extra: Vec<Value>) -> Value {
        let mut tags = vec![json!(["d", id]), json!(["maju-channel", channel])];
        tags.extend(extra);
    ...
    fn first_repo_channel_binding_is_authoritative() {
        ...
        announcement["tags"]
            .as_array_mut()
            .unwrap()
            .push(json!(["maju-channel", CHANNEL_ID]));
        assert!(pick_authoritative_project_home(
            &[project(&owner, "game", &coord)],
            &[announcement],
            CHANNEL_ID
