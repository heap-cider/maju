...
//! `PI_ACP_PI_COMMAND`. For Pi sessions, Maju points that variable at a
//! private launcher which adds `--system-prompt <file>` and the canonical Maju
...
/// Files backing the Pi launcher for one `maju-acp` process.
///
    ...
    pub(crate) fn prepare(
        ...
        if inherited_pi_command_is_set {
            return Err(io::Error::new(
                ...
                "PI_ACP_PI_COMMAND is managed by Maju; unset it before starting a managed Pi agent",
            ));
        }

        // Maju owns PI_ACP_PI_COMMAND and always uses it to point pi-acp at
        // this generated launcher. The launcher resolves the ordinary `pi`
        // command from Maju's effective PATH.
...
fn launcher_file_name() -> &'static str {
    "pi-with-maju-context"
}

#[cfg(windows)]
fn launcher_file_name() -> &'static str {
    "pi-with-maju-context.cmd"
}
...
mod tests {
    ...
    fn non_pi_adapter_keeps_base_prompt_for_acp_delivery() {
        let base = Some("Maju base".to_string());
    ...
    fn pi_adapter_rejects_inherited_pi_command() {
        ...
        assert!(error.to_string().contains("managed by Maju"));
    }
    ...
    #[test]
    fn pi_adapter_moves_maju_base_out_of_ordinary_acp_delivery() {
    ...
    fn pi_launcher_replaces_system_prompt_and_forwards_adapter_args() {
        ...
        let fixture_dir =
            std::env::temp_dir().join(format!("maju-acp-pi-system-prompt-test-{}", Uuid::new_v4()));
