...
impl Config {
    ...
    pub fn from_map(e: &HashMap<String, String>) -> Result<Self, ConfigError> {
        ...
        let grant_keys = parse_keyring(e, "MAJU_PUSH_GRANT_KEYS")?;
        let token_keys = parse_keyring(e, "MAJU_PUSH_TOKEN_KEYS")?;
        if grant_keys.iter().any(|grant| {
            token_keys
                .iter()
                .any(|token| grant.id == token.id || grant.key == token.key)
        }) {
            return Err(ConfigError::Invalid("MAJU_PUSH_TOKEN_KEYS"));
        }
        let public_delivery_url = req(e, "MAJU_PUSH_PUBLIC_DELIVERY_URL")?
            .parse::<url::Url>()
        ...
        {
            return Err(ConfigError::Invalid("MAJU_PUSH_PUBLIC_DELIVERY_URL"));
        }
        let max_grant_lifetime_seconds = req(e, "MAJU_PUSH_MAX_GRANT_LIFETIME_SECONDS")?
            .parse::<i64>()
            .ok()
            .filter(|seconds| (1..=31_536_000).contains(seconds))
            .ok_or(ConfigError::Invalid("MAJU_PUSH_MAX_GRANT_LIFETIME_SECONDS"))?;
        let max_installation_lifetime_seconds = e
            .get("MAJU_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS")
            .map(String::as_str)
            .unwrap_or("7776000")
            .parse::<i64>()
            .ok()
            ...
            .ok_or(ConfigError::Invalid(
                "MAJU_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS",
            ))?;
...
mod tests {
    ...
    fn malformed_or_empty_keyrings_fail_startup() {
        for (variable, value) in [
            ("MAJU_PUSH_GRANT_KEYS", ""),
            ("MAJU_PUSH_GRANT_KEYS", "missing_separator"),
            ("MAJU_PUSH_GRANT_KEYS", "id:bad-base64"),
            ("MAJU_PUSH_TOKEN_KEYS", ""),
            ("MAJU_PUSH_TOKEN_KEYS", "missing_separator"),
            ("MAJU_PUSH_TOKEN_KEYS", "id:bad-base64"),
        ] {
            let mut env = base();
