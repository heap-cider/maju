...
fn profile(v: &str) -> Result<AppProfile, AuthorityError> {
    match v {
        "buzz-ios-dogfood" => Ok(AppProfile::MajuIosDogfood),
...
mod postgres_tests {
    ...
    const TEST_DB_URL: &str = "postgres://maju:maju_dev@localhost:5432/maju"; // sadscan:disable np.postgres.1 -- fixed localhost-only test credential
    ...
    async fn cluster_global_readiness_requires_migrated_schema_dml_and_no_ddl() {
        let admin_url = std::env::var("MAJU_TEST_DATABASE_URL")
    ...
    async fn delegation_renews_and_expired_enrollment_recovers_token_ownership() {
        ...
        let installation = |id, expires_at| NewInstallation {
            ...
            assertion_counter: 0,
            profile: AppProfile::MajuIosDogfood,
