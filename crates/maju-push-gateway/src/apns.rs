...
mod tests {
    ...
    async fn live_sandbox_probe_reports_literal_status_and_body() {
        let cert_path = std::env::var("MAJU_PUSH_LIVE_APNS_CERT_PATH")
            .expect("set MAJU_PUSH_LIVE_APNS_CERT_PATH to the dogfood identity PEM");
        let topic = std::env::var("MAJU_PUSH_LIVE_APNS_TOPIC")
            .expect("set MAJU_PUSH_LIVE_APNS_TOPIC to the dogfood bundle id");
