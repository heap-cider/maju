...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::args().nth(1).as_deref() == Some("--migrate-only") {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .await?;
        let runtime_role = std::env::var("MAJU_PUSH_RUNTIME_DATABASE_ROLE")?;
        PostgresAuthorityStore::apply_migrations_and_grants(&pool, &runtime_role).await?;
        return Ok(());
    }
    let c = Config::from_env()?;
    let metrics_handle = maju_push_gateway::metrics::install()?;
    ...
    let profile = {
        ...
        maju_push_gateway::http::ProfileRuntime {
    ...
    let reaper = tokio::spawn(async move {
        ...
        loop {
            ...
            if reaper_authority
                ...
                .is_err()
            {
                maju_push_gateway::metrics::record_reaper_failure();
