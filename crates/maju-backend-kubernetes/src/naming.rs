...
pub const MANAGED_BY: &str = "maju-backend-kubernetes";

/// Label key carrying [`MANAGED_BY`].
pub const LABEL_MANAGED_BY: &str = "app.kubernetes.io/managed-by";

/// Label key carrying [`BINDING_VERSION`] — the marker's schema half.
pub const LABEL_BINDING_VERSION: &str = "maju.block.xyz/binding-version";
...
pub const LABEL_AGENT_PUBKEY: &str = "maju.block.xyz/agent-pubkey";
...
/// step 1).
pub const ANNOTATION_PUBKEY_FULL: &str = "maju.block.xyz/agent-pubkey-full";

/// Annotation key: the recorded create-intent fingerprint.
pub const ANNOTATION_CREATE_INTENT: &str = "maju.block.xyz/create-intent";

/// Annotation key: the image reference this generation actually resolved to,
/// for post-hoc attribution (§Image).
pub const ANNOTATION_IMAGE: &str = "maju.block.xyz/image";
...
impl AgentIdentity {
    ...
    pub fn pod_name(&self) -> String {
        format!("maju-agent-{}", &self.pubkey_hex[..12])
    }
    ...
    pub fn secret_name(&self, generation: &str) -> String {
        format!("maju-agent-{}-{}", &self.pubkey_hex[..12], generation)
    }
...
///
/// Appears in the Secret name and as `MAJU_MANAGED_AGENT_START_NONCE`, so the
...
mod tests {
    ...
    fn pod_name_is_deterministic_and_dns_safe() {
        ...
        assert_eq!(
            id.pod_name(),
            format!("maju-agent-{}", &id.pubkey_hex()[..12])
        );
