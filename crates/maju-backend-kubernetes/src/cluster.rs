...
mod tests {
    ...
    async fn list_through_real_request_path(
        ...
        let result = cluster
            .list_with_date::<Pod>("app.kubernetes.io/managed-by=maju-backend-kubernetes")
            .await
            .unwrap();
    ...
    async fn list_with_date_uses_a_quorum_request_and_returns_the_server_clock() {
        ...
        assert!(
            uri.contains("labelSelector=app.kubernetes.io%2Fmanaged-by%3Dmaju-backend-kubernetes")
        );
        assert!(
