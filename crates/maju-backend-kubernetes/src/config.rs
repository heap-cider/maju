...
//! operator supplies a Maju-compatible image pinned by digest).
...
/// `MAJU_ACP_EXIT_AFTER_INACTIVITY` are one knob, not two.
...
/// A fresh `maju-agents-<rand6>` namespace default.
///
    ...
    serde_json::json!({
        ...
        "properties": {
            ...
            "image": {
                "type": "string",
                "title": "Agent image",
                "description": "Supply a Maju-compatible agent image pinned as registry/image@sha256:<digest>. Tags alone are not accepted."
            },
            "cpu_request": {
...
mod tests {
    ...
    fn digest_ref() -> String {
        format!("registry.example/maju-sprig@sha256:{}", "a".repeat(64))
    }

    fn minimal() -> serde_json::Value {
        serde_json::json!({"namespace": "maju-agents-abc123", "image": digest_ref()})
    }
    ...
    fn image_is_required_and_must_be_digest_pinned() {
        ...
        cfg["image"] = "registry.example/maju-sprig:latest".into();
    ...
    fn generated_namespaces_are_fresh_and_valid() {
        ...
        assert!(a.starts_with("maju-agents-"));
        assert_eq!(a.len(), "maju-agents-".len() + 6);
    }
    ...
    #[test]
    fn image_schema_requires_an_operator_supplied_digest() {
        let schema = config_schema();
        assert!(schema["properties"]["image"].get("default").is_none());
        let missing = serde_json::json!({"namespace": "maju-agents-abc123"});
        assert!(parse(&missing).unwrap_err().contains("image"));
        let supplied =
            serde_json::json!({"namespace": "maju-agents-abc123", "image": digest_ref()});
        assert_eq!(parse(&supplied).unwrap().image.as_str(), digest_ref());
    }
