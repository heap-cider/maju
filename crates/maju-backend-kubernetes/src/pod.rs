...
mod tests {
    ...
    fn provider_config() -> ProviderConfig {
        config::parse(&serde_json::json!({
            "namespace": "maju-agents-test",
            "image": format!("registry.example/maju-sprig@sha256:{}", "a".repeat(64)),
        }))
        .unwrap()
    }
    ...
    fn pod() -> Pod {
        ...
        build_pod(
            ...
            &cfg,
            "gen00001",
            &intent_template(&cfg, ["MAJU_RELAY_URL".to_string()]).fingerprint(),
        )
    }
    ...
    fn pod_carries_identity_marker_and_recorded_intent() {
        ...
        assert_eq!(meta.namespace.as_deref(), Some("maju-agents-test"));
    ...
    fn secret_is_immutable_marked_and_holds_the_env() {
        ...
        let env: BTreeMap<String, String> =
            [("MAJU_RELAY_URL".to_string(), "wss://r".to_string())].into();
    ...
    fn recorded_fingerprint_matches_a_fresh_computation() {
        let cfg = provider_config();
        let keys = ["MAJU_RELAY_URL".to_string(), "GOOSE_MODE".to_string()];
    ...
    fn generation_does_not_change_the_recorded_fingerprint() {
        let cfg = provider_config();
        let keys = ["MAJU_RELAY_URL".to_string()];
