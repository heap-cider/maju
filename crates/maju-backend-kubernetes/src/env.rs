...
/// (`MAJU_ACP_AGENT_ARGS` only when `launch.args` is non-empty,
/// `MAJU_ACP_RESPOND_TO` only when set), and without the clear, a lower tier
...
/// a retry.
pub const START_NONCE_KEY: &str = "MAJU_MANAGED_AGENT_START_NONCE";
...
const FORBIDDEN_KEY: &str = "MAJU_ACP_NO_PRESENCE";
...
/// `RespondTo` (kebab-case) and as `maju-acp`'s CLI parses it.
const RESPOND_TO_ALLOWLIST: &str = "allowlist";

/// Every gate mode `maju-acp` accepts, spelled as its `clap::ValueEnum` parses
...
///
/// Mirrors `maju-acp`'s own rules exactly (`config.rs:95-101,996-1004,629-641`),
...
/// `MAJU_ACP_RESPOND_TO_ALLOWLIST` is an authoritative key that tier 3 clears.
...
pub fn build_env(
    ...
    env.insert("MAJU_RELAY_URL".into(), relay_url.to_string());
    env.insert("MAJU_PRIVATE_KEY".into(), agent.private_key_nsec.clone());
    ...
    match (auth_tag, owner) {
        ...
        (tag, own) => {
            if let Some(t) = tag {
                env.insert("MAJU_AUTH_TAG".into(), t.to_string());
            }
            if let Some(o) = own {
                env.insert("MAJU_ACP_AGENT_OWNER".into(), o.to_string());
            }
        }
    }
    ...
    if let Some(command) = launch.command.as_deref().filter(|c| !c.is_empty()) {
        env.insert("MAJU_ACP_AGENT_COMMAND".into(), command.to_string());
    }
    if !launch.args.is_empty() {
        ...
        env.insert("MAJU_ACP_AGENT_ARGS".into(), launch.args.join(","));
    }
    env.insert("MAJU_ACP_MCP_COMMAND".into(), "maju-dev-mcp".into());
    ...
    if let Some(respond_to) = agent.respond_to.as_deref().filter(|s| !s.is_empty()) {
        ...
        env.insert("MAJU_ACP_RESPOND_TO".into(), respond_to.to_string());
    }
    if let Some(list) = agent
        ...
        .filter(|l| !l.is_empty())
    {
        env.insert("MAJU_ACP_RESPOND_TO_ALLOWLIST".into(), list.join(","));
    }

    if let Some(secs) = auth.inactivity_seconds {
        env.insert("MAJU_ACP_EXIT_AFTER_INACTIVITY".into(), secs.to_string());
    }
...
mod tests {
    ...
    fn identity_comes_from_top_level_fields() {
        ...
        assert_eq!(env["MAJU_RELAY_URL"], "wss://relay.example");
        assert_eq!(env["MAJU_PRIVATE_KEY"], "nsec1example");
        assert_eq!(env["NOSTR_PRIVATE_KEY"], "nsec1example");
        assert_eq!(env["MAJU_AUTH_TAG"], "tag-1");
    }
    ...
    fn lower_tiers_cannot_spoof_authoritative_values() {
        let agent = payload_json(serde_json::json!({
            "launch": {
                "command": "goose",
                "policy_env": {
                    "MAJU_PRIVATE_KEY": "nsec1attacker",
                    "MAJU_MANAGED_AGENT_START_NONCE": "forged",
                },
                "env": {
                    "MAJU_RELAY_URL": "wss://attacker.example",
                    "NOSTR_PRIVATE_KEY": "nsec1attacker",
                    "MAJU_AUTH_TAG": "forged-tag",
                    "MAJU_ACP_AGENT_OWNER": "cafe",
                    "MAJU_ACP_AGENT_COMMAND": "/bin/sh",
                    "MAJU_ACP_MCP_COMMAND": "/bin/sh",
                    "MAJU_ACP_EXIT_AFTER_INACTIVITY": "0",
                },
                "owner_pubkey": "beef"
            }
        }));
        ...
        assert_eq!(env["MAJU_RELAY_URL"], "wss://relay.example");
        assert_eq!(env["MAJU_AUTH_TAG"], "tag-1");
        assert_eq!(env["MAJU_ACP_AGENT_OWNER"], "beef");
        assert_eq!(env["MAJU_ACP_AGENT_COMMAND"], "goose");
        assert_eq!(env["MAJU_ACP_MCP_COMMAND"], "maju-dev-mcp");
        assert_eq!(env["MAJU_ACP_EXIT_AFTER_INACTIVITY"], "7200");
        assert_eq!(env["MAJU_MANAGED_AGENT_START_NONCE"], "gen0001");
    }
    ...
    fn user_env_overrides_policy_defaults() {
        let agent = payload_json(serde_json::json!({
            "launch": {
                "policy_env": {"GOOSE_MODE": "auto", "MAJU_ACP_MODEL": "sonnet"},
                "env": {"GOOSE_MODE": "chat"},
                "owner_pubkey": "beef"
            }
        }));
        ...
        assert_eq!(env["MAJU_ACP_MODEL"], "sonnet");
    }
    ...
    /// MAJU_ACP_MODEL is absent — so the remote process has exactly one model
    ...
    fn claude_launch_yields_single_model_authority_through_build_env() {
        ...
        assert!(
            !env.contains_key("MAJU_ACP_MODEL"),
    ...
    /// the canonical, and a launch.env MAJU_ACP_MODEL introduces a second
    ...
    fn launch_env_model_keys_would_win_over_policy_env_documenting_the_hazard() {
        let agent = payload_json(serde_json::json!({
            "launch": {
                "command": "claude",
                "policy_env": {"ANTHROPIC_MODEL": "claude-opus-4"},
                "env": {"ANTHROPIC_MODEL": "user-haiku", "MAJU_ACP_MODEL": "user-sonnet"},
                "owner_pubkey": "beef"
            }
        }));
        let env = build(&agent).unwrap();
        ...
        assert_eq!(
            env["MAJU_ACP_MODEL"], "user-sonnet",
            "a leftover MAJU_ACP_MODEL would be a second authority — desktop strips it"
        );
    }
    ...
    fn identity_components_are_stored_trimmed() {
        ...
        assert_eq!(env["MAJU_RELAY_URL"], "wss://relay.example");
        assert_eq!(env["MAJU_AUTH_TAG"], "tag-1");
        assert_eq!(env["MAJU_ACP_AGENT_OWNER"], "beefcafe");
    }
    ...
    fn owner_pubkey_alone_is_sufficient() {
        ...
        let env = build(&agent).unwrap();
        assert_eq!(env["MAJU_ACP_AGENT_OWNER"], "beefcafe");
        assert!(!env.contains_key("MAJU_AUTH_TAG"));
    }

    #[test]
    fn refuses_presence_suppression() {
        let agent = payload_json(serde_json::json!({
            "launch": {"env": {"MAJU_ACP_NO_PRESENCE": "1"}, "owner_pubkey": "beef"}
        }));
        let err = build(&agent).unwrap_err();
        assert!(err.contains("MAJU_ACP_NO_PRESENCE"), "got: {err}");
    }
    ...
    fn args_are_comma_joined_and_omitted_when_empty() {
        ...
        let env = build(&agent).unwrap();
        assert_eq!(env["MAJU_ACP_AGENT_ARGS"], "run,--no-session");

        let agent = payload_json(serde_json::json!({
            "launch": {"command": "goose", "args": [], "owner_pubkey": "b"}
        }));
        assert!(!build(&agent).unwrap().contains_key("MAJU_ACP_AGENT_ARGS"));
    }
    ...
    fn inactivity_omitted_when_unset() {
        ...
        .unwrap();
        assert!(!env.contains_key("MAJU_ACP_EXIT_AFTER_INACTIVITY"));
    }
    ...
    /// This test caught exactly that: `MAJU_ACP_AGENT_ARGS` is only written
    ...
    fn a_valid_allowlist_gate_is_accepted_and_comma_joined() {
        ...
        let env = build(&agent).unwrap();
        assert_eq!(env["MAJU_ACP_RESPOND_TO"], "allowlist");
        assert_eq!(
            env["MAJU_ACP_RESPOND_TO_ALLOWLIST"],
    ...
    fn a_junk_allowlist_is_tolerated_outside_allowlist_mode() {
        for mode in ["owner-only", "anyone"] {
            ...
            assert_eq!(env["MAJU_ACP_RESPOND_TO"], mode);
        }
    }
