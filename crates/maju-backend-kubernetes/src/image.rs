...
//! fails closed with a named field. Maju does not prefill an upstream container
//! digest because its agent image must contain the Maju runtime.
...
mod tests {
    ...
    fn accepts_digest_pinned_reference() {
        let d = "a".repeat(64);
        let r = parse(&format!("registry.example/maju-sprig@sha256:{d}")).unwrap();
        assert_eq!(
            r.as_str(),
            format!("registry.example/maju-sprig@sha256:{d}")
        );
    }
    ...
    fn strips_tag_from_tag_plus_digest_form() {
        let d = "b".repeat(64);
        let tagged = parse(&format!("registry.example/maju-sprig:v1.2@sha256:{d}")).unwrap();
        let plain = parse(&format!("registry.example/maju-sprig@sha256:{d}")).unwrap();
    ...
    fn registry_port_is_not_mistaken_for_a_tag() {
        let d = "c".repeat(64);
        let r = parse(&format!("localhost:5000/maju-sprig@sha256:{d}")).unwrap();
        assert_eq!(r.as_str(), format!("localhost:5000/maju-sprig@sha256:{d}"));
    }

    #[test]
    fn port_and_tag_together_drops_only_the_tag() {
        let d = "d".repeat(64);
        let r = parse(&format!("localhost:5000/maju-sprig:dev@sha256:{d}")).unwrap();
        assert_eq!(r.as_str(), format!("localhost:5000/maju-sprig@sha256:{d}"));
    }
    ...
    fn rejects_every_tag_only_reference() {
        for bad in [
            "registry.example/maju-sprig:latest",
            "registry.example/maju-sprig:v1.2.3",
            "registry.example/maju-sprig:sha-abc1234",
            "registry.example/maju-sprig",
            "localhost:5000/maju-sprig",
        ] {
