...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/maju-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: maju-cli\ndescription: CLI reference for Maju managed agents\n---\nUse `maju` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("maju-cli: CLI reference for Maju managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `maju` to manage agents."),
