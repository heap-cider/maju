...
///   saved-agent model picker or `maju-agent auth databricks`. May open a
...
impl AuthError {
    ...
    /// newer maju-agent) yields `None`, treated as "no active record" rather
    /// than a hard failure.
    ...
    fn message(&self) -> String {
        match self {
            Self::NoCredential => {
                "no cached Databricks token; run `maju-agent auth databricks` first".into()
            }
...
/// `maju-agent/oauth/` root where the token JSON lives — separates providers'
/// caches cleanly.
#[derive(Debug, Clone)]
pub struct PkceOAuthConfig {
    ...
    /// `<platform config dir>/maju-agent/oauth/<cache_namespace>/`. Production code
...
impl PkceOAuthTokenSource {
    ...
    async fn acquire_leader(
        ...
        tracing::trace!(
            target: "maju_agent::auth::acquire_leader_snapshot",
            snapshot_gen,
            "snapshot taken"
        );
...
impl TokenSource for PkceOAuthTokenSource {
    ...
    /// job of `maju-agent auth databricks` ([`interactive_login`]).
    ///
...
const MAJU_AGENT_CONFIG_DIR_ENV: &str = "MAJU_AGENT_CONFIG_DIR";
...
fn oauth_cache_root_for(
    ...
    if let Some(root) = config_override {
        return Ok(root.join("maju-agent").join("oauth"));
    }
    Ok(home_dir
...
fn default_oauth_cache_root() -> Result<PathBuf, AgentError> {
    oauth_cache_root_for(
        std::env::var_os(MAJU_AGENT_CONFIG_DIR_ENV).map(PathBuf::from),
        dirs::home_dir(),
    )
}
...
/// write-path property: a world-readable cache left by an older maju-agent
...
fn callback_outcome(
    ...
    let page = match result {
        Ok(_) => "<h2>Maju: signed in</h2><p>You can close this window.</p>",
        Err(_) => "<h2>Maju auth failed</h2><p>You can close this window and try again.</p>",
    }
    .to_string();
    (result, page)
}
...
mod tests {
    ...
    fn production_and_demo_oauth_roots_are_concrete_and_distinct() {
        ...
        let first_demo_config = home
            ...
            .join("maju-demo-board-1234567812345678");
        let second_demo_config = home
            .join("Library/Application Support")
            .join("maju-demo-board-8765432187654321");
        ...
        assert_eq!(
            production,
            PathBuf::from("/Users/demo/.config/maju-agent/oauth")
        );
        assert_eq!(
            first_demo,
            PathBuf::from(
        ...
        assert_eq!(
            second_demo,
            PathBuf::from(
                "/Users/demo/Library/Application Support/maju-demo-board-8765432187654321/maju-agent/oauth"
            )
        );
    ...
    async fn test_save_repairs_preexisting_loose_mode_file() {
        ...
        // Simulate a world-readable cache written by an older maju-agent.
    ...
    async fn test_bearer_cache_hit_tightens_preexisting_loose_mode_file() {
        ...
        // A world-readable cache left by an older maju-agent must be tightened
