    ...
    let app = Router::new().route(
        ...
        get(move |Query(params): Query<HashMap<String, String>>| {
            ...
            async move {
                ...
                match result {
                    Ok(_) => Html(
                        "<h2>Maju: signed in</h2><p>You can close this window.</p>".to_string(),
                    ),
                    Err(e) => Html(format!("<h2>Maju auth failed</h2><pre>{e}</pre>")),
                }
            }
        }),
    );
...
mod tests {
    ...
    fn cache_path_includes_namespace_and_hash() {
        ...
        assert!(p.to_string_lossy().contains("/maju-agent/oauth/demo/"));
