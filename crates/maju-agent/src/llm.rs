...
/// consent for `databricks-cli` covers maju-agent too.
...
impl Llm {
    ...
    /// route through a mesh (Maju shared compute) resolve their own model name
        ...
        match phase {
            TimeoutPhase::Transport => format!(
                "read timeout: no response bytes received within {per_request_timeout:?} \
                 (consider raising MAJU_AGENT_LLM_TIMEOUT_SECS)"
            ),
...
async fn openrouter_post(
    ...
    'attempt: for attempt in 0..MAX_RETRIES {
        ...
        let resp = match http
            .post(url)
            .header("content-type", "application/json")
            .header("HTTP-Referer", "https://github.com/heap-cider/maju")
            .header("X-OpenRouter-Title", "Maju")
            .bearer_auth(bearer)
            .body(body_bytes.clone())
            .timeout(per_request_timeout)
            .send()
...
fn apply_openrouter_mutations(
    ...
    if let Some(obj) = body.as_object_mut() {
        ...
        // `prompt_caching` (`MAJU_AGENT_PROMPT_CACHING`) for the same reason as
...
mod tests {
    ...
    fn timeout_message_connect_true_shows_connect_timeout() {
        ...
        for phase in [TimeoutPhase::Transport, TimeoutPhase::BodyRead] {
            ...
            assert!(
                !msg.contains("MAJU_AGENT_LLM_TIMEOUT_SECS"),
    ...
    /// Maju agents stall (server connected but emitting no bytes).
    #[tokio::test]
    ...
    /// `MAJU_AGENT_PROMPT_CACHING=0` must reach the OpenRouter `anthropic/*`
    ...
    fn openrouter_body_caching_disabled_emits_no_cache_control() {
        ...
        assert!(
            ...
            "MAJU_AGENT_PROMPT_CACHING=0 must suppress every breakpoint: {body}"
        );
    ...
    async fn openrouter_post_sends_attribution_headers() {
        ...
        let header_str = headers
            ...
            .to_lowercase();
        assert!(
            header_str.contains("http-referer: https://github.com/heap-cider/maju"),
            "got: {header_str}"
        );
        assert!(
            header_str.contains("x-openrouter-title: maju"),
            "got: {header_str}"
        );
    }
