...
pub enum Inbound {
    ...
    /// to a request maju-agent issued. Today the only such request is
...
/// one. `expected_run_id` must match the run id maju-agent advertised via
...
/// session use `model_id` instead of the configured `MAJU_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) answer a request maju-agent
...
/// `session/request_permission`. maju-acp's answering side selects the option
...
/// is applied.
///
/// - **v2** (what maju-agent negotiates with current maju-acp): tool context
...
/// method goose uses for custom usage and status events.  Used by maju-agent
/// to emit the `usage_update` payload so maju-acp's `UsageTracker` can treat
/// maju-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so maju-acp's
...
/// | `contextLimit` | `u64` | `0` — maju-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (maju-agent has no context limit tracking).
...
mod tests {
    ...
    /// v2 (what maju-agent negotiates with current maju-acp): tool context is
    ...
    /// `optionId == kind` so maju-acp's `kind`-based selector and this side's
    ...
    /// request maju-agent issued; it routes to the broker with its `result`.
    #[test]
    ...
    /// fails closed. maju-agent never leaves the waiter hanging on an error.
    #[test]
