...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "TMPDIR",
    "XDG_CONFIG_HOME",
    // Explicit Maju-owned OAuth root for named demo builds. The agent may spawn
    // auth-capable child tools after clearing its ambient environment.
    "MAJU_AGENT_CONFIG_DIR",
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // Maju identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). MAJU_PRIVATE_KEY
    // and MAJU_RELAY_URL are kept for the maju CLI. MAJU_AUTH_TAG is a
    ...
    // spawn maju-agent without declaring it.
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_maju_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"MAJU_AUTH_TAG"));
    }
