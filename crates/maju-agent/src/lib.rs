...
struct Session {
    ...
    /// `_goose/unstable/session/update` usage notification so maju-acp's
...
/// cache root. `None` preserves maju-agent's production cache location.
...
/// `maju-agent auth <provider>` — run the interactive auth flow for a
...
async fn auth_subcommand(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match provider {
        Some("databricks" | "databricks_v2" | "databricks-v2") => {
            ...
            eprintln!("Authenticated. Token cached under ~/.config/maju-agent/oauth/databricks/.");
            Ok(())
        }
        Some(other) => Err(format!("auth: unknown provider {other:?}").into()),
        None => Err("auth: provider required (try: maju-agent auth databricks)".into()),
    }
}
    ...
    wire::send(
        ...
        wire::ok(
            ...
            json!({
                ...
                "agentInfo": { "name": "maju-agent", "version": env!("CARGO_PKG_VERSION") },
            }),
        ),
    )
    .await;
...
async fn run_prompt(app: Arc<App>, id: Value, params: Value, wire_tx: WireSender) {
    ...
    // BEFORE sending the session/prompt response. maju-acp's UsageTracker
