    ...
    match openai_efforts_for_model(model) {
        ...
        None if effort == ThinkingEffort::Max => {
            tracing::warn!(
                requested = "max",
                resolved = "xhigh",
                "MAJU_AGENT_THINKING_EFFORT=max not confirmed for unknown OpenAI model; clamping to xhigh"
            );
            ThinkingEffort::XHigh
        }
        None => effort,
...
pub struct Config {
    ...
    /// Set by Maju's relay-mesh provider via
    /// `MAJU_AGENT_PREFER_MESH_FOR_AUTO=1`; other providers keep their
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let cfg = Config {
            ...
            openai_api,
            prefer_mesh_for_auto: parse_env("MAJU_AGENT_PREFER_MESH_FOR_AUTO", 0u8)? != 0,
            max_rounds: parse_env("MAJU_AGENT_MAX_ROUNDS", 0)?,
            max_output_tokens: parse_env("MAJU_AGENT_MAX_OUTPUT_TOKENS", 32_768)?,
...
mod tests {
    ...
    // the TS test in `majuAgentConfig.test.mjs`) and verifies that this Rust
    ...
    /// `getProviderEffortConfig` in `majuAgentConfig.ts`.
    ///
