...
const REPLY_GUARD_SERVER: &str = "maju-agent";
...
const REPLY_GUARD_NAG: &str = "You are about to end this turn without calling `maju messages send`. \
...
/// Whether `call` is a recognized attempt to publish a reply to Maju.
///
...
fn is_maju_reply_call(call: &ToolCall, mcp: &McpRegistry) -> bool {
    mcp.has(&call.name) && !mcp.is_hook(&call.name) && is_reply_shaped(&call.name, &call.arguments)
}

/// Whether a tool name and arguments have the shape of a Maju publish command.
///
/// Split from [`is_maju_reply_call`] only so the matcher is testable without a
...
/// (`echo "maju messages send"`) matches. Missing a real post is the expensive
...
impl RunCtx<'_> {
    ...
    pub async fn run(&mut self, prompt: Vec<ContentBlock>) -> Result<StopReason, AgentError> {
        ...
        *self.turn_total_state = TurnTotalState::Unseen;
        ...
        // session) so `MAJU_AGENT_MAX_HANDOFFS` bounds compactions per
        ...
        // successful publish. See `is_maju_reply_call`.
        ...
        let mut maju_reply_call_seen = false;
        ...
        loop {
                ...
                || response.output_tokens.is_some()
            {
                ...
                *self.turn_cache_write_tokens = self
                ...
                // the consumer: maju-acp's UsageTracker advances its committed
            ...
            if response.tool_calls.is_empty() {
                ...
                //   1. `!maju_reply_call_seen` — no publish attempt in any
            ...
            if !maju_reply_call_seen {
                maju_reply_call_seen = calls.iter().any(|c| is_maju_reply_call(c, self.mcp));
            }
            self.history.push(HistoryItem::Assistant {
                text: response.text,
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("maju_hook_{hook}_{server}_{ordinal}")
}
...
///
/// `maju_reply_call_seen` is the publish-aware gate (from
/// `is_maju_reply_call`), updated unconditionally regardless of
...
fn warn_if_silent_turn(
    maju_reply_call_seen: bool,
    text_is_empty: bool,
    output_tokens: Option<u64>,
    stop: ProviderStop,
) {
    if maju_reply_call_seen || !text_is_empty {
        return;
    }
    match output_tokens {
...
mod tests {
    ...
    fn reply_shape_matches_documented_send_forms() {
        for cmd in [
            "maju messages send --channel X --content Y",
            "maju --relay wss://r messages send --channel X --content Y",
            "/abs/path/maju messages send",
            "printf 'hi' | maju messages send --content -",
            "maju messages send-diff --diff -",
            "maju reactions add --event E --emoji +",
            // Assembled through another shell: rev 3's tokenizer missed this.
            r#"sh -c "maju messages send --channel X""#,
        ] {
            assert!(
    ...
    fn reply_shape_rejects_non_reply_commands() {
        for cmd in [
            "maju messages get --channel X",
            "maju channels list",
            "maju reactions remove --event E",
            "maju pr open --title T",
            "maju social publish --content hi",
            "maju notes set --name n",
    ...
    fn reply_shape_requires_the_qname_separator() {
        let args = json!({ "command": "maju messages send --channel X" });
        for name in [
            "dev__powershell",
            "dev__noshell",
            "shell",
        ...
        assert!(is_reply_shaped("maju-dev-mcp__shell", &args));
    }
    ...
    fn reply_shape_reads_only_the_command_field() {
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "description": "maju messages send --channel X" })
        ));
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "workdir": "maju messages send" })
        ));
    ...
    ///
    /// Identifies silent-turn WARNs by target (`maju_agent::agent`) + WARN
    ...
    fn count_silent_turn_warnings(f: impl FnOnce()) -> usize {
        ...
        impl<S: tracing::Subscriber> tracing_subscriber::Layer<S> for Capture {
            fn on_event(
                ...
                if event.metadata().target() != "maju_agent::agent" {
                    return;
                }
    ...
    /// token check, or hardcoding `maju_reply_call_seen = true` are all caught.
    #[test]
    ...
    /// A turn that published (maju_reply_call_seen = true) then ended with a
    /// short final completion must not trigger the WARN.  This is the normal
    /// publish-then-wrap pattern.  Deleting the `maju_reply_call_seen` gate
