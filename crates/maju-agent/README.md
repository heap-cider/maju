...
[ACP](https://agentclientprotocol.com) is the Agent Client Protocol — JSON-RPC 2.0 over stdio between a client (Zed, JetBrains, maju-acp, …) and an agent. [MCP](https://modelcontextprotocol.io) is how the agent talks to its tools.

`maju-agent` is the agent.
...
← {"jsonrpc":"2.0","id":1,"result":{
    ...
    "agentInfo":{"name":"maju-agent","version":"0.1.0"}
  }}
...
|---|---|---|
| `MAJU_AGENT_PROVIDER` | — | Required. `anthropic`, `openai`, `openrouter`, `databricks`, or `databricks_v2`. No implicit fallback — the agent errors at startup when this is unset. |
...
| `MAJU_AGENT_TOOL_TIMEOUT_SECS` | `1260` | Per-tool call timeout in seconds |
| `MAJU_AGENT_MAX_PARALLEL_TOOLS` | `8` | Max concurrent tool calls per turn (1 = sequential) |
| `MAJU_AGENT_MAX_SESSIONS` | unlimited | Max concurrent ACP sessions. Sessions are cheap; default has no cap. |
| `MAJU_AGENT_MAX_LINE_BYTES` | `4194304` | 4 MiB. Hard cap on inbound JSON-RPC frames. |
| `MAJU_AGENT_MAX_HISTORY_BYTES` | `1048576` | 1 MiB. Old turns are evicted past this. |
| `MAJU_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` | `51200` | 50 KiB. Per-result cap on tool-output text; oversize is middle-elided (head + tail kept) with an inline marker. Images are exempt. |
| `MAJU_AGENT_REQUIRE_REPLY` | `0` (`1` on mesh) | `1` enables the [reply guard](#reply-guard) — remind the model to publish when a turn is about to end with nothing posted to Maju. Desktop defaults it to `1` for Maju shared-compute agents. |


## Reply Guard

Off by default, except on Maju shared-compute (mesh) agents, where Maju Desktop
sets `MAJU_AGENT_REQUIRE_REPLY=1` automatically. With it enabled, a turn that is
about to end without any recognized attempt to post to Maju gets a reminder that
its assistant text is invisible to humans, and is rerolled.

This exists because a Maju agent's reasoning and tool output are not shown to
...
Setting `MAJU_AGENT_REQUIRE_REPLY=0` on the agent, persona, or global env opts a
...
- whose qualified name ends in `__shell` — i.e. the bare tool name is exactly
  `shell`, which is `maju-dev-mcp`'s shell tool and any other server's, and
...
posted. Text that merely quotes a send (`echo "maju messages send"`) matches, so
...
`MAJU_AGENT_STOP_MAX_REJECTIONS` — the outer cap on every end-turn objection.
...
`maju-agent` speaks a few HTTP dialects. Pick with `MAJU_AGENT_PROVIDER`.

| Provider | `MAJU_AGENT_PROVIDER` | Endpoint (auto) | Tested with |
|---|---|---|---|
...
If `MAJU_AGENT_PROVIDER=anthropic` is selected without `ANTHROPIC_API_KEY`, `MAJU_AGENT_PROVIDER=openai` is selected without `OPENAI_COMPAT_API_KEY`, or `MAJU_AGENT_PROVIDER=openrouter` is selected without `OPENROUTER_API_KEY`, the agent returns an error — there is no implicit fallback to another provider.
...
- Retryable statuses (429 and typed `provider_overloaded` 503) honor the documented `Retry-After` header (clamped to a small ceiling — see `RETRY_AFTER_CAP_SECS` in `llm.rs` — since the sleep happens outside `MAJU_AGENT_LLM_TIMEOUT_SECS`); 502 and untyped 503 retry with jittered backoff instead. `401` is treated as an expired/invalid key and refreshed once, while `402` (no credits) and `403` (guardrail/moderation/permission) fail immediately without retry.
...
| Frame size | `MAJU_AGENT_MAX_LINE_BYTES` (default 4 MiB). Oversize → connection killed. |
...
| Session isolation | Unlimited concurrent sessions by default (configurable via `MAJU_AGENT_MAX_SESSIONS`). One prompt per session at a time. Each session gets its own MCP servers. |
...
| Limit | Default | Where |
|---|---|---|
| Inbound JSON-RPC frame | 4 MiB | `MAJU_AGENT_MAX_LINE_BYTES` |
| Single prompt | 1 MiB | `MAX_PROMPT_BYTES` |
| History window | 1 MiB | `MAJU_AGENT_MAX_HISTORY_BYTES` |
...
| Tool result text | 50 KiB | `MAJU_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` |
...
| Loop rounds | 0 (unlimited) | `MAJU_AGENT_MAX_ROUNDS` |
| LLM read inactivity timeout | 240 s | `MAJU_AGENT_LLM_TIMEOUT_SECS` |
| Tool call timeout | 1260 s | `MAJU_AGENT_TOOL_TIMEOUT_SECS` |
