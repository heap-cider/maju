...
|---|---|---|
| `MAJU_AGENT_PROVIDER` | — | Required. `anthropic`, `openai`, `databricks`, or `databricks_v2`. No implicit fallback — the agent errors at startup when this is unset. |
...
If `MAJU_AGENT_PROVIDER=anthropic` is selected without `ANTHROPIC_API_KEY`, or `MAJU_AGENT_PROVIDER=openai` is selected without `OPENAI_COMPAT_API_KEY`, the agent returns an error — there is no implicit fallback to another provider.
