...
//! exercised manually via the `maju-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `maju-agent` binary with
...
use maju_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real maju-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    ...
    async fn spawn_provider_with_options(
        ...
        cmd.env("MAJU_AGENT_PROVIDER", provider)
            ...
            .env("MAJU_AGENT_MAX_ROUNDS", "2")
            .env("MAJU_AGENT_MAX_SESSIONS", max_sessions.to_string())
...
async fn model_discovery_surfaces_rejected_static_token_as_auth_failure() {
    ...
    use maju_agent::config::{Config, Provider};
    use maju_agent::discover_databricks_models;
