...
//! surface.
//!
//! These drive a real `maju-agent` subprocess against a fake MCP server and a
...
fn call_log_path(tag: &str) -> String {
    let p = std::env::temp_dir().join(format!(
        "maju_perm_calllog_{tag}_{}_{:x}.log",
        std::process::id(),
...
/// a Maju client always answers, but a non-Maju ACP client may violate the spec
...
async fn test_crossed_parallel_decisions_authorize_only_matching_call() {
    ...
    .await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("MAJU_AGENT_MAX_PARALLEL_TOOLS", "1")]).await;
    let sid = init(
        &mut h,
        1,
        "/tmp",
