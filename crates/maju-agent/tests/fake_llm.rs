//! Integration test: fake LLM HTTP server + maju-agent subprocess.
//!
...
async fn rejects_oversized_line() {
    ...
    cmd.env("MAJU_AGENT_PROVIDER", "openai")
        ...
        .env("MAJU_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are maju-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are maju-agent"),
...
/// Wait for the `activeRunId` advert maju-agent emits at prompt start and
...
/// completion_tokens). maju-agent maps these to `accumulatedInputTokens` /
...
/// maju-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// maju-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, maju-agent must
...
/// (so token counts are observed), maju-agent must still emit the usage
