...
async fn handshake(h: &mut Harness) -> String {
    ...
    assert_eq!(init["result"]["agentInfo"]["name"], "maju-agent");
    assert_eq!(
...
/// maju-agent must emit a `usage_update` notification with `accumulatedInputTokens`
...
/// When the Anthropic parser produces an input-sum overflow, maju-agent must
