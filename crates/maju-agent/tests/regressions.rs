...
impl Harness {
    async fn spawn_with_env(base_url: &str, extra: &[(&str, &str)]) -> Self {
        let bin = env!("CARGO_BIN_EXE_maju-agent");
        let mut cmd = tokio::process::Command::new(bin);
        cmd.env("MAJU_AGENT_PROVIDER", "openai")
            .env("OPENAI_COMPAT_API_KEY", "test")
            .env("OPENAI_COMPAT_MODEL", "fake-model")
            .env("OPENAI_COMPAT_BASE_URL", base_url)
            .env("MAJU_AGENT_LLM_TIMEOUT_SECS", "5")
            .env("MAJU_AGENT_TOOL_TIMEOUT_SECS", "5")
            .env("MAJU_AGENT_MAX_ROUNDS", "8")
            .env("MAJU_AGENT_MCP_INIT_TIMEOUT_SECS", "2");
        for (k, v) in extra {
            cmd.env(k, v);
        }
        cmd.stdin(Stdio::piped())
            ...
            .stderr(Stdio::piped())
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn maju-agent");
...
async fn history_budget_evicts_old_turns() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("MAJU_AGENT_MAX_HISTORY_BYTES", &BUDGET.to_string()),
            ("MAJU_AGENT_MAX_HANDOFFS", "0"), // exercise truncation, not handoff
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn hook_stop_blocks_premature_end() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("MAJU_AGENT_STOP_MAX_REJECTIONS", "10"),
        ],
    )
    .await;
...
async fn hook_stop_budget_exhausted() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("MAJU_AGENT_STOP_MAX_REJECTIONS", "1"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_stop_consecutive_end_turn_uses_rejection_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("MAJU_AGENT_STOP_MAX_REJECTIONS", "2"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_post_compact_injects_after_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("MAJU_AGENT_MAX_HISTORY_BYTES", &(1024 * 1024).to_string()),
            // Allow at least one handoff.
            ("MAJU_AGENT_MAX_HANDOFFS", "3"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn handoff_summary_prompt_includes_full_history_within_context_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("MAJU_AGENT_MAX_CONTEXT_TOKENS", "10000"),
            ("MAJU_AGENT_MAX_OUTPUT_TOKENS", "1000"),
            ("MAJU_AGENT_MAX_HANDOFFS", "3"),
            (
                "MAJU_AGENT_MAX_HISTORY_BYTES",
...
async fn token_usage_over_budget_triggers_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("MAJU_AGENT_MAX_CONTEXT_TOKENS", "1000"),
            ("MAJU_AGENT_MAX_OUTPUT_TOKENS", "100"),
            ("MAJU_AGENT_MAX_HANDOFFS", "3"),
...
/// `_Stop` hook that takes longer than `MAJU_AGENT_HOOK_TIMEOUT_MS`
...
async fn hook_stop_timeout_failopen() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("MAJU_AGENT_HOOK_TIMEOUT_MS", "200"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
/// sends `notifications/cancelled` to the MCP server. With maju-dev-mcp,
...
async fn cancel_kills_inflight_tool_via_mcp_notification() {
    // maju-dev-mcp is a separate crate; locate its binary relative to
    // the maju-agent test binary (they share the same target dir).
    let self_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_maju-agent"));
    let dev_mcp_bin = self_bin.parent().unwrap().join("maju-dev-mcp");
    ...
    if !dev_mcp_is_executable {
        eprintln!(
            "SKIP: maju-dev-mcp not built at {}; run `cargo build -p maju-dev-mcp` first",
            dev_mcp_bin.display()
        );
        return;
    }
...
/// any MCP server (not just maju-dev-mcp) when a session is cancelled
...
// ---------------------------------------------------------------------------
// Reply guard (`MAJU_AGENT_REQUIRE_REPLY`)
//
// The guard reminds the model to publish when a turn is about to end without
// any recognized attempt to post to Maju. It rides the existing `_Stop` gate
...
/// in-process guard (`server: "maju-agent"`) at the `_Stop` hook point — the
                ...
                .filter(|m| {
                    m["role"] == "tool"
                        && serde_json::from_str::<Value>(m["content"].as_str().unwrap_or(""))
                            .map(|p| p["hook"] == "_Stop" && p["server"] == "maju-agent")
                            .unwrap_or(false)
                })
                .count()
        })
...
fn openai_shell_send(id: &str) -> Value {
    openai_tool_call(
        id,
        "fake__shell",
        json!({ "command": "maju messages send --channel c --content hi" }),
    )
}
...
/// `MAJU_AGENT_REQUIRE_REPLY=0` is off too — the toggle is numeric, so a
/// literal `0` must not read as "set, therefore on".
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reply_guard_explicit_zero_is_off() {
    let llm = spawn_capturing_llm(vec![openai_text("done"), openai_text("unexpected")]).await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("MAJU_AGENT_REQUIRE_REPLY", "0")]).await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn reply_guard_nags_twice_then_lets_the_turn_end() {
    ...
    .await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("MAJU_AGENT_REQUIRE_REPLY", "1")]).await;
    let sid = init_session(&mut h, json!([])).await;
    ...
    let nag = msgs
        ...
        .find(|p| p["server"] == "maju-agent")
        .expect("reminder body");
    let text = nag["text"].as_str().unwrap_or("");
    assert!(
        text.contains("maju messages send"),
...
async fn reply_guard_ignores_calls_lost_to_the_turn_cap() {
    ...
    calls.push(json!({
        ...
        "function": {
            "name": "fake__shell",
            "arguments": json!({ "command": "maju messages send --channel c --content hi" })
                .to_string(),
        },
    }));
...
async fn reply_guard_off_when_stop_budget_is_zero() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MAJU_AGENT_REQUIRE_REPLY", "1"),
            ("MAJU_AGENT_STOP_MAX_REJECTIONS", "0"),
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
fn reply_guard_rejects_unparseable_toggle() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_maju-agent"))
        .env("MAJU_AGENT_PROVIDER", "openai")
        .env("OPENAI_COMPAT_API_KEY", "test")
        .env("OPENAI_COMPAT_MODEL", "fake-model")
        .env("MAJU_AGENT_REQUIRE_REPLY", "true")
        .stdin(Stdio::null())
        .output()
        .expect("run maju-agent");
    assert!(
        !out.status.success(),
    ...
    assert!(
        stderr.contains("MAJU_AGENT_REQUIRE_REPLY"),
...
fn max_token_recoveries_rejects_unparseable_value() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_maju-agent"))
        ...
        .env("OPENAI_COMPAT_API_KEY", "test")
        .env("OPENAI_COMPAT_MODEL", "fake-model")
        .env("MAJU_AGENT_MAX_TOKEN_RECOVERIES", "unbounded")
        .stdin(Stdio::null())
        .output()
        .expect("run maju-agent");
    assert!(
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("MAJU_AGENT_MAX_TOKEN_RECOVERIES"),
...
async fn context_window_400_recovers_instead_of_sticking() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("MAJU_AGENT_MAX_CONTEXT_TOKENS", "200000"),
            ("MAJU_AGENT_MAX_OUTPUT_TOKENS", "8192"),
            (
                "MAJU_AGENT_MAX_HISTORY_BYTES",
                &(16 * 1024 * 1024).to_string(),
            ),
...
async fn max_tokens_recovery_respects_finite_round_cap() {
    ...
    .await;
    let mut h = Harness::spawn_with_env(&llm.url, &[("MAJU_AGENT_MAX_ROUNDS", "1")]).await;
    let sid = init_session(&mut h, json!([])).await;
    let prompt_id = h
        .send(
            "session/prompt",
...
async fn repeated_max_tokens_is_bounded() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("MAJU_AGENT_MAX_ROUNDS", "0"),
            ("MAJU_AGENT_MAX_TOKEN_RECOVERIES", "2"),
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn zero_max_token_recoveries_disables_retry() {
    ...
    .await;
    let mut h =
        Harness::spawn_with_env(&llm.url, &[("MAJU_AGENT_MAX_TOKEN_RECOVERIES", "0")]).await;
    let sid = init_session(&mut h, json!([])).await;
    let prompt_id = h
        .send(
            "session/prompt",
...
/// `MAJU_AGENT_MAX_ROUNDS=1` the turn would return `max_turn_requests` after
