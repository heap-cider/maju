//! End-to-end integration tests for the Maju relay.
//!
...
use base64::Engine;
use maju_test_client::{MajuTestClient, RelayMessage, TestClientError};
...
fn test_owner_keys() -> Keys {
    std::env::var("MAJU_TEST_OWNER_PRIVATE_KEY")
        .ok()
...
async fn e2e_db_pool() -> sqlx::Pool<sqlx::Postgres> {
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgres://maju:maju_dev@localhost:5432/maju".to_string() // sadscan:disable np.postgres.1
    });
...
async fn test_connect_and_authenticate() {
    ...
    let client = MajuTestClient::connect(&url, &keys)
        .await
...
async fn test_client_submitted_nip43_membership_snapshots_are_rejected() {
    ...
    let mut ws = MajuTestClient::connect(&url, &keys).await.expect("connect");
    let ok = ws
...
async fn test_send_event_and_receive_via_subscription() {
    ...
    let mut client_a = MajuTestClient::connect(&url, &keys_a)
        .await
    ...
    let mut client_b = MajuTestClient::connect(&url, &keys_b)
        .await
...
async fn test_large_event_frame_below_configured_limit_is_accepted() {
    ...
    let mut client = MajuTestClient::connect(&url, &keys).await.expect("connect");
...
async fn test_unauthenticated_rejected() {
    ...
    let mut client = MajuTestClient::connect_unauthenticated(&url)
        .await
...
async fn test_multiple_concurrent_clients() {
    ...
    let mut clients: Vec<MajuTestClient> =
        futures_util::future::try_join_all(keys.iter().map(|k| MajuTestClient::connect(&url, k)))
            .await
...
async fn subscribe_until_eose(client: &mut MajuTestClient, sid: &str, filter: Filter) {
    loop {
        client
            .subscribe(sid, vec![filter.clone()])
            .await
...
async fn test_pubkey_mismatch_rejected() {
    ...
    let mut client = MajuTestClient::connect(&url, &keys_a)
        .await
...
async fn test_nip29_put_user_default_policy_allows() {
    ...
    let mut ws = MajuTestClient::connect(&url, &channel_owner_keys)
        .await
...
async fn test_unarchive_emits_member_added_notification() {
    ...
    let mut ws = MajuTestClient::connect(&url, &owner_keys)
        .await
...
async fn test_nip29_put_user_self_add_bypasses_policy() {
    ...
    // Connect as agent.
    let mut ws = MajuTestClient::connect(&url, &agent_keys)
        .await
...
async fn test_nip29_standard_client_flow() {
    ...
    let mut client = MajuTestClient::connect(&url, &keys)
        .await
...
async fn test_membership_notification_emitted_on_add() {
    ...
    let mut agent_client = MajuTestClient::connect(&url, &agent_keys)
        .await
        .expect("connect as agent");
...
async fn create_private_channel_ws(client: &mut MajuTestClient, keys: &Keys) -> String {
...
async fn self_departure_ws(url: &str, channel_id: &str, actor: &Keys, kind: u16) -> (bool, String) {
    ...
    let mut client = MajuTestClient::connect(url, actor)
        .await
...
async fn promote_co_owner(url: &str, channel_id: &str, owner: &Keys, co_owner: &Keys) {
    let mut client = MajuTestClient::connect(url, owner)
        .await
...
async fn test_private_channel_any_member_can_invite() {
    ...
    let mut owner_client = MajuTestClient::connect(&url, &owner_keys)
        .await
        .expect("connect as owner");
    ...
    for (actor_role, actor_keys) in &actors {
        let mut actor_client = MajuTestClient::connect(&url, actor_keys)
            .await
...
async fn test_private_channel_admin_can_invite() {
    ...
    let mut admin_client = MajuTestClient::connect(&url, &admin_keys)
        .await
...
async fn test_private_channel_non_member_cannot_invite() {
    ...
    let mut outsider_client = MajuTestClient::connect(&url, &outsider_keys)
        .await
...
async fn test_private_channel_member_cannot_grant_admin() {
    ...
    let mut member_client = MajuTestClient::connect(&url, &member_keys)
        .await
...
async fn test_reply_ingest_pushes_live_thread_summary() {
    ...
    async fn recv_summary(client: &mut MajuTestClient) -> nostr::Event {
        loop {
            match client
                .recv_event(Duration::from_secs(5))
                .await
...
async fn test_nip29_put_user_cannot_demote_owner() {
    ...
    let mut ws = MajuTestClient::connect(&url, &attacker_keys)
        .await
...
async fn test_nip29_owner_demotion_recovery_paths() {
    ...
    let put_user = |signer: Keys, target_hex: String, role: &'static str| {
        ...
        let url = url.clone();
        async move {
            let mut ws = MajuTestClient::connect(&url, &signer)
                .await
                .expect("connect");
    ...
    let mut ws = MajuTestClient::connect(&url, &victim_keys)
        .await
        .expect("connect");
...
async fn test_nip29_put_user_without_role_tag_preserves_role() {
    ...
    let mut ws = MajuTestClient::connect(&url, &owner_a)
        .await
    ...
    let mut ws = MajuTestClient::connect(&url, &owner_b)
        .await
...
async fn test_nip29_relay_rejects_role_change_by_unprivileged_actor() {
    ...
    let mut ws = MajuTestClient::connect(&url, &attacker)
        .await
        .expect("connect as attacker");
...
async fn test_nip29_relay_rejects_last_owner_self_demotion() {
    ...
    let mut ws = MajuTestClient::connect(&url, &owner)
        .await
        .expect("connect as owner");
