//! End-to-end acceptance tests for Maju shared compute.
//!
//! These tests require a membership-gated maju-relay and a mesh-enabled desktop
...
//! | 5 | dropped member → typed auth failure reaches lastError | runbook (desktop harness) | maju-agent `-32001` unit |
...
const KIND_MAJU_MESH_MEMBER_STATUS: u16 = 30003;
const MESH_STATUS_D_TAG_PREFIX: &str = "maju-mesh-member-status:";
const MESH_STATUS_TYPE: &str = "maju-mesh-status";
...
fn mesh_status_filter() -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_MAJU_MESH_MEMBER_STATUS))
...
async fn trust_member_reads_mesh_status() {
    ...
    let mut client = MajuTestClient::connect(&url, &member)
        .await
    ...
    let status = events
        .iter()
        .find(|e| {
            e.kind == Kind::Custom(KIND_MAJU_MESH_MEMBER_STATUS) && e.pubkey == member.public_key()
        })
...
///
/// Requires a relay with `MAJU_REQUIRE_RELAY_MEMBERSHIP=true` and a published
...
async fn trust_nonmember_read_denied() {
    ...
    let mut client = match MajuTestClient::connect(&url, &stranger).await {
        Ok(c) => c,
    ...
    let leaked = events
        .iter()
        .any(|e| e.kind == Kind::Custom(KIND_MAJU_MESH_MEMBER_STATUS));
    assert!(
        !leaked,
...
/// B running a mesh client + a launched maju-agent pointed at B's local
...
/// split route. Auto-split is mesh runtime behavior (no Maju code); this row
