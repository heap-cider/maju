...
//! cargo test -p maju-test-client --test conformance_multitenant -- --ignored
//! ```
//!
...
mod row_zero_host_binding {
    ...
    /// properties of it (see channel: `maju-relay-rewrite`, 2026-06-27):
    ///
    ///   * [`super::channels_membership::same_channel_uuid_in_two_communities_is_isolated`]
    ///     (Mari, `maju-db`) asserts **coexistence**: a channel UUID that exists
...
mod nip11_relay_info {
    ...
    /// (`crates/maju-relay/src/nip11.rs::_RELAY_INFO_BUILD_STATIC_INPUT_FENCE`):
...
// ---------------------------------------------------------------------------
// API tokens and NIP-98 replay (Sami — maju-auth)
// ---------------------------------------------------------------------------
mod api_tokens_nip98_replay {
    ...
    /// in `maju-relay`** — there is no route by which a client can bind a token
    /// to a community over HTTP, so the "mint in A, present to B" precondition
    /// has no entry point.
    ///
    /// Verified on PR head:
    ///   * `crates/maju-relay/src/router.rs:52-79` — full route list is `/`,
    ...
    ///   * `crates/maju-relay/src/api/` directory is `{bridge, events, media,
    ...
    ///     `crates/maju-relay/src/api/media.rs:638`, which extracts the
    ///     `X-Auth-Token: maju_*` header and looks up
    ...
    /// contract (its deps are `maju-ws-client`/`reqwest`/`tokio-tungstenite`/
    /// `s3` — no `sqlx`, no `maju-db`).
    ///
    ...
    ///
    ///   1. `crates/maju-db/src/api_token.rs:425
    ...
    ///   2. `crates/maju-db/src/api_token.rs:488
    ...
    fn token_minted_in_a_does_not_authorize_in_b() {
        ...
        // If anyone adds a `/tokens` route to `maju-relay`, this row's shape
    ...
    /// in `crates/maju-relay/src/api/bridge.rs:79` (return `Ok(())` before
    ...
    ///
    /// The replay key shape is `maju:{community}:nip98:{event_id_hex}` (see
    /// `crates/maju-auth/src/nip98_replay.rs:103 nip98_replay_key`). The
    ...
    /// (`crates/maju-auth/src/nip98_replay.rs:163
    ...
    /// "simplifying" the key to just `maju:nip98:{event_id}`, or
    ...
    /// (relay-membership is open under `MAJU_REQUIRE_AUTH_TOKEN=false`).
    #[tokio::test]
    #[ignore]
    ...
    /// the pattern in `crates/maju-auth/src/nip98.rs`; kept local to this
...
// ---------------------------------------------------------------------------
// Membership / allowlist / archived identities (Sami — maju-auth)
// ---------------------------------------------------------------------------
...
mod users_profiles_nip05 {
    ...
    use maju_test_client::MajuTestClient;
    ...
    /// relay's ingest side-effect at `crates/maju-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// also syncs the parsed fields into `users` via
    /// `update_user_profile(tenant.community(), pubkey, ...)`.
    async fn publish_kind0(client: &mut MajuTestClient, keys: &Keys, content_json: &str) -> String {
    ...
    /// bridge endpoint; with `MAJU_REQUIRE_AUTH_TOKEN=false` the dev-mode
    ...
    /// `crates/maju-relay/src/api/nip05.rs::extract_domain` — strips the
    ...
    /// `crates/maju-db/src/user.rs:140` (`UPDATE users SET ... WHERE
    /// community_id = $N AND pubkey = $M`) AND the event-storage fence at
    /// `crates/maju-db/src/event.rs::get_events_by_ids` (or the equivalent
    ...
    async fn same_pubkey_distinct_profiles_in_two_communities() {
        ...
        let mut client_a = MajuTestClient::connect(&ws_a, &keys)
            .await
            .expect("connect A");
        let _id_a = publish_kind0(&mut client_a, &keys, &content_a).await;

        let mut client_b = MajuTestClient::connect(&ws_b, &keys)
            .await
            .expect("connect B");
    ...
    /// `crates/maju-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// validates the handle via `crates/maju-relay/src/api/nip05.rs::canonicalize_nip05`
    ...
    /// from `crates/maju-db/src/user.rs::get_user_by_nip05` (line 185 form
    ...
    async fn same_nip05_local_part_on_two_hosts_is_independent() {
        ...
        let mut client_a = MajuTestClient::connect(&ws_a, &keys_a)
            .await
            .expect("connect A");
        let _ = publish_kind0(&mut client_a, &keys_a, &content_a).await;

        let mut client_b = MajuTestClient::connect(&ws_b, &keys_b)
            .await
            .expect("connect B");
...
// ---------------------------------------------------------------------------
// Feed read-side isolation (Perci — maju-db/relay bridge)
// ---------------------------------------------------------------------------
mod feed_read_side_isolation {
    ...
    /// `crates/maju-db/src/feed.rs`; this row is the wire-level conformance
...
// ---------------------------------------------------------------------------
// Channels and channel membership (Mari — maju-db)
// ---------------------------------------------------------------------------
mod channels_membership {
    ...
    /// `crates/maju-relay/src/handlers/side_effects.rs::handle_create_group`.
    ...
    /// `MAJU_REQUIRE_AUTH_TOKEN=false` recipe). Returns the events as their
    ...
    /// `crates/maju-db/src/event.rs::query_events` (the non-p-tag branch,
...
mod workflows {
    ...
    /// the REST bridge (`POST /events`). In dev mode (`MAJU_REQUIRE_AUTH_TOKEN
    ...
    /// `crates/maju-relay/src/handlers/command_executor.rs:703`
    ...
    /// (`crates/maju-workflow/src/lib.rs` — "approval gates not yet implemented,
    ...
    /// minting), **not** maju-db — the scoping fence it will exercise is
    /// already landed.
    #[tokio::test]
    #[ignore]
...
// ---------------------------------------------------------------------------
// Search / FTS (Quinn — maju-search)
// ---------------------------------------------------------------------------
mod search_fts {
    use super::*;

    use maju_test_client::{MajuTestClient, RelayMessage};
    ...
    ///   - `crates/maju-search/src/query.rs::search` lines 160-161 (the FTS
    ///     `WHERE community_id = $ctx` predicate); and
    ///   - `crates/maju-db/src/event.rs::get_events_by_ids` lines 870-872
...
// ---------------------------------------------------------------------------
// Redis pub/sub, presence, typing, cache invalidation (Max — maju-pubsub)
// ---------------------------------------------------------------------------
mod pubsub_presence_typing {
    ...
    async fn publish_presence(client: &mut MajuTestClient, keys: &Keys, status: &str) {
    ...
    async fn subscribe_typing(client: &mut MajuTestClient, sub_id: &str, channel_id: &str) {
        let filter = Filter::new()
    ...
    /// Obligation: keys are `maju:{community}:…`; cross-node fan-out never
    ...
    ///    → `maju_pubsub::get_presence_bulk`). A's query must return only A's
    ///    status and B's only B's. This bites the Redis key format in
    ///    `crates/maju-pubsub/src/presence.rs::presence_key`, which must include
    ///    `ctx.community()` (`maju:{community}:presence:{pubkey}`). Same pubkey
    ...
    ///     `maju-pubsub/src/presence.rs::presence_key` (shared Redis key). The B
...
// ---------------------------------------------------------------------------
// Media / Blossom / S3 (Perci — maju-media)
// ---------------------------------------------------------------------------
mod media_blossom {
    use super::*;
...
// ---------------------------------------------------------------------------
// Git hosting / NIP-34 / object storage (Perci — maju-media/git)
// ---------------------------------------------------------------------------
mod git_hosting {
    use super::*;
...
// ---------------------------------------------------------------------------
// Audit log and observability (Dawn — maju-audit)
// ---------------------------------------------------------------------------
mod audit_log {
    ...
    //! `crates/maju-relay/src/router.rs` (the route list is `/`, `/info`,
    ...
    //! `maju_audit::AuditService::{verify_chain, get_entries}`, which are
    //! operator-internal (consumed by `maju-admin`). `crates/maju-audit/src/
    ...
    //! (its deps are `maju-ws-client`/`reqwest`/`tokio-tungstenite`/`s3` — no
    //! `sqlx`, no `maju-audit`), and a DB-direct read can never catch a
    ...
    //!      `maju_audit::service::tests::chains_are_independent_per_community`
    ...
    //!      `maju_relay::handlers::event::tests::
    ...
    //!   2. **Errors don't leak** —
    //!      `maju_audit::error::tests::audit_error_text_carries_no_community_id_or_constraint`
    //!      asserts no `AuditError` variant's rendered text embeds a
    //!      `community_id`, constraint name, or cross-community object id.
    //!
    //! Substrate on PR head: `crates/maju-audit/src/entry.rs` keys `AuditEntry`
