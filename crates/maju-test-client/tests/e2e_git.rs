...
//!   cargo test -p maju-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
use maju_media::S3AddressingStyle;
...
/// named by the announcement's `maju-channel` tag, so every repo these tests
...
/// Run `git` with the Maju credential helper and isolated config.
...
/// Run `git` with the Maju credential helper and isolated config. Asserts the
...
impl GitS3Probe {
    ...
    fn from_env() -> Self {
        ...
        // relay. Unit/live MinIO probes in maju-relay keep explicit local
        // fixtures and do not need provider overrides.
        let endpoint = std::env::var("MAJU_S3_ENDPOINT")
            .unwrap_or_else(|_| "http://localhost:9000".to_string());
        let access_key =
            std::env::var("MAJU_S3_ACCESS_KEY").unwrap_or_else(|_| "maju_dev".to_string());
        let secret_key =
            std::env::var("MAJU_S3_SECRET_KEY").unwrap_or_else(|_| "maju_dev_secret".to_string());
        let bucket_name =
            std::env::var("MAJU_S3_BUCKET").unwrap_or_else(|_| "maju-media".to_string());
        let region_name =
            std::env::var("MAJU_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string());
    ...
    fn pointer_key(owner: &str, repo: &str) -> String {
        ...
        if let Ok(community) = std::env::var("MAJU_E2E_GIT_COMMUNITY_ID") {
...
fn git_s3_probe_builds_both_addressing_styles() {
    ...
    assert_eq!(path.url(), "https://storage.example/maju-media");
    ...
    assert_eq!(virtual_hosted.url(), "https://maju-media.storage.example");
}
...
async fn git_clone_push_fetch_force_roundtrip() {
    ...
    // The `maju-channel` binding is the repo's ACL: without it the read gate
    ...
    let announce = EventBuilder::new(Kind::from(30617), "")
        .tags(vec![
            ...
            Tag::parse(["maju-channel", &channel]).unwrap(),
        ])
...
async fn git_concurrent_push_one_wins_and_repo_recovers() {
    ...
    let tmp = tempdir_named("maju-e2e-git-concurrent");
    let url = format!("{}/git/{}/{}", relay_http_url(), owner_hex, repo);
