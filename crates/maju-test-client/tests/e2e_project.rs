...
//!
//! The ingest unit tests in `maju-relay` pin the envelope contract in isolation.
...
//!
//! ```text
//! RELAY_URL=ws://localhost:3000 cargo test -p maju-test-client --test e2e_project -- --ignored
//! ```
...
async fn connect_agent_with_owner(agent: &Keys, owner: &Keys) -> MajuTestClient {
    ...
    let mut client = MajuTestClient::connect_unauthenticated(&relay_url())
        .await
...
async fn query(client: &mut MajuTestClient, name: &str, filter: Filter) -> Vec<nostr::Event> {
    let sid = sub_id(name);
    client
...
async fn test_project_publish_and_query_returns_cross_owner_members() {
    ...
    let members = vec![
        member_coord(&owner, "maju"),
        member_coord(&other, "maju-infra"),
    ];
...
async fn test_project_replacement_keeps_only_newest_for_same_author_and_d() {
    ...
    let members = vec![member_coord(&owner, "maju")];
...
async fn test_project_same_d_under_two_authors_are_independent() {
    ...
    let mut alice_client = MajuTestClient::connect(&url, &alice)
        .await
        .expect("connect");
    let ok = alice_client
    ...
    let mut bob_client = MajuTestClient::connect(&url, &bob).await.expect("connect");
    let ok = bob_client
...
async fn test_agent_owner_can_delete_agent_project_but_third_party_cannot() {
    ...
    let mut third_party_client = MajuTestClient::connect(&relay_url(), &third_party)
        .await
    ...
    let mut owner_client = MajuTestClient::connect(&relay_url(), &owner)
        .await
...
async fn test_project_malformed_envelope_rejected_by_relay() {
    ...
    let duplicate = member_coord(&owner, "maju");
    ...
    let cases: Vec<(&str, nostr::Event, &str)> = vec![
        ...
        (
            ...
            project_event(
                ...
                "Bad kind",
                &[format!("30618:{}:maju", owner.public_key().to_hex())],
                None,
            ),
        ...
        (
            ...
            project_event(
                ...
                "Uppercase",
                &[format!("{REPO_ANNOUNCEMENT_KIND}:{}:maju", "A".repeat(64))],
                None,
            ),
            "member `a` tag must be",
        ),
