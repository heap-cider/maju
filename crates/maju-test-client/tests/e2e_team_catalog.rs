...
async fn test_team_catalog_foreign_sees_only_shared() {
    ...
    let mut author = MajuTestClient::connect(&url, &author_keys)
        .await
    ...
    let mut foreign = MajuTestClient::connect(&url, &foreign_keys)
        .await
...
async fn test_team_catalog_live_fanout_and_unshare_retracts() {
    ...
    let delivered = loop {
        match foreign.recv_event(Duration::from_secs(5)).await {
            ...
            Ok(_) => continue,
            Err(maju_test_client::TestClientError::Timeout) => break false,
