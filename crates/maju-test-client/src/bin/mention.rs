//! Send an @mention event to a Maju channel targeting a specific pubkey.
...
async fn main() -> anyhow::Result<()> {
    ...
    let url = std::env::var("MAJU_RELAY_URL").unwrap_or_else(|_| "ws://localhost:3000".into());
    let keys = Keys::generate();
    println!("Sender pubkey: {}", keys.public_key().to_hex());

    let mut client = MajuTestClient::connect(&url, &keys).await?;
