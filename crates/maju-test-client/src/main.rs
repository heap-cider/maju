//! `maju-test-cli` — Manual testing CLI for the Maju relay.
//!
//! # Usage
//!
//! ```text
//! maju-test-cli [OPTIONS]
//!
//! Options:
...
//! Send a message:
//! ```text
//! maju-test-cli --channel my-channel --send "Hello, Maju!"
//! ```
//!
//! Subscribe and watch events:
//! ```text
//! maju-test-cli --channel my-channel --subscribe
//! ```
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "maju_test_client=debug".to_string())
                .as_str(),
        )
        .init();
    ...
    let keys = match std::env::var("MAJU_PRIVATE_KEY") {
        Ok(sk) => Keys::parse(&sk).expect("invalid MAJU_PRIVATE_KEY"),
        Err(_) => Keys::generate(),
    };
...
async fn run_send(url: &str, keys: &Keys, channel: &str, message: &str, kind: u16) {
    ...
    let mut client = match MajuTestClient::connect(url, keys).await {
        Ok(c) => c,
        Err(e) => {
...
async fn run_subscribe(url: &str, keys: &Keys, channel: &str, kind: u16) {
    ...
    loop {
        match client.recv_event(Duration::from_secs(30)).await {
            ...
            Ok(_) => {}
            Err(maju_test_client::TestClientError::Timeout) => {
                // Keep waiting.
            }
            Err(e) => {
                eprintln!("Error: {e}");
...
fn print_help() {
    println!(
        r#"maju-test-cli — Manual testing CLI for the Maju relay
...
EXAMPLES:
    ...
    maju-test-cli --url ws://relay.example.com --channel test --subscribe
"#
    );
}
