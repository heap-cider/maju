...
Welcome, and thank you for your interest in contributing! Maju is an
open-source project and we're glad you're here. This guide will help you
get from zero to a merged pull request.

If you have questions that aren't answered here, [open an issue](https://github.com/heap-cider/maju/issues/new).
...
unacceptable behavior to **conduct@maju-relay.org**.

---

## Before You Open a PR

Before starting, search [open PRs](https://github.com/heap-cider/maju/pulls) and [open issues](https://github.com/heap-cider/maju/issues) for duplicates — someone may already be working on the same thing. When you open your PR, link the closest existing one in the description (or say "none found").
...
Maju is an agent platform, so AI-assisted PRs are welcome. No need to disclose the tools you used, but you own and must have reviewed the final code. Submissions that are clearly unreviewed may be closed with a pointer here.
...
just reset   # Wipe all dev state and recreate it; installed Maju is preserved
```

Development desktop state uses separate bundle identifiers
(`xyz.block.maju.app.dev` and per-worktree variants), a separate keyring service
(`maju-desktop-dev`), and `~/.maju-dev`. `just reset` removes those dev-only
locations and the local Docker volumes. It does not touch the installed app's
`xyz.block.maju.app` data, `maju-desktop` keyring service, or `~/.maju` nest.
...
See the [maju-db testing guide](crates/maju-db/TESTING.md) for the crate-level
checklist.
...
`MAJU_TEST_DATABASE_URL`, `TEST_DATABASE_URL`, or `DATABASE_URL`; do not
...
with `MAJU_TEST_SCHEMA_MODE=migration`. Test helpers that normally call the
migrator honor `MAJU_TEST_SCHEMA_MODE=desired` so the desired-state contract is
not re-migrated.
...
Set `MAJU_POSTGRES_ADMIN_URL` to that role's maintenance database, and set
...
End-to-end tests live in `crates/maju-test-client/tests/`:
...
of `maju-core` types, which are shared everywhere).
...
Maju is developed across multiple repositories. This repo (`heap-cider/maju`)
...
**External contributors:** Fork `heap-cider/maju`, open a PR, and CI runs
...
1. **Define the kind constant** in `maju-core/src/kind.rs`:
...
2. **Define the payload type** in the appropriate module in `maju-core/src/`
...
3. **Register the kind's required scope** in
   `crates/maju-relay/src/handlers/ingest.rs` inside
...
4. **Handle post-storage side effects** by adding a match arm in
   `crates/maju-relay/src/handlers/side_effects.rs` inside
   `handle_side_effects()`:
   ...
   `crates/maju-relay/src/api/` and register it in
   `crates/maju-relay/src/router.rs`.

5. **Persist to the database** — if the event needs to be queryable, add a
   handler in `maju-db/src/` (e.g., `maju-db/src/my_feature.rs`) with
...
8. **Write tests** — add a unit test for payload serialization in
   `maju-core` and an integration test in `maju-test-client` that sends
...
1. **Define the handler** in the appropriate module under
   `crates/maju-relay/src/api/`. Resolve the request tenant before any auth or
   data lookup, use NIP-98 when the endpoint accepts user credentials, and keep
   community scoping explicit.

2. **Register the route** in `crates/maju-relay/src/router.rs` using the
   narrowest path possible. Do not add new `/api/*` compatibility routes unless
   the product decision explicitly calls for one.

3. **Add database queries** in `maju-db/src/` only when the endpoint cannot be
   expressed through the existing event query paths.

4. **Handle errors** using the `api_error()`, `internal_error()`, and
   `not_found()` helpers in `maju-relay/src/api/mod.rs`. Return
   `(StatusCode, Json<Value>)` tuples.

5. **Write tests** with the `maju-test-client` harness in
   `crates/maju-test-client/tests/`, covering auth, community scoping, and the
...
Maju is licensed under the **Apache License, Version 2.0**. See
...
*Thank you for contributing to Maju. Every bug report, documentation fix,
