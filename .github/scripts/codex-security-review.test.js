...
function pullRequest({
  ...
  return {
    ...
    base: {
      ref: "main",
      sha: baseSha,
      repo: { full_name: "block/maju" },
    },
    head: {
      sha: headSha,
      repo: { full_name: "outside/maju" },
    },
    changed_files: 1,
    labels,
  };
...
function harness({
  ...
  const context = {
    ...
    payload: {
      comment: { body: `@maju-security-review ${HEAD_SHA}` },
      issue: { number: 6816 },
    },
    repo: { owner: "block", repo: "maju" },
  };
  return {
    context,
    core,
...
async function postReview(state, review = NO_FINDINGS_REVIEW) {
  const environment = {
    ...
    GITHUB_REPOSITORY: "block/maju",
    GITHUB_RUN_ID: "1234",
    GITHUB_SERVER_URL: "https://github.com",
    REVIEW_BASE_SHA: BASE_SHA,
    REVIEW_COMMIT_RANGE: `${BASE_SHA}...${HEAD_SHA}`,
    REVIEW_HEAD_REPO: "outside/maju",
...
test("prepare binds a member command to the named head SHA", async () => {
  ...
  assert.match(
    moved.failures[0],
    new RegExp(`@maju-security-review ${OTHER_HEAD_SHA}`),
  );
});
...
test("prepare rejects review commands without a full exact SHA", async () => {
  const state = harness();
  state.context.payload.comment.body = "@maju-security-review";
...
test("invalidation compares the complete base and head range", async () => {
  ...
  assert.match(
    ...
    new RegExp(`@maju-security-review ${HEAD_SHA}`),
  );
