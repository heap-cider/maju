...
test("report rows render the relay response contract", async ({ page }) => {
  ...
  await page.goto("/reports");
  await expect(page.getByText("design.maju.xyz")).toBeVisible();
...
test("feedback can be searched and filtered by community and time", async ({
    ...
    route.fulfill({
      ...
      body: JSON.stringify([
        ...
        },
        {
          id: "old",
          communityId: "two",
          communityHost: "engineering.maju.xyz",
  ...
  await page.getByLabel("Community").selectOption("design.maju.xyz");
      ...
      body: JSON.stringify([
        {
          ...
          communityHost: `${"long-community-name.".repeat(4)}maju.example.com`,
          submitterPubkey: "21".repeat(32),
          category: "bug",
...
test("feedback attachments render from imeta without raw markdown", async ({
  ...
  const imageUrl = `https://design.maju.xyz/media/${"a".repeat(64)}.png`;
  const fileUrl = `https://design.maju.xyz/media/${"b".repeat(64)}.txt`;
  await page.route(`**/api/admin/v1/feedback/${id}`, (route) =>
    route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({
