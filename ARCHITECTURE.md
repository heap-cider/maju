...
A Maju **community** is the tenant-visible workspace selected by the request host.
...
Maju is a Rust monorepo, licensed Apache 2.0 under Block, Inc.
...
│                           CLIENTS                                    │
│                                                                      │
│  Human (Nostr app, web, mobile)    Agent (CLI tools via maju-cli)    │
│           │                                    │                     │
│           └──────────── WebSocket ─────────────┘                    │
└─────────────────────────────────────────────────────────────────────┘
                               │
...
└──────────┬──────────────┬──────────────────────────────────────────┘
     ...
     ┌──────────────┐
     │  Postgres    │  ← maju-search (FTS over the search_tsv
...
```
maju-core    (zero I/O — types, verification, filter matching, kind registry)
    │
    ├── maju-db          (Postgres: events, channels, tokens, workflows, audit)
    ├── maju-auth        (NIP-42, NIP-98, API tokens, scopes, rate limiting)
    ├── maju-pubsub      (Redis pub/sub, presence, typing indicators)
    ├── maju-search      (Postgres FTS: query, delete)
    ├── maju-audit       (hash-chain tamper-evident log)
    └── maju-workflow    (YAML-as-code automation engine)
         │
         └── maju-relay       (ties everything together — the server)

maju-acp            (agent harness — bridges relay @mentions → AI agents via ACP/JSON-RPC)
maju-sdk            (typed Nostr event builders — used by maju-acp and maju-cli)
maju-media          (Blossom/S3 media storage)
maju-cli            (agent-first CLI)
maju-admin          (operator CLI: relay membership + key generation)
maju-test-client    (integration test harness + manual CLI)
```

**Key architectural principle:** The relay is the single source of truth. `maju-relay` orchestrates all subsystems by calling them directly — it imports `maju-db`, `maju-auth`, `maju-pubsub`, `maju-search`, `maju-audit`, and `maju-workflow`. However, those subsystems are isolated from each other: `maju-workflow` never calls `maju-pubsub`, `maju-search` never calls `maju-db`, etc. Cross-subsystem coordination happens only through the relay. In multi-community mode, the relay also owns propagation of `TenantContext`; service crates should receive community-scoped inputs rather than independently deriving tenancy from client-controlled event tags.

---

## 2. The Protocol

Maju uses Nostr NIP-01 on the wire. Every action is a JSON event with six fields:
...
| 40000–49999 | Maju custom kinds |

### Maju Custom Kinds (selected)
...
Workflow loop prevention: workflow execution kinds (46001–46012), relay-signed messages with `maju:workflow` tag, and `KIND_GIFT_WRAP` are excluded from triggering workflows. All other stored events (including kind 9 stream messages) trigger workflow evaluation.
...
### maju-core — Shared Types and Verification
...
- Dev-only key derivation: `SHA-256("maju-test-key:{username}")` — gated behind `#[cfg(any(test, feature = "dev"))]`. The `dev` feature must not be enabled in production relay deployments.
...
### maju-pubsub — Redis Pub/Sub, Presence, Typing

Manages Redis pub/sub fan-out, presence tracking, and typing indicators. In multi-community mode all tenant-visible keys are prefixed or otherwise partitioned by community (`maju:{community}:...`) so channel fan-out, presence, typing, and cache invalidation cannot cross hosts.

**Architecture:**

```
Publisher  → pool connection   → PUBLISH maju:channel:{uuid}
Subscriber → dedicated PubSub  → PSUBSCRIBE maju:channel:*
...
**Presence:** `SET maju:presence:{pubkey_hex} {status} EX 180` — 180-second TTL (3× the 60-second heartbeat interval). Single missed heartbeat does not cause presence flap.

**Typing indicators:**
```
ZADD maju:typing:{channel_id} {now_unix} {pubkey_hex}
ZREMRANGEBYSCORE maju:typing:{channel_id} -inf {now - 5.0}
EXPIRE maju:typing:{channel_id} 60
```
...
### maju-search — Postgres FTS Integration
...
- Permission filtering is **caller's responsibility** — `maju-search` returns
...
Real-time voice lives inside `maju-relay` (`src/audio/`), not a separate crate. A WebSocket endpoint (`wss://.../huddle/{channel_id}/audio`) authenticates each participant with a NIP-42 challenge, checks channel membership, admits them to an in-memory room, and forwards opaque Opus frames between peers. No external SFU.
...
### maju-acp — Agent Communication Protocol Harness

Standalone binary that bridges Maju relay events to AI agents via the [Agent Communication Protocol](https://agentclientprotocol.com/) (ACP).

**Architecture:**

```
Maju Relay ──WS──→ maju-acp ──stdio (ACP/JSON-RPC)──→ Agent (goose/codex/claude)
```

`maju-acp` spawns AI agent subprocesses (1–32, default 1), connects to the relay via WebSocket with NIP-42 auth, discovers channels via REST API, and queues `@mention` events per channel. At most one prompt is in-flight per channel. Queued events are batched into a single prompt sent via `session/prompt` over ACP.
...
- Depends on `maju-core` (kind constants) and `maju-sdk` (relay/REST utilities).

**Does NOT:** persist state.

---

### maju-admin — Operator CLI
...
The `maju-admin` binary is shipped in the relay Docker image (`/usr/local/bin/maju-admin`) and is the recommended way to manage relay membership in production. Use `./run.sh add-member`, `./run.sh remove-member`, and `./run.sh list-members` in Docker Compose deployments.

---

### maju-test-client — Integration Test Harness

**`MajuTestClient`** wraps a WebSocket connection with a `VecDeque<RelayMessage>` buffer for message interleaving. Methods: `connect`, `connect_unauthenticated`, `authenticate`, `send_event`, `send_text_message`, `subscribe`, `close_subscription`, `recv_event`, `collect_until_eose`, `disconnect`.
...
`src/main.rs` is a manual testing CLI (`maju-test-cli`) with `--send`, `--subscribe`, `--channel`, `--url`, `--kind` flags.
...
| Concern | Mechanism |
|---------|-----------|
| Schnorr signatures | `verify_event()` in `maju-core` — every event verified before storage |
...
|---------|------|-----|---------|
| `maju:channel:{uuid}` | Pub/Sub channel | — | Event fan-out (single-community form; shared multi-community Redis must use `maju:{community}:channel:{uuid}` or equivalent) |
| `maju:presence:{pubkey_hex}` | String | 180s | Online/away status (single-community form; shared multi-community Redis must scope by community) |
| `maju:typing:{channel_uuid}` | Sorted Set | 60s | Active typers (5s window; shared multi-community Redis must scope by community) |
...
| 2 | **No rate limiting implementation** | `RateLimiter` trait exists in `maju-auth`. Only implementation is `AlwaysAllowRateLimiter` (test stub, gated behind `#[cfg(any(test, feature = "test-utils"))]`). `RateLimitConfig` defines 4 tiers (human, agent-standard, agent-elevated, agent-platform) but none are enforced. |
