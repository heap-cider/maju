...
Maju is pre-1.0. We do not maintain long-term support branches at this stage.
...
All events are written to a tamper-evident audit log (`maju-audit`). Each
...
The Maju desktop app stores nsec private keys in the operating system keyring
...
example), keys fall back to a `0o600` owner-only file. The `MAJU_PRIVATE_KEY`
