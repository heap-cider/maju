#!/usr/bin/env bash
# Local docker-desktop k8s testbed for the Maju relay mesh.
#
...
NS="${NS:-maju-mesh}"
RELEASE="${RELEASE:-maju}"
IMAGE_REPO="${IMAGE_REPO:-maju-relay}"
IMAGE_TAG="${IMAGE_TAG:-mesh-local}"
CHART="${REPO_ROOT}/deploy/charts/maju"
...
if [ "$helm_rc" != 0 ]; then
  ...
  kubectl -n "$NS" describe pods -l app.kubernetes.io/name=maju | tee "$EVID/describe-onfail.txt"
  kubectl -n "$NS" logs -l app.kubernetes.io/name=maju --tail=100 --all-containers | tee "$EVID/logs-onfail.txt"
  die "helm install returned rc=$helm_rc"
fi

# ── 4. verify 3/3 Ready ───────────────────────────────────────────────────────
# Find the relay Deployment: everything under this release named "maju" except
...
# chart name, so the name isn't always "<release>-maju".)
DEPLOY=""
...
# The bundled MinIO + init pods share app.kubernetes.io/name=maju, so select by
...
RELAY_PODS=$(kubectl -n "$NS" get pods \
  -l "app.kubernetes.io/name=maju,app.kubernetes.io/instance=$RELEASE" \
