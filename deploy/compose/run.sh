...
if [[ "${MAJU_COMPOSE_TLS:-false}" == "true" ]]; then
  COMPOSE_FILES+=(-f compose.caddy.yml)
fi
if [[ "${MAJU_COMPOSE_DEV:-false}" == "true" ]]; then
...
- deploy/compose/.env, especially MAJU_RELAY_PRIVATE_KEY, DB/Redis/S3 secrets, and MAJU_GIT_HOOK_HMAC_SECRET
...
- maju-git-data volume (MAJU_GIT_REPO_PATH=/data/git)
...
case "${1:-help}" in
  ...
  backup-hint)
    ...
    ;;
  add-member)
    docker compose exec relay /usr/local/bin/maju-admin add-member --pubkey "${2:?Usage: ./run.sh add-member <npub-or-hex> [--role member|admin]}" "${@:3}"
    ;;
  remove-member)
    docker compose exec relay /usr/local/bin/maju-admin remove-member --pubkey "${2:?Usage: ./run.sh remove-member <npub-or-hex> [--role member|admin]}" "${@:3}"
    ;;
  list-members)
    docker compose exec relay /usr/local/bin/maju-admin list-members
    ;;
  help|-h|--help)
    cat <<'MSG'
...
Commands:
  start         Start Maju with docker compose up -d --wait
...
Environment switches:
  MAJU_COMPOSE_TLS=true   Include compose.caddy.yml for automatic HTTPS
  MAJU_COMPOSE_DEV=true   Include compose.dev.yml for local admin ports/tools
MSG
    ;;
  *)
