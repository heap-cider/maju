...
# Production values must attach push.maju.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/maju-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.maju'
...
runtime={'app.kubernetes.io/name':'maju-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['MAJU_PUSH_RUNTIME_DATABASE_ROLE']['value']=='maju_push_gateway_runtime'
...
required={'DATABASE_URL','MAJU_PUSH_APNS_KEY_ID','MAJU_PUSH_APNS_TEAM_ID','MAJU_PUSH_APNS_TOPIC','MAJU_PUSH_GRANT_KEYS','MAJU_PUSH_TOKEN_KEYS','MAJU_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-maju-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-maju-push-gateway-migration')
...
assert 'push.maju.xyz' in route['spec']['hostnames']
PY
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-maju-push-gateway')
