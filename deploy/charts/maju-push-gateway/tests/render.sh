...
helm lint deploy/charts/maju-push-gateway >/dev/null
helm template push deploy/charts/maju-push-gateway >"$out"
...
production_args=(
  -f deploy/charts/maju-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'profiles.dogfood.appAttestAppId=REALTEAM.xyz.block.maju.dogfood.mobile'
  --set 'networkPolicy.postgresEgressCidrs[0]=10.42.0.0/16'
)
helm lint deploy/charts/maju-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/maju-push-gateway "${production_args[@]}" >"$production_out"

# Gateway API remains an explicit supported ingress mode when an operator opts
# in and supplies the environment-owned parent.
helm template push deploy/charts/maju-push-gateway \
...
assert!(jenv.dig("MAJU_PUSH_RUNTIME_DATABASE_ROLE", "value") == "maju_push_gateway_runtime")
...
required = Set.new(%w[
  DATABASE_URL MAJU_PUSH_DOGFOOD_APNS_CERT_PATH
  MAJU_PUSH_DOGFOOD_APNS_TOPIC MAJU_PUSH_DOGFOOD_APP_ATTEST_APP_ID
  MAJU_PUSH_GRANT_KEYS MAJU_PUSH_TOKEN_KEYS MAJU_PUSH_MAX_GRANT_LIFETIME_SECONDS
])
...
np = nps.find { |x| x.dig("metadata", "name") == "push-maju-push-gateway" }
migration_np = nps.find { |x| x.dig("metadata", "name") == "push-maju-push-gateway-migration" }
...
assert!(production_image == "ghcr.io/heap-cider/maju-push-gateway@sha256:#{"a" * 64}", production_image.inspect)
...
# certificate Secret.
if helm template push deploy/charts/maju-push-gateway \
...
if helm template push deploy/charts/maju-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/maju-push-gateway -f deploy/charts/maju-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
np = xs.find do |x|
  x["kind"] == "NetworkPolicy" && x.dig("metadata", "name") == "push-maju-push-gateway"
end
...
helm lint deploy/charts/maju-push-gateway \
  -f deploy/charts/maju-push-gateway/tests/datadog-values.yaml >/dev/null
helm template push deploy/charts/maju-push-gateway \
  -f deploy/charts/maju-push-gateway/tests/datadog-values.yaml \
  >"$datadog_out"
