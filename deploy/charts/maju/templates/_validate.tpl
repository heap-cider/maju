...
{{- $minimumReplicas := include "maju.minimumReplicas" . | int -}}
{{- if gt $minimumReplicas 1 -}}
  {{- if and (not .Values.redis.enabled) (not .Values.externalRedis.url) (not .Values.secrets.existingSecret) -}}
    {{- fail (printf "minimum replica count %d requires Redis for maju-pubsub. Enable redis.enabled=true, set externalRedis.url, or provide secrets.existingSecret with key REDIS_URL." $minimumReplicas) -}}
  {{- end -}}
{{- end -}}
...
{{/* Multiple replicas do NOT require ReadWriteMany git storage.
     ...
     multi-pod requirement for maju-pubsub. */}}
...
{{/* S3 / object-storage source must exist somewhere. With the default
     MAJU_GIT_CONFORMANCE_PROBE behavior, an unreachable bucket is detected
...
{{- if not (or .Values.minio.enabled .Values.s3.endpoint .Values.secrets.existingSecret) -}}
  {{- fail "S3/object-storage source missing: enable minio.enabled=true (quickstart in-cluster), set s3.endpoint + s3.bucket + credentials, or provide secrets.existingSecret with keys MAJU_S3_ACCESS_KEY + MAJU_S3_SECRET_KEY. By default the relay runs a startup S3 conformance probe and exits if storage is unreachable; disabling MAJU_GIT_CONFORMANCE_PROBE also removes that startup storage check." -}}
{{- end -}}
